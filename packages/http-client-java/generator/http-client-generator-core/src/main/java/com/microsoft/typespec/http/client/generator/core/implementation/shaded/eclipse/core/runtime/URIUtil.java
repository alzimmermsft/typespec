 ...
 * undesirable behavior of the {@link URI} class, and provides
...
public final class URIUtil {
     ...
     * {@link URI#resolve(String)} is in the treatment of the final
    ...
    public static URI append(URI base, String extension) {
        try {
            ...
            if (isFileURI(base)
                && path != null
                && path.startsWith(UNC_PREFIX)
                && (resultPath == null || !resultPath.startsWith(UNC_PREFIX))) {
                result
                    = new URI(result.getScheme(), ensureUNCPath(result.getSchemeSpecificPart()), result.getFragment());
            }
            return result;
     ...
     * <p>
     * This method is equivalent to {@link URI#relativize}, except for its
