...
abstract class DOMMember extends DOMNode implements IDOMMember {
     ...
     * contains two -1's.
     */
    DOMMember(char[] document, int[] sourceRange, String name, int[] nameRange, int[] commentRange, int flags,
        int[] modifierRange) {
    ...
    public String getComment() {
        ...
        if (hasComment()) {
            if (this.fComment != null) {
                ...
                return new String(this.fDocument, this.fCommentRange[0],
                    this.fCommentRange[1] + 1 - this.fCommentRange[0]);
            }
        } else {
            return null;
        }
