...
public class ProcessorInfo {
     ...
     * initialized (that is,
     * {@link Processor#init(com.microsoft.typespec.http.client.generator.core.implementation.shaded.javax.annotation.processing.ProcessingEnvironment)}
     * must already have
     ...
     * @param result an empty modifiable set, which upon return will contain a subset of <code>annotations</code>, which
    ...
    public boolean computeSupportedAnnotations(Set<TypeElement> annotations, Set<TypeElement> result) {
        if (null != annotations && !annotations.isEmpty() && null != this._supportedAnnotationTypesPattern) {
            for (TypeElement annotation : annotations) {
                Matcher matcher
                    = this._supportedAnnotationTypesPattern.matcher(annotation.getQualifiedName().toString());
                if (matcher.matches()) {
