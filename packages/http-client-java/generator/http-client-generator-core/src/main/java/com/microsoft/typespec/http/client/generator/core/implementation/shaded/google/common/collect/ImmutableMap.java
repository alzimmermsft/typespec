...
public abstract class ImmutableMap<K, V> implements Map<K, V>, Serializable {
     ...
     *         = new ImmutableMap.Builder<String, Integer>().put("one", 1).put("two", 2).put("three", 3).buildOrThrow();
     * }
     * </pre>
     *
    ...
    public static class Builder<K, V> {
        ...
        private void ensureCapacity(int minCapacity) {
            if (minCapacity > entries.length) {
                entries
                    = Arrays.copyOf(entries, ImmutableCollection.Builder.expandedCapacity(entries.length, minCapacity));
                entriesUsed = false;
            }
        }
        ...
        private ImmutableMap<K, V> build(boolean throwIfDuplicateKeys) {
            ...
            if (valueComparator == null) {
                ...
                Arrays.sort(nonNullEntries, 0, localSize,
        ...
        ImmutableMap<K, V> buildJdkBacked() {
            checkState(valueComparator == null, "buildJdkBacked is only for testing; can't use valueComparator");
            switch (size) {
                case 0:
                    return of();
    ...
    public static <K, V> ImmutableMap<K, V> copyOf(Map<? extends K, ? extends V> map) {
        if ((map instanceof ImmutableMap) && !(map instanceof SortedMap)) {
            ...
            @SuppressWarnings("unchecked") // safe since map is not writable
            ImmutableMap<K, V> kvMap = (ImmutableMap<K, V>) copyOfEnumMap((EnumMap<?, ? extends V>) map); // hide K
                                                                                                          // (violates
                                                                                                          // bounds)
                                                                                                          // from J2KT,
                                                                                                          // preserve V.
