...
public class ParameterizedQualifiedTypeReference extends ArrayQualifiedTypeReference {
    ...
    public ParameterizedQualifiedTypeReference(char[][] tokens, TypeReference[][] typeArguments, int dim,
        Annotation[][] annotationsOnDimensions, long[] positions) {
    ...
    public TypeReference augmentTypeWithAdditionalDimensions(int additionalDimensions,
        ...
        ParameterizedQualifiedTypeReference pqtr = new ParameterizedQualifiedTypeReference(this.tokens,
            this.typeArguments, totalDimensions, allAnnotations, this.sourcePositions);
    ...
    private TypeBinding internalResolveType(Scope scope, boolean checkBounds, int location) {
        ...
        if (this.dimensions > 0) {
            this.resolvedType = ArrayTypeReference.maybeMarkArrayContentsNonNull(scope, this.resolvedType,
                this.sourceStart, this.dimensions, null);
        }
    ...
    private TypeBinding internalResolveLeafType(Scope scope, boolean checkBounds) {
        ...
        for (int i = packageBinding == null ? 0 : packageBinding.compoundName.length; i < max; i++) {
            ...
            if (qualifyingType == null) {
                ...
                if (enclosingType != null && TypeBinding.notEquals(enclosingType.erasure(), qualifyingType.erasure())) { // qualifier
                                                                                                                         // !=
                                                                                                                         // declaring/enclosing
                    qualifyingType = enclosingType; // inherited member type, leave it associated with its enclosing
                                                    // rather than subtype
                }
            }
            ...
            if (args != null) {
                ...
                if (typeIsConsistent) {
                    if (!currentType.hasEnclosingInstanceContext()) {
                        if (qualifyingType != null && qualifyingType.isRawType())
                            this.typesPerToken[i - 1] = qualifyingType = qualifyingType.actualType(); // revert
                                                                                                      // rawification of
                                                                                                      // enclosing,
                                                                                                      // since its
                                                                                                      // generics are
                ...
                // check argument type compatibility for non <> cases - <> case needs no bounds check, we will scream
                // foul if needed during inference.
                if (!isDiamond) {
                ...
                if (currentOriginal.isGenericType()) {
                    if (typeIsConsistent
                        && qualifyingType != null
                        && qualifyingType.isParameterizedType()
                        && currentOriginal.hasEnclosingInstanceContext()) {
                        scope.problemReporter()
                            .parameterizedMemberTypeMissingArguments(this,
        ...
        if (visitor.visit(this, scope)) {
            ...
            if (annotationsOnDimensions != null) {
                for (Annotation[] annotationsOnDimension : annotationsOnDimensions) {
                    for (int j = 0, max2 = annotationsOnDimension == null ? 0 : annotationsOnDimension.length; j < max2;
                        j++) {
        ...
        if (this.resolvedType != null) {
            ...
            if (this.typesPerToken != null && this.typesPerToken[lastToken] != null) {
                for (int i = 0; i <= lastToken; i++) {
                    this.typesPerToken[i] = (ReferenceBinding) updateParameterizedTypeWithAnnotations(scope,
                        this.typesPerToken[i], this.typeArguments[i]);
                }
                updatedLeaf = this.typesPerToken[lastToken];
            } else {
                updatedLeaf
                    = updateParameterizedTypeWithAnnotations(scope, this.resolvedType, this.typeArguments[lastToken]);
            }
