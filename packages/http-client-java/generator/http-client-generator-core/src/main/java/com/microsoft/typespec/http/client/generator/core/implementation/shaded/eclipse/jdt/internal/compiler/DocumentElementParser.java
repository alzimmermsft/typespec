...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.RecoveredType;
...
public class DocumentElementParser extends Parser {
    ...
    public void checkComment() {
        ...
        nextComment: for (lastCommentIndex = this.scanner.commentPtr; lastCommentIndex >= 0; lastCommentIndex--) {
            ...
            if (commentSourceStart < 0 || // line comment
                ...
                (this.modifiersSourceStart != -1 && this.modifiersSourceStart < commentSourceStart)) // the comment is
                                                                                                     // after the
                                                                                                     // modifier
            {
    ...
    protected void consumeCatchFormalParameter() {
        ...
        this.intPtr -= 3;
        Argument arg = new Argument(parameterName, namePositions, type, this.intStack[this.intPtr + 1]);// modifiers
    ...
    protected void consumeClassBodyDeclaration() {
        ...
        this.requestor.acceptInitializer(initializer.declarationSourceStart, initializer.declarationSourceEnd,
            this.intArrayStack[this.intArrayPtr--], 0, this.modifiersSourceStart, initializer.block.sourceStart,
    ...
    protected void consumeClassHeader() {
        ...
        if (superclass == null) {
            this.requestor.enterClass(typeDecl.declarationSourceStart, this.intArrayStack[this.intArrayPtr--],
                typeDecl.modifiers, typeDecl.modifiersSourceStart, this.typeStartPosition, typeDecl.name,
                typeDecl.sourceStart, typeDecl.sourceEnd, null, -1, -1, interfaceNames, interfaceNameStarts,
                interfaceNameEnds, this.scanner.currentPosition - 1);
        } else {
            this.requestor.enterClass(typeDecl.declarationSourceStart, this.intArrayStack[this.intArrayPtr--],
                typeDecl.modifiers, typeDecl.modifiersSourceStart, this.typeStartPosition, typeDecl.name,
                typeDecl.sourceStart, typeDecl.sourceEnd, CharOperation.concatWith(superclass.getTypeName(), '.'),
                superclass.sourceStart, superclass.sourceEnd, interfaceNames, interfaceNameStarts, interfaceNameEnds,
    ...
    protected void consumeConstructorHeader() {
        ...
        this.requestor.enterConstructor(cd.declarationSourceStart, this.intArrayStack[this.intArrayPtr--], cd.modifiers,
            cd.modifiersSourceStart, cd.selector, cd.sourceStart, (int) (this.selectorSourcePositions & 0xFFFFFFFFL),
            // retrieve the source end of the name
            argumentTypes, argumentTypeStarts, argumentTypeEnds, argumentNames, argumentNameStarts, argumentNameEnds,
            this.rParenPos,
            // right parenthesis
            exceptionTypes, exceptionTypeStarts, exceptionTypeEnds, this.scanner.currentPosition - 1);
    }
    ...
    protected void consumeEnterVariable() {
        ...
        Annotation[][] annotationsOnExtendedDimensions
            = extendedTypeDimension == 0 ? null : getAnnotationsOnDimensions(extendedTypeDimension);
        ...
        declaration.type = extendedTypeDimension != 0
            ? augmentTypeWithAdditionalDimensions(type, extendedTypeDimension, annotationsOnExtendedDimensions, false)
            : type;
        ...
        if (!isLocalDeclaration) {
            this.requestor.enterField(declaration.declarationSourceStart, javadocPositions, declaration.modifiers,
                declaration.modifiersSourceStart, returnTypeName(declaration.type), type.sourceStart, type.sourceEnd,
                this.typeDims, varName, (int) (namePosition >>> 32), (int) namePosition, extendedTypeDimension,
    ...
    protected void consumeExitVariableWithInitialization() {
        ...
        this.lastFieldBodyEndPosition
            = ((AbstractVariableDeclaration) this.astStack[this.astPtr]).initialization.sourceEnd;
    }
    ...
    protected void consumeSingleVariableDeclarator(boolean isVarArgs) {
        ...
        Argument arg;
        if (isReceiver) {
            arg = new Receiver(parameterName, namePositions, type, qualifyingNameReference,
                this.intStack[this.intPtr + 1] & ~ClassFileConstants.AccDeprecated);
        } else {
            arg = new Argument(parameterName, namePositions, type, this.intStack[this.intPtr + 1]);// modifiers
        }
        // consume annotations
        if ((length = this.expressionLengthStack[this.expressionLengthPtr--]) != 0) {
            System.arraycopy(this.expressionStack, (this.expressionPtr -= length) + 1,
    ...
    protected void consumeInterfaceHeader() {
        ...
        this.scanner.commentPtr = -1;
        this.requestor.enterInterface(typeDecl.declarationSourceStart, this.intArrayStack[this.intArrayPtr--],
            typeDecl.modifiers, typeDecl.modifiersSourceStart, this.typeStartPosition, typeDecl.name,
            typeDecl.sourceStart, typeDecl.sourceEnd, interfaceNames, interfaceNameStarts, interfacenameEnds,
            this.scanner.currentPosition - 1);
    }
    ...
    protected void consumeMethodHeader() {
        ...
        this.requestor.enterMethod(md.declarationSourceStart, this.intArrayStack[this.intArrayPtr--], md.modifiers,
            md.modifiersSourceStart, returnTypeName, returnType.sourceStart, returnType.sourceEnd, this.typeDims,
            md.selector, md.sourceStart, (int) (this.selectorSourcePositions & 0xFFFFFFFFL), argumentTypes,
            argumentTypeStarts, argumentTypeEnds, argumentNames, argumentNameStarts, argumentNameEnds, this.rParenPos,
            this.extendsDim, this.extendsDim == 0 ? -1 : this.endPosition, exceptionTypes, exceptionTypeStarts,
            exceptionTypeEnds, this.scanner.currentPosition - 1);
    }
    ...
    protected void consumeMethodHeaderExtendedDims() {
        ...
        if (extendedDims != 0) {
            ...
            md.returnType = augmentTypeWithAdditionalDimensions(md.returnType, extendedDims,
                getAnnotationsOnDimensions(extendedDims), false);
            md.bits |= (md.returnType.bits & ASTNode.HasTypeAnnotations);
    ...
    protected void consumePackageDeclarationName() {
        ...
        this.requestor.acceptPackage(importReference.declarationSourceStart, importReference.declarationSourceEnd,
            this.intArrayStack[this.intArrayPtr--], CharOperation.concatWith(importReference.getImportName(), '.'),
    ...
    protected void consumeSingleStaticImportDeclarationName() {
        ...
        this.requestor.acceptImport(importReference.declarationSourceStart, importReference.declarationSourceEnd,
            this.intArrayStack[this.intArrayPtr--], CharOperation.concatWith(importReference.getImportName(), '.'),
            importReference.sourceStart, false, ClassFileConstants.AccStatic);
    }
    ...
    protected void consumeSingleTypeImportDeclarationName() {
        ...
        this.requestor.acceptImport(importReference.declarationSourceStart, importReference.declarationSourceEnd,
            this.intArrayStack[this.intArrayPtr--], CharOperation.concatWith(importReference.getImportName(), '.'),
            importReference.sourceStart, false, ClassFileConstants.AccDefault);
    }
    ...
    protected void consumeStaticImportOnDemandDeclarationName() {
        ...
        this.requestor.acceptImport(importReference.declarationSourceStart, importReference.declarationSourceEnd,
            this.intArrayStack[this.intArrayPtr--], CharOperation.concatWith(importReference.getImportName(), '.'),
            importReference.sourceStart, true, ClassFileConstants.AccStatic);
    }
    ...
    protected void consumeStaticInitializer() {
        ...
        Initializer initializer = (Initializer) this.astStack[this.astPtr];
        this.requestor.acceptInitializer(initializer.declarationSourceStart, initializer.declarationSourceEnd,
            this.intArrayStack[this.intArrayPtr--], ClassFileConstants.AccStatic, this.intStack[this.intPtr--],
            initializer.block.sourceStart, initializer.declarationSourceEnd);
    }
    ...
    protected void consumeTypeImportOnDemandDeclarationName() {
        ...
        this.requestor.acceptImport(importReference.declarationSourceStart, importReference.declarationSourceEnd,
            this.intArrayStack[this.intArrayPtr--], CharOperation.concatWith(importReference.getImportName(), '.'),
            importReference.sourceStart, true, ClassFileConstants.AccDefault);
    }
    /*
    ...
    public void parseCompilationUnit(ICompilationUnit unit) {
        ...
        try {
            ...
            this.referenceContext = this.compilationUnit = new CompilationUnitDeclaration(problemReporter(),
                new CompilationResult(unit, 0, 0, this.options.maxProblemsPerUnit), regionSource.length);
    ...
    public void parseConstructor(char[] regionSource) {
        try {
            ...
            this.referenceContext = this.compilationUnit = new CompilationUnitDeclaration(problemReporter(),
                new CompilationResult(regionSource, 0, 0, this.options.maxProblemsPerUnit), regionSource.length);
            this.scanner.resetTo(0, regionSource.length);
            this.scanner.setSource(regionSource);
            parse();
        } catch (AbortCompilation ex) {
    ...
    protected void pushOnIntArrayStack(int[] positions) {
        ...
        if (++this.intArrayPtr >= stackLength) {
            System.arraycopy(this.intArrayStack, 0, this.intArrayStack = new int[stackLength + StackIncrement][], 0,
                stackLength);
        }
