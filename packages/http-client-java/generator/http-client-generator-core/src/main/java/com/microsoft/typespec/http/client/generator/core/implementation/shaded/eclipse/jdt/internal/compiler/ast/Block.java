    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if ((this.bits & ASTNode.BlockShouldEndDead) != 0) { // switch rule blocks don't fall through
            if (flowInfo != FlowInfo.DEAD_END) {
                ...
                } else { // ... enforce that for statement switch, by having the code generator inject an automagic
                         // break ...
                    flowContext.recordBreakFrom(flowInfo);
                    return FlowInfo.DEAD_END;
                }
            } else {
                this.bits &= ~ASTNode.BlockShouldEndDead; // Already dead-ends; nothing special needed from code
                                                          // generator
            }
        }
        return flowInfo;
