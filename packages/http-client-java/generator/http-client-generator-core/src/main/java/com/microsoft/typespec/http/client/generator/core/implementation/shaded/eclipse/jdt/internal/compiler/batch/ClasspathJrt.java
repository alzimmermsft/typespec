...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.util.JrtFileSystem;
import java.io.File;
...
public class ClasspathJrt extends ClasspathLocation implements IMultiModuleEntry {
    ...
    public NameEnvironmentAnswer findClass(char[] typeName, String qualifiedPackageName, String moduleName,
        ...
        try {
            IBinaryType reader = JRTUtil.getClassfile(this.jrtFileSystem, qualifiedBinaryFileName, moduleName,
                this.moduleNamesCache::contains);
    ...
    protected IBinaryType maybeDecorateForExternalAnnotations(String qualifiedBinaryFileName, IBinaryType reader) {
        searchPaths: if (this.annotationPaths != null) {
            ...
            for (String annotationPath : this.annotationPaths) {
                try {
                    ...
                    reader = ExternalAnnotationDecorator.create(reader, annotationPath, qualifiedClassName,
                        this.annotationZipFile);
            ...
            // location is configured for external annotations, but no .eea found, decorate in order to answer
            // NO_EEA_FILE:
    ...
    public char[][][] findTypeNames(final String qualifiedPackageName, final String moduleName) {
        ...
        try {
            JRTUtil.walkModuleImage(this.jrtFileSystem, new JRTUtil.JrtFileVisitor<Path>() {

                @Override
                public FileVisitResult visitPackage(Path dir, Path modPath, BasicFileAttributes attrs)
                    throws IOException {
    ...
    public void loadModules() {
        Map<String, IModule> cache = ModulesCache.computeIfAbsent(this.file.getPath(), key -> {
            ...
            try {
                JRTUtil.walkModuleImage(this.jrtFileSystem,
                    new org.eclipse.jdt.internal.compiler.util.JRTUtil.JrtFileVisitor<Path>() {
                        ...
                        public FileVisitResult visitModule(Path p, String name) throws IOException {
                            try {
                                ClasspathJrt.this.acceptModule(
                                    ClasspathJrt.this.jrtFileSystem.getClassfile(IModule.MODULE_INFO_CLASS, name),
                                    newCache);
    ...
    @Override
    protected <T> List<String> allModules(Iterable<T> allSystemModules, Function<T, String> getModuleName,
        Function<T, IModule> getModule) {
