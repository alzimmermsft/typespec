...
public class TryStatement extends Statement {
    ...
    public static final ChildListPropertyDescriptor RESOURCES_PROPERTY = new ChildListPropertyDescriptor(
        TryStatement.class, "resources", VariableDeclarationExpression.class, CYCLE_RISK); //$NON-NLS-1$
     ...
     * @since 3.7
     */
    private NodeList resources = null;
     ...
     */
    private final NodeList catchClauses = new NodeList(CATCH_CLAUSES_PROPERTY);
    ...
    TryStatement(AST ast) {
        ...
        if (ast.apiLevel >= AST.JLS9_INTERNAL) {
            this.resources = new NodeList(RESOURCES2_PROPERTY);
        } else if (ast.apiLevel >= AST.JLS4_INTERNAL) {
            this.resources = new NodeList(RESOURCES_PROPERTY);
        }
    }
    ...
    ASTNode clone0(AST target) {
        ...
        if (this.ast.apiLevel >= AST.JLS4_INTERNAL) {
            result.resources().addAll(ASTNode.copySubtrees(target, resources()));
        }
     ...
     *
     * <p>A resource is either a {@link VariableDeclarationExpression} or (since JLS9) a {@link Name} or a
     * {@link FieldAccess} or a {@link SuperFieldAccess}.</p>
     *
    ...
    int treeSize() {
        return memSize() + (this.resources == null ? 0 : this.resources.listSize())
            + (this.body == null ? 0 : getBody().treeSize()) + this.catchClauses.listSize()
