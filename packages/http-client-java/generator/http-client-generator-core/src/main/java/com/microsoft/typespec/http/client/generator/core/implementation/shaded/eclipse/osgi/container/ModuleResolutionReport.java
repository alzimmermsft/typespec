...
class ModuleResolutionReport implements ResolutionReport {
    ...
    ModuleResolutionReport(Map<Resource, List<Wire>> resolutionResult, Map<Resource, List<Entry>> entries,
        ...
        this.resolutionResult
            = resolutionResult == null ? Collections.emptyMap() : Collections.unmodifiableMap(resolutionResult);
    ...
    private static String getResolutionReport0(String prepend, ModuleRevision revision,
        Map<Resource, List<Entry>> reportEntries, Set<BundleRevision> visited, boolean printOptional) {
        if (prepend == null) {
            prepend = ""; //$NON-NLS-1$
        }
        if (visited == null) {
        ...
        switch (entry.getType()) {
            case MISSING_CAPABILITY: {
                ...
                result.append(prepend)
                    .append(Msg.ModuleResolutionReport_UnresolvedReq)
                    .append(ModuleContainer.toString(requirement))
                    .append('\n');
            }
                break;
            ...
            case SINGLETON_SELECTION:
                result.append(prepend)
                    .append(Msg.ModuleResolutionReport_AnotherSingleton)
            ...
            case UNRESOLVED_PROVIDER: {
                ...
                Map<Requirement, Set<Capability>> unresolvedProviders
                    = (Map<Requirement, Set<Capability>>) entry.getData();
                for (Map.Entry<Requirement, Set<Capability>> unresolvedRequirement : unresolvedProviders.entrySet()) {
                    ...
                    if (!unresolvedCapabilities.isEmpty()) {
                        ...
                        if (!unresolvedRequirement.getKey().getResource().equals(unresolvedCapability.getResource())) {
                            ...
                            result.append(prepend)
                                .append("  -> ") //$NON-NLS-1$
                                .append(ModuleContainer.toString(unresolvedCapability))
                                .append('\n');
            ...
            default:
                result.append(Msg.ModuleResolutionReport_Unknown)
