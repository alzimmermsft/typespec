...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.NullAnnotationMatching.CheckMode;
...
public class ImplicitNullAnnotationVerifier {
    public static void ensureNullnessIsKnown(MethodBinding methodBinding, Scope scope) {
        if ((methodBinding.tagBits & TagBits.IsNullnessKnown) == 0) {
            ...
            // ensure nullness of methodBinding is known (but we are not interested in reporting problems against
            // methodBinding)
    ...
    ImplicitNullAnnotationVerifier(LookupEnvironment environment) {
        ...
        this.buddyImplicitNullAnnotationsVerifier
            = new ImplicitNullAnnotationVerifier(environment, options.inheritNullAnnotations);
     ...
     */
    public void checkImplicitNullAnnotations(MethodBinding currentMethod, AbstractMethodDeclaration srcMethod,
        boolean complain, Scope scope) {
        ...
        try {
            ...
            boolean needToApplyNonNullDefault
                = needToApplyReturnNonNullDefault | needToApplyParameterNonNullDefault.hasAnyNonNullDefault();
            ...
            if (isInstanceMethod) {
                ...
                findAllOverriddenMethods(currentMethod.original(), currentMethod.selector, paramLen, currentType,
                    new HashSet(), superMethodList);

                // prepare interim storage for nullness info so we don't pollute currentMethod before we know its
                // conflict-free:
                InheritedNonNullnessInfo[] inheritedNonNullnessInfos = new InheritedNonNullnessInfo[paramLen + 1]; // index
                                                                                                                   // 0
                                                                                                                   // is
                                                                                                                   // for
                                                                                                                   // the
                ...
                for (int i = length; --i >= 0;) {
                    ...
                    if ((currentSuper.tagBits & TagBits.IsNullnessKnown) == 0) {
                        ...
                        checkImplicitNullAnnotations(currentSuper, null, false, scope); // TODO (stephan) complain=true
                                                                                        // if currentSuper is source
                                                                                        // method??
                    }
                    checkNullSpecInheritance(currentMethod, srcMethod, needToApplyReturnNonNullDefault,
                        needToApplyParameterNonNullDefault, complain, currentSuper, null, scope,
                    ...
                    if (tagBits != 0) {
                        if (!usesTypeAnnotations) {
                            ...
                            if (!currentMethod.returnType.isBaseType()) {
                                ...
                                currentMethod.returnType = env.createAnnotatedType(currentMethod.returnType,
                                    env.nullAnnotationsFromTagBits(tagBits));
                            }
                        }
                    }
                }
                for (int i = 0; i < paramLen; i++) {
                    ...
                    if (!info.complained && info.inheritedNonNullness != null) {
                        ...
                        if (!usesTypeAnnotations)
                            ...
                            recordArgNonNullness18(currentMethod, i, currentArg, info.inheritedNonNullness,
                                scope.environment());
                    }
                }
            ...
            if (needToApplyNonNullDefault) {
                if (!usesTypeAnnotations)
                    currentMethod.fillInDefaultNonNullness(srcMethod, needToApplyReturnNonNullDefault,
    ...
    private void collectOverriddenMethods(MethodBinding original, char[] selector, int suggestedParameterLength,
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (MethodVerifier.doesMethodOverride(original, currentMethod, this.environment)) {
                ...
                added = true; // when overriding one or more methods from superType don't traverse to transitive
                              // superTypes
            }
        }
        if (!added)
    ...
    void checkNullSpecInheritance(MethodBinding currentMethod, AbstractMethodDeclaration srcMethod,
        boolean hasReturnNonNullDefault, ParameterNonNullDefaultProvider hasParameterNonNullDefault,
        boolean shouldComplain, MethodBinding inheritedMethod, MethodBinding[] allInheritedMethods, Scope scope,
        ...
        // (1) during MethodVerifyer15.checkMethods() we want to report errors (against srcMethod or against the current
        // type)
        // In this case this method is directly called from MethodVerifier15 (checkAgainstInheritedMethod /
        // checkConcreteInheritedMethod)
        // (2) during on-demand invocation we are mainly interested in the side effects of copying inherited null
        // annotations
        ...
        returnType: {
            ...
            if (currentNullnessBits == 0) {
                            ...
                            if (shouldComplain && inheritedNullnessBits == TagBits.AnnotationNullable) {
                                ...
                                if (srcMethod instanceof MethodDeclaration) {
                                    ...
                                    if (currentRecordAccessor.recordComponentBinding != null) {
                                        RecordComponent sourceRecordComponent
                                            = currentRecordAccessor.sourceRecordComponent();
                                ...
                                scope.problemReporter()
                                    .conflictingNullAnnotations(currentMethod, location, inheritedMethod);
                                // still use the inherited bits to avoid incompatibility
                            }
                        }
                        if (currentRecordAccessor == null) { // never transfer nullness from inherited method to record
                                                             // component
                ...
                if (hasReturnNonNullDefault
                    && (!useTypeAnnotations || currentMethod.returnType.acceptsNonNullDefault())) { // conflict with
                                                                                                    // inheritance
                                                                                                    // already checked
            ...
            if (shouldComplain) {
                if ((inheritedNullnessBits & TagBits.AnnotationNonNull) != 0
                    ...
                    if (srcMethod != null) {
                        scope.problemReporter()
                            .illegalReturnRedefinition(srcMethod, inheritedMethod,
                                this.environment.getNonNullAnnotationName());
                        break returnType;
                    } else {
                        scope.problemReporter()
                            .cannotImplementIncompatibleNullness(scope.referenceContext(), currentMethod,
                                inheritedMethod, useTypeAnnotations);
                        return;
                    }
                }
                if (useTypeAnnotations) {
                    TypeBinding substituteReturnType = null; // for TVB identity checks inside
                                                             // NullAnnotationMatching.analyze()
                    ...
                    if (NullAnnotationMatching
                        .analyse(inheritedMethod.returnType, currentMethod.returnType, substituteReturnType, null, 0,
                            null, CheckMode.OVERRIDE_RETURN)
                        .isAnyMismatch()) {
                        if (srcMethod != null)
                            scope.problemReporter()
                                .illegalReturnRedefinition(srcMethod, inheritedMethod,
        ...
        parameterLoop: for (int i = 0; i < length; i++) {
                ...
                if (inheritedNonNullNess != null) {
                    if (shouldInherit) {
                        if (hasParameterNonNullDefault.hasNonNullDefaultForParam(i)) {
                            // both inheritance and default: check for conflict?
                            if (shouldComplain && inheritedNonNullNess == Boolean.FALSE && currentArgument != null) {
                                scope.problemReporter()
                        ...
                        if (inheritedNonNullnessInfos != null && srcMethod != null) {
                            recordDeferredInheritedNullness(scope, srcMethod.arguments[i].type, inheritedMethod,
                                inheritedNonNullNess, inheritedNonNullnessInfos[i + 1]);
                        } else {
                            // no need to defer, record this info now:
                            if (!useTypeAnnotations)
                                recordArgNonNullness(currentMethod, length, i, currentArgument, inheritedNonNullNess);
                            else
                                recordArgNonNullness18(currentMethod, i, currentArgument, inheritedNonNullNess,
                                    this.environment);
                        }
                        continue; // compatible by construction, skip complain phase below
                    }
                }
                if (hasParameterNonNullDefault.hasNonNullDefaultForParam(i)) { // conflict with inheritance already
                                                                               // checked
            ...
            if (shouldComplain) {
                    ...
                    && currentNonNullNess == Boolean.TRUE)	// current parameter is restricted to @NonNull
                {
                    ...
                    if (currentArgument != null) {
                        scope.problemReporter()
                            .illegalRedefinitionToNonNullParameter(currentArgument, inheritedMethod.declaringClass,
                    ...
                    if (inheritedNonNullNess == Boolean.FALSE) {
                        if (currentArgument != null) {
                            scope.problemReporter()
                                .parameterLackingNullableAnnotation(currentArgument, inheritedMethod.declaringClass,
                                    annotationName);
                        } else {
                            scope.problemReporter()
                                .cannotImplementIncompatibleNullness(scope.referenceContext(), currentMethod,
                        ...
                        if (allInheritedMethods != null) {
                            // avoid this optional warning if the conflict already existed in one supertype (merging of
                        ...
                        if (currentArgument != null) {
                            scope.problemReporter()
                                .parameterLackingNonnullAnnotation(currentArgument, inheritedMethod.declaringClass,
                                    annotationName);
                        } else {
                            TypeDeclaration type = scope.classScope().referenceContext;
                            ASTNode location = type.superclass != null ? type.superclass : type;
                            scope.problemReporter()
                                .inheritedParameterLackingNonnullAnnotation(currentMethod, i + 1,
                                    inheritedMethod.declaringClass, location, annotationName);
                        }
                        continue;
                    }
                }
                if (useTypeAnnotations) {
                    ...
                    if (NullAnnotationMatching
                        .analyse(currentMethod.parameters[i], inheritedParameter, substituteParameter, null, 0, null,
                            CheckMode.OVERRIDE)
                        .isAnyMismatch()) {
                        if (currentArgument != null)
                            scope.problemReporter()
                                .illegalParameterRedefinition(currentArgument, inheritedMethod.declaringClass,
                                    inheritedParameter);
                        else
                            scope.problemReporter()
                                .cannotImplementIncompatibleNullness(scope.referenceContext(), currentMethod,
        ...
        if (shouldComplain && useTypeAnnotations && srcMethod != null) {
            ...
            if (currentTypeVariables != Binding.NO_TYPE_VARIABLES
                && currentTypeVariables.length == inheritedTypeVariables.length) {
                for (int i = 0; i < currentTypeVariables.length; i++) {
                    TypeVariableBinding inheritedVariable = inheritedTypeVariables[i];
                    if (NullAnnotationMatching
                        .analyse(inheritedVariable, currentTypeVariables[i], null, null, -1, null,
                            CheckMode.BOUND_CHECK)
                        .isAnyMismatch())
                        scope.problemReporter()
                            .cannotRedefineTypeArgumentNullity(inheritedVariable, inheritedMethod,
                                srcMethod.typeParameters()[i]);
                }
            }
        }
    }
    ...
    void applyReturnNullBits(MethodBinding method, long nullnessBits) {
        if (this.environment.usesNullTypeAnnotations()) {
            if (!method.returnType.isBaseType()) {
                method.returnType = this.environment.createAnnotatedType(method.returnType,
                    this.environment.nullAnnotationsFromTagBits(nullnessBits));
            }
        } else {
    ...
    private Boolean getParameterNonNullness(MethodBinding method, int i, boolean useTypeAnnotations) {
        ...
        return (method.parameterFlowBits == null) ? null : method.getParameterNullness(i);
    }
    ...
    private long getExplicitReturnTypeNullnessTagBits(MethodBinding method, boolean useTypeAnnotations) {
        if (useTypeAnnotations) {
            if (method.returnType == null)
                return 0L;
            if (method instanceof SyntheticMethodBinding synth
                && synth.purpose == SyntheticMethodBinding.RecordComponentReadAccess) {
    ...
    protected void recordDeferredInheritedNullness(Scope scope, ASTNode location, MethodBinding inheritedMethod,
        Boolean inheritedNonNullness, InheritedNonNullnessInfo nullnessInfo) {
        if (nullnessInfo.inheritedNonNullness != null && nullnessInfo.inheritedNonNullness != inheritedNonNullness) {
            scope.problemReporter()
                .conflictingInheritedNullAnnotations(location, nullnessInfo.inheritedNonNullness.booleanValue(),
                    nullnessInfo.annotationOrigin, inheritedNonNullness.booleanValue(), inheritedMethod);
    ...
    void recordArgNonNullness(MethodBinding method, int paramCount, int paramIdx, Argument currentArgument,
        Boolean nonNullNess) {
    ...
    void recordArgNonNullness18(MethodBinding method, int paramIdx, Argument currentArgument, Boolean nonNullNess,
        ...
        method.parameters[paramIdx]
            = env.createAnnotatedType(method.parameters[paramIdx], new AnnotationBinding[] { annotationBinding });
        if (currentArgument != null) {
    ...
    static boolean areParametersEqual(MethodBinding one, MethodBinding two) {
        ...
        for (i++; i < length; i++) {
            if (!areTypesEqual(oneArgs[i], twoArgs[i])) {
                ...
                return false; // no remaining parameter can be a Parameterized type (if one has been converted then all
                              // RAW types must be converted)
            }
        }
        return true;
    }
