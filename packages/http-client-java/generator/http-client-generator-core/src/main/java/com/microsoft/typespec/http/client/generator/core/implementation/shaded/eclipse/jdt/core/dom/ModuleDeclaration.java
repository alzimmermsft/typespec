...
public class ModuleDeclaration extends ASTNode {
     ...
     */
    public static final ChildListPropertyDescriptor MODULE_DIRECTIVES_PROPERTY = new ChildListPropertyDescriptor(
        ModuleDeclaration.class, "moduleDirectives", ModuleDirective.class, NO_CYCLE_RISK); //$NON-NLS-1$
     ...
     */
    private final NodeList annotations = new NodeList(ANNOTATIONS_PROPERTY);
     ...
     * Defaults to an empty list.
     */
    private final NodeList moduleStatements = new NodeList(MODULE_DIRECTIVES_PROPERTY);
            ...
            synchronized (this) {
                if (this.name == null) {
                    ...
                    this.name = postLazyInit(
                        this.ast.newQualifiedName(new SimpleName(this.ast), new SimpleName(this.ast)), NAME_PROPERTY);
                }
            }
        }
        return this.name;
    ...
    @Override
    int treeSize() {
        return memSize() + (this.optionalDocComment == null ? 0 : getJavadoc().treeSize()) + this.annotations.listSize()
            + (this.name == null ? 0 : getName().treeSize()) + this.moduleStatements.listSize();
    }
}
