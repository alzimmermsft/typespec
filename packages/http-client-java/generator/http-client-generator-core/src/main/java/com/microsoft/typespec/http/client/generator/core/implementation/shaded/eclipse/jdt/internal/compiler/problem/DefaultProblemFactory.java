...
public class DefaultProblemFactory implements IProblemFactory {
    ...
    public CategorizedProblem createProblem(char[] originatingFileName, int problemId, String[] problemArguments,
        ...
        return new DefaultProblem(originatingFileName,
            this.getLocalizedMessage(problemId, elaborationId, messageArguments), problemId, problemArguments, severity,
    ...
    public final String getLocalizedMessage(int id, int elaborationId, String[] problemArguments) {
        ...
        if ((id & IProblem.Javadoc) != 0) {
            ...
            output.append((String) this.messageTemplates
                .get(keyFromID(IProblem.JavadocMessagePrefix & IProblem.IgnoreCategoriesMask)));
        }
        while (true) {
            if ((end = CharOperation.indexOf('{', message, start)) > -1) {
                if (output == null)
                    output = new StringBuilder(length + problemArguments.length * 20);
    ...
    public static HashtableOfInt loadMessageTemplates(Locale loc) {
        ...
        String bundleName
            = "com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.problem.messages"; //$NON-NLS-1$
        try {
