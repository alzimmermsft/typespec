...
public class JavaElementDelta extends SimpleDelta implements IJavaElementDelta {
    ...
    protected JavaElementDelta createDeltaTree(IJavaElement element, JavaElementDelta delta) {
        ...
        if (ancestors == null) {
            if (equalsAndSameParent(delta.getElement(), getElement())) { // handle case of two jars that can be equals
    ...
    protected JavaElementDelta find(IJavaElement e) {
        if (equalsAndSameParent(getElement(), e)) // handle case of two jars that can be equals but not in the same
                                                  // project
            return this;
    ...
    public IResourceDelta[] getResourceDeltas() {
        ...
        if (this.resourceDeltas.length != this.resourceDeltasCounter) {
            System.arraycopy(this.resourceDeltas, 0,
                this.resourceDeltas = new IResourceDelta[this.resourceDeltasCounter], 0, this.resourceDeltasCounter);
        }
    ...
    protected void insertDeltaTree(IJavaElement element, JavaElementDelta delta) {
        ...
        if (!equalsAndSameParent(element, getElement())) { // handle case of two jars that can be equals but not in the
                                                           // same project
