 ...
 *******************************************************************************/
package com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser;

import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.TerminalToken.*;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.diagnose.DiagnoseParser;
...
public class Parser implements ParserBasicInformation, ConflictedParser, OperatorIds, TypeIds {
            ...
            if ("3".equals(tokens[i])) { //$NON-NLS-1$
                ...
                for (String terminalName : terminalNames) {
                    ...
                    if (symbol > -1) {
                        ...
                        if (length == newRecoveyTemplatesPtr + 1) {
                            System.arraycopy(newRecoveyTemplates, 0, newRecoveyTemplates = new char[length * 2], 0,
                                length);
                        }
    ...
    public final static void buildFilesFromLPG(String dataFilename, String dataFilename2) {
        ...
        // [com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.Parser.buildFilesFromLPG("d:/leapfrog/grammar/javadcl.java")]
    ...
    public final static void initTables() throws IOException {
    ...
    protected boolean diet = false; // tells the scanner to jump over some parts of the code/expressions like method
                                    // bodies
    protected int dietInt = 0; // if > 0 force the none-diet-parsing mode (even if diet if requested) [field parsing
                               // with anonymous inner classes...]
    protected int endPosition; // accurate only when used ! (the start position is pushed into intStack while the end
                               // the current one)
...
// used for recovery
    ...
    public com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ReadManager readManager;
    ...
    public Parser() {
        // Caveat Emptor: For inheritance purposes and then only in very special needs. Only minimal state is
        // initialized !
    }
    ...
    public Parser(ProblemReporter problemReporter, boolean optimizeStringLiterals) {
        ...
        this.previewEnabled
            = this.options.sourceLevel == ClassFileConstants.getLatestJDKLevel() && this.options.enablePreviewFeatures;
    ...
    public void arrayInitializer(int length) {
        ...
        if (length != 0) {
            ...
            System.arraycopy(this.expressionStack, this.expressionPtr + 1, ai.expressions = new Expression[length], 0,
                length);
        }
    ...
    public void checkComment() {
        ...
        if (lastComment >= 0) {
            ...
            if (this.forStartPosition != 0 || this.forStartPosition < lastCommentStart) {// if there is no 'for'
                                                                                         // in-between.
                this.modifiersSourceStart = lastCommentStart;
            }

            // check deprecation in last comment if javadoc (can be followed by non-javadoc comments which are simply
            // ignored)
    ...
    protected void classInstanceCreation(boolean isQualified) {
        ...
        if (((length = this.astLengthStack[this.astLengthPtr--]) == 1) && (this.astStack[this.astPtr] == null)) {
            ...
            if ((length = this.expressionLengthStack[this.expressionLengthPtr--]) != 0) {
                this.expressionPtr -= length;
                System.arraycopy(this.expressionStack, this.expressionPtr + 1, alloc.arguments = new Expression[length],
                    0, length);
            }
    ...
    protected void checkForDiamond(TypeReference allocType) {
        if (allocType instanceof ParameterizedSingleTypeReference type) {
            ...
            if (type.typeArguments[type.typeArguments.length - 1] == TypeReference.NO_TYPE_ARGUMENTS) { // Don't care
                                                                                                        // for X<>.Y<>
                                                                                                        // and
                                                                                                        // X<>.Y<String>
                type.bits |= ASTNode.IsDiamond;
    ...
    protected ParameterizedQualifiedTypeReference computeQualifiedGenericsFromRightSide(TypeReference rightSide,
        int dim, Annotation[][] annotationsOnDimensions) {
        ...
        ParameterizedQualifiedTypeReference typeRef
            = new ParameterizedQualifiedTypeReference(tokens, typeArguments, dim, annotationsOnDimensions, positions);
        ...
        while (nameSize > 0) {
            ...
            if ((length = this.typeAnnotationLengthStack[this.typeAnnotationLengthPtr--]) != 0) {
                ...
                System.arraycopy(this.typeAnnotationStack, (this.typeAnnotationPtr -= length) + 1,
                    typeAnnotations[nameSize - 1] = new Annotation[length], 0, length);
                if (nameSize == 1) {
    ...
    protected void consumeAnnotationName() {
        if (this.currentElement != null && !this.expectTypeAnnotation) {
            ...
            if (this.annotationRecoveryActivated) {
                this.currentElement
                    = this.currentElement.addAnnotationName(this.identifierPtr, this.identifierLengthPtr, start, 0);
            }
        }
    ...
    protected void consumeAnnotationTypeDeclarationHeaderName() {
        ...
        annotationTypeDeclaration.modifiers
            = this.intStack[this.intPtr--] | ClassFileConstants.AccAnnotation | ClassFileConstants.AccInterface;
...
//	annotationTypeDeclaration.typeParameters = null;
    ...
    protected void consumeAnnotationTypeMemberDeclaration() {
        // AnnotationTypeMemberDeclaration ::= AnnotationTypeMemberDeclarationHeader
        // AnnotationTypeMemberHeaderExtendedDims DefaultValueopt ';'
    ...
    protected void consumeArrayAccess(boolean unspecifiedReference) {
        ...
        if (unspecifiedReference) {
            exp = this.expressionStack[this.expressionPtr]
                = new ArrayReference(getUnspecifiedReferenceOptimized(), this.expressionStack[this.expressionPtr]);
        } else {
            this.expressionPtr--;
                    ...
                    if (expr1 instanceof StringLiteral string1) {
                        if (((expr1.bits & ASTNode.ParenthesizedMASK) >> ASTNode.ParenthesizedSHIFT) == 0) {
                            if (expr2 instanceof CharLiteral charLiteral) { // string+char
                                this.expressionStack[this.expressionPtr] = string1.extendWith(charLiteral);
                            } else if (expr2 instanceof StringLiteral string2) { // string+string
                                this.expressionStack[this.expressionPtr] = string1.extendWith(string2);
                            } else {
...
//						depthTracker + 1;
                    ...
                    if (expr2 instanceof StringLiteral
                        ...
                        // string + string
                        this.expressionStack[this.expressionPtr] = string.extendsWith((StringLiteral) expr2);
                    } else {
...
//						depthTracker + 1;
                ...
                if (this.optimizeStringLiterals) {
                    if (expr1 instanceof StringLiteral string1
                        ...
                        if (expr2 instanceof CharLiteral char2) { // string+char
                            this.expressionStack[this.expressionPtr] = string1.extendWith(char2);
                        } else if (expr2 instanceof StringLiteral string2) { // string+string
                            this.expressionStack[this.expressionPtr] = string1.extendWith(string2);
                        } else {
                            this.expressionStack[this.expressionPtr] = new BinaryExpression(expr1, expr2, PLUS);
    ...
    protected void consumeCastExpressionLL1() {
        ...
        this.expressionPtr--;
        this.expressionStack[this.expressionPtr]
            = cast = new CastExpression(exp = this.expressionStack[this.expressionPtr + 1],
    ...
    protected void consumeCastExpressionLL1WithBounds() {
        ...
        this.expressionStack[this.expressionPtr]
            = cast = new CastExpression(exp, createIntersectionCastTypeReference(bounds));
    ...
    protected void consumeCastExpressionWithGenericsArray() {
        // CastExpression ::= PushLPAREN Name TypeArguments Dimsopt AdditionalBoundsListOpt PushRPAREN
        ...
        this.expressionStack[this.expressionPtr]
            = cast = new CastExpression(exp = this.expressionStack[this.expressionPtr], castType);
    ...
    protected void consumeCastExpressionWithNameArray() {
        // CastExpression ::= PushLPAREN Name Dims AdditionalBoundsListOpt PushRPAREN InsideCastExpression
    ...
    protected void consumeCastExpressionWithPrimitiveType() {
        // CastExpression ::= PushLPAREN PrimitiveType Dimsopt AdditionalBoundsListOpt PushRPAREN InsideCastExpression
        // UnaryExpression
    ...
    protected void consumeCastExpressionWithQualifiedGenericsArray() {
        // CastExpression ::= PushLPAREN Name OnlyTypeArguments '.' ClassOrInterfaceType Dimsopt AdditionalBoundsOpt
    ...
    protected void consumeCatchType() {
        ...
        if (length != 1) {
            ...
            System.arraycopy(this.astStack, (this.astPtr -= length) + 1, (typeReferences = new TypeReference[length]),
                0, length);
        ...
        if (needDefaultConstructor) {
            switch (TypeDeclaration.kind(typeDecl.modifiers)) {
                ...
                case TypeDeclaration.ENUM_DECL:
                    ...
                    typeDecl.createDefaultConstructor(!(this.diet && this.dietInt == 0) || insideFieldInitializer,
                        true);
            }
        }
    ...
    protected void consumeClassInstanceCreationExpressionQualifiedWithTypeArguments() {
        // ClassInstanceCreationExpression ::= Primary '.' 'new' TypeArguments SimpleName '(' ArgumentListopt ')'
        // ClassBodyopt
        ...
        if (((length = this.astLengthStack[this.astLengthPtr--]) == 1) && (this.astStack[this.astPtr] == null)) {
            ...
            this.genericsPtr -= length;
            System.arraycopy(this.genericsStack, this.genericsPtr + 1, alloc.typeArguments = new TypeReference[length],
                0, length);
            this.intPtr--;
    ...
    protected void consumeConditionalExpression(int op) {
        ...
        this.expressionLengthPtr -= 2;
        this.expressionStack[this.expressionPtr] = new ConditionalExpression(this.expressionStack[this.expressionPtr],
    ...
    protected void consumeConditionalExpressionWithName(int op) {
        ...
        this.expressionLengthPtr -= 2;
        this.expressionStack[this.expressionPtr]
            = new ConditionalExpression(this.expressionStack[this.expressionPtr + 2],
                this.expressionStack[this.expressionPtr], this.expressionStack[this.expressionPtr + 1]);
    }
    ...
    protected void consumeEnterVariable() {
        ...
        declaration.type = extendedDimensions == 0
            ? type
            : augmentTypeWithAdditionalDimensions(type, extendedDimensions, annotationsOnExtendedDimensions, false);
            ...
            if (!(this.currentElement instanceof RecoveredType)
                && (this.currentToken == TokenNameDOT
                    ...
                    || (Util.getLineNumber(declaration.type.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr)
                        != Util.getLineNumber((int) (namePosition >>> 32), this.scanner.lineEnds, 0,
    ...
    protected void consumeEnumConstantHeaderName() {
        if (this.currentElement != null) {
            if (!(this.currentElement instanceof RecoveredType
                || (this.currentElement instanceof RecoveredField recoveredField
                    && recoveredField.fieldDeclaration.type == null))
...
//	}
    ...
    protected void consumeExplicitConstructorInvocation(int flag, int recFlag) {
        ...
        if ((length = this.expressionLengthStack[this.expressionLengthPtr--]) != 0) {
            this.expressionPtr -= length;
            System.arraycopy(this.expressionStack, this.expressionPtr + 1, ecc.arguments = new Expression[length], 0,
                length);
        }
        switch (flag) {
            case 0:
...
// Common consumer for formal parameters and record components both of which use single variable declarator syntax (as opposed to say fields)
    protected void consumeSingleVariableDeclarator(boolean isVarArgs) {
        ...
        boolean isReceiver = this.intStack[this.intPtr--] == 0;  // flag pushed in consumeExplicitThisParameter -> 0,
                                                                 // consumeVariableDeclaratorIdParameter -> 1
        if (isReceiver) {
        ...
        if (isVarArgs || extendedDimensions != 0) {
            ...
            if (extendedDimensions != 0) {
                type = augmentTypeWithAdditionalDimensions(type, extendedDimensions, annotationsOnExtendedDimensions,
                    false);
            }
        ...
        if ((length = this.expressionLengthStack[this.expressionLengthPtr--]) != 0) {
            System.arraycopy(this.expressionStack, (this.expressionPtr -= length) + 1,
                singleVariable.annotations = new Annotation[length], 0, length);
    ...
    protected void consumeGenericTypeWithDiamond() {
        ...
        concatGenericsLists();
        this.intPtr--;	// pop the null dimension pushed in by consumeReferenceType, as we have no type between <>,
                      	// getTypeReference won't kick in
    }
...
//	exp.modifiers = this.intStack[this.intPtr--];
    ...
    protected void consumePermittedTypes() {
        ...
        System.arraycopy(this.astStack, this.astPtr + 1, typeDecl.permittedTypes = new TypeReference[length], 0,
            length);
    ...
    protected void consumeInternalCompilationUnitWithPotentialImplicitlyDeclaredClass() {
        ...
        if ((length = this.astLengthStack[this.astLengthPtr--]) != 0) {
            ...
            if (!methods.isEmpty() || !fields.isEmpty()) {
                problemReporter().validateJavaFeatureSupport(JavaFeature.IMPLICIT_CLASSES_AND_INSTANCE_MAIN_METHODS, 0,
                    0);
    ...
    protected void consumeInvalidInterfaceDeclaration() {
        ...
        // InterfaceDeclaration ::= Modifiersopt 'interface' 'Identifier' ExtendsInterfacesopt InterfaceHeader
        // InterfaceBody
        if (this.options.sourceLevel >= ClassFileConstants.JDK16)
    ...
    protected void consumeInterfaceMethodDeclaration(boolean hasSemicolonBody) {
        ...
        // -- the next rule is illegal but allows to give a more canonical error message from inside
        ...
        if (!hasSemicolonBody) {
            ...
            if ((length = this.astLengthStack[this.astLengthPtr--]) != 0) {
                if (this.options.ignoreMethodBodies) {
                    ...
                    System.arraycopy(this.astStack, (this.astPtr -= length) + 1, statements = new Statement[length], 0,
                        length);
                }
            }
        }
        ...
        if (bodyAllowed && hasSemicolonBody) {
            md.modifiers |= ExtraCompilerModifiers.AccSemicolonBody; // avoid complaints regarding undocumented empty
                                                                     // body
        }
    ...
    protected void consumeLocalVariableDeclarationStatement() {
        ...
        if (variableDeclaratorsCounter == 1) {
            ...
            if (localDeclaration.isRecoveredFromLoneIdentifier()) {
                // https://bugs.eclipse.org/bugs/show_bug.cgi?id=430336, [1.8][compiler] Bad syntax error recovery:
                // Lonely identifier should be variable name, not type
    ...
    protected void consumeMethodHeader() {
        ...
        // MethodHeaderExtendedDims AnnotationMethodHeaderDefaultValueopt
        // RecoveryMethodHeader ::= RecoveryMethodHeaderName FormalParameterListopt MethodHeaderRightParen
        // MethodHeaderExtendedDims MethodHeaderThrowsClause
...
//		} else
    ...
    protected void consumeMethodHeaderExtendedDims() {
        ...
        if (extendedDimensions != 0) {
            ...
            md.returnType = augmentTypeWithAdditionalDimensions(md.returnType, extendedDimensions,
                getAnnotationsOnDimensions(extendedDimensions), false);
    ...
    protected void consumeMethodHeaderRightParen() {
        ...
        if (length != 0) {
            ...
            if (arg.isReceiver()) {
                ...
                if (length > 1) {
                    System.arraycopy(this.astStack, this.astPtr + 2, md.arguments = new Argument[length - 1], 0,
                        length - 1);
                }
    ...
    protected void consumeInvocationExpression() {
        // Trap all forms of invocation expressions. Note: Explicit constructor calls are not expressions. Top of
        // expression stack has the MessageSend or AllocationExpression.
    }
    ...
    protected void consumeNestedType() {
        ...
        if (++this.nestedType >= length) {
            ...
            // increase the size of the variablesCounter as well. It has to be consistent with the size of the
            // nestedMethod collection
...
// noop on 1.8+
        ...
        if (siName.annotations != null) {
            for (int j = 0; j < siName.annotations.length; j++) {
                ...
                if (qualifierAnnot != null && qualifierAnnot.length > 0) {
                    problemReporter().misplacedTypeAnnotations(qualifierAnnot[0],
    ...
    protected void consumeWithClause() {
        ...
        System.arraycopy(this.astStack, this.astPtr + 1, service.implementations = new TypeReference[length], 0,
            length);
...
//		this.javadoc = null;
            ...
            { // array copy
                switch (flagI) {
                    case 1:
                        ...
                        System.arraycopy(this.astStack, this.astPtr + start + 1, modul.requires, size1 - length2,
                            length2);
                        break;

                    case 2:
                        size2 += (length2 = end - start);
                        System.arraycopy(this.astStack, this.astPtr + start + 1, modul.exports, size2 - length2,
                            length2);
                        break;
                    ...
                    case 4:
                        ...
                        System.arraycopy(this.astStack, this.astPtr + start + 1, modul.services, size4 - length2,
                            length2);
                        break;
    ...
    protected void consumePackageDeclarationNameWithModifiers() {
        ...
        if ((length = this.expressionLengthStack[this.expressionLengthPtr--]) != 0) {
            ...
            packageModifiersSourceEnd = this.intStack[this.intPtr--] - 2; // we don't need the position of the 'package
                                                                          // keyword
        } else {
            impt.declarationSourceStart = this.intStack[this.intPtr--];
    ...
    protected void consumePrimaryNoNewArray() {
        ...
        int numberOfParenthesis
            = (parenthesizedExpression.bits & ASTNode.ParenthesizedMASK) >> ASTNode.ParenthesizedSHIFT;
    ...
    protected void rejectIllegalLeadingTypeAnnotations(TypeReference typeReference) {
        // Reject misplaced annotations prefixed to a type reference; Used when the grammar is permissive enough to
    ...
    private void rejectIllegalTypeAnnotations(TypeReference typeReference) {
        // Reject misplaced annotations on type reference; Used when grammar is permissive enough to allow them in the
        // first place.
        Annotation[][] annotations = typeReference.annotations;
        ...
        for (int i = 0, length = annotations == null ? 0 : annotations.length; i < length; i++) {
            ...
            if (misplacedAnnotations != null) {
                problemReporter().misplacedTypeAnnotations(misplacedAnnotations[0],
    ...
    protected void consumePushCombineModifiers() {
        ...
        // int stack on entry : ... Modifiers, ModifiersSourceStart, defaultSourceStart, defaultSourceEnd, Modifiers',
        // Modifiers'SourceStart <<--- intPtr
    ...
    protected void consumeReduceImports() {
        ...
        if ((length = this.astLengthStack[this.astLengthPtr--]) != 0) {
            this.astPtr -= length;
            System.arraycopy(this.astStack, this.astPtr + 1, this.compilationUnit.imports = new ImportReference[length],
                0, length);
        }
    }
    ...
    protected void consumeResourceAsLocalVariableDeclaration() {
        // Resource ::= Type PushModifiers VariableDeclaratorId EnterVariable '=' ForceNoDiet VariableInitializer
        // RestoreDiet ExitVariableWithInitialization
        // Resource ::= Modifiers Type PushRealModifiers VariableDeclaratorId EnterVariable '=' ForceNoDiet
        // VariableInitializer RestoreDiet ExitVariableWithInitialization
    ...
    protected void consumeNonTypeUseName() { // https://bugs.eclipse.org/bugs/show_bug.cgi?id=383596
        ...
        // We can get here with type annotation stack empty, because completion parser manipulates the identifier stacks
        // even without rule reduction. See completionIdentifierCheck
        for (int i = this.identifierLengthStack[this.identifierLengthPtr]; i > 0 && this.typeAnnotationLengthPtr >= 0;
            --i) {
            int length = this.typeAnnotationLengthStack[this.typeAnnotationLengthPtr--];
            Annotation[] typeAnnotations;
            if (length != 0) {
...
// This method is part of an automatic generation : do NOT edit-modify
    protected void consumeRule(int act) {
        switch (act) {
            ...
            case 68:
                if (DEBUG) {
                    System.out.println("Name ::= SimpleName"); //$NON-NLS-1$
                }
            ...
            case 73:
                if (DEBUG) {
                    System.out.println("UnannotatableName ::= UnannotatableName DOT SimpleName"); //$NON-NLS-1$
                }
            ...
            case 75:
                if (DEBUG) {
                    System.out.println("QualifiedName ::= Name DOT TypeAnnotations SimpleName"); //$NON-NLS-1$
                }
            ...
            case 76:
                if (DEBUG) {
                    System.out.println("TypeAnnotationsopt ::="); //$NON-NLS-1$
                }
                consumeZeroTypeAnnotations();
                break;
            ...
            case 80:
                if (DEBUG) {
                    System.out.println("TypeAnnotations0 ::= TypeAnnotations0 TypeAnnotation"); //$NON-NLS-1$
                }
            ...
            case 81:
                if (DEBUG) {
                    System.out.println("TypeAnnotation ::= NormalTypeAnnotation"); //$NON-NLS-1$
                }
            ...
            case 82:
                if (DEBUG) {
                    System.out.println("TypeAnnotation ::= MarkerTypeAnnotation"); //$NON-NLS-1$
                }
                consumeTypeAnnotation();
                break;
            ...
            case 83:
                if (DEBUG) {
                    System.out.println("TypeAnnotation ::= SingleMemberTypeAnnotation"); //$NON-NLS-1$
                }
                consumeTypeAnnotation();
                break;
            ...
            case 84:
                if (DEBUG) {
                    System.out.println("TypeAnnotationName ::= AT308 UnannotatableName"); //$NON-NLS-1$
                }
            ...
            case 85:
                if (DEBUG) {
                    System.out.println("NormalTypeAnnotation ::= TypeAnnotationName LPAREN..."); //$NON-NLS-1$
                }
            ...
            case 86:
                if (DEBUG) {
                    System.out.println("MarkerTypeAnnotation ::= TypeAnnotationName"); //$NON-NLS-1$
                }
            ...
            case 87:
                if (DEBUG) {
                    System.out.println("SingleMemberTypeAnnotation ::= TypeAnnotationName LPAREN"); //$NON-NLS-1$
                }
            ...
            case 88:
                if (DEBUG) {
                    System.out.println("RejectTypeAnnotations ::="); //$NON-NLS-1$
                }
            ...
            case 89:
                if (DEBUG) {
                    System.out.println("PushZeroTypeAnnotations ::="); //$NON-NLS-1$
                }
                consumeZeroTypeAnnotations();
                break;
            ...
            case 90:
                if (DEBUG) {
                    System.out.println("VariableDeclaratorIdOrThis ::= this"); //$NON-NLS-1$
                }
            ...
            case 91:
                if (DEBUG) {
                    System.out.println("VariableDeclaratorIdOrThis ::= UnannotatableName DOT this"); //$NON-NLS-1$
                }
            ...
            case 92:
                if (DEBUG) {
                    System.out.println("VariableDeclaratorIdOrThis ::= VariableDeclaratorId"); //$NON-NLS-1$
                }
            ...
            case 101:
                if (DEBUG) {
                    System.out.println("InternalCompilationUnit ::= ModuleDeclaration"); //$NON-NLS-1$
                }
                consumeInternalCompilationUnitWithModuleDeclaration();
                break;
            ...
            case 102:
                if (DEBUG) {
                    System.out.println("ModuleDeclaration ::= ModuleHeader ModuleBody"); //$NON-NLS-1$
                }
            ...
            case 105:
                if (DEBUG) {
                    System.out.println("ModuleHeader ::= Modifiersopt ModuleModifieropt module"); //$NON-NLS-1$
                }
                consumeModuleHeader();
                break;
            ...
            case 107:
                if (DEBUG) {
                    System.out.println("ModuleModifieropt ::= ModuleModifier"); //$NON-NLS-1$
                }
            ...
            case 110:
                if (DEBUG) {
                    System.out.println("ModuleStatementsOpt ::="); //$NON-NLS-1$
                }
            ...
            case 113:
                if (DEBUG) {
                    System.out.println("ModuleStatements ::= ModuleStatements ModuleStatement"); //$NON-NLS-1$
                }
            ...
            case 119:
                if (DEBUG) {
                    System.out.println("RequiresStatement ::= SingleRequiresModuleName SEMICOLON"); //$NON-NLS-1$
                }
            ...
            case 120:
                if (DEBUG) {
                    System.out.println("SingleRequiresModuleName ::= requires..."); //$NON-NLS-1$
                }
            ...
            case 121:
                if (DEBUG) {
                    System.out.println("RequiresModifiersopt ::= RequiresModifiers"); //$NON-NLS-1$
                }
                consumeModifiers();
                break;
            ...
            case 122:
                if (DEBUG) {
                    System.out.println("RequiresModifiersopt ::="); //$NON-NLS-1$
                }
            ...
            case 124:
                if (DEBUG) {
                    System.out.println("RequiresModifiers ::= RequiresModifiers RequiresModifier"); //$NON-NLS-1$
                }
                consumeModifiers2();
                break;
            ...
            case 127:
                if (DEBUG) {
                    System.out.println("ExportsStatement ::= ExportsHeader TargetModuleListopt"); //$NON-NLS-1$
                }
            ...
            case 128:
                if (DEBUG) {
                    System.out.println("ExportsHeader ::= exports SinglePkgName"); //$NON-NLS-1$
                }
            ...
            case 130:
                if (DEBUG) {
                    System.out.println("TargetModuleListopt ::= to TargetModuleNameList"); //$NON-NLS-1$
                }
            ...
            case 131:
                if (DEBUG) {
                    System.out.println("TargetModuleName ::= UnannotatableName"); //$NON-NLS-1$
                }
            ...
            case 133:
                if (DEBUG) {
                    System.out.println("TargetModuleNameList ::= TargetModuleNameList COMMA..."); //$NON-NLS-1$
                }
            ...
            case 134:
                if (DEBUG) {
                    System.out.println("SinglePkgName ::= UnannotatableName"); //$NON-NLS-1$
                }
            ...
            case 135:
                if (DEBUG) {
                    System.out.println("OpensStatement ::= OpensHeader TargetModuleListopt..."); //$NON-NLS-1$
                }
            ...
            case 136:
                if (DEBUG) {
                    System.out.println("OpensHeader ::= opens SinglePkgName"); //$NON-NLS-1$
                }
                consumeOpensHeader();
                break;
            ...
            case 137:
                if (DEBUG) {
                    System.out.println("UsesStatement ::= UsesHeader SEMICOLON"); //$NON-NLS-1$
                }
            ...
            case 138:
                if (DEBUG) {
                    System.out.println("UsesHeader ::= uses Name"); //$NON-NLS-1$
                }
                consumeUsesHeader();
                break;
            ...
            case 139:
                if (DEBUG) {
                    System.out.println("ProvidesStatement ::= ProvidesInterface WithClause..."); //$NON-NLS-1$
                }
            ...
            case 140:
                if (DEBUG) {
                    System.out.println("ProvidesInterface ::= provides Name"); //$NON-NLS-1$
                }
            ...
            case 141:
                if (DEBUG) {
                    System.out.println("ServiceImplName ::= Name"); //$NON-NLS-1$
                }
            ...
            case 143:
                if (DEBUG) {
                    System.out.println("ServiceImplNameList ::= ServiceImplNameList COMMA..."); //$NON-NLS-1$
                }
            ...
            case 144:
                if (DEBUG) {
                    System.out.println("WithClause ::= with ServiceImplNameList"); //$NON-NLS-1$
                }
                consumeWithClause();
                break;
            ...
            case 226:
                if (DEBUG) {
                    System.out.println("ImplicitlyDeclaredClassBodyDeclarations ::=..."); //$NON-NLS-1$
                }
            ...
            case 252:
                if (DEBUG) {
                    System.out.println("VariableDeclaratorId ::= UNDERSCORE"); //$NON-NLS-1$
                }
            ...
            case 257:
                if (DEBUG) {
                    System.out.println("MethodDeclaration ::= DefaultMethodHeader MethodBody"); //$NON-NLS-1$
                }
                // set to true to consume a method with a body
            ...
            case 260:
                if (DEBUG) {
                    System.out.println("DefaultMethodHeader ::= DefaultMethodHeaderName..."); //$NON-NLS-1$
                }
                consumeMethodHeader();
                break;
            ...
            case 263:
                if (DEBUG) {
                    System.out.println("DefaultMethodHeaderName ::= ModifiersWithDefault..."); //$NON-NLS-1$
                }
                consumeMethodHeaderNameWithTypeParameters(false);
                break;
            ...
            case 264:
                if (DEBUG) {
                    System.out.println("DefaultMethodHeaderName ::= ModifiersWithDefault Type..."); //$NON-NLS-1$
                }
                consumeMethodHeaderName(false);
                break;
            ...
            case 265:
                if (DEBUG) {
                    System.out.println("ModifiersWithDefault ::= Modifiersopt default..."); //$NON-NLS-1$
                }
            ...
            case 271:
                ...
                consumeConstructorHeaderName(false);
                break;

            case 272:
                if (DEBUG) {
                    System.out.println("CompactConstructorDeclaration ::=..."); //$NON-NLS-1$
                }
            ...
            case 273:
                if (DEBUG) {
                    System.out.println("CompactConstructorHeader ::=..."); //$NON-NLS-1$
                }
                consumeConstructorHeader();
                break;
            ...
            case 274:
                if (DEBUG) {
                    System.out.println("CompactConstructorHeaderName ::= Modifiersopt Identifier"); //$NON-NLS-1$
                }
                consumeConstructorHeaderName(true);
                break;

            case 276:
                if (DEBUG) {
                    System.out.println("SingleVariableDeclaratorList ::=..."); //$NON-NLS-1$
                }
            ...
            case 277:
                if (DEBUG) {
                    System.out.println("SingleVariableDeclarator ::= Modifiersopt Type..."); //$NON-NLS-1$
                }
                consumeSingleVariableDeclarator(false);
                break;
            ...
            case 278:
                ...
                consumeSingleVariableDeclarator(true);
                break;
            ...
            case 316:
                if (DEBUG) {
                    System.out.println("InterfaceMemberDeclaration ::= DefaultMethodHeader..."); //$NON-NLS-1$
                }
            ...
            case 331:
                if (DEBUG) {
                    System.out.println("RecordDeclaration ::= RecordHeaderPart ClassBody"); //$NON-NLS-1$
                }
                consumeClassDeclaration();
                break;
            ...
            case 332:
                if (DEBUG) {
                    System.out.println("RecordHeaderPart ::= RecordHeaderName RecordHeader..."); //$NON-NLS-1$
                }
            ...
            case 333:
                if (DEBUG) {
                    System.out.println("RecordHeaderName ::= RecordHeaderName1 TypeParameters"); //$NON-NLS-1$
                }
            ...
            case 334:
                if (DEBUG) {
                    System.out.println("RecordHeaderName ::= RecordHeaderName1"); //$NON-NLS-1$
                }
            ...
            case 335:
                if (DEBUG) {
                    System.out.println("RecordHeaderName1 ::= Modifiersopt..."); //$NON-NLS-1$
                }
            ...
            case 336:
                if (DEBUG) {
                    System.out.println("RecordComponentHeaderRightParen ::= RPAREN"); //$NON-NLS-1$
                }
            ...
            case 338:
                if (DEBUG) {
                    System.out.println("RecordComponentListOpt ::="); //$NON-NLS-1$
                }
            ...
            case 341:
                if (DEBUG) {
                    System.out.println("InstanceofExpression ::= InstanceofExpression..."); //$NON-NLS-1$
                }
            ...
            case 344:
                if (DEBUG) {
                    System.out.println("InstanceofClassic ::= instanceof Modifiersopt Type"); //$NON-NLS-1$
                }
            ...
            case 345:
                if (DEBUG) {
                    System.out.println("InstanceofPattern ::= instanceof Pattern"); //$NON-NLS-1$
                }
            ...
            case 348:
                if (DEBUG) {
                    System.out.println("TypePattern ::= Modifiersopt Type Identifier"); //$NON-NLS-1$
                }
                consumeTypePattern();
                break;
            ...
            case 349:
                if (DEBUG) {
                    System.out.println("TypePattern ::= Modifiersopt Type UNDERSCORE"); //$NON-NLS-1$
                }
                consumeTypePattern();
                break;
            ...
            case 350:
                if (DEBUG) {
                    System.out.println("RecordPattern ::= Modifiersopt ReferenceType PushLPAREN"); //$NON-NLS-1$
                }
            ...
            case 351:
                if (DEBUG) {
                    System.out.println("ComponentPatternListopt ::="); //$NON-NLS-1$
                }
            ...
            case 354:
                if (DEBUG) {
                    System.out.println("ComponentPatternList ::= ComponentPatternList COMMA..."); //$NON-NLS-1$
                }
                consumePatternList();
                break;
            ...
            case 357:
                if (DEBUG) {
                    System.out.println("UnnamedPattern ::= UNDERSCORE"); //$NON-NLS-1$
                }
            ...
            case 367:
                if (DEBUG) {
                    System.out.println("BlockStatements ::= BlockStatement"); //$NON-NLS-1$
                }
            ...
            case 425:
                if (DEBUG) {
                    System.out.println("PostExpressionInSwitchStatement ::="); //$NON-NLS-1$
                }
                consumePostExpressionInSwitch(true);
                break;

            case 426:
                if (DEBUG) {
                    System.out.println("PostExpressionInSwitchExpression ::="); //$NON-NLS-1$
                }
                consumePostExpressionInSwitch(false);
                break;

            case 427:
                if (DEBUG) {
                    System.out.println("PostExpressionInIf ::="); //$NON-NLS-1$
                }
            ...
            case 428:
                if (DEBUG) {
                    System.out.println("PostExpressionInWhile ::="); //$NON-NLS-1$
                }
            ...
            case 432:
                ...
                consumeSwitchStatementOrExpression(true);
                break;
            ...
            case 441:
                if (DEBUG) {
                    System.out.println("SwitchLabels ::= SwitchLabel COLON"); //$NON-NLS-1$
                }
                consumeSwitchLabels(false, false);
                break;

            case 442:
                if (DEBUG) {
                    System.out.println("SwitchLabels ::= SwitchLabels SwitchLabel COLON"); //$NON-NLS-1$
                }
                consumeSwitchLabels(true, false);
                break;

            case 443:
                if (DEBUG) {
                    System.out.println("PostCaseArrow ::="); //$NON-NLS-1$
                }
                consumeSwitchLabels(false, true);
                break;

            case 445:
                if (DEBUG) {
                    System.out.println("SwitchLabel ::= default"); //$NON-NLS-1$
                }
                consumeDefaultLabel();
                break;
            ...
            case 448:
                if (DEBUG) {
                    System.out.println("SwitchExpression ::= switch LPAREN Expression RPAREN..."); //$NON-NLS-1$
                }
                consumeSwitchStatementOrExpression(false);
                break;

            case 449:
                if (DEBUG) {
                    System.out.println("SwitchRule ::= SwitchLabel CaseArrow PostCaseArrow..."); //$NON-NLS-1$
                }
                consumeSwitchRule(SwitchRuleKind.EXPRESSION);
                break;
            ...
            case 450:
                ...
                consumeSwitchRule(SwitchRuleKind.BLOCK);
                break;
            ...
            case 451:
                ...
                consumeSwitchRule(SwitchRuleKind.THROW);
                break;

            case 453:
                if (DEBUG) {
                    System.out.println("CaseLabelElements ::= CaseLabelElements COMMA..."); //$NON-NLS-1$
                }
            ...
            case 454:
                if (DEBUG) {
                    System.out.println("CaseLabelElement ::= ConstantExpression"); //$NON-NLS-1$
                }
            ...
            case 455:
                if (DEBUG) {
                    System.out.println("CaseLabelElement ::= default"); //$NON-NLS-1$
                }
            ...
            case 456:
                if (DEBUG) {
                    System.out.println("CaseLabelElement ::= CaseLabelElementPattern"); //$NON-NLS-1$
                }
            ...
            case 457:
                if (DEBUG) {
                    System.out.println("CaseLabelElement ::= CaseLabelElementPattern Guard"); //$NON-NLS-1$
                }
                consumeCaseLabelElement(CaseLabelKind.CASE_PATTERN);
                break;
            ...
            case 459:
                if (DEBUG) {
                    System.out.println("Guard ::= RestrictedIdentifierWhen Expression"); //$NON-NLS-1$
                }
                consumeGuard();
                break;
            ...
            case 460:
                if (DEBUG) {
                    System.out.println("YieldStatement ::= RestrictedIdentifierYield Expression"); //$NON-NLS-1$
                }
            ...
            case 493:
                if (DEBUG) {
                    System.out.println("Resource ::= Name"); //$NON-NLS-1$
                }
            ...
            case 494:
                if (DEBUG) {
                    System.out.println("Resource ::= this"); //$NON-NLS-1$
                }
            ...
            case 495:
                if (DEBUG) {
                    System.out.println("Resource ::= FieldAccess"); //$NON-NLS-1$
                }
            ...
            case 514:
                if (DEBUG) {
                    System.out.println("QualifiedSuperReceiver ::= Name DOT super"); //$NON-NLS-1$
                }
            ...
            case 524:
                if (DEBUG) {
                    System.out.println("ReferenceExpressionTypeArgumentsAndTrunk0 ::=..."); //$NON-NLS-1$
                }
            ...
            case 526:
                if (DEBUG) {
                    System.out.println("ReferenceExpression ::= PrimitiveType Dims COLON_COLON"); //$NON-NLS-1$
                }
            ...
            case 527:
                if (DEBUG) {
                    System.out.println("ReferenceExpression ::= Name Dimsopt COLON_COLON..."); //$NON-NLS-1$
                }
            ...
            case 528:
                if (DEBUG) {
                    System.out.println("ReferenceExpression ::= Name BeginTypeArguments..."); //$NON-NLS-1$
                }
            ...
            case 529:
                if (DEBUG) {
                    System.out.println("ReferenceExpression ::= Primary COLON_COLON..."); //$NON-NLS-1$
                }
            ...
            case 530:
                if (DEBUG) {
                    System.out.println("ReferenceExpression ::= QualifiedSuperReceiver..."); //$NON-NLS-1$
                }
                consumeReferenceExpressionPrimaryForm();
                break;
            ...
            case 531:
                if (DEBUG) {
                    System.out.println("ReferenceExpression ::= super COLON_COLON..."); //$NON-NLS-1$
                }
            ...
            case 532:
                if (DEBUG) {
                    System.out.println("NonWildTypeArgumentsopt ::="); //$NON-NLS-1$
                }
            ...
            case 534:
                if (DEBUG) {
                    System.out.println("IdentifierOrNew ::= Identifier"); //$NON-NLS-1$
                }
            ...
            case 535:
                if (DEBUG) {
                    System.out.println("IdentifierOrNew ::= new"); //$NON-NLS-1$
                }
            ...
            case 536:
                if (DEBUG) {
                    System.out.println("LambdaExpression ::= LambdaParameters ARROW LambdaBody"); //$NON-NLS-1$
                }
            ...
            case 537:
                if (DEBUG) {
                    System.out.println("NestedLambda ::="); //$NON-NLS-1$
                }
                consumeNestedLambda();
                break;
            ...
            case 538:
                if (DEBUG) {
                    System.out.println("LambdaParameters ::= UNDERSCORE NestedLambda"); //$NON-NLS-1$
                }
            ...
            case 539:
                if (DEBUG) {
                    System.out.println("LambdaParameters ::= Identifier NestedLambda"); //$NON-NLS-1$
                }
                consumeTypeElidedLambdaParameter(false);
                break;
            ...
            case 545:
                if (DEBUG) {
                    System.out.println("TypeElidedFormalParameterList ::=..."); //$NON-NLS-1$
                }
                consumeSingleVariableDeclaratorList();
                break;
            ...
            case 546:
                if (DEBUG) {
                    System.out.println("TypeElidedFormalParameter ::= Modifiersopt Identifier"); //$NON-NLS-1$
                }
            ...
            case 547:
                if (DEBUG) {
                    System.out.println("TypeElidedFormalParameter ::= UNDERSCORE"); //$NON-NLS-1$
                }
            ...
            case 550:
                if (DEBUG) {
                    System.out.println("ElidedLeftBraceAndReturn ::="); //$NON-NLS-1$
                }
            ...
            case 553:
                if (DEBUG) {
                    System.out.println("ClassInstanceCreationExpression ::= new ClassType..."); //$NON-NLS-1$
                }
            ...
            case 558:
                if (DEBUG) {
                    System.out.println("EnterInstanceCreationArgumentList ::="); //$NON-NLS-1$
                }
            ...
            case 559:
                if (DEBUG) {
                    System.out.println("ClassInstanceCreationExpressionName ::= Name DOT new"); //$NON-NLS-1$
                }
            ...
            case 577:
                if (DEBUG) {
                    System.out.println("DimWithOrWithOutExpr ::= TypeAnnotationsopt LBRACKET..."); //$NON-NLS-1$
                }
            ...
            case 582:
                if (DEBUG) {
                    System.out.println("OneDimLoop ::= TypeAnnotations LBRACKET RBRACKET"); //$NON-NLS-1$
                }
            ...
            case 585:
                if (DEBUG) {
                    System.out.println("FieldAccess ::= QualifiedSuperReceiver DOT Identifier"); //$NON-NLS-1$
                }
                consumeFieldAccess(false);
                break;
            ...
            case 590:
                if (DEBUG) {
                    System.out.println("MethodInvocation ::= QualifiedSuperReceiver DOT..."); //$NON-NLS-1$
                }
                consumeMethodInvocationPrimary();
                break;
            ...
            case 619:
                if (DEBUG) {
                    System.out.println("CastExpression ::= BeginIntersectionCast PushLPAREN..."); //$NON-NLS-1$
                }
            ...
            case 620:
                if (DEBUG) {
                    System.out.println("CastExpression ::= PushLPAREN Name Dims..."); //$NON-NLS-1$
                }
            ...
            case 621:
                if (DEBUG) {
                    System.out.println("AdditionalBoundsListOpt ::="); //$NON-NLS-1$
                }
            ...
            case 628:
                if (DEBUG) {
                    System.out.println("InsideCastExpressionLL1WithBounds ::="); //$NON-NLS-1$
                }
            ...
            case 678:
                if (DEBUG) {
                    System.out.println("Expression ::= AssignmentExpression"); //$NON-NLS-1$
                }
                consumeExpression();
                break;
            ...
            case 704:
                if (DEBUG) {
                    System.out.println("PermittedTypesopt ::= RestrictedIdentifierpermits..."); //$NON-NLS-1$
                }
            ...
            case 746:
                if (DEBUG) {
                    System.out.println("SingleModuleImportDeclaration ::=..."); //$NON-NLS-1$
                }
                consumeImportDeclaration();
                break;
            ...
            case 747:
                if (DEBUG) {
                    System.out.println("SingleModuleImportDeclarationName ::= import module Name"); //$NON-NLS-1$
                }
            ...
            case 771:
                if (DEBUG) {
                    System.out.println("Wildcard ::= TypeAnnotationsopt QUESTION"); //$NON-NLS-1$
                }
                consumeWildcard();
                break;
            ...
            case 772:
                if (DEBUG) {
                    System.out.println("Wildcard ::= TypeAnnotationsopt QUESTION WildcardBounds"); //$NON-NLS-1$
                }
            ...
            case 775:
                if (DEBUG) {
                    System.out.println("Wildcard1 ::= TypeAnnotationsopt QUESTION GREATER"); //$NON-NLS-1$
                }
                consumeWildcard1();
                break;
            ...
            case 776:
                if (DEBUG) {
                    System.out.println("Wildcard1 ::= TypeAnnotationsopt QUESTION..."); //$NON-NLS-1$
                }
            ...
            case 779:
                if (DEBUG) {
                    System.out.println("Wildcard2 ::= TypeAnnotationsopt QUESTION RIGHT_SHIFT"); //$NON-NLS-1$
                }
                consumeWildcard2();
                break;
            ...
            case 780:
                if (DEBUG) {
                    System.out.println("Wildcard2 ::= TypeAnnotationsopt QUESTION..."); //$NON-NLS-1$
                }
            ...
            case 783:
                if (DEBUG) {
                    System.out.println("Wildcard3 ::= TypeAnnotationsopt QUESTION..."); //$NON-NLS-1$
                }
                consumeWildcard3();
                break;
            ...
            case 787:
                if (DEBUG) {
                    System.out.println("TypeParameterHeader ::= TypeAnnotationsopt Identifier"); //$NON-NLS-1$
                }
            ...
            case 846:
                if (DEBUG) {
                    System.out.println("InstanceofExpression_NotName ::= Name InstanceofRHS"); //$NON-NLS-1$
                }
            ...
            case 898:
                if (DEBUG) {
                    System.out.println("AnnotationName ::= AT UnannotatableName"); //$NON-NLS-1$
                }
                consumeAnnotationName();
                break;
            ...
            case 923:
                if (DEBUG) {
                    System.out.println("RecoveryMethodHeaderName ::= ModifiersWithDefault..."); //$NON-NLS-1$
                }
                consumeRecoveryMethodHeaderNameWithTypeParameters();
                break;
            ...
            case 924:
                if (DEBUG) {
                    System.out.println("RecoveryMethodHeaderName ::= ModifiersWithDefault Type"); //$NON-NLS-1$
                }
                consumeRecoveryMethodHeaderName();
                break;
...
// END_AUTOGENERATED_REGION_CONSUME_RULE
    ...
    protected void consumeExplicitThisParameter(boolean isQualified) {
        ...
        NameReference qualifyingNameReference = null;
        if (isQualified) {
            qualifyingNameReference = getUnspecifiedReference(false); // By construction the qualified name is
                                                                      // unannotated here, so we should not meddle with
                                                                      // the type annotation stack
        }
    ...
    protected void consumeNestedLambda() {
        // NestedLambda ::= $empty - we get here just after the type+parenthesis elided singleton parameter or just
        // before the '(' of the parameter list.
        consumeNestedType();
        this.nestedMethod[this.nestedType]++;
    ...
    protected void consumeLambdaHeader() {
        // LambdaHeader ::= LambdaParameters '->' Synthetic/fake production with a synthetic non-terminal. Body not seen
        // yet.
        ...
        if (this.currentElement != null) {
            this.lastCheckPoint = arrowPosition + 1; // we don't want the typed formal parameters to be processed by
                                                     // recovery.
    ...
    protected Argument typeElidedArgument() {
        ...
        Argument arg = new Argument(identifierName, namePositions, null, // elided type
            ClassFileConstants.AccDefault, true);
    ...
    protected void consumeTypeElidedLambdaParameter(boolean parenthesized) {
        ...
        if (parenthesized) { // The grammar is permissive enough to allow optional modifiers for the parenthesized
                             // version, they should be rejected if present.
    ...
    protected void consumeElidedLeftBraceAndReturn() {
        ...
        if (++this.valueLambdaNestDepth >= stackLength) {
            System.arraycopy(this.stateStackLengthStack, 0, this.stateStackLengthStack = new int[stackLength + 4], 0,
                stackLength);
        }
    ...
    protected void consumeExpression() {
         ...
         * Alert ! Sleight of hand - Part II: See if we are at the state with the item:
         * "ElidedLeftBraceAndReturn Expression .ElidedSemicolonAndRightBrace"
         * If so, push back the current token into the lexer stream, materialize the synthetic terminal marker symbol,
         * switch and continue.
         */
        if (this.valueLambdaNestDepth >= 0
            && this.stateStackLengthStack[this.valueLambdaNestDepth] == this.stateStackTop - 1) {
    ...
    protected void consumeReferenceExpressionTypeForm(boolean isPrimitive) { // actually Name or Type form.
        ...
        if (length > 0) {
            this.genericsPtr -= length;
            System.arraycopy(this.genericsStack, this.genericsPtr + 1, typeArguments = new TypeReference[length], 0,
                length);
        ...
        for (int i = this.identifierLengthStack[this.identifierLengthPtr], j = 0;
            i > 0 && this.typeAnnotationLengthPtr >= 0; --i, j++) {
        ...
        if (dimension > 0 || typeAnnotatedName) {
            ...
            referenceExpression.initialize(this.compilationUnit.compilationResult, getTypeReference(dimension),
                typeArguments, selector, sourceEnd);
        } else {
            referenceExpression.initialize(this.compilationUnit.compilationResult, getUnspecifiedReference(),
                typeArguments, selector, sourceEnd);
        }
    ...
    protected void consumeReferenceExpressionPrimaryForm() {
        ...
        this.expressionLengthPtr--;
        referenceExpression.initialize(this.compilationUnit.compilationResult, primary, typeArguments, selector,
            sourceEnd);
        consumeReferenceExpression(referenceExpression);
    }
    ...
    protected void consumeReferenceExpressionSuperForm() {
        ...
        referenceExpression.initialize(this.compilationUnit.compilationResult, superReference, typeArguments, selector,
            sourceEnd);
        consumeReferenceExpression(referenceExpression);
    }
    ...
    protected void consumeReferenceExpressionTypeArgumentsAndTrunk(boolean qualified) {
        ...
        // ReferenceExpressionTypeArgumentsAndTrunk ::= OnlyTypeArguments '.' ClassOrInterfaceType Dimsopt ==> qualified
        // == true
    ...
    protected void consumeReferenceExpressionGenericTypeForm() {
        ...
        // ReferenceExpression ::= Name BeginTypeArguments ReferenceExpressionTypeArgumentsAndTrunk '::'
        ...
        referenceExpression.initialize(this.compilationUnit.compilationResult, type, typeArguments, selector,
            sourceEnd);
    ...
    protected void consumeStatementYield() {
        ...
        if (this.expressionLengthStack[this.expressionLengthPtr--] != 0) {
            ...
            YieldStatement yieldStatement
                = new YieldStatement(expr, false, this.intStack[this.intPtr--], this.endStatementPosition);
    ...
    protected void consumeStatementContinueWithLabel() {
        ...
        pushOnAstStack(new ContinueStatement(this.identifierStack[this.identifierPtr--], this.intStack[this.intPtr--],
            this.endStatementPosition));
        this.identifierLengthPtr--;
    }
    ...
    protected void consumeStatementReturn() {
        ...
        if (this.expressionLengthStack[this.expressionLengthPtr--] != 0) {
            pushOnAstStack(new ReturnStatement(this.expressionStack[this.expressionPtr--], this.intStack[this.intPtr--],
                this.endStatementPosition));
        } else {
    ...
    protected void consumeSwitchStatementOrExpression(boolean isStmt) {
        ...
        // Expression and SwitchBlock are only non-epsilon productions. the block is inlined but a scope need to be
        // created if some declaration occurs.
        this.nestedType--;
    ...
    protected void consumeStatementSynchronized() {
        ...
        if (this.astLengthStack[this.astLengthPtr] == 0) {
            ...
            this.expressionLengthPtr--;
            this.astStack[++this.astPtr] = new SynchronizedStatement(this.expressionStack[this.expressionPtr--], null,
                this.intStack[this.intPtr--], this.endStatementPosition);
        } else {
            this.expressionLengthPtr--;
    ...
    protected void consumeStaticOnly() {
        ...
        // recovery
        if (this.currentElement != null) {
            this.recoveredStaticInitializerStart = this.intStack[this.intPtr]; // remember start position only for
                                                                               // static initializers
        }
    }

    private void consumeTextBlock() {
        problemReporter().validateJavaFeatureSupport(JavaFeature.TEXT_BLOCKS, this.scanner.startPosition,
            this.scanner.currentPosition - 1);
        boolean shouldRecordStringLiterals = this.recordStringLiterals
        ...
        TextBlock textBlock = new TextBlock(this.scanner.getCurrentTextBlock(), this.scanner.startPosition,
            this.scanner.currentPosition - 1,
            shouldRecordStringLiterals
    ...
    protected void consumeSwitchLabels(boolean shouldConcat, boolean isSwitchRule) {
        ...
        if ((length = this.expressionLengthStack[this.expressionLengthPtr--]) > 0) {
            this.expressionPtr -= length;
            System.arraycopy(this.expressionStack, this.expressionPtr + 1, labelExpressions = new Expression[length], 0,
                length);
    ...
    protected void consumeCaseLabelElement(CaseLabelKind kind) {
        ...
        if (pattern != null)
            problemReporter().validateJavaFeatureSupport(JavaFeature.PATTERN_MATCHING_IN_SWITCH, pattern.sourceStart,
                pattern.sourceEnd);
    }
    ...
    protected void consumeCaseLabelElements() {
        ...
        if (thisLabelIsPattern && lastLabelIsPattern) {
            ...
            // current pattern can't have alternatives, but getAlternatives() is useful to strip the guard (which will
            // be attached to the combined pattern below
            Pattern[] patterns = Stream
                .concat(Arrays.stream(lastPattern.getAlternatives()), Arrays.stream(thisPattern.getAlternatives()))
                .toArray(Pattern[]::new);
...
//	}
        ...
        switch (type) {
            ...
            case TokenNameRPAREN:
                ...
                this.rParenPos = this.scanner.currentPosition - 1; // position of the end of right parenthesis (in case
                                                                   // of unicode \u0029) lex00101
                break;
            ...
            case TokenNameAT308:
                ...
                pushOnIntStack(this.dimensions); // https://bugs.eclipse.org/bugs/show_bug.cgi?id=417660: Stack the
                                                 // dimensions, they get unstacked in consumeTypeAnnotation.
                this.dimensions = 0;
                //$FALL-THROUGH$
            case TokenNameAT:
                pushOnIntStack(this.scanner.startPosition);
    ...
    protected void consumeTypeHeaderNameWithTypeParameters() {
        ...
        int length = this.genericsLengthStack[this.genericsLengthPtr--];
        this.genericsPtr -= length;
        System.arraycopy(this.genericsStack, this.genericsPtr + 1, typeDecl.typeParameters = new TypeParameter[length],
            0, length);
    ...
    protected void consumeTypeParameterHeader() {
        ...
        if ((length = this.typeAnnotationLengthStack[this.typeAnnotationLengthPtr--]) != 0) {
            System.arraycopy(this.typeAnnotationStack, (this.typeAnnotationPtr -= length) + 1,
                typeParameter.annotations = new Annotation[length], 0, length);
    ...
    protected void consumeTypePattern() {
        ...
        problemReporter().validateJavaFeatureSupport(JavaFeature.PATTERN_MATCHING_IN_INSTANCEOF, type.sourceStart,
    ...
    protected void consumeZeroAdditionalBounds() {
        if (this.currentToken == TokenNameRPAREN)  // Signal zero additional bounds - do this only when the cast type is
                                                   // fully seen (i.e not in error path)
            pushOnGenericsLengthStack(0);          // Not all stacks are adjusted - this is not meant to be popped by
                                                   // getTypeReference
    }
    ...
    protected void consumeUnaryExpression(int op, boolean post) {
        ...
        if (leftHandSide instanceof Reference) {
            ...
            if (post) {
                this.expressionStack[this.expressionPtr]
                    = new PostfixExpression(leftHandSide, IntLiteral.One, op, this.endStatementPosition);
            } else {
                this.expressionStack[this.expressionPtr]
                    = new PrefixExpression(leftHandSide, IntLiteral.One, op, this.intStack[this.intPtr--]);
            }
        } else {
    ...
    protected TypeReference augmentTypeWithAdditionalDimensions(TypeReference typeReference, int additionalDimensions,
        Annotation[][] additionalAnnotations, boolean isVarargs) {
        if (this.parsingJava10Plus
            && typeReference instanceof SingleTypeReference singleTypeRef
            && CharOperation.equals(singleTypeRef.token, TypeConstants.VAR))
            problemReporter().varLocalCannotBeArray(singleTypeRef);
        return typeReference.augmentTypeWithAdditionalDimensions(additionalDimensions, additionalAnnotations,
            isVarargs);
    }
    ...
    public int flushCommentsDefinedPriorTo(int position) {
        ...
        if (validCount > 0) {
            int immediateCommentEnd = -this.scanner.commentStops[index + 1]; // non-javadoc comment end positions are
                                                                             // negative
        ...
        switch (nextCharacter) {
            case '\\':
                ...
                while (comment[index[0]] == 'u')
                    index[0]++;
                if (!(((c1 = ScannerHelper.getHexadecimalValue(comment[index[0]++])) > 15 || c1 < 0)
    ...
    protected void annotateTypeReference(Wildcard ref) {
        ...
        if ((length = this.typeAnnotationLengthStack[this.typeAnnotationLengthPtr--]) != 0) {
            ...
            System.arraycopy(this.typeAnnotationStack, (this.typeAnnotationPtr -= length) + 1,
                ref.annotations[0] = new Annotation[length], 0, length);
    ...
    protected final TypeReference getTypeReference(int dim) {
        ...
        if (this.parsingJava10Plus
            && typeRef instanceof ArrayTypeReference singleTypeRef
            && CharOperation.equals(singleTypeRef.token, TypeConstants.VAR)) {
        ...
        if (length < 0) { // flag for precompiled type reference on base types
            ...
            if (length != numberOfIdentifiers || this.genericsLengthStack[this.genericsLengthPtr] != 0) {
                ...
                if (dim == 0) {
                    ...
                    annotationsOnDimensions = getAnnotationsOnDimensions(dim);
                    ref = new ArrayTypeReference(this.identifierStack[this.identifierPtr], dim, annotationsOnDimensions,
                        this.identifierPositionStack[this.identifierPtr--]);
    ...
    protected TypeReference getTypeReferenceForGenericType(int dim, int identifierLength, int numberOfIdentifiers) {
        ...
        if (identifierLength == 1 && numberOfIdentifiers == 1) {
            ...
            ParameterizedSingleTypeReference parameterizedSingleTypeReference
                = new ParameterizedSingleTypeReference(this.identifierStack[this.identifierPtr], typeArguments, dim,
            ...
            /*
             * We used to eagerly mark the PSTR as constituting diamond usage if we encountered <>, but that is too
             * eager and
             * complicates error handling by making it hard to distinguish legitimate use cases from ill formed ones. We
             * are
            ...
            ParameterizedQualifiedTypeReference parameterizedQualifiedTypeReference
                = new ParameterizedQualifiedTypeReference(tokens, typeArguments, dim, annotationsOnDimensions,
                    positions);
            if (dim != 0) {
                parameterizedQualifiedTypeReference.sourceEnd = this.endStatementPosition;
            }
            /*
             * We used to eagerly mark the PQTR as constituting diamond usage if we encountered <>, but that is too
             * eager and
             * complicates error handling by making it hard to distinguish legitimate use cases from ill formed ones. We
             * are
             * more discriminating now and tag a type as being diamond only where <> can legally occur.
        ...
        if (this.recoveredTypes != null
            ...
            if (!isAnonymous) {
                ((RecoveryScanner) this.scanner)
                    .setPendingTokens(new TerminalToken[] { TokenNameSEMICOLON, TokenNamebreak });
            } else {
                ((RecoveryScanner) this.scanner)
                    .setPendingTokens(new TerminalToken[] { TokenNameIdentifier, TokenNameEQUAL, TokenNameIdentifier });
            }
    ...
    protected void markEnclosingMemberWithLocalOrFunctionalType(LocalTypeKind context) {
        for (int i = this.astPtr; i >= 0; i--) {
            ...
            if (node instanceof AbstractMethodDeclaration
                ...
                || (node instanceof TypeDeclaration type // mark type for now: all initializers will be marked when
                                                         // added to this type
    ...
    public boolean atConflictScenario(TerminalToken token) {
        ...
        /*
         * Answer true if the parser is at a configuration where the scanner must look ahead and help disambiguate
         * between (a) '<' as an operator and '<' as the
         * start of <type argument> and (b) the use of '(' in '(' expression ')' and '( type ')' and '(' lambda formal
         * parameters ')'. (c) whether the token @
         * begins a Java SE5 style declaration annotation or if it begins a SE8 style type annotation. When requested
         * thus, the scanner helps by fabricating
         * synthetic tokens and injecting them into the stream ahead of the tokens that trigger conflicts in the absence
         * of these artificial tokens. These
         * manufactured token help transform the grammar into LALR(1) by splitting the states so that they have
         * unambigious prefixes.
         * 
         * We do this by claiming to the automaton that the next token seen is the (suitable) synthetic token and
         * observing the response of the state machine.
         * Error signals we are NOT at a conflict site, while shift or shift/reduce signals that we are. Accept is
         * impossible, while there may be intermediate
         * reductions that are called for -- It is axiomatic of the push down automaton that corresponds to the LALR
         * grammar that it will never shift on invalid
         * input.
         * 
         * Obviously, the dry runs should not alter the parser state in any way or otherwise cause side effects. Proof
         * by argument that this is the case:
         * 
         * - The only pieces of state needed to answer the question are: this.stack, this.stateStackTop and the last
         * action variable `act`. None of the various
         ...
         * - this.stateStackTop and the last action variable `act` of the automaton are readily cloned, these being
         * primitives and changes are to the replicas.
         * - We never remove elements from the state stack here (or elsewhere for that matter). Pops are implemented by
         * mere adjustments of the stack pointer.
         * - During this algorithm, either the stack pointer monotonically decreases or stays fixed. (The only way for
         * the stack pointer to increase would call
         * for a shift or a shift/reduce at which point the algorithm is ready to terminate already.) This means that we
         * don't have to replicate the stack.
            ...
            ProcessTerminals: for (;;) {
                ...
                if (act == ERROR_ACTION || this.restartRecovery()) {
                    ...
                    switch (resumeOnSyntaxError()) {
                        ...
                        case RESTART:
                            if (act == ERROR_ACTION && previousToken != TokenNameNotAToken)
                ...
                if (act <= NUM_RULES) {
                    ...
                    if (DEBUG_AUTOMATON) {
                        System.out
                            .print("Shift/Reduce - (" + name[terminal_index[this.currentToken.tokenNumber()]] + ") ");  //$NON-NLS-1$ //$NON-NLS-2$
                    }
                    ...
                    if (act < ACCEPT_ACTION) { /* shift */
                        ...
                        if (DEBUG_AUTOMATON) {
                            System.out.println(
                                "Shift        - (" + name[terminal_index[this.currentToken.tokenNumber()]] + ")");  //$NON-NLS-1$ //$NON-NLS-2$
                        }
                        continue ProcessTerminals;
                    }
                    break ProcessTerminals;
...
// A P I
    ...
    public CompilationUnitDeclaration parse(ICompilationUnit sourceUnit, CompilationResult compilationResult, int start,
        int end) {
        // parses a compilation unit and manages error handling (even bugs....)
        ...
        try {
            ...
            if (this.problemReporter != null
                ...
                try {
                    ...
                    if (requestedVersion.compareTo(latestVersion) > 0) {
                        this.problemReporter.tooRecentJavaVersion(requestedVersion.toString(),
            ...
            try {
                contents
                    = this.readManager != null ? this.readManager.getContents(sourceUnit) : sourceUnit.getContents();
...
// A P I
    public void parse(MethodDeclaration md, CompilationUnitDeclaration unit) {
        ...
        if (this.astLengthPtr > -1 && (length = this.astLengthStack[this.astLengthPtr--]) != 0) {
            if (this.options.ignoreMethodBodies) {
                ...
                System.arraycopy(this.astStack, (this.astPtr -= length) + 1, md.statements = new Statement[length], 0,
                    length);
            }
        } else {
    ...
    public Expression parseLambdaExpression(char[] source, int offset, int length, CompilationUnitDeclaration unit,
    ...
    public char[][] parsePackageDeclaration(char[] source, CompilationResult result) {
        ...
        this.referenceContext
            = this.compilationUnit = new CompilationUnitDeclaration(problemReporter(), result, source.length);
        this.scanner.setSource(source);
        try {
            parse();
        } catch (AbortCompilation ex) {
    ...
    public Expression parseReferenceExpression(char[] source, int offset, int length, CompilationUnitDeclaration unit,
        boolean recordLineSeparators) {
        this.reparsingFunctionalExpression = true;
        return parseExpression(source, offset, length, unit, recordLineSeparators);
    }
    ...
    protected void pushIdentifier(char[] identifier, long position) {
        ...
        if (++this.identifierPtr >= stackLength) {
            System.arraycopy(this.identifierStack, 0, this.identifierStack = new char[stackLength + 20][], 0,
                stackLength);
            System.arraycopy(this.identifierPositionStack, 0, this.identifierPositionStack = new long[stackLength + 20],
                0, stackLength);
        }
        ...
        if (++this.identifierLengthPtr >= stackLength) {
            System.arraycopy(this.identifierLengthStack, 0, this.identifierLengthStack = new int[stackLength + 10], 0,
                stackLength);
        }
    ...
    protected void pushIdentifier() {
        ...
        pushIdentifier(this.scanner.getCurrentIdentifierSource(),
    ...
    protected void pushOnAstStack(ASTNode node) {
        ...
        if (++this.astLengthPtr >= stackLength) {
            System.arraycopy(this.astLengthStack, 0, this.astLengthStack = new int[stackLength + AstStackIncrement], 0,
                stackLength);
        }
        this.astLengthStack[this.astLengthPtr] = 1;
    }
    ...
    protected void pushOnTypeAnnotationStack(Annotation annotation) {
        ...
        if (++this.typeAnnotationPtr >= stackLength) {
            System.arraycopy(this.typeAnnotationStack, 0,
                this.typeAnnotationStack = new Annotation[stackLength + TypeAnnotationStackIncrement], 0, stackLength);
        }
        ...
        if (++this.typeAnnotationLengthPtr >= stackLength) {
            System.arraycopy(this.typeAnnotationLengthStack, 0,
                this.typeAnnotationLengthStack = new int[stackLength + TypeAnnotationStackIncrement], 0, stackLength);
        }
    ...
    protected void pushOnGenericsIdentifiersLengthStack(int pos) {
        ...
        if (++this.genericsIdentifiersLengthPtr >= stackLength) {
            System.arraycopy(this.genericsIdentifiersLengthStack, 0,
                this.genericsIdentifiersLengthStack = new int[stackLength + GenericsStackIncrement], 0, stackLength);
        }
...
// A P I
    protected void reportSyntaxErrors(boolean isDietParse, TerminalToken oldFirstToken) {
        ...
        int end
            = this.scanner.eofPosition == Integer.MAX_VALUE ? this.scanner.eofPosition : this.scanner.eofPosition - 1;
        if (isDietParse) {
            TypeDeclaration[] types = this.compilationUnit.types;
            int[][] intervalToSkip
                = com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.diagnose.RangeUtil
                        ...
                        if (fields[j]instanceof Initializer initializer) {
                            if ((initializer.bits & ASTNode.ErrorInSignature) != 0) {
                                DiagnoseParser diagnoseParser = new DiagnoseParser(this, TokenNameRIGHT_SHIFT,
                                    initializer.declarationSourceStart, initializer.declarationSourceEnd, this.options);
                                diagnoseParser.diagnoseParse(this.options.performStatementsRecovery);
                            }
                        }
                    }
    ...
    public void copyState(Parser from) {
        ...
        System.arraycopy(parser.identifierLengthStack, 0,
            this.identifierLengthStack = new int[length = parser.identifierLengthStack.length], 0, length);
        System.arraycopy(parser.identifierPositionStack, 0,
            this.identifierPositionStack = new long[length = parser.identifierPositionStack.length], 0, length);
        System.arraycopy(parser.astStack, 0, this.astStack = new ASTNode[length = parser.astStack.length], 0, length);
        System.arraycopy(parser.astLengthStack, 0, this.astLengthStack = new int[length = parser.astLengthStack.length],
            0, length);
        System.arraycopy(parser.expressionStack, 0,
            this.expressionStack = new Expression[length = parser.expressionStack.length], 0, length);
        System.arraycopy(parser.expressionLengthStack, 0,
            this.expressionLengthStack = new int[length = parser.expressionLengthStack.length], 0, length);
        System.arraycopy(parser.genericsStack, 0,
            this.genericsStack = new ASTNode[length = parser.genericsStack.length], 0, length);
        System.arraycopy(parser.genericsLengthStack, 0,
            this.genericsLengthStack = new int[length = parser.genericsLengthStack.length], 0, length);
        System.arraycopy(parser.genericsIdentifiersLengthStack, 0,
            this.genericsIdentifiersLengthStack = new int[length = parser.genericsIdentifiersLengthStack.length], 0,
            length);
        System.arraycopy(parser.typeAnnotationStack, 0,
            this.typeAnnotationStack = new Annotation[length = parser.typeAnnotationStack.length], 0, length);
        System.arraycopy(parser.typeAnnotationLengthStack, 0,
            this.typeAnnotationLengthStack = new int[length = parser.typeAnnotationLengthStack.length], 0, length);
        System.arraycopy(parser.intStack, 0, this.intStack = new int[length = parser.intStack.length], 0, length);
        System.arraycopy(parser.nestedMethod, 0, this.nestedMethod = new int[length = parser.nestedMethod.length], 0,
            length);
        System.arraycopy(parser.realBlockStack, 0, this.realBlockStack = new int[length = parser.realBlockStack.length],
            0, length);
        System.arraycopy(parser.stateStackLengthStack, 0,
            this.stateStackLengthStack = new int[length = parser.stateStackLengthStack.length], 0, length);
        System.arraycopy(parser.variablesCounter, 0,
            this.variablesCounter = new int[length = parser.variablesCounter.length], 0, length);
        System.arraycopy(parser.stack, 0, this.stack = new int[length = parser.stack.length], 0, length);
        System.arraycopy(parser.stack, 0, this.stack = new int[length = parser.stack.length], 0, length);
        System.arraycopy(parser.stack, 0, this.stack = new int[length = parser.stack.length], 0, length);
        ...
        // Parser.resetStacks is not clearing the modifiers, but AssistParser.resumeAfterRecovery is - why ? (the former
        // doesn't)
    ...
    public boolean automatonWillShift(TerminalToken token, int lastAction) {
        ...
        int stackTopState = this.stack[stackTop]; // single cell non write through "alternate stack" - the automaton's
                                                  // stack pointer either stays fixed during this manoeuvre or
                                                  // monotonically decreases.
        int highWaterMark = stackTop;
        // A rotated version of the automaton - cf. parse()'s for(;;)
        if (lastAction <= NUM_RULES) { // in recovery mode, we could take a detour to here, with a pending reduce
                                       // action.
            stackTop--;
