...
public abstract class ClassFileStruct {
    ...
    protected long i8At(byte[] reference, int relativeOffset, int structOffset) {
        int position = relativeOffset + structOffset;
        return (((long) (reference[position++] & 0xFF)) << 56) + (((long) (reference[position++] & 0xFF)) << 48)
            + (((long) (reference[position++] & 0xFF)) << 40) + (((long) (reference[position++] & 0xFF)) << 32)
            + (((long) (reference[position++] & 0xFF)) << 24) + (((long) (reference[position++] & 0xFF)) << 16)
            + (((long) (reference[position++] & 0xFF)) << 8) + (reference[position++] & 0xFF);
    }
    ...
    /**
     * Returns an 32 bit integer created of 4 bytes. The return value should be treated as an <strong>unsigned</strong>
     * integer! i.e. if the caller does math with
     * the result he should use the unsigned methods from Integer like {@link Integer#toUnsignedLong(int)} or
     * {@link Integer#compareUnsigned(int, int)}. Note that addition with the + operator is safe to use.
     * 
