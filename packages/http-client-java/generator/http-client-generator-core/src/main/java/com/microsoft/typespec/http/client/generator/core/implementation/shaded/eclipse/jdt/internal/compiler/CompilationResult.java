...
public class CompilationResult {
    ...
    public boolean hasInconsistentToplevelHierarchies = false; // record the fact some toplevel types have inconsistent
                                                               // hierarchies
    public boolean hasSyntaxError = false;
    public char[][] packageName;
    public boolean checkSecondaryTypes = false; // check for secondary types which were created after the initial
                                                // buildTypeBindings call
    ...
    public CompilationResult(ICompilationUnit compilationUnit, int unitIndex, int totalUnitsKnown,
    ...
    public CategorizedProblem[] getProblems() {
        ...
        if (this.problems != null) {
            if (this.problemCount != this.problems.length) {
                System.arraycopy(this.problems, 0, (this.problems = new CategorizedProblem[this.problemCount]), 0,
                    this.problemCount);
            }
    ...
    public CategorizedProblem[] getTasks() {
        ...
        if (this.tasks != null) {
            ...
            if (this.taskCount != this.tasks.length) {
                System.arraycopy(this.tasks, 0, (this.tasks = new CategorizedProblem[this.taskCount]), 0,
                    this.taskCount);
            }
            // Stable sort problems per source positions.
    ...
    public void record(CategorizedProblem newProblem, ReferenceContext referenceContext, boolean mandatoryError) {
        ...
        if (this.problemCount == 0) {
            ...
            System.arraycopy(this.problems, 0, (this.problems = new CategorizedProblem[this.problemCount * 2]), 0,
                this.problemCount);
        }
    ...
    private void recordTask(CategorizedProblem newProblem) {
        if (this.taskCount == 0) {
            ...
            System.arraycopy(this.tasks, 0, (this.tasks = new CategorizedProblem[this.taskCount * 2]), 0,
                this.taskCount);
        }
