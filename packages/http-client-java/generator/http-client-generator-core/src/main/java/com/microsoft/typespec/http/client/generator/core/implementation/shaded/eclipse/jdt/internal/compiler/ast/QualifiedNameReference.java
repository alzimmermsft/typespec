...
public class QualifiedNameReference extends NameReference {
    ...
    int[] otherDepths;
    public int indexOfFirstFieldBinding; 	// points into tokens & sourcePositions for the first token that corresponds
                                         	// to first FieldBinding
                                         	// *** the index is 1-based ***
                                         	// during BlockScope#getBinding(..) it will walk through positions until it
    ...
    public FlowInfo analyseAssignment(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo,
            ...
            case Binding.FIELD: // reading a field
                ...
                if (lastFieldBinding.isBlankFinal()
                    ...
                    FlowInfo fieldInits = flowContext
                        .getInitsForFinalBlankInitializationCheck(lastFieldBinding.declaringClass.original(), flowInfo);
        ...
        if (lastFieldBinding.isFinal()) {
            ...
            if (otherBindingsCount == 0
                ...
                if (otherBindingsCount == 0 && currentScope.allowBlankFinalFieldAssignment(lastFieldBinding)) { // pretend
                                                                                                                // it
                                                                                                                // got
                                                                                                                // assigned
                    flowInfo.markAsDefinitelyAssigned(lastFieldBinding);
            ...
            if (lastFieldBinding != null) {
                ...
                if (fieldConstant != Constant.NotAConstant) {
                    ...
                    boolean isFirst = lastFieldBinding == this.binding
                        && (this.indexOfFirstFieldBinding == 1
                            || TypeBinding.equalsEquals(lastFieldBinding.declaringClass,
                                currentScope.enclosingReceiverType()))
                        && this.otherBindings == null; // could be dup: next.next.next
                    TypeBinding requiredGenericCast
                        = getGenericCast(this.otherBindings == null ? 0 : this.otherBindings.length);
                    if (valueRequired
                        || (!isFirst)
    ...
    public void generateCompoundAssignment(BlockScope currentScope, CodeStream codeStream, Expression expression,
        ...
        TypeBinding constantPoolDeclaringClass
            = CodeStream.getConstantPoolDeclaringClass(currentScope, lastFieldBinding, getFinalReceiverType(), isFirst);
        SyntheticMethodBinding accessor = this.syntheticReadAccessors == null
            ? null
            : this.syntheticReadAccessors[this.syntheticReadAccessors.length - 1];
                    ...
                    if (fieldConstant != Constant.NotAConstant) {
                        ...
                        if (needValue || (i > 0) || lastGenericCast != null) {
                            MethodBinding accessor
                                = this.syntheticReadAccessors == null ? null : this.syntheticReadAccessors[i];
                            if (accessor == null) {
                                TypeBinding constantPoolDeclaringClass
                                    = CodeStream.getConstantPoolDeclaringClass(currentScope, lastFieldBinding,
                                        lastReceiverType, i == 0 && this.indexOfFirstFieldBinding == 1);
                                if (lastFieldBinding.isStatic()) {
                                    codeStream.fieldAccess(Opcodes.OPC_getstatic, lastFieldBinding,
                                        constantPoolDeclaringClass);
                                } else {
                            ...
                            if (lastFieldBinding == initialFieldBinding) {
                                if (lastFieldBinding.isStatic()) {
                                    // if no valueRequired, still need possible side-effects of <clinit> invocation, if
                                    // field belongs to different class
                                    if (TypeBinding.notEquals(initialFieldBinding.declaringClass,
                                        this.actualReceiverType.erasure())) {
                                        MethodBinding accessor = this.syntheticReadAccessors == null
                                            ? null
                        ...
                        if ((positionsLength - otherBindingsCount + i - 1) >= 0) {
                            int fieldPosition
                                = (int) (this.sourcePositions[positionsLength - otherBindingsCount + i - 1] >>> 32);
    ...
    protected TypeBinding getFinalReceiverType() {
        ...
        switch (otherBindingsCount) {
            ...
            default:
                ...
                return previousGenericCast != null
                    ? previousGenericCast
...
// get the matching generic cast
    ...
    public TypeBinding getOtherFieldBindings(BlockScope scope) {
        // At this point restrictiveFlag may ONLY have two potential value : FIELD LOCAL (i.e cast <<(VariableBinding)
        // binding>> is valid)
        ...
        while (index < length) {
            ...
            field = scope.getField(
                type.capture(scope, (int) (this.sourcePositions[index] >>> 32), (int) this.sourcePositions[index]),
                token, this);
            ...
            if (field.isValidBinding()) {
                ...
                if (previousField != null) {
                    ...
                    if (TypeBinding.notEquals(fieldReceiverType, oldReceiverType)
                        || originalBinding.type.leafComponentType().isTypeVariable()) { // record need for explicit cast
                                                                                        // at codegen
                        setGenericCast(place, originalBinding.type.genericCast(fieldReceiverType)); // type cannot be
                                                                                                    // base-type even in
                                                                                                    // boxing case
                    }
                }
    ...
    public void manageSyntheticAccessIfNecessary(BlockScope currentScope, FieldBinding fieldBinding, int index,
        ...
        if ((flowInfo.tagBits & FlowInfo.UNREACHABLE_OR_DEAD) != 0)
            return;
        // index == 0 denotes the first fieldBinding, index > 0 denotes one of the 'otherBindings', index < 0 denotes a
        // write access (to last binding)
        ...
        if (fieldBinding.isPrivate()) { // private access
            ...
            if (!currentScope.enclosingSourceType().isNestmateOf(declaringClass)
                ...
                currentScope.problemReporter()
                    .needToEmulateFieldAccess(codegenField, this, index >= 0 /* read-access? */);
                return;
            }
                    ...
                    if (this.binding instanceof FieldBinding) {
                        ...
                        if (scope.kind != Scope.MODULE_SCOPE) {
                            if ((this.indexOfFirstFieldBinding == 1
                                ...
                                || (!fieldBinding.isFinal() && declaringClass.isEnum())) // enum constants are checked
                                                                                         // even when qualified
                                && TypeBinding.equalsEquals(sourceType, declaringClass)
                                && methodScope.lastVisibleFieldID >= 0
                                ...
                                if (methodScope.insideTypeAnnotation
                                    && fieldBinding.id == methodScope.lastVisibleFieldID) {
                                    // false alarm, location is NOT a field initializer but the value in a
                                    // memberValuePair
                                } else {
                                    scope.problemReporter()
                                        .forwardReference(this, this.indexOfFirstFieldBinding - 1, fieldBinding);
                                }
                            }
                        }
                        ...
                        if (fieldBinding.isStatic()) {
                            ...
                            if (declaringClass.isEnum() && scope.kind != Scope.MODULE_SCOPE) {
                                if ((TypeBinding.equalsEquals(sourceType, declaringClass)
                                    || TypeBinding.equalsEquals(sourceType.superclass, declaringClass)) // enum constant
                                                                                                        // body
                            ...
                            if (scope.isInsideEarlyConstructionContext(fieldBinding.declaringClass, false))
                                scope.problemReporter()
                                    .fieldReadInEarlyConstructionContext(this.tokens[0], this.sourceStart,
                                        (int) (this.sourcePositions[0] >>> 32));
                            if (this.indexOfFirstFieldBinding == 1) {
                            ...
                            if (this.indexOfFirstFieldBinding > 1  // accessing to a field using a type as "receiver" is
                                                                   // allowed only with static field
