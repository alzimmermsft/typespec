...
public interface ExtraCompilerModifiers { // modifier constant
    ...
    final int AccDeprecatedImplicitly = ASTNode.Bit22; // record whether deprecated itself or contained by a deprecated
                                                       // type
    ...
    final int AccLocallyUsed = ASTNode.Bit28; // used to diagnose unused (a) private/local members or (b) members of
                                              // private classes
                                              // generally set when actual usage has been detected
                                              // or, (b) when member of a private class is exposed via a non-private
                                              // subclass
                                              // see https://bugs.eclipse.org/bugs/show_bug.cgi?id=328281
    final int AccVisibilityMASK
    ...
    final int AccImplementing = ASTNode.Bit30; // record fact a method implements another one (it is concrete and
                                               // overrides an abstract one)
    final int AccImplicitlyDeclared = ASTNode.Bit30; // used for implicitly declared classes
    final int AccGenericSignature = ASTNode.Bit31; // record fact a type/method/field involves generics in its signature
                                                   // (and need special signature attr)
