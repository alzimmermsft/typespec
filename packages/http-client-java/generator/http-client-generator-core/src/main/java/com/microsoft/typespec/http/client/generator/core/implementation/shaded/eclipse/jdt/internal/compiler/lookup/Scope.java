...
public abstract class Scope {
    ...
    public static TypeBinding convertEliminatingTypeVariables(TypeBinding originalType, ReferenceBinding genericType,
        int rank, Set<TypeBinding> eliminatedVariables) {
        if ((originalType.tagBits & TagBits.HasTypeVariable) != 0) {
            switch (originalType.kind()) {
                ...
                case Binding.PARAMETERIZED_TYPE:
                    ...
                    for (int i = 0, length = originalArguments == null ? 0 : originalArguments.length; i < length;
                        i++) {
                    ...
                    if (TypeBinding.notEquals(originalEnclosing, substitutedEnclosing)
                        || originalArguments != substitutedArguments) {
                ...
                case Binding.TYPE_PARAMETER:
                    ...
                    TypeBinding substitutedUpperBound
                        = convertEliminatingTypeVariables(originalUpperBound, genericType, rank, eliminatedVariables);
                ...
                case Binding.GENERIC_TYPE:
                    ...
                    for (int i = 0, length = originalArguments == null ? 0 : originalArguments.length; i < length;
                        i++) {
                        TypeBinding originalArgument = originalArguments[i];
                        TypeBinding substitutedArgument
                            = convertEliminatingTypeVariables(originalArgument, currentType, i, eliminatedVariables);
                        if (TypeBinding.notEquals(substitutedArgument, originalArgument)) {
                            if (substitutedArguments == originalArguments) {
                                System.arraycopy(originalArguments, 0, substitutedArguments = new TypeBinding[length],
                                    0, i);
                ...
                case Binding.WILDCARD_TYPE:
                    ...
                    if (originalBound != null) {
                        substitutedBound
                            = convertEliminatingTypeVariables(originalBound, genericType, rank, eliminatedVariables);
                ...
                case Binding.INTERSECTION_TYPE:
                    ...
                    for (int i = 0, length = originalOtherBounds == null ? 0 : originalOtherBounds.length; i < length;
                        i++) {
                    ...
                    if (TypeBinding.notEquals(substitutedBound, originalBound)
                        || substitutedOtherBounds != originalOtherBounds) {
    ...
    static <T extends TypeBinding> T[] filterValidTypes(T[] allTypes, Function<Integer, T[]> ctor) {
        ...
        if (count == 0 && allTypes.length > 0)
            return Arrays.copyOf(allTypes, 1); // if all are invalid pick the first as a placeholder to prevent general
                                               // glb failure
    ...
    private static boolean isTypeVariableClassConflict(TypeVariableBinding typeVariable, boolean fromInferenceVariable,
        ...
        public TypeBinding substitute(Substitution substitution, TypeBinding originalType) {
            ...
            switch (originalType.kind()) {
                ...
                case Binding.ARRAY_TYPE:
                    ArrayBinding originalArrayType = (ArrayBinding) originalType;
                    TypeBinding originalLeafComponentType = originalArrayType.leafComponentType;
                    TypeBinding substitute = substitute(substitution, originalLeafComponentType); // substitute could
                                                                                                  // itself be array
                                                                                                  // type,
                                                                                                  // TODO(Srikanth):
                                                                                                  // need a test case.
                    if (substitute != originalLeafComponentType) { //$IDENTITY-COMPARISON$
                        return originalArrayType.environment.createArrayType(substitute.leafComponentType(),
                            substitute.dimensions() + originalType.dimensions(), originalType.getTypeAnnotations());
                    }
                    break;
                    ...
                    if (wildcard.boundKind != Wildcard.UNBOUND) {
                        ...
                        if (substitutedBound != originalBound || originalOtherBounds != substitutedOtherBounds) { //$IDENTITY-COMPARISON$
                            if (originalOtherBounds != null) {
                                /*
                                 * https://bugs.eclipse.org/bugs/show_bug.cgi?id=347145: the constituent intersecting
                                 * types have changed
                                 * in the last round of substitution. Reevaluate the composite intersection type, as
                                 * there is a possibility
                                 * of the intersection collapsing into one of the constituents, the other being fully
                                 * subsumed.
                                 */
                                ...
                                if (glb != null && glb != bounds) {
                                    ...
                                    if (glb.length == 1) {
                                        ...
                                        System.arraycopy(glb, 1,
                                            substitutedOtherBounds = new TypeBinding[glb.length - 1], 0,
                                            glb.length - 1);
                                    }
                                }
                            }
                    ...
                    if (originalEnclosing != null) {
                        ...
                        if (isMemberTypeOfRaw(originalType, substitutedEnclosing))
                            return substitution.environment()
                                .createRawType(originalReferenceType, substitutedEnclosing,
                                    originalType.getTypeAnnotations());
                    }
                        ...
                        return substitution.isRawSubstitution()
                            ? substitution.environment()
                                .createRawType(originalReferenceType, substitutedEnclosing,
                                    originalType.getTypeAnnotations())
                            : substitution.environment()
                                .createParameterizedType(originalReferenceType, null, substitutedEnclosing,
                                    originalType.getTypeAnnotations());
                    }
                    break;
                ...
                case Binding.GENERIC_TYPE:
                    ...
                    substitutedEnclosing = originalEnclosing;
                    if (originalEnclosing != null) {
                        substitutedEnclosing = (ReferenceBinding) (originalType.isStatic()
                    ...
                    // potentially treat as if parameterized with its type variables (non generic type gets 'null'
                    // arguments)
                    if (TypeBinding.equalsEquals(this.staticContext, originalType))
                        return originalType; // substitution happens on a static member of the generic type, where its
                                             // type variables are not available
                    originalArguments = originalReferenceType.typeVariables();
                    substitutedArguments = substitute(substitution, originalArguments);
                    return substitution.environment()
                        .createParameterizedType(originalReferenceType, substitutedArguments, substitutedEnclosing,
                            originalType.getTypeAnnotations());
            }
            return originalType;
        }
    ...
    protected final MethodBinding computeCompatibleMethod(MethodBinding method, TypeBinding[] arguments,
        ...
        if (level > NOT_COMPATIBLE) {
            ...
            return new ProblemMethodBinding(method, method.selector, method.parameters,
                ProblemReasons.MissingTypeInSignature);
        }
        ...
        // 18.5.1 ignores arguments not pertinent to applicability. When these are taken into consideration method could
        // fail applicability (e.g, lambda shape/arity mismatch ...)
        if (method instanceof PolyParameterizedGenericMethodBinding) // Not reached, but left in for now.
            return new ProblemMethodBinding(method, method.selector, method.parameters,
                ProblemReasons.InferredApplicableMethodInapplicable);
        return null; // incompatible
    }
    ...
    protected boolean connectTypeVariables(TypeParameter[] typeParameters, boolean checkForErasedCandidateCollisions) {
        ...
        nextVariable: for (int i = 0; i < paramLength; i++) {
            ...
            if (superType == null) {
                ...
                firstBound: {
                        ...
                        default:
                            if (((ReferenceBinding) superType).isFinal()) {
                                if (!environment().usesNullTypeAnnotations()
                                    || (superType.tagBits & TagBits.AnnotationNullable) == 0) {
                    ...
                    typeVariable.tagBits
                        |= superType.tagBits & (TagBits.ContainsNestedTypeReferences | TagBits.HasMissingType);
                ...
                nextBound: for (int j = 0, boundLength = boundRefs.length; j < boundLength; j++) {
                    ...
                    if (superType == null) {
                        ...
                        System.arraycopy(typeVariable.superInterfaces, 0,
                            typeVariable.setSuperInterfaces(new ReferenceBinding[size + 1]), 0, size);
        ...
        if (declaresNullTypeAnnotation)
            for (int i = 0; i < paramLength; i++)
                typeParameters[i].binding.updateTagBits(); // <T extends List<U>, @NonNull U> --> tag T as having null
                                                           // type annotations
        return noProblems;
    }
    ...
    public TypeVariableBinding[] createTypeVariables(TypeParameter[] typeParameters, Binding declaringElement) {
        // https://bugs.eclipse.org/bugs/show_bug.cgi?id=324850, If they exist at all, process type parameters
        // irrespective of source level.
        if (typeParameters == null || typeParameters.length == 0)
        ...
        for (int i = 0; i < length; i++) {
            TypeParameter typeParameter = typeParameters[i];
            TypeVariableBinding parameterBinding
                = new TypeVariableBinding(typeParameter.name, declaringElement, i, environment());
            ...
            // detect duplicates, but keep each variable to reduce secondary errors with instantiating this generic type
            // (assume number of variables is correct)
...
//				}
    ...
    /*
     * Abstract method lookup (since maybe missing default abstract methods). "Default abstract methods" are methods
     * that used to be emitted into
     * abstract classes for unimplemented interface methods at JDK 1.1 time frame. See
     * SourceTypeBinding.addDefaultAbstractMethods()
     * See also https://bugs.eclipse.org/bugs/show_bug.cgi?id=174588 for details of problem addressed here. Problem was
     * in the method call in the
     * abstract* class. Unless the interface methods are looked up, we will emit code that results in infinite
     * recursion.
     */
    protected MethodBinding findDefaultAbstractMethod(ReferenceBinding receiverType, char[] selector,
        ...
        ObjectVector found, MethodBinding[] concreteMatches) {
        ...
        if (foundSize > startFoundSize) {
            ...
            next: for (int i = startFoundSize; i < foundSize; i++) {
                ...
                if (compatibleMethod != null) {
                    if (compatibleMethod.isValidBinding()) {
                        ...
                        for (int j = 0; j < startFoundSize; j++) {
                            ...
                            if (classMethod != null
                                && methodVerifier.areMethodsCompatible(classMethod, compatibleMethod))
    ...
    // Internal use only
    public MethodBinding findExactMethod(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes,
        InvocationSite invocationSite) {
        CompilationUnitScope unitScope = compilationUnitScope();
        ...
        if (exactMethod != null && exactMethod.typeVariables == Binding.NO_TYPE_VARIABLES && !exactMethod.isBridge()) {
            ...
            for (int i = argumentTypes.length; --i >= 0;) {
                // workaround for bug 464229: The type * cannot be resolved. It is indirectly referenced from required
                // .class files
                ...
                // TODO: should also check if any supertype of r is raw, but can't do this without resolving the whole
                // hierarchy
            }
            // must find both methods for this case: <S extends A> void foo() {} and <N extends B> N foo() { return
            // null; }
    ...
    public FieldBinding findField(TypeBinding receiverType, char[] fieldName, InvocationSite invocationSite,
        ...
        while (keepLooking) {
                ...
                if (interfacesToVisit == null) {
                    ...
                    if (nextPosition + itsLength >= interfacesToVisit.length)
                        System.arraycopy(interfacesToVisit, 0,
                            interfacesToVisit = new ReferenceBinding[nextPosition + itsLength + 5], 0, nextPosition);
            ...
            currentType = (ReferenceBinding) currentType.capture(this,
                invocationSite == null ? 0 : invocationSite.sourceStart(),
    ...
    public ReferenceBinding findMemberType(char[] typeName, ReferenceBinding enclosingType) {
        ...
        if (memberType != null) {
            ...
            if (enclosingSourceType == null
                || (this.parent == unitScope && (enclosingSourceType.tagBits & TagBits.TypeVariablesAreConnected) == 0)
        ...
        if (method != null && method.isValidBinding() && method.isVarargs()) {
            ...
            if (elementType instanceof ReferenceBinding) {
                if (!elementType.erasure().canBeSeenBy(this)) {
                    return new ProblemMethodBinding(method, method.selector, invocationSite.genericTypeArguments(),
    ...
    public MethodBinding findMethod0(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes,
        InvocationSite invocationSite, boolean inStaticContext) {
        ...
        while (currentType != null) {
            ...
            if (currentLength > 0) {
                if ((receiverTypeIsInterface || found.size > 0)) {
                    nextMethod: for (int i = 0, l = currentLength; i < l; i++) { // currentLength can be modified inside
                                                                                 // the loop
                        MethodBinding currentMethod = currentMethods[i];
                        if (currentMethod == null)
                            continue nextMethod;
                        if (receiverTypeIsInterface && !currentMethod.isPublic()) { // only public methods from Object
                                                                                    // are visible to interface
                                                                                    // receiverTypes
                            currentLength--;
                        ...
                        // protected method need to be checked only - default access is already dealt with in #canBeSeen
                        // implementation
                        // when checking that p.C -> q.B -> p.A cannot see default access members from A through B.
                        // if ((currentMethod.modifiers & AccProtected) == 0) continue nextMethod;
                        // BUT we can also ignore any overridden method since we already know the better match (fixes
                        // 80028)
                        for (int j = 0, max = found.size; j < max; j++) {
                            ...
                            if (currentOriginal != null
                                ...
                                if (matchingMethod.isBridge() && !currentMethod.isBridge())
                                    continue nextMethod; // keep inherited methods to find concrete method over a bridge
                                                         // method
                                currentLength--;
                                currentMethods[i] = null;
                                continue nextMethod;
        ...
        if (foundSize > 0) {
            ...
            for (int i = 0; i < foundSize; i++) {
                ...
                if (compatibleMethod != null) {
                    if (compatibleMethod.isValidBinding()
                        || compatibleMethod.problemId() == ProblemReasons.InvocationTypeInferenceFailure) {
                        // we need to accept methods with InvocationTypeInferenceFailure, because logically overload
                        // resolution happens *before* invocation type inference
                        if (foundSize == 1 && compatibleMethod.canBeSeenBy(receiverType, invocationSite, this)) {
                            ...
                            return findDefaultAbstractMethod(receiverType, selector, argumentTypes, invocationSite,
                                classHierarchyStart, found, new MethodBinding[] { compatibleMethod });
                        }
                        if (candidatesCount == 0)
                            candidates = new MethodBinding[foundSize];
                        candidates[candidatesCount++] = compatibleMethod;
                    } else if (compatibleMethod.problemId() == ProblemReasons.MissingTypeInSignature) {
                        return compatibleMethod; // use this method for error message to give a hint about the missing
                                                 // type
                    } else if (problemMethod == null) {
                        problemMethod = compatibleMethod;
                    }
        ...
        if (candidatesCount == 0) {
            ...
            if (interfaceMethod != null) {
                if (foundSize > 0
                    && interfaceMethod.isVarargs()
                    && interfaceMethod instanceof ParameterizedGenericMethodBinding) {
                    ...
                    for (int i = 0; i < foundSize; i++) {
                        ...
                        if (!classMethod.isAbstract()) { // this check shouldn't matter, but to compatible with javac...
                            ...
                            if (substitute != null
                                && verifier.isSubstituteParameterSubsignature(classMethod, substitute))
                                return new ProblemMethodBinding(interfaceMethod, selector, argumentTypes,
                                    ProblemReasons.ApplicableMethodOverriddenByInapplicable);
                        }
                    }
                }
            ...
            if (found.size == 0) {
                if (singlePrivateMethod != null) {
                    ...
                    return new ProblemMethodBinding(singlePrivateMethod, selector, singlePrivateMethod.parameters,
                        ProblemReasons.NotVisible);
                }
                return null;
            }
            ...
            MethodBinding bestGuess = (MethodBinding) found.elementAt(0); // if no good match so just use the first one
                                                                          // found
            ...
            nextMethod: for (int i = 0; i < foundSize; i++) {
                ...
                next: for (int a = 0; a < argLength; a++) {
                    ...
                    for (int p = a == 0 ? 0 : a - 1; p < paramLength && p < a + 1; p++) { // look one slot before &
                                                                                          // after to see if the type
                                                                                          // matches
                ...
                if (bestGuess != methodBinding
                    && MethodVerifier.doesMethodOverride(bestGuess, methodBinding, this.environment()))
                    continue;
        ...
        switch (visiblesCount) {
            ...
            case 1:
                return findDefaultAbstractMethod(receiverType, selector, argumentTypes, invocationSite,
                    classHierarchyStart, found, new MethodBinding[] { candidates[0] });
        ...
        return findDefaultAbstractMethod(receiverType, selector, argumentTypes, invocationSite, classHierarchyStart,
            found, candidates);
    }
    ...
    protected void findMethodInSuperInterfaces(ReferenceBinding receiverType, char[] selector, ObjectVector found,
        List<TypeBinding> visitedTypes, InvocationSite invocationSite) {
        ReferenceBinding currentType = receiverType;
        ReferenceBinding[] itsInterfaces = currentType.superInterfaces();
        if (itsInterfaces != null && itsInterfaces != Binding.NO_SUPERINTERFACES) {
                                ...
                                if (shouldTrackOuterLocals) {
                                    if (invocationSite instanceof NameReference) {
                                        ...
                                        AbstractVariableDeclaration variableDeclaration
                                            = (AbstractVariableDeclaration) invocationSite;
                        ...
                        case CLASS_SCOPE:
                            ...
                            if (!insideTypeAnnotation) {
                                FieldBinding fieldBinding
                                    = classScope.findField(receiverType, name, invocationSite, needResolve);
                                ...
                                if (fieldBinding != null) { // skip it if we did not find anything
                                    if (fieldBinding.problemId() == ProblemReasons.Ambiguous) {
                                        if (foundField == null || foundField.problemId() == ProblemReasons.NotVisible)
                                            ...
                                            return fieldBinding;
                                        // make the user qualify the field, likely wants the first inherited field
                                        // (javac generates an ambiguous error instead)
                                        return new ProblemFieldBinding(foundField, // closest match
                                            foundField.declaringClass, name,
                                            ProblemReasons.InheritedNameHidesEnclosingName);
                                    }
                                    ...
                                    if (fieldBinding.isValidBinding()) {
                                        if (!fieldBinding.isStatic()) {
                                            if (insideConstructorCall) {
                                                if (invocationSite instanceof ASTNode node
                                                    ...
                                                    && JavaFeature.FLEXIBLE_CONSTRUCTOR_BODIES
                                                        .matchesCompliance(compilerOptions())) {
                                                    // enablement check for assignment deferred to
                                                    // Reference.checkFieldAccessInEarlyConstructionContext()
                                                } else if (!JavaFeature.FLEXIBLE_CONSTRUCTOR_BODIES
                                                    .isSupported(compilerOptions())) {
                                                    insideProblem = new ProblemFieldBinding(fieldBinding, // closest
                                                                                                          // match
                                        ...
                                        if (foundField == null || foundField.problemId() == ProblemReasons.NotVisible) {
                                            ...
                                            // return the fieldBinding if it is not declared in a superclass of the
                                            // scope's binding (that is, inherited)
                                            return insideProblem == null ? fieldBinding : insideProblem;
                                        }
                                        if (foundField.isValidBinding())
                                            // if a valid field was found, complain when another is found in an
                                            // 'immediate' enclosing type (that is, not inherited)
                                            // but only if "valid field" was inherited in the first place.
                                            if (TypeBinding.notEquals(foundField.declaringClass,
                                                fieldBinding.declaringClass)
                                                && TypeBinding.notEquals(foundField.declaringClass,
                                                    foundActualReceiverType)) // https://bugs.eclipse.org/bugs/show_bug.cgi?id=316956
                                    ...
                                    if (foundField == null
                                        || (foundField.problemId() == ProblemReasons.NotVisible
                                            && fieldBinding.problemId() != ProblemReasons.NotVisible)) {
                                        // only remember the fieldBinding if its the first one found or the previous one
                                        // was not visible & fieldBinding is...
                                        foundDepth = depth;
                        ...
                        if (importBinding.isStatic() && !importBinding.onDemand) {
                            if (CharOperation.equals(importBinding.getSimpleName(), name)) {
                                if (unitScope.resolveSingleImport(importBinding,
                                    ...
                                    TypeBinding importedType = this.getType(importBinding.compoundName,
                                        importBinding.compoundName.length - 1);
                    ...
                    for (ImportBinding importBinding : imports) {
                        if (importBinding.isStatic() && importBinding.onDemand) {
                            ...
                            if (resolvedImport instanceof ReferenceBinding) {
                                ...
                                FieldBinding temp
                                    = findField(importedReferenceBinding, name, invocationSite, needResolve);
                                if (temp != null) {
                                    if (!temp.isValidBinding()) {
                                        ...
                                        if (foundInImport)
                                            // Answer error binding -- import on demand conflict; name found in two
                                            // import on demand packages.
                                            return new ProblemFieldBinding(foundField, // closest match
    ...
    private MethodBinding getExactMethod(TypeBinding receiverType, TypeBinding type, char[] selector,
        InvocationSite invocationSite, MethodBinding candidate) {
        ...
        for (int i = 0, typesLength = typePlusSupertypes.length; i < typesLength; i++) {
            MethodBinding[] methods = i == 0
                ...
                : new MethodBinding[] {
                    getExactMethod(receiverType, typePlusSupertypes[i], selector, invocationSite, candidate) };
            for (MethodBinding currentMethod : methods) {
                if (currentMethod == null || candidate == currentMethod)
                    continue;
                if (i == 0
                    && (!currentMethod.canBeSeenBy(receiverType, invocationSite, this)
    ...
    public MethodBinding getExactMethod(TypeBinding receiverType, char[] selector, InvocationSite invocationSite) {
        ...
        if (exactMethod.isVarargs()
            || (typeVariables != Binding.NO_TYPE_VARIABLES
                && (typeArguments == null || typeArguments.length != typeVariables.length)))
            return null;
    ...
    public MethodBinding getExactConstructor(TypeBinding receiverType, InvocationSite invocationSite) {
        if (receiverType == null
            ...
            || !receiverType.canBeInstantiated()
            || receiverType.isBaseType())
            return null;
        if (receiverType.isArrayType()) {
            TypeBinding leafType = receiverType.leafComponentType();
            ...
            return new MethodBinding(ClassFileConstants.AccPublic | ClassFileConstants.AccSynthetic, TypeConstants.INIT,
                receiverType, new TypeBinding[] { TypeBinding.INT }, Binding.NO_EXCEPTIONS, getJavaLangObject()); // just
                                                                                                                  // lie.
        }
        ...
        if (exactConstructor != null) {
            ...
            if (typeVariables != Binding.NO_TYPE_VARIABLES) {
                if (typeVariables.length != genericTypeArguments.length)
                    return null;
                exactConstructor
                    = environment().createParameterizedGenericMethod(exactConstructor, genericTypeArguments);
            }
        }
    ...
    public MethodBinding getImplicitMethod(char[] selector, TypeBinding[] argumentTypes,
            ...
            switch (scope.kind) {
                ...
                case CLASS_SCOPE:
                    ...
                    if (!insideTypeAnnotation) {
                        ...
                        // compilationUnitScope().recordTypeReference(receiverType); not needed since receiver is the
                        // source type
                        MethodBinding methodBinding
                            = classScope.findExactMethod(receiverType, selector, argumentTypes, invocationSite);
                        if (methodBinding == null)
                            methodBinding
                                = classScope.findMethod(receiverType, selector, argumentTypes, invocationSite, false);
                        if (methodBinding != null) { // skip it if we did not find anything
                            if (methodBinding.isValidBinding()) {
                                if (lacksRequiredInstanceScope(methodBinding, insideConstructorCall,
                                    insideStaticContext)) {
                                    ...
                                    tagAsAccessingEnclosingInstanceStateOf(receiverType,
                                        false /* type variable access */);
                                }
                                // found a valid method in the 'immediate' scope (i.e. not inherited)
                                // OR in 1.4 mode (inherited visible shadows enclosing)
                                // OR the receiverType implemented a method with the correct name
                                // return the methodBinding if it is not declared in a superclass of the scope's binding
                                // (that is, inherited)
                                ...
                                // special treatment for Object.getClass() in 1.5 mode (substitute parameterized return
                                // type)
                                if (argumentTypes == Binding.NO_PARAMETERS
                                    && CharOperation.equals(selector, TypeConstants.GETCLASS)
                                    && methodBinding.returnType.isParameterizedType()/* 1.5 */) {
                                ...
                                if (foundProblem == null) {
                                    foundProblem = methodBinding; // hold onto the first not visible/found error and
                                                                  // keep the second not found if first is not visible
                                }
                                if (!foundProblemVisible && methodBinding.problemId() == ProblemReasons.NotFound) {
                                    ...
                                    if (closestMatch != null
                                        ...
                                        foundProblem = methodBinding; // hold onto the first not visible/found error and
                                                                      // keep the second not found if first is not
                                                                      // visible
        ...
        if (insideStaticContext) {
            ...
            unitScope.faultInImports(); // field constants can cause static imports to be accessed before they're
                                        // resolved
            ImportBinding[] imports = unitScope.imports;
            if (imports != null) {
                ObjectVector visible = null;
                boolean skipOnDemand = false; // set to true when matched static import of method name so stop looking
                                              // for on demand methods
                for (ImportBinding importBinding : imports) {
                    if (importBinding.isStatic()) {
                        ...
                        if (importBinding.onDemand) {
                            if (!skipOnDemand && resolvedImport instanceof ReferenceBinding) {
                                ...
                                possible = findMethod(resolvedImportReferenceBinding, selector, argumentTypes,
                                    invocationSite, true);
                            ...
                            if (resolvedImport instanceof MethodBinding) {
                                ...
                                if (CharOperation.equals(staticMethod.selector, selector)) {
                                    ...
                                    possible = findMethod(staticMethod.declaringClass, selector, argumentTypes,
                                        invocationSite, true);
                                    if (possible != null && possible.isValidBinding()) {
                                        TypeBinding importedType = getType(importBinding.compoundName,
                                            importBinding.compoundName.length - 1);
                                ...
                                if (CharOperation.equals(staticField.name, selector)) {
                                    // must find the importRef's type again since the field can be from an inherited
                                    // type
                        ...
                        if (possible != null && possible != foundProblem) {
                            if (!possible.isValidBinding()) {
                                ...
                                MethodBinding compatibleMethod
                                    = computeCompatibleMethod(possible, argumentTypes, invocationSite);
                                if (compatibleMethod != null) {
                                    if (compatibleMethod.isValidBinding()) {
                                        method2sourceDeclaring.put(compatibleMethod,
                                            method2sourceDeclaring.get(possible));
    ...
    boolean lacksRequiredInstanceScope(MethodBinding methodBinding, boolean insideConstructorCall,
    ...
    public final ReferenceBinding getJavaLangIncompatibleClassChangeError() {
        ...
        return unitScope.environment.getResolvedJavaBaseType(TypeConstants.JAVA_LANG_INCOMPATIBLECLASSCHANGEERROR,
            this);
    }
    ...
    public final ReferenceBinding getJavaLangInvokeStringConcatFactory() {
        ...
        return unitScope.environment.getResolvedJavaBaseType(TypeConstants.JAVA_LANG_INVOKE_STRING_CONCAT_FACTORY,
            this);
    }
    ...
    public final ReferenceBinding getJavaLangInvokeMethodHandlesLookup() {
        ...
        ReferenceBinding outerType
            = unitScope.environment.getResolvedJavaBaseType(TypeConstants.JAVA_LANG_INVOKE_METHODHANDLES, this);
    ...
    public final ReferenceBinding getMemberType(char[] typeName, ReferenceBinding enclosingType) {
        ...
        if (enclosingType.isHierarchyBeingConnected() && memberType == null && this.kind == CLASS_SCOPE) {
            ...
            if (memberType == null
                && scope.referenceContext.superInterfaces != null
    ...
    Binding problemType(char[][] compoundName, int currentIndex, Binding previousProblem) {
        ...
        return previousProblem != null
            ? previousProblem
            : new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex), null,
                ProblemReasons.NotFound);
    }
    ...
    public final TypeBinding getType(char[] name, PackageBinding packageBinding) {
        ...
        if (!binding.isValidBinding()) {
            return new ProblemReferenceBinding(
                binding instanceof ReferenceBinding
                    ? ((ReferenceBinding) binding).compoundName
    ...
    public final TypeBinding getType(char[][] compoundName, int typeNameLength) {
        ...
        if (binding instanceof PackageBinding) {
            ...
            while (currentIndex < typeNameLength) {
                binding = packageBinding.getTypeOrPackage(compoundName[currentIndex++], module(),
                    currentIndex < typeNameLength); // does not check visibility
                if (binding == null) {
                    char[][] qName = CharOperation.subarray(compoundName, 0, currentIndex);
                    return new ProblemReferenceBinding(qName, environment().createMissingType(packageBinding, qName),
                        ProblemReasons.NotFound);
                }
                if (!binding.isValidBinding())
                    return new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex),
        ...
        if (checkVisibility) // handles the fall through case
            if (!typeBinding.canBeSeenBy(this))
                return new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex), typeBinding,
                    ProblemReasons.NotVisible);
    ...
    final Binding getTypeOrPackage(char[] name, int mask, boolean needResolve) {
                ...
                switch (scope.kind) {
                    case METHOD_SCOPE:
                        ...
                        if (methodDecl != null) {
                            if (methodDecl.binding != null) {
                                ...
                                // use the methodDecl's typeParameters to handle problem cases when the method binding
                                // doesn't exist
                        ...
                        //$FALL-THROUGH$
                    case BLOCK_SCOPE:
                        ReferenceBinding localType = ((BlockScope) scope).findLocalType(name); // looks in this scope
                                                                                               // only
                        if (localType != null) {
                    ...
                    case CLASS_SCOPE:
                        ...
                        if (scope == this && (sourceType.tagBits & TagBits.TypeVariablesAreConnected) == 0) {
                            // type variables take precedence over the source type, ex. class X <X> extends X == class X
                            // <Y> extends Y
                        ...
                        if (!insideTypeAnnotation) {
                            ...
                            if (memberType != null) { // skip it if we did not find anything
                                ...
                                if (memberType.isValidBinding()) {
                                    ...
                                    if (foundType == null || (foundType.problemId() == ProblemReasons.NotVisible))
                                        return memberType;
                                    // if a valid type was found, complain when another is found in an 'immediate'
                                    // enclosing type (i.e. not inherited)
                                ...
                                if (foundType == null
                                    ...
                                    // only remember the memberType if its the first one found or the previous one was
                                    // not visible & memberType is...
                                    foundType = memberType;
                            }
                        }
                        TypeVariableBinding typeVariable = sourceType.getTypeVariable(name);
                        if (typeVariable != null) {
                            if (insideStaticContext) // do not consider this type modifiers: access is legite within
                                                     // same type
                                return new ProblemReferenceBinding(new char[][] { name }, typeVariable,
                                    ProblemReasons.NonStaticReferenceInStaticContext);
                            return typeVariable;
                        ...
                        if (CharOperation.equals(sourceType.sourceName, name)) {
                            ...
                            if (foundType != null
                                && TypeBinding.notEquals(foundType, sourceType)
                                && foundType.problemId() != ProblemReasons.NotVisible)
                                return new ProblemReferenceBinding(new char[][] { name }, foundType,
                                    ProblemReasons.InheritedNameHidesEnclosingName);
                            return sourceType;
        ...
        if (typeOrPackageCache != null) {
            ...
            if (cachedBinding != null) { // can also include NotFound ProblemReferenceBindings if we already know this
                                         // name is not found
                if (cachedBinding instanceof ImportBinding) { // single type import cached in faultInImports(), replace
                                                              // it in the cache with the type
                    ...
                    if (cachedBinding instanceof ImportConflictBinding)
                        typeOrPackageCache.put(name,
                            cachedBinding = ((ImportConflictBinding) cachedBinding).conflictingTypeBinding); // already
                                                                                                             // know its
                                                                                                             // visible
                    else
                ...
                if ((mask & Binding.TYPE) != 0) {
                    if (foundType != null
                        ...
                        return foundType; // problem type from above supercedes NotFound type but not Ambiguous import
                                          // case
        ...
        if ((mask & Binding.TYPE) != 0) {
            ImportBinding[] imports = unitScope.imports;
            if (imports != null && typeOrPackageCache == null) { // walk single type imports since faultInImports() has
                                                                 // not run yet
                    ...
                    if (!importBinding.onDemand) {
                        if (CharOperation.equals(importBinding.getSimpleName(), name)) {
                            ...
                            if (resolvedImport instanceof TypeBinding) {
                                ImportReference importReference = importBinding.reference;
                                if (importReference != null
                                    && !isUnnecessarySamePackageImport(importBinding.getResolvedImport(), unitScope))
                                    importReference.bits |= ASTNode.Used;
            ...
            if (imports != null) {
                ReferenceBinding type
                    = findTypeInOnDemandImports(imports, currentPackage, name, false, typeOrPackageCache);
                if (type == null) { // module imports would otherwise be shadowed
                    type = findTypeInOnDemandImports(imports, currentPackage, name, true/* module imports */,
                        typeOrPackageCache);
                }
        ...
        if (foundType == null) {
            ...
            if (typeOrPackageCache != null && (mask & Binding.PACKAGE) != 0) { // only put NotFound type in cache if you
                                                                               // know its not a package
    ...
    private ReferenceBinding findTypeInOnDemandImports(ImportBinding[] imports, PackageBinding currentPackage,
        char[] name, boolean inModules, HashtableOfObject typeOrPackageCache) {
                ...
                if (inModules) {
                    ...
                    if (resolvedImport instanceof PackageBinding packageBinding) {
                        ...
                        if (someImport.isStatic()) {
                            ...
                            temp = compilationUnitScope().findMemberType(name, referenceBinding); // static imports are
                                                                                                  // allowed to see
                                                                                                  // inherited member
                                                                                                  // types
    ...
    // DO NOT USE to resolve import references since this method assumes 'A.B' is relative to a single type import of
    // 'p1.A'
    ...
    public boolean hasErasedCandidatesCollisions(TypeBinding one, TypeBinding two, Map<TypeBinding, Object> invocations,
    ...
    private TypeBinding leastContainingInvocation(TypeBinding mec, Object invocationData,
        ArrayList<TypeBinding[]> lubStack) {
        if (invocationData == null)
            return mec; // no alternate invocation
        if (invocationData instanceof TypeBinding) { // only one invocation, simply return it (array only allocated if
                                                     // more than one)
        ...
        for (TypeBinding binding : invocations) {
            ...
            switch (invocation.kind()) {
                case Binding.GENERIC_TYPE:
                    ...
                    for (int j = 0; j < argLength; j++) {
                        TypeBinding bestArgument = leastContainingTypeArgument(bestArguments[j], invocationVariables[j],
                            (ReferenceBinding) mec, j, new ArrayList<>(lubStack));
                ...
                case Binding.PARAMETERIZED_TYPE:
                    ...
                    for (int j = 0; j < argLength; j++) {
                        TypeBinding bestArgument = leastContainingTypeArgument(bestArguments[j],
                            parameterizedType.arguments[j], (ReferenceBinding) mec, j, new ArrayList<>(lubStack));
                        if (bestArgument == null)
                            return null;
                        bestArguments[j] = bestArgument;
                    }
                ...
                case Binding.RAW_TYPE:
                    return dim == 0 ? invocation : environment().createArrayType(invocation, dim); // raw type is taking
                                                                                                   // precedence
            }
        }
    ...
    // JLS 15.12.2
    private TypeBinding leastContainingTypeArgument(TypeBinding u, TypeBinding v, ReferenceBinding genericType,
        int rank, ArrayList<TypeBinding[]> lubStack) {
        if (u == null)
            return v;
        ...
        if (v.isWildcard()) {
            ...
            if (u.isWildcard()) {
                ...
                switch (wildU.boundKind) {
                    ...
                    case Wildcard.EXTENDS:
                        switch (wildV.boundKind) {
                            ...
                            case Wildcard.EXTENDS:
                                TypeBinding lub
                                    = lowerUpperBound(new TypeBinding[] { wildU.bound, wildV.bound }, lubStack);
                                if (lub == null)
                                    return null;
                                // int is returned to denote cycle detected in lub computation - stop recursion by
                                // answering unbound wildcard
                                if (TypeBinding.equalsEquals(lub, TypeBinding.INT))
                                    return environment().createWildcard(genericType, rank, null,
                                        null /* no extra bound */, Wildcard.UNBOUND);
                ...
                switch (wildV.boundKind) {
                    ...
                    case Wildcard.EXTENDS:
                        ...
                        if (lub == null)
                            return null;
                        // int is returned to denote cycle detected in lub computation - stop recursion by answering
                        // unbound wildcard
                        if (TypeBinding.equalsEquals(lub, TypeBinding.INT))
                            return environment().createWildcard(genericType, rank, null, null /* no extra bound */,
                                Wildcard.UNBOUND);
                    ...
                    case Wildcard.SUPER:
                        TypeBinding[] glb
                            = greaterLowerBound(new TypeBinding[] { u, wildV.bound }, this, this.environment());
                        if (glb == null)
                            return null;
                        return environment().createWildcard(genericType, rank, glb[0], null /* no extra bound */,
                            Wildcard.SUPER);	// TODO (philippe) need to capture entire bounds
            ...
            switch (wildU.boundKind) {
                ...
                case Wildcard.SUPER:
                    TypeBinding[] glb
                        = greaterLowerBound(new TypeBinding[] { wildU.bound, v }, this, this.environment());
                    if (glb == null)
                        return null;
                    return environment().createWildcard(genericType, rank, glb[0], null /* no extra bound */,
                        Wildcard.SUPER); // TODO (philippe) need to capture entire bounds
        ...
        nextLubCheck: for (int i = 0; i < stackLength; i++) {
            ...
            nextTypeCheck: for (int j = 0; j < typeLength; j++) {
                ...
                for (int k = 0; k < lubTypeLength; k++) {
                    ...
                    if (lubType == null)
                        continue; // ignore
                    if (TypeBinding.equalsEquals(lubType, type) || lubType.isEquivalentTo(type))
    ...
    protected TypeBinding[] minimalErasedCandidates(TypeBinding[] types, Map<TypeBinding, Object> allInvocations) {
        ...
        for (int i = 0; i < max; i++) {
            ...
            if (currentType.isCapture()) {
                ...
                if (firstBound != null && firstBound.isArrayType()) {
                    TypeBinding superType
                        = dim == 0 ? firstBound : (TypeBinding) environment().createArrayType(firstBound, dim); // recreate
                                                                                                                // array
                                                                                                                // if
                                                                                                                // needed
                    if (!typesToVisit.contains(superType)) {
                        ...
                        TypeBinding superTypeErasure = (firstBound.isTypeVariable()
                            || firstBound.isWildcard() /* && !itsInterface.isCapture() */)
                                ? superType
                                : superType.erasure();
            ...
            ReferenceBinding[] itsInterfaces = currentType.superInterfaces();
            if (itsInterfaces != null) { // can be null during code assist operations that use
                                         // LookupEnvironment.completeTypeBindings(parsedUnit, buildFieldsAndMethods)
                for (ReferenceBinding itsInterface : itsInterfaces) {
                    TypeBinding superType
                        = dim == 0 ? itsInterface : (TypeBinding) environment().createArrayType(itsInterface, dim); // recreate
                                                                                                                    // array
                                                                                                                    // if
                                                                                                                    // needed
                    if (!typesToVisit.contains(superType)) {
                        ...
                        TypeBinding superTypeErasure = (itsInterface.isTypeVariable()
                            || itsInterface.isWildcard() /* && !itsInterface.isCapture() */)
                                ? superType
                                : superType.erasure();
                        if (TypeBinding.notEquals(superTypeErasure, superType)) {
                            allInvocations.put(superTypeErasure, superType);
            ...
            if (itsSuperclass != null) {
                TypeBinding superType
                    = dim == 0 ? itsSuperclass : (TypeBinding) environment().createArrayType(itsSuperclass, dim); // recreate
                                                                                                                  // array
                                                                                                                  // if
                                                                                                                  // needed
                if (!typesToVisit.contains(superType)) {
                    ...
                    TypeBinding superTypeErasure = (itsSuperclass.isTypeVariable()
                        || itsSuperclass.isWildcard() /* && !itsSuperclass.isCapture() */)
                            ? superType
                            : superType.erasure();
                    if (TypeBinding.notEquals(superTypeErasure, superType)) {
                        allInvocations.put(superTypeErasure, superType);
        ...
        for (Object typeToVisit : typesToVisit) {
            ...
            erasedSuperTypes[rank++]
                = (leafType.isTypeVariable() || leafType.isWildcard() /* && !leafType.isCapture() */)
                    ? type
                    : type.erasure();
        }
    ...
    protected final MethodBinding mostSpecificMethodBinding(MethodBinding[] visible, int visibleSize,
        ...
        for (int i = 0; i < visibleSize; i++)
            if ((compatibilityLevels[i]
                = parameterCompatibilityLevelFromInference(visible[i], argumentTypes, invocationSite))
                != NOT_COMPATIBLE) {
                if (compatibilityLevels[i] == COMPATIBLE_IGNORING_MISSING_TYPE) {
                    ...
                    return new ProblemMethodBinding(visible[i], visible[i].selector, visible[i].parameters,
                        ProblemReasons.Ambiguous);
                }
        ...
        if (compatibleCount != visibleSize) {
            System.arraycopy(visible, 0, visible = new MethodBinding[visibleSize = compatibleCount], 0,
                compatibleCount);
            System.arraycopy(compatibilityLevels, 0, compatibilityLevels = new int[compatibleCount], 0,
                compatibleCount);
        }
        ...
        nextJ: for (int j = 0; j < visibleSize; j++) {
            ...
            nextK: for (int k = 0; k < visibleSize; k++) {
                ...
                if (((invocationSite instanceof Invocation) || (invocationSite instanceof ReferenceExpression))
                    ...
                    if (!ic18.isMoreSpecificThan(mbj, mbk, levelj == VARARGS_COMPATIBLE,
                        levelk == VARARGS_COMPATIBLE)) {
                        continue nextJ;
                    }
                } else {
        ...
        if (count == 0) {
            ...
            if (visibleSize > 1
                && candidate instanceof ParameterizedMethodBinding
                && invocationSite instanceof MessageSend) {
                for (TypeBinding typeBinding : argumentTypes) {
                    if (!typeBinding.isProperType(true)) {
                        InferenceContext18 ic18 = ((MessageSend) invocationSite)
                            .getInferenceContext((ParameterizedMethodBinding) candidate);
                        if (ic18 != null)
        ...
        // so now with the first acceptable method, find the 'correct' inherited method for each other acceptable method
        // AND
        // see if they are equal after substitution of type variables (do the type variables have to be equal to be
        ...
        // perform 1 or 2 attempts, the second being the safety net, in case considering null contracts may have
            ...
            nextSpecific: for (int i = 0; i < visibleSize; i++) {
                ...
                if (current != null) {
                    ...
                    boolean shouldIntersectExceptions
                        = original.declaringClass.isAbstract() && original.thrownExceptions != Binding.NO_EXCEPTIONS; // only
                                                                                                                      // needed
                                                                                                                      // when
                                                                                                                      // selecting
                                                                                                                      // from
                    ...
                    for (int j = 0; j < visibleSize; j++) {
                        ...
                        if (!original.isAbstract()) {
                            ...
                            if (original2 == null)
                                continue nextSpecific; // current's declaringClass is not a subtype of next's
                                                       // declaringClass
                            ...
                            TypeBinding superType
                                = receiverType.findSuperTypeOriginatingFrom(original.declaringClass.erasure());
                            if (TypeBinding.equalsEquals(original.declaringClass, superType)
                                || !(superType instanceof ReferenceBinding)) {
                                // keep original
                            } else {
                            ...
                            if (TypeBinding.equalsEquals(original2.declaringClass, superType)
                                || !(superType instanceof ReferenceBinding)) {
                                // keep original2
                            } else {
                                // must find inherited method with the same substituted variables
                            ...
                            if (shouldIntersectExceptions && original2.declaringClass.isInterface()) {
                                if (current.thrownExceptions != next.thrownExceptions) {
                                    if (next.thrownExceptions == Binding.NO_EXCEPTIONS) {
                                        ...
                                        if (changed) {
                                            mostSpecificExceptions = temp.elementSize == 0
    ...
    public int parameterCompatibilityLevelFromInference(MethodBinding method, TypeBinding[] arguments,
        InvocationSite site) {
        if (method.problemId() == ProblemReasons.InvocationTypeInferenceFailure) {
            // we need to accept methods with InvocationTypeInferenceFailure, because logically overload resolution
            // happens *before* invocation type inference
        ...
        if (method instanceof ParameterizedGenericMethodBinding) {
            ...
            /*
             * 1.8+ Post inference compatibility check policy: For non-functional-type arguments, trust inference. For
             * functional type arguments apply compatibility checks after inference
             * has completed to ensure arguments that were not pertinent to applicability which have only seen potential
             * compatibility checks are actually compatible.
             */
            if (site instanceof Invocation
                && context != null
                && context.stepCompleted >= InferenceContext18.TYPE_INFERRED) {
                for (int i = 0, length = arguments.length; i < length; i++) {
                    ...
                    if (!argument.isCompatibleWith(parameter, this)) {
                        if (argument.isPolyType()) {
                            parameter
                                = InferenceContext18.getParameter(method.original().parameters, i, context.isVarArgs());
    ...
    public int parameterCompatibilityLevel(MethodBinding method, TypeBinding[] arguments, InvocationSite site) {
        ...
        if (method.isVarargs()) {
            ...
            if (paramLength == argLength) { // accept X or X[] but not X[][]
                ...
                if (paramLength < argLength) { // all remaining argument types must be compatible with the elementsType
                                               // of varArgType
    ...
    private int parameterCompatibilityLevel(TypeBinding arg, TypeBinding param, LookupEnvironment env,
        ...
        if (arg instanceof PolyTypeBinding
            && !((PolyTypeBinding) arg).expression.isPertinentToApplicability(param, method)) {
    ...
    public MethodBinding getStaticFactory(ParameterizedTypeBinding allocationType,
        ReferenceBinding originalEnclosingType, TypeBinding[] argumentTypes, final InvocationSite allocationSite) {

        // allocationType is the diamond type. originalEnclosingType is the real enclosing type ==> may be
        // parameterized, parameterized with own type variables, raw, just plain type or null.
        ...
        while (currentType != null) {
            ...
            if (length > 0) {
                System.arraycopy(classTypeVariables, 0,
                    classTypeVariables = new TypeVariableBinding[classTypeVariablesArity + length], 0,
                    classTypeVariablesArity);
        ...
        for (MethodBinding method : methods) {
            ...
            MethodBinding targetMethod
                = isInterface ? new MethodBinding(method.original(), genericType) : method.original();
            MethodBinding staticFactory
                = new SyntheticFactoryMethodBinding(targetMethod, environment, originalEnclosingType);
            ...
            // Rename each type variable T of the type to T' or T'' or T''' based on the enclosing level to avoid a
            // clash.
            ...
            for (int j = 0; j < classTypeVariablesArity; j++) {
                ...
                map.put(original.unannotated(), staticFactory.typeVariables[j] = new TypeVariableBinding(
            ...
            for (int j = classTypeVariablesArity, k = 0; j < factoryArity; j++, k++) {
                map.put(methodTypeVariables[k].unannotated(),
                    (staticFactory.typeVariables[j] = new TypeVariableBinding(
                        CharOperation.concat(methodTypeVariables[k].sourceName, prime.toCharArray()), staticFactory, j,
                        environment)));
            ...
            Substitution substitution = new Substitution() {
                ...
                public TypeBinding substitute(TypeVariableBinding typeVariable) {
                    ...
                    return retVal == null
                        ? typeVariable
                        : typeVariable.hasTypeAnnotations()
                            ? environment().createAnnotatedType(retVal, typeVariable.getTypeAnnotations())
                            : retVal;
                }
            };
            ...
            for (int j = 0; j < factoryArity; j++) {
                TypeVariableBinding originalVariable = j < classTypeVariablesArity
                    ? classTypeVariables[j]
                ...
                switch (substitutedSuperclass.kind()) {
                    case Binding.ARRAY_TYPE:
                        substitutedVariable
                            .setSuperClass(environment.getResolvedJavaBaseType(TypeConstants.JAVA_LANG_OBJECT, null));
                        substitutedVariable.setSuperInterfaces(substitutedInterfaces);
                        break;

                    default:
                        if (substitutedSuperclass.isInterface()) {
                            substitutedVariable.setSuperClass(
                                environment.getResolvedJavaBaseType(TypeConstants.JAVA_LANG_OBJECT, null));
                            int interfaceCount = substitutedInterfaces.length;
                            System.arraycopy(substitutedInterfaces, 0,
                                substitutedInterfaces = new ReferenceBinding[interfaceCount + 1], 1, interfaceCount);
                            substitutedInterfaces[0] = (ReferenceBinding) substitutedSuperclass;
                            substitutedVariable.setSuperInterfaces(substitutedInterfaces);
                        } else {
                            substitutedVariable.setSuperClass((ReferenceBinding) substitutedSuperclass); // typeVar was
                                                                                                         // extending
                                                                                                         // other
                                                                                                         // typeVar
                                                                                                         // which got
            ...
            staticFactories[sfi++] = new ParameterizedMethodBinding(
                (ParameterizedTypeBinding) environment
                    .convertToParameterizedType(isInterface ? allocationType : staticFactory.declaringClass),
                staticFactory);
        }
        if (sfi == 0)
            return null;
        ...
        return compatibleIndex == 1
            ? compatible[0]
            : mostSpecificMethodBinding(compatible, compatibleIndex, argumentTypes, allocationSite, allocationType);
    }
     ...
     * @return <code>true</code> if the annotation was newly recorded, <code>false</code> if a corresponding entry
     * already existed.
     */
    public boolean recordNonNullByDefault(Binding target, int value, Annotation annotation, int scopeStart,
        int scopeEnd) {
    ...
    // Some entity in the receiver scope is referencing instance data of enclosing type. Tag all intervening methods as
    // instance methods.
    public void tagAsAccessingEnclosingInstanceStateOf(ReferenceBinding enclosingType, boolean typeVariableAccess) {
        ...
        if (methodScope != null && methodScope.referenceContext instanceof TypeDeclaration) {
            if (!methodScope.enclosingReceiverType().isCompatibleWith(enclosingType)) { // unless invoking a method of
                                                                                        // the local type ...
                // anonymous type, find enclosing method
                methodScope = methodScope.enclosingMethodScope();
            }
        }
        boolean isFlexibleConstructorsEnabled
            = methodScope != null && JavaFeature.FLEXIBLE_CONSTRUCTOR_BODIES.isSupported(methodScope.compilerOptions());
            ...
            while (methodScope != null && methodScope.referenceContext instanceof LambdaExpression) {
                ...
                if (methodScope.isConstructorCall || insideEarlyConstructionContext) {
                    ...
                    while ((tmp = tmp.enclosingType()) != null) {
                        if (!TypeBinding.equalsEquals(enclosingType, tmp))
                            continue;
                        lambda.ensureSyntheticOuterAccess((SourceTypeBinding) enclosingType);
                        lambda.hasOuterClassMemberReference = true; // ref to Outer class members allowed - interpreting
                                                                    // 8.8.7.1
                        break;
                    }
                }
                if (!typeVariableAccess
                    && !lambda.scope.isStatic
                    && !lambda.hasOuterClassMemberReference
                    && !insideEarlyConstructionContext)
                    lambda.shouldCaptureInstance = true;  // lambda can still be static, only when `this' is touched
                                                          // (implicitly or otherwise) it cannot be.
                methodScope = methodScope.enclosingMethodScope();
            }
            if (methodScope != null) {
                AbstractMethodDeclaration methodDecl = methodScope.referenceMethod();
                ...
                if (enclosingClassScope != null) {
                    ...
                    if (type != null
                        && type.binding != null
                        && enclosingType != null
                        && !type.binding.isCompatibleWith(enclosingType.original())) {
    ...
    private Map<String, Supplier<ReferenceBinding>> initializeCommonTypeBindings() {
        ...
        t.put(new String(ConstantPool.JavaLangIncompatibleClassChangeErrorConstantPoolName),
            this::getJavaLangIncompatibleClassChangeError);
        t.put(new String(ConstantPool.JavaLangNoClassDefFoundErrorConstantPoolName),
            this::getJavaLangNoClassDefFoundError);
            ...
            while (currentEnclosing != null) {
                ...
                while (currentTarget != null) {
                    ...
                    if (!considerEnclosings
                        || (currentTarget instanceof ReferenceBinding currentRefBind
                            && !currentRefBind.hasEnclosingInstanceContext())) {
                        break;
                    }
