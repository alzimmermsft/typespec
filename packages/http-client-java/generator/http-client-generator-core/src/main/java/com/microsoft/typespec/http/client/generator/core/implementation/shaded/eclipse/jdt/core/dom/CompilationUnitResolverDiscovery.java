...
class CompilationUnitResolverDiscovery {
    ...
    static ICompilationUnitResolver getInstance() {
        ...
        if (configElement != null) {
            try {
                ...
                if (!setErrorLogged()) {
                    ILog.get()
                        .error("Could not instantiate ICompilationUnitResolver: '" + id + "' with class: " //$NON-NLS-1$ //$NON-NLS-2$
    ...
    private static IConfigurationElement getConfigurationElement(String id) {
        ...
        IExtensionPoint extension = Platform.getExtensionRegistry()
            .getExtensionPoint(JavaCore.PLUGIN_ID, COMPILATION_UNIT_RESOLVER_EXTPOINT_ID);
        if (extension != null) {
