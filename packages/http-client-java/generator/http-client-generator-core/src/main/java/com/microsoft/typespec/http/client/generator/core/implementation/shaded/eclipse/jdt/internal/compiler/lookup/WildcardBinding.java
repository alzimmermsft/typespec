...
public class WildcardBinding extends ReferenceBinding implements HotSwappable {
    ...
    public int rank;
    public TypeBinding bound; // when unbound denotes the corresponding type variable (so as to retrieve its bound
                              // lazily)
    public TypeBinding[] otherBounds; // only positionned by lub computations (if so, #bound is also set) and associated
                                      // to EXTENDS mode
     ...
     */
    public WildcardBinding(ReferenceBinding genericType, int rank, TypeBinding bound, TypeBinding[] otherBounds,
        int boundKind, LookupEnvironment environment) {
        this.rank = rank;
        this.boundKind = boundKind;
        this.modifiers = ClassFileConstants.AccPublic | ExtraCompilerModifiers.AccGenericSignature; // treat wildcard as
                                                                                                    // public
     ...
     * @param wildcard (may be null. if non-null, errors are reported and type annotations are dropped from this.bound
                ...
                if (this.boundKind == Wildcard.SUPER) {
                    if ((boundNullTagBits & TagBits.AnnotationNullable) != 0) {
                        if (nullTagBits == 0L) {
                            ...
                            if (annotation == null) { // false alarm, implicit annotation is no conflict, but should be
                                                      // removed:
                                // may not be reachable, how could we have an implicit @Nullable (not via
                                // @NonNullByDefault)?
...
//						}
                    ...
                    case Wildcard.EXTENDS: // F={? extends U}
                        switch (actualType.kind()) {
                            ...
                            case Binding.INTERSECTION_TYPE: // A={? extends V1&...&Vn} << F={? extends U} ---> V1 << U,
                                                            // ..., Vn << U
                                WildcardBinding actualIntersection = (WildcardBinding) actualType;
                                this.bound.collectSubstitutes(scope, actualIntersection.bound, inferenceContext,
                                    TypeConstants.CONSTRAINT_EXTENDS);
                                for (TypeBinding otherBound : actualIntersection.otherBounds) {
                                    this.bound.collectSubstitutes(scope, otherBound, inferenceContext,
                                        TypeConstants.CONSTRAINT_EXTENDS);
                                }
                                break;

                            default: // A=V << F={? extends U} ---> V << U
                                this.bound.collectSubstitutes(scope, actualType, inferenceContext,
                                    TypeConstants.CONSTRAINT_EXTENDS);
                                break;
                        }
                        break;
                            ...
                            case Binding.WILDCARD_TYPE:
                                ...
                                switch (actualWildcard.boundKind) {
                                    ...
                                    case Wildcard.SUPER: // A={? super V} << F={? super U} ---> 0
                                        ...
                                        for (int i = 0, length = actualWildcard.otherBounds == null
                                            ? 0
                                            : actualWildcard.otherBounds.length; i < length; i++) {
                                            this.bound.collectSubstitutes(scope, actualWildcard.otherBounds[i],
                    ...
                    case Wildcard.EXTENDS: // F={? extends U}
                        switch (actualType.kind()) {
                                ...
                                switch (actualWildcard.boundKind) {
                                    ...
                                    case Wildcard.EXTENDS: // A={? extends V} == F={? extends U} ---> V == U
                                        ...
                                        for (int i = 0, length = actualWildcard.otherBounds == null
                                            ...
                                            : actualWildcard.otherBounds.length; i < length; i++) {
                                            this.bound.collectSubstitutes(scope, actualWildcard.otherBounds[i],
                                                inferenceContext, TypeConstants.CONSTRAINT_EQUAL);
                                        }
                                        break;
                            ...
                            case Binding.INTERSECTION_TYPE: // A={? extends V1&...&Vn} == F={? extends U} ---> V1 == U,
                                ...
                                this.bound.collectSubstitutes(scope, actuaIntersection.bound, inferenceContext,
                                    TypeConstants.CONSTRAINT_EQUAL);
                                for (int i = 0, length
                                    = actuaIntersection.otherBounds == null ? 0 : actuaIntersection.otherBounds.length;
                                    i < length; i++) {
                                    this.bound.collectSubstitutes(scope, actuaIntersection.otherBounds[i],
                                        inferenceContext, TypeConstants.CONSTRAINT_EQUAL);
                                }
                                break;
...
//						}
                    ...
                    case Wildcard.EXTENDS: // F={? extends U}
                        switch (actualType.kind()) {
                            ...
                            case Binding.INTERSECTION_TYPE: // A={? extends V1&...&Vn} >> F={? extends U} ---> V1 >> U,
                                ...
                                this.bound.collectSubstitutes(scope, actualIntersection.bound, inferenceContext,
                                    TypeConstants.CONSTRAINT_SUPER);
                                for (int i = 0, length = actualIntersection.otherBounds == null
                                    ? 0
                                    : actualIntersection.otherBounds.length; i < length; i++) {
                                    this.bound.collectSubstitutes(scope, actualIntersection.otherBounds[i],
                                        inferenceContext, TypeConstants.CONSTRAINT_SUPER);
                                }
                                break;
    ...
    public char[] computeUniqueKey(boolean isLeaf) {
        ...
        switch (this.boundKind) {
            ...
            case Wildcard.EXTENDS:
                wildCardKey = CharOperation.concat(TypeConstants.WILDCARD_PLUS,
                    this.bound.computeUniqueKey(false/* not a leaf */));
                break;

            default: // SUPER
                wildCardKey = CharOperation.concat(TypeConstants.WILDCARD_MINUS,
                    this.bound.computeUniqueKey(false/* not a leaf */));
                break;
        }
    ...
    public TypeBinding clone(TypeBinding immaterial) {
        return new WildcardBinding(this.genericType, this.rank, this.bound, this.otherBounds, this.boundKind,
            this.environment);
    }
        ...
        switch (this.boundKind) {
            ...
            case Wildcard.EXTENDS:
                if (this.otherBounds == null)
                    return buffer.append(CharOperation.concat(TypeConstants.WILDCARD_NAME,
                        TypeConstants.WILDCARD_EXTENDS, this.bound.annotatedDebugName().toCharArray())).toString();
            ...
            default: // SUPER
                return buffer.append(CharOperation.concat(TypeConstants.WILDCARD_NAME, TypeConstants.WILDCARD_SUPER,
                    this.bound.annotatedDebugName().toCharArray())).toString();
        }
    }
    ...
    public TypeBinding erasure() {
        if (this.otherBounds == null) {
            ...
            if (var != null)
                return var.erasure();
            return this.genericType; // if typeVariable() == null, then its inconsistent & return this.genericType to
                                     // avoid NPE case
        }
        // intersection type
    ...
    TypeBinding substituteInferenceVariable(InferenceVariable var, TypeBinding substituteType) {
                ...
                if (currentOtherBound != null) {
                    ...
                    if (TypeBinding.notEquals(currentOtherBound, this.otherBounds[i])) {
                        if (currentOtherBounds == null)
                            System.arraycopy(this.otherBounds, 0, currentOtherBounds = new ReferenceBinding[length], 0,
                                length);
        ...
        if (haveSubstitution) {
            WildcardBinding wildcard = this.environment.createWildcard(this.genericType, this.rank, currentBound,
                currentOtherBounds, this.boundKind);
    ...
    public char[] readableName() {
        switch (this.boundKind) {
            ...
            case Wildcard.EXTENDS:
                if (this.otherBounds == null)
                    return CharOperation.concat(TypeConstants.WILDCARD_NAME, TypeConstants.WILDCARD_EXTENDS,
                        this.bound.readableName());
            ...
            default: // SUPER
                return CharOperation.concat(TypeConstants.WILDCARD_NAME, TypeConstants.WILDCARD_SUPER,
                    this.bound.readableName());
        }
    }
    ...
    ReferenceBinding resolve() {
        ...
        switch (this.boundKind) {
            case Wildcard.EXTENDS:
                ...
                this.tagBits
                    |= resolveType.tagBits & TagBits.ContainsNestedTypeReferences | TagBits.HasCapturedWildcard;
                for (int i = 0, length = this.otherBounds == null ? 0 : this.otherBounds.length; i < length; i++) {
                    resolveType = BinaryTypeBinding.resolveType(this.otherBounds[i], this.environment,
                        true /* raw conversion */);
        ...
        if (this.superInterfaces == null) {
            ...
            if (this.boundKind == Wildcard.EXTENDS) {
                if (this.bound.isInterface()) {
                    ...
                    System.arraycopy(this.superInterfaces, 0, this.superInterfaces = new ReferenceBinding[length + 1],
                        1, length);
                ...
                if (this.otherBounds != null) {
                    ...
                    System.arraycopy(this.superInterfaces, 0,
                        this.superInterfaces = new ReferenceBinding[length + otherLength], 0, length);
    ...
    public String toString() {
        ...
        switch (this.boundKind) {
            ...
            case Wildcard.EXTENDS:
                if (this.otherBounds == null)
                    return new String(CharOperation.concat(TypeConstants.WILDCARD_NAME, TypeConstants.WILDCARD_EXTENDS,
            ...
            default: // SUPER
                return new String(CharOperation.concat(TypeConstants.WILDCARD_NAME, TypeConstants.WILDCARD_SUPER,
                    this.bound.debugName().toCharArray()));
        }
    }
    ...
    public TypeBinding withoutToplevelNullAnnotation() {
        ...
        return this.environment.createWildcard(this.genericType, this.rank, this.bound, this.otherBounds,
            this.boundKind, newAnnotations);
    }
    ...
    public TypeBinding uncapture(Scope scope) {
        ...
        int length = 0;
        TypeBinding[] freeOtherBounds
            = this.otherBounds == null ? null : new TypeBinding[length = this.otherBounds.length];
        for (int i = 0; i < length; i++) {
            freeOtherBounds[i] = this.otherBounds[i] == null ? null : this.otherBounds[i].uncapture(scope);
        }
        return scope.environment()
            .createWildcard(this.genericType, this.rank, freeBound, freeOtherBounds, this.boundKind,
                getTypeAnnotations());
    }
    ...
    TypeBinding propagateNonConflictingNullAnnotations(TypeBinding type) {
        ...
        AnnotationBinding[] annots
            = this.environment.nullAnnotationsFromTagBits(this.tagBits & TagBits.AnnotationNullMASK);
        if (annots == null)
            return type;
