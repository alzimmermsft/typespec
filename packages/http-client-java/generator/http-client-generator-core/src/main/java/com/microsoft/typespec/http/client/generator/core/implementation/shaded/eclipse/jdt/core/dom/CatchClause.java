...
public class CatchClause extends ASTNode {
    ...
    public static final ChildPropertyDescriptor EXCEPTION_PROPERTY = new ChildPropertyDescriptor(CatchClause.class,
        "exception", SingleVariableDeclaration.class, MANDATORY, CYCLE_RISK); //$NON-NLS-1$
    ...
    ASTNode clone0(AST target) {
        ...
        result.setException((SingleVariableDeclaration) ASTNode.copySubtree(target, getException()));
        return result;
    }
    ...
    @Override
    int treeSize() {
        return memSize() + (this.exceptionDecl == null ? 0 : getException().treeSize())
