...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.google.common.base.Strings;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.google.common.collect.TreeRangeMap;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.javax.tools.DiagnosticCollector;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.javax.tools.SimpleJavaFileObject;
...
public final class StringWrapper {
...

        {
            ...
            if (!expected.equals(actual)) {
                throw new FormatterException(
                    String.format("Something has gone terribly wrong. We planned to make the below formatting change,"
    ...
    private static class Reflower {
        ...
        private void indentTextBlocks(TreeRangeMap<Integer, String> replacements, List<Tree> textBlocks) {
            for (Tree tree : textBlocks) {
                ...
                boolean deindent
                    = getLast(initialLines).stripTrailing().length() == getLast(lines).stripTrailing().length();
                ...
                for (int i = 0; i < lines.size(); i++) {
                    ...
                    if (i == lines.size() - 1) {
                        String withoutDelimiter
                            = trimmed.substring(0, trimmed.length() - TEXT_BLOCK_DELIMITER.length()).stripTrailing();
     ...
     */
    private static ImmutableList<String> stringComponents(String input, JCTree.JCCompilationUnit unit,
        List<Tree> flat) {
     ...
     */
    private static String reflow(String separator, int columnLimit, int startColumn, int trailing,
        ImmutableList<String> components, boolean first0) {
        ...
        return lines.stream()
            .collect(
                joining("\"" + separator + Strings.repeat(" ", startColumn + (first0 ? 4 : -2)) + "+ \"", "\"", "\""));
    }
     ...
     */
    private static List<Tree> flatten(String input, JCTree.JCCompilationUnit unit, TreePath path, TreePath parent,
    ...
    private static boolean noComments(String input, JCTree.JCCompilationUnit unit, Tree one, Tree two) {
        return STRING_CONCAT_DELIMITER
            .matchesAllOf(input.subSequence(getEndPosition(unit, one), getStartPosition(two)));
    }
    ...
    private static JCTree.JCCompilationUnit parse(String source, boolean allowStringFolding) throws FormatterException {
        ...
        JavacParser parser = parserFactory.newParser(source, /* keepDocComments= */ true, /* keepEndPos= */ true,
            /* keepLineMap= */ true);
