...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.SwitchStatement;
...
public class SyntheticMethodBinding extends MethodBinding {
    ...
    public SyntheticMethodBinding(FieldBinding targetField, boolean isReadAccess, boolean isSuperAccess,
        ...
        int methodId = knownAccessMethods == null ? 0 : knownAccessMethods[knownAccessMethods.length - 1].index + 1; // index
                                                                                                                     // may
                                                                                                                     // miss
                                                                                                                     // some
                                                                                                                     // numbers
                                                                                                                     ...
                                                                                                                     // number.;
        this.index = methodId;
        this.selector = CharOperation.concat(TypeConstants.SYNTHETIC_ACCESS_METHOD_PREFIX,
        ...
        if (isReadAccess) {
            ...
            this.purpose
                = isSuperAccess ? SyntheticMethodBinding.SuperFieldReadAccess : SyntheticMethodBinding.FieldReadAccess;
        } else {
            ...
            this.purpose = isSuperAccess
                ? SyntheticMethodBinding.SuperFieldWriteAccess
                : SyntheticMethodBinding.FieldWriteAccess;
        }
        ...
        do {
            ...
            if (needRename) { // retry with a selector postfixed by a growing methodId
                setSelector(CharOperation.concat(TypeConstants.SYNTHETIC_ACCESS_METHOD_PREFIX,
                    String.valueOf(++methodId).toCharArray()));
            }
        } while (needRename);
        ...
        this.sourceStart = declaringSourceType.scope.referenceContext.sourceStart; // use the target declaring class
    ...
    public SyntheticMethodBinding(FieldBinding targetField, ReferenceBinding declaringClass, TypeBinding enumBinding,
        char[] selector, SwitchStatement switchStatement) {
        this.modifiers = (declaringClass.isInterface() ? ClassFileConstants.AccPublic : ClassFileConstants.AccDefault)
            | ClassFileConstants.AccStatic | ClassFileConstants.AccSynthetic;
        this.extendedTagBits |= ExtendedTagBits.AllAnnotationsResolved;
        SourceTypeBinding declaringSourceType = (SourceTypeBinding) declaringClass;
        SyntheticMethodBinding[] knownAccessMethods = declaringSourceType.syntheticMethods();
        int methodId = knownAccessMethods == null ? 0 : knownAccessMethods[knownAccessMethods.length - 1].index + 1; // index
    ...
    public SyntheticMethodBinding(MethodBinding overridenMethodToBridge, MethodBinding targetMethod,
        ...
        // amongst other, clear the AccGenericSignature, so as to ensure no remains of original inherited persist
        // (101794)
        ...
        this.modifiers = (targetMethod.modifiers | ClassFileConstants.AccBridge | ClassFileConstants.AccSynthetic)
            & ~(ClassFileConstants.AccSynchronized | ClassFileConstants.AccAbstract | ClassFileConstants.AccNative
                | ClassFileConstants.AccFinal | ExtraCompilerModifiers.AccGenericSignature);
        this.extendedTagBits |= ExtendedTagBits.AllAnnotationsResolved;
    ...
    public SyntheticMethodBinding(SourceTypeBinding declaringClass, char[] selector) {
        ...
        this.modifiers = ClassFileConstants.AccPublic
            | (declaringClass.isEnum() ? ClassFileConstants.AccStatic : ClassFileConstants.AccFinal);
        this.extendedTagBits |= ExtendedTagBits.AllAnnotationsResolved;
    ...
    public SyntheticMethodBinding(MethodBinding overridenMethodToBridge, SourceTypeBinding declaringClass) {
        ...
        // (101794)
        this.modifiers
            = (overridenMethodToBridge.modifiers | ClassFileConstants.AccBridge | ClassFileConstants.AccSynthetic)
                & ~(ClassFileConstants.AccSynchronized | ClassFileConstants.AccAbstract | ClassFileConstants.AccNative
                    | ClassFileConstants.AccFinal | ExtraCompilerModifiers.AccGenericSignature);
        this.extendedTagBits |= ExtendedTagBits.AllAnnotationsResolved;
        this.returnType = overridenMethodToBridge.returnType;
    ...
    public SyntheticMethodBinding(int purpose, ArrayBinding arrayType, char[] selector,
        SourceTypeBinding declaringClass) {
        this.declaringClass = declaringClass;
        this.selector = selector;
        ...
        if (environment.globalOptions.isAnnotationBasedNullAnalysisEnabled) {
            // mark X[]::new and X[]::clone as returning 'X @NonNull' (don't wait (cf. markNonNull()), because we're
            // called as late as codeGen):
            if (environment.usesNullTypeAnnotations())
                this.returnType = environment.createNonNullAnnotatedType(this.returnType);
            else
                this.tagBits |= TagBits.AnnotationNonNull;
        }
        this.parameters = new TypeBinding[] {
            purpose == SyntheticMethodBinding.ArrayConstructor ? TypeBinding.INT : (TypeBinding) arrayType };
        this.thrownExceptions = Binding.NO_EXCEPTIONS;
    ...
    public SyntheticMethodBinding(LambdaExpression lambda, char[] lambdaName, SourceTypeBinding declaringClass) {
        ...
        TypeVariableBinding[] vars
            = Stream.of(this.parameters).filter(TypeBinding::isTypeVariable).toArray(TypeVariableBinding[]::new);
    ...
    public SyntheticMethodBinding(MethodBinding privateConstructor, MethodBinding publicConstructor, char[] selector,
        TypeBinding[] enclosingInstances, SourceTypeBinding declaringClass) {
        this.declaringClass = declaringClass;
        this.selector = selector;
        this.modifiers = ClassFileConstants.AccSynthetic | ClassFileConstants.AccPrivate | ClassFileConstants.AccStatic;
        this.extendedTagBits |= ExtendedTagBits.AllAnnotationsResolved;
        ...
        System.arraycopy(privateConstructor.parameters, 0, this.parameters, enclosingInstancesLength,
            realParametersLength);
    ...
    public SyntheticMethodBinding(SourceTypeBinding declaringClass, RecordComponentBinding[] rcb) {
        this.declaringClass = declaringClass;
        this.modifiers = declaringClass.modifiers
        ...
        boolean useFlowAnnotations = env.globalOptions.isAnnotationBasedNullAnalysisEnabled
            || env.globalOptions.isAnnotationBasedResourceAnalysisEnabled;
        ...
        for (int i = 0, length = rcb.length; i < length; i++) {
            ...
            if (useFlowAnnotations) {
                AbstractMethodDeclaration.computeParamFlowBits(this, length, rcb[i].tagBits, i,
                    usesNullTypeAnnotations);
            }

            List<AnnotationBinding> relevantAnnotationBindings = new ArrayList<>();
            Annotation[] relevantAnnotations = ASTNode.getRelevantAnnotations(
                rcb[i].sourceRecordComponent().annotations, TagBits.AnnotationForParameter, relevantAnnotationBindings);
    ...
    public void initializeConstructorAccessor(MethodBinding accessedConstructor) {
        ...
        SyntheticMethodBinding[] knownSyntheticMethods = sourceType.syntheticMethods();   // returns synthetic methods
                                                                                          // sorted with index.
        this.index
            = knownSyntheticMethods == null ? 0 : knownSyntheticMethods[knownSyntheticMethods.length - 1].index + 1; // index
                                                                                                                     // may
                                                                                                                     // miss
                                                                                                                     // some
                                                                                                                     // numbers
    ...
    public void initializeMethodAccessor(MethodBinding accessedMethod, boolean isSuperAccess,
        ...
        if (isSuperAccess && receiverType.isInterface() && !accessedMethod.isStatic())
            ...
            if (receiverType.isInterface()) // default is not allowed. TODO: do we need a target level check here?
                this.modifiers
                    = ClassFileConstants.AccPublic | ClassFileConstants.AccStatic | ClassFileConstants.AccSynthetic;
            else
                this.modifiers
                ...
                if (knownAccessMethods != null) {
                    for (int i = 0, length = knownAccessMethods.length; i < length; i++) {
                        if (knownAccessMethods[i] == null)
                            continue;
                        if (CharOperation.equals(this.selector, knownAccessMethods[i].selector)
                            && areParameterErasuresEqual(knownAccessMethods[i])) {
                            needRename = true;
                            break check;
                        }
                    }
    ...
    public boolean hasNonNullDefaultForReturnType(AbstractMethodDeclaration srcMethod) {
        if (this.purpose == RecordComponentReadAccess)
            return hasNonNullDefaultForType(this.returnType, Binding.DefaultLocationRecordComponent, srcMethod,
        ...
        switch (purpose) {
            case EnumValues:
                if (environment.usesNullTypeAnnotations()) {
                    ...
                    method.returnType = environment.createArrayType(elementType, 1,
                        new AnnotationBinding[] { nonNullAnnotation, null });
                } else {
            ...
            for (AnnotationBinding annot : annotations) {
                if ((annot.getAnnotationType().id == TypeIds.T_JavaLangSafeVarargs)) {
                    scope.problemReporter()
                        .safeVarargsOnOnSyntheticRecordAccessor(this.recordComponentBinding.sourceRecordComponent());
                }
            }
        }
