...
public class ArrayInitializer extends Expression {
        ...
        if (this.expressions != null) {
            ...
            for (Expression expression : this.expressions) {
                ...
                if (analyseResources && FakedTrackingVariable.isAnyCloseable(expression.resolvedType)) {
                    flowInfo = FakedTrackingVariable.markPassedToOutside(currentScope, expression, flowInfo,
                        flowContext, false);
                }
                if (evalNullTypeAnnotations) {
                    checkAgainstNullTypeAnnotation(currentScope, this.binding.elementsType(), expression, flowContext,
                        flowInfo);
                }
            }
        }
     ...
     */
    public void generateCode(TypeReference typeReference, ArrayAllocationExpression allocationExpression,
    ...
    public TypeBinding resolveTypeExpecting(BlockScope scope, TypeBinding expectedType) {
        ...
        if (expectedType instanceof ArrayBinding) {
            // allow new List<?>[5]
            if ((this.bits & IsAnnotationDefaultValue) == 0) { // annotation default value need only to be commensurate
                                                               // JLS9.7
                // allow new List<?>[5] - only check for generic array when no initializer, since also checked inside
            ...
            for (Expression expression : this.expressions) {
                ...
                if (TypeBinding.notEquals(elementType, expressionType)) // must call before computeConversion() and
                                                                        // typeMismatchError()
