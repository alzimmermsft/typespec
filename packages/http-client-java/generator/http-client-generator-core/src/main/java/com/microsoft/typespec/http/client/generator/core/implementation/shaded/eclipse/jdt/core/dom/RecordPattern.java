...
public class RecordPattern extends Pattern {
     ...
     */
    private final NodeList patterns = new NodeList(PATTERNS_PROPERTY);
        ...
        if (this.patternType == null) {
            ...
            synchronized (this) {
                if (this.patternType == null) {
                    preLazyInit();
                    this.patternType
                        = postLazyInit(this.ast.newPrimitiveType(PrimitiveType.INT), PATTERN_TYPE_PROPERTY);
                }
            }
        }
    ...
    @Override
    int treeSize() {
        return memSize() + (this.patternType == null ? 0 : getPatternType().treeSize()) + this.patterns.listSize();
    }
