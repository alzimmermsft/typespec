...
public class FinallyFlowContext extends TryFlowContext {
        ...
        if ((this.tagBits & FlowContext.DEFER_NULL_DIAGNOSTIC) != 0) { // within an enclosing loop, be conservative
            for (int i = 0; i < this.nullCount; i++) {
                ...
                switch (this.nullCheckTypes[i] & ~HIDE_NULL_COMPARISON_WARNING_MASK) {
                    ...
                    default:
                        this.parent.recordUsingNullReference(scope, this.nullLocals[i], this.nullReferences[i],
                ...
                switch (checkTypeWithoutHideNullWarning) {
                    ...
                    case CAN_ONLY_NULL | IN_INSTANCEOF:
                        ...
                        if (flowInfo.cannotBeNull(local)) {
                            ...
                            } else if (checkTypeWithoutHideNullWarning
                            ...
                            switch (checkTypeWithoutHideNullWarning & CONTEXT_MASK) {
                                case FlowContext.IN_COMPARISON_NULL:
                                    if (((checkTypeWithoutHideNullWarning & CHECK_MASK) == CAN_ONLY_NULL)
                                        && (reference.implicitConversion & TypeIds.UNBOXING) != 0) { // check for
                                                                                                     // auto-unboxing
                                                                                                     // first and report
                                                                                                     // appropriate
                                                                                                     // warning
    ...
    protected void recordNullReferenceWithAnnotationStatus(LocalVariableBinding local, ASTNode expression,
        ...
        if (this.nullCount == 0) {
            ...
            System.arraycopy(this.nullLocals, 0, this.nullLocals = new LocalVariableBinding[newLength], 0,
                this.nullCount);
