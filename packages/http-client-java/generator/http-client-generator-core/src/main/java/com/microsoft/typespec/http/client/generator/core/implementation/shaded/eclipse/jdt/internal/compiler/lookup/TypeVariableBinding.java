...
public class TypeVariableBinding extends ReferenceBinding {
    ...
    public TypeVariableBinding(char[] sourceName, Binding declaringElement, int rank, LookupEnvironment environment) {
        ...
        this.rank = rank;
        this.modifiers = ClassFileConstants.AccPublic | ExtraCompilerModifiers.AccGenericSignature; // treat type var as
                                                                                                    // public
    ...
    public TypeVariableBinding(TypeVariableBinding prototype) {
        ...
        if (prototype.superInterfaces != null) {
            ...
            if (len > 0)
                System.arraycopy(prototype.superInterfaces, 0, this.superInterfaces = new ReferenceBinding[len], 0,
                    len);
            else
     ...
     */
    public BoundCheckStatus boundCheck(Substitution substitution, TypeBinding argumentType, Scope scope,
        ASTNode location) {
        BoundCheckStatus code = internalBoundCheck(substitution, argumentType, scope, location);
        if (code == BoundCheckStatus.MISMATCH) {
            if (argumentType instanceof TypeVariableBinding && scope != null) {
                ...
                if (bound instanceof ParameterizedTypeBinding) {
                    BoundCheckStatus code2 = boundCheck(substitution, bound.capture(scope, -1, -1), scope, location); // no
                                                                                                                      // capture
                                                                                                                      // position
                                                                                                                      // needed
                                                                                                                      // as
    ...
    private BoundCheckStatus internalBoundCheck(Substitution substitution, TypeBinding argumentType, Scope scope,
        ASTNode location) {
        ...
        // can request additional types during hierarchy walk that are found as source types that also 'need' to connect
        // their hierarchy
        if (this.superclass == null)
            return BoundCheckStatus.OK;

        BoundCheckStatus nullStatus = BoundCheckStatus.OK;
        boolean checkNullAnnotations = scope != null
            && scope.environment().usesNullTypeAnnotations()
            && (location == null || (location.bits & ASTNode.InsideJavadoc) == 0);
        ...
        if (argumentType.kind() == Binding.WILDCARD_TYPE) {
            ...
            switch (wildcard.boundKind) {
                case Wildcard.EXTENDS:
                    ...
                    if (!wildcardBound.isInterface()) {
                                ...
                                if (isArrayBound) {
                                    ...
                                    if (match != null) {
                                        ...
                                        if (match != null) {
                                            ...
                                            // (c) and then the glb should be checked for well-formedness (see
                                            // Scope.isMalformedPair() - this part missing in JLS).
                                            // Since we don't do (a), nor (b) for this case, we just directly proceed to
                                            // (b) here.
                                            // For (a) see ParameterizedTypeBinding.boundCheck() - comment added as of
                                            // this commit
                                            // for (b) see CaptureBinding.initializeBounds() - comment added as of this
                                            // commit
                                            if (Scope.greaterLowerBound(
                                                new TypeBinding[] { substitutedSuperType, wildcardBound }, scope,
                        ...
                        if (checkNullAnnotations && argumentType.hasNullTypeAnnotations()) {
                            nullStatus = nullBoundCheck(scope, argumentType, substitutedSuperType, substitution,
                                location, nullStatus);
                        }
                    }
                    ...
                    for (ReferenceBinding superInterface : this.superInterfaces) {
                        TypeBinding substitutedSuperType
                            = hasSubstitution ? Scope.substitute(substitution, superInterface) : superInterface;
                        if (!checkedAsOK) {
                            if (isArrayBound) {
                                if (!wildcardBound.isCompatibleWith(substitutedSuperType, scope))
                                    return BoundCheckStatus.MISMATCH;
                                ...
                                if (match != null) {
                                    ...
                                    return BoundCheckStatus.MISMATCH; // cannot be extended further to satisfy missing
                                                                      // bounds
                                }
                            }
                        }
                ...
                case Wildcard.SUPER:
                    // if the wildcard is lower-bounded by a type variable that has no relevant upper bound there's
                    // nothing to check here (bug 282152):
                    if (wildcard.bound.isTypeVariable()
                        ...
                        if (checkNullAnnotations
                            && this.environment.containsNullTypeAnnotation(wildcard.typeAnnotations))
                            bound = this.environment.createAnnotatedType(bound.withoutToplevelNullAnnotation(),
                                wildcard.getTypeAnnotations());
                        BoundCheckStatus status = boundCheck(substitution, bound, scope, null); // do not report
                                                                                                // null-errors against
                                                                                                // the tweaked bound ...
                        if (status == BoundCheckStatus.NULL_PROBLEM && location != null)
                            scope.problemReporter().nullityMismatchTypeArgument(this, wildcard, location); // ... but
                                                                                                           // against
                                                                                                           // the
                                                                                                           // wildcard
                        return status;
            ...
            return BoundCheckStatus.OK;
        } else if (checkNullAnnotations
            && argumentType.kind() == Binding.TYPE_PARAMETER
            && !argumentType.hasNullTypeAnnotations()) {
        ...
        if (this.superclass.id != TypeIds.T_JavaLangObject) {
            ...
            if (checkNullAnnotations) {
                nullStatus
                    = nullBoundCheck(scope, argumentType, substitutedSuperType, substitution, location, nullStatus);
            }
        }
        for (ReferenceBinding superInterface : this.superInterfaces) {
            TypeBinding substitutedSuperType
    ...
    private BoundCheckStatus nullBoundCheck(Scope scope, TypeBinding argumentType, TypeBinding substitutedSuperType,
        Substitution substitution, ASTNode location, BoundCheckStatus previousStatus) {
        NullAnnotationMatching status = NullAnnotationMatching.analyse(this, argumentType, substitutedSuperType,
            substitution, -1, null, CheckMode.BOUND_CHECK);
    ...
    TypeBinding substituteInferenceVariable(InferenceVariable var, TypeBinding substituteType) {
        ...
        try {
            ...
            if (this.superInterfaces != null) {
                int length = this.superInterfaces.length;
                if (haveSubstitution)
                    System.arraycopy(this.superInterfaces, 0, currentSuperInterfaces = new ReferenceBinding[length], 0,
                        length);
                    ...
                    if (currentSuperInterface != null) {
                        ...
                        if (TypeBinding.notEquals(currentSuperInterface, this.superInterfaces[i])) {
                            if (currentSuperInterfaces == null)
                                System.arraycopy(this.superInterfaces, 0,
                                    currentSuperInterfaces = new ReferenceBinding[length], 0, length);
            ...
            if (haveSubstitution) {
                TypeVariableBinding newVar
                    = new TypeVariableBinding(this.sourceName, this.declaringElement, this.rank, this.environment);
...
//	}
    ...
    ReferenceBinding resolve() {
        ...
        if (this.superclass != null) {
            ReferenceBinding resolveType = (ReferenceBinding) BinaryTypeBinding.resolveType(this.superclass,
...
//					System.err.println("TODO(stephan): report proper error: conflict binary TypeVariable vs. first bound");
        ...
        if ((length = interfaces.length) != 0) {
            ...
            for (int i = length; --i >= 0;) {
                ReferenceBinding resolveType = (ReferenceBinding) BinaryTypeBinding.resolveType(interfaces[i],
                    this.environment, true /* raw conversion */);
                this.tagBits |= resolveType.tagBits & (TagBits.ContainsNestedTypeReferences | TagBits.HasMissingType);
                long superNullTagBits = resolveType.tagBits & TagBits.AnnotationNullMASK;
                if (superNullTagBits != 0L) {
...
//						System.err.println("TODO(stephan): report proper error: conflict binary TypeVariable vs. bound "+i);
    ...
    public void setTypeAnnotations(AnnotationBinding[] annotations, boolean evalNullAnnotations) {
        if (getClass() == TypeVariableBinding.class && this.environment.globalOptions.storeAnnotations) {
            ...
            this.environment.getUnannotatedType(this); // exposes original TVB/capture to type system for id stamping
                                                       // purposes.
        }
            ...
            try {
                if (this.superclass != null && TypeBinding.equalsEquals(this.firstBound, this.superclass)) {
                    nameBuffer.append(" extends ") //$NON-NLS-1$
                        .append(this.superclass.nullAnnotatedReadableName(options, shortNames));
                }
                if (this.superInterfaces != null && this.superInterfaces != Binding.NO_SUPERINTERFACES) {
                    if (TypeBinding.notEquals(this.firstBound, this.superclass)) {
                        nameBuffer.append(" extends "); //$NON-NLS-1$
            ...
            for (int i = length; --i >= 0;) {
                ...
                if (superNullTagBits != 0L) {
                    if (nullTagBits == 0L) {
                        ...
                        if (parameter != null)
                            interfaces[i] = (ReferenceBinding) nullMismatchOnBound(parameter, resolveType,
                                superNullTagBits, nullTagBits, scope);
                    }
                }
            }
        }
    ...
    private TypeBinding nullMismatchOnBound(TypeParameter parameter, TypeBinding boundType, long superNullTagBits,
        long nullTagBits, Scope scope) {
    ...
    private boolean hasRelevantTypeUseNullAnnotations() {
        ...
        if (this.declaringElement instanceof ReferenceBinding) {
            ...
            throw new IllegalStateException(
                "Unexpected declaring element:" + String.valueOf(this.declaringElement.readableName())); //$NON-NLS-1$
        }
        TypeVariableBinding parameter = parameters[this.rank];
        // recognize explicit annots by their effect on null tag bits, if there's no effect, then the annot is not
        // considered relevant
