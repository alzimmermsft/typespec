...
public class ClassScope extends Scope {
     ...
     * <p>Main access is via {@link Scope#enterEarlyConstructionContext()},
     * {@link Scope#leaveEarlyConstructionContext()}
    ...
    public void buildAnonymousTypeBinding(SourceTypeBinding enclosingType, ReferenceBinding supertype) {
        ...
        int inheritedBits = supertype.typeBits; // for anonymous class assume same properties as its super (as a
                                                // closeable) ...
    ...
    void buildFields() {
        ...
        FieldDeclaration[] fields
            = this.referenceContext.fields == null ? ASTNode.NO_FIELD_DECLARATIONS : this.referenceContext.fields;
        ...
        int i = -1;
        nextVariable: for (AbstractVariableDeclaration variableDeclaration : variableDeclarations) {
            ...
            // Create a field binding for the declared or derived field - its type will be patched up later while
            // resolving for field
            if (variableDeclaration instanceof FieldDeclaration field) {
                fieldBinding
                    = new FieldBinding(field, null, field.modifiers | ExtraCompilerModifiers.AccUnresolved, sourceType);
                ...
                TypeBinding type = componentDeclaration.binding != null
                    ? componentDeclaration.binding.type
                    : variableDeclaration.type.resolvedType;
                ...
                if (componentDeclaration.binding != null)
                    fieldBinding.tagBits |= componentDeclaration.binding.tagBits
                        & (TagBits.AnnotationNullMASK | TagBits.AnnotationOwningMASK);
            } else {
            ...
            if (knownFieldNames.containsKey(variableDeclaration.name)) {
                ...
                if (previousBinding != null) {
                    for (int f = 0; f < i; f++) {
                        ...
                        if (CharOperation.equals(previousField.name, variableDeclaration.name)) {
                            if (previousField.getKind() == AbstractVariableDeclaration.RECORD_COMPONENT
                                && variableDeclaration.getKind() == AbstractVariableDeclaration.RECORD_COMPONENT)
                                continue nextVariable; // already complained about duplicate components, don't also
                                                       // complain about fields
                            problemReporter().duplicateFieldInType(sourceType, previousField);
                            break;
                        }
                    }
                }
                knownFieldNames.put(variableDeclaration.name, null); // ensure that the duplicate field is found &
                                                                     // removed
                ...
                if (variableDeclaration instanceof RecordComponent componentDecl
                    && fieldBinding instanceof SyntheticFieldBinding)
        ...
        sourceType.tagBits &= ~(TagBits.AreFieldsSorted | TagBits.AreFieldsComplete); // in case some static imports
                                                                                      // reached already into this type
    ...
    void buildFieldsAndMethods() {
        ...
        if (!sourceType.isPrivate()
            && sourceType.superclass instanceof SourceTypeBinding
            && sourceType.superclass.isPrivate())
        ...
        if (this.referenceContext.memberTypes != null) {
            ...
            nextMember: for (int i = 0; i < length; i++) {
                ...
                if (this.environment().root.isProcessingAnnotations
                    && this.environment().isMissingType(memberContext.name)) {
    ...
    void buildMethods() {
        ...
        sourceType.tagBits &= ~(TagBits.AreMethodsSorted | TagBits.AreMethodsComplete); // in case some static imports
        ...
        if (isEnum && compilerOptions().isAnnotationBasedNullAnalysisEnabled) {
            // mark return types of values & valueOf as nonnull (needed to wait till after setMethods() to avoid
            // reentrance):
    ...
    SourceTypeBinding buildType(SourceTypeBinding enclosingType, PackageBinding packageBinding,
        ...
        if (compilationUnit != null && compilerOptions().isAnnotationBasedNullAnalysisEnabled) {
            String externalAnnotationPath
                = compilationUnit.getExternalAnnotationPath(CharOperation.toString(sourceType.compoundName));
        ...
        sourceType.typeVariables
            = typeParameters == null || typeParameters.length == 0 ? Binding.NO_TYPE_VARIABLES : null;
    ...
    private void checkAndSetModifiers() {
        ...
        boolean hierarchySealed
            = (modifiers & (ExtraCompilerModifiers.AccSealed | ExtraCompilerModifiers.AccNonSealed)) != 0;

        switch (modifiers
            & (ExtraCompilerModifiers.AccSealed | ExtraCompilerModifiers.AccNonSealed | ClassFileConstants.AccFinal)) {
            case ExtraCompilerModifiers.AccSealed, ExtraCompilerModifiers.AccNonSealed, ClassFileConstants.AccFinal, ClassFileConstants.AccDefault:
                break;
        ...
        if (isMemberType) {
            ...
            if (sourceType.isEnum()) {
                ...
                if ((modifiers & ExtraCompilerModifiers.AccJustFlag & UNEXPECTED_MODIFIERS) != 0 || hierarchySealed) {
            ...
            if (sourceType.isAnonymousType()) {
                ...
                if (this.referenceContext.allocation.type == null) {
                    ...
                    // 8.1.1.4 local enum classes are implicitly static - we can't trust isLocalType() which answers
                    // true for all anonymous types.
                    Scope scope = this;
        ...
        if ((realModifiers & ClassFileConstants.AccInterface) != 0) { // interface and annotation type
            ...
            if (isMemberType) {
                final int UNEXPECTED_MODIFIERS = ~(ClassFileConstants.AccPublic | ClassFileConstants.AccPrivate
                    | ClassFileConstants.AccProtected | ClassFileConstants.AccStatic | ClassFileConstants.AccAbstract
                    | ClassFileConstants.AccInterface | ClassFileConstants.AccStrictfp
...
//				}
                ...
                final int UNEXPECTED_MODIFIERS
                    = ~(ClassFileConstants.AccPublic | ClassFileConstants.AccAbstract | ClassFileConstants.AccInterface
                        | ClassFileConstants.AccStrictfp | ClassFileConstants.AccAnnotation);
                if ((realModifiers & UNEXPECTED_MODIFIERS) != 0) {
                    if ((realModifiers & ClassFileConstants.AccAnnotation) != 0)
            ...
            if (isMemberType) { // includes member types defined inside local types
                final int UNEXPECTED_MODIFIERS
                    = ~(ClassFileConstants.AccPublic | ClassFileConstants.AccPrivate | ClassFileConstants.AccProtected
                        | ClassFileConstants.AccStatic | ClassFileConstants.AccStrictfp | ClassFileConstants.AccEnum);
                if ((realModifiers & UNEXPECTED_MODIFIERS) != 0 || hierarchySealed) {
...
//					realModifiers = modifiers & ExtraCompilerModifiers.AccJustFlag;
                ...
                final int UNEXPECTED_MODIFIERS
                    = ~(ClassFileConstants.AccPublic | ClassFileConstants.AccStrictfp | ClassFileConstants.AccEnum);
                if ((realModifiers & UNEXPECTED_MODIFIERS) != 0 || hierarchySealed)
            ...
            if (isMemberType) {
                final int EXPECTED_MODIFIERS
                    = (ClassFileConstants.AccPublic | ClassFileConstants.AccPrivate | ClassFileConstants.AccProtected
                        | ClassFileConstants.AccStatic | ClassFileConstants.AccFinal | ClassFileConstants.AccStrictfp);
                ...
                final int EXPECTED_MODIFIERS
                    = (ClassFileConstants.AccFinal | ClassFileConstants.AccStrictfp | ClassFileConstants.AccStatic);
                if ((realModifiers & ~EXPECTED_MODIFIERS) != 0 || (modifiers & UNEXPECTED_MODIFIERS) != 0)
                    problemReporter().illegalModifierForLocalRecord(sourceType);
            } else {
                final int EXPECTED_MODIFIERS
                    = (ClassFileConstants.AccPublic | ClassFileConstants.AccFinal | ClassFileConstants.AccStrictfp);
                if ((realModifiers & ~EXPECTED_MODIFIERS) != 0 || (modifiers & UNEXPECTED_MODIFIERS) != 0)
            ...
            if (isMemberType) { // includes member types defined inside local types
                final int UNEXPECTED_MODIFIERS = ~(ClassFileConstants.AccPublic | ClassFileConstants.AccPrivate
                    | ClassFileConstants.AccProtected | ClassFileConstants.AccStatic | ClassFileConstants.AccAbstract
                    | ClassFileConstants.AccFinal | ClassFileConstants.AccStrictfp);
                if ((realModifiers & UNEXPECTED_MODIFIERS) != 0)
                    problemReporter().illegalModifierForMemberClass(sourceType);
            } else if (sourceType.isLocalType()) {
                final int UNEXPECTED_MODIFIERS
                    = ~(ClassFileConstants.AccAbstract | ClassFileConstants.AccFinal | ClassFileConstants.AccStrictfp
                        | ((is16Plus && this.parent instanceof ClassScope) ? ClassFileConstants.AccStatic : 0));
                if ((realModifiers & UNEXPECTED_MODIFIERS) != 0 || hierarchySealed)
                    problemReporter().illegalModifierForLocalClass(sourceType);
            } else {
                final int UNEXPECTED_MODIFIERS = ~(ClassFileConstants.AccPublic | ClassFileConstants.AccAbstract
                    | ClassFileConstants.AccFinal | ClassFileConstants.AccStrictfp);
            ...
            if ((realModifiers & (ClassFileConstants.AccFinal | ClassFileConstants.AccAbstract))
                == (ClassFileConstants.AccFinal | ClassFileConstants.AccAbstract))
...
//				else
    ...
    private void checkAndSetModifiersForField(FieldBinding fieldBinding, FieldDeclaration fieldDecl) {
        ...
        if (declaringClass.isInterface()) {
            ...
            final int IMPLICIT_MODIFIERS = ClassFileConstants.AccPublic | ClassFileConstants.AccStatic
                | ClassFileConstants.AccFinal | ClassFileConstants.AccEnum | ExtraCompilerModifiers.AccLocallyUsed;
        ...
        int realModifiers = modifiers & ExtraCompilerModifiers.AccJustFlag;
        final int UNEXPECTED_MODIFIERS = ~(ClassFileConstants.AccPublic | ClassFileConstants.AccPrivate
            | ClassFileConstants.AccProtected | ClassFileConstants.AccFinal | ClassFileConstants.AccStatic
            | ClassFileConstants.AccTransient | ClassFileConstants.AccVolatile);
        if ((realModifiers & UNEXPECTED_MODIFIERS) != 0) {
        ...
        if ((realModifiers & (ClassFileConstants.AccFinal | ClassFileConstants.AccVolatile))
            == (ClassFileConstants.AccFinal | ClassFileConstants.AccVolatile))
    ...
    public void checkParameterizedSuperTypeCollisions() {
        ...
        nextInterface: for (int i = 0, length = interfaces.length; i < length; i++) {
            ...
            if (itsSuperclass != null
                && hasErasedCandidatesCollisions(itsSuperclass, one, invocations, sourceType, this.referenceContext))
                continue nextInterface;
        ...
        TypeParameter[] typeParameters = this.referenceContext.typeParameters;
        nextVariable: for (int i = 0, paramLength = typeParameters == null ? 0 : typeParameters.length; i < paramLength;
            i++) {
            ...
            if (boundRefs != null) {
                ...
                for (TypeReference typeRef : boundRefs) {
                    ...
                    if (checkSuperclass)
                        if (hasErasedCandidatesCollisions(superType, typeVariable.superclass, invocations, typeVariable,
                            typeRef))
                            continue nextVariable;
                    // check against superinterfaces
                    for (int index = typeVariable.superInterfaces.length; --index >= 0;)
                        if (hasErasedCandidatesCollisions(superType, typeVariable.superInterfaces[index], invocations,
    ...
    private void checkForInheritedMemberTypes(SourceTypeBinding sourceType) {
        ...
        do {
            ...
            && (currentType.tagBits & TagBits.HasNoMemberTypes) == 0);

        if (interfacesToVisit != null) {
            // contains the interfaces between the sourceType and any superclass, which was tagged as having no member
            // types
            boolean needToTag = false;
            for (int i = 0; i < nextPosition; i++) {
                ReferenceBinding anInterface = interfacesToVisit[i];
                if ((anInterface.tagBits & TagBits.HasNoMemberTypes) == 0) { // skip interface if it already knows it
                                                                             // has no member types
    ...
    private boolean connectSuperclass() {
        ...
        if (sourceType.id == TypeIds.T_JavaLangObject) { // handle the case of redefining java.lang.Object up front
            ...
            if (this.referenceContext.superclass != null
                || (this.referenceContext.superInterfaces != null && this.referenceContext.superInterfaces.length > 0))
        ...
        if (superclass != null) { // is null if a cycle was detected cycle or a problem
            if (!superclass.isClass() && (superclass.tagBits & TagBits.HasMissingType) == 0) {
            ...
            } else if (superclass.isFinal() && !superclass.isSealed()) { // sealed AND final complained about elsewhere,
                                                                         // no value in additional complaint here.
                ...
                && !(sourceType.isFinal() || sourceType.isSealed() || sourceType.isNonSealed())) {
                sourceType.setSuperClass(superclass);
    ...
    private boolean connectEnumSuperclass() {
        ...
        if (refTypeVariables == Binding.NO_TYPE_VARIABLES) { // check generic
            problemReporter().nonGenericTypeCannotBeParameterized(0, null, rootEnumType,
                new TypeBinding[] { sourceType });
        ...
        ParameterizedTypeBinding superType = environment().createParameterizedType(rootEnumType,
            new TypeBinding[] {
    ...
    /*
     * Check that the permitted subtype and the sealed type are located in close proximity: either in the same module
     * (if the superclass is in a named module)
     ...
     */
    private boolean checkSealingProximity(ReferenceBinding subType, TypeReference subTypeReference,
        ReferenceBinding sealedType) {
        ...
        if (subType.getPackage() != sealedTypePackage) {
            if (sealedTypeModule.isUnnamed())
                problemReporter().permittedTypeOutsideOfPackage(subType, sealedType, subTypeReference,
                    sealedTypePackage);
    ...
    void connectPermittedTypes() {
        ...
        if (this.referenceContext.permittedTypes != null) {
            ...
            try {
                ...
                nextPermittedType: for (int i = 0; i < length; i++) {
                    ...
                    if (sourceType.isClass()) {
                        ...
                        if (!TypeBinding.equalsEquals(sourceType, superClass))
                            problemReporter().sealedClassNotDirectSuperClassOf(permittedType, permittedTypeRef,
                                sourceType);
                    } else if (sourceType.isInterface()) {
                        ...
                        if (superInterfaces != null) {
                            ...
                            if (!hierarchyOK)
                                problemReporter().sealedInterfaceNotDirectSuperInterfaceOf(permittedType,
                                    permittedTypeRef, sourceType);
                        }
                    }
                ...
                if (count > 0) {
                    if (count != length)
                        System.arraycopy(permittedTypeBindings, 0, permittedTypeBindings = new ReferenceBinding[count],
                            0, count);
            ...
            if (permittedTypes == null || permittedTypes.length == 0) {
                sourceType.setPermittedTypes(Binding.NO_PERMITTED_TYPES);
                if (sourceType.isSealed()) {
                    if (!sourceType.isLocalType() && !sourceType.isRecord() && !sourceType.isEnum()) // error flagged
                                                                                                     // alread
    ...
    private boolean connectSuperInterfaces() {
        ...
        try {
            ...
            nextInterface: for (int i = 0; i < length; i++) {
                ...
                // Check for a duplicate interface once the name is resolved, otherwise we may be confused (i.e. a.b.I
                // and c.d.I)
                for (int j = 0; j < i; j++) {
                ...
                if (superInterface.isSealed() && sourceType.isLocalType()) {
                    ...
                    noProblems = false;
                } else if (superInterface.isSealed()
                    && !(sourceType.isFinal() || sourceType.isSealed() || sourceType.isNonSealed())) {
            ...
            if (sourceType.isNonSealed() && !hasSealedSupertype) {
                if (!sourceType.isRecord()
                    ...
                    && !sourceType.isEnum()
                    && !sourceType.isSealed()) // avoid double jeopardy
    ...
    void collateRecordComponents() {
        ...
        if (sourceType.components() == null) {
            ...
            for (RecordComponent component : components) {
                RecordComponentBinding rcb = new RecordComponentBinding(sourceType, component, null,
            ...
            if (count != rcbs.length) // remove duplicate or broken components
                System.arraycopy(rcbs, 0, rcbs = count == 0 ? Binding.NO_COMPONENTS : new RecordComponentBinding[count],
                    0, count);
    ...
    private boolean detectHierarchyCycle(SourceTypeBinding sourceType, ReferenceBinding superType,
        ...
        if (superType.isBinaryBinding()) {
            // force its superclass & superinterfaces to be found... 2 possibilities exist - the source type is included
            // in the hierarchy of:
    ...
    private ReferenceBinding findSupertype(TypeReference typeReference) {
        ...
        try {
            ...
            if (sourceType.superInterfaces == null)
                sourceType.setSuperInterfaces(Binding.NO_SUPERINTERFACES); // be more resilient for hierarchies (144976)
    ...
    private ReferenceBinding findPermittedtype(TypeReference typeReference) {
        ...
        try {
            ...
            return permittedType != null ? permittedType.actualType() : permittedType; // while permitted
                                                                                       // classes/interfaces cannot be
                                                                                       // parameterized with type
                                                                                       // arguments, they are not raw
                                                                                       // either
        } catch (AbortCompilation e) {
            SourceTypeBinding sourceType = this.referenceContext.binding;
            if (sourceType.permittedTypes == null)
    ...
    public final MethodBinding enclosingMethod() {
        ...
        while ((scope = scope.parent) != null) {
            if (scope instanceof MethodScope) {
                MethodScope methodScope = (MethodScope) scope;
                /*
                 * 4.7.7 The EnclosingMethod Attribute: ... In particular, method_index must be zero if the current
                 * class
                 * was immediately enclosed in source code by an instance initializer, static initializer, instance
                 * variable initializer, or
