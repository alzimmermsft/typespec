...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ISourceElementRequestor.ParameterInfo;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ISourceElementRequestor.TypeParameterInfo;
...
public class SourceElementNotifier {
    ...
    private void gatherPermittedTypesOf(TypeDeclaration potentialSubtype, TypeDeclaration sealedType, List<char[]> list,
        char[] prefix) {
    ...
    protected char[][] getTypeParameterBounds(TypeParameter typeParameter) {
        ...
        if (firstBound != null) {
            if (otherBounds != null) {
                ...
                typeParameterBounds
                    = new char[][] { CharOperation.concatWith(firstBound.getParameterizedTypeName(), '.') };
            }
        } else {
    ...
    protected void notifySourceElementRequestor(AbstractMethodDeclaration methodDeclaration,
        ...
        // range check
        boolean isInRange = this.initialPosition <= methodDeclaration.declarationSourceStart
                ...
                if (constructorCall != null) {
                    switch (constructorCall.accessMode) {
                        case ExplicitConstructorCall.This:
                            this.requestor.acceptConstructorReference(this.typeNames[this.nestedTypeIndex - 1],
                        ...
                        case ExplicitConstructorCall.ImplicitSuper:
                            this.requestor.acceptConstructorReference(this.superTypeNames[this.nestedTypeIndex - 1],
                                constructorCall.arguments == null ? 0 : constructorCall.arguments.length,
                                constructorCall.sourceStart);
                            break;
                    }
        ...
        if (methodDeclaration.isConstructor()) {
            ...
            if (isInRange) {
                ...
                methodInfo.declaringPackageName = currentPackage == null
        ...
        if (isInRange) {
            int currentModifiers = methodDeclaration.modifiers;
            currentModifiers &= ExtraCompilerModifiers.AccJustFlag | ClassFileConstants.AccDeprecated
                | ClassFileConstants.AccAnnotationDefault | ExtraCompilerModifiers.AccDefaultMethod;
            if (isVarArgs)
                currentModifiers |= ClassFileConstants.AccVarargs;
            if (hasDeprecatedAnnotation(methodDeclaration.annotations))
                currentModifiers |= ClassFileConstants.AccDeprecated;
            ...
            methodInfo.declaringPackageName
                = currentPackage == null ? CharOperation.NO_CHAR : CharOperation.concatWith(currentPackage.tokens, '.');
        ...
        if (isInRange) {
            if (methodDeclaration instanceof AnnotationMethodDeclaration) {
                AnnotationMethodDeclaration annotationMethodDeclaration
                    = (AnnotationMethodDeclaration) methodDeclaration;
    ...
    public void notifySourceElementRequestor(CompilationUnitDeclaration parsedUnit, int sourceStart, int sourceEnd,
        ...
        try {
            // range check
            boolean isInRange
                = this.initialPosition <= parsedUnit.sourceStart && this.eofPosition >= parsedUnit.sourceEnd;
            ...
            length = (currentPackage == null ? 0 : 1) + (imports == null ? 0 : imports.length)
                + (types == null ? 0 : types.length) + (parsedUnit.moduleDeclaration == null ? 0 : 1);
            ...
            if (length > 0) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    if (node instanceof ImportReference) {
                        ...
                        && !new String(parsedUnit.getFileName()).endsWith(TypeConstants.MODULE_INFO_FILE_NAME_STRING)) {
     ...
     * Update the bodyStart of the corresponding parse node
     */
    protected void notifySourceElementRequestor(AbstractVariableDeclaration fieldDeclaration,
        TypeDeclaration declaringType) {

        // range check
        boolean isInRange = this.initialPosition <= fieldDeclaration.declarationSourceStart
        ...
        switch (fieldDeclaration.getKind()) {
            case AbstractVariableDeclaration.ENUM_CONSTANT:
                if (this.reportReferenceInfo) {
                    ...
                    if (fieldDeclaration.initialization instanceof AllocationExpression) {
                        ...
                        this.requestor.acceptConstructorReference(declaringType.name,
            ...
            case AbstractVariableDeclaration.INITIALIZER:
                if (isInRange) {
                    this.requestor.enterInitializer(fieldDeclaration.declarationSourceStart,
    ...
    protected void notifySourceElementRequestor(ImportReference importReference, boolean isPackage) {
        if (isPackage) {
            ...
            this.requestor.acceptImport(importReference.declarationSourceStart, importReference.declarationSourceEnd,
                ...
                onDemand ? importReference.trailingStarPosition : importReference.sourceEnd, importReference.tokens,
                onDemand, importReference.modifiers);
        }
    }

    protected void notifySourceElementRequestor(ModuleDeclaration moduleDeclaration) {
        boolean isInRange = this.initialPosition <= moduleDeclaration.declarationSourceStart
        ...
        if (isInRange) {
            ...
            // remember deprecation so as to not lose it below
            boolean deprecated = (currentModifiers & ClassFileConstants.AccDeprecated) != 0
                || hasDeprecatedAnnotation(moduleDeclaration.annotations);

            info.declarationStart = moduleDeclaration.declarationSourceStart;
            info.modifiers = deprecated
                ? (currentModifiers & ExtraCompilerModifiers.AccJustFlag) | ClassFileConstants.AccDeprecated
                : currentModifiers & ExtraCompilerModifiers.AccJustFlag;
...
//}
    protected void notifySourceElementRequestor(CompilationUnitDeclaration parsedUnit, Block block) {
        boolean isInRange = this.initialPosition <= block.sourceStart && this.eofPosition >= block.sourceEnd;
        if (isInRange) {
    ...
    protected void notifySourceElementRequestor(CompilationUnitDeclaration parsedUnit, TypeDeclaration typeDeclaration,
        boolean notifyTypePresence, TypeDeclaration declaringType, ImportReference currentPackage) {

        if (CharOperation.equals(TypeConstants.PACKAGE_INFO_NAME, typeDeclaration.name))
            return;

        // range check
        boolean isInRange = this.initialPosition <= typeDeclaration.declarationSourceStart
        ...
        if (notifyTypePresence) {
            ...
            if (isInRange) {
                ...
                boolean isEnumInit
                    = typeDeclaration.allocation != null && typeDeclaration.allocation.enumConstant != null;
                char[] superclassName;
                if (isEnumInit) {
                    currentModifiers |= ClassFileConstants.AccEnum;
                ...
                typeInfo.modifiers = deprecated
                    ...
                    : currentModifiers & ExtraCompilerModifiers.AccJustFlag;
                typeInfo.modifiers
                    |= currentModifiers & (ExtraCompilerModifiers.AccSealed | ExtraCompilerModifiers.AccNonSealed);
                typeInfo.modifiers |= typeDeclaration instanceof ImplicitTypeDeclaration
                    ? ExtraCompilerModifiers.AccImplicitlyDeclared
                    : 0;
                typeInfo.name = typeDeclaration.name;
                typeInfo.nameSourceStart
                    = isEnumInit ? typeDeclaration.allocation.enumConstant.sourceStart : typeDeclaration.sourceStart;
                ...
                typeInfo.anonymousMember
                    = typeDeclaration.allocation != null && typeDeclaration.allocation.enclosingInstance != null;
            ...
            if (this.nestedTypeIndex == this.typeNames.length) {
                ...
                System.arraycopy(this.superTypeNames, 0, (this.superTypeNames = new char[this.nestedTypeIndex * 2][]),
                    0, this.nestedTypeIndex);
            }
    ...
    private void fillModuleInfo(ModuleDeclaration mod, ISourceElementRequestor.ModuleInfo modInfo) {
        ...
        if (mod.exportsCount > 0) {
            ISourceElementRequestor.PackageExportInfo exps[]
                = new ISourceElementRequestor.PackageExportInfo[mod.exportsCount];
        ...
        if (mod.servicesCount > 0) {
            ISourceElementRequestor.ServicesInfo[] services
                = new ISourceElementRequestor.ServicesInfo[mod.servicesCount];
            for (int i = 0; i < services.length; i++) {
                ISourceElementRequestor.ServicesInfo ser = new ISourceElementRequestor.ServicesInfo();
                ser.serviceName
                    = CharOperation.concatWith(mod.services[i].serviceInterface.getParameterizedTypeName(), '.');
                ser.implNames = new char[mod.services[i].implementations.length][];
                for (int j = 0; j < ser.implNames.length; j++) {
                    ser.implNames[j]
                        = CharOperation.concatWith(mod.services[i].implementations[j].getParameterizedTypeName(), '.');
                }
                services[i] = ser;
            }
        ...
        if (mod.opensCount > 0) {
            ISourceElementRequestor.PackageExportInfo opens[]
                = new ISourceElementRequestor.PackageExportInfo[mod.opensCount];
