...
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.TerminalToken.TokenNameTWIDDLE;
...
public class CodeSnippetParser extends Parser implements EvaluationConstants {
     ...
     */
    public CodeSnippetParser(ProblemReporter problemReporter, EvaluationContext evaluationContext,
        boolean optimizeStringLiterals, int codeSnippetStart, int codeSnippetEnd) {
    ...
    protected void consumeFieldAccess(boolean isSuperAccess) {
        ...
        FieldReference fr = new CodeSnippetFieldReference(this.identifierStack[this.identifierPtr],
            this.identifierPositionStack[this.identifierPtr--], this.evaluationContext);
        this.identifierLengthPtr--;
        if (isSuperAccess) {
    ...
    protected void consumeMethodDeclaration(boolean isNotAbstract, boolean isDefaultMethod) {
        ...
        if (isTopLevelType()) {
            ...
            if (last >= 0
                && methodDecl.statements[last] instanceof Expression
                && ((Expression) methodDecl.statements[last]).isTrulyExpression()) {
                Expression lastExpression = (Expression) methodDecl.statements[last];
                methodDecl.statements[last] = new CodeSnippetReturnStatement(lastExpression, lastExpression.sourceStart,
        ...
        if (this.evaluationContext.localVariableNames != null) {
            ...
            for (int i = 0; i < varCount; i++) {
                ...
                if (CharOperation.indexOf('.', trimmedTypeName) == -1) {
                    ...
                    typeReference = new QualifiedTypeReference(CharOperation.splitOn('.', trimmedTypeName), positions);
                }
    ...
    protected void consumePrimaryNoNewArrayThis() {
        ...
        if (this.scanner.startPosition >= this.codeSnippetStart
            && this.scanner.startPosition <= this.codeSnippetEnd + 1 + this.lineSeparatorLength // 14838
            && isTopLevelType()) {
            pushOnExpressionStack(new CodeSnippetThisReference(this.intStack[this.intPtr--], this.endPosition,
                this.evaluationContext, false));
        } else {
    ...
    protected CompilationUnitDeclaration endParse(int act) {
        if (this.hasRecoveredOnExpression) {
            ...
            if (act != ERROR_ACTION) { // expression recovery worked
                ...
                int fieldsCount = (this.evaluationContext.localVariableNames == null
                    ? 0
                ...
                for (int i = 0; i < this.problemCountBeforeRecovery; i++) {
                    ...
                    if (unitResult.problems[i].getID() == IProblem.UnmatchedBracket)
                        continue;
                ...
                if (maxRecoveryPos > maxRegularPos) {
                    System.arraycopy(unitResult.problems, this.problemCountBeforeRecovery, unitResult.problems, 0,
    ...
    protected NameReference getUnspecifiedReference(boolean rejectTypeAnnotations) {
        ...
        if (this.scanner.startPosition >= this.codeSnippetStart
            ...
            if ((length = this.identifierLengthStack[this.identifierLengthPtr--]) == 1) {
                ...
                ref = new CodeSnippetSingleNameReference(this.identifierStack[this.identifierPtr],
                    this.identifierPositionStack[this.identifierPtr--], this.evaluationContext);
            } else {
                ...
                ref = new CodeSnippetQualifiedNameReference(tokens, positions,
                    (int) (this.identifierPositionStack[this.identifierPtr + 1] >> 32), // sourceStart
                    (int) this.identifierPositionStack[this.identifierPtr + length], this.evaluationContext); // sourceEnd
            }
            return ref;
        } else {
    ...
    private void recordLastStatementIfNeeded() {
        if ((isTopLevelType())
            && (this.scanner.startPosition <= this.codeSnippetEnd + this.lineSeparatorLength /* 14838 */)) {
    ...
    protected int resumeOnSyntaxError() {
        ...
        this.scanner.eofPosition
            = this.codeSnippetEnd < Integer.MAX_VALUE ? this.codeSnippetEnd + 1 : this.codeSnippetEnd; // stop after
                                                                                                       // expression
