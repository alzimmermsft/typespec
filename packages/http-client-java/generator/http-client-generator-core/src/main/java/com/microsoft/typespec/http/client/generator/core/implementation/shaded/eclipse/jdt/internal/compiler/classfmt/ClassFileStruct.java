...
abstract public class ClassFileStruct {
    ...
    public int i4At(int relativeOffset) {
        ...
        return ((this.reference[position++] & 0xFF) << 24) | ((this.reference[position++] & 0xFF) << 16)
            | ((this.reference[position++] & 0xFF) << 8) + (this.reference[position] & 0xFF);
    }
    ...
    public long i8At(int relativeOffset) {
        ...
        return (((long) (this.reference[position++] & 0xFF)) << 56)
            ...
            | (((long) (this.reference[position++] & 0xFF)) << 16) | (((long) (this.reference[position++] & 0xFF)) << 8)
    ...
    public char[] utf8At(int relativeOffset, int bytesAvailable) {
        ...
        while (length != 0) {
            ...
            if ((0x80 & x) != 0) {
                if ((x & 0x20) != 0) {
                    length -= 2;
                    x = ((x & 0xF) << 12) | ((this.reference[readOffset++] & 0x3F) << 6)
                        | (this.reference[readOffset++] & 0x3F);
                } else {
                    length--;
