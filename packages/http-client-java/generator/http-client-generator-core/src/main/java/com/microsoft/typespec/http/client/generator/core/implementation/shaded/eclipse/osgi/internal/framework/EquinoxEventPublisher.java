...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.osgi.framework.hooks.bundle.EventHook;
...
public class EquinoxEventPublisher {
    ...
    void publishBundleEventPrivileged(BundleEvent event) {
        ...
        synchronized (allSyncBundleListeners) {
            ...
            for (Map.Entry<BundleContextImpl, CopyOnWriteIdentityMap<SynchronousBundleListener, SynchronousBundleListener>> entry : allSyncBundleListeners
                ...
                if (!listeners.isEmpty()) {
                    Set<Map.Entry<SynchronousBundleListener, SynchronousBundleListener>> listenerEntries
                        = listeners.entrySet();
        ...
        if (!listenersSync.isEmpty()) {
            ...
            for (Map.Entry<BundleContextImpl, Set<Map.Entry<SynchronousBundleListener, SynchronousBundleListener>>> entry : listenersSync
                ...
                EventDispatcher<SynchronousBundleListener, SynchronousBundleListener, BundleEvent> dispatcher
        ...
        if ((listenersAsync != null) && !listenersAsync.isEmpty()) {
            ...
            for (Map.Entry<BundleContextImpl, Set<Map.Entry<BundleListener, BundleListener>>> entry : listenersAsync
                .entrySet()) {
                @SuppressWarnings({ "rawtypes", "unchecked" })
                EventDispatcher<BundleListener, BundleListener, BundleEvent> dispatcher
                    = (EventDispatcher) entry.getKey();
    ...
    public void publishFrameworkEventPrivileged(FrameworkEvent event, FrameworkListener... callerListeners) {
        ...
        if (callerListeners != null && callerListeners.length > 0) {
            ...
            if (listeners.size() > 0) {
                BundleContextImpl systemContext = (BundleContextImpl) container.getStorage()
                    .getModuleContainer()
                    .getModule(0)
                    .getBundle()
                    .getBundleContext();
                @SuppressWarnings({ "rawtypes", "unchecked" })
                EventDispatcher<FrameworkListener, FrameworkListener, FrameworkEvent> dispatcher
                    = (EventDispatcher) systemContext;
    ...
    void addBundleListener(BundleListener listener, BundleContextImpl context) {
        if (listener instanceof SynchronousBundleListener) {
            ...
            synchronized (allSyncBundleListeners) {
                CopyOnWriteIdentityMap<SynchronousBundleListener, SynchronousBundleListener> listeners
                    = allSyncBundleListeners.get(context);
                if (listeners == null) {
