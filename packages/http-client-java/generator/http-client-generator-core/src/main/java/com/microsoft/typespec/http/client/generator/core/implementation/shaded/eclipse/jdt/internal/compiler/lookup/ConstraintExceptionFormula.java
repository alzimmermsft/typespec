    ...
    public Object reduce(InferenceContext18 inferenceContext) {
        ...
        for (TypeBinding ex : thrown)
            if (!ex.isProperType(true))
                e[n++] = (InferenceVariable) ex; // thrown[i] is not a proper type, since it's an exception it must be
                                                 // an inferenceVariable, right?

        /*
         * If throw specification does not encode any type parameters, there are no constraints to be gleaned/gathered
         * from the throw sites.
        ...
        if (this.left instanceof LambdaExpression) {
            LambdaExpression lambda = ((LambdaExpression) this.left).resolveExpressionExpecting(this.right,
                inferenceContext.scope, inferenceContext);
            if (lambda == null)
                return TRUE; // cannot make use of this buggy constraint
            ePrime = lambda.getThrownExceptions().toArray(TypeBinding[]::new);
        } else {
            ReferenceExpression referenceExpression
                = ((ReferenceExpression) this.left).resolveExpressionExpecting(this.right, scope, inferenceContext);
    ...
    Collection<InferenceVariable> inputVariables(final InferenceContext18 context) {
        ...
        if (this.left instanceof LambdaExpression) {
            ...
            if (this.right.isFunctionalInterface(context.scope)) {
                ...
                MethodBinding sam = this.right.getSingleAbstractMethod(context.scope, true); // TODO derive with target
                                                                                             // type?
