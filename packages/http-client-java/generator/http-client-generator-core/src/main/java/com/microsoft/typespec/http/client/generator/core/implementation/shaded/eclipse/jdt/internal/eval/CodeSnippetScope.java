...
public class CodeSnippetScope extends BlockScope {
    ...
    public final boolean canBeSeenByForCodeSnippet(FieldBinding fieldBinding, TypeBinding receiverType,
        ...
        if (TypeBinding.equalsEquals(invocationType, fieldBinding.declaringClass))
            return true;
        ...
        if (fieldBinding.isPrivate()) {
            ...
            if (TypeBinding.notEquals(receiverType, fieldBinding.declaringClass))
                return false;
        ...
        do {
            if (type.isCapture()) { // https://bugs.eclipse.org/bugs/show_bug.cgi?id=285002
                if (TypeBinding.equalsEquals(originalDeclaringClass, type.erasure().original()))
                    return true;
            } else {
                if (TypeBinding.equalsEquals(originalDeclaringClass, type.original()))
                    return true;
            }
     ...
     * NOTE: Cannot invoke this method with a compilation unit scope.
     */
    public final boolean canBeSeenByForCodeSnippet(MethodBinding methodBinding, TypeBinding receiverType,
        InvocationSite invocationSite, Scope scope) {
        if (methodBinding.isPublic())
            return true;

        ReferenceBinding invocationType = (ReferenceBinding) receiverType;
        if (TypeBinding.equalsEquals(invocationType, methodBinding.declaringClass))
            return true;
        ...
        if (methodBinding.isPrivate()) {
            // answer true if the receiverType is the declaringClass
            // AND the invocationType and the declaringClass have a common enclosingType
            if (TypeBinding.notEquals(receiverType, methodBinding.declaringClass))
                return false;
    ...
    public final boolean canBeSeenByForCodeSnippet(ReferenceBinding referenceBinding, ReferenceBinding receiverType) {
        ...
        if (TypeBinding.equalsEquals(receiverType, referenceBinding))
            return true;
        ...
        if (referenceBinding.isProtected()) {
            ...
            return receiverType.fPackage == referenceBinding.fPackage
                ...
                || referenceBinding.enclosingType().isSuperclassOf(receiverType); // protected types always have an
                                                                                  // enclosing one
        }
...
// Internal use only
    ...
    public FieldBinding findFieldForCodeSnippet(TypeBinding receiverType, char[] fieldName,
        InvocationSite invocationSite) {
        ...
        while (keepLooking) {
            ...
            if (itsInterfaces != Binding.NO_SUPERINTERFACES) {
                ...
                if (++lastPosition == interfacesToVisit.length)
                    System.arraycopy(interfacesToVisit, 0, interfacesToVisit = new ReferenceBinding[lastPosition * 2][],
                        0, lastPosition);
            ...
            if ((field = currentType.getField(fieldName, true /* resolve */)) != null) {
                ...
                if (canBeSeenByForCodeSnippet(field, receiverType, invocationSite, this)) {
                    if (visibleField == null)
                        ...
                        return new ProblemFieldBinding(visibleField, visibleField.declaringClass, fieldName,
        ...
        if (interfacesToVisit != null) {
            ...
            com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.util.SimpleSet interfacesSeen
                = new org.eclipse.jdt.internal.compiler.util.SimpleSet(lastPosition * 2);
                    ...
                    if (interfacesSeen.addIfNotIncluded(anInterface) == anInterface) {
                        ...
                        if ((field = anInterface.getField(fieldName, true /* resolve */)) != null) {
                            if (visibleField == null) {
                                ...
                                ambiguous = new ProblemFieldBinding(visibleField, visibleField.declaringClass,
                            ...
                            if (itsInterfaces != Binding.NO_SUPERINTERFACES) {
                                if (++lastPosition == interfacesToVisit.length)
                                    System.arraycopy(interfacesToVisit, 0,
                                        interfacesToVisit = new ReferenceBinding[lastPosition * 2][], 0, lastPosition);
                                interfacesToVisit[lastPosition] = itsInterfaces;
                            }
                        }
                    }
...
// Internal use only
    ...
    public MethodBinding findMethod(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes,
        ...
        MethodBinding methodBinding
            = super.findMethod(receiverType, selector, argumentTypes, invocationSite, inStaticContext);
...
// Internal use only
    ...
    public MethodBinding findMethodForArray(ArrayBinding receiverType, char[] selector, TypeBinding[] argumentTypes,
        ...
        if (methodBinding != null) {
            ...
            if (argumentTypes == Binding.NO_PARAMETERS && CharOperation.equals(selector, TypeConstants.CLONE))
                return new MethodBinding(
                    ...
                    TypeConstants.CLONE, methodBinding.returnType, argumentTypes, null, object);
            if (canBeSeenByForCodeSnippet(methodBinding, receiverType, invocationSite, this))
                return methodBinding;
        }
        ...
        if (methodBinding.isValidBinding()) {
            ...
            if (!canBeSeenByForCodeSnippet(methodBinding, receiverType, invocationSite, this))
                return new ProblemMethodBinding(methodBinding, selector, methodBinding.parameters,
                    ProblemReasons.NotVisible);
        }
        return methodBinding;
    }
    ...
    public Binding getBinding(char[][] compoundName, int mask, InvocationSite invocationSite,
        ...
        Binding binding
            = getBinding(compoundName[0], mask | Binding.TYPE | Binding.PACKAGE, invocationSite, true /* resolve */);
        ...
        foundType: if (binding instanceof PackageBinding) {
            ...
            while (currentIndex < length) {
                ...
                if (binding == null) {
                    if (currentIndex == length) // must be a type if its the last name, otherwise we have no idea if its
                                                // a package or type
        ...
        if ((mask & Binding.FIELD) != 0 && (binding instanceof FieldBinding)) { // was looking for a field and found a
                                                                                // field
        ...
        if ((mask & Binding.TYPE) != 0 && (binding instanceof ReferenceBinding)) { // was looking for a type and found a
                                                                                   // type
            return binding;
        }
    ...
    public MethodBinding getConstructor(ReferenceBinding receiverType, TypeBinding[] argumentTypes,
        ...
        if (compatibleIndex == 0)
            return new ProblemMethodBinding(TypeConstants.INIT, argumentTypes, ProblemReasons.NotFound); // need a more
                                                                                                         // descriptive
                                                                                                         // error...
                                                                                                         // cannot
                                                                                                         // convert from
    ...
    public FieldBinding getFieldForCodeSnippet(TypeBinding receiverType, char[] fieldName,
        InvocationSite invocationSite) {
    ...
    public MethodBinding getImplicitMethod(ReferenceBinding receiverType, char[] selector, TypeBinding[] argumentTypes,
        InvocationSite invocationSite) {
