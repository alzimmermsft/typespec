...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.osgi.internal.framework.EquinoxConfiguration.ConfigValues;
...
public class EquinoxLocations {
    ...
    public EquinoxLocations(ConfigValues equinoxConfig, EquinoxContainer container, AtomicBoolean debugLocations,
        ...
        buildLocation(PROP_USER_AREA, defaultLocation, false, false, null);
        ...
        if (defaultLocation == null)
            defaultLocation = buildURL(new File(System.getProperty(PROP_USER_DIR), "workspace").getAbsolutePath(), //$NON-NLS-1$
                true);
        buildLocation(PROP_INSTANCE_AREA, defaultLocation, false, false, // $NON-NLS-1$
        ...
        if (parentLocation != null && !parentLocation.equals(configurationLocation.getURL())) {
            Location parent
                = new BasicLocation(null, parentLocation, true, null, equinoxConfig, container, debugLocations);
        ...
        buildLocation(PROP_HOME_LOCATION_AREA, null, true, true, null);
    }
    ...
    private static String getEclipseHomeLocation(String launcher, ConfigValues configValues) {
        ...
        String macosx
            = com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.osgi.service.environment.Constants.OS_MACOSX;
    ...
    private BasicLocation buildLocation(String property, URL defaultLocation, boolean readOnlyDefault,
        boolean computeReadOnly, String dataAreaPrefix) {
        ...
        boolean readOnly
            = (userReadOnlySetting == null ? readOnlyDefault : Boolean.valueOf(userReadOnlySetting).booleanValue());
        // if the instance location is not set, predict where the workspace will be and
        // put the instance area inside the workspace meta area.
        if (location == null)
            return new BasicLocation(property, defaultLocation,
                userReadOnlySetting != null || !computeReadOnly ? readOnly : !canWrite(defaultLocation, getOS()),
        ...
        if (url != null) {
            result = new BasicLocation(property, null,
                userReadOnlySetting != null || !computeReadOnly ? readOnly : !canWrite(url, getOS()), dataAreaPrefix,
    ...
    private String computeDefaultConfigurationLocation() {
        ...
        // home dir
        return computeDefaultUserAreaLocation();
    }
    ...
    private static boolean canWrite(URL location, String os) {
        if (location != null && "file".equals(location.getProtocol())) { //$NON-NLS-1$
            ...
            if (!locationDir.exists())
                locationDir.mkdirs();
            return locationDir.exists() && StorageUtil.canWrite(locationDir, os);
        }
        return false;
    }

    private String computeDefaultUserAreaLocation() {
        // we store the state in <user.home>/.eclipse/<application-id>_<version> where
        // <user.home>
        // is unique for each local user, and <application-id> is the one
        // defined in .eclipseproduct marker file. If .eclipseproduct does not
        ...
        return new File(userHome, appName + "/" + EquinoxLocations.CONFIG_DIR).getAbsolutePath(); //$NON-NLS-1$
    }
