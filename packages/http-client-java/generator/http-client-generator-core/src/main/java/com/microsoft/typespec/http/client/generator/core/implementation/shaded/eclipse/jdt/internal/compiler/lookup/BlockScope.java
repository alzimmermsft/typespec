...
public class BlockScope extends Scope {
    ...
    public CaseStatement enclosingCase; // from 1.4 on, local types should not be accessed across switch case blocks
                                        // (52221)
    ...
    public final void addAnonymousType(TypeDeclaration anonymousType, ReferenceBinding superBinding) {
        ...
        /*
         * Tag any enclosing lambdas as instance capturing. Strictly speaking they need not be, unless the
         * local/anonymous type references enclosing instance state.
         * but the types themselves track enclosing types regardless of whether the state is accessed or not. This
         * creates a mismatch in expectations in code generation
         * time, if we choose to make the lambda method static. To keep things simple and avoid a messy rollback, we
         * force the lambda to be an instance method under
         * this situation. However if per source, the lambda occurs in a static context, we would generate a static
         * synthetic method.
         */
    ...
    public final boolean allowBlankFinalFieldAssignment(FieldBinding binding) {
        ...
        return methodScope.isInsideInitializer() // inside initializer
            || ((AbstractMethodDeclaration) methodScope.referenceContext).isInitializationMethod(); // inside
                                                                                                    // constructor or
                                                                                                    // clinit
    }
        ...
        for (LocalVariableBinding lvb : this.locals) {
            if (lvb != null && lvb.resolvedPosition != -1) {
                ...
                if (lvb.resolvedPosition > 0xFFFF) { // no more than 65535 words of locals
                    problemReporter().noMoreAvailableSpaceForLocal(lvb,
            ...
            if (hasMoreScopes && (!hasMoreVariables || (this.subscopes[iscope].startIndex() <= ilocal))) {
                ...
                if (local.useFlag == LocalVariableBinding.UNUSED
                    ...
                    if (local.isCatchParameter()) {
                        problemReporter().unusedExceptionParameter((LocalDeclaration) local.declaration); // report
                                                                                                          // unused
                                                                                                          // catch
                                                                                                          // arguments
                    } else {
                ...
                if (!generateCurrentLocalVar) {
                    if ((local.declaration != null && compilerOptions().preserveAllLocalVariables)
                        || local.isPatternVariable()
                        || // too much voodoo around pattern codegen. Having warned, just treat them as used.
    ...
    public final ReferenceBinding findLocalType(char[] name) {
        for (int i = this.subscopeCount - 1; i >= 0; i--) {
            if (this.subscopes[i] instanceof ClassScope) {
                LocalTypeBinding sourceType
                    = (LocalTypeBinding) ((ClassScope) this.subscopes[i]).referenceContext.binding;
    ...
    public Binding getBinding(char[][] compoundName, int mask, InvocationSite invocationSite, boolean needResolve) {
        Binding binding
            = getBinding(compoundName[0], mask | Binding.TYPE | Binding.PACKAGE, invocationSite, needResolve);
        ...
        foundType: if (binding instanceof PackageBinding) {
            ...
            while (currentIndex < length) {
                ...
                binding
                    = packageBinding.getTypeOrPackage(compoundName[currentIndex++], module(), currentIndex < length);
                ...
                if (binding instanceof ReferenceBinding) {
                    ...
                    if (packageBinding instanceof SplitPackageBinding) {
                        ...
                        if (packageBinding instanceof SplitPackageBinding) {
                            problemReporter().conflictingPackagesFromModules((SplitPackageBinding) packageBinding,
                                module(), invocationSite.sourceStart(), invocationSite.sourceEnd());
                        }
                    }
                    break foundType;
                }
        ...
        while (currentIndex < length) {
            ...
            if ((binding = findMemberType(nextName, referenceBinding)) == null) {
                ...
                if ((mask & Binding.FIELD) != 0) {
                    ...
                    return new ProblemBinding(CharOperation.subarray(compoundName, 0, currentIndex), referenceBinding,
                        ProblemReasons.NotFound);
                }
                return new ProblemReferenceBinding(CharOperation.subarray(compoundName, 0, currentIndex),
        ...
        if ((mask & Binding.FIELD) != 0 && (binding instanceof FieldBinding)) {
            ...
            // Since a qualified reference must be for a static member, it won't affect static-ness of the enclosing
            // method,
...
// Added for code assist... NOT Public API
    public final Binding getBinding(char[][] compoundName, InvocationSite invocationSite) {
        int currentIndex = 0;
        int length = compoundName.length;
        Binding binding = getBinding(compoundName[currentIndex++], Binding.VARIABLE | Binding.TYPE | Binding.PACKAGE,
            invocationSite, true /* resolve */);
        if (!binding.isValidBinding())
            return binding;
        ...
        foundField: if (binding instanceof ReferenceBinding) {
            while (currentIndex < length) {
                ...
                TypeBinding receiverType
                    = typeBinding.capture(this, invocationSite.sourceStart(), invocationSite.sourceEnd());
                if ((binding = findField(receiverType, nextName, invocationSite, true /* resolve */)) != null) {
                    if (!binding.isValidBinding()) {
                        return new ProblemFieldBinding((FieldBinding) binding, ((FieldBinding) binding).declaringClass,
                            CharOperation.concatWith(CharOperation.subarray(compoundName, 0, currentIndex), '.'),
                            binding.problemId());
                    }
     ...
     * #implicitThis : the implicit this will be ok
     * #((arg) this$n) : available as a constructor arg
     * #((arg) this$n access$m... access$p) : available as as a constructor arg + a sequence of synthetic accessors to
     * synthetic fields
     * #((fieldDescr) this$n access#m... access$p) : available as a first synthetic field + a sequence of synthetic
     * accessors to synthetic fields
     * null : not found
    ...
    public Object[] getEmulationPath(ReferenceBinding targetEnclosingType, boolean onlyExactMatch,
                ...
                if (enclosingArgument != null) {
                    ...
                    if (syntheticField != null) {
                        if (TypeBinding.equalsEquals(syntheticField.type, targetEnclosingType)
                            || (!onlyExactMatch
                                && syntheticField.type.findSuperTypeOriginatingFrom(targetEnclosingType) != null))
        ...
        if ((methodScope().referenceContext instanceof ConstructorDeclaration)
            ...
            // this allows us to "skip over" any intermediate early construction context not having an enclosing
            // instance
            ReferenceBinding outer = currentType;
            while (outer != null && outer.depth() >= targetEnclosingType.depth()) {
                SyntheticArgumentBinding arg = ((NestedTypeBinding) sourceType).getSyntheticArgument(outer,
                    onlyExactMatch, currentMethodScope.isConstructorCall);
                if (arg != null) {
                    currentType = outer;
                    path[0] = arg;
                }
            ...
            while ((currentEnclosingType = currentType.enclosingType()) != null) {
                ...
                if (currentMethodScope != null) {
                    ...
                    while (enclosingScope != null) {
                        if (enclosingScope instanceof ClassScope cs
                            && TypeBinding.equalsEquals(cs.referenceContext.binding, targetEnclosingType)) {
                ...
                // TODO JEP 482: do we need a search for far outer starting at currentType, like in the above JEP 482
                // section?
                syntheticField
                    = ((NestedTypeBinding) currentType).getSyntheticField(currentEnclosingType, onlyExactMatch);
                if (syntheticField == null)
                    break;
                ...
                path[count++] = ((SourceTypeBinding) syntheticField.declaringClass).addSyntheticMethod(syntheticField,
                    true/* read */, false /* not super access */);
...
// start position in this scope - for ordering scopes vs. variables
    ...
    public void checkUnclosedCloseables(FlowInfo flowInfo, FlowContext flowContext, ASTNode location,
        ...
        if (!compilerOptions().analyseResourceLeaks)
            return;
        boolean exitAtEndOfMethod
            = location != null && locationScope == this && locationScope.isLastInMethod(null, location);
        if (this.trackingVariables == null || (!(this instanceof MethodScope) && exitAtEndOfMethod)) {
        ...
        FakedTrackingVariable returnVar = (location instanceof ReturnStatement)
            ? FakedTrackingVariable.getCloseTrackingVariable(((ReturnStatement) location).expression, flowInfo,
                flowContext, useOwningAnnotations)
            : null;

        // iterate variables according to the priorities defined in FakedTrackingVariable.IteratorForReporting.Stage
        Iterator<FakedTrackingVariable> iterator
            = new FakedTrackingVariable.IteratorForReporting(this.trackingVariables, this, location != null);
        while (iterator.hasNext()) {
            ...
            if (returnVar != null && trackingVar.isResourceBeingReturned(returnVar, useOwningAnnotations)) {
                if (useOwningAnnotations) {
                    ...
                    if ((methodTagBits & TagBits.AnnotationOwning) == 0) {
                        ...
                        if (original != null
                            && original.isParameter()
                            && (original.tagBits & TagBits.AnnotationOwning) == 0) {
            ...
            if (!trackingVar.isShared()
                && !trackingVar.closeSeen()
                && locationScope != null
                && locationScope.isLastInMethod(null, location)) {
                locToBlame = null; // at end of method there's no point in specifically blaming the final return (unless
                                   // other returns may have seen a close)
            }
