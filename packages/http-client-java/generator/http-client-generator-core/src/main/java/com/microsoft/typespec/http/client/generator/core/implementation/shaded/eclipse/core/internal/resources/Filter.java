...
public class Filter {
    ...
    public boolean match(IContainer parent, IFileInfo fileInfo) throws CoreException {
        if (provider == null) {
            ...
            if (provider == null) {
                ...
                Policy.log(new Status(IStatus.ERROR, ResourcesPlugin.PI_RESOURCES, Platform.PLUGIN_ERROR, message,
                    new Error()));
    ...
    public static IFileInfo[] filter(IProject project, LinkedList<Filter> includeFilters,
        LinkedList<Filter> excludeFilters, IContainer parent, IFileInfo[] list) throws CoreException {
    ...
    public static IFileInfo[] filterIncludes(IProject project, LinkedList<Filter> filters, IContainer parent,
        IFileInfo[] list) throws CoreException {
        if (filters.size() > 0) {
    ...
    public static IFileInfo[] filterExcludes(IProject project, LinkedList<Filter> filters, IContainer parent,
        IFileInfo[] list) throws CoreException {
        if (filters.size() > 0) {
            IFileInfo[] result = new IFileInfo[list.length];
            int outputIndex = 0;
