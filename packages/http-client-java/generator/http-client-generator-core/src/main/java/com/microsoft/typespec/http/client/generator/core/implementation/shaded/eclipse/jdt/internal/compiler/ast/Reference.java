...
public abstract class Reference extends Expression {
    ...
    protected boolean checkNullableFieldDereference(Scope scope, FieldBinding field, long sourcePosition,
        FlowContext flowContext, int ttlForFieldCheck) {
        if (field != null) {
        ...
        if (fieldBinding.isStatic()) {
            ...
            if (syntheticWriteAccessor == null) {
                TypeBinding constantPoolDeclaringClass = CodeStream.getConstantPoolDeclaringClass(currentScope,
                    fieldBinding, receiverType, isImplicitThisReceiver);
        ...
        if (valueRequired) {
            ...
            if (fieldBinding.isUsedOnlyInCompound()) {
                ...
                if (fieldBinding.compoundUseFlag == 0					// report only the last usage
                    ...
                    && (this.implicitConversion & TypeIds.UNBOXING) == 0) // don't report if unboxing is involved (might
                                                                          // cause NPE)
                {
    ...
    protected void checkFieldAccessInEarlyConstructionContext(BlockScope scope, char[] token, FieldBinding fieldBinding,
        TypeBinding actualReceiverType) {
        if (actualReceiverType != null
            && JavaFeature.FLEXIBLE_CONSTRUCTOR_BODIES.matchesCompliance(scope.compilerOptions())) {
            if (scope.isInsideEarlyConstructionContext(actualReceiverType, false)) {
                ...
                if ((this.bits & ASTNode.IsStrictlyAssigned) == 0) {
                    ...
                    if (JavaFeature.FLEXIBLE_CONSTRUCTOR_BODIES.isSupported(scope.compilerOptions())) {
                        scope.problemReporter()
                            .fieldReadInEarlyConstructionContext(token, this.sourceStart, this.sourceEnd);
                    }
                    ...
                    if (sourceField != null && sourceField.initialization != null) {
                        ...
                        scope.problemReporter()
                            .assignFieldWithInitializerInEarlyConstructionContext(token, this.sourceStart,
                                this.sourceEnd);
                        return;
                    }
                }
                ...
                scope.problemReporter()
                    .validateJavaFeatureSupport(JavaFeature.FLEXIBLE_CONSTRUCTOR_BODIES, this.sourceStart,
                        this.sourceEnd);
            }
        }
    }
