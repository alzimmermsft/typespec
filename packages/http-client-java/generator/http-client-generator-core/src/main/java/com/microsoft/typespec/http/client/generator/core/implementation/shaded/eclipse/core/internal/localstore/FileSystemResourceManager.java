...
public class FileSystemResourceManager implements ICoreConstants {
	 ...
	 */
	public boolean internalWrite(IProject target, IProjectDescription description, boolean hasPublicChanges, boolean hasPrivateChanges) throws CoreException {
		...
		try (content) {
			...
			try {
                try (OutputStream out = store.openOutputStream(options, subMonitor.split(1))) {
					...
					if (!(e.getCause() instanceof FileNotFoundException) || !Platform.getOS().equals(Platform.OS_WIN32)) {
						throw e;
					}
					fileInfo = store.fetchInfo();
