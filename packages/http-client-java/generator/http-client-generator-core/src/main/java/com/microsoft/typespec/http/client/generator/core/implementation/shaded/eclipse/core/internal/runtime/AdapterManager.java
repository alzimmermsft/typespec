...
public final class AdapterManager implements IAdapterManager {
     ...
     */
    private Map<String, List<IAdapterFactory>> getFactories(Class<?> adaptable) {
    ...
    private static Class<?>[] doComputeClassOrder(Class<?> adaptable) {
        ...
        Class<?>[] classHierarchy = classes.toArray(new Class[0]);
    ...
    public <T> T getAdapter(Object adaptable, Class<T> adapterType) {
        ...
        T adapterObject
            = getFactories(adaptable.getClass()).getOrDefault(adapterType.getName(), Collections.emptyList()) //
                .stream() //
        ...
        if (adapterObject == null) {
            if (!incorrectAdapters.isEmpty()) {
                throw new AssertionFailedException(incorrectAdapters.stream()
                    .map(entry -> "Adapter factory " //$NON-NLS-1$
                        + entry.getKey() + " returned " + entry.getValue().getName() //$NON-NLS-1$
                        + " that is not an instance of " + adapterType.getName()) //$NON-NLS-1$
    ...
    private Object getAdapter(Object adaptable, String adapterType, boolean force) {
        ...
        return getFactories(adaptable.getClass()).getOrDefault(adapterType, Collections.emptyList()) //
            .stream() //
            .map(factory -> force && factory instanceof IAdapterFactoryExt i ? i.loadFactory(true) : factory) //
