...
public class CompletionJavadocParser extends JavadocParser {
    ...
    protected Object createArgumentReference(char[] name, int dim, boolean isVarargs, Object typeRef,
        ...
        Expression expression
            = (Expression) super.createArgumentReference(argName, dim, isVarargs, typeRef, dimPositions, argNamePos);
        ...
        boolean inCompletion = (refStart <= this.cursorLocation && this.cursorLocation <= refEnd) // completion cursor
                                                                                                  // is between first
                                                                                                  // and last stacked
                                                                                                  // identifiers
        ...
        if (this.completionNode == null && inCompletion) {
            ...
            if (expressionType instanceof JavadocSingleTypeReference) {
                this.completionNode
                    = new CompletionOnJavadocSingleTypeReference((JavadocSingleTypeReference) expressionType);
    ...
    protected Object createFieldReference(Object receiver) throws InvalidInputException {
        ...
        boolean inCompletion = (refStart <= (this.cursorLocation + 1) && this.cursorLocation <= refEnd) // completion
                                                                                                        // cursor is
                                                                                                        // between first
                                                                                                        // and last
                                                                                                        // stacked
                                                                                                        // identifiers
            || ((refStart == (refEnd + 1) && refEnd == this.cursorLocation)) // or it's a completion on empty token
            || (this.memberStart == this.cursorLocation); // or it's a completion just after the member separator with
                                                          // an identifier after the cursor
        if (inCompletion) {
    ...
    protected Object createTypeReference(TerminalToken primitiveToken, boolean canBeModule) {
        ...
        if (nbIdentifiers == 1) { // Single Type ref
            this.completionNode = new CompletionOnJavadocSingleTypeReference(this.identifierStack[this.identifierPtr],
            ...
            for (int i = startPtr; i < this.identifierPtr; i++) {
                ...
                if (start <= this.cursorLocation && this.cursorLocation <= end) {
                    if (i == startPtr) {
                        this.completionNode = new CompletionOnJavadocSingleTypeReference(this.identifierStack[startPtr],
                            this.identifierPositionStack[startPtr], this.tagSourceStart, this.tagSourceEnd);
                    } else {
                        ...
                        this.completionNode = new CompletionOnJavadocQualifiedTypeReference(tokens,
                            this.identifierStack[i], positions, this.tagSourceStart, this.tagSourceEnd);
                    }
                    break;
                }
            }
            if (this.completionNode == null) {
                ...
                this.completionNode = new CompletionOnJavadocQualifiedTypeReference(tokens,
                    this.identifierStack[this.identifierPtr], positions, this.tagSourceStart, this.tagSourceEnd);
            }
        }
    ...
    protected Object createModuleTypeReference(TerminalToken primitiveToken, int moduleRefTokenCount) {
        ...
        if (newSize == 1) { // Single Type ref
            typeRef = new CompletionOnJavadocSingleTypeReference(this.identifierStack[moduleRefTokenCount],
                this.identifierPositionStack[moduleRefTokenCount], this.tagSourceStart, this.tagSourceEnd);
            ...
            for (int i = moduleRefTokenCount + 1; i < this.identifierPtr; i++) {
                ...
                if (start <= this.cursorLocation && this.cursorLocation <= end) {
                    if (i == moduleRefTokenCount) {
                        this.completionNode
                            = new CompletionOnJavadocSingleTypeReference(this.identifierStack[moduleRefTokenCount + 1],
                                this.identifierPositionStack[moduleRefTokenCount + 1], this.tagSourceStart,
                                this.tagSourceEnd);
                    } else {
                        char[][] tokens = new char[i][];
                        ...
                        typeRef = new CompletionOnJavadocQualifiedTypeReference(tokens, this.identifierStack[i],
                            positions, this.tagSourceStart, this.tagSourceEnd);
                    }
                    break;
                }
            }
            if (this.completionNode == null) {
                ...
                typeRef = new CompletionOnJavadocQualifiedTypeReference(tokens,
                    this.identifierStack[this.identifierPtr], positions, this.tagSourceStart, this.tagSourceEnd);
            }
    ...
    protected JavadocModuleReference createModuleReference(int moduleRefTokenCount) {
        ...
        moduleRef = new CompletionOnJavadocModuleReference(tokens, this.identifierStack[this.identifierPtr], positions,
    ...
    private char[][][] possibleTags(char[] prefix, boolean newLine, boolean inSnippet) {
        ...
        if (newLine && inSnippet == false) {
            System.arraycopy(this.levelTags[BLOCK_IDX], 0,
                possibleTags[BLOCK_IDX] = new char[this.levelTagsLength[BLOCK_IDX]][], 0,
                this.levelTagsLength[BLOCK_IDX]);
        } else {
            possibleTags[BLOCK_IDX] = CharOperation.NO_CHAR_CHAR;
        }
        if (inSnippet == false) {
            System.arraycopy(this.levelTags[INLINE_IDX], 0,
                possibleTags[INLINE_IDX] = new char[this.levelTagsLength[INLINE_IDX]][], 0,
                this.levelTagsLength[INLINE_IDX]);
        } else {
            possibleTags[INLINE_IDX] = CharOperation.NO_CHAR_CHAR;
        }

        if (inSnippet) {
            System.arraycopy(this.levelTags[SNIPPET_IDX], 0,
                possibleTags[SNIPPET_IDX] = new char[this.levelTagsLength[SNIPPET_IDX]][], 0,
                this.levelTagsLength[SNIPPET_IDX]);
    ...
    private void initLevelTags() {
        ...
        if (this.levelTagsLength[BLOCK_IDX] < BLOCK_ALL_TAGS_LENGTH) {
            System.arraycopy(this.levelTags[BLOCK_IDX], 0,
                this.levelTags[BLOCK_IDX] = new char[this.levelTagsLength[BLOCK_IDX]][], 0,
                this.levelTagsLength[BLOCK_IDX]);
        }
        // Init inline tags
        ...
        if (this.levelTagsLength[INLINE_IDX] < INLINE_ALL_TAGS_LENGTH) {
            System.arraycopy(this.levelTags[INLINE_IDX], 0,
                this.levelTags[INLINE_IDX] = new char[this.levelTagsLength[INLINE_IDX]][], 0,
                this.levelTagsLength[INLINE_IDX]);
        }
        // Init inline tags
        ...
        for (int i = 0; i < 1; i++) {// since only in java 18
            ...
            System.arraycopy(IN_SNIPPET_TAGS[i], 0, this.levelTags[SNIPPET_IDX], this.levelTagsLength[SNIPPET_IDX],
                length);
            this.levelTagsLength[SNIPPET_IDX] += length;
        }
        if (this.levelTagsLength[SNIPPET_IDX] < INLINE_ALL_TAGS_LENGTH) {
            System.arraycopy(this.levelTags[SNIPPET_IDX], 0,
                this.levelTags[SNIPPET_IDX] = new char[this.levelTagsLength[SNIPPET_IDX]][], 0,
                this.levelTagsLength[SNIPPET_IDX]);
        }
    }
        ...
        try {
            ...
            nextArg: while (this.index < this.scanner.eofPosition) {
                ...
                if (this.index >= this.scanner.eofPosition) {
                    ...
                    Object argument = createArgumentReference(this.scanner.getCurrentIdentifierSource(), 0, false,
                        typeRef, null, (((long) argumentStart) << 32) + this.tokenPreviousPosition - 1);
                ...
                if (this.index >= this.cursorLocation) {
                    if (this.completionNode instanceof CompletionOnJavadocSingleTypeReference) {
                        CompletionOnJavadocSingleTypeReference singleTypeReference
                            = (CompletionOnJavadocSingleTypeReference) this.completionNode;
                    ...
                    if (this.completionNode instanceof CompletionOnJavadocQualifiedTypeReference) {
                        CompletionOnJavadocQualifiedTypeReference qualifiedTypeReference
                            = (CompletionOnJavadocQualifiedTypeReference) this.completionNode;
                        if (qualifiedTypeReference.tokens == null
                            || qualifiedTypeReference.tokens.length < qualifiedTypeReference.sourcePositions.length) {
                            Object methodRef = createMethodReference(receiver, arguments);
                            return syntaxRecoverEmptyArgumentType(methodRef);
                        }
                    }
        ...
        if (this.identifierPtr >= 0) {
            ...
            switch (this.identifierPtr) {
                case 2:
                    if (!valid
                        && completionScanner.completionIdentifier != null
                        && completionScanner.completionIdentifier.length == 0) {
            ...
            if (identifier != null
                && identifier.length > 0
                && ScannerHelper.isJavaIdentifierPart(this.complianceLevel, identifier[0])) {
                name = identifier;
            }
            startPosition = (int) (this.identifierPositionStack[0] >> 32);
            endPosition = (int) this.identifierPositionStack[this.identifierPtr];
        }
        boolean inCompletion = (startPosition <= (this.cursorLocation + 1) && this.cursorLocation <= endPosition) // completion
                                                                                                                  // cursor
                                                                                                                  // is
                                                                                                                  // between
                                                                                                                  // first
                                                                                                                  ...
                                                                                                                  // stacked
                                                                                                                  // identifiers
            || ((startPosition == (endPosition + 1) && endPosition == this.cursorLocation)); // or it's a completion on
                                                                                             // empty token
        if (inCompletion) {
            if (this.completionNode == null) {
                if (isTypeParam) {
                    this.completionNode
                        = new CompletionOnJavadocTypeParamReference(name, namePosition, startPosition, endPosition);
                } else {
                    this.completionNode
                ...
                CompletionOnJavadocParamNameReference paramNameRef
                ...
                CompletionOnJavadocTypeParamReference typeParamRef
                    = (CompletionOnJavadocTypeParamReference) this.completionNode;
                int nameStart = (int) (namePosition >> 32);
    ...
    protected boolean parseTag(int previousPosition) throws InvalidInputException {
        ...
        boolean inCompletion
            = (this.tagSourceStart <= (this.cursorLocation + 1) && this.cursorLocation <= this.tagSourceEnd) // completion
                                                                                                             // cursor
                                                                                                             // is
                                                                                                             // between
                                                                                                             // first
                                                                                                             ...
                                                                                                             // stacked
                                                                                                             // identifiers
                || ((this.tagSourceStart == (this.tagSourceEnd + 1) && this.tagSourceEnd == this.cursorLocation)); // or
                                                                                                                   // it's
                                                                                                                   // a
                                                                                                                   // completion
                                                                                                                   // on
        ...
        if (inCompletion) {
            ...
            if (tags != null) {
                this.completionNode
                    = new CompletionOnJavadocTag(tag, position, startPosition, end, tags, this.allPossibleTags);
            }
        }
        return valid;
    }
    ...
    protected boolean pushParamName(boolean isTypeParam) {
        if (super.pushParamName(isTypeParam)) {
            ...
            if (expression.sourceStart <= (this.cursorLocation + 1) && this.cursorLocation <= expression.sourceEnd) {
                if (isTypeParam) {
                    this.completionNode
                        = new CompletionOnJavadocTypeParamReference((JavadocSingleTypeReference) expression);
                } else {
                    this.completionNode
                        = new CompletionOnJavadocParamNameReference((JavadocSingleNameReference) expression);
                }
                if (CompletionEngine.DEBUG) {
                    System.out.println("	completion param=" + this.completionNode); //$NON-NLS-1$
                }
                    ...
                    switch (token) {
                        ...
                        case TokenNameERROR:
                            ...
                            if (this.scanner.currentCharacter == '#') { // @see ...#member
                                ...
                                this.scanner.tokenizeWhiteSpace = true;
                                if (this.completionNode != null) {
                                    int flags = this.inlineTagStarted
                                        ? 0
                                        : CompletionOnJavadoc.TEXT | CompletionOnJavadoc.ONLY_INLINE_TAG;
                                    if (member instanceof JavadocMessageSend) {
                                        ...
                                        this.completionNode = new CompletionOnJavadocAllocationExpression(alloc,
                                            this.memberStart, flags);
                                        if (CompletionEngine.DEBUG) {
                                            System.out.println("	new completion method=" + this.completionNode); //$NON-NLS-1$
                                        }
                                    } else {
                            ...
                            try {
                                ...
                                if (this.completionNode != null) {
                                    if (this.inlineTagStarted) {
                                         ...
                                         * break;
                                         * }
                                         * } else if (this.completionNode instanceof
                                         * CompletionOnJavadocQualifiedTypeReference) {
                                         * CompletionOnJavadocQualifiedTypeReference qualifiedTypeRef =
                                         * (CompletionOnJavadocQualifiedTypeReference) this.completionNode;
    ...
    protected Object syntaxRecoverQualifiedName(TerminalToken primitiveToken) throws InvalidInputException {
        ...
        this.completionNode = new CompletionOnJavadocQualifiedTypeReference(tokens, CharOperation.NO_CHAR, positions,
            this.tagSourceStart, this.tagSourceEnd);
    ...
    protected boolean verifySpaceOrEndComment() {
        ...
        if (completionScanner.completionIdentifier != null
            && completionScanner.completedIdentifierStart <= this.cursorLocation
            && this.cursorLocation <= completionScanner.completedIdentifierEnd) {
