...
public class UnaryExpression extends OperatorExpression {
        ...
        switch ((this.bits & OperatorMASK) >> OperatorSHIFT) {
            case NOT:
                switch ((this.expression.implicitConversion & IMPLICIT_CONVERSION_MASK) >> 4) /* runtime type */ {
                    case T_boolean:
                        ...
                        this.expression.generateOptimizedBoolean(currentScope, codeStream, null,
            ...
            case MINUS:
                ...
                if (this.constant != Constant.NotAConstant) {
                    ...
                    this.expression.generateCode(currentScope, codeStream, valueRequired);
                    if (valueRequired) {
                        switch ((this.expression.implicitConversion & IMPLICIT_CONVERSION_MASK)
                            >> 4) { /* runtime type */
                            case T_int:
                                codeStream.ineg();
                                break;
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (this.expression.constant != Constant.NotAConstant) {
            this.constant = Constant.computeConstantOperation(this.expression.constant, expressionTypeID,
        ...
        if (expressionIsCast) {
            ...
            CastExpression.checkNeedForArgumentCast(scope, tableId, operatorSignature, this.expression,
                expressionTypeID);
        }
    ...
    public LocalVariableBinding[] bindingsWhenFalse() {
        return ((this.bits & OperatorMASK) >> OperatorSHIFT) == NOT ? this.expression.bindingsWhenTrue() : NO_VARIABLES;
    }

    @Override
    public LocalVariableBinding[] bindingsWhenTrue() {
        return ((this.bits & OperatorMASK) >> OperatorSHIFT) == NOT
            ? this.expression.bindingsWhenFalse()
            : NO_VARIABLES;
    }
