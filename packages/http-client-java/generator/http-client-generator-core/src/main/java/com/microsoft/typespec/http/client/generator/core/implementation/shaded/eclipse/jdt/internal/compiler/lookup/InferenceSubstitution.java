...
public class InferenceSubstitution extends Scope.Substitutor implements Substitution {
    ...
    /**
     * Answer a substitution that is able to substitute into inference variables of several inference contexts (outer
     * and inner)
     */
...
//		System.arraycopy(otherContext.inferenceVariables, 0, subst.variables, l1, l2);
    ...
    public TypeBinding substitute(Substitution substitution, TypeBinding originalType) {
        for (int i = 0; i < this.variables.length; i++) {
            ...
            if (variable.isFromInitialSubstitution
                && isInSites(variable.site)
                && isSameParameter(getP(i), originalType)) {
                if (this.environment.globalOptions.isAnnotationBasedNullAnalysisEnabled
                    && originalType.hasNullTypeAnnotations())
                    return this.environment.createAnnotatedType(variable.withoutToplevelNullAnnotation(),
    ...
    public TypeBinding substitute(TypeVariableBinding typeVariable) {
                ...
                for (int j = 0; j < ifcLen; j++) {
                    if (TypeBinding.equalsEquals(pi, superInterfaces[j])) {
                        if (superInterfaces == typeVariable.superInterfaces)
                            System.arraycopy(superInterfaces, 0, superInterfaces = new ReferenceBinding[ifcLen], 0,
                                ifcLen);
        ...
        if (hasSubstituted) {
            typeVariable = new TypeVariableBinding(typeVariable.sourceName, typeVariable.declaringElement,
                typeVariable.rank, this.environment);
            ...
            typeVariable.tagBits
                |= typeVariable.firstBound.tagBits & (TagBits.HasNullTypeAnnotation | TagBits.HasMissingType);
        }
        return typeVariable;
    }
