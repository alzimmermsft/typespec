...
public interface LMErr {
    ...
    int NERR_DupNameReboot
        = NERR_BASE + 44; /* The computer name already exists on the network. Change it and restart the computer. */
    /*
     * Config API related
    ...
    /* UNUSED BASE + 45 */
    int NERR_CfgCompNotFound
        = NERR_BASE + 46; /* The specified component could not be found in the configuration information. */
    int NERR_CfgParamNotFound
        = NERR_BASE + 47; /* The specified parameter could not be found in the configuration information. */
    ...
    int NERR_ServiceCtlNotValid
        = NERR_BASE + 91; /* The requested pause, continue, or stop is not valid for this service. */
    int NERR_NotInDispatchTbl
        = NERR_BASE + 92; /* The service control dispatcher could not find the service name in the dispatch table. */
    ...
    int NERR_NotPrimary
        = NERR_BASE + 126; /* This operation is only allowed on the primary domain controller of the domain. */
    ...
    int NERR_DatabaseUpToDate
        = NERR_BASE + 148; /* No updates are necessary to this replicant network/local security database. */
    ...
    int NERR_NoComputerName = NERR_BASE
        + 170; /* The computer name could not be added as a message alias. The name may already exist on the network. */
    ...
    int NERR_IsDfsShare
        = NERR_BASE + 221; /* The share must be removed from the Distributed File System before it can be deleted. */
    ...
    int NERR_ProfileSaveErr
        = NERR_BASE + 275; /* @W Errors occurred while saving the profile file. The profile was partially saved. */
    ...
    int NERR_TimeDiffAtDC
        = NERR_BASE + 357; /* This server's clock is not synchronized with the primary domain controller's clock. */
    ...
    int NERR_NoSuchConnection
        = NERR_BASE + 362; /* The connection identification does not specify a valid connection. */
    int NERR_TooManyServers
    ...
    int NERR_UPSSignalAsserted
        = NERR_BASE + 383; /* The UPS indicated a line fail or low battery situation. Service not started. */
     ...
     *
     */
    int NERR_BrowserConfiguredToNotRun
        = NERR_BASE + 450; /* The browser service was configured with MaintainServerList = No. */
     ...
     * NERR_BASE + = 510-550;
     */
    int NERR_RplNoAdaptersStarted
        = NERR_BASE + 510; /* Service failed to start since none of the network adapters started with this service. */
    int NERR_RplBadRegistry
        = NERR_BASE + 511; /* Service failed to start due to bad startup information in the registry. */
    ...
    int NERR_RplAdapterNameUnavailable
        = NERR_BASE + 533; /* Chosen network adapter id is in use by some other workstation. */
    int NERR_RplConfigNotEmpty = NERR_BASE + 534; /* There are profiles using this configuration. */
    int NERR_RplBootInUse
        = NERR_BASE + 535; /* There are workstations, profiles or configurations using this boot block. */
    ...
    int NERR_RplBootNameUnavailable
        = NERR_BASE + 540; /* (boot name, vendor id; is in use by some other boot block record. */
    ...
    int NERR_DfsInternalCorruption
        = NERR_BASE + 560; /* The internal database maintained by the DFS service is corrupt */
    ...
    int NERR_DfsNoSuchShare
        = NERR_BASE + 565; /* The indicated server share does not support the indicated DFS namespace */
    ...
    int NERR_DfsVolumeHasMultipleServers
        = NERR_BASE + 568; /* The operation is ambiguous because the link has multiple servers */
    ...
    int NERR_DfsCantRemoveLastServerShare
        = NERR_BASE + 577; /* Can't remove the last server-share supporting this root or link */
    ...
    int NERR_SetupDomainController
        = NERR_BASE + 593; /* This machine is a domain controller and cannot be unjoined from a domain. */
    int NERR_DefaultJoinRequired
        = NERR_BASE + 594; /* The destination domain controller does not support creating machine accounts in OUs. */
    int NERR_InvalidWorkgroupName = NERR_BASE + 595; /* The specified workgroup name is invalid. */
    int NERR_NameUsesIncompatibleCodePage = NERR_BASE
        + 596; /*
                * The specified computer name is incompatible with the default language used on the domain controller.
                */
    int NERR_ComputerAccountNotFound = NERR_BASE
        + 597; /*
                * The specified computer account could not be found. Contact an administrator to verify the account is
                * in the domain. If the account has been deleted unjoin, reboot, and rejoin the domain.
                */
    int NERR_PersonalSku = NERR_BASE + 598; /* This version of Windows cannot be joined to a domain. */
    int NERR_SetupCheckDNSConfig = NERR_BASE
        + 599; /*
                * An attempt to resolve the DNS name of a DC in the domain being joined has failed. Please verify this
                * client is configured to reach a DNS server that can resolve DNS names in the target domain.
                */
