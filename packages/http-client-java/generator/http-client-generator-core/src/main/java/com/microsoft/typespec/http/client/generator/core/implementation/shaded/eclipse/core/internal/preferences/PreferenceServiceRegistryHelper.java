...
public class PreferenceServiceRegistryHelper implements IRegistryChangeListener {
    ...
    private IExtension[] getPrefExtensions() {
        ...
        // "old"
        IExtensionPoint pointOld
            = registry.getExtensionPoint(IPreferencesConstants.RUNTIME_NAME, IPreferencesConstants.PT_PREFERENCES);
        if (pointOld != null) {
            extensionsOld = pointOld.getExtensions();
        }
        // "new"
        IExtensionPoint pointNew
            = registry.getExtensionPoint(IPreferencesConstants.PREFERS_NAME, IPreferencesConstants.PT_PREFERENCES);
        if (pointNew != null) {
    ...
    public void registryChanged(IRegistryChangeEvent event) {
        IExtensionDelta[] deltasOld
            = event.getExtensionDeltas(IPreferencesConstants.RUNTIME_NAME, IPreferencesConstants.PT_PREFERENCES);
        IExtensionDelta[] deltasNew
            = event.getExtensionDeltas(IPreferencesConstants.PREFERS_NAME, IPreferencesConstants.PT_PREFERENCES);
    ...
    private void runInitializer(IConfigurationElement element) {
        try {
            final AbstractPreferenceInitializer initializer
