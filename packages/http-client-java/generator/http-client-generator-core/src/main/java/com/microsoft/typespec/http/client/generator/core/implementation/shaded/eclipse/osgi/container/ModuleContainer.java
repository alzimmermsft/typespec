...
public final class ModuleContainer {
	...
	private ResolutionReport resolveAndApply(Collection<Module> triggers, boolean triggersMandatory, Permits resolutionPermits) {
		if (triggers == null) {
		...
		return applyDelta(deltaWiring, modulesResolved, timestamp, resolutionPermits)
				? report
				: null;
	}
	...
	private boolean applyDelta(Map<ModuleRevision, ModuleWiring> deltaWiring, Collection<Module> modulesResolved, long timestamp, Permits resolutionPermits) {
					...
					if (current != null) {
						...
						if ((revision.getTypes() & BundleRevision.TYPE_FRAGMENT) != 0) {
							for (ModuleWire hostWire : deltaEntry.getValue()
								...
								if (hostWiring != null) {
                                    Collection<ModuleRevision> dynamicFragments = hostsWithDynamicFrags.computeIfAbsent(
                                        hostWiring, k -> new ArrayList<>());
