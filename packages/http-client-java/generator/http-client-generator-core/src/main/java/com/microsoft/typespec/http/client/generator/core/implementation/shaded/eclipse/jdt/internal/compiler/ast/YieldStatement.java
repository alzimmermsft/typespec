...
public class YieldStatement extends BranchStatement {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        /*
         * Lookup target context: since an implicit yield immediately follows '->', it cannot be wrapped by another
         * context,
         * so the immediate breakable context is guaranteed to be the one intended, while explicit yield should move up
         * the parents
            ...
            if ((stmt = traversedContext.statementWithFinallyBlock()) != null) {
                if (finallys == this.statementsWithFinallyBlock.length)
                    System.arraycopy(this.statementsWithFinallyBlock, 0,
                        (this.statementsWithFinallyBlock = new StatementWithFinallyBlock[finallys * 2]), 0, finallys); // grow
        ...
        if (finallys != this.statementsWithFinallyBlock.length)
            System.arraycopy(this.statementsWithFinallyBlock, 0,
                (this.statementsWithFinallyBlock = new StatementWithFinallyBlock[finallys]), 0, finallys);
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream) {
        ...
        boolean expressionGenerationDeferred = true; // If possible defer generating the expression to until after
                                                     // inlining of enclosing try's finally {}
        boolean valueRequired;
        if (this.expression.hasSideEffects() || this.statementsWithFinallyBlock.length == 0) { // can't defer or no need
                                                                                               // to defer
            expressionGenerationDeferred = false;
            valueRequired = this.switchExpression != null && (this.bits & ASTNode.IsAnyFinallyBlockEscaping) == 0; // no
                                                                                                                   // value
                                                                                                                   // needed
                                                                                                                   // if
                                                                                                                   // finally
            ...
            codeStream.nop(); // prevent exception ranges from being empty on account of deferral : try { yield 42; }
                              // catch (Exception ex) {} ...
    ...
    public void resolve(BlockScope scope) {
        ...
        if (this.switchExpression == null) {
            ...
            if (this.switchExpression != null) {
                this.expression.setExpressionContext(this.switchExpression.expressionContext); // result expressions
                                                                                               // feature in same
                                                                                               // context ...
                this.expression.setExpectedType(this.switchExpression.expectedType);           // ... with the same
                                                                                               // target type
            }
        }
