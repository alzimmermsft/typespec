 ...
 *******************************************************************************/
package com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.internal.resources.refresh.win32;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.resources.refresh.IRefreshMonitor;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.resources.refresh.IRefreshResult;
...
class Win32Monitor extends Job implements IRefreshMonitor {
	...
	protected abstract class Handle implements Closeable {
		...
		private long createHandleValue(String path, boolean monitorSubtree) {
			long handle = Win32Natives.FindFirstChangeNotification(path, monitorSubtree, 27);
		...
		private void openHandleOn(String path, boolean subtree) {
			setHandleValue(createHandleValue(path, subtree));
			if (isOpen()) {
	 ...
	 */
	private long[][] balancedSplit(final long[] array) {
		int elementCount = array.length;
		// want to handle [1, max] rather than [0, max)
		int subArrayCount = ((elementCount - 1) / Win32Natives.MAXIMUM_WAIT_OBJECTS) + 1;
