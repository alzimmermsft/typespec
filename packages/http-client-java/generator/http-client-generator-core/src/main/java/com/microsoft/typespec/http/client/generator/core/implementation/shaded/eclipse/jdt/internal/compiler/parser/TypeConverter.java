...
public abstract class TypeConverter {
    ...
    private void addIdentifiers(String typeSignature, int start, int endExclusive, int identCount,
        ArrayList fragments) {
        if (identCount == 1) {
            char[] identifier;
    ...
    private TypeReference decodeType(String typeSignature, int length, int start, int end) {
        ...
        typeLoop: while (this.namePos < length) {
            ...
            switch (currentChar) {
                case Signature.C_BOOLEAN:
                    if (!nameStarted) {
                        ...
                        if (dim == 0)
                            ...
                            return new ArrayTypeReference(TypeBinding.BOOLEAN.simpleName, dim,
                                ((long) start << 32) + end);
                    }
                    break;
                ...
                case Signature.C_DOUBLE:
                    if (!nameStarted) {
                        ...
                        if (dim == 0)
                            ...
                            return new ArrayTypeReference(TypeBinding.DOUBLE.simpleName, dim,
                                ((long) start << 32) + end);
                    }
                    break;
                ...
                case Signature.C_FLOAT:
                    if (!nameStarted) {
                        ...
                        if (dim == 0)
                            ...
                            return new ArrayTypeReference(TypeBinding.FLOAT.simpleName, dim,
                                ((long) start << 32) + end);
                    }
                    break;
                ...
                case Signature.C_SHORT:
                    if (!nameStarted) {
                        ...
                        if (dim == 0)
                            ...
                            return new ArrayTypeReference(TypeBinding.SHORT.simpleName, dim,
                                ((long) start << 32) + end);
                    }
                    break;
                ...
                case Signature.C_GENERIC_START:
                    ...
                    this.namePos++; // skip '<'
                    TypeReference[] arguments = decodeTypeArguments(typeSignature, length, start, end); // positionned
                                                                                                        // on '>' at end
        ...
        if (fragments == null) { // non parameterized
            ...
            if (identCount == 1) { // simple type reference
                ...
                char[][] identifiers
                    = extractIdentifiers(typeSignature, nameFragmentStart, nameFragmentEnd, identCount);
                if (dim == 0) {
            ...
            if (fragmentLength == 2) {
                ...
                if (firstFragment instanceof char[]) {
                    ...
                    return new ParameterizedSingleTypeReference((char[]) firstFragment,
                        (TypeReference[]) fragments.get(1), dim, ((long) start << 32) + end);
                }
            }
            ...
            switch (currentChar) {
                ...
                case '<':
                    ...
                    TypeReference[] arguments
                        = decodeTypeArguments(typeName, length, start, end, includeGenericsAnyway); // positionned on
                                                                                                    // '>' at end
        ...
        if (fragments == null) { // non parameterized
            ...
            if (fragmentLength == 2) {
                ...
                if (firstFragment.length == 1) {
                    // parameterized single type
                    return new ParameterizedSingleTypeReference(firstFragment[0], (TypeReference[]) fragments.get(1),
