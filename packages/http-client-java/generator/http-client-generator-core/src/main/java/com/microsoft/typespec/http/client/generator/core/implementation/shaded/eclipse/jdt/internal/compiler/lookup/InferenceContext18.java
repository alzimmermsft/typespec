...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.TypeConstants.BoundCheckStatus;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.util.Sorting;
import java.util.*;
 ...
 * are accepted via {@link BoundSet#reduceOneConstraint(InferenceContext18, ConstraintFormula)} (combining 18.2 and
 * 18.3)</li>
 ...
 * Called indirectly from {@link Scope#findMethod(ReferenceBinding, char[], TypeBinding[], InvocationSite, boolean)} et
 * al
 ...
 * to solve 18.5.1 in mode {@link #CHECK_LOOSE}. Only if that fails, another attempt is made in mode
 * {@link #CHECK_VARARG}.
...
public class InferenceContext18 {
     ...
     */
    static final boolean SHOULD_WORKAROUND_BUG_JDK_8054721 = true; // See
                                                                   // https://bugs.eclipse.org/bugs/show_bug.cgi?id=437444#c24
                                                                   // onwards

    static final boolean SHOULD_WORKAROUND_BUG_JDK_8153748 = true; // emulating javac behaviour after private email
                                                                   // communication

    /**
     * Detail flag to control the extent of {@link #SIMULATE_BUG_JDK_8026527}.
     * A setting of 'false' implements the advice from
    ...
    /**
     * Not per JLS: pushing bounds from inner to outer may have to be deferred till after overload resolution, store
     * here a runnable to perform the push.
     */
    ...
    static class SuspendedInferenceRecord {
        ...
        SuspendedInferenceRecord(InvocationSite site, Expression[] invocationArguments,
            InferenceVariable[] inferenceVariables, int inferenceKind, boolean usesUncheckedConversion) {
            this.site = site;
    ...
    public void createInitialConstraintsForParameters(TypeBinding[] parameters, boolean checkVararg,
        TypeBinding varArgsType, MethodBinding method) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (this.invocationArguments[i].isPertinentToApplicability(parameters[i], method)) {
                this.initialConstraints[numConstraints++] = new ConstraintExpressionFormula(this.invocationArguments[i],
                    thetaF, ReductionResult.COMPATIBLE, ARGUMENT_CONSTRAINTS_ARE_SOFT);
            } else if (!isTypeVariableOfCandidate(parameters[i], method)) {
                this.initialConstraints[numConstraints++] = new ConstraintExpressionFormula(this.invocationArguments[i],
                    thetaF, ReductionResult.POTENTIALLY_COMPATIBLE);
        ...
        if (checkVararg && varArgsType instanceof ArrayBinding) {
            ...
            for (int i = len; i < this.invocationArguments.length; i++) {
                if (this.invocationArguments[i].isPertinentToApplicability(varArgsType, method)) {
                    this.initialConstraints[numConstraints++] = new ConstraintExpressionFormula(
                        this.invocationArguments[i], thetaF, ReductionResult.COMPATIBLE, ARGUMENT_CONSTRAINTS_ARE_SOFT);
                } else if (!isTypeVariableOfCandidate(varArgsType, method)) {
                    this.initialConstraints[numConstraints++] = new ConstraintExpressionFormula(
                        this.invocationArguments[i], thetaF, ReductionResult.POTENTIALLY_COMPATIBLE);
                } // else we know it is potentially compatible, no need to assert.
            }
        }
        if (numConstraints == 0)
            ...
            System.arraycopy(this.initialConstraints, 0,
                this.initialConstraints = new ConstraintFormula[numConstraints], 0, numConstraints);
        if (ownConstraints) { // lifted constraints get validated at their own context.
            final int length = this.initialConstraints.length;
            System.arraycopy(this.initialConstraints, 0,
                this.finalConstraints = new ConstraintExpressionFormula[length], 0, length);
        }
    }
    ...
    private InferenceVariable[] addInitialTypeVariableSubstitutions(TypeBinding[] typeVariables) {
        ...
        for (int i = 0; i < len; i++)
            newVariables[i]
                = InferenceVariable.get(typeVariables[i], i, this.currentInvocation, this.scope, this.object, true);
    ...
    private void addInferenceVariables(InferenceVariable[] newVariables) {
        if (this.inferenceVariables == null || this.inferenceVariables.length == 0) {
            ...
            System.arraycopy(this.inferenceVariables, 0, this.inferenceVariables = new InferenceVariable[len + prev], 0,
                prev);
    ...
    public InferenceVariable[] addTypeVariableSubstitutions(TypeBinding[] typeArguments, boolean capturesOnly) {
        ...
        for (int i = 0; i < typeArguments.length; i++) {
            if (typeArguments[i] instanceof InferenceVariable)
                newVariables[i] = (InferenceVariable) typeArguments[i]; // prevent double substitution of an
                                                                        // already-substituted inferenceVariable
            else if (!capturesOnly || typeArguments[i] instanceof CaptureBinding)
                toAdd[numToAdd++] = newVariables[i] = InferenceVariable.get(typeArguments[i], i, this.currentInvocation,
                    this.scope, this.object, false);
        }
        if (numToAdd > 0) {
            int start = 0;
            if (this.inferenceVariables != null) {
    ...
    public void inferInvocationApplicability(MethodBinding method, TypeBinding[] arguments, boolean isDiamond) {
        ConstraintExpressionFormula.inferInvocationApplicability(this, method, arguments, isDiamond,
            this.inferenceKind);
    }
     ...
     */
    public BoundSet inferInvocationType(TypeBinding expectedType, InvocationSite invocationSite, MethodBinding method)
        throws InferenceFailureException {
        ...
        try {
            ...
            if (expectedType != null
                ...
                && ((Expression) invocationSite).isTrulyExpression()
            ...
            if (SHOULD_WORKAROUND_BUG_JDK_8153748) { // "before 18.5.2", but should not spill into b3 ...
                                                     // (heuristically)
                ReductionResult jdk8153748result = addJDK_8153748ConstraintsFromInvocation(this.invocationArguments,
                    method, new InferenceSubstitution(this));
            ...
            List<Set<InferenceVariable>> components
                = this.currentBounds.computeConnectedComponents(this.inferenceVariables);
            while (!c.isEmpty()) {
                // *
            ...
            if (solution == null || !isResolved(solution)) {
                this.currentBounds = this.b2; // don't let bounds from unsuccessful attempt leak into subsequent
                                              // attempts
                return null;
            }
    ...
    public void pushBoundsTo(InferenceContext18 outer) {
        if (outer != null && outer.stepCompleted >= APPLICABILITY_INFERRED) {
            boolean deferred = outer.currentInvocation instanceof Invocation; // need to wait till after overload
                                                                              // resolution?
            ...
            Runnable job = () -> {
                if (outer.directlyAcceptingInnerBounds) {
                    ...
                    outer.innerInbox = deferred ? toPush : toPush.copy(); // copy now, unless already copied on behalf
                                                                          // of 'deferred'
                } else {
            ...
            if (deferred) {
                ...
                job.run(); // TODO(stephan): ever reached? for ReferenceExpression? (would need a corresponding new call
                           // to flushBoundOutbox()).
            }
        }
    }
    ...
    private ReductionResult addJDK_8153748ConstraintsFromInvocation(Expression[] arguments, MethodBinding method,
        InferenceSubstitution substitution) throws InferenceFailureException {
        ...
        if (arguments != null) {
            for (int i = 0; i < arguments.length; i++) {
                ...
                ReductionResult result
                    = addJDK_8153748ConstraintsFromExpression(argument, parameter, method, substitution);
    ...
    private ReductionResult addJDK_8153748ConstraintsFromExpression(Expression argument, TypeBinding parameter,
        MethodBinding method, InferenceSubstitution substitution) throws InferenceFailureException {
        if (argument instanceof FunctionalExpression) {
            ...
            if (addJDK_8153748ConstraintsFromExpression(ce.valueIfTrue, parameter, method, substitution)
    ...
    private ReductionResult addJDK_8153748ConstraintsFromFunctionalExpr(FunctionalExpression functionalExpr,
        TypeBinding targetType, MethodBinding method) throws InferenceFailureException {
        if (!functionalExpr.isPertinentToApplicability(targetType, method)) {
            ConstraintFormula exprConstraint = new ConstraintExpressionFormula(functionalExpr, targetType,
                ReductionResult.COMPATIBLE, ARGUMENT_CONSTRAINTS_ARE_SOFT);
            if (collectingInnerBounds(() -> exprConstraint.inputVariables(this).isEmpty())) { // input variable would
                                                                                              // signal: not ready for
                                                                                              // inference
    ...
    InferenceSubstitution enrichSubstitution(InferenceSubstitution substitution, Invocation innerInvocation,
        MethodBinding innerMethod) {
        if (innerMethod instanceof ParameterizedGenericMethodBinding) {
            InferenceContext18 innerContext
                = innerInvocation.getInferenceContext((ParameterizedMethodBinding) innerMethod);
    ...
    private boolean addConstraintsToC(Expression[] exprs, Set<ConstraintFormula> c, MethodBinding method,
        int inferenceKindForMethod, InvocationSite site) throws InferenceFailureException {
        TypeBinding[] fs;
        if (exprs != null) {
            int k = exprs.length;
            ...
            for (int i = 0; i < k; i++) {
                ...
                InferenceSubstitution inferenceSubstitution
                    = new InferenceSubstitution(this.environment, this.inferenceVariables, site);
    ...
    private boolean addConstraintsToC_OneExpr(Expression expri, Set<ConstraintFormula> c, TypeBinding fsi,
        TypeBinding substF, MethodBinding method) throws InferenceFailureException {
        ...
        if (!expri.isPertinentToApplicability(fsi, method)) {
            c.add(new ConstraintExpressionFormula(expri, substF, ReductionResult.COMPATIBLE,
        ...
        if (expri instanceof FunctionalExpression) {
            ...
            if (expri instanceof LambdaExpression) {
                ...
                if (substF.isFunctionalInterface(skope)) { // could be an inference variable.
                    ...
                    if (t != null
                        ...
                        for (int i = 0, length = resultExpressions == null ? 0 : resultExpressions.length; i < length;
                            i++) {
            ...
            if (innerContext != null && !innerContext.isInexactVarargsInference()) {
                ...
                if (innerContext.stepCompleted < InferenceContext18.APPLICABILITY_INFERRED) // shouldn't happen, but
                                                                                            // let's play safe
                    innerContext.inferInvocationApplicability(shallowMethod, argumentTypes,
                        shallowMethod.isConstructor());
                if (!innerContext.computeB3(invocation, substF, shallowMethod))
                    return false;
                if (innerContext.addConstraintsToC(arguments, c, innerMethod.genericMethod(),
                    innerContext.inferenceKind, invocation)) {
    ...
    protected int getInferenceKind(MethodBinding nonGenericMethod, TypeBinding[] argumentTypes) {
        switch (this.scope.parameterCompatibilityLevel(nonGenericMethod, argumentTypes, this.currentInvocation)) {
            ...
            case Scope.COMPATIBLE_IGNORING_MISSING_TYPE: // if in doubt the method with missing types should be accepted
                                                         // to signal its relevance for resolution
                return CHECK_LOOSE;
    ...
    public ReferenceBinding inferFunctionalInterfaceParameterization(LambdaExpression lambda, BlockScope blockScope,
        ...
        if (q == null || q.length != lambda.arguments().length) {
            ...
            if (reduceWithEqualityConstraints(lambda.argumentTypes(), q)) {
                ...
                TypeBinding[] a = targetTypeWithWildCards.arguments; // a is not-null by construction of
                ...
                ParameterizedTypeBinding f_aprime = blockScope.environment()
                    .createParameterizedType(genericType, aprime, targetTypeWithWildCards.enclosingType());
                ...
                // or the non-wildcard parameterization (§9.9) of F<A'1, ..., A'm>, if one or more type arguments are
                // still wildcards.
                if (hasWildcard) {
     ...
     */
    TypeBinding[]
        createBoundsForFunctionalInterfaceParameterizationInference(ParameterizedTypeBinding functionalInterface) {
        if (this.currentBounds == null)
            this.currentBounds = new BoundSet();
    ...
    private Boolean moreSpecificMain(TypeBinding si, TypeBinding ti, Expression expri)
        throws InferenceFailureException {
        if (si.isProperType(true) && ti.isProperType(true)) {
            return expri.sIsMoreSpecific(si, ti, this.scope) ? Boolean.TRUE : Boolean.FALSE;
        }
        // "if Ti is not a functional interface type" specifically requests the si <: ti constraint created by our
        // caller
        ...
        if (si.isFunctionalInterface(this.scope)) {			// bullet 1
            ...
            if (si instanceof IntersectionTypeBinding18) {
                ...
                checkSuper: {
                    ...
                    // each element of the intersection is a superinterface of I, or a parameterization of a
                    // superinterface of I.
                }
                for (TypeBinding binding : elements)
                    if (siSubI(binding, funcI))
                        return null;						// bullet 5
                // some element of the intersection is a subinterface of I, or a parameterization of a subinterface of
                // I.
            }
            // all passed, time to do some work:
            TypeBinding siCapture = si.capture(this.scope, expri.sourceStart, expri.sourceEnd);
            MethodBinding sam = siCapture.getSingleAbstractMethod(this.scope, false); // no wildcards should be left
                                                                                      // needing replacement
    ...
    private boolean checkExpression(Expression expri, TypeBinding[] u, TypeBinding r1, TypeBinding[] v, TypeBinding r2)
        ...
        if (expri instanceof LambdaExpression && !((LambdaExpression) expri).argumentsTypeElided()) {
            ...
            if (results != Expression.NO_EXPRESSIONS) {
                if (r1.isFunctionalInterface(this.scope)
                    && r2.isFunctionalInterface(this.scope)
                ...
                checkPrimitive1: if (r1.isPrimitiveType() && !r2.isPrimitiveType()) {
                    ...
                    for (int i = 0; i < results.length; i++) {
                        if (results[i].isPolyExpression()
                            || (results[i].resolvedType != null && !results[i].resolvedType.isPrimitiveType()))
                ...
                checkPrimitive2: if (r2.isPrimitiveType() && !r1.isPrimitiveType()) {
                    for (int i = 0; i < results.length; i++) {
                        ...
                        if (!((!results[i].isPolyExpression()
                            && (results[i].resolvedType != null && !results[i].resolvedType.isPrimitiveType())) // standalone
                                                                                                                // of a
                                                                                                                // referencetype
    ...
    private /* @Nullable */ BoundSet solve(boolean inferringApplicability, ASTNode location)
        ...
        try {
            ...
            if (inferringApplicability)
                this.b2 = this.currentBounds.copy(); // Preserve the result after reduction, without effects of
                                                     // resolve() for later use in invocation type inference.

            BoundSet solution = resolve(this.inferenceVariables, isRecordPatternTypeInference);

            /*
             * If inferring applicability make a final pass over the initial constraints preserved as final constraints
             * to make sure they hold true at a macroscopic level.
            ...
            if (inferringApplicability && solution != null && this.finalConstraints != null) {
                for (ConstraintExpressionFormula constraint : this.finalConstraints) {
                    if (constraint.left.isPolyExpression())
                        continue; // avoid redundant re-inference, inner poly's own constraints get validated in its own
                                  // context & poly invocation type inference proved compatibility against target.
    ...
    public /* @Nullable */ BoundSet solve(InferenceVariable[] toResolve, boolean isRecordPatternTypeInference)
        throws InferenceFailureException {
        if (!reduce())
            return null;
        if (!this.currentBounds.incorporate(this))
     ...
     * @return array containing the substituted types or <code>null</code> elements for any type variable that could not
     * be substituted.
     */
    public TypeBinding /* @Nullable */[] getSolutions(TypeVariableBinding[] typeParameters, InvocationSite site,
        BoundSet boundSet) {
        ...
        for (int i = 0; i < typeParameters.length; i++) {
            for (InferenceVariable variable : this.inferenceVariables) {
                if (isSameSite(variable.site, site)
                    && TypeBinding.equalsEquals(variable.typeParameter, typeParameters[i])) {
                    TypeBinding outerVar = null;
                    if (outerVariables != null
                        && (outerVar = boundSet.getEquivalentOuterVariable(variable, outerVariables)) != null)
    ...
    public boolean reduceAndIncorporate(ConstraintFormula constraint) throws InferenceFailureException {
        return this.currentBounds.reduceOneConstraint(this, constraint); // TODO(SH): should we immediately call a diat
                                                                         // incorporate, or can we simply wait for the
                                                                         // next round?
    }
     ...
     */
    private /* @Nullable */ BoundSet resolve(InferenceVariable[] toResolve, boolean isRecordPatternTypeInference)
        throws InferenceFailureException {
        this.captureId = 0;
        // NOTE: 18.5.2 ...
                    ...
                    variables: if (!isRecordPatternTypeInference && !tmpBoundSet.hasCaptureBound(variableSet)) {
                        ...
                        for (int j = 0; j < variables.length; j++) {
                            ...
                            if (lowerBounds != Binding.NO_TYPES) {
                                ...
                                if (tmpBoundSet.inThrows.contains(variable.prototype())
                                    && tmpBoundSet.hasOnlyTrivialExceptionBounds(variable, upperBounds)) {
                                    TypeBinding runtimeException
                                        = this.scope.getType(TypeConstants.JAVA_LANG_RUNTIMEEXCEPTION, 3);
                                    tmpBoundSet.addBound(
                                        new TypeBound(variable, runtimeException, ReductionResult.SAME),
                                        this.environment);
                                } else {
                                    // try upper bounds:
                                    ...
                                    if (upperBounds != Binding.NO_TYPES) {
                                        if (upperBounds.length == 1) {
                                            ...
                                            TypeBinding[] glbs
                                                = Scope.greaterLowerBound(upperBounds, this.scope, this.environment);
                                            if (glbs == null) {
                                                return null;
                        ...
                        public TypeBinding substitute(TypeVariableBinding typeVariable) {
                            for (int j = 0; j < numVars; j++)
                                if (TypeBinding.equalsEquals(variables[j], typeVariable))
                                    return zs[j];
                            /*
                             * If we have an instantiation, lower it to the instantiation. We don't want downstream
                             * abstractions to be confused about multiple versions of bounds without
                             * and with instantiations propagated by incorporation. See
                             * https://bugs.eclipse.org/bugs/show_bug.cgi?id=430686. There is no value whatsoever in
                             * continuing
...
//						}
    ...
    private TypeBinding intersectionFromGlb(TypeBinding[] glbs) {
        ...
        IntersectionTypeBinding18 intersection
            = (IntersectionTypeBinding18) this.environment.createIntersectionType18(refGlbs);
    ...
    private CaptureBinding18 freshCapture(InferenceVariable variable) {
        ...
        char[] sourceName
            = CharOperation.concat("Z".toCharArray(), '#', String.valueOf(id).toCharArray(), '-', variable.sourceName); //$NON-NLS-1$
        ...
        return new CaptureBinding18(this.scope.enclosingSourceType(), sourceName,
            variable.typeParameter.shortReadableName(), start, end, id, this.environment);
    }
    // === ===
    ...
    private Set<InferenceVariable> getSmallestVariableSet(BoundSet bounds, InferenceVariable[] subSet) {
        ...
        Map<InferenceVariable, Set<InferenceVariable>> dependencies = new HashMap<>(); // compute only once, store for
                                                                                       // the final loop over 'v'.
    ...
    private void addDependencies(BoundSet boundSet, Set<InferenceVariable> variableSet,
        InferenceVariable currentVariable) {
    ...
    private ConstraintFormula pickFromCycle(Set<ConstraintFormula> c) {
        ...
        if (candidatesII.isEmpty())
            candidatesII = c; // not spec'ed but needed to avoid returning null below, witness:
        ...
        if (candidatesIII.isEmpty()) {
            ...
            Map<ConstraintExpressionFormula, Set<ConstraintExpressionFormula>> containmentForest
                = new LinkedHashMap<>();
            for (Map.Entry<ConstraintExpressionFormula, ConstraintExpressionFormula> parentRelation : expressionContainedBy
                .entrySet()) {
     ...
     */
    private boolean dependsOn(Collection<InferenceVariable> inputsOfFirst,
        Collection<InferenceVariable> outputsOfOther) {
    ...
    private boolean isReachable(Map<ConstraintFormula, Set<ConstraintFormula>> deps, ConstraintFormula from,
        ConstraintFormula to, Set<ConstraintFormula> nodesVisited, Set<ConstraintFormula> nodesInCycle) {
        if (from == to) {
    ...
    private Set<ConstraintFormula> findBottomSet(Set<ConstraintFormula> constraints,
        ...
        // An inference variable α can influence an inference variable β if α depends on the resolution of β (§18.4), or
        // vice versa;
        ...
        constraintLoop: for (ConstraintFormula constraint : constraints) {
    ...
    private boolean canInfluenceAnyOf(InferenceVariable in, Set<InferenceVariable> allOuts,
        List<Set<InferenceVariable>> components) {
    ...
    public SuspendedInferenceRecord enterPolyInvocation(InvocationSite invocation, Expression[] innerArguments) {
        SuspendedInferenceRecord record = new SuspendedInferenceRecord(this.currentInvocation, this.invocationArguments,
            this.inferenceVariables, this.inferenceKind, this.usesUncheckedConversion);
    ...
    public void resumeSuspendedInference(SuspendedInferenceRecord record, InferenceContext18 innerContext) {
        ...
        if (this.inferenceVariables == null) { // no new ones, assume we aborted prematurely
            ...
            System.arraycopy(this.inferenceVariables, 0, this.inferenceVariables = new InferenceVariable[l1 + l2], l2,
                l1);
    ...
    private Substitution getResultSubstitution(final BoundSet result) {
        return new Substitution() {
            ...
            public TypeBinding substitute(TypeVariableBinding typeVariable) {
                if (typeVariable instanceof InferenceVariable) {
                    TypeBinding instantiation = result.getInstantiation((InferenceVariable) typeVariable,
                        InferenceContext18.this.environment);
                    if (instantiation != null)
                        return instantiation;
                }
                return typeVariable;
    ...
    public MethodBinding getReturnProblemMethodIfNeeded(TypeBinding expectedType, MethodBinding method) {
        if (InferenceContext18.SIMULATE_BUG_JDK_8026527
            && expectedType != null
        ...
        /*
         * We used to check if expected type is null and if so return method, but that is wrong - it injects an
         * incompatible method into overload resolution.
         * if we get here with expected type set to null at all, the target context does not define a target type
         * (vanilla context), so inference has done its
         * best and nothing more to do than to signal error.
         */
        ProblemMethodBinding problemMethod = new ProblemMethodBinding(method, method.selector, method.parameters,
            ProblemReasons.InvocationTypeInferenceFailure);
    ...
    public String toString() {
        ...
        switch (this.stepCompleted) {
            case NOT_INFERRED:
                buf.append(" (initial)"); //$NON-NLS-1$
                break;

            case APPLICABILITY_INFERRED:
                buf.append(" (applicability inferred)"); //$NON-NLS-1$
                break;

            case TYPE_INFERRED:
                buf.append(" (type inferred)"); //$NON-NLS-1$
                break;

            case TYPE_INFERRED_FINAL:
                buf.append(" (type inferred final)"); //$NON-NLS-1$
                break;
        }
        switch (this.inferenceKind) {
            case CHECK_STRICT:
                buf.append(" (strict)"); //$NON-NLS-1$
                break;

            case CHECK_LOOSE:
                buf.append(" (loose)"); //$NON-NLS-1$
                break;

            case CHECK_VARARG:
                buf.append(" (vararg)"); //$NON-NLS-1$
                break;
        }
    ...
    void reportUncheckedConversions(BoundSet solution) {
        if (this.constraintsWithUncheckedConversion != null) {
            ...
            for (int i = 0; i < len; i++) {
                ConstraintTypeFormula constraint
                    = (ConstraintTypeFormula) this.constraintsWithUncheckedConversion.get(i);
                 ...
                 * this.scope.problemReporter().unsafeTypeConversion((Expression) this.currentInvocation, providedType,
                 * expectedType);
                 */
            }
        }
    ...
    public void forwardResults(BoundSet result, Invocation invocation, ParameterizedMethodBinding pmb,
        ...
        for (int i = 0, length = arguments == null ? 0 : arguments.length; i < length; i++) {
            ...
            for (Expression expression : expressions) {
                ...
                if (binding instanceof ParameterizedGenericMethodBinding) {
                    ...
                    if (solutions == null)  // in CEF.reduce, we lift inner poly expressions into outer context only if
                                            // their target type has inference variables.
                        continue;
                    ...
                    if (solutions == null)  // in CEF.reduce, we lift inner poly expressions into outer context only if
                                            // their target type has inference variables.
                        continue;
                    ParameterizedTypeBinding parameterizedType = this.environment.createParameterizedType(genericType,
                        solutions, binding.declaringClass.enclosingType());
                ...
                if (variableArity && parameters.length == arguments.length && i == length - 1) {
                    TypeBinding returnType
                        = methodSubstitute.returnType.capture(this.scope, expression.sourceStart, expression.sourceEnd);
    ...
    // section 18.5.5
    public ReferenceBinding inferRecordPatternParameterization(RecordPattern recordPattern, BlockScope scope2,
    ...
    private ReferenceBinding getRecordPatternTypeFromUpwardsProjection(TypeBinding typeBinding,
        ...
        ParameterizedTypeBinding rA
            = this.environment.createParameterizedType(r, instantiations, r.enclosingType(), r.getTypeAnnotations());
        if (rA == null)
            return null;
     ...
     */
    private boolean findRPrimeAndResultingBounds(TypeBinding typeBinding, InferenceVariable[] alphas,
        TypeBinding tPrime) {
        if (tPrime == null)
            return false;
        ReferenceBinding rAlpha = this.environment.createParameterizedType((ReferenceBinding) typeBinding.original(),
            alphas, typeBinding.enclosingType(), typeBinding.getTypeAnnotations());
    ...
    private TypeBinding deriveTPrime(RecordPattern recordPattern, TypeBinding candidateT, InferenceVariable[] alphas,
        ...
        if (parameterizedType != null && parameterizedType.arguments != null) {
            TypeBinding[] arguments
                = parameterizedType.capture(this.scope, recordPattern.sourceStart, recordPattern.sourceEnd).arguments;
            /*
            ...
            TypeVariableBinding[] typeVariables
                = getTPrimeArgumentsAndCreateBounds(parameterizedType, notJust18_5_5_item_3_bullet_1Betas);
            tPrime = this.environment.createParameterizedType(parameterizedType.genericType(), typeVariables,
                parameterizedType.enclosingType(), parameterizedType.getTypeAnnotations());
            createAdditionalBoundswithU((ParameterizedTypeBinding) tPrime, notJust18_5_5_item_3_bullet_1Betas,
                typeVariables);
    ...
    private void createAdditionalBoundswithU(ParameterizedTypeBinding tPrime,
        InferenceVariable[] notJust18_5_5_item_3_bullet_1Betas, TypeVariableBinding[] typeVariables) {
        ...
        for (int i = 0, l = notJust18_5_5_item_3_bullet_1Betas.length; i < l; ++i) {
            ...
            if (beta == null || !beta.equals(typeVariables[i]))
                continue; // not an expected inference variable.
            ...
            if (uArr == null || uArr.length == 0) {
                 ...
                 * or if no proper upper bounds are derived from the TypeBound (only dependencies), //TODO:HOW TO check
                 * this condition?
            ...
            for (TypeBinding u : uArr) {
                ...
                if (!u.isProperType(false)) {
                    ...
                    rhs = this.environment.createParameterizedType((ReferenceBinding) u.original(), aArr,
    ...
    private TypeVariableBinding[] getTPrimeArgumentsAndCreateBounds(ParameterizedTypeBinding parameterizedType,
