...
public class ReadManager {
     ...
     * (first in first out).
     */
    private static final ExecutorService READER_SERVICE
        = createExecutor(Math.max(0, Math.min(CACHE_SIZE, Runtime.getRuntime().availableProcessors() - 2)));

    private static ExecutorService createExecutor(int threadCount) {
        if (threadCount <= 0)
            return null;
        else {
            ThreadPoolExecutor executor = new ThreadPoolExecutor(threadCount, threadCount, /* keepAliveTime */ 5,
    ...
    // distinct methods "getW*" with same content to make it possible to observe with method sampler which case took how
    // long:
