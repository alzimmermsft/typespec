...
public class ClasspathMultiDirectory extends ClasspathDirectory {
    ...
    ClasspathMultiDirectory(IContainer sourceFolder, IContainer binaryFolder, char[][] inclusionPatterns,
        char[][] exclusionPatterns, boolean ignoreOptionalProblems, IPath externalAnnotationPath, int release) {
...
//			return false;
        return this.ignoreOptionalProblems == md.ignoreOptionalProblems
            && this.sourceFolder.equals(md.sourceFolder)
            && this.binaryFolder.equals(md.binaryFolder)
            ...
            if (container instanceof IContainer binaryContainer) {
                ...
                if (members.length > 0) {
                    for (IResource m : members) {
                        ...
                        boolean isClass = m.getType() == IResource.FILE
                            ...
                            for (IResource m : members) {
                                ...
                                if (m.getType() == IResource.FOLDER
                                    || (m.getType() == IResource.FILE
                                        && org.eclipse.jdt.internal.compiler.util.Util.isJavaFileName(name))) {
