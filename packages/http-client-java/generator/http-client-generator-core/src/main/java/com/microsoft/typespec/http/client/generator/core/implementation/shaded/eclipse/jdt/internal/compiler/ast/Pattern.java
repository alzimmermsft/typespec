...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.NullTypeBinding;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.VoidTypeBinding;
...
public abstract class Pattern extends Expression {
    ...
    protected TypeBinding outerExpressionType; // the expression type of the enclosing instanceof, switch or outer
                                               // record pattern
    ...
    public boolean coversType(TypeBinding type, Scope scope) {
        ...
        if (type instanceof TypeVariableBinding && type.superclass().isBoxedPrimitiveType())
            type = type.superclass(); // when a boxing type is in supers it must be superclass, because all boxing types
                                      // are classes
    ...
    public abstract void generateCode(BlockScope currentScope, CodeStream codeStream, BranchLabel patternMatchLabel,
    ...
    protected boolean isApplicable(TypeBinding expressionType, BlockScope scope, ASTNode location) {
        ...
        if (patternType.isBaseType() != expressionType.isBaseType()
        ...
        if (patternType.isBaseType()) {
            PrimitiveConversionRoute route
                = Pattern.findPrimitiveConversionRoute(this.resolvedType, this.outerExpressionType, scope);
    ...
    public abstract void setIsEitherOrPattern(); // if set, is one of multiple (case label) patterns and so pattern
    ...
    public static PrimitiveConversionRoute findPrimitiveConversionRoute(TypeBinding destinationType,
        TypeBinding expressionType, Scope scope) {
        ...
        if (destinationIsBaseType && expressionIsBaseType) {
            ...
            if (expressionIsBaseType) {
                ...
                if (expressionType instanceof TypeVariableBinding
                    && expressionType.superclass().isBoxedPrimitiveType()) { // <T extends Integer> / <? extends Short>
                                                                             // ...
