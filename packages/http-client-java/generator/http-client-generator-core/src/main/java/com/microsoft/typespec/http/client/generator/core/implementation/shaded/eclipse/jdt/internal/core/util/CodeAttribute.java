...
public class CodeAttribute extends ClassFileAttribute implements ICodeAttribute {
    ...
    CodeAttribute(byte[] classFileBytes, IConstantPool constantPool, int offset) throws ClassFormatException {
        ...
        if (this.exceptionTableLength != 0) {
            ...
            for (int i = 0; i < this.exceptionTableLength; i++) {
                this.exceptionTableEntries[i]
                    = new ExceptionTableEntry(classFileBytes, constantPool, offset + readOffset);
                readOffset += 8;
            }
        }
        ...
        for (int i = 0; i < this.attributesCount; i++) {
            ...
            if (equals(attributeName, IAttributeNamesConstants.LINE_NUMBER)) {
                ...
                this.localVariableAttribute
                    = new LocalVariableAttribute(classFileBytes, constantPool, offset + readOffset);
                ...
                this.attributes[attributesIndex++]
                    = new LocalVariableTypeAttribute(classFileBytes, constantPool, offset + readOffset);
            } else if (equals(attributeName, IAttributeNamesConstants.STACK_MAP_TABLE)) {
                this.attributes[attributesIndex++]
                    = new StackMapTableAttribute(classFileBytes, constantPool, offset + readOffset);
    ...
    public byte[] getBytecodes() {
        if (this.bytecodes == null) {
            System.arraycopy(this.classFileBytes, this.codeOffset, (this.bytecodes = new byte[this.codeLength]), 0,
                this.codeLength);
        }
