...
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.ExpressionContext.CASTING_CONTEXT;
...
public class CastExpression extends Expression {
    ...
    public TypeBinding instanceofType; // set by InstanceofExpression to ensure we don't flag a necessary cast
                                       // unnecessary
    public boolean isVarTypeDeclaration; // set by LocalDeclaration to indicate we are initializing a var type
                                         // declaration
...
//expression.implicitConversion holds the cast for baseType casting
    ...
    public static void checkNeedForCastCast(BlockScope scope, CastExpression enclosingCast) {
        ...
        if (!alternateCast.checkCastTypesCompatibility(scope, enclosingCast.resolvedType,
            nestedCast.expression.resolvedType, null /* no expr to avoid side-effects */, true))
            return;
    ...
    private boolean losesPrecision(Scope scope) {
        ...
        // "A widening primitive conversion from int to float, or from long to float, or from long to double, may result
    ...
    public static void checkNeedForEnclosingInstanceCast(BlockScope scope, Expression enclosingInstance,
        ...
        if (TypeBinding.equalsEquals(castedExpressionType, enclosingInstanceType)) {
            ...
            if (castedExpressionType.isBaseType() || castedExpressionType.isArrayType())
                return; // error case
            if (TypeBinding.equalsEquals(memberType,
                scope.getMemberType(memberType.sourceName(), (ReferenceBinding) alternateEnclosingInstanceType))) {
                scope.problemReporter().unnecessaryCast((CastExpression) enclosingInstance);
            }
        }
    }
    ...
    /**
     * Only complain for identity cast, since other type of casts may be useful: e.g. {@code ~((~(long) 0) << 32)} is
     * different from: {@code ~((~0) << 32)}
     */
    ...
    public static void checkNeedForArgumentCasts(BlockScope scope, Expression receiver, TypeBinding receiverType,
        ...
        if (rawArgumentTypes != argumentTypes) {
            checkAlternateBinding(scope, receiver, receiverType, binding, arguments, argumentTypes, rawArgumentTypes,
                invocationSite);
        }
    }
    ...
    public static void checkNeedForArgumentCasts(BlockScope scope, int operator, int operatorSignature, Expression left,
        ...
        if (leftIsCast || rightIsCast) {
            if (alternateLeftTypeId > 15 || alternateRightTypeId > 15) { // must convert String + Object || Object +
                                                                         // String
            ...
            if ((operatorSignature & CompareMASK) == (alternateOperatorSignature & CompareMASK)) { // same promotions
                                                                                                   // and result
                if (leftIsCast)
                    scope.problemReporter().unnecessaryCast((CastExpression) left);
                if (rightIsCast)
    ...
    private static void checkAlternateBinding(BlockScope scope, Expression receiver, TypeBinding receiverType,
        ...
        if (binding.isConstructor()) {
            bindingIfNoCast
                = scope.getConstructor((ReferenceBinding) receiverType, alternateArgumentTypes, fakeInvocationSite);
        } else {
        ...
        if (bindingIfNoCast == binding) {
            ...
            for (int i = 0; i < argumentLength; i++) {
                ...
                /* && !originalArgumentTypes[i].needsUncheckedConversion(alternateArgumentTypes[i]) */) {
                    if (!preventsUnlikelyTypeWarning(originalArgumentTypes[i], alternateArgumentTypes[i], receiverType,
                        binding, scope))
    ...
    public static boolean checkUnsafeCast(Expression expression, Scope scope, TypeBinding castType,
        TypeBinding expressionType, TypeBinding match, boolean isNarrowing) {
        ...
        if (TypeBinding.equalsEquals(match, castType)) {
            if (!isNarrowing
                && TypeBinding.equalsEquals(match, resolvedType.leafComponentType()) // do not tag as unnecessary when
                                                                                     // recursing through upper bounds
                    ...
                    switch (match.kind()) {
                        case Binding.PARAMETERIZED_TYPE:
                            if (isNarrowing) {
                                ...
                                // [JLS 5.1.6.2] S has no subtype X other than T where the type arguments of X are not
                                // contained in the type arguments of T
                                // S will have all the type arguments that were specified in S
                                // If T2<T> extends T1<T> then a cast from T1<? extends ArrayList<?>> to T2<? extends
                                // List<?>> is checked while a cast from T1<? extends List<?>> to T2<? extends
                                // ArrayList<?>> is unchecked
                                if (expressionType.isRawType() || !expressionType.isEquivalentTo(match)) {
                                    expression.bits |= ASTNode.UnsafeCast;
                                    return true;
                                }

                                // T will have all the type arguments that were introduced in subtypes of S so the type
                                // arguments of T need to be unbound wildcards since the type arguments couldn't have
                                // been specified by S
                                // If T2<T,U> extends T1<T> then a cast from T1<? extends List<?>> to T2<? extends
                                // List<?>, ? extends List<?>> is unchecked
                                ...
                                for (int i = 0; i < length; i++) {
                                    ...
                                    alternateArguments[i] = TypeBinding.equalsEquals(paramCastType.arguments[i],
                                        scope.getJavaLangObject())
                                            ? scope.getJavaLangBoolean()
                                            : scope.getJavaLangObject();
                                    LookupEnvironment environment = scope.environment();
                                    ParameterizedTypeBinding alternateCastType
                                        = environment.createParameterizedType((ReferenceBinding) castType.erasure(),
                                            alternateArguments, castType.enclosingType());
                                ...
                                // Type arguments added by subtypes of S and removed by supertypes of T don't need to be
                                // checked since the type arguments aren't specified by either S or T
                                return true;
                            } else {
                                // [JLS 5.1.5] S <: T
                        ...
                        case Binding.RAW_TYPE:
                            expression.bits |= ASTNode.UnsafeCast; // upcast since castType is known to be bound
                                                                   // paramType
                            return true;
                        ...
                        default:
                            if (isNarrowing) {
                                /*
                                 * So we have a cast `(G<T1, ... Tn>) match` where at least one of the type arguments is
                                 * NOT an unbounded wildcard
                                 * as castType would have been reifiable otherwise and we won't be here. Since `match`
                                 * is not parameterized or raw,
                                 * should the runtime checkcast succeed, match will amount to G<?, ... ?> and so the
                                 * question distills to whether
                                ...
                                TypeVariableBinding[] typeVariables
                                    = ParameterizedCastType.genericType().typeVariablesIncludingEnclosing();
                                ...
                                for (int i = 0, length = typeArguments.length; i < length; i++) {
                                    ...
                                    TypeBinding checkedTopOfStackTypeArgument = scope.environment()
                                        .createWildcard((ReferenceBinding) tvb.declaringElement, tvb.rank, null, null,
                                            Wildcard.UNBOUND);
...
//			break;
        ...
        if (!isNarrowing && TypeBinding.equalsEquals(match, resolvedType.leafComponentType())) { // do not tag as
                                                                                                 // unnecessary when
                                                                                                 // recursing through
                                                                                                 // upper bounds
            expression.tagAsUnnecessaryCast(scope, castType);
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
        ...
        if (this.constant != Constant.NotAConstant) {
            if (valueRequired || needRuntimeCheckcast || annotatedCast) { // Added for: 1F1W9IG: IVJCOM:WINNT - Compiler
                                                                          // omits casting check
        ...
        if (annotatedCast
            || (needRuntimeCheckcast
                && TypeBinding.notEquals(this.expression.postConversionType(currentScope),
                    this.resolvedType.erasure()))) { // no need to issue a checkcast if already done as genericCast
            codeStream.checkcast(this.type, this.resolvedType, pc);
        }
        if (valueRequired) {
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (castType != null) {
            ...
            this.resolvedType = castType.capture(scope, this.type.sourceStart, this.type.sourceEnd); // make it unique,
                                                                                                     // a cast
                                                                                                     // expression
                                                                                                     // shares source
                                                                                                     // end with the
