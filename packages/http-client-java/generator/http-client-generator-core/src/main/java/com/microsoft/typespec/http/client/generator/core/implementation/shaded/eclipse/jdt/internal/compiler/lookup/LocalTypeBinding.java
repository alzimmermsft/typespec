...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.CaseStatement;
...
public final class LocalTypeBinding extends NestedTypeBinding {
    ...
    public LocalTypeBinding(ClassScope scope, SourceTypeBinding enclosingType, CaseStatement switchCase) {
        super(new char[][] { CharOperation.concat(LocalTypeBinding.LocalTypePrefix, scope.referenceContext.name) },
            scope, enclosingType);
    ...
    public char[] readableName() /* java.lang.Object, p.X<T> */ {
        char[] readableName;
        if (isAnonymousType()) {
            readableName = CharOperation.concat(TypeConstants.ANONYM_PREFIX,
                anonymousOriginalSuperType().readableName(), TypeConstants.ANONYM_SUFFIX);
    ...
    public char[] shortReadableName() /* Object */ {
        ...
        if (isAnonymousType()) {
            shortReadableName = CharOperation.concat(TypeConstants.ANONYM_PREFIX,
                anonymousOriginalSuperType().shortReadableName(), TypeConstants.ANONYM_SUFFIX);
        } else if (isMemberType()) {
...
// Record that the type is a local member type
    ...
    public void transferConstantPoolNameTo(TypeBinding substType) {
        if (this.constantPoolName != null && substType instanceof LocalTypeBinding) {
            ...
            if (substLocalType.constantPoolName == null) {
                ...
                this.scope.compilationUnitScope().constantPoolNameUsage.put(substLocalType.constantPoolName,
                    substLocalType);
            }
        }
    }
    ...
    public char[] sourceName() {
        if (isAnonymousType()) {
            return CharOperation.concat(TypeConstants.ANONYM_PREFIX, anonymousOriginalSuperType().sourceName(),
