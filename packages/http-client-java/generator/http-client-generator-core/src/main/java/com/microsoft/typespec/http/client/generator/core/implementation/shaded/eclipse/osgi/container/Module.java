...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.osgi.framework.util.ThreadInfoReport;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.osgi.internal.container.EquinoxReentrantLock;
...
public abstract class Module implements BundleReference, BundleStartLevel, Comparable<Module> {
     ...
     */
    public static final EnumSet<State> ACTIVE_SET
        = EnumSet.of(State.STARTING, State.LAZY_STARTING, State.ACTIVE, State.STOPPING);
    /**
     * A set of {@link State states} that indicate a module is resolved.
     */
    public static final EnumSet<State> RESOLVED_SET
        = EnumSet.of(State.RESOLVED, State.STARTING, State.LAZY_STARTING, State.ACTIVE, State.STOPPING);
    ...
    private static final EnumSet<ModuleEvent> VALID_STOPPED_TRANSITION
        = EnumSet.of(ModuleEvent.UPDATED, ModuleEvent.UNRESOLVED, ModuleEvent.UNINSTALLED);
    ...
    protected final void lockStateChange(ModuleEvent transitionEvent) throws BundleException {
        ...
        try {
            boolean acquired
                = stateChangeLock.tryLock(revisions.getContainer().getModuleLockTimeout(), TimeUnit.SECONDS);
            Set<ModuleEvent> currentTransition;
            if (acquired) {
