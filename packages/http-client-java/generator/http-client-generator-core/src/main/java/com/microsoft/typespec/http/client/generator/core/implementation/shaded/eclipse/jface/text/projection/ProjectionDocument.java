...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jface.text.AbstractDocument;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jface.text.DefaultLineTracker;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jface.text.IDocumentExtension;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jface.text.IDocumentInformationMapping;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jface.text.IDocumentListener;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jface.text.ILineTracker;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jface.text.IRegion;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jface.text.ITextStore;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jface.text.TextUtilities;
...
public class ProjectionDocument extends AbstractDocument {
     ...
     * document
     */
    private void internalAddMasterDocumentRange(int offsetInMaster, int lengthInMaster,
        DocumentEvent masterDocumentEvent) throws BadLocationException {
        if (lengthInMaster == 0)
            return;
        ...
        try {
            ...
            if (index < fragments.length) {
                ...
                if (offsetInMaster == fragment.offset)
                    if (fragment.length == 0) // the fragment does not overlap - it is a zero-length fragment at the
                                              // same offset
                        left = fragment;
                    else
            ...
            ProjectionDocumentEvent event = new ProjectionDocumentEvent(this, offsetInSlave, 0,
                fMasterDocument.get(offsetInMaster, lengthInMaster), offsetInMaster, lengthInMaster,
                masterDocumentEvent);
    ...
    private void internalRemoveMasterDocumentRange(int offsetInMaster, int lengthInMaster) throws BadLocationException {
        try {
            ...
            ProjectionDocumentEvent event = new ProjectionDocumentEvent(this, imageRegion.getOffset(),
                imageRegion.getLength(), "", offsetInMaster, lengthInMaster); //$NON-NLS-1$
            super.fireDocumentAboutToBeChanged(event);
    ...
    private ProjectionDocumentEvent normalize(DocumentEvent masterEvent) throws BadLocationException {
        if (!isUpdating()) {
            IRegion imageRegion
                = fMapping.toExactImageRegion(new Region(masterEvent.getOffset(), masterEvent.getLength()));
            if (imageRegion != null)
                return new ProjectionDocumentEvent(this, imageRegion.getOffset(), imageRegion.getLength(),
                    masterEvent.getText(), masterEvent);
            return null;
        }

        ProjectionDocumentEvent event = new ProjectionDocumentEvent(this, fOriginalEvent.getOffset(),
            fOriginalEvent.getLength(), fOriginalEvent.getText(), masterEvent);
        fOriginalEvent = null;
        return event;
    }
    ...
    protected final boolean adaptProjectionToMasterChange(DocumentEvent masterEvent) throws BadLocationException {
        if (!isUpdating() && fFragmentsUpdater.affectsPositions(masterEvent)
            || fIsAutoExpanding && masterEvent.getLength() > 0) {
            ...
            if (fragments.length == 0) {
                ...
                // need to bypass the usual infrastructure as the new segment/fragment would be of length 0 and thus the
                // segmentation be not well formed
                try {
    ...
    private boolean ensureWellFormedSegmentation(int anchorOffset) {
        ...
        for (int i = 0; i < segments.length; i++) {
            ...
            if (segment.isDeleted()
                || (segment.getLength() == 0 && (i < segments.length - 1 || (i > 0 && segments[i - 1].isDeleted())))) {
                try {
    ...
    public void replaceMasterDocumentRanges(int offsetInMaster, int lengthInMaster) throws BadLocationException {
        try {
            ...
            ProjectionDocumentEvent event = new ProjectionDocumentEvent(this, 0, fMapping.getImageLength(),
                fMasterDocument.get(offsetInMaster, lengthInMaster), offsetInMaster, lengthInMaster);
            super.fireDocumentAboutToBeChanged(event);
