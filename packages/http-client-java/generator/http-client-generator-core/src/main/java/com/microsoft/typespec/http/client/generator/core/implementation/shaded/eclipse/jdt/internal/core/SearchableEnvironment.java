...
public class SearchableEnvironment implements IModuleAwareNameEnvironment, IJavaSearchConstants {
    ...
    public SearchableEnvironment(JavaProject project,
        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.ICompilationUnit[] workingCopies)
     ...
     * Creates a SearchableEnvironment on the given project
     */
    public SearchableEnvironment(JavaProject project, WorkingCopyOwner owner, boolean excludeTestCode, int release)
        throws JavaModelException {
        this(project,
            owner == null
                ? null
                : JavaModelManager.getJavaModelManager().getWorkingCopies(owner, true/* add primary WCs */),
            excludeTestCode, release);
        this.owner = owner;
    }
    ...
    protected NameEnvironmentAnswer find(String typeName, String packageName, IPackageFragmentRoot[] moduleContext) {
        ...
        if (this.owner != null) {
            ...
            if (source != null) {
                IJavaElement moduleElement
                    = (moduleContext != null && moduleContext.length > 0) ? moduleContext[0] : null;
        ...
        Answer answer = this.nameLookup.findType(typeName, packageName, false/* exact match */, NameLookup.ACCEPT_ALL,
            this.checkAccessRestrictions, moduleContext, this.release);
        if (answer != null) {
            ...
            if (answer.type instanceof BinaryType) { // BinaryType
                ...
                try {
                    ...
                    SourceTypeElementInfo sourceType
                        = (SourceTypeElementInfo) ((SourceType) answer.type).getElementInfo();
    ...
    public NameEnvironmentAnswer findType(char[][] compoundTypeName, char[] moduleName) {
        ...
        return find(DeduplicationUtil.toString(compoundTypeName[lengthM1]), CharOperation.toString(packageName),
            moduleLocations);
    }
    ...
    public NameEnvironmentAnswer findType(char[] name, char[][] packageName, char[] moduleName) {
        ...
        boolean isNamedStrategy = LookupStrategy.get(moduleName) == LookupStrategy.Named;
        IPackageFragmentRoot[] moduleLocations = isNamedStrategy ? findModuleContext(moduleName) : null;
        return find(DeduplicationUtil.toString(name),
            packageName == null || packageName.length == 0 ? null : CharOperation.toString(packageName),
            moduleLocations);
    }

    /**
     * @see org.eclipse.jdt.internal.compiler.env.IModuleAwareNameEnvironment#getModulesDeclaringPackage(char[][],
     * char[])
     */
    @Override
                ...
                case AnyNamed:
                    ...
                    if (matchingRoots != null) {
                        ...
                        for (IPackageFragmentRoot packageRoot : matchingRoots) {
                            ...
                            if (strategy.matches(singleton, locs -> locs[0] instanceof JrtPackageFragmentRoot
                                || getModuleDescription(locs) != null)) {
                        ...
                        switch (current.getElementType()) {
                            case IJavaElement.PACKAGE_FRAGMENT_ROOT:
                                if (!((IPackageFragmentRoot) current).isExternal()
                                    ...
                                    moduleContext = new IPackageFragmentRoot[] { (IPackageFragmentRoot) current }; // TODO:
                                                                                                                   // validate
                                    break;
                                }
                                //$FALL-THROUGH$
                                        ...
                                        if (resource != null) {
                                            ...
                                            if (otherProject != null && !otherProject.equals(currentProject)) {
                                                ...
                                                if (otherJavaProject.exists())
                                                    moduleContext
                                                        = getRootsForOutputLocation(otherJavaProject, resource);
                                            }
                                        }
                                    } catch (JavaModelException e) {
    ...
    private IPackageFragmentRoot[] getRootsForOutputLocation(IJavaProject otherJavaProject, IResource outputLocation)
        throws JavaModelException {
