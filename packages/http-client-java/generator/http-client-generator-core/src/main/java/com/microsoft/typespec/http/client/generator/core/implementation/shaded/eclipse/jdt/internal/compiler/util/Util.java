...
public class Util implements SuffixConstants {
                ...
                if (f.exists()) {
                    ...
                    if (!f.mkdir()) {
                        if (f.exists()) {
                            ...
                            // no one could create f -- complain
                            throw new IOException(Messages.bind(Messages.output_notValid,
                                outDir.substring(outputPath.length() + 1, outDir.length() - 1), outputPath));
                        }
                    }
                }
                if (checkFileType) {
    ...
    public static char[] getFileCharContent(File file, String encoding) throws IOException {
        return org.eclipse.jdt.internal.compiler.util.Util.getBytesAsCharArray(Files.readAllBytes(file.toPath()),
            encoding);
    }

    private static File getFile(boolean generatePackagesStructure, String outputPath, String relativeFileName)
        throws IOException {
        if (generatePackagesStructure) {
            ...
            if (indexOfPackageSeparator == -1) {
                ...
                if (outputPath.endsWith(fileSeparator)) {
                    ...
                    fileName
                        = outputPath + fileSeparator + relativeFileName.substring(indexOfPackageSeparator + 1, length);
                }
            }
        ...
        inclusionCheck: if (inclusionPatterns != null) {
            for (int i = 0, length = inclusionPatterns.length; i < length; i++) {
                ...
                if (isFolderPath) {
                    ...
                    if (lastSlash != -1 && lastSlash != pattern.length - 1) { // trailing slash -> adds '**' for free
                                                                              // (see
    ...
    public static void recordNestedType(ClassFile classFile, TypeBinding typeBinding) {
        ...
        if (typeBinding.isParameterizedType() && ((typeBinding.tagBits & TagBits.ContainsNestedTypeReferences) != 0)) {
            ...
            if (arguments != null) {
                for (TypeBinding argument : arguments) {
                    if (argument.isWildcard()) {
                        ...
                        if (superclass != null && ((superclass.tagBits & TagBits.ContainsNestedTypeReferences) != 0)) {
    ...
    public static List<Classpath> collectFilesNames() {
        return collectPlatformLibraries(null);
    }

    public static List<Classpath> collectPlatformLibraries(File javaHome) {
        /*
        ...
        if (jdkLevel >= ClassFileConstants.JDK9) {
            List<Classpath> filePaths = new ArrayList<>();
            if (javaHome == null) {
        ...
        for (String filePath : filePaths) {
            Classpath currentClasspath = FileSystem.getClasspath(filePath, null, null, null, null);
    ...
    public static int scanTypeBoundSignature(char[] string, int start) {
        ...
        c = string[++start];
        if (c != C_STAR && start >= string.length - 1) { // unless "-*" we need at least one more char, e.g. after "+[",
