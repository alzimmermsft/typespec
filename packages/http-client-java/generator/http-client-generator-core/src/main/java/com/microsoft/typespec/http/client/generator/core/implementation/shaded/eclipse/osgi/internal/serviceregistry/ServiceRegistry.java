...
public class ServiceRegistry {
	...
	public ServiceRegistrationImpl<?> registerService(BundleContextImpl context, String[] clazzes, Object service,
		...
		ServiceRegistrationImpl<?> registration = hookTypes != null
				? new FrameworkHookRegistration<>(this, context, clazzes, service,
	...
	void addServiceRegistration(BundleContextImpl context, ServiceRegistrationImpl<?> registration) {
		...
		// BundleContextImpl.
        List<ServiceRegistrationImpl<?>> contextServices = publishedServicesByContext.computeIfAbsent(context,
            k -> new ArrayList<>(initialSubCapacity));
		...
		for (String clazz : registration.getClasses()) {
            List<ServiceRegistrationImpl<?>> services = publishedServicesByClass.computeIfAbsent(clazz,
                k -> new ArrayList<>(initialSubCapacity));
	...
	static String checkServiceClass(final String[] clazzes, final Object serviceObject) {
		ClassLoader cl = AccessController.doPrivileged(
            (PrivilegedAction<ClassLoader>) () -> serviceObject.getClass().getClassLoader());
	...
	private void notifyEventHooksPrivileged(final ServiceEvent event, final Collection<BundleContext> result) {
		notifyHooksPrivileged(com.microsoft.typespec.http.client.generator.core.implementation.shaded.osgi.framework.hooks.service.EventHook.class, "event", //$NON-NLS-1$
