...
public class Locker_JavaNio implements Locker {
    ...
    public synchronized boolean lock() throws IOException {
        ...
        try {
            ...
            String specificMessage = NLS.bind(Msg.location_cannotLockNIO,
                new Object[] { lockFile, ioe.getMessage(), "\"-D" + LocationHelper.PROP_OSGI_LOCKING + "=none\"" }); //$NON-NLS-1$ //$NON-NLS-2$
            ...
            try {
                ...
                try {
                    ...
                    if (debug)
                        System.out.println(NLS.bind(Msg.location_cannotLock, lockFile));
                    // produce a more specific message for clients
                    String specificMessage = NLS.bind(Msg.location_cannotLockNIO, lockFile, ioe.getMessage(),
                        "\"-D" + LocationHelper.PROP_OSGI_LOCKING + "=none\""); //$NON-NLS-1$ //$NON-NLS-2$
