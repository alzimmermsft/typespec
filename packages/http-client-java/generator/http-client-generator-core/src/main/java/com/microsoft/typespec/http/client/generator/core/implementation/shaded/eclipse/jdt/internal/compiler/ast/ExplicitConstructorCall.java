...
public class ExplicitConstructorCall extends Statement implements Invocation {
        ...
        if (this.arguments != null) {
            for (Expression arg : this.arguments) {
                if (arg.constant != Constant.NotAConstant)
                    continue;
                if (arg instanceof SingleNameReference ref && ref.binding != null && ref.binding.isParameter())
                    continue;
                return true;
            }
        }
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream) {
        ...
        try {
            ...
            if (this.syntheticAccessor != null) {
                ...
                for (int i = 0, max = this.syntheticAccessor.parameters.length - codegenBinding.parameters.length;
                    i < max; i++) {
        ...
        try {
            ...
            if (this.accessMode != ExplicitConstructorCall.This) {
                ...
                if (superclassRef != null
                    && superclassRef.resolvedType != null
            ...
            if (receiverType != null) {
                ...
                if (this.accessMode == ExplicitConstructorCall.Super
                    ...
                    scope.problemReporter()
                        .cannotInvokeSuperConstructorInEnum(this, methodScope.referenceMethod().binding);
                }
                if (!receiverType.isEnum()
                    ...
                    MethodScope allocationStaticEnclosing = scope.parent.nearestEnclosingStaticScope(); // Constructor
                                                                                                        // scope already
                                                                                                        // has static,
                                                                                                        // start from
                                                                                                        // parent scope
                    ...
                    if (allocationStaticEnclosing != null
                        && typesEnclosingStaticScope != null
                        && allocationStaticEnclosing != typesEnclosingStaticScope)
                ...
                if (this.qualification != null) {
                    if (this.accessMode != ExplicitConstructorCall.Super) {
                        scope.problemReporter()
                            .unnecessaryEnclosingInstanceSpecification(this.qualification, receiverType);
                    }
                    if (!rcvHasError) {
            ...
            if (this.arguments != null) {
                ...
                if (argHasError) {
                    ...
                    for (int i = length; --i >= 0;) {
                        pseudoArgs[i] = argumentTypes[i] == null ? TypeBinding.NULL : argumentTypes[i]; // replace args
                                                                                                        // with errors
                                                                                                        // with null
                                                                                                        // type
                    }
                    ...
                    if (this.binding != null && !this.binding.isValidBinding()) {
                        ...
                        if (closestMatch != null) {
                            if (closestMatch.original().typeVariables != Binding.NO_TYPE_VARIABLES) { // generic method
                                // shouldn't return generic method outside its context, rather convert it to raw method
                                // (175409)
                                closestMatch = scope.environment()
                                    .createParameterizedGenericMethod(closestMatch.original(), (RawTypeBinding) null);
                            }
            ...
            if (this.binding.isValidBinding()) {
                ...
                if (checkInvocationArguments(scope, null, receiverType, this.binding, this.arguments, argumentTypes,
                    argsContainCast, this)) {
    ...
    private boolean checkAndFlagExplicitConstructorCallInCanonicalConstructor(AbstractMethodDeclaration methodDecl,
        BlockScope scope) {

        if (methodDecl.binding == null
            || methodDecl.binding.declaringClass == null
