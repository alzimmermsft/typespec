...
public class ServiceVersionTemplate implements IJavaTemplate<ServiceVersion, JavaFile> {
    ...
    public void write(ServiceVersion serviceVersion, JavaFile javaFile) {
        // imports
        ClassType.SERVICE_VERSION.addImportsTo(javaFile::declareImport, false);
        ...
        javaFile.publicEnum(className + " implements ServiceVersion", classBlock -> {
            ...
            classBlock.constructor(className + "(String version)",
                javaBlock -> javaBlock.line("this.version = version;"));

            classBlock.javadocComment(JavaJavadocComment::inheritDoc);
            classBlock.annotation("Override");
            classBlock.publicMethod("String getVersion()", javaBlock -> javaBlock.line("return this.version;"));
            ...
            classBlock.publicStaticMethod(className + " getLatest()", javaBlock -> javaBlock
                .methodReturn(getVersionIdentifier(serviceVersions.get(serviceVersions.size() - 1))));
        });
    }
