...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.osgi.internal.framework.DTOBuilder;
...
public class ServiceReferenceImpl<S> implements ServiceReference<S> {
	...
	ServiceReferenceImpl(ServiceRegistrationImpl<S> registration) {
		...
		/*
		 * We must not dereference registration in the constructor since it is "leaked"
		 * to us in the ServiceRegistrationImpl constructor.
		 */
	}

	/**
	 * Returns the property value to which the specified property key is mapped in
	 * the properties <code>Dictionary</code> object of the service referenced by
	 ...
	 * <code>ServiceReference</code> objects stored in the log) can still be
	 * interrogated.
	 *
	 * <p>
	 ...
	 *         this <code>ServiceReference</code> object is greater than zero;
	 ...
	 * service id} they are equal. This <code>ServiceReference</code> is less than
	 ...
	 * <code>ServiceReference</code> if it has a higher {@link Constants#SERVICE_ID
	 ...
	 *         the specified <code>ServiceReference</code>.
	 * @since 1.4
	 */
	@Override
	 ...
	 * unregistered. This is so references to unregistered services (for example,
	 * {@code ServiceReference} objects stored in the log) can still be
	 * interrogated.
	 * <p>
	 * The returned {@code Dictionary} object:
	 * <ul>
	 * <li>Must map property values by using property keys in a <i>case-insensitive
	 * manner</i>.</li>
	 * <li>Must return property keys is a <i>case-preserving</i> manner. This means
	 * that the keys must have the same case as the corresponding key in the
	 ...
	 * <li>Is the property of the caller and can be modified by the caller but any
	 * changes are not reflected in the properties of the service.
	...
	private <A> void checkAdaptPermission(Class<A> adapterType) {
		...
		sm.checkPermission(
				new AdaptPermission(adapterType.getName(), registration.getRegisteringBundle(), AdaptPermission.ADAPT));
	}
}
