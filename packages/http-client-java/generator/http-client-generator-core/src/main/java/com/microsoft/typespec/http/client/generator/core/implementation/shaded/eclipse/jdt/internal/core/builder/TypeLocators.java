...
/**
 * The {@link TypeLocators} maintain a mapping of type names (in the form <code>p1/p2/A</code>) to code locations (in
 * the
        ...
        if (length > 0) {
            for (int i = 0; i < length; i++) {
                recordLocatorForType(in.readStringUsingLast(),
                    internedTypeLocators[in.readIntInRange(internedTypeLocators.length)], JavaProject.NO_RELEASE);
            }
        }
        length = in.readInt();
        if (length == 0) {
