...
// differs in erasure from overridingMethod could override `inheritedMethod'
    ...
    boolean canSkipInheritedMethods(MethodBinding one, MethodBinding two) {
        return two == null // already know one is not null
            || (TypeBinding.equalsEquals(one.declaringClass, two.declaringClass)
                && !one.declaringClass.isParameterizedType());
    }
    ...
    void checkConcreteInheritedMethod(MethodBinding concreteMethod, MethodBinding[] abstractMethods) {
        ...
        if (analyseNullAnnotations && this.type.equals(concreteMethod.declaringClass)) // is currentMethod from the
                                                                                       // current type?
            srcMethod = concreteMethod.sourceMethod();
        boolean hasReturnNonNullDefault
            = analyseNullAnnotations && concreteMethod.hasNonNullDefaultForReturnType(srcMethod);
        ParameterNonNullDefaultProvider hasParameterNonNullDefault = analyseNullAnnotations
            ? concreteMethod.hasNonNullDefaultForParameter(srcMethod)
            : ParameterNonNullDefaultProvider.FALSE_PROVIDER;
        ...
        for (MethodBinding abstractMethod : abstractMethods) {
            ...
            // so the parameters are equal and the return type is compatible b/w the currentMethod & the substituted
            // inheritedMethod
            ...
            if (originalInherited.declaringClass.isInterface()) {
                if ((TypeBinding.equalsEquals(concreteMethod.declaringClass, this.type.superclass)
                    && this.type.superclass.isParameterizedType()
                    && !areMethodsCompatible(concreteMethod, originalInherited))
                    || this.type.superclass.erasure().findSuperTypeOriginatingFrom(originalInherited.declaringClass)
                        == null)
                    this.type.addSyntheticBridgeMethod(originalInherited, concreteMethod.original());
            }
            if (analyseNullAnnotations && !concreteMethod.isStatic() && !abstractMethod.isStatic()) {
                checkNullSpecInheritance(concreteMethod, srcMethod, hasReturnNonNullDefault, hasParameterNonNullDefault,
                    true, abstractMethod, abstractMethods, this.type.scope, null);
            }
        }
    }
        ...
        if (bridge != null) {
            ...
            for (int i = current.length - 1; i >= 0; --i) {
                ...
                if (thisMethod.areParameterErasuresEqual(bridge)
                    && TypeBinding.equalsEquals(thisMethod.returnType.erasure(), bridge.returnType.erasure())) {
                    // use inherited method for problem reporting.
                    problemReporter(thisMethod).methodNameClash(thisMethod,
                        inheritedMethod.declaringClass.isRawType() ? inheritedMethod : inheritedMethod.original(),
    ...
    void checkForNameClash(MethodBinding currentMethod, MethodBinding inheritedMethod) {
        ...
        if (inheritedMethod.isStatic() || currentMethod.isStatic()) {
            ...
            if (currentMethod.areParameterErasuresEqual(original)) {
                problemReporter(currentMethod).methodNameClashHidden(currentMethod,
                    inheritedMethod.declaringClass.isRawType() ? inheritedMethod : original);
            }
            return; // no chance of bridge method's clashing
        }

        if (!detectNameClash(currentMethod, inheritedMethod, false)) { // check up the hierarchy for skipped inherited
                                                                       // methods
            ...
            while (superType != null && superType.isValidBinding()) {
                ...
                for (MethodBinding method : methods) {
                    ...
                    if (substitute != null
                        && !isSubstituteParameterSubsignature(currentMethod, substitute)
...
// 8.4.8.4
    ...
    void checkInheritedMethods(MethodBinding[] methods, int length, boolean[] isOverridden, boolean[] isInherited) {
        ...
        if (continueInvestigation) {
            if (playingTrump) {
                // multiple abstract & default methods are OK on this branch, but then the class must be declared
                // abstract:
        ...
        findDefaultMethod: for (int i = 0; i < length; i++) {
            if (methods[i].isDefaultMethod() && !isOverridden[i]) {
                findEquivalent: for (int j = 0; j < length; j++) {
                    if (j == i || isOverridden[j])
                        continue findEquivalent;
                    if (isMethodSubsignature(methods[i], methods[j])) {
                        if (!doesMethodOverride(methods[i], methods[j])
                            && !doesMethodOverride(methods[j], methods[i])) {
                            problemReporter().inheritedDefaultMethodConflictsWithOtherInherited(this.type, methods[i],
                                methods[j]);
                            ok = false;
    ...
    boolean checkInheritedReturnTypes(MethodBinding method, MethodBinding otherMethod) {
        ...
        /*
         * We used to have some checks here to see if we would have already blamed the super type and if so avoid
         * blaming
         * the current type again. I have gotten rid of them as they in fact short circuit error reporting in cases
         * where
         * they should not. This means that occasionally we would report the error twice - the diagnostics is valid
         * however,
         * albeit arguably redundant. See https://bugs.eclipse.org/bugs/show_bug.cgi?id=334313. For an example of a test
         * where we do this extra reporting see
         * org.eclipse.jdt.core.tests.compiler.regression.MethodVerifyTest.test159()
         */
    ...
    void checkAgainstInheritedMethods(MethodBinding currentMethod, MethodBinding[] methods, int length,
        ...
        if (options.isAnnotationBasedNullAnalysisEnabled && (currentMethod.tagBits & TagBits.IsNullnessKnown) == 0) {
            // if annotations are inherited these have been checked during STB.resolveTypesFor() (for methods explicit
            // in this.type)
            AbstractMethodDeclaration srcMethod = null;
            ...
            ParameterNonNullDefaultProvider hasParameterNonNullDefault
                = currentMethod.hasNonNullDefaultForParameter(srcMethod);
            for (int i = length; --i >= 0;)
                if (!currentMethod.isStatic() && !methods[i].isStatic())
                    checkNullSpecInheritance(currentMethod, srcMethod, hasReturnNonNullDefault,
                        hasParameterNonNullDefault, true, methods[i], methods, this.type.scope, null);
        }
    }

    @Override
    void checkNullSpecInheritance(MethodBinding currentMethod, AbstractMethodDeclaration srcMethod,
        boolean hasReturnNonNullDefault, ParameterNonNullDefaultProvider hasParameterNonNullDefault, boolean complain,
        MethodBinding inheritedMethod, MethodBinding[] allInherited, Scope scope,
        InheritedNonNullnessInfo[] inheritedNonNullnessInfos) {
        complain &= !currentMethod.isConstructor();
        if (!hasReturnNonNullDefault
            && !hasParameterNonNullDefault.hasAnyNonNullDefault()
            && !complain
            && !this.environment.globalOptions.inheritNullAnnotations) {
        ...
        if (TypeBinding.notEquals(currentMethod.declaringClass, this.type)
            ...
            this.buddyImplicitNullAnnotationsVerifier.checkImplicitNullAnnotations(currentMethod, srcMethod, complain,
                scope);
        }
        super.checkNullSpecInheritance(currentMethod, srcMethod, hasReturnNonNullDefault, hasParameterNonNullDefault,
            complain, inheritedMethod, allInherited, scope, inheritedNonNullnessInfos);
    }
    ...
    void checkMethods() {
        ...
        boolean skipInheritedMethods = mustImplementAbstractMethods && canSkipInheritedMethods(); // have a single
                                                                                                  // concrete superclass
                                                                                                  // so only check
                                                                                                  // overridden methods
        ...
        nextSelector: for (int s = methodSelectors.length; --s >= 0;) {
            ...
            MethodBinding[] foundMatch = new MethodBinding[inheritedLength]; // null is no match, otherwise value is
                                                                             // matching currentMethod
            ...
            if (current != null) {
                for (MethodBinding currentMethod : current) {
                    ...
                    for (int j = 0; j < inheritedLength; j++) {
                        ...
                        if (inheritedMethod != null) {
                            if (foundMatch[j] == null
                                ...
                                // already checked compatibility, do visibility etc. also indicate overriding? If so
                                // ignore inheritedMethod further downstream
                    ...
                    if (index >= 0) {
                        ...
                        // if (index > 0 && currentMethod.declaringClass.isInterface()) // only check when inherited
                        // methods are from interfaces
                        // checkInheritedReturnTypes(matchingInherited, index + 1);
                        checkAgainstInheritedMethods(currentMethod, matchingInherited, index + 1, nonMatchingInherited); // pass
                                                                                                                         // in
                                                                                                                         // the
                                                                                                                         // length
                                                                                                                         // of
            ...
            for (int i = 0; i < inheritedLength; i++) {
                ...
                if (matchMethod == null && current != null && this.type.isPublic()) { // current == null case handled
                                                                                      // already.
                    MethodBinding inheritedMethod = inherited[i];
                    if (inheritedMethod.isPublic()
                        && (!inheritedMethod.declaringClass.isInterface()
                ...
                for (int j = i + 1; j < inheritedLength; j++) {
                    ...
                    if (TypeBinding.notEquals(inheritedMethod.declaringClass, otherInheritedMethod.declaringClass)) {
                        ...
                        if (isSkippableOrOverridden(inheritedMethod, otherInheritedMethod, skip, isOverridden,
                            isInherited, j))
                            continue;
                        if (isSkippableOrOverridden(otherInheritedMethod, inheritedMethod, skip, isOverridden,
                            isInherited, i))
                            continue;
                    }
                }
            ...
            for (int i = 0; i < inheritedLength; i++) {
                ...
                if (index > 0) {
                    ...
                    if (length != inheritedLength) { // transfer inherited & overridden status to align with subset of
                                                     // methods.
                    ...
                    checkInheritedMethods(matchingInherited, length, matchingIsOverridden, matchingIsInherited); // pass
                                                                                                                 // in
                                                                                                                 // the
                                                                                                                 // length
                                                                                                                 // of
     ...
     * - any skippable method as defined above iff it is actually overridden by the specific method (disregarding
     * visibility etc.)
     ...
     */
    boolean isSkippableOrOverridden(MethodBinding specific, MethodBinding general, boolean[] skip,
        boolean[] isOverridden, boolean[] isInherited, int idx) {
        ...
        if (!specificIsInterface && generalIsInterface) {
            if (!specific.isAbstract() && isParameterSubsignature(specific, general)) {
                // 8.4.8: abstract and default methods are not inherited if a concrete method with a subsignature is
                // defined or inherited in C
            ...
            if (specific.declaringClass.isCompatibleWith(general.declaringClass)
                && isMethodSubsignature(specific, general)) {
                skip[idx] = true;
                isOverridden[idx] = true;
                return true;
            }
     ...
     * - 'specific' has a signature that is a subsignature of the substituted signature of 'general' (as seen from
     * specific's declaring class)
     * - default methods should also be considered replaced by class methods that meet the signature that is a
     * subsignature criteria.
     */
    MethodBinding findReplacedMethod(MethodBinding specific, MethodBinding general) {
        ...
        if (generalSubstitute != null
            && (!specific.isAbstract()
                || general.isAbstract()
                || (general.isDefaultMethod() && specific.declaringClass.isClass()))	// if (abstract(specific) =>
                                                                                    	// abstract(general)) check if
                                                                                    	// 'specific' overrides
                                                                                    	// 'general'
                ...
                if (index > 0) {
                    ...
                    while (--count > 0) {
                        ...
                        if (interfaceMethod != null
                            ...
                            && !isAsVisible(implementation, interfaceMethod))
                            problemReporter().inheritedMethodReducesVisibility(typeParameter, implementation,
                                new MethodBinding[] { interfaceMethod });

                        if (areReturnTypesCompatible(first, match))
                            continue;
                        // unrelated interfaces - check to see if return types are compatible
                        if (first.declaringClass.isInterface()
                            && match.declaringClass.isInterface()
                            && areReturnTypesCompatible(match, first))
                            continue;
                        break;
                    }
                    if (count > 0) {  // All inherited methods do NOT have the same vmSignature
                        problemReporter().inheritedMethodsHaveIncompatibleReturnTypes(typeParameter, matchingInherited,
                            index + 1);
                        continue nextSelector;
                    }
                }
    ...
    boolean isAcceptableReturnTypeOverride(MethodBinding currentMethod, MethodBinding inheritedMethod) {
        ...
        if (originalInheritedReturnType.isParameterizedTypeWithActualArguments())
            return !currentMethod.returnType.leafComponentType().isRawType(); // raw types issue a warning if inherited
                                                                              // is parameterized
...
// caveat: returns false if a method is implemented, but with a return type that is incompatible with that of the interface method
    @Override
    boolean isInterfaceMethodImplemented(MethodBinding inheritedMethod, MethodBinding existingMethod,
        ...
        return TypeBinding.equalsEquals(inheritedMethod.returnType, existingMethod.returnType)
            || (TypeBinding.notEquals(this.type, existingMethod.declaringClass) // ... not if inheriting the bridge
                                                                                // situation from a superclass
                && !existingMethod.declaringClass.isInterface()
                && areReturnTypesCompatible(existingMethod, inheritedMethod)); // may have to report incompatible return
                                                                               // type
    }

    @Override
    public boolean isMethodSubsignature(MethodBinding method, MethodBinding inheritedMethod) {
        if (!com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.compiler.CharOperation
            .equals(method.selector, inheritedMethod.selector))
            return false;
    ...
    boolean isUnsafeReturnTypeOverride(MethodBinding currentMethod, MethodBinding inheritedMethod) {
        ...
        if (currentMethod.typeVariables == Binding.NO_TYPE_VARIABLES
            ...
            && currentMethod.returnType.erasure().findSuperTypeOriginatingFrom(inheritedMethod.returnType.erasure())
                != null) {
            return true;
        }
        return false;
