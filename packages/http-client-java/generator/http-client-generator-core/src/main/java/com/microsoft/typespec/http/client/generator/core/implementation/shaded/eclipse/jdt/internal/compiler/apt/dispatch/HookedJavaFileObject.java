...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.javax.tools.ForwardingJavaFileObject;
    ...
    protected void closed() {
        if (!this._closed) {
            ...
            switch (this.getKind()) {
                case SOURCE:
                    try {
                        CompilationUnit unit = new CompilationUnit(getCharContent(false).toString().toCharArray(),
                            this._fileName, this._encoding, null /* destination path */,
                ...
                case CLASS:
                    ...
                    try {
                        ...
                        /*
                         * When the annotation processor produces garbage, javac seems to show some resilience, by
                         * hooking the source type,
                         * which since is resolved can answer annotations during discovery - Not sure if this sanctioned
                         * by the spec, to be taken
                         * up with Oracle. Here we mimic the bug, see that addNewClassFile is simply collecting
                         * ReferenceBinding's, so adding
                         * a SourceTypeBinding works just fine.
                         */
                        ReferenceBinding type = this._filer._env._compiler.lookupEnvironment
                        ...
                        if (type != null && type.isValidBinding()) {
                            if (type.isBinaryBinding()) {
                                ...
                                BinaryTypeBinding binaryBinding = new BinaryTypeBinding(type.getPackage(), binaryType,
                                    this._filer._env._compiler.lookupEnvironment, true);
