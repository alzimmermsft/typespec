...
final class ModifierOrderer {
    ...
    static JavaInput reorderModifiers(JavaInput javaInput, Collection<Range<Integer>> characterRanges)
        ...
        while (it.hasNext()) {
            ...
            if (tokens.isEmpty()
                || !tokens.tokens().stream().allMatch(token -> tokenRanges.contains(token.getTok().getIndex()))) {
                continue;
            }
     ...
     * {@link com.microsoft.typespec.http.client.generator.core.implementation.shaded.javax.lang.model.element.Modifier},
     * or {@code null} if it
     * is not a modifier.
     */
