...
public abstract class TypeReference extends Expression {
    ...
    static class AnnotationCollector extends ASTVisitor {
        ...
        public AnnotationCollector(LocalDeclaration localDeclaration, int targetType,
            LocalVariableBinding localVariable, List<AnnotationContext> annotationContexts) {
            this.annotationContexts = annotationContexts;
        ...
        public AnnotationCollector(AbstractVariableDeclaration localDeclaration, int targetType, int parameterIndex,
            List<AnnotationContext> annotationContexts) {
            this.annotationContexts = annotationContexts;
            this.typeReference = localDeclaration.type;
            this.targetType = targetType;
        ...
        public AnnotationCollector(Expression typeReference, int targetType, int info,
            List<AnnotationContext> annotationContexts) {
            this.annotationContexts = annotationContexts;
            this.typeReference = typeReference;
            this.info = info;
        ...
        public AnnotationCollector(TypeReference typeReference, int targetType, int info, int typeIndex,
            List<AnnotationContext> annotationContexts) {
            this.annotationContexts = annotationContexts;
            this.typeReference = typeReference;
            this.info = info;
        ...
        public AnnotationCollector(TypeReference typeReference, int targetType, int info,
            List<AnnotationContext> annotationContexts, Annotation[][] annotationsOnDimensions, int dimensions) {
            this.annotationContexts = annotationContexts;
            this.typeReference = typeReference;
            this.info = info;
            this.targetType = targetType;
        ...
        public AnnotationCollector(RecordComponent recordComponent, int targetType,
            List<AnnotationContext> annotationContexts) {
            this.annotationContexts = annotationContexts;
        ...
        private boolean targetingTypeParameter() {
            return this.targetType == AnnotationTargetTypeConstants.CLASS_TYPE_PARAMETER
                || this.targetType == AnnotationTargetTypeConstants.METHOD_TYPE_PARAMETER;
        }

        private boolean internalVisit(Annotation annotation) {
            AnnotationContext annotationContext = null;
            if (annotation.isRuntimeTypeInvisible(targetingTypeParameter())) {
                annotationContext = new AnnotationContext(annotation, this.typeReference, this.targetType,
                    AnnotationContext.INVISIBLE);
            } else if (annotation.isRuntimeTypeVisible(targetingTypeParameter())) {
                annotationContext
                    = new AnnotationContext(annotation, this.typeReference, this.targetType, AnnotationContext.VISIBLE);
            }
...
// 				case AnnotationTargetTypeConstants.RECORD_COMPONENT : // value same as FIELD
        ...
        public boolean visit(TypeParameter typeParameter, BlockScope scope) {
            ...
            // their annotations are explicitly handled in TypeParameter.getAllAnnotationContexts(int, int,
...
// allows us to trap completion & selection nodes
    ...
    protected Annotation[][] getMergedAnnotationsOnDimensions(int additionalDimensions,
        ...
        /*
         * Note, we actually concatenate the additional annotations after base annotations, in bindings, they should
         * appear before base annotations.
         * Given @English int @Nullable [] x @NonNull []; the type x is a @NonNull arrays of of @Nullable arrays
         * of @English Strings, not the other
         * way about. Changing this in the compiler AST representation will cause too many ripples, so we leave it as
         * is. On the bindings, the type
     ...
     */
    public void getAllAnnotationContexts(int targetType, int info, List<AnnotationContext> allAnnotationContexts,
        Annotation[][] annotationsOnDimensions, int dimensions) {
        AnnotationCollector collector = new AnnotationCollector(this, targetType, info, allAnnotationContexts,
            annotationsOnDimensions, dimensions);
        this.traverse(collector, (BlockScope) null);
        if (annotationsOnDimensions != null) {
    ...
    public void getAllAnnotationContexts(int targetType, int info, int typeIndex,
        List<AnnotationContext> allAnnotationContexts) {
        AnnotationCollector collector
            = new AnnotationCollector(this, targetType, info, typeIndex, allAnnotationContexts);
        this.traverse(collector, (BlockScope) null);
    }
    ...
    /**
     * @param useSourceOrder if true annotations on dimensions are returned in source order, otherwise they are returned
     * per
     * how they ought to be interpreted by a type system, or external persistence view. For example, given the
     * following:
     * int @Nullable [] f @NonNull [] ==> f is really a @NonNull array of @Nullable arrays of ints. This is the type
     * system
     * view since extended dimensions bind more readily than type components that precede the identifier. This is how it
     * ought
     * to be encoded in bindings and how it ought to be persisted in class files. However for DOM/AST construction, we
     * need the
    ...
    protected TypeBinding internalResolveType(Scope scope, int location) {
        ...
        if (!(this instanceof QualifiedTypeReference)   // QualifiedTypeReference#getTypeBinding called above will have
    ...
    public TypeBinding resolveSuperType(ClassScope scope) {
        ...
        if (superType.isTypeVariable()) {
            if (this.resolvedType.isValidBinding()) {
                this.resolvedType = new ProblemReferenceBinding(getTypeName(), (ReferenceBinding) this.resolvedType,
    ...
    protected TypeBinding updateParameterizedTypeWithAnnotations(Scope scope, TypeBinding type,
        TypeReference[] argRefs) {
        if (argRefs != null) {
            ...
            if (type instanceof ParameterizedTypeBinding) {
                ...
                if (updatedArgs != null) {
                    return scope.environment()
                        .createParameterizedType(parameterizedType.genericType(), updatedArgs,
    ...
    protected void resolveAnnotations(Scope scope, int location) {
        ...
        if (this.annotations != null || annotationsOnDimensions != null) {
            ...
            if (resolutionScope != null) {
                ...
                if (this.annotations != null) {
                    ...
                    this.resolvedType = dimensions > 0
                        ? scope.environment().createArrayType(leafComponentType, dimensions)
                        : leafComponentType;
                    // contradictory null annotations on the type are already detected in Annotation.resolveType() (SE7
                    // treatment)
                }
                if (annotationsOnDimensions != null) {
                    ...
                    if (this.resolvedType instanceof ArrayBinding) {
                        ...
                        if (nullTagBitsPerDimension != null) {
                            for (int i = 0; i < dimensions; i++) { // skip last annotations at [dimensions] (concerns
                                                                   // the leaf type)
                                ...
                                if (nullTagBits == TagBits.AnnotationNullMASK) {
                                    ...
                                    if (scope.hasDefaultNullnessFor(Binding.DefaultLocationArrayContents,
                                        this.sourceStart))
                                        scope.problemReporter()
                                            .nullAnnotationIsRedundant(this, annotationsOnDimensions[i]);
                                }
                            }
                        }
                    }
        ...
        if (scope.compilerOptions().isAnnotationBasedNullAnalysisEnabled
            ...
            if (nullTagBits == 0) {
                ...
                if (location != Binding.DefaultLocationParameter) { // parameters are handled in
        ...
        if (variables != null && variables.length > rank) {
            ...
            if (variable.hasNullTypeAnnotations()) {
                if (NullAnnotationMatching
                    .analyse(variable, this.resolvedType, null, substitution, -1, null, CheckMode.BOUND_CHECK)
                    .isAnyMismatch())
    ...
    protected void checkIllegalNullAnnotation(Scope scope) {
        if (this.resolvedType.leafComponentType().isBaseType() && hasNullTypeAnnotation(AnnotationPosition.LEAF_TYPE))
            scope.problemReporter()
                .illegalAnnotationForBaseType(this, this.annotations[0],
                    this.resolvedType.tagBits & TagBits.AnnotationNullMASK);
    }
    ...
    public Annotation findAnnotation(long nullTagBits) {
        if (this.annotations != null) {
            ...
            if (innerAnnotations != null) {
                int annBit = nullTagBits == TagBits.AnnotationNonNull
    ...
    public static boolean containsNullAnnotation(Annotation[] annotations) {
        if (annotations != null) {
            for (Annotation annotation : annotations) {
                if (annotation != null
                    && (annotation.hasNullBit(TypeIds.BitNonNullAnnotation | TypeIds.BitNullableAnnotation)))
                    return true;
            }
        }
        return false;
