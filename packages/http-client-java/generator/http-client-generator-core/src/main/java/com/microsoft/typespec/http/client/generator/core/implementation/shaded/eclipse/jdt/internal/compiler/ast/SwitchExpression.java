...
public class SwitchExpression extends SwitchStatement implements IPolyExpression {
    ...
    class Results { // Abstraction to help with 15.28.1 determination of the type of a switch expression.
        ...
        private boolean allWellFormed = true; // true as long as result expression completely fail to resolve
                                              // (resolvedType == null)
        ...
        private TypeBinding resultType() {
            ...
            if (this.allUniform) {
                ...
                for (Expression rExpression : this.rExpressions)
                    uniformType = uniformType == null
                        ? rExpression.resolvedType
                        : NullAnnotationMatching.moreDangerousType(uniformType, rExpression.resolvedType);
                    ...
                    TypeBinding result = switch (type.id) {
                        ...
                        case T_short, T_byte, T_char -> {
                            if (this.rTypes.contains(type)) {
                                ...
                                for (Expression rExpression : this.rExpressions) {
                                    if (rExpression.resolvedType.id == T_int
                                        && rExpression.constant != Constant.NotAConstant
                                        && !rExpression.isConstantValueOfTypeAssignableToType(rExpression.resolvedType,
                                            type))
                                        yield TypeBinding.INT;
                                }
                                yield type;
            ...
            // Non-uniform, non-boolean, non-numeric: Force to reference versions, compute lub and apply capture and we
            // are done!
            ...
            int i = 0;
            for (Expression rExpression : this.rExpressions)
                resultReferenceTypes[i++] = rExpression.resolvedType.isBaseType()
                    ? env.computeBoxingType(rExpression.resolvedType)
                    : rExpression.resolvedType;
            ...
            SwitchExpression.this.scope.problemReporter().incompatibleSwitchExpressionResults(SwitchExpression.this); // Is
                                                                                                                      // this
                                                                                                                      // unreachable
                                                                                                                      // ?
                                                                                                                      // can
        ...
        /**
         * Add an expression to known result expressions, gather some aggregate characteristics if in standalone
         * context.
         * 
        ...
        public boolean add(/* @NonNull */ Expression rxpression, TypeBinding rxpressionType) {
            ...
            if (rxpressionType == null) { // tolerate poly-expression resolving to null in the absence of target type.
                if (!rxpression.isPolyExpression()
                    || ((IPolyExpression) rxpression).expectedType() != null
                    || SwitchExpression.this.expressionContext == VANILLA_CONTEXT)
            ...
            this.allUniform
                &= TypeBinding.equalsEquals(this.rExpressions.iterator().next().resolvedType, rxpression.resolvedType);
            ...
            if (this.allNumeric) {
                ...
                if (rxpressionType.id == T_int && rxpression.constant != Constant.NotAConstant) {
                    ...
                    if (i <= Character.MAX_VALUE && i >= Short.MIN_VALUE)
                        definiteType = false;
                    // else int constants may get reclassified as they undergo narrowing conversions - can't pin them
                    // yet.
                }
                if (definiteType)
    ...
    private LocalVariableBinding createStackSpillSlot(CodeStream codeStream, TypeBinding type, int typeId, int index,
        ...
        LocalVariableBinding lvb = new LocalVariableBinding(name, type, ClassFileConstants.AccDefault, false);
    ...
    private void spillOperandStackIfNeeded(CodeStream codeStream) {
        ...
        if (!this.jvmStackVolatile || (codeStream.stackDepth == 0 && operandStackSize == 0)) // nothing to spill and
                                                                                             // nothing to compain
            return;
        ...
        while (operandStackSize > 0) {
            ...
            LocalVariableBinding lvb
                = createStackSpillSlot(codeStream, type, TypeIds.T_undefined, index++, nextResolvedPosition);
        ...
        if (codeStream.stackDepth != 0 || codeStream.operandStack.size() != 0)
            codeStream.classFile.referenceBinding.scope.problemReporter()
                .operandStackSizeInappropriate(codeStream.classFile.referenceBinding.scope.referenceContext);
    ...
    public void refillOperandStackIfNeeded(CodeStream codeStream, YieldStatement yield) {
        if (!this.jvmStackVolatile
            || this.typesOnStack == null
            || this.typesOnStack.isEmpty()
            || (yield.bits & ASTNode.IsAnyFinallyBlockEscaping) != 0) // nothing spilled or no point restoring.
            return;
    ...
    public TypeBinding resolveType(BlockScope upperScope) {
        try {
            if (this.constant != Constant.NotAConstant) {
                ...
                super.resolve(upperScope); // drills down into switch block, which will cause yield expressions to be
                                           // discovered and added to `this.results`
                ...
                if (rExpression.isConstantValueOfTypeAssignableToType(rExpression.resolvedType, switchType)
                    ...
                    if (rExpression.resolvedType.needsUncheckedConversion(switchType))
                        this.scope.problemReporter()
                            .unsafeTypeConversion(rExpression, rExpression.resolvedType, switchType);
                    if (rExpression instanceof CastExpression
                        && (rExpression.bits & (ASTNode.UnnecessaryCast | ASTNode.DisableUnnecessaryCastCheck)) == 0)
                    ...
                    this.scope.problemReporter()
                        .typeMismatchError(rExpression.resolvedType, switchType, rExpression, null);
                    return null;
                }
            }
        }
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if (currentScope.compilerOptions().enableSyntacticNullAnalysisForFields)
            flowContext.expireNullCheckedFieldInfo(); // wipe information that was meant only for this result
                                                      // expression:
