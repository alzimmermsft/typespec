...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.codeassist.impl.AssistOptions;
...
public class InternalNamingConventions {
    ...
    private static Scanner getNameScanner(CompilerOptions compilerOptions) {
        return new Scanner(false /* comment */, false /* whitespace */, false /* nls */,
            compilerOptions.sourceLevel /* sourceLevel */, null /* taskTags */, null/* taskPriorities */,
            true/* taskCaseSensitive */, compilerOptions.enablePreviewFeatures);
    }

    private static void acceptName(char[] name, char[] prefix, char[] suffix, boolean isFirstPrefix,
        boolean isFirstSuffix, int reusedCharacters, INamingRequestor requestor) {
    ...
    private static char[][] computeNonBaseTypeNames(char[] sourceName, boolean isConstantField, boolean onlyLongest) {
        ...
        int previousCharKind = ScannerHelper.isLowerCase(c)
            ? IS_LOWER_CASE
            : ScannerHelper.isUpperCase(c) ? IS_UPPER_CASE : c == '_' ? IS_UNDERSCORE : IS_OTHER;

        for (int i = length - 1; i >= 0; i--) {
            c = sourceName[i];

            int charKind = ScannerHelper.isLowerCase(c)
                ? IS_LOWER_CASE
                : ScannerHelper.isUpperCase(c) ? IS_UPPER_CASE : c == '_' ? IS_UNDERSCORE : IS_OTHER;
                ...
                case IS_UPPER_CASE:
                    if (previousCharKind == IS_LOWER_CASE) {
                        ...
                        if (i > 0) {
                            ...
                            previousCharKind = ScannerHelper.isLowerCase(pc)
                                ? IS_LOWER_CASE
    ...
    public static char[] getBaseName(int variableKind, IJavaProject javaProject, char[] name,
    ...
    private static char[] getBaseName(char[] name, char[][] prefixes, char[][] suffixes, boolean isConstant,
        ...
        char[] nameWithoutPrefixAndSiffix
            = removeVariablePrefixAndSuffix(name, prefixes, suffixes, updateFirstCharacter);
        ...
        if (prefixes != null) {
            ...
            for (char[] prefix : prefixes) {
                if (CharOperation.prefixEquals(prefix, name)) {
                    ...
                    if (!lastCharIsLetter
                        || (lastCharIsLetter && name.length > currLen && ScannerHelper.isUpperCase(name[currLen]))) {
            ...
            for (char[] suffix : suffixes) {
                if (CharOperation.endsWith(withoutPrefixName, suffix)) {
                    ...
                    if (bestLength < currLen && withoutPrefixName.length != currLen) {
                        withoutSuffixName
                            = CharOperation.subarray(withoutPrefixName, 0, withoutPrefixName.length - currLen);
                        bestLength = currLen;
                    }
                }
            }
...
//		}
    ...
    public static void suggestVariableNames(int variableKind, int baseNameKind, char[] baseName,
        IJavaProject javaProject, int dim, char[] internalPrefix, char[][] excluded, boolean evaluateDefault,
            ...
            if (!isConstantField) {
                unprefixedName[0] = ScannerHelper.toUpperCase(unprefixedName[0]);
                    ...
                    if (j == internalPrefix.length
                        || CharOperation.prefixEquals(CharOperation.subarray(internalPrefix, j, -1), unprefixedName,
                            j != 0 /* do not check case when there is no prefix */)) {
                        matchingIndex = j;
                        break done;
                    }
                }
                    ...
                    if (j == internalPrefix.length) {
                        matchingIndex = j;
                        break done;
                    } else if (CharOperation.prefixEquals(CharOperation.subarray(internalPrefix, j, -1), unprefixedName,
                        j != 0 /* do not check case when there is no prefix */)) {
            ...
            if (matchingIndex > -1) {
                if (!isConstantField) {
                    ...
                    if (matchingIndex != 0 && tempName[0] != '_' && internalPrefix[matchingIndex - 1] != '_') {
                        tempName = CharOperation.concat(
                            CharOperation.subarray(CharOperation.toUpperCase(internalPrefix), 0, matchingIndex),
                            unprefixedName, '_');
                    } else {
                        tempName = CharOperation.concat(
                            CharOperation.subarray(CharOperation.toUpperCase(internalPrefix), 0, matchingIndex),
                ...
                for (int k = 0; k < prefixes.length; k++) {
                    ...
                    for (int l = 0; l < suffixes.length; l++) {
                        ...
                        suffixName = excludeNames(suffixName, prefixName, suffixes[l], excluded);
                        try {
                            ...
                            switch (nameScanner.getNextToken()) {
                                case TokenNameIdentifier:
                                    ...
                                    if (token == TerminalToken.TokenNameEOF
                                        && nameScanner.startPosition == suffixName.length) {
                                        if (!foundNames.includes(suffixName)) {
                                            acceptName(suffixName, prefixes[k], suffixes[l], k == 0, l == 0,
                                                internalPrefix.length - matchingIndex, requestor);
                                ...
                                default:
                                    suffixName = CharOperation.concat(prefixName, String.valueOf(1).toCharArray(),
                                        suffixes[l]);
                                    suffixName = excludeNames(suffixName, prefixName, suffixes[l], excluded);
                                    nameScanner.setSource(suffixName);
                                    switch (nameScanner.getNextToken()) {
