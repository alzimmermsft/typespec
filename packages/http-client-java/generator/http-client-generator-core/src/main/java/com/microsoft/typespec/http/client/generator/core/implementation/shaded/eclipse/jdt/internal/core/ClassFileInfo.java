...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.env.IBinaryField;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.env.IBinaryNestedType;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.env.IRecordComponent;
...
class ClassFileInfo extends OpenableElementInfo implements SuffixConstants {
    ...
    private void generateAnnotationsInfos(JavaElement member, IBinaryAnnotation[] binaryAnnotations, long tagBits,
     ...
     */
    private void generateAnnotationsInfos(JavaElement member, char[] parameterName,
        IBinaryAnnotation[] binaryAnnotations, long tagBits, Map<IJavaElement, IElementInfo> newElements) {
    ...
    private void generateAnnotationInfo(JavaElement parent, Map<IJavaElement, IElementInfo> newElements,
    ...
    private void generateAnnotationInfo(JavaElement parent, char[] parameterName,
        Map<IJavaElement, IElementInfo> newElements, IBinaryAnnotation annotationInfo, String memberValuePairName) {
    ...
    private void generateStandardAnnotationsInfos(JavaElement javaElement, char[] parameterName, long tagBits,
        ...
        if ((tagBits & TagBits.AnnotationPolymorphicSignature) != 0) {
            generateStandardAnnotation(javaElement, TypeConstants.JAVA_LANG_INVOKE_METHODHANDLE_$_POLYMORPHICSIGNATURE,
                Annotation.NO_MEMBER_VALUE_PAIRS, newElements);
        }
    ...
    private IMemberValuePair[] getRetentionPolicy(long tagBits) {
        ...
        if ((tagBits & TagBits.AnnotationRuntimeRetention) == TagBits.AnnotationRuntimeRetention) {
            ...
            retention = new String(CharOperation.concatWith(TypeConstants.JAVA_LANG_ANNOTATION_RETENTIONPOLICY, '.'))
     ...
     * Adds new handles to the given vector.
     */
    private void generateFieldInfos(IType type, IBinaryType typeInfo, Map<IJavaElement, IElementInfo> newElements,
        ArrayList<JavaElement> childrenHandles) {
        // Make the fields
     ...
     * Adds new handles to the given vector.
     */
    private void generateRecordComponentInfos(IType type, IBinaryType typeInfo,
        Map<IJavaElement, IElementInfo> newElements, ArrayList<JavaElement> childrenHandles) {
        // Make the fields
        ...
        for (IRecordComponent componentInfo : components) {
            BinaryField component
                = new BinaryField((JavaElement) type, DeduplicationUtil.toString(componentInfo.getName())) {
                    @Override
            ...
            generateAnnotationsInfos(component, componentInfo.getAnnotations(), componentInfo.getTagBits(),
                newElements);
        }
    }
    ...
    private void generateInnerClassHandles(IType type, IBinaryType typeInfo, ArrayList<JavaElement> childrenHandles) {
        ...
        if (innerTypes != null) {
            ...
            for (IBinaryNestedType binaryType : innerTypes) {
                IClassFile parentClassFile = pkg
                    .getClassFile(new String(ClassFile.unqualifiedName(binaryType.getName())) + SUFFIX_STRING_class);
                BinaryType innerType = new BinaryType((JavaElement) parentClassFile,
                    DeduplicationUtil.intern(ClassFile.simpleName(binaryType.getName())));
     ...
     * Adds new handles to the given vector.
     */
    private void generateMethodInfos(IType type, IBinaryType typeInfo, Map<IJavaElement, IElementInfo> newElements,
        ArrayList<JavaElement> childrenHandles, ArrayList<ITypeParameter> typeParameterHandles) {
        ...
        for (IBinaryMethod methodInfo : methods) {
            ...
            try {
                ...
                // protect against malformed .class file (e.g. com/sun/crypto/provider/SunJCE_b.class has a 'a' generic
                // signature)
                signature = methodInfo.getMethodDescriptor();
                pNames = Signature.getParameterTypes(new String(signature));
            }
            ...
            int occurrenceCount = 0;
            BinaryMethod method;
            // ensure that 2 binary methods with the same signature but with different return types have different
            // occurrence counts.
            ...
            for (int j = startIndex; j < max; j++) {
                IBinaryAnnotation[] parameterAnnotations
                    = methodInfo.getParameterAnnotations(j - startIndex, typeInfo.getFileName());
                if (parameterAnnotations != null) {
                    LocalVariable localVariable
                        = new LocalVariable(method, DeduplicationUtil.toString(argumentNames[j]), 0, -1, 0, -1,
                            method.parameterTypes[j], null, -1, true);
                    generateAnnotationsInfos(localVariable, argumentNames[j], parameterAnnotations,
                        methodInfo.getTagBits(), newElements);
                }
            }
     ...
     * Adds new handles to the given vector.
     */
    private void generateTypeParameterInfos(BinaryMember parent, char[] signature,
        Map<IJavaElement, IElementInfo> newElements, ArrayList<ITypeParameter> typeParameterHandles) {
        if (signature == null)
            return;
     ...
     */
    protected void readBinaryChildren(ClassFile classFile, Map<IJavaElement, IElementInfo> newElements,
        ...
        if (typeInfo != null) { // may not be a valid class file
            ...
            generateInnerClassHandles(type, typeInfo, childrenHandles); // Note inner class are separate openables that
                                                                        // are not opened here: no need to pass in
                                                                        // newElements
        }
