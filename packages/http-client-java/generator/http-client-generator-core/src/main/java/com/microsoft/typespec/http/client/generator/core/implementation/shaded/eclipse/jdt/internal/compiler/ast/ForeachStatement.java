...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.flow.LoopingFlowContext;
...
public class ForeachStatement extends Statement {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if (currentScope.compilerOptions().isAnnotationBasedNullAnalysisEnabled) {
            ...
            int nullStatus = NullAnnotationMatching.checkAssignment(currentScope, flowContext, elementVarBinding, null, // have
                                                                                                                        // no
                                                                                                                        // useful
                                                                                                                        // flowinfo
                                                                                                                        // for
            ...
            if ((this.kind == GENERIC_ITERABLE || this.kind == RAW_ITERABLE)
                && !currentScope.compilerOptions().usesNullTypeAnnotations()) {
                // respect declaration annotation on Iterator.next():
                ReferenceBinding iterator = currentScope.getJavaUtilIterator();
                if (iterator != null) {
                    MethodBinding next = iterator.getExactMethod(TypeConstants.NEXT, Binding.NO_TYPES,
                        currentScope.compilationUnitScope);
        ...
        if (!(this.action == null)) {
            ...
            if (this.action.complainIfUnreachable(actionInfo, this.scope, initialComplaintLevel, true)
                < Statement.COMPLAINED_UNREACHABLE) {
                actionInfo = this.action.analyseCode(this.scope, loopingContext, actionInfo).unconditionalCopy();
                FakedTrackingVariable.markForeachElementVar(this.elementVariable);
                // action.analyseCode() missed the following check due to identical scopes of ForeachStatement and
                // action:
                FlowInfo actionNullInfo = condInfo.copy().addNullInfoFrom(actionInfo); // previously action did not see
                                                                                       // nullinfo from condInfo
        ...
        if (loopingContext.hasEscapingExceptions()) { // https://bugs.eclipse.org/bugs/show_bug.cgi?id=321926
            ...
            if (this.continueLabel != null) {  // we do get to the bottom
                ...
                loopbackFlowInfo = loopbackFlowInfo
                    .mergedWith(loopbackFlowInfo.unconditionalCopy().addNullInfoFrom(actionInfo).unconditionalInits());
            }
        ...
        switch (this.kind) {
            case ARRAY:
                ...
                if (this.continueLabel != null) {
                    ...
                    // leave collectionVariable on execution stack (will be consumed when swapping condition further
                    // down)
                }
                break;
    ...
    public static TypeBinding getCollectionElementType(BlockScope scope, TypeBinding collectionType) {
        ...
        if (collectionType.isCapture()) {
            ...
            if (upperBound != null && upperBound.isArrayType())
                collectionType = upperBound; // partially anticipating the fix for
                                             // https://bugs.openjdk.java.net/browse/JDK-8013843
        }
        if (collectionType.isArrayType()) { // for(E e : E[])
            return ((ArrayBinding) collectionType).elementsType();
        } else if (collectionType instanceof ReferenceBinding) {
            ReferenceBinding iterableType
                = collectionType.findSuperTypeOriginatingFrom(T_JavaLangIterable, false /* Iterable is not a class */);
            ...
            if (arguments.length != 1)
                return null; // per construction can only be one
            return arguments[0];
        }
        return null;
    }
            ...
            if (elementType instanceof ReferenceBinding) {
                ...
                if (!elementType.canBeSeenBy(upperScope)) {
                    upperScope.problemReporter()
                        .invalidType(this.elementVariable,
                            new ProblemReferenceBinding(CharOperation.splitOn('.', refBinding.shortReadableName()),
                                refBinding, ProblemReasons.NotVisible));
                }
            }
        ...
        if (elementType != null && collectionType != null) {
            ...
            if (collectionType.isArrayType()) { // for(E e : E[])
                ...
                if (!this.collectionElementType.isCompatibleWith(elementType)
                    ...
                    this.scope.problemReporter()
                        .notCompatibleTypesErrorInForeach(this.collection, this.collectionElementType, elementType);
                    ...
                    this.scope.problemReporter()
                        .unsafeElementTypeConversion(this.collection, this.collectionElementType, elementType);
                }
                ...
                if (elementType.isBaseType()) {
                    ...
                    if (!this.collectionElementType.isBaseType()) {
                        ...
                        if (elementType.isBaseType()) {
                            ...
                            this.scope.problemReporter()
                                .autoboxing(this.collection, this.collectionElementType, elementType);
                        }
                    } else {
                        this.elementVariableImplicitWidening = (elementType.id << 4) + compileTimeTypeID;
                    }
                ...
                ReferenceBinding iterableType = collectionType.findSuperTypeOriginatingFrom(T_JavaLangIterable,
                    false /* Iterable is not a class */);
                checkIterable: {
                    if (iterableType == null)
                        break checkIterable;
                    ...
                    if (this.iteratorReceiverType.findSuperTypeOriginatingFrom(T_JavaLangIterable, false) == null) {
                        this.iteratorReceiverType = iterableType; // handle indirect inheritance thru variable secondary
                                                                  // bound
                    ...
                    switch (iterableType.kind()) {
                        case Binding.RAW_TYPE: // for(Object o : Iterable)
                            ...
                            if (!this.collectionElementType.isCompatibleWith(elementType)
                                && !this.scope.isBoxingCompatibleWith(this.collectionElementType, elementType)) {
                                this.scope.problemReporter()
                                    .notCompatibleTypesErrorInForeach(this.collection, this.collectionElementType,
                                        elementType);
                            }
                    ...
                    if (elementType.isBaseType()) {
                        if (!this.collectionElementType.isBaseType()) {
                            compileTimeTypeID
                                = this.scope.environment().computeBoxingType(this.collectionElementType).id;
                            this.elementVariableImplicitWidening = UNBOXING;
                            if (elementType.isBaseType()) {
                                this.elementVariableImplicitWidening |= (elementType.id << 4) + compileTimeTypeID;
                            }
                        ...
                        if (this.collectionElementType.isBaseType()) {
                            this.elementVariableImplicitWidening
                                = BOXING | (compileTimeTypeID << 4) | compileTimeTypeID; // use primitive type in
                                                                                         // implicit conversion
                        }
                    }
                }
            ...
            switch (this.kind) {
                case ARRAY:
                    ...
                    this.indexVariable = new LocalVariableBinding(SecretIndexVariableName, TypeBinding.INT,
                    ...
                    this.maxVariable = new LocalVariableBinding(SecretMaxVariableName, TypeBinding.INT,
                        ClassFileConstants.AccDefault, false);
                    ...
                    if (expectedCollectionType == null) {
                        this.collectionVariable = new LocalVariableBinding(SecretCollectionVariableName, collectionType,
                            ClassFileConstants.AccDefault, false);
                    } else {
                        this.collectionVariable = new LocalVariableBinding(SecretCollectionVariableName,
                            expectedCollectionType, ClassFileConstants.AccDefault, false);
                    }
                ...
                case GENERIC_ITERABLE:
                    ...
                    this.indexVariable = new LocalVariableBinding(SecretIteratorVariableName,
                        this.scope.getJavaUtilIterator(), ClassFileConstants.AccDefault, false);
                    this.scope.addLocalVariable(this.indexVariable);
                    this.indexVariable.setConstant(Constant.NotAConstant); // not inlinable
                    break;
