...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.codegen.ExceptionLabel;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.codegen.MultiCatchExceptionLabel;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.codegen.StackMapFrameCodeStream;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.flow.InsideStatementWithFinallyBlockFlowContext;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.tool.EclipseCompiler;
...
public class TryStatement extends StatementWithFinallyBlock {
    ...
    public LocalVariableBinding anyExceptionVariable, secretReturnValue;
    ...
    private static final int FINALLY_DOES_NOT_COMPLETE = 2;		// non returning finally is optimized with only one
                                                           		// instance of finally block
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if (this.finallyBlockStartLabel == null) {
            ...
            ExceptionHandlingFlowContext handlingContext = new ExceptionHandlingFlowContext(flowContext, this,
                this.caughtExceptionTypes, this.caughtExceptionsCatchBlocks, null, this.scope, flowInfo);
            ...
            for (int i = 0; i < resourcesLength; i++) {
                ...
                if (resource instanceof LocalDeclaration) {
                    ...
                    if (resource instanceof NameReference nameReference
                        && nameReference.binding instanceof LocalVariableBinding) {
                ...
                if (closeMethod != null
                    && closeMethod.isValidBinding()
                    && closeMethod.returnType.id == TypeIds.T_void) {
                    ReferenceBinding[] thrownExceptions = closeMethod.thrownExceptions;
                    for (ReferenceBinding thrownException : thrownExceptions) {
                        handlingContext.checkExceptionHandlers(thrownException, this.resources[i], tryInfo,
                            currentScope, true);
                    }
                }
            }
            ...
            if (resourcesLength > 0) {
                ...
                // to avoid polluting the state indices. However, do this after the postTryInitStateIndex is calculated
                // since
            ...
            if (this.catchArguments != null) {
                ...
                for (int i = 0; i < catchCount; i++) {
                    // keep track of the inits that could potentially have led to this exception handler (for final
                    // assignments diagnosis)
            ...
            ExceptionHandlingFlowContext handlingContext = new ExceptionHandlingFlowContext(insideSubContext, this,
                this.caughtExceptionTypes, this.caughtExceptionsCatchBlocks, null, this.scope, flowInfo);
            insideSubContext.initsOnFinally = handlingContext.initsOnFinally;

            finallyInfo
                = this.finallyBlock
                    .analyseCode(currentScope,
                        finallyContext = new FinallyFlowContext(flowContext, this.finallyBlock, handlingContext),
                        flowInfo.nullInfoLessUnconditionalCopy())
                    .unconditionalInits();
            handlingContext.conditionalLevel = 0; // start collection initsOnFinally only after analysing the finally
                                                  // block
            ...
            for (int i = 0; i < resourcesLength; i++) {
                ...
                if (resource instanceof LocalDeclaration) {
                    ...
                    if (localVariableBinding.closeTracker != null) {
                        // this was false alarm, we don't need to track the resource
                        localVariableBinding.closeTracker.withdraw();
                        // keep the tracking variable in the resourceBinding in order to prevent creating a new one
                        // while analyzing the try block
                    }
                } else { // Expression
                    if (resource instanceof NameReference
                        && ((NameReference) resource).binding instanceof LocalVariableBinding) {
            ...
            finallyContext.complainOnDeferredChecks(((tryInfo.tagBits & FlowInfo.UNREACHABLE) == 0
                ? flowInfo.unconditionalCopy().addPotentialInitializationsFrom(tryInfo).
                // lighten the influence of the try block, which may have
                // exited at any point
                    addPotentialInitializationsFrom(insideSubContext.initsOnReturn)
                : insideSubContext.initsOnReturn).addNullInfoFrom(handlingContext.initsOnFinally), currentScope);
            ...
            if (finallyInfo == FlowInfo.DEAD_END) {
                this.mergedInitStateIndex = currentScope.methodScope().recordInitializationStates(finallyInfo);
                return finallyInfo;
            } else {
    ...
    private void recordCallingClose(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo,
        Expression closeTarget) {
        FakedTrackingVariable trackingVariable = FakedTrackingVariable.getCloseTrackingVariable(closeTarget, flowInfo,
            flowContext, currentScope.compilerOptions().isAnnotationBasedResourceAnalysisEnabled);
    ...
    private MethodBinding findCloseMethod(final ASTNode resource, TypeBinding type) {
        ...
        if (type != null && type.isValidBinding() && type instanceof ReferenceBinding) {
            ...
            if (closeMethod == null) {
                ...
                closeMethod = this.scope.compilationUnitScope()
                    .findMethod(binding, ConstantPool.Close, new TypeBinding[0], site, false);
            }
        }
        return closeMethod;
    }

    private FlowInfo prepareCatchInfo(FlowInfo flowInfo, ExceptionHandlingFlowContext handlingContext, FlowInfo tryInfo,
        int i) {
        FlowInfo catchInfo;
...
// Return true if the catch block corresponds to an unchecked exception making allowance for multi-catch blocks.
    ...
    public void enterDeclaredExceptionHandlers(CodeStream codeStream) {
        for (int i = 0, length = this.declaredExceptionLabels == null ? 0 : this.declaredExceptionLabels.length;
            i < length; i++) {
        ...
        if (maxCatches > 0) {
            ...
            for (int i = 0; i < maxCatches; i++) {
                ...
                if ((argument.binding.tagBits & TagBits.MultiCatchParameter) != 0) {
                    ...
                    exceptionLabel
                        = new ExceptionLabel(codeStream, argument.binding.type, argument.type, argument.annotations);
                }
        ...
        try {
            ...
            if (resourceCount > 0) {
                ...
                for (int i = 0; i <= resourceCount; i++) {
                    // put null for the exception type to treat them as any exception handlers (equivalent to a
                    // try/finally)
            ...
            if (resourceCount > 0) {
                for (int i = resourceCount; i >= 0; i--) {
                    ...
                    if (this.resourceExceptionLabels[i].getCount() % 2 != 0) {
                        this.resourceExceptionLabels[i].placeEnd(); // outer handler if any is the one that should catch
                                                                    // exceptions out of close()
                    }
                    ...
                    if ((this.bits & ASTNode.IsTryBlockExiting) == 0) {
                        ...
                        if (i > 0) {
                            ...
                            if (this.postTryInitStateIndex != -1) {
                                /*
                                 * https://bugs.eclipse.org/bugs/show_bug.cgi?id=361053, we are just past a synthetic
                                 * instance of try-catch-finally.
                                 * Our initialization type state is the same as it was at the end of the just concluded
                                 * try (catch rethrows)
                                 */
                                codeStream.removeNotDefinitelyAssignedVariables(currentScope,
                                    this.postTryInitStateIndex);
                    ...
                    if (i == resourceCount) {
                        ...
                        BranchLabel elseLabel = new BranchLabel(codeStream),
                            postElseLabel = new BranchLabel(codeStream);
                    ...
                    if (i > 0) {
                        ...
                        generateCodeSnippet(stmt, codeStream, postCloseLabel, true /* record */, i,
                            codeStream.position);
    ...
    private void generateCodeSnippet(Statement statement, CodeStream codeStream, BranchLabel postCloseLabel,
        boolean record, int... values) {
        ...
        if (statement instanceof LocalDeclaration)
            generateCodeSnippet((LocalDeclaration) statement, codeStream, postCloseLabel, record, i,
                invokeCloseStartPc);
    ...
    private void generateCodeSnippet(Reference reference, CodeStream codeStream, BranchLabel postCloseLabel,
        boolean record, int i, int invokeCloseStartPc) {
    ...
    private void generateCodeSnippet(LocalDeclaration localDeclaration, CodeStream codeStream,
        BranchLabel postCloseLabel, boolean record, int i, int invokeCloseStartPc) {
    ...
    private boolean isDuplicateResourceReference(int index) {
        ...
        if (index < len && this.resources[index] instanceof Reference) {
            ...
            Binding refBinding = ref instanceof NameReference
                ? ((NameReference) ref).binding
            ...
            for (int i = 0; i < index; i++) {
                ...
                Binding b = stmt instanceof LocalDeclaration
                    ? ((LocalDeclaration) stmt).binding
                    : stmt instanceof NameReference
                        ? ((NameReference) stmt).binding
    ...
    public void resolve(BlockScope upperScope) {
        ...
        if ((swich = enclosingSwitchExpression(upperScope)) != null) {
            swich.jvmStackVolatile = true; // ought to prepare for any raised exception blowing up the the operand stack
                                           // to smithereens
        }
        BlockScope finallyScope = null;
        BlockScope resourceManagementScope = null; // Single scope to hold all resources and additional secret
                                                   // variables.
        int resourceCount = this.resources.length;
        if (resourceCount > 0) {
            ...
            this.caughtThrowableVariable = new LocalVariableBinding(TryStatement.SECRET_CAUGHT_THROWABLE_VARIABLE_NAME,
                this.scope.getJavaLangThrowable(), ClassFileConstants.AccDefault, false);
            ...
            if (this.resources[i] instanceof LocalDeclaration) {
                ...
                if (localVariableBinding != null && localVariableBinding.isValidBinding()) {
                    ...
                    if (resourceType instanceof ReferenceBinding) {
                        if (resourceType.findSuperTypeOriginatingFrom(TypeIds.T_JavaLangAutoCloseable,
                            false /* AutoCloseable is not a class */) == null && resourceType.isValidBinding()) {
                            upperScope.problemReporter().resourceHasToImplementAutoCloseable(resourceType, node.type);
                            localVariableBinding.type = new ProblemReferenceBinding(
                                CharOperation.splitOn('.', resourceType.shortReadableName()), null,
                                ProblemReasons.InvalidTypeForAutoManagedResource);
                        }
                    } else if (resourceType != null) { // https://bugs.eclipse.org/bugs/show_bug.cgi?id=349862, avoid
                                                       // secondary error in problematic null case
                        upperScope.problemReporter().resourceHasToImplementAutoCloseable(resourceType, node.type);
                        localVariableBinding.type
                            = new ProblemReferenceBinding(CharOperation.splitOn('.', resourceType.shortReadableName()),
                                null, ProblemReasons.InvalidTypeForAutoManagedResource);
                    }
                }
            } else { // expression
                ...
                if (node instanceof NameReference && (node.bits & Binding.VARIABLE) == 0) {
                    ...
                    if (resourceType instanceof ReferenceBinding) {
                        if (resourceType.findSuperTypeOriginatingFrom(TypeIds.T_JavaLangAutoCloseable,
                            ...
                            ((Expression) this.resources[i]).resolvedType = new ProblemReferenceBinding(
                                CharOperation.splitOn('.', resourceType.shortReadableName()), null,
                                ProblemReasons.InvalidTypeForAutoManagedResource);
                        }
                    } else if (resourceType != null) { // https://bugs.eclipse.org/bugs/show_bug.cgi?id=349862, avoid
                                                       // secondary error in problematic null case
                        upperScope.problemReporter().resourceHasToImplementAutoCloseable(resourceType, node);
                        ((Expression) this.resources[i]).resolvedType
                            = new ProblemReferenceBinding(CharOperation.splitOn('.', resourceType.shortReadableName()),
                                null, ProblemReasons.InvalidTypeForAutoManagedResource);
                    }
            ...
            if (this.finallyBlock.isEmptyBlock()) {
                if ((this.finallyBlock.bits & ASTNode.UndocumentedEmptyBlock) != 0) {
                    this.scope.problemReporter()
                        .undocumentedEmptyBlock(this.finallyBlock.sourceStart, this.finallyBlock.sourceEnd);
                }
            } else {
                ...
                if (!methodScope.isInsideInitializer()) {
                    MethodBinding methodBinding = methodScope.referenceContext instanceof AbstractMethodDeclaration
                        ...
                        : (methodScope.referenceContext instanceof LambdaExpression
                            ? ((LambdaExpression) methodScope.referenceContext).binding
                            : null);
                    ...
                    for (int k = 0; k < i; k++) {
                        ...
                        for (int n = 0, max2 = exceptions.length; n < max2; n++) {
                            ...
                            if (exception.isCompatibleWith(currentException)) {
                                ...
                                this.scope.problemReporter()
                                    .wrongSequenceOfExceptionTypesError(catchArgumentType, exception, currentException);
                                break loop;
                            }
                        }
                    }
            ...
            for (int i = 0; i < length; i++) {
                ...
                for (int j = 0; j < i; j++) {
                    if (this.caughtExceptionTypes[i].isCompatibleWith(argumentTypes[j])) {
                        this.scope.problemReporter()
                            .wrongSequenceOfExceptionTypesError(this.catchArguments[i].type,
