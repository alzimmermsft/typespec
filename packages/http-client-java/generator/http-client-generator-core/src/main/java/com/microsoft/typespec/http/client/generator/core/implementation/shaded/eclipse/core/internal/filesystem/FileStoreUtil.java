...
public final class FileStoreUtil {
    ...
    private static int compareNormalisedUri(URI uri1, URI uri2) {
        ...
        if (((c = compareStringOrNull(uri1.getAuthority(), uri2.getAuthority())) != 0)
            || ((c = compareStringOrNull(uri1.getScheme(), uri2.getScheme())) != 0)
            || ((c = comparePathSegments(uri1.getPath(), uri2.getPath())) != 0)
            || ((c = compareStringOrNull(uri1.getQuery(), uri2.getQuery())) != 0)) {
            return c;
        }
        return c;
    }
    ...
    public static int compareFileStore(IFileStore fileStore1, IFileStore fileStore2) {
        try {
            ...
            throw (NullPointerException) new NullPointerException(
                String.format("(fileStore1: %s %s; fileStore2: %s %s)", fileStore1.getClass().getName(), fileStore1, //$NON-NLS-1$
                    fileStore2.getClass().getName(), fileStore2)).initCause(e);
        }
