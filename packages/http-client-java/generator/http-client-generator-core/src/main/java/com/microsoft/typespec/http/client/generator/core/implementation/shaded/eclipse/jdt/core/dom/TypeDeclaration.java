...
public class TypeDeclaration extends AbstractTypeDeclaration {
    ...
    /**
     * The "bodyDeclarations" structural property of this node type (element type: {@link BodyDeclaration}) (added in
     * JLS3 API).
     * 
     * @since 3.0
     */
     ...
     * (see constructor).
     */
    private NodeList superInterfaceNames = null;
     ...
     * @since 3.1
     */
    private NodeList superInterfaceTypes = null;
     ...
     * @since 3.22
     */
    private NodeList permittedTypes = null;
    ...
    TypeDeclaration(AST ast) {
        ...
        if (ast.apiLevel == AST.JLS2_INTERNAL) {
            this.superInterfaceNames = new NodeList(SUPER_INTERFACES_PROPERTY);
        }
        if (ast.apiLevel >= AST.JLS3_INTERNAL) {
            this.typeParameters = new NodeList(TYPE_PARAMETERS_PROPERTY);
            this.superInterfaceTypes = new NodeList(SUPER_INTERFACE_TYPES_PROPERTY);
        }
        if (DOMASTUtil.isFeatureSupportedinAST(ast, Modifier.SEALED)) {
            this.permittedTypes = new NodeList(PERMITS_TYPES_PROPERTY);
        }
    }
    ...
    ASTNode clone0(AST target) {
        ...
        if (DOMASTUtil.isFeatureSupportedinAST(this.ast, Modifier.SEALED)) {
            result.permittedTypes().addAll(ASTNode.copySubtrees(target, permittedTypes()));
    ...
    int treeSize() {
        return memSize() + (this.optionalDocComment == null ? 0 : getJavadoc().treeSize())
            ...
            + (this.permittedTypes == null ? 0 : this.permittedTypes.listSize()) + this.bodyDeclarations.listSize();
    }
