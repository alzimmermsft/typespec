...
public abstract class AbstractDataTreeNode {
     ...
     */
    static AbstractDataTreeNode[] assembleWith(AbstractDataTreeNode[] oldNodes, AbstractDataTreeNode[] newNodes,
        boolean keepDeleted) {
        ...
        while (oldIndex < oldNodes.length && newIndex < newNodes.length) {
            ...
            if (log2 > 1 && (newNodes.length - newIndex) <= (oldNodes.length - oldIndex) / log2) {
                // We can expect to fare better using binary search. In particular, this will optimize the case of a
                // folder refresh (new linked
                // folder with many files in a flat hierarchy), where this is called repeatedly, with oldNodes
                // containing the files added so far,
                // and newNodes containing exactly one new node for the next file to be added. The old algorithm has
                // quadratic performance
                // (O((n+1)*n/2); number of string comparisons is the dominating component here) in this case; this new
                // algorithm does O(n*log(n))
                // string comparisons.
    ...
    protected static AbstractDataTreeNode[] compareWith(AbstractDataTreeNode[] oldNodes,
        AbstractDataTreeNode[] newNodes, IComparator comparator) {
    ...
    protected static AbstractDataTreeNode[] compareWithParent(AbstractDataTreeNode[] nodes, IPath key,
        DeltaDataTree parent, IComparator comparator) {
    ...
    static AbstractDataTreeNode convertToAddedComparisonNode(AbstractDataTreeNode newNode, int userComparison) {
        ...
        return new DataTreeNode(newNode.name,
            new NodeComparison(null, newNode.getData(), NodeComparison.K_ADDED, userComparison), convertedChildren);
    }
    ...
    static AbstractDataTreeNode convertToRemovedComparisonNode(AbstractDataTreeNode oldNode, int userComparison) {
        ...
        return new DataTreeNode(oldNode.name,
            new NodeComparison(oldNode.getData(), null, NodeComparison.K_REMOVED, userComparison), convertedChildren);
    }
     ...
     */
    protected static AbstractDataTreeNode[] simplifyWithParent(AbstractDataTreeNode[] nodes, IPath key,
        DeltaDataTree parent, IComparator comparer) {
        ...
        if (simplifiedCount < simplifiedNodes.length) {
            System.arraycopy(simplifiedNodes, 0, simplifiedNodes = new AbstractDataTreeNode[simplifiedCount], 0,
                simplifiedCount);
        }
