...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.LambdaExpression.LocalTypeSubstitutor;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.NLSTag;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.util.HashSetOfInt;
...
public class CompilationUnitDeclaration extends ASTNode implements ProblemSeverities, ReferenceContext {
    ...
    public void finalizeProblems() {
        ...
        nextProblem: for (int iProblem = 0, length = problemCount; iProblem < length; iProblem++) {
            ...
            nextSuppress: for (int iSuppress = 0, suppressCount = this.suppressWarningsCount; iSuppress < suppressCount;
                iSuppress++) {
        ...
        if (!hasMandatoryErrors) {
            ...
            if (severity != ProblemSeverities.Ignore) {
                ...
                for (int iSuppress = 0, suppressCount = this.suppressWarningsCount; iSuppress < suppressCount;
                    iSuppress++) {
                                        ...
                                        for (int iToken = 0, tokenCount = inits.length; iToken < tokenCount; iToken++) {
                                            ...
                                            if (cst != Constant.NotAConstant
                                                ...
                                                if (tokenIrritants != null) {
                                                    if (!tokenIrritants.areAllSet() // no complaint against
                                                                                    // @SuppressWarnings("all")
                                                        && (foundIrritants[iSuppress] == null
                                                            || !foundIrritants[iSuppress].isAnySet(tokenIrritants))) { // if
                                                                                                                       // irritant
                                                                                                                       // had
                                                                                                                       // no
                                                                                                                       // matching
                                                        ...
                                                        if (unusedWarningTokenIsWarning) {
                                                            ...
                                                            nextSuppress: for (int jSuppress = iSuppress - 1;
                                                                jSuppress >= 0; jSuppress--) {
                                                                long position
                                                                    = this.suppressWarningScopePositions[jSuppress];
                                                                int startSuppress = (int) (position >>> 32);
                                                                int endSuppress = (int) position;
                                                                if (start < startSuppress)
                                                                    continue nextSuppress;
                                                                if (end > endSuppress)
                                                                    continue nextSuppress;
                                                                if (this.suppressWarningIrritants[jSuppress]
                                                                    .areAllSet())
                                                        ...
                                                        if (id > 0) {
                                                            ...
                                                            this.scope.problemReporter()
                                                                .problemNotAnalysed(inits[iToken], key);
                                                        } else {
                                                            this.scope.problemReporter()
                                                                .unusedWarningToken(inits[iToken]);
                                                        }
                                                    }
                                                }
                                            }
                                    ...
                                    if (cst != Constant.NotAConstant && cst.typeID() == T_JavaLangString) {
                                        ...
                                        if (tokenIrritants != null) {
                                            if (!tokenIrritants.areAllSet() // no complaint against
                                                ...
                                                if (unusedWarningTokenIsWarning) {
                                                    int start = value.sourceStart, end = value.sourceEnd;
                                                    nextSuppress: for (int jSuppress = iSuppress - 1; jSuppress >= 0;
                                                        jSuppress--) {
    ...
    public void recordStringLiteral(StringLiteral literal, boolean fromRecovery) {
        ...
        if (this.stringLiterals == null) {
            ...
            if (this.stringLiteralsPtr == stackLength) {
                System.arraycopy(this.stringLiterals, 0,
                    this.stringLiterals = new StringLiteral[stackLength + STRING_LITERALS_INCREMENT], 0, stackLength);
            }
        }
    ...
    private boolean isLambdaExpressionCopyContext(ReferenceContext context) {
        ...
        Scope cScope = context instanceof AbstractMethodDeclaration
            ? ((AbstractMethodDeclaration) context).scope
            : context instanceof TypeDeclaration
                ? ((TypeDeclaration) context).scope
                : context instanceof LambdaExpression ? ((LambdaExpression) context).scope : null;
    ...
    public void recordSuppressWarnings(IrritantSet irritants, Annotation annotation, int scopeStart, int scopeEnd,
        ...
        if (this.suppressWarningIrritants == null) {
            ...
            System.arraycopy(this.suppressWarningIrritants, 0,
                this.suppressWarningIrritants = new IrritantSet[2 * this.suppressWarningsCount], 0,
                this.suppressWarningsCount);
            System.arraycopy(this.suppressWarningAnnotations, 0,
                this.suppressWarningAnnotations = new Annotation[2 * this.suppressWarningsCount], 0,
                this.suppressWarningsCount);
            System.arraycopy(this.suppressWarningScopePositions, 0,
                this.suppressWarningScopePositions = new long[2 * this.suppressWarningsCount], 0,
                this.suppressWarningsCount);
        }
    ...
    public void updateLocalTypesInMethod(MethodBinding methodBinding) {
        if (this.localTypes == Collections.EMPTY_MAP)
            return;
        LambdaExpression.updateLocalTypesInMethod(methodBinding,
            new LocalTypeSubstitutor(this.localTypes, methodBinding), new NullSubstitution(this.scope.environment()));
    }
    ...
    public int record(FunctionalExpression expression) {
        if (this.functionalExpressionsCount == 0) {
            ...
            System.arraycopy(this.functionalExpressions, 0,
                (this.functionalExpressions = new FunctionalExpression[this.functionalExpressionsCount * 2]), 0,
    ...
    public void resolve() {
        ...
        if (this.types != null && isPackageInfo) {
            ...
            /*
             * resolve javadoc package if any, skip this step if we don't have a valid scope due to an earlier error
             * (bug 252555)
             * we do it now as the javadoc in the fake type won't be resolved. The peculiar usage of MethodScope to
             * resolve the
             * package level javadoc is because the CU level resolve method is a NOP to mimic Javadoc's behavior and
             * can't be used
             * as such.
             */
            ...
            if (this.javadoc != null) {
                ...
                if (severity != ProblemSeverities.Ignore) {
                    reporter.javadocModuleMissing(this.moduleDeclaration.declarationSourceStart,
                        this.moduleDeclaration.bodyStart, severity);
                }
            }
        } else {
        ...
        if (this.nlsTags != null || this.stringLiterals != null) {
            ...
            if (stringLiteralsLength == 0) {
                ...
                if (this.stringLiterals.length != stringLiteralsLength) {
                    System.arraycopy(this.stringLiterals, 0,
                        (this.stringLiterals = new StringLiteral[stringLiteralsLength]), 0, stringLiteralsLength);
                }
                ...
                stringLiteralsLoop: for (; i < stringLiteralsLength; i++) {
                    ...
                    final int literalLineNumber
                        = literal instanceof TextBlock textBlock ? textBlock.endLineNumber : literal.getLineNumber();
