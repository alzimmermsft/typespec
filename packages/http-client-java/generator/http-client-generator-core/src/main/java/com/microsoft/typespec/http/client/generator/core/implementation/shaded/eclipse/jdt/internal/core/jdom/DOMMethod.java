...
class DOMMethod extends DOMMember implements IDOMMethod {
     ...
     */
    DOMMethod(char[] document, int[] sourceRange, String name, int[] nameRange, int[] commentRange, int flags,
        int[] modifierRange, boolean isConstructor, String returnType, int[] returnTypeRange, String[] parameterTypes,
        String[] parameterNames, int[] parameterRange, String[] exceptions, int[] exceptionRange, int[] bodyRange) {
     ...
     * or <code>null</code> if the method throws no exceptions
     */
    DOMMethod(char[] document, int[] sourceRange, String name, int[] nameRange, int flags, boolean isConstructor,
        String returnType, String[] parameterTypes, String[] parameterNames, String[] exceptions) {
        this(document, sourceRange, name, nameRange, new int[] { -1, -1 }, flags, new int[] { -1, -1 }, isConstructor,
            returnType, new int[] { -1, -1 }, parameterTypes, parameterNames, new int[] { -1, -1 }, exceptions,
    ...
    protected void appendMemberDeclarationContents(CharArrayBuffer buffer) {
        ...
        if (isConstructor()) {
            ...
            if (this.fReturnTypeRange[0] >= 0) {
                buffer.append(this.fDocument, this.fReturnTypeRange[1] + 1,
                    this.fNameRange[0] - this.fReturnTypeRange[1] - 1);
            } else {
                buffer.append(' ');
            }
        ...
        if (this.fParameterList != null) {
            ...
            buffer.append(this.fDocument, this.fParameterRange[0],
                this.fParameterRange[1] + 1 - this.fParameterRange[0]);
        }
        int start;
        ...
        if (this.fExceptions != null) {
            ...
            if (this.fExceptionList != null) {
                ...
                if (this.fExceptionRange[0] >= 0) {
                    buffer.append(this.fDocument, this.fExceptionRange[1] + 1,
                        this.fBodyRange[0] - this.fExceptionRange[1] - 1);
                } else {
                    buffer.append(this.fDocument, this.fParameterRange[1] + 1,
                        this.fBodyRange[0] - this.fParameterRange[1] - 1);
                }
            } else {
    ...
    public boolean isSignatureEqual(IDOMNode node) {
        ...
        if (ok) {
            ...
            ok = (isConstructor() && method.isConstructor()) || (!isConstructor() && !method.isConstructor());
            if (ok && !isConstructor()) {
    ...
    /**
     * @see IDOMMethod#setDefault(String)
     * @since 3.0
     */
    @Override
    ...
    /**
     * @see IDOMMethod#setTypeParameters(String[])
     * @since 3.0
     */
    @Override
