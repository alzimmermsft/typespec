                    ...
                    if (args[i] instanceof PointerArray) {
                        ...
                        if (Structure.ByReference[].class.isAssignableFrom(inArg.getClass())) {
                            Class<? extends Structure> type
                                = (Class<? extends Structure>) inArg.getClass().getComponentType();
    ...
    private Object convertArgument(Object[] args, int index, Method invokingMethod, TypeMapper mapper,
        ...
        if (arg instanceof Structure) {
                ...
                if (!Structure.ByReference[].class.isAssignableFrom(expectedType)) {
                    if (byRef) {
                        throw new IllegalArgumentException("Function " + getName()
                            + " declared Structure[] at parameter " + index + " but array of " + type + " was passed");
                    }
                    for (int i = 0; i < ss.length; i++) {
                        if (ss[i] instanceof Structure.ByReference) {
                            throw new IllegalArgumentException(
                                "Function " + getName() + " declared Structure[] at parameter " + index
                                    + " but element " + i + " is of Structure.ByReference type");
                        }
                    }
                }
            }
            ...
            throw new IllegalArgumentException("Unsupported argument type " + arg.getClass().getName()
                + " at parameter " + index + " of function " + getName());
        }
        return arg;
    }
