 ...
 * <p>
 * Replaces
 * {@link IDocument#search(int, String, boolean, boolean, boolean)}.
 *
 * @since 3.0
 */
public class FindReplaceDocumentAdapter implements CharSequence {
     ...
     * @return the find or replace region or <code>null</code> if there was no match
     * @throws BadLocationException if startOffset is an invalid document offset
     * @throws IllegalStateException if a REPLACE or REPLACE_FIND operation is not preceded by a successful FIND
     * operation
     * @throws PatternSyntaxException if a regular expression has invalid syntax
     */
    private IRegion findReplace(final FindReplaceOperationCode operationCode, int startOffset, String findString,
        String replaceText, boolean forwardSearch, boolean caseSensitive, boolean wholeWord, boolean regExSearch)
        ...
        if (operationCode == FIND_FIRST) {
            ...
            if (fFindReplaceMatcher != null
                && fFindReplaceMatcher.pattern().pattern().equals(findString)
                && fFindReplaceMatcher.pattern().flags() == patternFlags) {
                /*
        ...
        if (operationCode == REPLACE || operationCode == REPLACE_FIND_NEXT) {
            ...
            if (fDocument instanceof IRepairableDocumentExtension
                && ((IRepairableDocumentExtension) fDocument).isLineInformationRepairNeeded(offset, length,
                    replaceText)) {
        ...
        if (operationCode != REPLACE) {
            try {
                ...
                int length = -1;
                while (found
                    && fFindReplaceMatcher.start() + fFindReplaceMatcher.group().length()
        ...
        switch (ch) {
            ...
            case 'c':
                if (i + 1 < length) {
                    ...
                    String msg
                        = TextMessages.getFormattedString("FindReplaceDocumentAdapter.illegalControlEscape", "\\c"); //$NON-NLS-1$ //$NON-NLS-2$
            ...
            case 'x':
                if (i + 2 < length) {
                    ...
                    try {
                        ...
                        String msg = TextMessages.getFormattedString("FindReplaceDocumentAdapter.illegalHexEscape", //$NON-NLS-1$
            ...
            case 'u':
                if (i + 4 < length) {
                    ...
                    try {
                        ...
                        String msg = TextMessages.getFormattedString("FindReplaceDocumentAdapter.illegalUnicodeEscape", //$NON-NLS-1$
