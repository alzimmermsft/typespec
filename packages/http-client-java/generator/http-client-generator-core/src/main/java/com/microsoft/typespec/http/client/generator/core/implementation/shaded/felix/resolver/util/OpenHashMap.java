...
public class OpenHashMap<K, V> implements Serializable, Cloneable, SortedMap<K, V> {
    ...
    protected transient Iterable<Entry<K, V>> fast;
    protected transient SortedSet<Entry<K, V>> entries;
    ...
    public String toString() {
        ...
        Iterator<Entry<K, V>> i = this.fast().iterator();
        int n = this.size();
        boolean first = true;
        s.append("{");
    ...
    public Iterable<Entry<K, V>> fast() {
        if (fast == null) {
    ...
    private class MapIterator {
        ...
        int next;
        /**
         * The last entry that was returned (or -1 if we did not iterate or used {@link Iterator#remove()}).
         */
        int curr = -1;
        /**
