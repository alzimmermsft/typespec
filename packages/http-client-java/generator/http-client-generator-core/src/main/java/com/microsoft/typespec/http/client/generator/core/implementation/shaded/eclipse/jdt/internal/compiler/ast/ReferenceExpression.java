...
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.ExpressionContext.INVOCATION_CONTEXT;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.codegen.CodeStream;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.codegen.Opcodes;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.flow.FieldInitsFakingFlowContext;
...
public class ReferenceExpression extends FunctionalExpression implements IPolyExpression, InvocationSite {
    ...
    private TypeBinding[] freeParameters; // descriptor parameters as used for method lookup - may or may not include
                                          // the receiver
    ...
    public void initialize(CompilationResult result, Expression expression, TypeReference[] optionalTypeArguments,
        char[] identifierOrNew, int sourceEndPosition) {
    ...
    private ReferenceExpression copy() {
        ...
        ReferenceExpression copy = (ReferenceExpression) parser.parseReferenceExpression(source, this.sourceStart,
            this.sourceEnd - this.sourceStart + 1, this.enclosingScope.referenceCompilationUnit(),
    ...
    public void generateImplicitLambda(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
        ...
        LambdaExpression implicitLambda = new LambdaExpression(this.compilationResult, false,
            (this.binding.modifiers & ExtraCompilerModifiers.AccGenericSignature) != 0);
        Argument[] arguments = new Argument[argc];
        for (int i = 0; i < argc; i++)
            arguments[i] = new Argument(CharOperation.append(ImplicitArgName, Integer.toString(i).toCharArray()), 0,
                null, 0, true);
        ...
        if (isMethodReference()) {
            ...
            Expression receiver
                = generateSecretReceiverVariable ? new SingleNameReference(this.receiverVariable.name, 0) : copy.lhs;
            ...
            message.receiver = this.receiverPrecedesParameters
                ? new SingleNameReference(CharOperation.append(ImplicitArgName, Integer.toString(0).toCharArray()), 0)
                : receiver;
            ...
            if (this.lhs instanceof ArrayTypeReference) {
                ...
                arrayAllocationExpression.type = arrayTypeReference.dimensions == 1
                ...
                arrayAllocationExpression.type = arrayQualifiedTypeReference.dimensions == 1
                    ? new QualifiedTypeReference(arrayQualifiedTypeReference.tokens,
                        arrayQualifiedTypeReference.sourcePositions)
                    : new ArrayQualifiedTypeReference(arrayQualifiedTypeReference.tokens,
                        arrayQualifiedTypeReference.dimensions - 1, arrayQualifiedTypeReference.sourcePositions);
            }
            ...
            if (this.lhs instanceof TypeReference) {
                ...
                allocation.type = new QualifiedTypeReference(((QualifiedNameReference) this.lhs).tokens,
                    new long[((QualifiedNameReference) this.lhs).tokens.length]);
            } else {
        ...
        // Process the lambda, taking care not to double report diagnostics. Don't expect any from resolve, Any from
        // code generation should surface, but not those from flow analysis.
        BlockScope lambdaScope = this.receiverVariable != null ? this.receiverVariable.declaringScope : currentScope;
        IErrorHandlingPolicy oldPolicy
            = lambdaScope.problemReporter().switchErrorHandlingPolicy(silentErrorHandlingPolicy);
        try {
            implicitLambda.resolveTypeWithBindings(patternVariablesInScope, lambdaScope, true);
            implicitLambda.analyseCode(lambdaScope,
                new FieldInitsFakingFlowContext(null, this, Binding.NO_EXCEPTIONS, null, lambdaScope,
                    FlowInfo.DEAD_END),
                UnconditionalFlowInfo.fakeInitializedFlowInfo(implicitLambda.firstLocalLocal,
                    lambdaScope.referenceType().maxFieldCount));
        } finally {
    ...
    private boolean shouldGenerateImplicitLambda(BlockScope currentScope) {
        ...
        return (this.binding.isVarargs()
            || (isConstructorReference()
                && (this.receiverType.syntheticOuterLocalVariables() != null || this.shouldCaptureInstance))
        ...
        if (this.binding != null) {
                ...
                if (TypeBinding.notEquals(this.binding.declaringClass, this.lhs.resolvedType.erasure())) {
                    ...
                    if (!this.binding.declaringClass.canBeSeenBy(this.enclosingScope)) {
                        if (this.binding.isDefaultMethod())
                            return false; // workaround for bug in MethodHandle lookup, see
                                          // https://bugs.openjdk.java.net/browse/JDK-8068253
            ...
            if (descriptorParams.length > 0) {
                ...
                if (this.receiverType.isIntersectionType18()
                    || (this.receiverType.isTypeVariable()
                        && ((TypeVariableBinding) this.receiverType).boundsCount() > 1)) {
                    if (!CharOperation.equals(this.binding.original().declaringClass.signature(),
                        this.receiverType.signature()))
                        return false;
                }
            }
        }
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
        ...
        if (this.receiverType.isArrayType()) {
            ...
            if (isConstructorReference()) {
                this.actualMethodBinding
                    = this.binding = sourceType.addSyntheticArrayMethod((ArrayBinding) this.receiverType,
                        SyntheticMethodBinding.ArrayConstructor, lambdaName);
            } else if (CharOperation.equals(this.selector, TypeConstants.CLONE)) {
                this.actualMethodBinding
                    = this.binding = sourceType.addSyntheticArrayMethod((ArrayBinding) this.receiverType,
                        SyntheticMethodBinding.ArrayClone, lambdaName);
            }
        } else if (this.syntheticAccessor != null) {
            if (this.lhs.isSuper() || isMethodReference())
                this.binding = this.syntheticAccessor;
        } else { // cf. MessageSend.generateCode()'s call to CodeStream.getConstantPoolDeclaringClass. We have extracted
                 // the relevant portions sans side effect here.
            if (this.binding != null && isMethodReference()) {
                if (TypeBinding.notEquals(this.binding.declaringClass, this.lhs.resolvedType.erasure())) {
                    if (!this.binding.declaringClass.canBeSeenBy(currentScope)) {
                        this.binding = new MethodBinding(this.binding.original(),
                            (ReferenceBinding) this.lhs.resolvedType.erasure());
                    }
                }
            }
        }
        ...
        if (this.haveReceiver) {
            ...
            if (isMethodReference() && !this.lhs.isThis() && !this.lhs.isSuper()) {
                MethodBinding mb = currentScope.getJavaLangObject()
                    .getExactMethod(TypeConstants.GETCLASS, Binding.NO_PARAMETERS, currentScope.compilationUnitScope());
                codeStream.dup();
            ...
            if (this.isConstructorReference()) {
                ...
                if (this.receiverType.isNestedType()) {
                    ...
                    if ((enclosingInstances = nestedType.syntheticEnclosingInstanceTypes()) != null) {
                        ...
                        for (int i = 0; i < length; i++) {
                            ...
                            Object[] emulationPath = currentScope.getEmulationPath(syntheticArgumentType,
                ...
                if (this.syntheticAccessor != null) {
                    char[] lambdaName = CharOperation.concat(TypeConstants.ANONYMOUS_METHOD,
                        Integer.toString(this.ordinal).toCharArray());
                    this.binding = sourceType.addSyntheticFactoryMethod(this.binding, this.syntheticAccessor,
                        enclosingInstances, lambdaName);
        ...
        codeStream.invokeDynamic(invokeDynamicNumber, argumentsSize, 1, this.descriptor.selector,
            buffer.toString().toCharArray(), this.isConstructorReference(),
            (this.lhs instanceof TypeReference ? (TypeReference) this.lhs : null), this.typeArguments,
            this.resolvedType);
        if (!valueRequired)
            codeStream.pop();
    ...
    public void manageSyntheticAccessIfNecessary(BlockScope currentScope, FlowInfo flowInfo) {
            ...
            if (codegenBinding.isPrivate()
                ...
                if ((allocatedType.tagBits & TagBits.IsLocalType) != 0) {
                    ...
                    this.syntheticAccessor
                        = ((SourceTypeBinding) allocatedType).addSyntheticMethod(codegenBinding, false);
        ...
        if (this.binding.isPrivate()) {
            if (TypeBinding.notEquals(enclosingSourceType, codegenBinding.declaringClass)) {
                this.syntheticAccessor = ((SourceTypeBinding) codegenBinding.declaringClass)
                    .addSyntheticMethod(codegenBinding, false /* not super access */);
        ...
        if (this.lhs.isSuper()) {
            ...
            if (this.lhs instanceof QualifiedSuperReference) { 	// qualified super
                QualifiedSuperReference qualifiedSuperReference = (QualifiedSuperReference) this.lhs;
                TypeReference qualification = qualifiedSuperReference.qualification;
                if (!qualification.resolvedType.isInterface()) // we can't drop the bridge in I, it may not even be a
                                                               // source type.
        ...
        if (this.binding.isProtected()
            && (this.bits & ASTNode.DepthMASK) != 0
            && codegenBinding.declaringClass.getPackage() != enclosingSourceType.getPackage()) {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if (compilerOptions.isAnyEnabled(IrritantSet.UNLIKELY_ARGUMENT_TYPE)
            && this.binding.isValidBinding()
            && this.binding != null
            && this.binding.parameters != null) {
            if (this.binding.parameters.length == 1
                ...
                final TypeBinding actualReceiverType
                    = this.receiverPrecedesParameters ? this.descriptor.parameters[0] : this.binding.declaringClass;
                UnlikelyArgumentCheck argumentCheck
                    = UnlikelyArgumentCheck.determineCheckForNonStaticSingleArgumentMethod(argumentType, currentScope,
                        this.selector, actualReceiverType, this.binding.parameters);
                if (argumentCheck != null && argumentCheck.isDangerous(currentScope)) {
                    currentScope.problemReporter()
                        .unlikelyArgumentType(this, this.binding, argumentType, argumentCheck.typeToReport,
                            argumentCheck.dangerousMethod);
                }
            } else if (this.binding.parameters.length == 2
                && this.descriptor.parameters.length == 2
                && this.binding.isStatic()) {
                final TypeBinding argumentType1 = this.descriptor.parameters[0];
                final TypeBinding argumentType2 = this.descriptor.parameters[1];
                UnlikelyArgumentCheck argumentCheck
                    = UnlikelyArgumentCheck.determineCheckForStaticTwoArgumentMethod(argumentType2, currentScope,
                        this.selector, argumentType1, this.binding.parameters, this.receiverType);
                if (argumentCheck != null && argumentCheck.isDangerous(currentScope)) {
                    currentScope.problemReporter()
                        .unlikelyArgumentType(this, this.binding, argumentType2, argumentCheck.typeToReport,
                            argumentCheck.dangerousMethod);
                }
            }
        }
        ...
        if (compilerOptions.analyseResourceLeaks) {
            if (this.haveReceiver && CharOperation.equals(this.selector, TypeConstants.CLOSE)) {
                FakedTrackingVariable trackingVariable = FakedTrackingVariable.getCloseTrackingVariable(this.lhs,
                    flowInfo, flowContext, compilerOptions.isAnnotationBasedResourceAnalysisEnabled);
                if (trackingVariable != null) { // null happens if target is not a local variable or not an
                                                // AutoCloseable
                    trackingVariable.markClosedInNestedMethod(); // there is a close()-call, but we don't know if it
                                                                 // will be invoked
                }
            }
        }
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (this.constant != Constant.NotAConstant) {
            ...
            if (this.typeArguments != null) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    if ((this.resolvedTypeArguments[i] = typeReference.resolveType(scope, true /* check bounds */))
                        == null) {
                        this.typeArgumentsHaveErrors = true;
                    }
                    if (this.typeArgumentsHaveErrors && typeReference instanceof Wildcard) { // resolveType on wildcard
                                                                                             // always return null
                                                                                             // above,
                                                                                             // resolveTypeArgument is
                                                                                             // the real workhorse.
            ...
            if (this.lhs instanceof NameReference) {
                if ((this.lhs.bits & ASTNode.RestrictiveFlagMASK) == Binding.TYPE) {
                    ...
                    scope.problemReporter()
                        .invalidType(this.lhs, new ProblemReferenceBinding(((NameReference) this.lhs).getName(), null,
            ...
            if (!lhsType.isRawType()) // RawType::m and RawType::new are not exact method references
                this.binding = this.exactMethodBinding = isMethodReference()
            ...
            if (this.lhs instanceof TypeReference
                && ((TypeReference) this.lhs).hasNullTypeAnnotation(AnnotationPosition.ANY)) {
            ...
            if (isMethodReference() && this.haveReceiver && (this.original == this)) {
                this.receiverVariable
                    = new LocalVariableBinding((SecretReceiverVariableName + this.nameSourceStart).toCharArray(),
                        this.lhs.resolvedType, ClassFileConstants.AccDefault, false);
        ...
        /*
         * For Reference expressions unlike other call sites, we always have a receiver _type_ since LHS of :: cannot be
         * empty.
        ...
        /*
         * 15.13: "If a method reference expression has the form super :: [TypeArguments] Identifier or TypeName . super
         * :: [TypeArguments] Identifier,
         * it is a compile-time error if the expression occurs in a static context. ": This is nop since the primary
         * when it resolves
         * itself will complain automatically.
         * 
         * 15.13: "The immediately enclosing instance of an inner class instance (15.9.2) must be provided for a
         * constructor reference by a lexically
         * enclosing instance of this (8.1.3)", we will actually implement this check in code generation. Emulation path
         * computation will fail if there
         * is no suitable enclosing instance. While this could be pulled up to here, leaving it to code generation is
         * more consistent with Java 5,6,7
         * modus operandi.
         */
        ...
        if (isConstructorReference() && lhsType.isArrayType()) {
            if (parametersLength != 1
                || scope.parameterCompatibilityLevel(descriptorParameters[0], TypeBinding.INT)
                    == Scope.NOT_COMPATIBLE) {
            ...
            if (this.descriptor.returnType.isProperType(true)
                ...
                && this.descriptor.returnType.id != TypeIds.T_void) {
        ...
        if (isMethodReference) {
            ...
            if (this.receiverType instanceof LocalTypeBinding local && !local.isRecord()) { // local records are
                                                                                            // implicitly static and
                                                                                            // don't have enclosing
                                                                                            // instance
            ...
            if (argumentsTypeElided() && this.receiverType.isRawType()) {
                ...
                someMethod = AllocationExpression.inferDiamondConstructor(scope, this, this.receiverType,
                    this.descriptor.parameters, inferredReturnType);
            }
            if (someMethod == null)
        ...
        if (this.typeArguments != null && this.binding.original().typeVariables == Binding.NO_TYPE_VARIABLES)
            scope.problemReporter()
                .unnecessaryTypeArgumentsForMethodInvocation(this.binding, this.resolvedTypeArguments,
                    this.typeArguments);
        ...
        if (checkInvocationArguments(scope, null, this.receiverType, this.binding, null, descriptorParameters, false,
            this))
    ...
    protected void checkNullAnnotations(BlockScope scope) {
        ...
        if (compilerOptions.isAnnotationBasedNullAnalysisEnabled) {
            if (this.expectedType == null || !NullAnnotationMatching.hasContradictions(this.expectedType)) { // otherwise
                                                                                                             // assume
                                                                                                             // it has
                                                                                                             // been
                                                                                                             // reported
                ...
                if (this.receiverPrecedesParameters) {
                    ...
                    if ((descriptorParameter.tagBits & TagBits.AnnotationNullable) != 0) { // Note: normal dereferencing
                                                                                           // of 'unchecked' values is
                                                                                           // not reported, either
                        final TypeBinding receiver
                            = scope.environment().createNonNullAnnotatedType(this.binding.declaringClass);
                        scope.problemReporter()
                            .referenceExpressionArgumentNullityMismatch(this, receiver, descriptorParameter,
                                this.descriptor, -1, NullAnnotationMatching.NULL_ANNOTATIONS_MISMATCH);
                    }
                }
                boolean isVarArgs = false;
                if (this.binding.isVarargs()) {
                    isVarArgs = (providedLen == expectedlen)
                        ? !this.descriptor.parameters[expectedlen - 1]
                            .isCompatibleWith(this.binding.parameters[expectedlen - 1])
                        : true;
                ...
                for (int i = 0; i < len; i++) {
                    TypeBinding descriptorParameter
                        = this.descriptor.parameters[i + (this.receiverPrecedesParameters ? 1 : 0)];
                    TypeBinding bindingParameter
                        = InferenceContext18.getParameter(this.binding.parameters, i, isVarArgs);
                    ...
                    if (bindingParameter.isPrimitiveType() && !descriptorParameter.isPrimitiveType()) {
                        ...
                        bindingParameterToCheck = scope.environment()
                            .createAnnotatedType(scope.boxing(bindingParameter),
                                new AnnotationBinding[] { scope.environment().getNonNullAnnotation() });
                    } else {
                        bindingParameterToCheck = bindingParameter;
                    }
                    NullAnnotationMatching annotationStatus = NullAnnotationMatching.analyse(bindingParameterToCheck,
                        descriptorParameter, FlowInfo.UNKNOWN);
                    if (annotationStatus.isAnyMismatch()) {
                        // immediate reporting:
                        scope.problemReporter()
                            .referenceExpressionArgumentNullityMismatch(this, bindingParameter, descriptorParameter,
                                this.descriptor, i, annotationStatus);
                    }
                }
                TypeBinding returnType = this.binding.returnType;
                if (!returnType.isPrimitiveType()) {
                    if (this.binding.isConstructor()) {
                        returnType = scope.environment()
                            .createAnnotatedType(this.receiverType,
                                new AnnotationBinding[] { scope.environment().getNonNullAnnotation() });
                    }
                    NullAnnotationMatching annotationStatus
                        = NullAnnotationMatching.analyse(this.descriptor.returnType, returnType, FlowInfo.UNKNOWN);
                    if (annotationStatus.isAnyMismatch()) {
                        scope.problemReporter()
                            .illegalReturnRedefinition(this, this.descriptor, annotationStatus.isUnchecked(),
                                returnType);
                    }
                }
            }
    ...
    // Cache resolved copies against various target types, so repeat overload resolution and possibly type inference
    // could be avoided.
    private ReferenceExpression cachedResolvedCopy(TypeBinding targetType) {
        ...
        IErrorHandlingPolicy oldPolicy
            = this.enclosingScope.problemReporter().switchErrorHandlingPolicy(silentErrorHandlingPolicy);
        try {
            copy = copy();
    ...
    public ReferenceExpression resolveExpressionExpecting(TypeBinding targetType, Scope scope,
        ...
        return copy != null
            && copy.resolvedType != null
            && copy.resolvedType.isValidBinding()
            && copy.binding != null
            && copy.binding.isValidBinding() ? copy : null;
    }
    ...
    public Expression[] createPseudoExpressions(TypeBinding[] p) {
        ...
        // (the different sets of types are passed from our resolveType to scope.getMethod(..), see someMethod,
        // anotherMethod)
    ...
    public boolean isPotentiallyCompatibleWith(TypeBinding targetType, Scope scope) {
        ...
        try {
            MethodBinding compileTimeDeclaration
                = getCompileTimeDeclaration(scope, isConstructorRef, descriptorParameters);
            ...
            if (compileTimeDeclaration != null && compileTimeDeclaration.isValidBinding()) // we have the mSMB.
                ...
                /*
                 * We EITHER have potential methods that are input to Scope.mSMb already captured in
                 * this.potentialMethods
                 * OR there is no potentially compatible compile time declaration ...
                 */
            }

            /*
             * 15.12.2.1: A method reference expression (§15.13) is potentially compatible with a functional interface
             * type if, where the type's function type arity is n,
             * there exists at least one potentially applicable method for the method reference expression with arity n
             * (§15.13.1), and one of the following is true:
             * – The method reference expression has the form ReferenceType ::[TypeArguments] Identifier and at least
             * one potentially applicable method is
             * i) static and supports arity n, or ii) not static and supports arity n-1.
             * – The method reference expression has some other form and at least one potentially applicable method is
             * not static.
             */
            ...
            System.arraycopy(descriptorParameters, 1, descriptorParameters = new TypeBinding[parametersLength - 1], 0,
                parametersLength - 1);
            this.freeParameters = descriptorParameters;
    ...
    MethodBinding getCompileTimeDeclaration(Scope scope, boolean isConstructorRef, TypeBinding[] parameters) {
        if (this.exactMethodBinding != null)
            ...
            return scope.findMethodForArray((ArrayBinding) this.receiverType, this.selector, Binding.NO_PARAMETERS,
                this);
    ...
    public boolean isCompatibleWith(TypeBinding targetType, Scope scope) {
        ...
        } else if (copy.resolvedType != null
            && copy.resolvedType.isValidBinding()
            && copy.binding != null
            && copy.binding.isValidBinding()) {
            return true;
        } else {
            boolean notPertinentToApplicability
                = targetType instanceof ParameterizedTypeBinding && !isPertinentToApplicability(targetType, null); // not
                                                                                                                   // mentioned
                                                                                                                   // in
                                                                                                                   // JLS
                                                                                                                   // (see
