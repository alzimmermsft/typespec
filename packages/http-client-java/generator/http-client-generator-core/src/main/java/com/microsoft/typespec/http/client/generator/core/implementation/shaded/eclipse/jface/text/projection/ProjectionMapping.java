...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jface.text.IDocumentInformationMappingExtension;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jface.text.IDocumentInformationMappingExtension2;
 ...
 * <p>
 * Implementation of {@link IDocumentInformationMapping}
 ...
 */
public class ProjectionMapping implements IDocumentInformationMapping, IDocumentInformationMappingExtension,
    IDocumentInformationMappingExtension2, IMinimalMapping {
     ...
     */
    public ProjectionMapping(IDocument masterDocument, String fragmentsCategory, IDocument slaveDocument,
     ...
     */
    private IRegion toImageRegion(IRegion originRegion, boolean exact, boolean takeClosestImage)
     ...
     * @throws BadLocationException if the region is not a valid origin region
     */
    private Fragment[] findFragments(IRegion originRegion, boolean exact, boolean takeClosestImage)
        throws BadLocationException {
        Position[] fragments = getFragments();
        if (fragments.length == 0)
            return null;
    ...
    public IRegion toOriginRegion(IRegion imageRegion) throws BadLocationException {
        ...
        if (imageLength == 0) {
            if (imageOffset == 0) {
                Position[] fragments = getFragments();
                if (fragments.length == 0
                    || (fragments.length == 1 && fragments[0].getOffset() == 0 && fragments[0].getLength() == 0))
    ...
    public IRegion[] toExactOriginRegions(IRegion imageRegion) throws BadLocationException {
        ...
        // first
        result[0] = createOriginStartRegion((Segment) segments[firstIndex],
            imageRegion.getOffset() - segments[firstIndex].getOffset());
        // middles
    ...
    public IRegion[] toExactImageRegions(IRegion originRegion) throws BadLocationException {
        ...
        // first
        result[0]
            = createImageStartRegion((Fragment) fragments[firstIndex], offset - fragments[firstIndex].getOffset());
        // middles
        for (int i = 1; i < resultLength - 1; i++)
    ...
    public IRegion[] getExactCoverage(IRegion originRegion) throws BadLocationException {
        ...
        // first
        result[0] = createOriginStartRegion((Fragment) fragments[firstIndex],
            originOffset - fragments[firstIndex].getOffset());
        // middles
        for (int i = 1; i < resultLength - 1; i++)
