...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.ParameterizedSingleTypeReference;
...
public class ParameterizedTypeBinding extends ReferenceBinding implements Substitution, HotSwappable {
    ...
    public ParameterizedTypeBinding(ReferenceBinding type, TypeBinding[] arguments, ReferenceBinding enclosingType,
        ...
        this.enclosingType = enclosingType; // never unresolved, but if type is an unresolved nested type, enclosingType
                                            // is null here but set later in swapUnresolved.
        ...
        if ((this.tagBits & TagBits.PassedBoundCheck) == 0) {
            ...
            if (this.arguments != null && typeVariables != null) { // arguments may be null in error cases
                ...
                for (int i = 0, length = typeVariables.length; i < length; i++) {
                    BoundCheckStatus checkStatus
                        = typeVariables[i].boundCheck(this, this.arguments[i], scope, argumentReferences[i]);
                    ...
                    if (!checkStatus.isOKbyJLS() && (this.arguments[i].tagBits & TagBits.HasMissingType) == 0) {
                        ...
                        scope.problemReporter()
                            .typeMismatchError(this.arguments[i], typeVariables[i], this.type, argumentReferences[i]);
                    }
                }
            }
            if (!hasErrors)
    ...
    public boolean canBeInstantiated() {
        return ((this.tagBits & TagBits.HasDirectWildcard) == 0) && super.canBeInstantiated(); // cannot instantiate
                                                                                               // param type with
                                                                                               // wildcard arguments
    }
    ...
    public TypeBinding findSuperTypeOriginatingFrom(TypeBinding otherType) {
        ...
        if (otherType instanceof ReferenceBinding && this.typeArgumentDepth() <= otherType.typeArgumentDepth()) { // protect
                                                                                                                  // against
                                                                                                                  // infinite
                                                                                                                  // growth
                                                                                                                  // of
        ...
        if (otherType instanceof ReferenceBinding otherRef
            && TypeBinding.equalsEquals(this.type, otherRef.actualType()))
            return this;
    ...
    public ParameterizedTypeBinding capture(Scope scope, int start, int end) {
        ...
        try {
            ...
            if (contextType != null)
                contextType = contextType.outermostEnclosingType(); // maybe null when used programmatically by DOM
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (argument.kind() == Binding.WILDCARD_TYPE) { // no capture for intersection types
                    ...
                    if (wildcard.boundKind == Wildcard.SUPER && wildcard.bound.id == TypeIds.T_JavaLangObject)
                        ...
                        capturedArguments[i] = this.environment.createCapturedWildcard(wildcard, contextType, start,
                            end, cud, compilationUnitScope::nextCaptureID);
                } else {
                    capturedArguments[i] = argument;
                }
            }
            ParameterizedTypeBinding capturedParameterizedType = this.environment.createParameterizedType(this.type,
                capturedArguments, enclosingType(), this.typeAnnotations);
            for (int i = 0; i < length; i++) {
    ...
    public TypeBinding uncapture(Scope scope) {
        ...
        return scope.environment()
            .createParameterizedType(this.type, freeTypes,
                (ReferenceBinding) (this.enclosingType != null ? this.enclosingType.uncapture(scope) : null),
                this.typeAnnotations);
    }

    /**
     * @see org.eclipse.jdt.internal.compiler.lookup.TypeBinding#collectMissingTypes(List)
     */
    @Override
     ...
     * e.g. {@code Collection<T>.collectSubstitutes(Collection<List<X>>, Map)} will populate Map with:
     * {@code T --> List<X>}
     * Constraints:
     * 
     * <pre>
    ...
    public void collectSubstitutes(Scope scope, TypeBinding actualType, InferenceContext inferenceContext,
        ...
        if (formalEnclosingType != null) {
            formalEnclosingType.collectSubstitutes(scope, actualEquivalent.enclosingType(), inferenceContext,
                constraint);
        }
                    ...
                    if (constraint == TypeConstants.CONSTRAINT_SUPER) { // JLS 15.12.7, p.459
                        switch (actualWildcardArgument.boundKind) {
                            case Wildcard.EXTENDS:
                                formalArgument.collectSubstitutes(scope, actualWildcardArgument.bound, inferenceContext,
    ...
    public ReferenceBinding enclosingType() {
        if (this.type instanceof UnresolvedReferenceBinding && ((UnresolvedReferenceBinding) this.type).depth() > 0) {
            ((UnresolvedReferenceBinding) this.type).resolve(this.environment, false); // may set enclosingType as side
                                                                                       // effect
        }
     ...
     * (non-Javadoc)
     * 
     * @see org.eclipse.jdt.internal.compiler.lookup.TypeBinding#upwardsProjection(com.microsoft.typespec.http.client.
     * generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.Scope,
    ...
    public ReferenceBinding upwardsProjection(Scope scope, TypeBinding[] mentionedTypeVariables) {
        ...
        if (typeVariables == null)
            return this; // How would that be possible?
        ...
        for (int i = 0, length = typeVariables.length; i < length; i++) {
            ...
            if (!a_i.mentionsAny(mentionedTypeVariables, -1)) {
                ...
                if (g_vars == null || g_vars.length == 0)
                    return this; // Careful - could be a MissingTypeBinding here
                ...
                if (u.id != TypeIds.T_JavaLangObject
                    ...
                    if (l != null) {
                        ...
                        a_i_primes[i]
                            = this.environment().createWildcard(genericType(), i, null, null, Wildcard.UNBOUND);
                    }
                }
            } else {
                ...
                if (wildcard.boundKind() == Wildcard.EXTENDS) {
                    ...
                    if (l != null) {
                        ...
                        // if the downward projection of the wildcard bound is undefined, then Ai' is an unbounded
                        // wildcard, ?.
                    ...
                    if (method.parameters.length == argCount) {
                        ...
                        for (int iarg = 0; iarg < argCount; iarg++)
                            if (TypeBinding.notEquals(toMatch[iarg], argumentTypes[iarg]))
                                continue nextMethod;
                        if (match != null)
                            return null; // collision case
                        match = method;
                    }
                }
            }
        } else {
            MethodBinding[] matchingMethods = getMethods(TypeConstants.INIT); // takes care of duplicates & default
                                                                              // abstract methods
    ...
    /**
     * @see org.eclipse.jdt.internal.compiler.lookup.ReferenceBinding#getExactMethod(char[],
    ...
    public MethodBinding getExactMethod(char[] selector, TypeBinding[] argumentTypes, CompilationUnitScope refScope) {
        ...
        if ((this.tagBits & TagBits.AreMethodsComplete) != 0) { // have resolved all arg types & return type of the
            ...
            MethodBinding[] matchingMethods = getMethods(selector); // takes care of duplicates & default abstract
                                                                    // methods
        ...
        try {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (useNullTypeAnnotations)
                    parameterizedMethods[i]
                        = NullAnnotationMatching.checkForContradictions(parameterizedMethods[i], null, null);
            }
            if (this.methods == null) {
            ...
            // if the original methods cannot be retrieved (ex. AbortCompilation), then assume we do not have any
            // methods
    ...
    void initialize(ReferenceBinding someType, TypeBinding[] someArguments) {
        ...
        // this.fields = null;
        // this.methods = null;
        this.modifiers = someType.modifiers & ~ExtraCompilerModifiers.AccGenericSignature; // discard generic signature,
                                                                                           // will compute later
        ...
        if (someArguments != null) {
            ...
            this.tagBits |= this.enclosingType.tagBits
                & (TagBits.HasTypeVariable | TagBits.HasMissingType | TagBits.HasCapturedWildcard);
        }
        if (someArguments != null) {
            ...
            for (TypeBinding someArgument : someArguments) {
                switch (someArgument.kind()) {
                    ...
                    case Binding.INTERSECTION_TYPE:
                        this.tagBits |= TagBits.HasDirectWildcard | TagBits.IsBoundParameterizedType; // Surely NOT
                                                                                                      // X<?,?>, see
                ...
                this.tagBits |= someArgument.tagBits & (TagBits.HasTypeVariable | TagBits.HasMissingType
                    | TagBits.ContainsNestedTypeReferences | TagBits.HasCapturedWildcard);
            }
        }
        this.tagBits |= someType.tagBits
            & (TagBits.IsLocalType | TagBits.IsMemberType | TagBits.IsNestedType | TagBits.ContainsNestedTypeReferences
        ...
        for (ReferenceBinding pt : this.type.permittedTypes()) {
            ...
            Substitution substitution = new Substitution() {
                ...
                public TypeBinding substitute(TypeVariableBinding typeVariable) {
                    ...
                    if (retVal == null) {
                        retVal = ParameterizedTypeBinding.this.environment.createWildcard(
                            (ReferenceBinding) typeVariable.declaringElement, typeVariable.rank, null, null,
                            Wildcard.UNBOUND);
    ...
    public TypeBinding withoutToplevelNullAnnotation() {
        ...
        return this.environment.createParameterizedType(unannotatedGenericType, this.arguments, this.enclosingType,
            newAnnotations);
    }
    ...
    public ReferenceBinding[] memberTypes() {
        if (this.memberTypes == null) {
            try {
                ...
                // if the original fields cannot be retrieved (ex. AbortCompilation), then assume we do not have any
                // fields
                if (this.memberTypes == null)
    ...
    public char[] readableName(boolean showGenerics) {
        ...
        if (isMemberType()) {
            nameBuffer.append(
                CharOperation.concat(enclosingType().readableName(showGenerics && !isStatic()), this.sourceName, '.'));
        } else {
            nameBuffer.append(CharOperation.concatWith(this.type.compoundName, '.'));
        }
        if (showGenerics) {
            if (this.arguments != null && this.arguments.length > 0) { // empty arguments array happens when PTB has
                                                                       // been created just to capture type annotations
                nameBuffer.append('<');
                for (int i = 0, length = this.arguments.length; i < length; i++) {
                    if (i > 0)
                        nameBuffer.append(',');
    ...
    ReferenceBinding resolve() {
        ...
        ReferenceBinding resolvedType = (ReferenceBinding) BinaryTypeBinding.resolveType(this.type, this.environment,
            false /* no raw conversion */); // still part of parameterized type ref
        ...
        if (this.arguments != null) {
            ...
            for (int i = 0; i < argLength; i++) {
                TypeBinding resolveType
                    = BinaryTypeBinding.resolveType(this.arguments[i], this.environment, true /* raw conversion */);
...
//			}
            // check argument type compatibility... REMOVED for now since incremental build will propagate change &
            // detect in source
//			for (int i = 0; i < argLength; i++) {
...
//			}
    ...
    public char[] shortReadableName(boolean showGenerics) {
        StringBuilder nameBuffer = new StringBuilder(10);
        if (isMemberType()) {
            nameBuffer.append(CharOperation.concat(enclosingType().shortReadableName(showGenerics && !isStatic()),
                this.sourceName, '.'));
        } else {
            nameBuffer.append(this.type.sourceName);
        }
    ...
    char[] nullAnnotatedReadableName(CompilerOptions options) {
        ...
        if (this.arguments != null && this.arguments.length > 0 && !isRawType()) { // empty arguments array happens when
                                                                                   // PTB has been created just to
    ...
    /**
     * @see org.eclipse.jdt.internal.compiler.lookup.Substitution#substitute(TypeVariableBinding)
     */
    @Override
    public TypeBinding substitute(TypeVariableBinding originalVariable) {
        ...
        while (true) {
            ...
            if (originalVariable.rank < length
                && TypeBinding.equalsEquals(typeVariables[originalVariable.rank], originalVariable)) {
    ...
    public ReferenceBinding[] superInterfaces() {
        if (this.superInterfaces == null) {
            if (this.type.isHierarchyBeingConnected())
                return Binding.NO_SUPERINTERFACES; // prevent superinterfaces from being assigned before they are
                                                   // connected
            ...
            if (this.superInterfaces != null) {
                for (int i = this.superInterfaces.length; --i >= 0;) {
                    ...
                    if ((this.typeBits & (TypeIds.BitAutoCloseable | TypeIds.BitCloseable)) != 0) // avoid the
                                                                                                  // side-effects of
                                                                                                  // hasTypeBit()!
                        this.typeBits |= applyCloseableWhitelists(this.environment.globalOptions);
                }
    ...
    public void swapUnresolved(UnresolvedReferenceBinding unresolvedType, ReferenceBinding resolvedType,
        ...
        if (this.type == unresolvedType) { //$IDENTITY-COMPARISON$
            ...
            if (enclosing != null) {
                this.enclosingType = resolvedType.isStatic()
                    ? enclosing
                    : (ReferenceBinding) env.convertUnresolvedBinaryToRawType(enclosing); // needed when binding
                                                                                          // unresolved member type
            }
        }
        if (this.arguments != null) {
            ...
            for (int i = 0; i < types.length; i++) {
                if (TypeBinding.notEquals(types[i], this.arguments[i])) {
                    ...
                    ParameterizedTypeBinding declaringType
                        = scope.environment().createParameterizedType(this.type, types, this.type.enclosingType());
    ...
    public MethodBinding getSingleAbstractMethod(final Scope scope, boolean replaceWildcards, int start, int end) {
        ...
        if (replaceWildcards) {
            ...
            if (types == null)
                return this.singleAbstractMethod[index] = new ProblemMethodBinding(TypeConstants.ANONYMOUS_METHOD, null,
                    ProblemReasons.NotAWellFormedParameterizedType);
        } else if (types == null) {
            types = NO_TYPES;
        }
        if (end >= 0) {
            // caller is going to require the sam's parameters to be treated as argument expressions, post substitution
            // capture will lose identity, where substitution results in fan out
        ...
        ReferenceBinding substitutedDeclaringType
            = (ReferenceBinding) declaringType.findSuperTypeOriginatingFrom(theAbstractMethod.declaringClass);
                ...
                switch (wildcard.boundKind) {
                    case Wildcard.EXTENDS:
                        ...
                        if (typeParameters[i].firstBound != null)
                            len++;
                        TypeBinding[] allBounds = new TypeBinding[len]; // TypeBinding so that in this round we accept
                                                                        // ArrayBinding, too.
                        int idx = 0;
                        // Ui
                        ...
                        if (glb == null || glb.length == 0) {
                            ...
                            try {
                                ...
                                System.arraycopy(glb, 0, refs, 0, glb.length); // TODO: if an array type plus more types
                                                                               // get here, we get ArrayStoreException!
                                types[i] = this.environment.createIntersectionType18(refs);
                            } catch (ArrayStoreException ase) {
                                scope.problemReporter()
                                    .genericInferenceError("Cannot compute glb of " + Arrays.toString(glb), null); //$NON-NLS-1$
                                return null;
                            }
                        }
                        break;
