...
public interface IPreferencesService {
	...
	/**
	 * Return the value stored in the preference store for the given key. If the key
	 ...
	 * {@link Preferences} nodes in the preference hierarchy to use and then call
	 * the {@link #get(String, String, Preferences[])} method. The order of the
	 * nodes is calculated by consulting the default scope lookup order as set by
	 * {@link #setDefaultLookupOrder(String, String, String[])}.
	 * </p>
	 * <p>
	 * The specified key may either refer to a simple key or be the concatenation of
	 * the path of a child node and key. If the key contains a slash ("/")
	 * character, then a double-slash must be used to denote the end of they child
	 * path and the beginning of the key. Otherwise it is assumed that the key is
	 * the last segment of the path. The following are some examples of keys and
	 * their meanings:
	 * </p>
	 * <ul>
	 ...
	 * lookup order, not by the order of the scope contexts that are being passed
	 * in. The context objects are only consulted to help determine which nodes to
	 * look in, not the order of the nodes.
	 * </p>
	 * <p>
	 * Callers may specify an array of scope context objects to aid in the
	 * determination of the correct nodes. For each entry in the lookup order, the
	 * array of contexts is consulted and if one matching the scope exists, then it
	 * is used to calculate the node. Otherwise a default calculation algorithm is
	 * used.
	 * </p>
	 * <p>
	 * An example of a qualifier for an Eclipse 2.1 preference is the plug-in
	 * identifier. (e.g. "com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.resources" for "description.autobuild")
	 * </p>
	 *
	 ...
	 * @param contexts     optional context objects to help scopes determine which
	 *                     nodes to search, or <code>null</code>
	...
	/**
	 * Exports all preferences for the given preference node and all its children to
	 * the specified output stream. It is the responsibility of the client to close
	 * the given output stream.
	 * <p>
	 * If the given export list is <code>null</code> then all preferences for all
	 * sub-nodes of the given node are exported to the given stream. Otherwise the
	 * export list is consulted before exporting each preference value. If there is
	 * a string match then the preference is not exported. The exclusion can also
	 * occur at a per-node level. Wild cards are <em>not</em> accepted in the
	 ...
	 * </pre>
	 * <p>
	 * The values stored in the resulting stream are suitable for later being read
	 ...
	 * @throws CoreException            if there was a problem exporting the
	 *                                  preferences
	...
	/**
	 * Loads preferences from the given file and stores them in the preferences
	 * store. Existing values are over-ridden by those from the stream. The stream
	 * must not be <code>null</code> and is closed upon return from this method.
	 * <p>
	...
	/**
	 * Return an array with the default lookup order for the preference keyed by the
	 * given qualifier and simple name. Return <code>null</code> if no default has
	 * been set.
	 * <p>
	 * The lookup order returned is based on an exact match to the specified
	 * qualifier and simple name. For instance, if the given key is
	 * non-<code>null</code> and no default lookup order is found, the default
	 * lookup order for the qualifier (and a <code>null</code> key) is <em>NOT</em>
	 ...
	 * <p>
	 * First do an exact match lookup with the given qualifier and simple name. If a
	 * match is found then return it. Otherwise if the key is non-<code>null</code>
	 * then do a lookup based on only the qualifier and return the set value. Return
	 ...
	 * qualifier and simple name. If the given order is <code>null</code> then the
	 * set ordering (if it exists) is removed.
	 * <p>
	 * If the given simple name is <code>null</code> then set the given lookup order
	 * to be used for all keys with the given qualifier.
	 * </p>
	 * <p>
	 ...
	 *                                  <ul>
	 *                                  <li>if the qualifier is
	 *                                  <code>null</code></li>
	 *                                  <li>if an entry in the order array is
	 *                                  <code>null</code> (the array itself is
	 ...
	 * stream. Apply the given list of preference filters, only exporting preference
	 * node and keys which are applicable to at least one filter in the list.
	 * <p>
	 * The given node and output stream must not be <code>null</code>. If the list
	 * of filters is <code>null</code> or empty then do nothing.
	 * </p>
	 * <p>
	...
	/**
	 * Apply the preference tree rooted at the given node, to the system's
	 * preference tree. The list of preference filters will act as a filter and only
	 * preferences in the tree which apply to at least one filter in the list, will
	 * be applied.
	 * <p>
