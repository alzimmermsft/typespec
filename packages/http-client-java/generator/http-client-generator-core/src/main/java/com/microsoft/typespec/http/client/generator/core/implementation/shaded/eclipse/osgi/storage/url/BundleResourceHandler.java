...
package com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.osgi.storage.url;

import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.equinox.plurl.PlurlStreamHandlerBase;
...
public abstract class BundleResourceHandler extends PlurlStreamHandlerBase {
    ...
    public String toExternalForm(URL url) {
        ...
        String path = url.getPath();
        if (path != null) {
            if (!path.isEmpty() && path.charAt(0) != '/') { // if name doesn't have a leading slash
                result.append("/"); //$NON-NLS-1$
            }
            result.append(path);
        }
    ...
    static long parseBundleIDFromURLHost(String host) {
        ...
        return (dotIndex >= 0 && dotIndex < host.length() - 1)
            ? Long.parseLong(host.substring(0, dotIndex))
            : Long.parseLong(host);
    }
}
