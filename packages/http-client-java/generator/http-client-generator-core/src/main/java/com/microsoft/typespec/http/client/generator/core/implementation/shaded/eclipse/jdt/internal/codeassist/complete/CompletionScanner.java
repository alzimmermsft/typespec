...
public class CompletionScanner extends Scanner {
    ...
    public CompletionScanner(long sourceLevel) {
        this(sourceLevel, false /* previewEnabled */);
    }

    public CompletionScanner(long sourceLevel, boolean previewEnabled) {
        super(false /* comment */, false /* whitespace */, false /* nls */, sourceLevel, null /* taskTags */,
            null/* taskPriorities */, true/* taskCaseSensitive */, previewEnabled);
    }

    @Override
    protected boolean isAtAssistIdentifier() {
        if (this.cursorLocation < this.startPosition && this.currentPosition == this.startPosition) { // fake empty
                                                                                                      // identifier got
                                                                                                      // issued
            return true;
        }
        ...
        if (this.completionIdentifier == null) {
            ...
            if (this.cursorLocation + 1 >= this.startPosition && this.cursorLocation < this.currentPosition) {
                ...
                if (this.withoutUnicodePtr != 0) {			// check unicode scenario
                    ...
                    System.arraycopy(this.withoutUnicodeBuffer, 1, this.completionIdentifier = new char[length], 0,
                        length);
                } else {
                    // no char[] sharing around completionIdentifier, we want it to be unique so as to use identity
                    // checks
                    int length = this.cursorLocation + 1 - this.startPosition;
                    System.arraycopy(this.source, this.startPosition, (this.completionIdentifier = new char[length]), 0,
                        length);
                }
        ...
        if (this.completionIdentifier == null) {
            if (this.cursorLocation + 1 >= this.startPosition && this.cursorLocation < this.currentPosition) {
                ...
                if (this.withoutUnicodePtr != 0) {			// check unicode scenario
                    ...
                    System.arraycopy(this.withoutUnicodeBuffer, 2, this.completionIdentifier = new char[length], 0,
                        length);
                } else {
                    // no char[] sharing around completionIdentifier, we want it to be unique so as to use identity
                    // checks
                    ...
                    System.arraycopy(this.source, this.startPosition + 1,
                        (this.completionIdentifier = new char[length]), 0, length);
                }
                return this.completionIdentifier;
            }
        }
            ...
            while (true) { // loop for jumping over comments
                    ...
                    try {
                        ...
                        if (this.currentPosition > this.eofPosition) {
                            ...
                            if (this.completionIdentifier == null && this.startPosition == this.cursorLocation + 1) {
                                this.currentPosition = this.startPosition; // for being detected as empty free
                                                                           // identifier
                ...
                switch (this.currentCharacter) {
                    ...
                    case '\'': {
                        ...
                        if (test > 0) {
                            // relocate if finding another quote fairly close: thus unicode '/u000D' will be fully
                            // consumed
                        ...
                        if (test == 0 || test == 2) { // line comment
                            ...
                            try { // get the next char
                                if (((this.currentCharacter = this.source[this.currentPosition++]) == '\\')
                                    ...
                                    if ((c1 = ScannerHelper.getHexadecimalValue(this.source[this.currentPosition++]))
                                        > 15
                                        || c1 < 0
                                        || (c2 = ScannerHelper.getHexadecimalValue(this.source[this.currentPosition++]))
                                            > 15
                                        || c2 < 0
                                        || (c3 = ScannerHelper.getHexadecimalValue(this.source[this.currentPosition++]))
                                            > 15
                                        || c3 < 0
                                        || (c4 = ScannerHelper.getHexadecimalValue(this.source[this.currentPosition++]))
                                            > 15
                                        || c4 < 0) {
                                ...
                                while (true) {
                                    ...
                                    if (((this.currentCharacter = this.source[this.currentPosition++]) == '\\')
                                        ...
                                        if ((c1
                                            = ScannerHelper.getHexadecimalValue(this.source[this.currentPosition++]))
                                            > 15
                                            || c1 < 0
                                            || (c2 = ScannerHelper
                                                .getHexadecimalValue(this.source[this.currentPosition++])) > 15
                                            || c2 < 0
                                            || (c3 = ScannerHelper
                                                .getHexadecimalValue(this.source[this.currentPosition++])) > 15
                                            || c3 < 0
                                ...
                                if (test == 0
                                    && this.startPosition <= this.cursorLocation
                                    && this.cursorLocation < this.currentPosition - 1) {
                    ...
                    case '\u001a':
                        if (atEnd())
                            return TokenNameEOF;
                        // the atEnd may not be <this.currentPosition == this.source.length> if source is only some part
                        // of a real (external) stream
                        throw invalidEof();
