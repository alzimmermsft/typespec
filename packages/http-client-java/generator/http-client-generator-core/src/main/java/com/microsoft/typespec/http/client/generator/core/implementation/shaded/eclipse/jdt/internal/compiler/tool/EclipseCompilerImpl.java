...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.batch.ClasspathJrt;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.batch.ClasspathJsr199;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.batch.CompilationUnit;
...
public class EclipseCompilerImpl extends Main {
                ...
                if (isModuleInfo == (round == 0)) { // 1st round: modules, 2nd round others (to ensure populating
                    ...
                    try {
                        CompilationUnit cu = new CompilationUnit(
                            javaFileObject.getCharContent(false).toString().toCharArray(), name, encoding,
    ...
    public IProblemFactory getProblemFactory() {
        return new DefaultProblemFactory() {
            ...
            public CategorizedProblem createProblem(final char[] originatingFileName, final int problemId,
                ...
                final int startPosition, final int endPosition, final int lineNumber, final int columnNumber) {
                ...
                CategorizedProblem problem = super.createProblem(originatingFileName, problemId, problemArguments,
            ...
            @Override
            public CategorizedProblem createProblem(final char[] originatingFileName, final int problemId,
                final String[] problemArguments, final int elaborationID, final String[] messageArguments,
                final int severity, final int startPosition, final int endPosition, final int lineNumber,
                ...
                CategorizedProblem problem = super.createProblem(originatingFileName, problemId, problemArguments,
                    elaborationID, messageArguments, severity, startPosition, endPosition, lineNumber, columnNumber);
                if (problem instanceof DefaultProblem && diagnostic != null) {
                    return new Jsr199ProblemWrapper((DefaultProblem) problem, diagnostic, diagListener);
                }
                return problem;
        ...
        if (!((unitResult == null) || (unitResult.hasErrors() && !this.proceedOnError))) {
            ...
            for (ClassFile classFile : classFiles) {
                ...
                if (this.compilerOptions.verbose) {
                    EclipseCompilerImpl.this.out.println(Messages.bind(Messages.compilation_write,
                        new String[] { String.valueOf(this.exportedClassFilesCounter + 1), relativeStringName }));
                }
                try {
                    ...
                    if (modName == null) {
                        ...
                        location
                            = this.fileManager.getLocationForModule(StandardLocation.CLASS_OUTPUT, new String(modName));
                    }
                    JavaFileObject javaFileForOutput
                        = this.fileManager.getJavaFileForOutput(location, new String(filename),
                            JavaFileObject.Kind.CLASS, this.javaFileObjectMap.get(unitResult.compilationUnit));
    ...
    protected void handleLocations() {
        ArrayList<Classpath> fileSystemClasspaths = new ArrayList<>();
        ...
        if (standardJavaFileManager != null) {
                            ...
                            if (platformLocations.size() == 1) {
                                ...
                                if (jrt instanceof ClasspathJrt) {
                                    ...
                                    try {
                                        ...
                                        @SuppressWarnings("resource") // XXX EclipseFileManager should close jrtfs but
                                                                      // it looks like standardJavaFileManager is never
                                                                      // closed
                                        // Was leaking new JrtFileSystem(classpathJrt.file):
                                        JrtFileSystem jrtfs = efm.getJrtFileSystem(classpathJrt.file); // XXX use
                                                                                                       // classpathJrt.jrtFileSystem??
            ...
            if (jdkLevel >= ClassFileConstants.JDK9) {
                ...
                Classpath classpath
                    = new ClasspathJsr199(systemClasspath, this.fileManager, StandardLocation.SYSTEM_MODULES);
                fileSystemClasspaths.add(classpath);
                classpath
                    = new ClasspathJsr199(systemClasspath, this.fileManager, StandardLocation.PLATFORM_CLASS_PATH);
                fileSystemClasspaths.add(classpath);
            } else {
        ...
        if (eclipseJavaFileManager != null) {
            if ((eclipseJavaFileManager.flags & EclipseFileManager.HAS_EXT_DIRS) == 0) {
                ...
                if (CompilerOptions.versionToJdkLevel(this.options.get(CompilerOptions.OPTION_TargetPlatform))
        ...
        if (standardJavaFileManager != null) {
            ...
            if (locationFiles != null) {
                for (File file : locationFiles) {
                    if (file.isDirectory()) {
                        ...
                        long jdkLevel = javaVersion.equals("") //$NON-NLS-1$
                            ? this.complianceLevel
                            : CompilerOptions.versionToJdkLevel(javaVersion);
                        Classpath systemClasspath = getSystemClasspath(file, jdkLevel);
                        Classpath classpath = new ClasspathJsr199(systemClasspath, this.fileManager,
                            StandardLocation.PLATFORM_CLASS_PATH);
                        fileSystemClasspaths.add(classpath);
                        // Copy over to modules location as well
                        if (standardJavaFileManager.getLocation(StandardLocation.SYSTEM_MODULES) == null) {
                            classpath = new ClasspathJsr199(systemClasspath, this.fileManager,
            ...
            try {
                Iterable<? extends Path> locationAsPaths
                    = standardJavaFileManager.getLocationAsPaths(StandardLocation.MODULE_SOURCE_PATH);
            ...
            if (this.fileManager.hasLocation(StandardLocation.SOURCE_PATH)) {
                classpath = new ClasspathJsr199(this.fileManager, StandardLocation.SOURCE_PATH,
                    new HashSet<>(this.compilationUnits), this::getNewParser);
                fileSystemClasspaths.add(classpath);
            }
            ...
            if (this.fileManager.hasLocation(StandardLocation.MODULE_SOURCE_PATH)) {
                try {
                    for (Set<JavaFileManager.Location> locs : this.fileManager
                        .listLocationsForModules(StandardLocation.MODULE_SOURCE_PATH)) {
                        for (JavaFileManager.Location loc : locs) {
                            classpath = new ClasspathJsr199(this.fileManager, loc, new HashSet<>(this.compilationUnits),
                                this::getNewParser);
                            fileSystemClasspaths.add(classpath);
                        }
                    }
            ...
            if (this.fileManager.hasLocation(StandardLocation.MODULE_PATH)) {
                try {
                    for (Set<JavaFileManager.Location> locs : this.fileManager
                        .listLocationsForModules(StandardLocation.MODULE_PATH)) {
                        for (JavaFileManager.Location loc : locs) {
        ...
        if (size != 0) {
            this.checkedClasspaths = new Classpath[size];
            int i = 0;
            for (Classpath classpath : fileSystemClasspaths) {
    ...
    private String getJavaVersion(File javaHome) {
        ...
        try (Stream<String> lines
            = Files.lines(Paths.get(javaHome.getAbsolutePath(), RELEASE_FILE), Charset.defaultCharset())) {
    ...
    class Jsr199ProblemWrapper extends DefaultProblem {
        ...
        public Jsr199ProblemWrapper(DefaultProblem original, Diagnostic<JavaFileObject> diagnostic,
            DiagnosticListener<? super JavaFileObject> listener) {
            super(original.getOriginatingFileName(), original.getMessage(), original.getID(), original.getArguments(),
                original.severity, original.getSourceStart(), original.getSourceEnd(), original.getSourceLineNumber(),
                original.column);
