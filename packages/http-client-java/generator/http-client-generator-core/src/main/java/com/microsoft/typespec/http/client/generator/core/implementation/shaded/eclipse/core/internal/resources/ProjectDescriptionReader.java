...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.javax.xml.parsers.SAXParser;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.javax.xml.parsers.SAXParserFactory;
...
public class ProjectDescriptionReader extends DefaultHandler implements IModelObjectConstants {
    ...
    private void endFilteredResourcesElement(String elementName) {
        if (elementName.equals(FILTERED_RESOURCES)) {
            HashMap<IPath, LinkedList<FilterDescription>> filteredResources
                = (HashMap<IPath, LinkedList<FilterDescription>>) objectStack.pop();
            state = S_PROJECT_DESC;
    ...
    private void endVariableListElement(String elementName) {
        if (elementName.equals(VARIABLE_LIST)) {
            HashMap<String, VariableDescription> variableList
                = (HashMap<String, VariableDescription>) objectStack.pop();
            state = S_PROJECT_DESC;
    ...
    private void endMatcherElement(String elementName) {
        if (elementName.equals(MATCHER)) {
            ...
            if (objectStack.peek() instanceof ArrayList) {
                ...
                ArrayList<FileInfoMatcherDescription> list
                    = ((ArrayList<FileInfoMatcherDescription>) objectStack.peek());
    ...
    private void endFilterElement(String elementName) {
        if (elementName.equals(FILTER)) {
            ...
            if (project != null) {
                ...
                HashMap<IPath, LinkedList<FilterDescription>> map
                    = ((HashMap<IPath, LinkedList<FilterDescription>>) objectStack.peek());
                ...
                HashMap<String, LinkedList<FilterDescription>> map
                    = ((HashMap<String, LinkedList<FilterDescription>>) objectStack.peek());
        ...
        if (elementName.equals(LOCATION_URI)) {
            ...
            if (oldLocation != null) {
                ...
                try {
                    ...
                    problems.add(new Status(IStatus.WARNING, ResourcesPlugin.PI_RESOURCES,
                        IResourceStatus.FAILED_READ_METADATA, msg, e));
                }
            }
            state = S_LINK;
        }
    ...
    private void endFilterPath(String elementName) {
        if (elementName.equals(NAME)) {
            ...
            if (oldResource != null) {
                ...
                if (project != null) {
                    ((FilterDescription) objectStack.peek())
                        .setResource(newPath.isEmpty() ? (IResource) project : project.getFolder(newPath));
                } else {
                    // if the project is null, that means that we're loading a project description to retrieve
                    // some meta data only.
    ...
    private void endFilterType(String elementName) {
        if (elementName.equals(TYPE)) {
            ...
            if (oldType != -1) {
                parseProblem(
                    NLS.bind(Messages.projRead_badFilterType2, Integer.toString(oldType), Integer.toString(newType)));
            } else {
    ...
    private void endLinkType(String elementName) {
        if (elementName.equals(TYPE)) {
            ...
            if (oldType != -1) {
                parseProblem(
                    NLS.bind(Messages.projRead_badLinkType2, Integer.toString(oldType), Integer.toString(newType)));
            } else {
    ...
    public void fatalError(SAXParseException error) throws SAXException {
        ...
        problems.add(new Status(IStatus.ERROR, ResourcesPlugin.PI_RESOURCES, IResourceStatus.FAILED_READ_METADATA,
            message == null ? "" : message, error)); //$NON-NLS-1$
        throw error;
    }
    ...
    protected void log(Exception ex) {
        ...
        problems.add(new Status(IStatus.WARNING, ResourcesPlugin.PI_RESOURCES, IResourceStatus.FAILED_READ_METADATA,
            message == null ? "" : message, ex)); //$NON-NLS-1$
    }
    ...
    public ProjectDescription read(InputSource input) {
        problems = new MultiStatus(ResourcesPlugin.PI_RESOURCES, IResourceStatus.FAILED_READ_METADATA,
            Messages.projRead_failureReadingProjectDesc, null);
    ...
    public ProjectDescription read(IPath location) throws IOException {
        try (BufferedInputStream file = new BufferedInputStream(new FileInputStream(location.toFile()));) {
    ...
    public ProjectDescription read(IPath location, IPath tempLocation) throws IOException {
        try (SafeFileInputStream file = new SafeFileInputStream(location.toOSString(), tempLocation.toOSString());) {
            return read(new InputSource(file));
        }
    }
