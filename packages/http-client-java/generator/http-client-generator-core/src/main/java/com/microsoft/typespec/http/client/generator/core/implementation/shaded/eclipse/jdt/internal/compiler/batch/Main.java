...
public class Main implements ProblemSeverities, SuffixConstants {
	...
	public static class Logger {
		...
		private void logProblem(CategorizedProblem problem, int localErrorCount,
			...
			if ((this.tagBits & Logger.EMACS) != 0) {
				...
				if (!errorReportSource.isEmpty()) this.printlnErr(errorReportSource);
			} else {
				if (localErrorCount == 0) {
					this.printlnErr("----------"); //$NON-NLS-1$
				}
			...
			try {
				...
				if (index != -1) {
					if (logFileName.substring(index).equalsIgnoreCase(".xml")) { //$NON-NLS-1$
						this.log = new GenericXMLWriter(new OutputStreamWriter(new FileOutputStream(logFileName, false),
                            StandardCharsets.UTF_8), Util.LINE_SEPARATOR, true);
	...
	public final static String bundleName = "com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.batch.messages"; //$NON-NLS-1$
...
protected void addNewEntry(ArrayList<Classpath> paths, String currentClasspathName,
	...
	Classpath currentClasspath = FileSystem.getClasspath(
			currentClasspathName,
			customEncoding,
			isSourceOnly,
			accessRuleSet,
		...
		switch(mode) {
			case DEFAULT :
                ...
                switch (currentArg) {
                    ...
                    case "--module-source-path" -> {
                        if (sourcepathClasspathArg != null) {
                            throw new IllegalArgumentException(
                                this.bind("configure.OneOfModuleOrSourcePath")); //$NON-NLS-1$
                        }
                ...
                if (currentArg.startsWith("-g")) { //$NON-NLS-1$
					...
					if (length > 3) {
						...
						if (length == 7 && currentArg.equals("-g:" + NONE)) //$NON-NLS-1$
							continue;
						StringTokenizer tokenizer =
							new StringTokenizer(currentArg.substring(3), ","); //$NON-NLS-1$
						while (tokenizer.hasMoreTokens()) {
							String token = tokenizer.nextToken();
                            switch (token) {
                                case "vars" ->  //$NON-NLS-1$
				...
				if (currentArg.startsWith("-info")) { //$NON-NLS-1$
					...
					StringTokenizer tokenizer =
						new StringTokenizer(currentArg.substring(infoTokenStart), ","); //$NON-NLS-1$
					int tokenCounter = 0;
				...
				if (currentArg.startsWith("-warn")) { //$NON-NLS-1$
					...
					StringTokenizer tokenizer =
						new StringTokenizer(currentArg.substring(warnTokenStart), ","); //$NON-NLS-1$
					int tokenCounter = 0;
				...
				if (currentArg.startsWith("-err")) { //$NON-NLS-1$
					...
					StringTokenizer tokenizer =
						new StringTokenizer(currentArg.substring(errorTokenStart), ","); //$NON-NLS-1$
					int tokenCounter = 0;
...
private void initializeWarnings(String propertiesFile) {
	...
	for (Entry entry : properties.entrySet()) {
		...
		if (key.startsWith("com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.compiler.")) { //$NON-NLS-1$
...
protected ArrayList<Classpath> handleClasspath(ArrayList<String> classpaths, String customEncoding) {
	...
	if (classpaths != null && !classpaths.isEmpty()) {
		...
		if ((classProp == null) || (classProp.isEmpty())) {
			...
			while (tokenizer.hasMoreTokens()) {
				token = tokenizer.nextToken();
				Classpath currentClasspath = FileSystem
	...
	switch(token.charAt(0)) {
                    ...
                    if (isEnabling) {
                        switch (severity) {
                            case ProblemSeverities.Error:
                                ...
                                break;
                            case ProblemSeverities.Warning:
                                if (CompilerOptions.IGNORE.equals(
                                    this.options.get(CompilerOptions.OPTION_ReportIncompleteEnumSwitch))) {
                                    setSeverity(CompilerOptions.OPTION_ReportIncompleteEnumSwitch, severity, true);
                                }
                                break;
		...
		case 'n' :
			if (token.equals("nls")) { //$NON-NLS-1$
				...
				setSeverity(CompilerOptions.OPTION_ReportNullReference, severity, isEnabling);
				if (!isEnabling) {
					setSeverity(CompilerOptions.OPTION_ReportPotentialNullReference, ProblemSeverities.Ignore, false);
				...
				if (isEnabling && start >= 0 && end >= 0 && start < end){
					...
					if (isPrimarySet && nullableAnnotName.isEmpty()) throw new IllegalArgumentException(this.bind("configure.invalidNullAnnot", token)); //$NON-NLS-1$
					...
					if (isPrimarySet && nonNullAnnotName.isEmpty()) throw new IllegalArgumentException(this.bind("configure.invalidNullAnnot", token)); //$NON-NLS-1$
					nonNullByDefaultAnnotName = annotationNames.substring(separator2 + 1).trim();
					if (isPrimarySet && nonNullByDefaultAnnotName.isEmpty()) throw new IllegalArgumentException(this.bind("configure.invalidNullAnnot", token)); //$NON-NLS-1$
					if (isPrimarySet) {
...
protected void initializeAnnotationProcessorManager() {
	String className = "com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.apt.dispatch.BatchAnnotationProcessorManager"; //$NON-NLS-1$
	try {
 ...
 * External API
 */
public void processPathEntries(final int defaultSize, final ArrayList<Classpath> paths,
