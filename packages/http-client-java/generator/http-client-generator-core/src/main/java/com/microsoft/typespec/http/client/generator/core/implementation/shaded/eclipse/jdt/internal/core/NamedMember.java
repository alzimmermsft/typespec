...
public abstract class NamedMember extends Member {
        ...
        if (forceOpen) {
            ...
            if (length > 0) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    for (int j = 0; j < boundsLength; j++) {
                        boundSignatures[j]
                            = Signature.createCharArrayTypeSignature(bounds[j].toCharArray(), method.isBinary());
                        CharOperation.replace(boundSignatures[j], '.', '/');
                    }
                    char[] sig = Signature.createTypeParameterSignature(typeParameter.getElementName().toCharArray(),
                        boundSignatures);
                    key.append(sig);
                }
                key.append('>');
    ...
    public String getTypeQualifiedName(char enclosingTypeSeparator, boolean showParameters) throws JavaModelException {
        ...
        switch (this.getParent().getElementType()) {
            ...
            case IJavaElement.CLASS_FILE:
                ...
                if (classFileName.indexOf('$') == -1) {
                    ...
                    typeName = classFileName.substring(0, classFileName.lastIndexOf('.'))
                        /* remove .class */.replace('$', enclosingTypeSeparator);
                }
                if (showParameters) {
        ...
        StringBuilder buffer
            = new StringBuilder(declaringType.getTypeQualifiedName(enclosingTypeSeparator, showParameters));
