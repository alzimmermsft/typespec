    ...
    public FlowInfo analyseCode(MethodScope initializationScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if (this.binding != null
            ...
            if (initializationScope.compilerOptions().sourceLevel < ClassFileConstants.JDK16) {
                initializationScope.problemReporter()
                    .unexpectedStaticModifierForField((SourceTypeBinding) this.binding.declaringClass, this);
            }
        }
        ...
        if (this.initialization != null && this.binding != null) {
            if (options.isAnnotationBasedNullAnalysisEnabled) {
                ...
                NullAnnotationMatching.checkAssignment(initializationScope, flowContext, this.binding, flowInfo,
...
//	}
    ...
    public void resolve(MethodScope initializationScope) {
        if (this.isUnnamed(initializationScope)) {
            initializationScope.problemReporter()
                .illegalUseOfUnderscoreAsAnIdentifier(this.sourceStart, this.sourceEnd,
                    initializationScope.compilerOptions().sourceLevel > ClassFileConstants.JDK1_8, true);
        }
        ...
        if (classScope != null) {
            ...
            checkHiding: {
                checkHidingSuperField: {
                    ...
                    FieldBinding existingVariable = classScope.findField(declaringType.superclass, this.name, this,
                ...
                // only corner case is: lookup of outer field through static declaringType, which isn't detected by
                // #getBinding as lookup starts
        ...
        try {
            ...
            if (this.annotations != null) {
                for (Annotation annotation : this.annotations) {
                    ...
                    if (resolvedAnnotationType != null
                        && (resolvedAnnotationType.getAnnotationTagBits() & TagBits.AnnotationForTypeUse) != 0) {
            ...
            if (this.initialization == null) {
                ...
                if (this.initialization instanceof ArrayInitializer) {
                    ...
                    if ((initializationType = this.initialization.resolveTypeExpecting(initializationScope, fieldType))
                        != null) {
                    ...
                    if (TypeBinding.notEquals(fieldType, initializationType)) // must call before computeConversion()
                                                                              // and typeMismatchError()
                    ...
                    if (this.initialization.isConstantValueOfTypeAssignableToType(initializationType, fieldType)
                        ...
                        if (initializationType.needsUncheckedConversion(fieldType)) {
                            initializationScope.problemReporter()
                                .unsafeTypeConversion(this.initialization, initializationType, fieldType);
                        }
                        if (this.initialization instanceof CastExpression
                            && (this.initialization.bits & ASTNode.UnnecessaryCast) == 0) {
                            CastExpression.checkNeedForAssignedCast(initializationScope, fieldType,
                                (CastExpression) this.initialization);
                        }
                    } else if (isBoxingCompatible(initializationType, fieldType, this.initialization,
                        initializationScope)) {
                        this.initialization.computeConversion(initializationScope, fieldType, initializationType);
                        if (this.initialization instanceof CastExpression
                            && (this.initialization.bits & ASTNode.UnnecessaryCast) == 0) {
                        ...
                        if (((fieldType.tagBits | initializationType.tagBits) & TagBits.HasMissingType) == 0) {
                            ...
                            initializationScope.problemReporter()
                                .typeMismatchError(initializationType, fieldType, this.initialization, null);
                        }
                    }
                    if (this.binding.isFinal()) { // cast from constant actual type to variable type
                        this.binding.setConstant(this.initialization.constant
                            .castTo((this.binding.type.id << 4) + this.initialization.constant.typeID()));
                    }
                } else {
                    this.binding.setConstant(Constant.NotAConstant);
                }
