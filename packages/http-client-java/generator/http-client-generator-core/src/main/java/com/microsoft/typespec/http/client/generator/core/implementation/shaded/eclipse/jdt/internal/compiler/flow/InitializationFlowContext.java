...
public class InitializationFlowContext extends ExceptionHandlingFlowContext {
    ...
    public InitializationFlowContext(FlowContext parent, ASTNode associatedNode, FlowInfo initsBeforeContext,
        FlowContext initializationParent, BlockScope scope) {
        super(parent, associatedNode, Binding.NO_EXCEPTIONS, // no exception allowed by default
            initializationParent, scope, FlowInfo.DEAD_END);
    ...
    public void checkInitializerExceptions(BlockScope currentScope, FlowContext initializerContext, FlowInfo flowInfo) {
        for (int i = 0; i < this.exceptionCount; i++) {
            initializerContext.checkExceptionHandlers(this.thrownExceptions[i], this.exceptionThrowers[i],
                this.exceptionThrowerFlowInfos[i], currentScope);
        }
    }
    ...
    public void recordHandlingException(ReferenceBinding exceptionType, UnconditionalFlowInfo flowInfo,
        TypeBinding raisedException, TypeBinding caughtException, ASTNode invocationSite, boolean wasMasked) {
        ...
        if (this.exceptionCount == size) {
            ...
            System.arraycopy(this.exceptionThrowerFlowInfos, 0,
                (this.exceptionThrowerFlowInfos = new FlowInfo[size * 2]), 0, size);
        }
