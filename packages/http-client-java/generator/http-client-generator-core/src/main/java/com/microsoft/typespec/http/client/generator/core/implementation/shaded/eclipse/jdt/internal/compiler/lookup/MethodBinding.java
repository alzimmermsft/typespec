...
public class MethodBinding extends Binding {
    ...
    public MethodBinding(int modifiers, char[] selector, TypeBinding returnType, TypeBinding[] parameters,
        ...
        this.thrownExceptions
            = (thrownExceptions == null || thrownExceptions.length == 0) ? Binding.NO_EXCEPTIONS : thrownExceptions;
    ...
    public MethodBinding(int modifiers, TypeBinding[] parameters, ReferenceBinding[] thrownExceptions,
...
// special API used to change method declaring class for runtime visibility check
    public MethodBinding(MethodBinding initialMethodBinding, ReferenceBinding declaringClass) {
        ...
        this.declaringClass = declaringClass;
        declaringClass.storeAnnotationHolder(this,
            initialMethodBinding.declaringClass.retrieveAnnotationHolder(initialMethodBinding, true));
    }
    ...
    public final boolean areParameterErasuresEqual(MethodBinding method) {
        ...
        for (int i = 0; i < length; i++)
            if (TypeBinding.notEquals(this.parameters[i], args[i])
                && TypeBinding.notEquals(this.parameters[i].erasure(), args[i].erasure()))
                return false;
        return true;
    }
    ...
    public final boolean areParametersCompatibleWith(TypeBinding[] arguments) {
        ...
        if (isVarargs()) {
            ...
            if (paramLength == argLength) { // accept X[] but not X or X[][]
                ...
                if (TypeBinding.notEquals(varArgType, lastArgument) && !lastArgument.isCompatibleWith(varArgType))
                    return false;
            } else if (paramLength < argLength) { // all remainig argument types must be compatible with the
                                                  // elementsType of varArgType
        ...
        for (int i = 0; i < lastIndex; i++)
            if (TypeBinding.notEquals(this.parameters[i], arguments[i])
                && !arguments[i].isCompatibleWith(this.parameters[i]))
                return false;
        return true;
    }
    ...
    public final boolean areTypeVariableErasuresEqual(MethodBinding method) {
        ...
        for (int i = 0; i < length; i++)
            if (TypeBinding.notEquals(this.typeVariables[i], vars[i])
                && TypeBinding.notEquals(this.typeVariables[i].erasure(), vars[i].erasure()))
                return false;
        return true;
    }
    ...
    private TypeBinding makeRawArgument(LookupEnvironment env, TypeVariableBinding var) {
        if (var.boundsCount() <= 1) {
            ...
            return env.createWildcard(null, 0, rawFirstBound, rawOtherBounds,
                com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.Wildcard.EXTENDS);
        }
    }
    ...
    public final boolean canBeSeenBy(TypeBinding receiverType, InvocationSite invocationSite, Scope scope) {
        ...
        if (this.declaringClass.isInterface() && isStatic() && !isPrivate()) {
            if ((invocationSite.isTypeAccess() || invocationSite.receiverIsImplicitThis())
        ...
        if (TypeBinding.equalsEquals(invocationType, this.declaringClass)
            && TypeBinding.equalsEquals(invocationType, receiverType))
            return true;
        ...
        if (isProtected()) {
            ...
            do {
                if (currentType.findSuperTypeOriginatingFrom(declaringErasure) != null) {
                    ...
                    if (TypeBinding.equalsEquals(currentType, receiverErasure)
    ...
    public char[] computeUniqueKey(boolean isLeaf) {
        ...
        boolean addThrownExceptions
            = thrownExceptionsLength > 0 && (!isGeneric || CharOperation.lastIndexOf('^', sig) < 0);
        if (addThrownExceptions) {
            ...
            for (int i = 0; i < thrownExceptionsLength; i++) {
                if (this.thrownExceptions[i] != null) {
                    ...
                    thrownExceptionsSignatureLength += thrownExceptionsSignatures[i].length + 1;	// add one char for
                                                                                                	// separator
                }
            }
        }
        ...
        char[] uniqueKey
            = new char[declaringLength + 1 + selectorLength + signatureLength + thrownExceptionsSignatureLength];
        int index = 0;
    ...
    protected void fillInDefaultNonNullness(AbstractMethodDeclaration sourceMethod,
        boolean needToApplyReturnNonNullDefault, ParameterNonNullDefaultProvider needToApplyParameterNonNullDefault) {
        ...
        if (this.returnType != null
            ...
            sourceMethod.scope.problemReporter()
                .nullAnnotationIsRedundant(sourceMethod, -1/* signifies method return */);
        }
    }
...
//pre: null annotation analysis is enabled
    protected void fillInDefaultNonNullness18(AbstractMethodDeclaration sourceMethod, LookupEnvironment env) {
        ...
        if (original.returnType != null
            && hasNonNullDefaultForReturnType(sourceMethod)
            && original.returnType.acceptsNonNullDefault()) {
            if ((this.returnType.tagBits & TagBits.AnnotationNullMASK) == 0) {
                this.returnType
                    = env.createAnnotatedType(this.returnType, new AnnotationBinding[] { env.getNonNullAnnotation() });
            } else if (sourceMethod instanceof MethodDeclaration
                && (this.returnType.tagBits & TagBits.AnnotationNonNull) != 0
    ...
    public MethodBinding findOriginalInheritedMethod(MethodBinding inheritedMethod) {
        ...
        if (TypeBinding.notEquals(inheritedOriginal.declaringClass, superType)) {
            ...
            MethodBinding[] superMethods = ((ReferenceBinding) superType).getMethods(inheritedOriginal.selector,
                inheritedOriginal.parameters.length);
    ...
    public long getAnnotationTagBits() {
        MethodBinding originalMethod = original();
        if ((originalMethod.extendedTagBits & ExtendedTagBits.AnnotationResolved) == 0
            && originalMethod.declaringClass instanceof SourceTypeBinding) {
            ...
            if (scope != null) {
                ...
                if (options.isAnnotationBasedNullAnalysisEnabled) {
                    ...
                    if (nullDefaultBits != 0 && this.declaringClass instanceof SourceTypeBinding) {
                        Binding target = scope.checkRedundantDefaultNullness(this.defaultNullness,
                            typeDecl.declarationSourceStart);
                        if (target != null) {
                            methodDecl.scope.problemReporter()
                                .nullDefaultAnnotationIsRedundant(methodDecl, methodDecl.annotations, target);
                        }
                    }
                }
            }
            ...
            if (this.declaringClass instanceof SourceTypeBinding) {
                ...
                if (sourceType.scope != null) {
                    AbstractMethodDeclaration methodDecl
                        = sourceType.scope.referenceType().declarationOf(originalMethod);
                    for (int i = 0; i < length; i++) {
    ...
    public final boolean isViewedAsDeprecated() {
        return (this.modifiers & (ClassFileConstants.AccDeprecated | ExtraCompilerModifiers.AccDeprecatedImplicitly))
            != 0;
    }
    ...
    public void setAnnotations(AnnotationBinding[] annotations, AnnotationBinding[][] parameterAnnotations,
        Object defaultValue, LookupEnvironment optionalEnv) {
        this.declaringClass.storeAnnotationHolder(this,
            AnnotationHolder.storeAnnotations(annotations, parameterAnnotations, defaultValue, optionalEnv));
    }
    ...
    public final char[] computeSignature(ClassFile classFile) {
        ...
        boolean needSynthetics = isConstructor && this.declaringClass.isNestedType() && !this.declaringClass.isStatic();
        if (needSynthetics) {
        ...
        if (this.signature != null) {
            if ((this.tagBits & TagBits.ContainsNestedTypeReferences) != 0) {
                ...
                boolean needSynthetics
                    = isConstructor && this.declaringClass.isNestedType() && !this.declaringClass.isStatic();
                if (needSynthetics) {
                    // take into account the synthetic argument type signatures as well
                    ReferenceBinding[] syntheticArgumentTypes = this.declaringClass.syntheticEnclosingInstanceTypes();
                    if (syntheticArgumentTypes != null) {
    ...
    public AbstractMethodDeclaration sourceMethod() {
        ...
        AbstractMethodDeclaration[] methods
            = sourceType.scope != null ? sourceType.scope.referenceContext.methods : null;
        if (methods != null) {
        ...
        if (this.thrownExceptions != null) {
            if (this.thrownExceptions != Binding.NO_EXCEPTIONS) {
                ...
                for (int i = 0, length = this.thrownExceptions.length; i < length; i++) {
                    if (i > 0)
                        output.append(", "); //$NON-NLS-1$
                    output.append((this.thrownExceptions[i] != null)
                        ? this.thrownExceptions[i].debugName()
                        : "<no exception type>"); //$NON-NLS-1$
                }
            }
        } else {
...
//pre: null annotation analysis is enabled
    public boolean hasNonNullDefaultForReturnType(AbstractMethodDeclaration srcMethod) {
        return hasNonNullDefaultForType(this.returnType, Binding.DefaultLocationReturnType, srcMethod,
            srcMethod == null ? -1 : srcMethod.declarationSourceStart);
    }
...
//pre: null annotation analysis is enabled
    public ParameterNonNullDefaultProvider hasNonNullDefaultForParameter(AbstractMethodDeclaration srcMethod) {
        ...
        for (int i = 0; i < len; i++) {
            int start = srcMethod == null || srcMethod.arguments == null || srcMethod.arguments.length == 0
                ? -1
                : srcMethod.arguments[i].declarationSourceStart;
            int nonNullByDefaultValue
                = srcMethod != null && start >= 0 ? srcMethod.scope.localNonNullByDefaultValue(start) : 0;
        ...
        return trueFound
            ? ParameterNonNullDefaultProvider.TRUE_PROVIDER
            : ParameterNonNullDefaultProvider.FALSE_PROVIDER;
    }

//pre: null annotation analysis is enabled
    protected boolean hasNonNullDefaultForType(TypeBinding type, int location, AbstractMethodDeclaration srcMethod,
        int start) {
        if (type != null
            && !type.acceptsNonNullDefault()
            && srcMethod != null
            && srcMethod.scope.environment().usesNullTypeAnnotations())
            return false;
    ...
    public boolean redeclaresPublicObjectMethod(Scope scope) {
        if (this.selector[0] == 'h')
            return this.parameters.length == 0
                && this.selector.length == 8
                && CharOperation.equals(this.selector, TypeConstants.HASHCODE);
        if (this.selector[0] == 't')
            return this.parameters.length == 0
                && this.selector.length == 8
                && CharOperation.equals(this.selector, TypeConstants.TOSTRING);
        return this.selector[0] == 'e'
            && this.parameters.length == 1
            && this.selector.length == 6
            && CharOperation.equals(this.selector, TypeConstants.EQUALS)
            && TypeBinding.equalsEquals(this.parameters[0], scope.getJavaLangObject());
    }
    ...
    /**
     * Identifies whether the method has Polymorphic signature based on <a
     * href=https://docs.oracle.com/javase/specs/jls/se11/html/jls-15.html#jls-15.12.3>jls-15.12.3</a><br/>
     *
        ...
        if (this.isNative() && this.isVarargs() && this.parameters.length == 1) {
            ...
            if (this.parameters[0].leafComponentType().id == TypeIds.T_JavaLangObject) {
                ...
                if ((declaringClassLocal != null)
                    && (declaringClassLocal.id == scope.getJavaLangInvokeMethodHandle().id
    ...
    public boolean isClosingMethod() {
        boolean isCloseMethod
            = CharOperation.equals(this.selector, TypeConstants.CLOSE) && this.parameters == NO_PARAMETERS;  // close()
