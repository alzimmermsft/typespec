 ...
 * also implements {@link ISynchronizable}. All
...
public class AnnotationModel
     ...
     *
     * @since 3.4
     * @see RegionIterator#RegionIterator(Iterator, IAnnotationModel, int, int, boolean, boolean)
     */
    private static final class RegionIterator implements Iterator<Annotation> {
         ...
         */
        public RegionIterator(Iterator<Annotation> parentIterator, IAnnotationModel model, int offset, int length,
    ...
    protected void replaceAnnotations(Annotation[] annotationsToRemove,
        Map<? extends Annotation, ? extends Position> annotationsToAdd, boolean fireModelChanged)
        ...
        if (annotationsToAdd != null) {
            Iterator<? extends Entry<? extends Annotation, ? extends Position>> iter
                = annotationsToAdd.entrySet().iterator();
            while (iter.hasNext()) {
                Entry<? extends Annotation, ? extends Position> mapEntry = iter.next();
     ...
     */
    @Override
    public Iterator<Annotation> getAnnotationIterator(int offset, int length, boolean canStartBefore,
        boolean canEndAfter) {
        ...
        while (it.hasNext()) {
            ...
            if (attachment instanceof IAnnotationModelExtension2)
                iterators.add(((IAnnotationModelExtension2) attachment).getAnnotationIterator(offset, length,
                    canStartBefore, canEndAfter));
            else
                iterators.add(new RegionIterator(attachment.getAnnotationIterator(), attachment, offset, length,
                    canStartBefore, canEndAfter));
        }
    ...
    /**
     * Returns an iterator as specified in
     * {@link IAnnotationModelExtension2#getAnnotationIterator(int, int, boolean, boolean)}
     *
     ...
     * @since 3.4
     */
    private Iterator<Annotation> getRegionAnnotationIterator(int offset, int length, boolean canStartBefore,
        boolean canEndAfter) {
        ...
        try {
            Position[] positions
                = document.getPositions(IDocument.DEFAULT_CATEGORY, offset, length, canStartBefore, canEndAfter);
