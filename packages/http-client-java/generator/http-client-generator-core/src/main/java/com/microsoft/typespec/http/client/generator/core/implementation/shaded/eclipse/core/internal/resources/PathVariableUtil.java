...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.internal.resources.projectvariables.ParentVariableResolver;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.internal.resources.projectvariables.ProjectLocationVariableResolver;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.internal.resources.projectvariables.WorkspaceLocationVariableResolver;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.internal.resources.projectvariables.WorkspaceParentLocationVariableResolver;
...
public class PathVariableUtil {
    ...
    static public String getUniqueVariableName(String variable, IResource resource) {
        ...
        if (variable.startsWith(ParentVariableResolver.NAME)
            || variable.startsWith(ProjectLocationVariableResolver.NAME)) {
    ...
    public static IPath convertToPathRelativeMacro(IPathVariableManager pathVariableManager, IPath originalPath,
        IResource resource, boolean force, String variableHint) throws CoreException {
        return convertToRelative(pathVariableManager, originalPath, resource, force, variableHint, true, true);
    }

    static public URI convertToRelative(IPathVariableManager pathVariableManager, URI originalPath, IResource resource,
        boolean force, String variableHint) throws CoreException {
        return URIUtil.toURI(convertToRelative(pathVariableManager, URIUtil.toPath(originalPath), resource, force,
            variableHint, true, false));
    }

    static private IPath convertToRelative(IPathVariableManager pathVariableManager, IPath originalPath,
        IResource resource, boolean force, String variableHint, boolean skipWorkspace, boolean generateMacro)
        ...
        if (variableHint != null && pathVariableManager.isDefined(variableHint)) {
            ...
            if (value != null) {
                return wrapInProperFormat(
                    makeRelativeToVariable(pathVariableManager, originalPath, force, variableHint), generateMacro);
            }
        }
        ...
        for (String variable : existingVariables) {
            if (skipWorkspace) {
                // Variables relative to the workspace are not portable, and defeat the purpose of having linked
                // resource locations,
        ...
        if (force) {
            ...
            for (int j = 0; j <= originalSegmentCount; j++) {
                ...
                for (String variable : existingVariables) {
                    ...
                    if (value != null) {
                        value
                            = convertToProperCase(URIUtil.toPath(pathVariableManager.resolveURI(URIUtil.toURI(value))));
        ...
        if (skipWorkspace) {
            return convertToRelative(pathVariableManager, originalPath, resource, force, variableHint, false,
                generateMacro);
        }
        return originalPath;
    }
    ...
    private static IPath makeRelativeToVariable(IPathVariableManager pathVariableManager, IPath originalPath,
        boolean force, String variableHint) {
        IPath path = convertToProperCase(originalPath);
        IPath value = URIUtil.toPath(pathVariableManager.getURIValue(variableHint));
        value = convertToProperCase(URIUtil.toPath(pathVariableManager.resolveURI(URIUtil.toURI(value))));
        ...
        if (force) {
            if (devicesAreCompatible(path, value)) {
                ...
                if (matchingFirstSegments >= 0) {
                    String originalName
                        = buildParentPathVariable(variableHint, valueSegmentCount - matchingFirstSegments, true);
    ...
    private static void convertVariableToUserFormat(StringBuilder buffer, String component, String variable,
        ...
        if (PathVariableUtil.isParentVariable(variable)) {
            ...
            if (argument != null && count != -1) {
                buffer.append(generateMacro
                    ? PathVariableUtil.buildVariableMacro(IPath.fromOSString(argument))
