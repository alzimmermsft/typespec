...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.util.IStackMapAttribute;
...
public class StackMapAttribute extends ClassFileAttribute implements IStackMapAttribute {
     ...
     */
    public StackMapAttribute(byte[] classFileBytes, IConstantPool constantPool, int offset)
        ...
        if (length != 0) {
            ...
            for (int i = 0; i < length; i++) {
                DefaultStackMapFrame frame
                    = new DefaultStackMapFrame(classFileBytes, constantPool, offset + readOffset);
                this.frames[i] = frame;
