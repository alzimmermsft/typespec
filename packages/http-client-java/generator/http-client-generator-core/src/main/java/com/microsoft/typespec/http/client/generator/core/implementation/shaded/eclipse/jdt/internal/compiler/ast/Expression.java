...
public abstract class Expression extends Statement {
    ...
    /**
     * Back-propagation of flow info: before analysing a branch where a given condition is known to hold true/false
     * respectively,
    ...
    public boolean hasSideEffects() {
        return (this.constant == Constant.NotAConstant || (this.implicitConversion & TypeIds.BOXING) != 0)
    ...
    public final boolean checkCastTypesCompatibility(Scope scope, TypeBinding castType, TypeBinding expressionType,
        Expression expression, boolean useAutoBoxing) {
        ...
        if (castType.isBaseType()) {
            if (expressionType.isBaseType()) {
                ...
                && checkCastTypesCompatibility(scope, scope.boxing(castType), expressionType, expression,
                    useAutoBoxing)) {
                ...
                && scope.environment().computeBoxingType(expressionType).isCompatibleWith(castType)) { // unboxing -
                                                                                                       // only widening
                                                                                                       // match is
                                                                                                       // allowed
                tagAsUnnecessaryCast(scope, castType);
            ...
            && scope.environment().computeBoxingType(expressionType).isCompatibleWith(castType)) { // boxing - only
                                                                                                   // widening match is
                                                                                                   // allowed
            tagAsUnnecessaryCast(scope, castType);
            return true;
     ...
     * @param ttlForFieldCheck if this is a reference to a field we will mark that field as nonnull for the specified
     * timeToLive
    ...
    public void computeConversion(Scope scope, TypeBinding runtimeType, TypeBinding compileTimeType) {
        ...
        if (this.implicitConversion != 0)
            return; // already set independently
        ...
        if ((compileTimeTypeID = compileTimeType.id) >= TypeIds.T_LastWellKnownTypeId) { // e.g. ? extends String ==>
                                                                                         // String (103227); >=
                                                                                         // TypeIds.T_LastWellKnownTypeId
                                                                                         // implies TypeIds.NoId
            compileTimeTypeID = compileTimeType.erasure().id == TypeIds.T_JavaLangString
                ? TypeIds.T_JavaLangString
                : TypeIds.T_JavaLangObject;
        } else if (runtimeType.isPrimitiveType()
            && compileTimeType instanceof ReferenceBinding
            && !compileTimeType.isBoxedPrimitiveType()) {
...
//				}
    ...
    public void generateOptimizedStringConcatenation(BlockScope blockScope, CodeStream codeStream, int typeID) {
        if (typeID == TypeIds.T_JavaLangString
    ...
    public void generateOptimizedStringConcatenationCreation(BlockScope blockScope, CodeStream codeStream, int typeID) {
        ...
        switch (typeID) {
            case T_JavaLangObject:
            case T_undefined:
                // in the case the runtime value of valueOf(Object) returns null, we have to use append(Object) instead
                // of directly valueOf(Object)
                // append(Object) returns append(valueOf(Object)), which means that the null case is handled by the next
                // case.
    ...
    private void addArgumentToRecipe(BlockScope blockScope, CodeStream codeStream, StringBuilder recipe,
        TypeBinding argType, List<TypeBinding> args) {
        recipe.append(STRING_CONCAT_FACTORY_TAG_ARG);
        args.add(argType);
        if (args.size() > 190) {
            // StringConcatFactory#makeConcatWithConstants() can take only 200 arguments. Commit whatever we have
            // accumulated so far
            // Get the intermediate result pushed to the stack and to be used as an operand for the subsequent concat
            // operation
    ...
    public void buildStringForConcatenation(BlockScope blockScope, CodeStream codeStream, int typeID,
        StringBuilder recipe, List<TypeBinding> argTypes) {
        if (this.constant == Constant.NotAConstant) {
            switch (typeID) {
                case T_JavaLangString:
                case TypeIds.T_int:
            ...
            if (stringValue.indexOf(TypeConstants.STRING_CONCAT_FACTORY_TAG_ARG) != -1
                || stringValue.indexOf(TypeConstants.STRING_CONCAT_FACTORY_TAG_CONST) != -1) {
                codeStream.ldc(stringValue);
                addArgumentToRecipe(blockScope, codeStream, recipe, blockScope.getJavaLangString(), argTypes);
            } else {
...
// this is no more called for now, waiting for inter procedural null reference analysis
    ...
    public TypeBinding resolveTypeExpectingWithBindings(LocalVariableBinding[] bindings, BlockScope scope,
            ...
            if (receiverBinding.isParameter()
                ...
                if (field.type.isRawType()) {
                    if (referenceContext instanceof AbstractMethodDeclaration) {
                        ...
                        if (TypeBinding.notEquals(field.declaringClass, declaringClass)) { // inherited raw field, see
                                                                                           // https://bugs.eclipse.org/bugs/show_bug.cgi?id=337962
                            return true;
                        }
                    } else if (referenceContext instanceof TypeDeclaration) {
                        TypeDeclaration type = (TypeDeclaration) referenceContext;
                        if (TypeBinding.notEquals(field.declaringClass, type.binding)) { // inherited raw field, see
                                                                                         // https://bugs.eclipse.org/bugs/show_bug.cgi?id=337962
                            return true;
                        }
                    }
    ...
    /*
     * Answer if the receiver is a poly expression in the prevailing context. Caveat emptor: Some constructs (notably
     * method calls)
     * cannot answer this question until after resolution is over and may throw unsupported operation exception if
     * queried ahead of
     * resolution. Default implementation here returns false which is true for vast majority of AST nodes. The ones that
     * are poly
