...
public class EnumDeclaration extends AbstractTypeDeclaration {
     ...
     */
    public static final ChildListPropertyDescriptor ENUM_CONSTANTS_PROPERTY = new ChildListPropertyDescriptor(
        EnumDeclaration.class, "enumConstants", EnumConstantDeclaration.class, CYCLE_RISK); //$NON-NLS-1$
     ...
     * Defaults to an empty list.
     */
    private final NodeList enumConstants = new NodeList(ENUM_CONSTANTS_PROPERTY);
    ...
    ASTNode clone0(AST target) {
        ...
        result.enumConstants().addAll(ASTNode.copySubtrees(target, enumConstants()));
    ...
    int treeSize() {
        return memSize() + (this.optionalDocComment == null ? 0 : getJavadoc().treeSize()) + this.modifiers.listSize()
            + (this.typeName == null ? 0 : getName().treeSize()) + this.superInterfaceTypes.listSize()
            + this.enumConstants.listSize() + this.bodyDeclarations.listSize();
    }
}
