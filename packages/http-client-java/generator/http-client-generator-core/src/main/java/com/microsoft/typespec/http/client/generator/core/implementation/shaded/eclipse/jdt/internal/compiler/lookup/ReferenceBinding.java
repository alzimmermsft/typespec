 ...
 *								bug 400421 - [compiler] Null analysis for fields does not take @com.microsoft.typespec.http.client.generator.core.implementation.shaded.google.inject.Inject into account
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.NullAnnotationMatching;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.RecordComponent;
...
abstract public class ReferenceBinding extends TypeBinding {
    ...
    protected static final DysfunctionalInterfaceException DYSFUNCTIONAL_INTERFACE_EXCEPTION
        = new DysfunctionalInterfaceException("Not a functional interface"); //$NON-NLS-1$
    ...
    static protected ProblemMethodBinding samProblemBinding
        = new ProblemMethodBinding(TypeConstants.ANONYMOUS_METHOD, null, ProblemReasons.NoSuchSingleAbstractMethod);
    ...
    /**
     * Returns a combined range value representing: {@code (start + (end<<32))}, where start is the index of the first
     * matching method
     * (remember methods are sorted alphabetically on selectors), and end is the index of last contiguous methods with
     * same
     * selector.
    ...
    public static long binarySearch(char[] selector, MethodBinding[] sortedMethods) {
        ...
        while (left <= right) {
            mid = left + (right - left) / 2;
            int compare
                = compare(selector, midSelector = sortedMethods[mid].selector, selectorLength, midSelector.length);
            if (compare < 0) {
                right = mid - 1;
            } else if (compare > 0) {
                left = mid + 1;
     ...
     */
    static final Comparator<ReferenceBinding> BASIC_MEMBER_TYPES_COMPARATOR
        = (ReferenceBinding o1, ReferenceBinding o2) -> {
    ...
    public boolean canBeSeenBy(ReferenceBinding receiverType, ReferenceBinding invocationType) {
        ...
        if (isPrivate()) {
            ...
            if (!(TypeBinding.equalsEquals(receiverType, this)
                || TypeBinding.equalsEquals(receiverType, enclosingType()))) {
                return false;
            }
        ...
        do {
            if (currentType.isCapture()) {  // https://bugs.eclipse.org/bugs/show_bug.cgi?id=285002
                ...
                if (TypeBinding.equalsEquals(originalDeclaringClass, currentType.original()))
                    return true;
            }
        ...
        switch (this.compoundName.length) {
                ...
                switch (packageName.length) {
                    ...
                    case 5: // javax
                        switch (packageName[1]) {
                            case 'a':
                                if (CharOperation.equals(TypeConstants.JAVAX_ANNOTATION_INJECT_INJECT,
                                    this.compoundName))
                    ...
                    case 7: // jakarta
                        switch (packageName[1]) {
                            case 'a':
                                if (CharOperation.equals(TypeConstants.JAKARTA_ANNOTATION_INJECT_INJECT,
                                    this.compoundName))
                                ...
                                switch (typeName[0]) {
                                    case 'C':
                                        if (CharOperation.equals(typeName, TypeConstants.JAVA_IO_CLOSEABLE[2]))
                                            this.typeBits |= TypeIds.BitCloseable; // don't assign id, only typeBit (for
                                                                                   // analysis of resource leaks)
                                        return;
                                    ...
                                    case 'O':
                                        if (CharOperation.equals(typeName,
                                            TypeConstants.JAVA_IO_OBJECTSTREAMEXCEPTION[2]))
                    ...
                    case 'a':
                        if (CharOperation.equals(packageName, TypeConstants.ANNOTATION)) {
                            switch (typeName[0]) {
                                case 'A':
                                    if (CharOperation.equals(typeName,
                                        TypeConstants.JAVA_LANG_ANNOTATION_ANNOTATION[3]))
                                        this.id = TypeIds.T_JavaLangAnnotationAnnotation;
                                    return;

                                case 'D':
                                    if (CharOperation.equals(typeName,
                                        TypeConstants.JAVA_LANG_ANNOTATION_DOCUMENTED[3]))
                                        this.id = TypeIds.T_JavaLangAnnotationDocumented;
                                    return;

                                case 'E':
                                    if (CharOperation.equals(typeName,
                                        TypeConstants.JAVA_LANG_ANNOTATION_ELEMENTTYPE[3]))
                                ...
                                case 'R':
                                    switch (typeName.length) {
                                        case 9:
                                            if (CharOperation.equals(typeName,
                                                TypeConstants.JAVA_LANG_ANNOTATION_RETENTION[3]))
                                                this.id = TypeIds.T_JavaLangAnnotationRetention;
                                            return;

                                        case 10:
                                            if (CharOperation.equals(typeName,
                                                TypeConstants.JAVA_LANG_ANNOTATION_REPEATABLE[3]))
                ...
                switch (packageName[0]) {
                    ...
                    case 'o':
                        ...
                        switch (packageName[0]) {
                                                ...
                                                switch (typeName[0]) {
                                                    case 'r':
                                                        ...
                                                        if (memberTypeName.length == 0)
                                                            return; // just to be safe
                                                        if (CharOperation.equals(typeName,
                                                            TypeConstants.ORG_ECLIPSE_CORE_RUNTIME_ASSERT[3])
                                                            && CharOperation.equals(memberTypeName,
                                                                TypeConstants.ORG_ECLIPSE_CORE_RUNTIME_ASSERT[4]))
                            ...
                            case 'a':
                                if (CharOperation.equals(TypeConstants.APACHE, packageName)) {
                                    if (CharOperation.equals(TypeConstants.COMMONS, this.compoundName[2])) {
                                        if (CharOperation.equals(TypeConstants.ORG_APACHE_COMMONS_LANG_VALIDATE,
                                            this.compoundName))
                                            this.id = TypeIds.T_OrgApacheCommonsLangValidate;
                                        else if (CharOperation.equals(TypeConstants.ORG_APACHE_COMMONS_LANG3_VALIDATE,
                                            this.compoundName))
                            ...
                            case 'j':
                                if (CharOperation.equals(TypeConstants.ORG_JUNIT_JUPITER_API_ASSERTIONS,
                                    this.compoundName))
            ...
            case 6:
                if (CharOperation.equals(TypeConstants.ORG, this.compoundName[0])) {
                    ...
                    if (!CharOperation.equals(TypeConstants.JDT, this.compoundName[2])
                        || !CharOperation.equals(TypeConstants.ITYPEBINDING, this.compoundName[5]))
                        return;
            ...
            case 7:
                if (!CharOperation.equals(TypeConstants.JDT, this.compoundName[2])
                    || !CharOperation.equals(TypeConstants.TYPEBINDING, this.compoundName[6]))
                    return;
                if (CharOperation.equals(TypeConstants.ORG_ECLIPSE_JDT_INTERNAL_COMPILER_LOOKUP_TYPEBINDING,
                    this.compoundName))
                    this.typeBits |= TypeIds.BitUninternedType;
                break;
        }
    ...
    public String debugName() {
        return (this.compoundName != null)
            ? this.hasTypeAnnotations() ? annotatedDebugName() : new String(readableName())
    ...
    public boolean detectAnnotationCycle() {
        if ((this.tagBits & TagBits.EndAnnotationCheck) == TagBits.EndAnnotationCheck)
            return false; // already checked
        if ((this.tagBits & TagBits.BeginAnnotationCheck) == TagBits.BeginAnnotationCheck)
        ...
        for (MethodBinding currentMethod : currentMethods) {
            ...
            if (TypeBinding.equalsEquals(this, returnType)) {
                if (this instanceof SourceTypeBinding) {
                    ...
                    ((SourceTypeBinding) this).scope.problemReporter()
                        .annotationCircularity(this, this, decl != null ? decl.returnType : null);
                }
            } else if (returnType.isAnnotationType() && ((ReferenceBinding) returnType).detectAnnotationCycle()) {
                if (this instanceof SourceTypeBinding) {
                    MethodDeclaration decl = (MethodDeclaration) currentMethod.sourceMethod();
                    ((SourceTypeBinding) this).scope.problemReporter()
                        .annotationCircularity(this, returnType, decl != null ? decl.returnType : null);
                }
                inCycle = true;
            }
        }
...
// The suggested parameter length is optional and may not be guaranteed by every type.
    ...
    @Override
    public int hashCode() {
        // ensure ReferenceBindings hash to the same position as UnresolvedReferenceBindings so they can be replaced
        // without rehashing
...
// pre: null annotation analysis is enabled
    ...
    public boolean implementsInterface(ReferenceBinding anInterface, boolean searchHierarchy) {
        ...
        do {
            ReferenceBinding[] itsInterfaces = currentType.superInterfaces();
            if (itsInterfaces != null && itsInterfaces != Binding.NO_SUPERINTERFACES) { // in code assist cases when
                                                                                        // source types are added late,
                                                                                        // may not be finished
...
// Internal method... assume its only sent to classes NOT interfaces
            ...
            case Binding.INTERSECTION_TYPE18:
                ...
                if (otherReferenceType.isIntersectionType18()) {
                    ReferenceBinding[] intersectingTypes
                        = ((IntersectionTypeBinding18) otherReferenceType).intersectingTypes;
                ...
                if (otherReferenceType.isInterface()) { // could be annotation type
                    ...
                    if (this instanceof TypeVariableBinding && captureScope != null) {
                        ...
                        if (typeVariable.firstBound instanceof ParameterizedTypeBinding) {
                            TypeBinding bound = typeVariable.firstBound.capture(captureScope, -1, -1); // no position
                                                                                                       // needed as this
                                                                                                       // capture will
                                                                                                       // never escape
                                                                                                       // this context
    ...
    public final boolean isDefault() {
        return (this.modifiers
            & (ClassFileConstants.AccPublic | ClassFileConstants.AccProtected | ClassFileConstants.AccPrivate)) == 0;
    }
    ...
    public boolean isHierarchyBeingActivelyConnected() {
        return (this.tagBits & TagBits.EndHierarchyCheck) == 0
            && (this.tagBits & TagBits.BeginHierarchyCheck) != 0
    ...
    public final boolean isOrEnclosedByPrivateType() {
        if (isLocalType())
            return true; // catch all local types
        ReferenceBinding type = this;
        while (type != null) {
    ...
    public TypeVariableBinding[] typeVariablesIncludingEnclosing() {
        ...
        TypeVariableBinding[] outerVariables = hasEnclosingInstanceContext() && enclosingType != null
            ? enclosingType.typeVariablesIncludingEnclosing()
        ...
        System.arraycopy(outerVariables, 0,
            allVariables = new TypeVariableBinding[outerVariablesCount + ownVariables.length], 0, outerVariablesCount);
    ...
    public TypeBinding[] typeArgumentsIncludingEnclosing() {
        ...
        TypeBinding[] outerArguments
            = hasEnclosingInstanceContext() && enclosingType != null ? enclosingType.typeArguments() : Binding.NO_TYPES;
        ...
        System.arraycopy(outerArguments, 0, allArguments = new TypeBinding[outerArgumentsCount + ownArguments.length],
            0, outerArgumentsCount);
        ...
        if (options.isAnnotationBasedNullAnalysisEnabled) {
            if (options.usesNullTypeAnnotations()) {
                ...
                if ((this.tagBits & TagBits.AnnotationNullable) != 0) {
                    ...
                    nameBuffer.append('@')
                        .append(nullableAnnotationName[nullableAnnotationName.length - 1])
                        .append(' ');
                }
            }
        }
    ...
    char[] nullAnnotatedReadableName(CompilerOptions options) {
        ...
        if (arguments != null && arguments.length > 0) { // empty arguments array happens when PTB has been created just
                                                         // to capture type annotations
            nameBuffer.append('<');
    ...
    public char[] shortReadableName(boolean showGenerics) /* Object */ {
        ...
        if (isMemberType()) {
            shortReadableName = CharOperation.concat(
                enclosingType().shortReadableName(showGenerics && hasEnclosingInstanceContext()), this.sourceName, '.');
        } else {
    ...
    protected int applyCloseableWhitelists(CompilerOptions options) {
        return isInterface() ? applyCloseableInterfaceWhitelists(options) : applyCloseableClassWhitelists(options);
    }
            ...
            if (numResourceFields == 1) {
                ...
                for (MethodBinding method : methods()) {
                    if (method.isConstructor() && !method.isPrivate() && method.parameters.length > 0) {
                        ...
                        if (param.hasTypeBit(TypeIds.BitAutoCloseable | TypeIds.BitCloseable)
                            && method.ownsParameter(0)) {
    ...
    private MethodBinding[] getFunctionalInterfaceAbstractContracts(Scope scope, boolean replaceWildcards)
        throws DysfunctionalInterfaceException {
        ...
        for (int i = 0, length = methods.length; i < length; i++) {
            ...
            for (int j = i + 1; j < length; j++) {
                ...
                if (method.selector.length != otherMethod.selector.length
                    || !CharOperation.equals(method.selector, otherMethod.selector))
                    break; // Skip comparing apple to oranges (input sorted by selector, so no viable overrides past
                           // this point)
                if (MethodVerifier.doesMethodOverride(otherMethod, method, environment)) {
                    methods[i] = method = null;
                    break; // Skip comparing rotten apple with remaining lot: is overridden already, no use checking if
                           // is overridden also by another
                }
            }
    ...
    protected Stream<MethodBinding> collateFunctionalInterfaceContracts(Scope scope, boolean replaceWildcards,
        Set<ReferenceBinding> visitedInterfaces) throws DysfunctionalInterfaceException {
        ...
        return Stream.concat( //
            Arrays.stream(superInterfaces())
                .filter(iface -> visitedInterfaces.add(iface))
                .flatMap(superInterface -> superInterface.collateFunctionalInterfaceContracts(scope, replaceWildcards,
                    visitedInterfaces)), //
    ...
    public MethodBinding getSingleAbstractMethod(Scope scope, boolean replaceWildcards) {
        ...
        try {
            ...
            for (MethodBinding method : methods) {
                ...
                if (contractSelector == null) {
                    ...
                    if (methodParameterLength != contractParameterLength
                        || !CharOperation.equals(method.selector, contractSelector))
                        return this.singleAbstractMethod[index] = samProblemBinding;
                }
            }
        ...
        next: for (int i = length - 1; i >= 0; --i) {
            ...
            // If we reach here, we found a method that is override equivalent with every other method and is also
            // return type substitutable. Compute kosher exceptions now ...
            ...
            boolean shouldEraseThrows
                = theAbstractMethod.typeVariables == Binding.NO_TYPE_VARIABLES && genericMethodSeen;
            ...
            none: for (i = 0; i < length; i++) {
                ...
                if (shouldAdaptThrows && method != theAbstractMethod) {
                    System.arraycopy(methodThrownExceptions, 0,
                        methodThrownExceptions = new ReferenceBinding[methodExceptionsLength], 0,
                        methodExceptionsLength);
                ...
                nextException: for (int j = 0; j < methodExceptionsLength; j++) {
                    ...
                    nextMethod: for (int k = 0; k < length; k++) {
                        ...
                        int otherMethodExceptionsLength
                            = otherMethodThrownExceptions == null ? 0 : otherMethodThrownExceptions.length;
                        if (otherMethodExceptionsLength == 0)
                            break none;
                        if (shouldAdaptThrows && otherMethod != theAbstractMethod) {
                            System.arraycopy(otherMethodThrownExceptions, 0,
                                otherMethodThrownExceptions = new ReferenceBinding[otherMethodExceptionsLength], 0,
                    ...
                    if (exceptionsCount == exceptionsLength) {
                        System.arraycopy(exceptions, 0, exceptions = new ReferenceBinding[exceptionsLength += 16], 0,
                            exceptionsCount);
                    }
            ...
            this.singleAbstractMethod[index]
                = new MethodBinding(theAbstractMethod.modifiers | ClassFileConstants.AccSynthetic,
                    theAbstractMethod.selector, returnType, parameters, exceptions, theAbstractMethod.declaringClass);
...
// 5.1.6.1 Allowed Narrowing Reference Conversion
    public boolean isDisjointFrom(ReferenceBinding that) {
        if (this.isInterface()) {
            if (that.isInterface()) {
                /*
                 * • An interface named I is disjoint from another interface named J if (i) it is not that case that I
                 * <: J, and (ii) it is not the case that J <: I, and
            ...
            if (that.isInterface()) {
                /*
                 * • A class named C is disjoint from an interface named I if (i) it is not the case that C <: I, and
                 * (ii) one of the following cases applies:
                 * – C is final.
                 * – C is sealed, and all of the permitted direct subclasses of C are disjoint from I.
                 * – C is freely extensible (§8.1.1.2), and I is sealed, and C is disjoint from all of the permitted
                 * direct subclasses and subinterfaces of I
                 */
                ...
                // • A class named C is disjoint from another class named D if (i) it is not the case that C <: D, and
                // (ii) it is not the case that D <: C.
                return this.findSuperTypeOriginatingFrom(that) == null
                    && that.findSuperTypeOriginatingFrom(this) == null;
            }
        }
    }
