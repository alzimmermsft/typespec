...
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.ExpressionContext.ASSIGNMENT_CONTEXT;
...
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.ExpressionContext.VANILLA_CONTEXT;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.flow.ConditionalFlowInfo;
...
public class MessageSend extends Expression implements IPolyExpression, Invocation {
    ...
    // hold on to this context from invocation applicability inference until invocation type inference (per method
    // candidate):
    ...
    BiConsumer<FlowInfo, Boolean> flowUpdateOnBooleanResult; // we assume only one arg can be affected, hence no need
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if (analyseResources) {
            if (nonStatic) {
                ...
                && FakedTrackingVariable.isAnyCloseable(this.arguments[0].resolvedType)) {
        ...
        if (compilerOptions.isAnyEnabled(IrritantSet.UNLIKELY_ARGUMENT_TYPE)
            ...
            if (this.arguments.length == 1 && !this.binding.isStatic()) {
                UnlikelyArgumentCheck argumentChecks
                    = UnlikelyArgumentCheck.determineCheckForNonStaticSingleArgumentMethod(this.argumentTypes[0],
                        currentScope, this.selector, this.actualReceiverType, this.binding.parameters);

                if (argumentChecks != null && argumentChecks.isDangerous(currentScope)) {
                    currentScope.problemReporter()
                        .unlikelyArgumentType(this.arguments[0], this.binding, this.argumentTypes[0],
                            argumentChecks.typeToReport, argumentChecks.dangerousMethod);
                }
                ...
                UnlikelyArgumentCheck argumentChecks = UnlikelyArgumentCheck.determineCheckForStaticTwoArgumentMethod(
                    this.argumentTypes[1], currentScope, this.selector, this.argumentTypes[0], this.binding.parameters,
                    this.actualReceiverType);

                if (argumentChecks != null && argumentChecks.isDangerous(currentScope)) {
                    currentScope.problemReporter()
                        .unlikelyArgumentType(this.arguments[1], this.binding, this.argumentTypes[1],
                            argumentChecks.typeToReport, argumentChecks.dangerousMethod);
                }
            }
        }
        ...
        if (this.arguments != null) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                switch (detectAssertionUtility(i)) {
                    case TRUE_ASSERTION:
                        flowInfo = analyseBooleanAssertion(currentScope, argument, flowContext, flowInfo,
                            wasInsideAssert, true);
                        break;

                    case FALSE_ASSERTION:
                        flowInfo = analyseBooleanAssertion(currentScope, argument, flowContext, flowInfo,
                            wasInsideAssert, false);
                        break;
                    ...
                    case ARG_NONNULL_IF_TRUE_NEGATABLE:
                        ...
                        flowInfo = argument.analyseCode(currentScope, flowContext, flowInfo).unconditionalInits();
                        flowInfo = setUnreachable(flowInfo, flowContext, localBinding,
                            AssertUtil.ARG_NONNULL_IF_TRUE_NEGATABLE);
                        break;
                ...
                if (analyseResources) {
                    flowInfo = handleResourcePassedToInvocation(currentScope, this.binding, argument, i, flowContext,
                        flowInfo);
                }
            }
            ...
            switch (assertUtil) {
                case ARG_NONNULL_IF_TRUE: {
                    ...
                    if (isDefinitellyNull) {
                        ...
                        ((ConditionalFlowInfo) flowInfo).initsWhenTrue()
                            .setReachMode(FlowInfo.UNREACHABLE_BY_NULLANALYSIS);
                    }
                    break;
                }
                ...
                case ARG_NULL_IF_TRUE: {
                    ...
                    flowInfo = FlowInfo.conditional(flowInfo.copy(), flowInfo.copy());
                    if (isDefinitellyNull) {
                        ((ConditionalFlowInfo) flowInfo).initsWhenFalse()
                            .setReachMode(FlowInfo.UNREACHABLE_BY_NULLANALYSIS);
    ...
    private void recordCallingClose(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo,
        ...
        if (closeTarget.isThis() || closeTarget.isSuper()) {
            ...
            ReferenceBinding currentClass = this.binding.declaringClass; // responsibility starts at the class and
                                                                         // upwards
            while (currentClass != null) {
                for (FieldBinding fieldBinding : currentClass.fields()) {
                    if (fieldBinding.closeTracker != null) {
                        ...
                        if (trackingVariable.methodScope == null
                            || trackingVariable.methodScope == currentScope.methodScope()) {
            ...
            FakedTrackingVariable trackingVariable = FakedTrackingVariable.getCloseTrackingVariable(closeTarget,
                flowInfo, flowContext, currentScope.compilerOptions().isAnnotationBasedResourceAnalysisEnabled);
...
// if so answer what kind of assertion we are facing.
            ...
            if (declaringClass != null && parameterType != null) {
                switch (declaringClass.original().id) {
                    case TypeIds.T_OrgEclipseCoreRuntimeAssert:
                        ...
                        if (parameterType.id == TypeIds.T_JavaLangObject
                            && CharOperation.equals(TypeConstants.IS_NOTNULL, this.selector))
    ...
    private FlowInfo analyseBooleanAssertion(BlockScope currentScope, Expression argument, FlowContext flowContext,
        FlowInfo flowInfo, boolean wasInsideAssert, boolean passOnTrue) {
        ...
        if (!passOnTrue)
            flowContext.tagBits |= FlowContext.INSIDE_NEGATION; // this affects syntactic analysis for fields in
                                                                // EqualExpression
    ...
    private FlowInfo analyseNullAssertion(BlockScope currentScope, Expression argument, FlowContext flowContext,
        FlowInfo flowInfo, boolean expectingNull) {
        ...
        if (local != null) {// beyond this point the argument can only be null/nonnull
            ...
            if (!expectingNull
                ...
                if (field != null && (field.type.tagBits & TagBits.IsBaseType) == 0) {
                    flowContext.recordNullCheckedFieldReference((Reference) argument, 3); // survive this assert as a
                                                                                          // MessageSend and as a
                                                                                          // Statement
                }
            }
        }
    ...
    public boolean checkNPE(BlockScope scope, FlowContext flowContext, FlowInfo flowInfo, int ttlForFieldCheck) {
        ...
        if ((nullStatus & FlowInfo.POTENTIALLY_NULL) != 0) {
            if (this.binding.returnType.isTypeVariable()
                && nullStatus == FlowInfo.FREE_TYPEVARIABLE
    ...
    /**
     * @see org.eclipse.jdt.internal.compiler.ast.Expression#computeConversion(Scope, TypeBinding, TypeBinding)
     */
    @Override
    public void computeConversion(Scope scope, TypeBinding runtimeTimeType, TypeBinding compileTimeType) {
        ...
        if (this.binding != null && this.binding.isValidBinding()) {
            ...
            if (ArrayBinding.isArrayClone(this.actualReceiverType, this.binding)
                ...
                // from 1.5 source level on, array#clone() resolves to array type, but codegen to #clone()Object - thus
                // require extra inserted cast
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
        ...
        if (this.syntheticAccessor == null) {
            TypeBinding constantPoolDeclaringClass = CodeStream.getConstantPoolDeclaringClass(currentScope,
                codegenBinding, this.actualReceiverType, this.receiver.isImplicitThis());
            if (isStatic) {
    ...
    public int nullStatus(FlowInfo flowInfo, FlowContext flowContext) {
        ...
        if (this.binding.isValidBinding()) {
            ...
            if ((tagBits & TagBits.AnnotationNullMASK) == 0L) // alternatively look for type annotation (will only be
                                                              // present in 1.8+):
    ...
    public TypeBinding resolveType(BlockScope scope) {
                ...
                if (this.argumentsHaveErrors) {
                    if (this.actualReceiverType instanceof ReferenceBinding) {
                        ...
                        for (int i = length; --i >= 0;)
                            pseudoArgs[i] = this.argumentTypes[i] == null ? TypeBinding.NULL : this.argumentTypes[i]; // replace
                                                                                                                      // args
                                                                                                                      // with
                                                                                                                      // errors
                                                                                                                      // with
                        ...
                        this.binding = this.receiver.isImplicitThis()
                            ...
                            : scope.findMethod((ReferenceBinding) this.actualReceiverType, this.selector, pseudoArgs,
                                this, false);
                        ...
                        if (this.binding != null && !this.binding.isValidBinding()) {
                            ...
                            if (closestMatch != null) {
                                if (closestMatch.original().typeVariables != Binding.NO_TYPE_VARIABLES) { // generic
                                                                                                          // method
                                    // shouldn't return generic method outside its context, rather convert it to raw
                                    // method (175409)
                                    closestMatch = scope.environment()
                                        .createParameterizedGenericMethod(closestMatch.original(),
                                            (RawTypeBinding) null);
                                }
        ...
        if (compilerOptions.isAnnotationBasedNullAnalysisEnabled) {
            ...
            if (this.binding instanceof ParameterizedGenericMethodBinding && this.typeArguments != null) {
                ...
                for (int i = 0; i < this.typeArguments.length; i++)
                    this.typeArguments[i].checkNullConstraints(scope, (ParameterizedGenericMethodBinding) this.binding,
                        typeVariables, i);
            }
        }
        ...
        if (this.binding.isPolymorphic()) {
            ...
            if (!resultDetermined && ((this.bits & ASTNode.InsideExpressionStatement) != 0)) {
                // we only set the return type to be void if this method invocation is used inside an expression
                // statement
                this.binding = scope.environment()
                    .updatePolymorphicMethodReturnType((PolymorphicMethodBinding) this.binding, TypeBinding.VOID);
            }
        }
        ...
        if (!this.binding.isStatic()) {
            ...
            if (this.receiverIsType) {
                ...
                if (TypeBinding.notEquals(this.actualReceiverType, oldReceiverType)
                    && TypeBinding.notEquals(this.receiver.postConversionType(scope), this.actualReceiverType)) { // record
                                                                                                                  // need
                                                                                                                  // for
                                                                                                                  // explicit
                                                                                                                  // cast
            ...
            if (this.actualReceiverType != null
                && scope.isInsideEarlyConstructionContext(this.actualReceiverType, true)
            ...
            if (this.binding.declaringClass.isInterface()
                && !((isTypeAccess() || this.receiver.isImplicitThis())
                    && TypeBinding.equalsEquals(this.binding.declaringClass, this.actualReceiverType))) {
            ...
            if (!this.receiver.isImplicitThis()
                && TypeBinding.notEquals(this.binding.declaringClass, this.actualReceiverType)) {
                scope.problemReporter().indirectAccessToStaticMethod(this, this.binding);
            }
        }
        if (checkInvocationArguments(scope, this.receiver, this.actualReceiverType, this.binding, this.arguments,
        ...
        if (this.typeArguments != null && this.binding.original().typeVariables == Binding.NO_TYPE_VARIABLES) {
            scope.problemReporter()
                .unnecessaryTypeArgumentsForMethodInvocation(this.binding, this.genericTypeArguments,
                    this.typeArguments);
        }
    ...
    protected boolean isMissingTypeRelevant() {
        ...
        if ((this.binding.returnType.tagBits & TagBits.HasMissingType) == 0 && this.binding.isVarargs()) {
            ...
            if (argLen < this.binding.parameters.length) {
                ...
                for (int i = 0; i < argLen; i++) {
                    if ((this.binding.parameters[i].tagBits & TagBits.HasMissingType) != 0)
                        return true; // this one *is* relevant - actually this case is already detected during
                                     // findMethodBinding()
                }
                return false;
            }
                ...
                if (this.arguments[0] instanceof ReferenceExpression) {
                    ...
                    if (argumentBinding.isWellknownMethod(TypeIds.T_JavaUtilObjects, TypeConstants.NON_NULL)) {
                        ...
                        if (parameterizedType.arguments.length == 1) {
                            ...
                            TypeBinding updatedTypeArgument
                                = parameterizedType.arguments[0].withoutToplevelNullAnnotation();
                            updatedTypeArgument = environment.createNonNullAnnotatedType(updatedTypeArgument);
                            return environment.createParameterizedType(parameterizedType.genericType(),
                                new TypeBinding[] { updatedTypeArgument }, parameterizedType.enclosingType());
                        }
                    }
                }
            }
    ...
    /**
     * @see org.eclipse.jdt.internal.compiler.ast.Expression#setExpectedType(TypeBinding)
     */
    @Override
    ...
    public boolean isBoxingCompatibleWith(TypeBinding targetType, Scope scope) {
        if (this.argumentsHaveErrors
            ...
            || scope == null)
            return false;
        if (isPolyExpression() && !targetType.isPrimitiveOrBoxedPrimitiveType()) // i.e it is dumb to trigger inference,
                                                                                 // checking boxing compatibility
                                                                                 // against say Collector<? super T, A,
                                                                                 // R>.
            return false;
        TypeBinding originalExpectedType = this.expectedType;
        try {
            MethodBinding method
                = this.solutionsPerTargetType != null ? this.solutionsPerTargetType.get(targetType) : null;
            if (method == null) {
                this.expectedType = targetType;
                // No need to tunnel through overload resolution. this.binding is the MSMB.
                method = isPolyExpression()
                    ? ParameterizedGenericMethodBinding.computeCompatibleMethod18(this.binding.shallowOriginal(),
                        this.argumentTypes, scope, this)
                    : this.binding;
            ...
            if (method == null
                ...
                || !method.returnType.isValidBinding())
                return false;
            return super.isBoxingCompatible(method.returnType.capture(scope, this.sourceStart, this.sourceEnd),
    ...
    public boolean isPolyExpression(MethodBinding resolutionCandidate) {
        ...
        if (this.constant != Constant.NotAConstant)
            throw new UnsupportedOperationException(
                "Unresolved MessageSend can't be queried if it is a polyexpression"); //$NON-NLS-1$
