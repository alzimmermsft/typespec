...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.Pattern.TestContextRecord;
...
public class InstanceOfExpression extends OperatorExpression implements IGenerateTypeCheck {
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if (local != null && (local.type.tagBits & TagBits.IsBaseType) == 0) {
            ...
            flowContext.recordUsingNullReference(currentScope, local, this.expression,
                FlowContext.CAN_ONLY_NULL | FlowContext.IN_INSTANCEOF, flowInfo);
        ...
        if (this.pattern != null) {
            FlowInfo patternFlow = this.pattern.analyseCode(currentScope, flowContext,
                (initsWhenTrue == null) ? flowInfo : initsWhenTrue);
    ...
    public void generateOptimizedBoolean(BlockScope currentScope, CodeStream codeStream, BranchLabel trueLabel,
         ...
         * 
         * In this case, since we have no control over placement of the trueLabel, we won't know where to emit the
         * pattern binding code.
         * So what we do is always emit ifeq even when optimization would call for ifne and treat the whole "blob" of
         * pattern matching
         * code as part of the predicate. if you think about long and hard you can convince yourself this is
        ...
        BranchLabel internalFalseLabel
            = falseLabel != null ? falseLabel : this.pattern != null ? new BranchLabel(codeStream) : null;
        ...
        if (valueRequired) {
            if (falseLabel == null) {
                ...
                if (trueLabel == null) {
                    ...
                    // No implicit fall through TRUE/FALSE --> classic instanceof code generation called from
                    // generateCode above
                }
            }
        }
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (this.expression instanceof CastExpression) {
            ((CastExpression) this.expression).setInstanceofType(checkedType); // for cast expression we need to know
                                                                               // instanceof type to not tag unnecessary
                                                                               // when needed
        }
        TypeBinding expressionType = this.expression.resolveType(scope);
        if (this.pattern != null) {
            this.expression.computeConversion(scope, expressionType, expressionType); // avoid that a total pattern
                                                                                      // would skip a checkCast, needed
                                                                                      // due to generics
        ...
        if (expressionType != null && checkedType != null && this.type.hasNullTypeAnnotation(AnnotationPosition.ANY)) {
            ...
            if (!expressionType.isCompatibleWith(checkedType)
                || NullAnnotationMatching.analyse(checkedType, expressionType, -1).isAnyMismatch())
        ...
        if (!checkedType.isReifiable()) {
            ...
            if (options.complianceLevel < ClassFileConstants.JDK16) {
                ...
                if (expressionType != TypeBinding.NULL) {
                    boolean isLegal
                        = checkCastTypesCompatibility(scope, checkedType, expressionType, this.expression, true);
                    if (!isLegal || (this.bits & ASTNode.UnsafeCast) != 0) {
                        scope.problemReporter()
                            .unsafeCastInTestingContext(this.expression, checkedType, expressionType);
                    } else {
    ...
    private void checkRefForPrimitivesAndAddSecretVariable(BlockScope scope, TypeBinding checkedType,
    ...
    private void addSecretExpressionValue(BlockScope scope, TypeBinding expressionType) {
        if ((this.expression.bits & ASTNode.RestrictiveFlagMASK) != Binding.LOCAL) {
            ...
            TypeBinding type1 = (this.expression.resolvedType != null && this.expression.resolvedType.isBaseType())
            ...
            LocalVariableBinding local = new LocalVariableBinding(InstanceOfExpression.SECRET_EXPRESSION_VALUE, type1,
