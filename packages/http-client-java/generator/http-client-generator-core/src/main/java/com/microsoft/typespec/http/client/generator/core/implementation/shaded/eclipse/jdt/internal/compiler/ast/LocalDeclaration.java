...
public class LocalDeclaration extends AbstractVariableDeclaration {
            ...
            if (this.initialization != null) {
                ...
                if (this.binding.type.isArrayType()
                    && ((this.initialization instanceof CastExpression)	// arrayLoc = (type[])null
                        && (((CastExpression) this.initialization).innermostCastedExpression().resolvedType
                            == TypeBinding.NULL))) {
    ...
    public void resolve(BlockScope scope) {
        ...
        if (varTypedLocal) {
            ...
            if (isPatternVariable)
                variableType = this.type.resolvedType; // set already if this is a component pattern or is problem
                                                       // binding otherwise as it should be.
        } else {
        ...
        if (this.type != null) {
            ...
            if (existingVariable != null && existingVariable.isValidBinding() && !unnamedLocal) {
                ...
                if (localExists
                    && (this.bits & ASTNode.ShadowsOuterLocal) != 0
                    && scope.isLambdaSubscope()
                    ...
                    if (existingVariable.isPatternVariable()) {
                        scope.problemReporter()
                            .illegalRedeclarationOfPatternVar((LocalVariableBinding) existingVariable, this);
                    } else {
        ...
        this.binding = new LocalVariableBinding(this,
            varTypedLocal && variableType == null ? scope.getJavaLangObject() : variableType, this.modifiers,
            false /* isArgument */);
        if (isPatternVariable)
        ...
        if (this.initialization != null) {
            if (this.initialization instanceof ArrayInitializer) {
                ...
                if (varTypedLocal) {
                    this.binding.useFlag = LocalVariableBinding.ILLEGAL_SELF_REFERENCE_IF_USED; // hijack analysis phase
                                                                                                // flag
                ...
                if (varTypedLocal)
                    this.binding.useFlag = LocalVariableBinding.UNUSED; // hand-over hijacked flag; let flow analysis do
                                                                        // what it does with it.

                if ((variableType == null && !varTypedLocal) || // having waited until any additional errors in
                                                                // initializer are surfaced, bail out
                ...
                if (initializationType != null) {
                    ...
                    if (TypeBinding.notEquals(variableType, initializationType)) // must call before computeConversion()
                    ...
                    if (this.initialization.isConstantValueOfTypeAssignableToType(initializationType, variableType)
                        ...
                        if ((variableType.tagBits & TagBits.HasMissingType) == 0) {
                            ...
                            scope.problemReporter()
                                .typeMismatchError(initializationType, variableType, this.initialization, null);
                        }
                    }
                }
            }
            ...
            this.binding.setConstant(this.binding.isFinal()
    ...
    public boolean isRecoveredFromLoneIdentifier() { // recovered from lonely identifier or identifier cluster ?
        return this.name == RecoveryScanner.FAKE_IDENTIFIER
            && (this.type instanceof SingleTypeReference
                || (this.type instanceof QualifiedTypeReference && !(this.type instanceof ArrayQualifiedTypeReference)))
