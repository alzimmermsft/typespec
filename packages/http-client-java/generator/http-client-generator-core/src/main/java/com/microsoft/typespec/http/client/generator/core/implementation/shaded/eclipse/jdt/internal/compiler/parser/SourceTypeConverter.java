...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.IAnnotatable;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.ILocalVariable;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.env.ISourceImport;
...
//		long start = System.currentTimeMillis();
        ...
        try {
            ...
            /*
             * } finally {
             * System.out.println("Spent " + (System.currentTimeMillis() - start) + "ms to convert " + ((JavaElement)
             * converter.cu).toStringWithAncestors());
             */ }
    }

    public static CompilationUnitDeclaration buildModularCompilationUnit(IModule module,
        ProblemReporter problemReporter, CompilationResult compilationResult) {
    ...
    private CompilationUnitDeclaration convert(ISourceType[] sourceTypes, CompilationResult compilationResult)
        ...
        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.ICompilationUnit cuHandle
        ...
        if ((compilationUnitElementInfo.annotationNumber
        ...
        for (int i = 0; i < importCount; i++) {
            ...
            this.unit.imports[i]
                = createImportReference(Util.splitOn('.', nameWithoutStar, 0, nameWithoutStar.length()),
    ...
    private CompilationUnitDeclaration convert(IModule module, CompilationResult compilationResult)
        throws JavaModelException {
        this.unit = new CompilationUnitDeclaration(this.problemReporter, compilationResult, 0);
        // not filled at this point
        ...
        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.ICompilationUnit cuHandle
            = moduleInfo.getHandle().getCompilationUnit();
        this.cu = (ICompilationUnit) cuHandle;
        // always parse, because (a) dietParse is always sufficient, (b) we don't yet have the necessary conversion
        // methods for module directives
        return new Parser(this.problemReporter, true).dietParse(this.cu, compilationResult);
    }
     ...
     */
    private RecordComponent convertRecordComponent(SourceField componentHandle, TypeDeclaration type,
    ...
    private FieldDeclaration convert(SourceField fieldHandle, TypeDeclaration type, CompilationResult compilationResult)
        ...
        if (isEnumConstant) {
            field.modifiers = modifiers & ~ClassFileConstants.AccEnum; // clear AccEnum bit onto AST (binding will add
                                                                       // it)
        } else {
        ...
        if (methodInfo.isConstructor()) {
            ...
            if (methodInfo.isAnnotationMethod()) {
                ...
                boolean hasDefaultValue
                    = annotationMethodInfo.defaultValueStart != -1 || annotationMethodInfo.defaultValueEnd != -1;
                if ((this.flags & FIELD_INITIALIZATION) != 0) {
                    if (hasDefaultValue) {
    ...
    private TypeDeclaration convert(SourceType typeHandle, CompilationResult compilationResult)
        ...
        if ((TypeDeclaration.kind(typeInfo.getModifiers()) == TypeDeclaration.RECORD_DECL)) {
            ...
            for (int i = 0, length = recordComponents.length; i < length; i++) {
                type.recordComponents[i]
                    = convertRecordComponent((SourceField) recordComponents[i], type, compilationResult);
            }
        }
        ...
        if (typeInfo.getSuperclassName() != null) {
            type.superclass
                = createTypeReference(typeInfo.getSuperclassName(), start, end, true /* include generics */);
        }
        ...
        if (permittedSubtypeCount > 0) {
            ...
            for (int i = 0; i < permittedSubtypeCount; i++) {
                type.permittedTypes[i]
                    = createTypeReference(permittedSubtypeNames[i], start, end, true /* include generics */);
            }
        }
            ...
            if (!isAbstract) {
                ...
                for (int i = 0; i < sourceMethodCount; i++) {
                    if (sourceMethods[i].isConstructor()) {
                        if (needConstructor) {
                            extraConstructor = 0; // Does not need the extra constructor since one constructor already
                                                  // exists.
                            methodCount++;
                        }
                    } else if (needMethod) {
