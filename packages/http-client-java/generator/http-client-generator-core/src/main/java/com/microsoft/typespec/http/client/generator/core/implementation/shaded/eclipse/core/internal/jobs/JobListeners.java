...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.runtime.jobs.*;
    ...
    void waitAndSendEvents(InternalJob job, boolean shouldSend) {
        ...
        while (true) {
            ...
            if (eventQueueThread != null) {
                ...
                String msg
                    = "IJobChangeListener timeout detected. Further calls to IJobChangeListener may occur in random order and join(family) can return too soon. IJobChangeListener should return within " //$NON-NLS-1$
                        + timeout + " ms. IJobChangeListener methods should not block. Possible deadlock."; //$NON-NLS-1$
                MultiStatus status
                    = new MultiStatus(JobManager.PI_JOBS, JobManager.PLUGIN_ERROR, msg, new TimeoutException(msg));
                StringBuilder buf
                    = new StringBuilder("Thread that is running the IJobChangeListener: " + eventQueueThread.getName()); //$NON-NLS-1$
                ...
                Status child
                    = new Status(IStatus.ERROR, JobManager.PI_JOBS, JobManager.PLUGIN_ERROR, buf.toString(), null);
                status.add(child);
