...
public interface IDispatch extends IUnknown {
     ...
     *
     * @param pctinfo The number of type information interfaces provided by the object. If the object provides type
     * information, this number is 1; otherwise the number is 0.
     ...
     * interface.
     *
     * @param iTInfo The type information to return. Pass 0 to retrieve type information for the IDispatch
     * implementation.
     ...
     * be used
     * on subsequent calls to Invoke. The dispatch function DispGetIDsOfNames provides a standard implementation of
     * GetIDsOfNames.
     *
     ...
     * @param rgDispId Caller-allocated array, each element of which contains an identifier (ID) corresponding to one of
     * the names passed in
     * the rgszNames array. The first element represents the member name. The subsequent elements represent each of the
     * member's parameters.
     ...
     * standard implementation of Invoke.
     *
     * @param dispIdMember Identifies the member. Use GetIDsOfNames or the object's documentation to obtain the dispatch
     * identifier.
     * @param riid Reserved for future use. Must be IID_NULL.
     * @param lcid The locale context in which to interpret arguments. The lcid is used by the GetIDsOfNames function,
     * and is also
     ...
     * DISPATCH_METHOD
     * The member is invoked as a method. If a property has the same name, both this and the DISPATCH_PROPERTYGET flag
     * can be set.
     ...
     * The member is changed by a reference assignment, rather than a value assignment. This flag is valid only when the
     * property accepts a reference to an object.
     * @param pDispParams Pointer to a DISPPARAMS structure containing an array of arguments, an array of argument
     * DISPIDs for named arguments, and counts for the number of elements in the arrays.
     * @param pVarResult Pointer to the location where the result is to be stored, or NULL if the caller expects no
     * result. This argument is ignored if DISPATCH_PROPERTYPUT or DISPATCH_PROPERTYPUTREF is specified.
     * @param pExcepInfo Pointer to a structure that contains exception information. This structure should be filled in
     * if DISP_E_EXCEPTION is returned. Can be NULL.
     * @param puArgErr The index within rgvarg of the first argument that has an error. Arguments are stored in
     * pDispParams-&gt;rgvarg in reverse order,
     * so the first argument is the one with the highest index in the array. This parameter is returned only when the
     * resulting return
     * value is DISP_E_TYPEMISMATCH or DISP_E_PARAMNOTFOUND. This argument can be set to null. For details, see
     * Returning Errors.
     * @return This method can return one of these values.
     * S_OK
     * Success.
     * DISP_E_BADPARAMCOUNT
     * The number of elements provided to DISPPARAMS is different from the number of arguments accepted by the method or
     * property.
     * DISP_E_BADVARTYPE
     * One of the arguments in DISPPARAMS is not a valid variant type.
     * DISP_E_EXCEPTION
     * The application needs to raise an exception. In this case, the structure passed in pexcepinfo should be filled
     * in.
     ...
     * One of the parameter IDs does not correspond to a parameter on the method. In this case, puArgErr is set to the
     * first argument that contains the error.
     * DISP_E_TYPEMISMATCH
     * One or more of the arguments could not be coerced. The index of the first parameter with the incorrect type
     * within rgvarg is returned in puArgErr.
     * DISP_E_UNKNOWNINTERFACE
     * The interface identifier passed in riid is not IID_NULL.
     * DISP_E_UNKNOWNLCID
     * The member being invoked interprets string arguments according to the LCID, and the LCID is not recognized. If
     * the LCID is not needed to interpret arguments, this error should not be returned
