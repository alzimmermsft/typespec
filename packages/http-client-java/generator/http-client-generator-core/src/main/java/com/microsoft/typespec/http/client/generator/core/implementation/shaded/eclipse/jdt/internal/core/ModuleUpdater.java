...
public class ModuleUpdater {
    ...
    public void computeModuleUpdates(IClasspathEntry entry) throws JavaModelException {
        for (IClasspathAttribute attribute : entry.getExtraAttributes()) {
            ...
            String values = attribute.getValue(); // the attributes considered here may have multiple values separated
                                                  // by ':'
            if (attributeName.equals(IClasspathAttribute.ADD_EXPORTS)) {
                for (String value : values.split(":")) { // format: //$NON-NLS-1$
                    ...
                    if (slash != -1 && equals != -1) {
                        ...
                        addModuleUpdate(modName, new IUpdatableModule.AddExports(packName, targets),
                            UpdateKind.PACKAGE);
                    } else {
                ...
                addModuleUpdate(module.getElementName(), m -> m.setMainClassName(values.toCharArray()),
                    UpdateKind.MODULE);
            }
        }
    }
