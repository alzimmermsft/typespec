...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.util.Tuples.Pair;
import java.util.*;
...
class BoundSet {
    ...
    public void addBoundsFromTypeParameters(InferenceContext18 context, TypeVariableBinding[] typeParameters,
    ...
    public BoundSet copy() {
        ...
        if (this.incorporatedBounds.length > 0)
            System.arraycopy(this.incorporatedBounds, 0,
                copy.incorporatedBounds = new TypeBound[this.incorporatedBounds.length], 0,
                this.incorporatedBounds.length);
        if (this.unincorporatedBoundsCount > 0)
            System.arraycopy(this.unincorporatedBounds, 0,
                copy.unincorporatedBounds = new TypeBound[this.unincorporatedBounds.length], 0,
                this.unincorporatedBounds.length);
    ...
    public void addBound(TypeBound bound, LookupEnvironment environment) {
            ...
            if (bound.equals(this.mostRecentBounds[recent])) {
                if (environment.globalOptions.isAnnotationBasedNullAnalysisEnabled) {
                    ...
                    if (boundNullBits != existingNullBits) {
                        ...
                        else if (boundNullBits != 0) // combine bits from both sources, even if this creates a
                                                     // contradiction
                            existing.right = environment.createAnnotatedType(existing.right,
                                environment.nullAnnotationsFromTagBits(boundNullBits));
                    }
                }
                return;
            }
        ...
        if (three.addBound(bound)) {
            ...
            if (this.unincorporatedBoundsCount >= unincorporatedBoundsLength)
                System.arraycopy(this.unincorporatedBounds, 0,
                    this.unincorporatedBounds = new TypeBound[unincorporatedBoundsLength * 2], 0,
                    unincorporatedBoundsLength);
    ...
    public TypeBinding getInstantiation(InferenceVariable inferenceVariable, LookupEnvironment environment) {
        ...
        if (three != null) {
            ...
            if (environment != null
                ...
                && instantiation != null
                && (instantiation.tagBits & TagBits.AnnotationNullMASK) == 0)
    ...
    boolean incorporate(InferenceContext18 context) throws InferenceFailureException {
        ...
        try {
            do {
                ...
                System.arraycopy(this.unincorporatedBounds, 0,
                    freshBounds = new TypeBound[this.unincorporatedBoundsCount], 0, this.unincorporatedBoundsCount);
     ...
     */
    boolean incorporate(InferenceContext18 context, TypeBound[] first, TypeBound[] next)
        throws InferenceFailureException {
        boolean analyzeNull = context.environment.globalOptions.isAnnotationBasedNullAnalysisEnabled;
        ConstraintTypeFormula[] mostRecentFormulas = new ConstraintTypeFormula[4]; // poor man's cache to toss out
                                                                                   // duplicates, in pathological cases
                                                                                   // there are a good quarter million
                                                                                   // of them.
            ...
            for (TypeBound bound2 : next) {
                ...
                do {
                    ...
                    if (newConstraint != null) {
                        ...
                        if (analyzeNull) {
                            // not per JLS: if the new constraint relates types where at least one has a null
                            // annotations,
                            // record all null tagBits as hints for the final inference solution.
                            long nullHints = (newConstraint.left.tagBits | newConstraint.right.tagBits)
                                & TagBits.AnnotationNullMASK;
                            if (nullHints != 0) {
                                if (TypeBinding.equalsEquals(bound1.left, bound2.left)
                                    || (bound1.relation == ReductionResult.SAME
                                        && TypeBinding.equalsEquals(bound1.right, bound2.left))
                                    || (bound2.relation == ReductionResult.SAME
                                        && TypeBinding.equalsEquals(bound1.left, bound2.right))) {
                    ...
                    if (deriveTypeArgumentConstraints) {
                        for (ConstraintTypeFormula typeArgumentConstraint : deriveTypeArgumentConstraints(bound1,
                            bound2, context)) {
        ...
        for (Entry<ParameterizedTypeBinding, ParameterizedTypeBinding> capt : this.captures.entrySet()) {
            ...
            InferenceSubstitution theta
                = new InferenceSubstitution(context.environment, alphas, context.currentInvocation) {
                    @Override
            ...
            for (int i = 0, length = parameters.length; i < length; i++) {
                // A set of bounds on α1, ..., αn, constructed from the declared bounds of P1, ..., Pn as described in
                // 18.1.3, is immediately implied.
                            ...
                            while (it.hasNext()) {
                                ...
                                if (!(bound.right instanceof InferenceVariable)) {
                                    if (wildcardBinding.boundKind == Wildcard.SUPER)
                                        reduceOneConstraint(context,
                                            ConstraintTypeFormula.create(bound.right, t, ReductionResult.SUBTYPE));
                                    else
                                        return false;
                                }
                            }
            ...
            if (!this.captures.isEmpty()) {
                for (Entry<ParameterizedTypeBinding, ParameterizedTypeBinding> entry : this.captures.entrySet()) {
                    System.out.println("Dropping capture bound " + //$NON-NLS-1$
                        String.valueOf(entry.getKey().shortReadableName()) + "=capture(" //$NON-NLS-1$
                        + String.valueOf(entry.getKey().shortReadableName()) + ")");  //$NON-NLS-1$
                }
            }
        }
        this.captures.clear();
    ...
    void addTypeBoundsFromWildcardBound(InferenceContext18 context, InferenceSubstitution theta, int boundKind,
        TypeBinding t, TypeBinding r, TypeBinding bi) throws InferenceFailureException {
    ...
    private ConstraintTypeFormula combineSameSame(TypeBound boundS, TypeBound boundT,
        ...
        if (TypeBinding.equalsEquals(boundS.left, boundT.left))
            return ConstraintTypeFormula.create(boundS.right, boundT.right, ReductionResult.SAME,
                boundS.isSoft || boundT.isSoft);
    ...
    private ConstraintTypeFormula combineSameSameWithProperType(TypeBound boundLeft, TypeBound boundRight,
        ...
        if (enableOptimizationForBug543480 && isParameterizedDependency(boundRight)) {
            // Performance optimization: do not incorporate arguments one by one, which yielt 2^n new bounds (n=number
            // of type arguments) in the past.
            // Instead, all arguments of a parameterized dependency are incorporated at once - but only when they are
            // available.
        ...
        if (u.isProperType(true)) {
            ...
            TypeBinding left = boundRight.left; // no substitution since S inference variable and (S != α) per
                                                // precondition
            ...
            // but it creates regressions in GenericTypeTest (4), GenericsRegressionTest (1), GenericsRegressionTest_1_8
            // (1)
...
//				return null; // no new information
            return ConstraintTypeFormula.create(left, right, ReductionResult.SAME,
                boundLeft.isSoft || boundRight.isSoft);
        }
        return null;
    }
    ...
    private ConstraintTypeFormula combineSameSubSuperWithProperType(TypeBound boundLeft, TypeBound boundRight,
        ...
        if (u.isProperType(true)) {
            ...
            if (substitute) // avoid redundant constraint
                return ConstraintTypeFormula.create(left, right, boundRight.relation,
                    boundRight.isSoft || boundLeft.isSoft);
        }
        return null;
    }
    ...
    private ConstraintTypeFormula combineSuperAndSub(TypeBound boundS, TypeBound boundT) {
        ...
        if (TypeBinding.equalsEquals(alpha, boundT.left))
            // α >: S and α <: T imply ⟨S <: T⟩
            return ConstraintTypeFormula.create(boundS.right, boundT.right, ReductionResult.SUBTYPE,
                boundT.isSoft || boundS.isSoft);
        if (boundS.right instanceof InferenceVariable) {
            // try reverse:
            alpha = (InferenceVariable) boundS.right;
            if (TypeBinding.equalsEquals(alpha, boundT.right))
                // S :> α and T <: α imply ⟨S :> T⟩
                return ConstraintTypeFormula.create(boundS.left, boundT.left, ReductionResult.SUPERTYPE,
                    boundT.isSoft || boundS.isSoft);
        }
        return null;
    }
    ...
    private ConstraintTypeFormula combineEqualSupers(TypeBound boundS, TypeBound boundT) {
        ...
        if (TypeBinding.equalsEquals(boundS.left, boundT.right))
            ...
            return ConstraintTypeFormula.create(boundT.left, boundS.right, boundS.relation,
                boundT.isSoft || boundS.isSoft);
        if (TypeBinding.equalsEquals(boundS.right, boundT.left))
            // came in as: S REL α and α REL T imply ⟨S REL T⟩
            return ConstraintTypeFormula.create(boundS.left, boundT.right, boundS.relation,
                boundT.isSoft || boundS.isSoft);
        return null;
    }
    ...
    private Collection<TypeBound> getProperTypesForAllInferenceVariablesOrNull(
        ParameterizedTypeBinding parameterizedType, Map<InferenceVariable, TypeBound> properTypesByInferenceVariable) {
    ...
    private Map<InferenceVariable, TypeBound> properTypesByInferenceVariable(TypeBound[] firstBounds,
        TypeBound[] nextBounds) {
        return getBoundsStream(firstBounds, nextBounds).filter(bound -> bound.relation == ReductionResult.SAME)
    ...
    private ConstraintTypeFormula combineWithProperTypes(Collection<TypeBound> properTypesForAllInferenceVariables,
        TypeBound boundRight) {
    ...
    private List<ConstraintTypeFormula> deriveTypeArgumentConstraints(TypeBound boundS, TypeBound boundT,
        InferenceContext18 context) {
        /*
         * From 18.4:
         * If two bounds have the form α <: S and α <: T, and if for some generic class or interface, G,
         * there exists a supertype (4.10) of S of the form G<S1, ..., Sn> and a supertype of T of the form G<T1, ...,
         * Tn>,
        ...
        for (Pair<TypeBinding> pair : superPairs) {
            // future JLS should apply upwards projection according to
            // https://mail.openjdk.org/pipermail/compiler-dev/2024-May/026579.html
    ...
    private List<ConstraintTypeFormula> typeArgumentEqualityConstraints(TypeBinding g_s, TypeBinding g_t,
        boolean isSoft) {
        if (g_s == null
            || g_s.kind() != Binding.PARAMETERIZED_TYPE
            || g_t == null
            || g_t.kind() != Binding.PARAMETERIZED_TYPE)
            return Collections.emptyList();
    ...
    public boolean dependsOnResolutionOf(InferenceVariable alpha, InferenceVariable beta) {
        ...
        for (Entry<ParameterizedTypeBinding, ParameterizedTypeBinding> entry : this.captures.entrySet()) { // TODO:
                                                                                                           // optimization:
                                                                                                           // consider
                                                                                                           // separate
                                                                                                           // index
            ...
            for (int i = 0; i < g.arguments.length; i++) {
                if (TypeBinding.equalsEquals(g.arguments[i], alpha)) {
                    ...
                    // capture(G<...>)
                    // depends on the resolution of every other inference variable mentioned in this bound (on both
                    // sides of the = sign).
    ...
    List<Set<InferenceVariable>> computeConnectedComponents(InferenceVariable[] inferenceVariables) {
        ...
        for (int i = 0; i < inferenceVariables.length; i++) {
            ...
            allEdges.put(iv1, targetSet); // eventually ensures: forall iv in inferenceVariables : allEdges.get(iv) !=
                                          // null
            for (int j = 0; j < i; j++) {
    ...
    public boolean condition18_5_2_bullet_3_3_1(InferenceVariable alpha, TypeBinding targetType) {
        ...
        // S1 and S2 have supertypes (4.10) that are two different parameterizations of the same generic class or
        // interface.
        if (ts.superBounds != null) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                for (int j = i + 1; j < len; j++) {
                    ...
                    for (Pair<TypeBinding> pair : pairs) {
                        /*
                         * HashMap<K#8,V#9> and HashMap<K#8,ArrayList<T>> with an instantiation for V9 = ArrayList<T>
                         * already in the
                         * bound set should not be seen as two different parameterizations of the same generic class or
                         * interface.
                         * See https://bugs.eclipse.org/bugs/show_bug.cgi?id=432626 for a test that triggers this
                         * condition.
                         * See https://bugs.openjdk.java.net/browse/JDK-8056092: recommendation is to check for proper
                         * types.
                         */
                        if (pair.left().isProperType(true)
                            && pair.right().isProperType(true)
                            && !TypeBinding.equalsEquals(pair.left(), pair.right()))
                            return true;
                    }
                }
            }
    ...
    public boolean condition18_5_2_bullet_3_3_2(InferenceVariable alpha, TypeBinding targetType,
        InferenceContext18 ctx18) {
        // T is a parameterization of a generic class or interface, G, and
        // B2 contains a bound of one of the forms α = S or S <: α,
        // where there exists no type of the form G<...> that is a supertype of S, but the raw type G is a supertype of
        // S.
    ...
    private void allSuperPairsWithCommonGenericTypeRecursive(TypeBinding s, TypeBinding t,
        List<Pair<TypeBinding>> result, HashSet<Integer> visited) {
        ...
        if (s.isParameterizedType()) { // optimization here and below: clients of this method only want to compare type
                                       // arguments
    ...
    public TypeBinding condition18_5_5_item_4(ReferenceBinding rAlpha, InferenceVariable[] alpha, TypeBinding tPrime,
        InferenceContext18 ctx18) {
