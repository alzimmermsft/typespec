...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.runtime.content.*;
...
public final class ContentType implements IContentType, IContentTypeInfo {
    ...
    public static void log(String message, Throwable reason) {
        ...
        IStatus status = new Status(IStatus.ERROR, ContentMessages.OWNER_NAME, 0, message,
            reason instanceof CoreException ? null : reason);
    ...
    int describe(IContentDescriber selectedDescriber, ILazySource contents, ContentDescription description)
        throws IOException {
        try {
            return contents.isText()
                ? ((ITextContentDescriber) selectedDescriber).describe((Reader) contents, description)
    ...
    public IContentDescriber getDescriber() {
        try {
            ...
            final String describerValue
                = contentTypeElement != null ? contentTypeElement.getAttribute(DESCRIBER_ELEMENT) : null;
