...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.Invocation;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.util.HashtableOfType;
...
public class CompilationUnitScope extends Scope {
    ...
    private ImportBinding[] tempImports;	// to keep a record of resolved imports while traversing all in
                                        	// faultInImports()
    ...
    /**
     * Cache of interned inference variables. Access only via
     * {@link InferenceVariable#get(TypeBinding, int, InvocationSite, Scope, ReferenceBinding, boolean)}.
     */
    Map<InferenceVariable.InferenceVarKey, InferenceVariable> uniqueInferenceVariables = new HashMap<>();

    private RuntimeException deferredException; // enables deferring a CompletionNodeFound exception, not used during
                                                // normal compilation
    ...
    void buildTypeBindings(AccessRestriction accessRestriction) {
        ...
        if (this.referenceContext.compilationResult.compilationUnit != null) {
            ...
            if (expectedPackageName != null
                && !this.referenceContext.isModuleInfo()
        ...
        if (this.currentPackageName == CharOperation.NO_CHAR_CHAR) {
            ...
            if ((this.fPackage = this.environment.createPlainPackage(this.currentPackageName)) == null) {
                if (this.referenceContext.currentPackage != null) {
                    problemReporter().packageCollidesWithType(this.referenceContext); // only report when the unit has a
                                                                                      // package statement
                }
        ...
        nextType: for (int i = 0; i < typeLength; i++) {
            ...
            if (this.fPackage.hasType0Any(typeDecl.name)) {
                ...
                // it can be a NotFound problem type if its a secondary type referenced before its primary type found in
                // additional units
            ...
            if ((typeDecl.modifiers & ClassFileConstants.AccPublic) != 0) {
                char[] mainTypeName;
                if ((mainTypeName = this.referenceContext.getMainTypeName()) != null // mainTypeName == null means that
                                                                                     // implementor of ICompilationUnit
        ...
        if (types != null) {
            for (TypeDeclaration typeDecl : types) {
                if (this.fPackage != this.environment.defaultPackage
                    && this.fPackage.getPackage(typeDecl.name, module()) != null) {
                    // if a package exists, it must be a valid package - cannot be a NotFound problem package
                    // this is now a warning since a package does not really 'exist' until it contains a type, see JLS
                    // v2, 7.4.3
            ...
            if ((importReference.bits & ASTNode.OnDemand) != 0) {
                if (importReference.isStatic()) {
                    if (this.referenceContext.isSimpleCompilationUnit()
                        && CharOperation.equals(TypeConstants.JAVA_IO_IO, importReference.tokens)) {
                        numberOfImports--;
                    }
                } else {
    ...
    private int resolveImports(int numberOfStatements, ImportBinding[] resolvedImports, int index,
        Predicate<ImportReference> filter1, Predicate<ImportReference> filter2) {
    ...
    public char[] computeConstantPoolName(LocalTypeBinding localType) {
        ...
        // delegates to the outermost enclosing classfile, since it is the only one with a global vision of its
        // innertypes.
            ...
            if (localType.isMemberType()) {
                if (index == 0) {
                    ...
                    candidateName = CharOperation.concat(localType.enclosingType().constantPoolName(), '$',
                        String.valueOf(index).toCharArray(), '$', localType.sourceName);
                }
                ...
                candidateName = CharOperation.concat(CharOperation.concat(localType.enclosingType().constantPoolName(),
    ...
    void faultInImports() {
        ...
        nextImport: for (int i = 0; i < numberOfStatements; i++) {
            ...
            for (int j = 0; j < this.importPtr; j++) {
                ...
                if (resolved.onDemand == ((importReference.bits & ASTNode.OnDemand) != 0)
                    ...
                    if (CharOperation.equals(compoundName, resolved.compoundName)
                        && CharOperation.equals(importReference.getSimpleName(), resolved.getSimpleName())) {
            ...
            if ((importReference.modifiers & ClassFileConstants.AccModule) != 0) {
                problemReporter().validateJavaFeatureSupport(JavaFeature.MODULE_IMPORTS, importReference.sourceStart,
                    importReference.sourceEnd);
                if (!(JavaFeature.MODULE_IMPORTS.isSupported(compilerOptions().sourceLevel,
                ...
                if (importBinding instanceof PackageBinding) {
                    ...
                    if (uniquePackage instanceof SplitPackageBinding && !inJdtDebugCompileMode) {
                        ...
                        problemReporter().conflictingPackagesFromModules(splitPackage, module(),
                ...
                Binding importBinding = findSingleImport(compoundName, Binding.TYPE | Binding.FIELD | Binding.METHOD,
                ...
                if (importBinding instanceof SplitPackageBinding) {
                    ...
                    int sourceEnd
                        = (int) (importReference.sourcePositions[splitPackage.compoundName.length - 1] & 0xFFFF);
                    problemReporter().conflictingPackagesFromModules((SplitPackageBinding) importBinding, module(),
                        importReference.sourceStart, sourceEnd);
                    continue nextImport;
                }
                if (!importBinding.isValidBinding()) {
                    ...
                    if (importedPackage != null) {
                        ...
                        if (!inJdtDebugCompileMode) {
                            ...
                            if (importedPackage instanceof SplitPackageBinding) {
                                ...
                                int sourceEnd
                                    = (int) importReference.sourcePositions[splitPackage.compoundName.length - 1];
                                problemReporter().conflictingPackagesFromModules(splitPackage, module(),
                                    importReference.sourceStart, sourceEnd);
                                continue nextImport;
                            }
                ...
                if (importReference.isStatic()) {
                    ...
                    if (importBinding.kind() == Binding.FIELD) {
                        checkMoreStaticBindings(compoundName, typesBySimpleNames, Binding.TYPE | Binding.METHOD,
                            importReference);
        ...
        if (this.tempImports.length > this.importPtr)
            System.arraycopy(this.tempImports, 0, this.tempImports = new ImportBinding[this.importPtr], 0,
                this.importPtr);
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (!binding.onDemand && binding.getResolvedBindingKind() == Binding.TYPE
...
// this API is for code assist purpose
    ...
    private Binding findImport(char[][] compoundName, int length) {
        ...
        foundNothingOrType: if (binding != null) {
            ...
            while (i < length) {
                ...
                if (binding instanceof ReferenceBinding && binding.problemId() == ProblemReasons.NotAccessible) {
                    return this.environment.convertToRawType((TypeBinding) binding,
                        false /* do not force conversion of enclosing types */);
                }
                if (binding == null) {
                    break foundNothingOrType;
                } else if (!binding.isValidBinding()) {
                    if (binding.problemId() == ProblemReasons.Ambiguous
                        && packageBinding instanceof SplitPackageBinding)
                        return packageBinding; // pass the split package to the caller so they can report
                                               // conflictingPackagesFromModules()
                    binding = null;
                    break foundNothingOrType;
                }
                if (!(binding instanceof PackageBinding)) {
                    PackageBinding visibleFor = packageBinding.getVisibleFor(module, false); // filter out empty
                                                                                             // parent-packages
        ...
        while (i < length) {
            type = (ReferenceBinding) this.environment.convertToRawType(type,
                false /* do not force conversion of enclosing types */); // type imports are necessarily raw for all
                                                                         // except last
    ...
    private Binding findSingleStaticImport(char[][] compoundName, int mask) {
        ...
        if (binding instanceof PackageBinding) {
            ...
            if (temp != null && temp instanceof ReferenceBinding) // must resolve to a member type or field, not a top
                                                                  // level type
                return new ProblemReferenceBinding(compoundName, (ReferenceBinding) temp,
                    ProblemReasons.InvalidTypeForStaticImport);
        ...
        if (type.problemId() == ProblemReasons.NotVisible) // ensure compoundName is correct
            return new ProblemReferenceBinding(compoundName, ((ProblemReferenceBinding) type).closestMatch,
                ProblemReasons.NotVisible);
        return type;
    }
    ...
    ImportBinding[] getDefaultImports() {
        ...
        if (JavaFeature.IMPLICIT_CLASSES_AND_INSTANCE_MAIN_METHODS.isSupported(this.environment.globalOptions)
            ...
            if (module != null) {
                ...
                return javaIOImport != null
                    ? new ImportBinding[] { javaBase, javaIOImport }
        ...
        if (importBinding != null)
            importBinding
                = ((PackageBinding) importBinding).getTypeOrPackage(TypeConstants.JAVA_LANG[1], module(), false);
        ...
        if (importBinding == null || !importBinding.isValidBinding()) {
            ...
            problemReporter().isClassPathCorrect(TypeConstants.JAVA_LANG_OBJECT, this.referenceContext,
                this.environment.missingClassFileLocation, false,
                null/* resolving j.l.O is not specific to any referencing type */);
...
// NOT Public API
    public final Binding getImport(char[][] compoundName, boolean onDemand, int modifiers) {
        if (onDemand) {
            ...
            return findSingleImport(compoundName, Binding.TYPE | Binding.FIELD | Binding.METHOD,
    ...
    public void recordTypeConversion(TypeBinding superType, TypeBinding subType) {
        recordSuperTypeReference(subType); // must record the hierarchy of the subType that is converted to the
                                           // superType
    }
    ...
    Binding resolveSingleImport(ImportBinding importBinding, int mask) {
        ...
        if (resolvedBinding != null) {
            ...
            resolvedBinding = importBinding
                .setResolvedImport(findSingleImport(importBinding.compoundName, mask, importBinding.isStatic()));
        ...
        for (ReferenceBindingSetWrapper wrapper : this.referencedTypes) {
            ...
            if (!type.isLocalType())
                recordQualifiedReference(
                    type.isMemberType() ? CharOperation.splitOn('.', type.readableName()) : type.compoundName);
        }
    ...
    private void recordImportBinding(ImportBinding bindingToAdd) {
        if (this.tempImports.length == this.importPtr) {
            System.arraycopy(this.tempImports, 0, (this.tempImports = new ImportBinding[this.importPtr + 1]), 0,
                this.importPtr);
        }
     ...
     */
    private void checkMoreStaticBindings(char[][] compoundName, HashtableOfType typesBySimpleNames, int mask,
     ...
     */
    private int checkAndRecordImportBinding(Binding importBinding, HashtableOfType typesBySimpleNames,
        ImportReference importReference, char[][] compoundName) {
        ...
        // collisions between an imported static field & a type should be checked according to spec... but currently not
        // by javac
        ...
        if (importBinding instanceof ReferenceBinding || conflictingType != null) {
            ...
            if (existingType != null) {
                ...
                if (TypeBinding.equalsEquals(existingType, referenceBinding)) {
                    ...
                    for (int j = 0; j < this.importPtr; j++) {
                        ...
                        if (resolved instanceof ImportConflictBinding) {
                            ...
                            if (TypeBinding.equalsEquals(importConflictBinding.conflictingTypeBinding,
                                referenceBinding)) {
                                if (!importReference.isStatic()) {
                ...
                if (importReference.isStatic()
                    ...
                    // 7.5.3 says nothing about collision of single static imports and JDK8 tolerates them, though use
                    // is flagged.
                    for (int j = 0; j < this.importPtr; j++) {
                        ImportBinding resolved = this.tempImports[j];
                        ...
                        if (resolved.isStatic()
                            ...
                            if (CharOperation.equals(compoundName[compoundName.length - 1],
                                resolved.compoundName[resolved.compoundName.length - 1])) {
                                resolved.setResolvedImport(
                                    new ProblemReferenceBinding(new char[][] { name }, type, ProblemReasons.Ambiguous));
                                return -1;
                            }
                        }
                    }
                ...
                if (resolved.isStatic()
                    ...
                    if (CharOperation.equals(name, resolved.compoundName[resolved.compoundName.length - 1])) {
                        if (compilerOptions().sourceLevel >= ClassFileConstants.JDK1_8) {
                            ...
                            resolved.setResolvedImport(
                                new ProblemFieldBinding(field, field.declaringClass, name, ProblemReasons.Ambiguous));
                            return -1;
                        } else {
                            problemReporter().duplicateImport(importReference);
                            return -1;
