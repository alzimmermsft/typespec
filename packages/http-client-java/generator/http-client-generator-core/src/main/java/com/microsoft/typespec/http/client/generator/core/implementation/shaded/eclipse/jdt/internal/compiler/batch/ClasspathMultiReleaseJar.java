...
public class ClasspathMultiReleaseJar extends ClasspathJar {
    ...
    public ClasspathMultiReleaseJar(File file, boolean closeZipFileAtEnd, AccessRuleSet accessRuleSet,
        String destinationPath, String compliance) {
            ...
            if (this.releasePath != null && Files.exists(this.releasePath)) {
                ...
                try (DirectoryStream<Path> stream = Files.newDirectoryStream(this.releasePath)) {
                    for (final Path subdir : stream) {
                        Files.walkFileTree(subdir, new FileVisitor<Path>() {
                            @Override
    ...
    @Override
    public NameEnvironmentAnswer findClass(char[] binaryFileName, String qualifiedPackageName, String moduleName,
            ...
            try {
                ...
                if (reader != null) {
                    ...
                    searchPaths: if (this.annotationPaths != null) {
                        String qualifiedClassName = qualifiedBinaryFileName.substring(0,
                            qualifiedBinaryFileName.length() - SuffixConstants.EXTENSION_CLASS.length() - 1);
                        for (String annotationPath : this.annotationPaths) {
                            try {
                                if (this.annotationZipFile == null) {
                                    this.annotationZipFile
                                        = ExternalAnnotationDecorator.getAnnotationZipFile(annotationPath, null);
                                }
                                reader = ExternalAnnotationDecorator.create(reader, annotationPath, qualifiedClassName,
                                    this.annotationZipFile);

                                if (reader.getExternalAnnotationStatus()
                                    == ExternalAnnotationStatus.TYPE_IS_ANNOTATED) {
                                    break searchPaths;
                                }
                            } catch (IOException e) {
                        ...
                        // location is configured for external annotations, but no .eea found, decorate in order to
                        // answer NO_EEA_FILE:
