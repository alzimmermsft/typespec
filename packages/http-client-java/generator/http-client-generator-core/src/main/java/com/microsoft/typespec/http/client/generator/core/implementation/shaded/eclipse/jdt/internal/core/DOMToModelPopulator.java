...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.ImportDeclaration;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.Initializer;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.LambdaExpression;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.MemberValuePair;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.PackageDeclaration;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.core.ModuleDescriptionInfo.ModuleReferenceInfo;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.core.ModuleDescriptionInfo.PackageExportInfo;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.core.ModuleDescriptionInfo.ServiceInfo;
...
public class DOMToModelPopulator extends ASTVisitor {
    ...
    public DOMToModelPopulator(Map<IJavaElement, IElementInfo> newElements, CompilationUnit root,
    ...
    private void addAsChild(JavaElementInfo parentInfo, IJavaElement childElement) {
        if (childElement instanceof SourceRefElement element) {
            ...
            if (childElement instanceof SourceType anonymousType && anonymousType.isAnonymous()) {
                ...
                if (parent instanceof SourceType nestType) {
                    anonymousType.localOccurrenceCount = this.nestedTypesCount.compute(nestType,
                        (nest, currentCount) -> currentCount == null ? 1 : currentCount + 1); // occurrences count are
                                                                                              // 1-based
                }
            }
        }
        ...
        if (parentInfo instanceof ImportContainerInfo current
            && childElement instanceof org.eclipse.jdt.internal.core.ImportDeclaration importDecl) {
        ...
        if (parentInfo instanceof SourceMethodElementInfo method // also matches constructor
            ...
            ILocalVariable[] parameters = method.arguments != null
                ? Arrays.copyOf(method.arguments, method.arguments.length + 1)
                : new ILocalVariable[1];
    ...
    public boolean visit(
        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.CompilationUnit node) {
    ...
    public boolean visit(PackageDeclaration node) {
        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.core.PackageDeclaration newElement
            = new org.eclipse.jdt.internal.core.PackageDeclaration(this.root, node.getName().toString());
    ...
    public boolean visit(ImportDeclaration node) {
        ...
        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.core.ImportDeclaration newElement
            = new org.eclipse.jdt.internal.core.ImportDeclaration(this.importContainer, node.getName().toString(),
                node.isOnDemand());
        this.elements.push(newElement);
        ...
        if (node.isOnDemand()) {
            ...
            do {
                ...
                int newEnd = comments.stream()
                    .filter(comment -> comment.getStartPosition() <= currentEnd
                        && comment.getStartPosition() + comment.getLength() >= currentEnd)
                    .findAny()
    ...
    public boolean visit(ImplicitTypeDeclaration node) {
        SourceType newElement = new SourceType(this.elements.peek(), this.root.getElementName().endsWith(".java") //$NON-NLS-1$
            ? this.root.getElementName().substring(0, this.root.getElementName().length() - 5)
            : this.root.getElementName());
        this.elements.push(newElement);
        addAsChild(this.infos.peek(), newElement);
    ...
    public boolean visit(TypeDeclaration node) {
        ...
        if (node.getAST().apiLevel() > 2) {
            ((List<com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.TypeParameter>) node
                .typeParameters())
                    .stream()
                    .map(
                        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.TypeParameter::getName)
        ...
        if (node.getAST().apiLevel() > 2) {
            char[][] superInterfaces = ((List<Type>) node.superInterfaceTypes()).stream()
                .map(Type::toString)
        ...
        if (node.getAST().apiLevel() >= AST.JLS17) {
            char[][] permitted = ((List<Type>) node.permittedTypes()).stream()
                .map(Type::toString)
                .map(String::toCharArray)
                .toArray(char[][]::new);
        ...
        setSourceRange(newInfo, node);
        newInfo
            .setFlags(toModelFlags(node.getModifiers(), isDeprecated) | (node.isInterface() ? Flags.AccInterface : 0));
    ...
    public void endVisit(TypeDeclaration decl) {
        ...
        if (decl.getAST().apiLevel() > 2) {
            ((List<com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.TypeParameter>) decl
                .typeParameters())
                    .stream()
                    .map(
                        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.TypeParameter::getName)
    ...
    public boolean visit(EnumDeclaration node) {
        ...
        newInfo.setNameSourceStart(node.getName().getStartPosition());
        newInfo.setNameSourceEnd(node.getName().getStartPosition() + node.getName().getLength() - 1);
        newInfo.setSuperInterfaceNames(((List<Type>) node.superInterfaceTypes()).stream()
            .map(Type::toString)
            .map(String::toCharArray)
            .toArray(char[][]::new));
        this.infos.push(newInfo);
    ...
    public boolean visit(MethodDeclaration method) {
        if (method.getAST().apiLevel() > 2) {
            ((List<com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.TypeParameter>) method
                .typeParameters())
                    .stream()
                    .map(
                        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.TypeParameter::getName)
        ...
        SourceMethod newElement = new SourceMethod(this.elements.peek(), method.getName().getIdentifier(),
            parameters.stream().map(this::createSignature).toArray(String[]::new));
        this.elements.push(newElement);
        addAsChild(this.infos.peek(), newElement);
        ...
        info.setArgumentNames(
            parameters.stream().map(param -> param.getName().toString().toCharArray()).toArray(char[][]::new));
        if (method.getAST().apiLevel() > 2) {
        ...
        if (method.getAST().apiLevel() >= AST.JLS8) {
            info.setExceptionTypeNames(((List<Type>) method.thrownExceptionTypes()).stream()
                .map(Type::toString)
                .map(String::toCharArray)
                .toArray(char[][]::new));
        }
        ...
        info.setFlags(toModelFlags(method.getModifiers(), isDeprecated) | ((method.getAST().apiLevel() > AST.JLS2
            && ((List<SingleVariableDeclaration>) method.parameters()).stream()
                .anyMatch(SingleVariableDeclaration::isVarargs)) ? Flags.AccVarargs : 0));
        ...
        if (method.getAST().apiLevel() >= AST.JLS16 && method.isCompactConstructor()) {
            info.arguments = parameters.stream()
                .map(param -> toLocalVariable(param, newElement, true))
                .toArray(ILocalVariable[]::new);
        }
        this.infos.push(info);
        this.toPopulate.put(newElement, info);
        return true;
    ...
    public boolean visit(AnnotationTypeMemberDeclaration method) {
        SourceMethod newElement
            = new SourceMethod(this.elements.peek(), method.getName().getIdentifier(), new String[0]);
        this.elements.push(newElement);
        addAsChild(this.infos.peek(), newElement);
        ...
        if (defaultExpr != null) {
            ...
            com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.core.MemberValuePair mvp
                = new org.eclipse.jdt.internal.core.MemberValuePair(newElement.getElementName(), value.getKey(),
                    value.getValue());
    ...
    @Override
    public boolean visit(
        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.TypeParameter node) {
        ...
        info.bounds = ((List<Type>) node.typeBounds()).stream()
            .map(Type::toString)
            .map(String::toCharArray)
            .toArray(char[][]::new);
        info.boundsSignatures = ((List<Type>) node.typeBounds()).stream()
    ...
    public void endVisit(
        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.TypeParameter typeParam) {
        this.elements.pop();
        this.infos.pop();
    }
    ...
    public boolean visit(NormalAnnotation node) {
        ...
        newInfo.members = ((List<MemberValuePair>) node.values()).stream().map(domMemberValuePair -> {
            Entry<Object, Integer> value = memberValue(domMemberValuePair.getValue());
            return new org.eclipse.jdt.internal.core.MemberValuePair(domMemberValuePair.getName().toString(),
    ...
    public boolean visit(SingleMemberAnnotation node) {
        ...
        newInfo.members = new IMemberValuePair[] {
            new org.eclipse.jdt.internal.core.MemberValuePair("value", value.getKey(), value.getValue()) }; //$NON-NLS-1$
        this.infos.push(newInfo);
        this.toPopulate.put(newElement, newInfo);
        return true;
    }
    ...
    public boolean visit(AnonymousClassDeclaration decl) {
        ...
        if (decl.getParent()instanceof EnumConstantDeclaration enumConstantDeclaration) {
            ...
            newInfo.setNameSourceEnd(enumConstantDeclaration.getName().getStartPosition()
                + enumConstantDeclaration.getName().getLength() - 1);
        } else if (decl.getParent()instanceof ClassInstanceCreation constructorInvocation) {
            if (constructorInvocation.getAST().apiLevel() > 2) {
                ((List<SimpleType>) constructorInvocation.typeArguments()).stream()
                ...
                newInfo.setNameSourceEnd(constructorInvocation.getType().getStartPosition()
                    + constructorInvocation.getType().getLength() - 1);
            }
        }
        this.infos.push(newInfo);
        this.toPopulate.put(newElement, newInfo);
    ...
    public Entry<Object, Integer> memberValue(Expression dom) {
        if (dom == null
            ...
            || (dom instanceof SimpleName name && ("MISSING".equals(name.getIdentifier()) || //$NON-NLS-1$ // better
        ...
        if (dom instanceof ArrayInitializer arrayInitializer) {
            ...
            return new SimpleEntry<>(values.stream().map(Entry::getKey).toArray(),
                types.size() == 1 ? types.get(0) : IMemberValuePair.K_UNKNOWN);
        }
        if (dom instanceof NumberLiteral number) {
            ...
            if (value instanceof String valueString) {
                ...
                switch (type) {
                    ...
                    default:
                        throw new IllegalArgumentException("Type not (yet?) supported"); //$NON-NLS-1$
                }
            }
    ...
    private int toAnnotationValuePairType(String token) {
        ...
        try {
            ...
            return switch (tokenType) {
                ...
                case TokenNameMINUS -> switch (scanner.getNextToken()) {
                        case TokenNameDoubleLiteral -> IMemberValuePair.K_DOUBLE;
                        case TokenNameIntegerLiteral -> IMemberValuePair.K_INT;
                        case TokenNameFloatingPointLiteral -> IMemberValuePair.K_FLOAT;
                        case TokenNameLongLiteral -> IMemberValuePair.K_LONG;
    ...
    private Annotation toModelAnnotation(
        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.Annotation domAnnotation,
        JavaElement parent) {
    ...
    public static LocalVariable toLocalVariable(VariableDeclarationFragment fragment, JavaElement parent) {
        if (fragment.getParent()instanceof VariableDeclarationStatement variableDeclaration) {
            return new LocalVariable(parent, fragment.getName().getIdentifier(), variableDeclaration.getStartPosition(),
                ...
                Util.getSignature(variableDeclaration.getType()), null, // I don't think we need this, also it's the
                                                                        // ECJ's annotation node
                toModelFlags(variableDeclaration.getModifiers(), false), false);
    ...
    private static LocalVariable toLocalVariable(SingleVariableDeclaration parameter, JavaElement parent,
        boolean isParameter) {
        return new LocalVariable(parent, parameter.getName().getIdentifier(),
            getStartConsideringLeadingComments(parameter), parameter.getStartPosition() + parameter.getLength() - 1,
            parameter.getName().getStartPosition(),
            parameter.getName().getStartPosition() + parameter.getName().getLength() - 1,
            Util.getSignature(parameter.getType()), null, // should be populated while navigating children
            toModelFlags(parameter.getModifiers(), false), isParameter);
    }
    ...
    public boolean visit(FieldDeclaration field) {
        ...
        for (VariableDeclarationFragment fragment : (Collection<VariableDeclarationFragment>) field.fragments()) {
            ...
            if (((field.getParent()instanceof TypeDeclaration type && type.isInterface())
                || Flags.isFinal(field.getModifiers())) && initializer != null && initializer.getStartPosition() >= 0) {
                info.initializationSource = Arrays.copyOfRange(this.root.getContents(), initializer.getStartPosition(),
            ...
            if (field.getAST().apiLevel() >= AST.JLS3) {
                ...
                if (modifiers != null) {
                    modifiers.stream()
                        .filter(
                            com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.Annotation.class::isInstance)
                        .map(
                            com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.Annotation.class::cast)
                        .forEach(annotation -> annotation.accept(this)); // force processing of annotation on each
                                                                         // fragment
                }
            }
        }
    ...
    public boolean visit(Initializer node) {
        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.core.Initializer newElement
            = new org.eclipse.jdt.internal.core.Initializer(this.elements.peek(), 1);
        this.elements.push(newElement);
        addAsChild(this.infos.peek(), newElement);
    ...
    public boolean visit(ModuleDeclaration node) {
        ...
        newInfo.setNameSourceEnd(node.getName().getStartPosition() + node.getName().getLength() - 1);
        setSourceRange(newInfo, node);
        newInfo.setFlags((hasDeprecatedComment(node.getJavadoc()) || hasDeprecatedAnnotation(node.annotations()))
            ? Flags.AccDeprecated
            : 0);
        ...
        try {
            if (this.root.getPackageFragmentRoot().getResolvedClasspathEntry().getEntryKind()
    ...
    private ModuleReferenceInfo toModuleReferenceInfo(RequiresDirective node) {
        ...
        res.modifiers = (ModuleModifier.isTransitive(node.getModifiers()) ? ClassFileConstants.ACC_TRANSITIVE : 0)
    ...
    private PackageExportInfo toPackageExportInfo(ModulePackageAccess node) {
        ...
        res.target = modules == null || modules.isEmpty()
            ? null
    ...
    private ServiceInfo toServiceInfo(ProvidesDirective node) {
        ...
        res.implNames = ((List<Name>) node.implementations()).stream()
            .map(Name::toString)
            .map(String::toCharArray)
            .toArray(char[][]::new);
        setSourceRange(res, node);
    ...
    private boolean hasDeprecatedAnnotation(List<IExtendedModifier> modifiers) {
        return modifiers != null
            && modifiers.stream() //
                .anyMatch(modifier -> modifier instanceof org.eclipse.jdt.core.dom.Annotation annotation
                    && (Deprecated.class.getName().equals(annotation.getTypeName().toString())
                        || (Deprecated.class.getSimpleName().equals(annotation.getTypeName().toString())
                            && !hasAlternativeDeprecated())));
    }
        ...
        if (this.importContainer != null) {
            try {
                ...
                for (IJavaElement child : importElements) {
                    ...
                    if (!importDeclaration.isOnDemand() && importDeclaration.getElementName().endsWith("Deprecated")) { //$NON-NLS-1$
    ...
    private static int toModelFlags(int domModifiers, boolean isDeprecated) {
        ...
        if (Modifier.isStrictfp(domModifiers))
            res |= Flags.AccStrictfp;
        if (Modifier.isSynchronized(domModifiers))
            res |= Flags.AccSynchronized;
        if (Modifier.isTransient(domModifiers))
            res |= Flags.AccTransient;
        if (Modifier.isVolatile(domModifiers))
            res |= Flags.AccVolatile;
        if (isDeprecated)
            res |= Flags.AccDeprecated;
        return res;
    }
    ...
    private Javadoc javadoc(ASTNode node) {
        ...
        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.CompilationUnit unit
            = domUnit(node);
        ...
        if (commentIndex >= 0) {
            for (int i = commentIndex; i < unit.getCommentList().size(); i++) {
                ...
                if (comment instanceof Javadoc javadoc && javadoc.getStartPosition() <= node.getStartPosition()) {
                    return javadoc;
                }
            }
        }
