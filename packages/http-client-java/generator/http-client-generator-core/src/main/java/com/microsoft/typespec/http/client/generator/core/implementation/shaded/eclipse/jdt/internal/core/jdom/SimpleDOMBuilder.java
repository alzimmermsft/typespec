...
public class SimpleDOMBuilder extends AbstractDOMBuilder implements ISourceElementRequestor {
    ...
    protected void enterAbstractMethod(MethodInfo methodInfo) {
        ...
        this.fNode = new DOMMethod(this.fDocument, sourceRange, CharOperation.charToString(methodInfo.name), nameRange,
            methodInfo.modifiers, methodInfo.isConstructor, CharOperation.charToString(methodInfo.returnType),
    ...
    public void enterConstructor(MethodInfo methodInfo) {
        /* see 1FVIIQZ */
        String nameString = new String(this.fDocument, methodInfo.nameSourceStart,
            methodInfo.nameSourceEnd - methodInfo.nameSourceStart);
    ...
    public void enterType(TypeInfo typeInfo) {
        if (this.fBuildingType) {
            ...
            this.fNode = new DOMType(this.fDocument, sourceRange, new String(typeInfo.name), nameRange,
                typeInfo.modifiers, CharOperation.charArrayToStringArray(typeInfo.superinterfaces),
                TypeDeclaration.kind(typeInfo.modifiers) == TypeDeclaration.CLASS_DECL); // TODO (jerome) should pass in
                                                                                         // kind
            addChild(this.fNode);
            this.fStack.push(this.fNode);
    ...
    protected SourceElementParser getParser(Map<String, String> settings) {
        return new SourceElementParser(this, new DefaultProblemFactory(), new CompilerOptions(settings),
            false/* don't report local declarations */, true/* optimize string literals */);
    }
