...
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.util.Util.*;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.SignatureWrapper;
...
public class ExternalAnnotationProvider {
    ...
    void initialize(InputStream input) throws IOException {
        try (LineNumberReader reader = new LineNumberReader(new InputStreamReader(input))) {
            ...
            if (line.startsWith(TYPE_PARAMETER_PREFIX)) {
                if ((line = reader.readLine()) == null) // skip first line, second line may contain type parameter
                                                        // annotations
                    return;
                if (line.startsWith(TYPE_PARAMETER_PREFIX)) {
            ...
            do {
                ...
                if (!Character.isJavaIdentifierStart(selector.charAt(0))
                    && !new String(TypeConstants.INIT).equals(trimTail(selector))) {
                    throw new IOException("Illegal selector in external annotation file for " + this.typeName //$NON-NLS-1$
                        + " at line " + reader.getLineNumber() + ": \"" + selector + '"');  //$NON-NLS-1$ //$NON-NLS-2$
                }
                ...
                if (rawSig == null || annotSig == null || !errDetail.isEmpty()) {
                    if (errLine == -1)
                        errLine = reader.getLineNumber();
                    throw new IOException("Illegal format in external annotation file for " + this.typeName //$NON-NLS-1$
                        + " at line " + errLine + errDetail);  //$NON-NLS-1$
                }
    ...
    private boolean isValidTypeSignature(char[] typeSignature) {
        // simplified variant of org.eclipse.jdt.core.Signature.getTypeSignatureKind(char[]) -- which is inaccessible
        // here
    ...
    public static void assertClassHeader(String line, String typeName) throws IOException {
        ...
        if (!trimTail(line).equals(typeName)) {
            throw new IOException(
                "mismatching class name in annotation file, expected " + typeName + ", but header said " + line); //$NON-NLS-1$ //$NON-NLS-2$
        }
    }
    ...
    public ITypeAnnotationWalker forField(char[] selector, char[] signature, LookupEnvironment environment) {
        if (this.fieldAnnotationSources != null) {
            String source
                = this.fieldAnnotationSources.get(String.valueOf(CharOperation.concat(selector, signature, ':')));
            if (source != null)
    ...
    void initAnnotations(final LookupEnvironment environment) {
        if (this.NULLABLE_ANNOTATION == null) {
            this.NULLABLE_ANNOTATION = new SingleMarkerAnnotation() {
                ...
                public char[] getTypeName() {
                    return getBinaryTypeName(environment.getNullableAnnotationName());
                }
            };
        }
        if (this.NONNULL_ANNOTATION == null) {
            this.NONNULL_ANNOTATION = new SingleMarkerAnnotation() {
                @Override
                public char[] getTypeName() {
                    return getBinaryTypeName(environment.getNonNullAnnotationName());
                }
            };
        }
    }
        ...
        public ITypeAnnotationWalker toTypeParameter(boolean isClassTypeParameter, int rank) {
            ...
            if (source != null) {
                if (this.typeParametersWalker == null)
                    this.typeParametersWalker
                        = new TypeParametersAnnotationWalker(source.toCharArray(), 0, 0, null, this.environment);
    ...
    public class TypeParametersAnnotationWalker extends BasicAnnotationWalker {
        ...
        TypeParametersAnnotationWalker(char[] source, int pos, int rank, int[] rankStarts,
            LookupEnvironment environment) {
        ...
        public ITypeAnnotationWalker toTypeParameter(boolean isClassTypeParameter, int rank) {
            ...
            if (rank < this.rankStarts.length)
                return new TypeParametersAnnotationWalker(this.source, this.rankStarts[rank], rank, this.rankStarts,
                    this.environment);
            return ITypeAnnotationWalker.EMPTY_ANNOTATION_WALKER;
        }
        ...
        @Override
        public ITypeAnnotationWalker toTypeParameterBounds(boolean isClassTypeParameter, int parameterRank) {
            return new TypeParametersAnnotationWalker(this.source, this.rankStarts[parameterRank], parameterRank,
                this.rankStarts, this.environment);
        }
            ...
            if (this.source[0] == '<') {
                if (this.typeParametersWalker == null)
                    return this.typeParametersWalker
                        = new TypeParametersAnnotationWalker(this.source, this.pos + 1, rank, null, this.environment);
                return this.typeParametersWalker.toTypeParameter(isClassTypeParameter, rank);
            }
            return ITypeAnnotationWalker.EMPTY_ANNOTATION_WALKER;
        }
