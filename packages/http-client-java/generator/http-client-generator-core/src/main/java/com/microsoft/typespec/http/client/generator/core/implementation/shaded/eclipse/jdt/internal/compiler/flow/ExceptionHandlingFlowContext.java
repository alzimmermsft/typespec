...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.StatementWithFinallyBlock;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.TryStatement;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.CatchParameterBinding;
...
public class ExceptionHandlingFlowContext extends FlowContext {
    ...
    int[] isReached;
    int[] isNeeded;
    // WARNING: This is an array that maps to catch blocks, not caught exceptions (which could be more than catch blocks
    // in a multi-catch block)
    ...
    public ExceptionHandlingFlowContext(FlowContext parent, ASTNode associatedNode,
        ReferenceBinding[] handledExceptions, FlowContext initializationParent, BlockScope scope,
        UnconditionalFlowInfo flowInfo) {
        this(parent, associatedNode, handledExceptions, null, ASTNode.NO_ARGUMENTS, initializationParent, scope,
            flowInfo);
    }
    ...
    public ExceptionHandlingFlowContext(FlowContext parent, TryStatement tryStatement,
        ...
        this(parent, tryStatement, handledExceptions, exceptionToCatchBlockMap, tryStatement.catchArguments,
            initializationParent, scope, flowInfo.unconditionalInits());
    ...
    ExceptionHandlingFlowContext(FlowContext parent, ASTNode associatedNode, ReferenceBinding[] handledExceptions,
        int[] exceptionToCatchBlockMap, Argument[] catchArguments, FlowContext initializationParent, BlockScope scope,
        UnconditionalFlowInfo flowInfo) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (handledException.isUncheckedException(true)) {
                if (markExceptionsAndThrowableAsReached
                    ...
                    this.isReached[i / ExceptionHandlingFlowContext.BitCacheSize]
    ...
    public void complainIfUnusedExceptionHandlers(AbstractMethodDeclaration method) {
        ...
        if ((method.binding.modifiers & (ExtraCompilerModifiers.AccOverriding | ExtraCompilerModifiers.AccImplementing))
            != 0
        ...
        nextHandledException: for (ReferenceBinding handledException : this.handledExceptions) {
            ...
            if ((this.isReached[index / ExceptionHandlingFlowContext.BitCacheSize]
                & 1 << (index % ExceptionHandlingFlowContext.BitCacheSize)) == 0) {
                ...
                scope.problemReporter()
                    .unusedDeclaredThrownException(this.handledExceptions[index], method,
    ...
    public void complainIfUnusedExceptionHandlers(BlockScope scope, TryStatement tryStatement) {
        ...
        for (int index = 0, count = this.handledExceptions.length; index < count; index++) {
            ...
            if ((this.isReached[cacheIndex] & bitMask) == 0) {
                scope.problemReporter().unreachableCatchBlock(this.handledExceptions[index], getExceptionType(index));
            } else {
                if ((this.isNeeded[cacheIndex] & bitMask) == 0) {
                    scope.problemReporter().hiddenCatchBlock(this.handledExceptions[index], getExceptionType(index));
                }
            }
        }
    }
...
// applied for multi catch
    ...
    public void recordHandlingException(ReferenceBinding exceptionType, UnconditionalFlowInfo flowInfo,
        ...
        this.initsOnExceptions[catchBlock] = (this.initsOnExceptions[catchBlock].tagBits & FlowInfo.UNREACHABLE) == 0
    ...
    public StatementWithFinallyBlock statementWithFinallyBlock() {
        if (this.associatedNode instanceof StatementWithFinallyBlock) {
            // exception handler context may be child of InsideStatementWithFinallyBlockFlowContext, which maps to same
            // handler
