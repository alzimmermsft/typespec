...
public class TypeBound extends ReductionResult {
    ...
    static TypeBound createBoundOrDependency(InferenceSubstitution theta, TypeBinding type,
    ...
    TypeBound(InferenceVariable inferenceVariable, TypeBinding typeBinding, int relation, boolean isSoft) {
        ...
        if (((inferenceVariable.tagBits | this.right.tagBits) & TagBits.AnnotationNullMASK) != 0) {
            if ((inferenceVariable.tagBits & TagBits.AnnotationNullMASK)
                == (this.right.tagBits & TagBits.AnnotationNullMASK)) {
                ...
                switch (relation) {
                    case SAME:
                        mask = TagBits.AnnotationNullMASK;
                        break;

                    case SUBTYPE:
                        mask = TagBits.AnnotationNonNull;
                        break;  // sub of @Nullable is irrelevant

                    case SUPERTYPE:
                        mask = TagBits.AnnotationNullable;
                        break; // super of @NonNull is irrelevant
                }
    ...
    public boolean equals(Object obj) {
        if (obj instanceof TypeBound) {
            ...
            return (this.relation == other.relation)
                && TypeBinding.equalsEquals(this.left, other.left)
                && TypeBinding.equalsEquals(this.right, other.right);
        }
        return false;
    }
