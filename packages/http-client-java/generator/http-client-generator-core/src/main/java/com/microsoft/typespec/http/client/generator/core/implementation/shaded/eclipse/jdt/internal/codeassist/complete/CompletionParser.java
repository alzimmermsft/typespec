...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.codeassist.impl.AssistParser;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.codeassist.impl.RestrictedIdentifiers;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.*;
...
public class CompletionParser extends AssistParser {
            ...
            if (this.currentElement instanceof RecoveredUnit) {
                ...
                if (recoveredType.foundOpeningBrace || isAtRecordType) {
                    ...
                    if (orphan instanceof TypeReference) {
                        ...
                        CompletionOnFieldType fieldDeclaration
                            = new CompletionOnFieldType(fieldType, !recoveredType.foundOpeningBrace // mark as a local
                                                                                                    // variable
                            );
                        ...
                        if (!isAtRecordType
                            && this.intPtr >= 2
                            && this.intStack[this.intPtr - 1] == this.lastModifiersStart
                ...
                if (method != null) {
                    ...
                    if ((methodDecl.bodyStart == methodDecl.sourceEnd + 1) // was missing opening brace
                        && (Util.getLineNumber(orphan.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr)
                            == Util.getLineNumber(methodDecl.sourceEnd, this.scanner.lineEnds, 0,
                ...
                if (expression == this.assistNode) {
                    if (this.topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_MEMBER_VALUE_ARRAY_INITIALIZER) {
                        ...
                        this.assistNodeParent
                            = new AssistNodeParentAnnotationArrayInitializer(typeReference, memberValueName);
                    } else if (this.topKnownElementKind(COMPLETION_OR_ASSIST_PARSER)
                        ...
                        if ((index = lastIndexOfElement(K_ATTRIBUTE_VALUE_DELIMITER)) != -1) {
                            ...
                            MemberValuePair memberValuePair
                                = new MemberValuePair(this.identifierStack[this.identifierPtr--],
            ...
            if (this.currentElement instanceof RecoveredType) {
                ...
                if (recoveredType.foundOpeningBrace && this.genericsPtr > -1) {
                    if (this.genericsStack[this.genericsPtr] instanceof TypeParameter) {
                        ...
                        if (detector.containsCompletionNode()) {
                            this.currentElement.add(new CompletionOnMethodTypeParameter(
                                new TypeParameter[] { typeParameter }, this.compilationUnit.compilationResult()), 0);
                        }
                        return;
                    }
                }
        ...
        if (this.genericsPtr > -1) {
            ...
            if (detector.containsCompletionNode()) {
                ...
                if (method != null) {
                    AbstractMethodDeclaration methodDecl = method.methodDeclaration;
                    if ((methodDecl.bodyStart == methodDecl.sourceEnd + 1) // was missing opening brace
                        && (Util.getLineNumber(node.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr) == Util
                            .getLineNumber(methodDecl.sourceEnd, this.scanner.lineEnds, 0, this.scanner.linePtr))) {
                        return;
                    }
                }
                if (node == this.assistNode) {
                ...
                if (expression == this.assistNode
                    || (expression instanceof Assignment	// https://bugs.eclipse.org/bugs/show_bug.cgi?id=287939
                        ...
                        && ((this.expressionPtr > 0
                            ...
                            // In case of error in compilation unit, expression stack might not have instanceof exp, so
                            // try elementObjectInfoStack
                            || (this.elementPtr >= 0
    ...
    public Object becomeSimpleParser() {
        ...
        SavedState parserState
            = new SavedState(this.cursorLocation, completionScanner.cursorLocation, this.assistNodeParent);
    ...
    private void buildMoreAnnotationCompletionContext(MemberValuePair memberValuePair) {
        ...
        if (memberValuePair instanceof CompletionOnMemberValueName) {
            ...
            if (this.astLengthPtr > -1 && (length = this.astLengthStack[this.astLengthPtr--]) > nodesToRemove) {
                if (this.astStack[this.astPtr] instanceof MemberValuePair) {
                    System.arraycopy(this.astStack, (this.astPtr -= length) + 1,
                        memberValuePairs = new MemberValuePair[length - nodesToRemove + 1], 0, length - nodesToRemove);
                }
    ...
    private void buildMoreCompletionContext(Expression expression) {
            ...
            nextElement: switch (kind) {
                case K_SELECTOR_QUALIFIER:
                    ...
                    if (selector == THIS_CONSTRUCTOR || selector == SUPER_CONSTRUCTOR) {
                        ExplicitConstructorCall call = new ExplicitConstructorCall((selector == THIS_CONSTRUCTOR)
                ...
                case K_INSIDE_RETURN_STATEMENT:
                    if (info == this.bracketDepth) {
                        ReturnStatement returnStatement
                            = new ReturnStatement(expression, expression.sourceStart, expression.sourceEnd);
                ...
                case K_UNARY_OPERATOR:
                    if (this.expressionPtr > -1) {
                        ...
                        switch (info) {
                            case PLUS_PLUS:
                                operatorExpression
                                    = new PrefixExpression(expression, IntLiteral.One, PLUS, expression.sourceStart);
                                break;

                            case MINUS_MINUS:
                                operatorExpression
                                    = new PrefixExpression(expression, IntLiteral.One, MINUS, expression.sourceStart);
                                break;
                ...
                case K_BETWEEN_IF_AND_RIGHT_PAREN:
                    IfStatement ifStatement
                        = new IfStatement(expression, new EmptyStatement(expression.sourceEnd, expression.sourceEnd),
                            expression.sourceStart, expression.sourceEnd);
                ...
                case K_BETWEEN_WHILE_AND_RIGHT_PAREN:
                    WhileStatement whileStatement
                        = new WhileStatement(expression, new EmptyStatement(expression.sourceEnd, expression.sourceEnd),
                            expression.sourceStart, expression.sourceEnd);
                ...
                case K_INSIDE_FOR_CONDITIONAL: // https://bugs.eclipse.org/bugs/show_bug.cgi?id=253008
                    ForStatement forStatement = new ForStatement(new Statement[0], expression, new Statement[0],
                        new EmptyStatement(expression.sourceEnd, expression.sourceEnd), false, expression.sourceStart,
                        expression.sourceEnd);
                ...
                case K_INSIDE_THROW_STATEMENT:
                    if (info == this.bracketDepth) {
                        ThrowStatement throwStatement
                            = new ThrowStatement(expression, expression.sourceStart, expression.sourceEnd);
                ...
                case K_INSIDE_ASSERT_EXCEPTION:
                    if (info == this.bracketDepth) {
                        AssertStatement assertStatement = new AssertStatement(expression,
                            new TrueLiteral(expression.sourceStart, expression.sourceStart), expression.sourceStart);
                        this.assistNodeParent = assertStatement;
                    }
                    break nextElement;
            }
        ...
        if (this.assistNodeParent != null) {
            this.currentElement
                = this.currentElement.add(buildMoreCompletionEnclosingContext((Statement) this.assistNodeParent), 0);
        } else {
            if (this.currentElement instanceof RecoveredField
                && !(this.currentElement instanceof RecoveredInitializer)
                && ((RecoveredField) this.currentElement).fieldDeclaration.initialization == null) {
                if (lastIndexOfElement(K_LAMBDA_EXPRESSION_DELIMITER)
                    <= lastIndexOfElement(K_FIELD_INITIALIZER_DELIMITER))
                            ...
                            if (this.currentElement instanceof RecoveredType) {
                                ...
                                if (prevKind == K_BETWEEN_NEW_AND_LEFT_BRACKET) {
                                    ...
                                    if (this.expressionPtr > -1
                                        ...
                                        exp.type = ref;
                                        ((QualifiedAllocationExpression) exp).enclosingInstance
                                    ...
                                    if (isInsideReturn()) {
                                        ReturnStatement returnStatement
                                            = new ReturnStatement(exp, exp.sourceStart, exp.sourceEnd);
                                        this.enclosingNode = returnStatement;
                                        this.currentElement = this.currentElement.add(returnStatement, 0);
                                    } else if (this.currentElement instanceof RecoveredLocalVariable) {
                                        if (((RecoveredLocalVariable) this.currentElement).localDeclaration.initialization
                                            == null) {
                                            this.enclosingNode
                                                = ((RecoveredLocalVariable) this.currentElement).localDeclaration;
                                            this.currentElement = this.currentElement.add(exp, 0);
                                        }
                                    } else if (this.currentElement instanceof RecoveredField) {
                                        if (((RecoveredField) this.currentElement).fieldDeclaration.initialization
                                            == null) {
                                            this.enclosingNode
                                                = ((RecoveredField) this.currentElement).fieldDeclaration;
                                            this.currentElement = this.currentElement.add(exp, 0);
                                        }
                                    } else {
        ...
        if (this.identifierLengthPtr >= 1 && this.previousToken == TokenNameDOT) { // (NB: the top id length is 1 and it
            ...
            if ((length = this.identifierLengthStack[this.identifierLengthPtr - 1]) < 0) {
                ...
                SingleTypeReference typeRef
                    = (SingleTypeReference) TypeReference.baseTypeReference(-length, dim, annotationsOnDimensions);
        ...
        if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR_QUALIFIER
            && ((isEmptyNameCompletion = topExpression == this.assistNode && isEmptyNameCompletion()) // e.g. it is
                ...
                || (isEmptyAssistIdentifier
                    = this.indexOfAssistIdentifier() >= 0 && this.identifierStack[this.identifierPtr].length == 0))) { // e.g.
                                                                                                                       // it
                                                                                                                       // is
                                                                                                                       // something
                                                                                                                       // like
            ...
            if (invocType != ALLOCATION && invocType != QUALIFIED_ALLOCATION) {
                ...
                if (selectorPtr == THIS_CONSTRUCTOR || selectorPtr == SUPER_CONSTRUCTOR) {
                    ...
                    CompletionOnExplicitConstructorCall call
                        = new CompletionOnExplicitConstructorCall((selectorPtr == THIS_CONSTRUCTOR)
                            ? ExplicitConstructorCall.This
                            : ExplicitConstructorCall.Super);
    ...
    private boolean checkLabelStatement() {
        if (isInsideMethod() || isInsideFieldInitialization()) {
            ...
            this.assistNode = kind == K_INSIDE_BREAK_STATEMENT
                ? new CompletionOnBreakStatement(this.identifierStack[this.identifierPtr--], (int) (position >>> 32),
                    (int) position, labels)
                : new CompletionOnContinueStatement(this.identifierStack[this.identifierPtr--], (int) (position >>> 32),
                    (int) position, labels);
            this.lastCheckPoint = this.assistNode.sourceEnd + 1;
            this.isOrphanCompletionNode = true;
            return true;
        ...
        if (this.currentElement instanceof RecoveredMethod) {
            ...
            if (this.lastErrorEndPosition <= this.cursorLocation
                && Util.getLineNumber(this.lastErrorEndPosition, this.scanner.lineEnds, 0, this.scanner.linePtr)
                    == Util.getLineNumber(((CompletionScanner) this.scanner).completedIdentifierStart,
    ...
    protected void consumeClassDeclaration() {
        if (this.astPtr >= 0) {
            ...
            if (this.javadoc != null
                && this.cursorLocation > this.javadoc.sourceStart
                    ...
                    if (!(this.currentElement instanceof RecoveredType)
                        && (this.currentToken == TokenNameDOT
                            || (Util.getLineNumber(type.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr)
                                != Util.getLineNumber(nameSourceStart, this.scanner.lineEnds, 0,
                                    this.scanner.linePtr)))) {
            ...
            if (this.indexOfAssistIdentifier() != 0
                ...
                if (this.currentElement != null) {
                    ...
                    if (Util.getLineNumber(type.sourceStart, this.scanner.lineEnds, 0, this.scanner.linePtr) != Util
                        .getLineNumber((int) (selectorSource >>> 32), this.scanner.lineEnds, 0, this.scanner.linePtr)) {
                        FieldDeclaration completionFieldDecl = new CompletionOnFieldType(type, false);
                        // consume annotations
                        int length;
                        if ((length = this.expressionLengthStack[this.expressionLengthPtr--]) != 0) {
                        ...
                        CompletionOnMethodReturnType md
                            = new CompletionOnMethodReturnType(type, this.compilationUnit.compilationResult);
                        // consume annotations
                        int length;
                        if ((length = this.expressionLengthStack[this.expressionLengthPtr--]) != 0) {
                            System.arraycopy(this.expressionStack, (this.expressionPtr -= length) + 1,
    ...
    protected void consumeLambdaExpression() {
        ...
        if (this.assistNode == null
            || !(this.assistNode.sourceStart >= expression.sourceStart
            ...
            if (this.expressionPtr >= 0
                ...
                if (this.currentElement != null) {
                    ...
                    if (this.currentElement instanceof RecoveredAnnotation) {
                        this.currentElement
                            = ((RecoveredAnnotation) this.currentElement).addAnnotation(annotation, this.identifierPtr);
                    }
                }
        ...
        if (token != TokenNameEOF && this.scanner.currentPosition > this.cursorLocation) {
            ...
            if (!this.diet) { // only when parsing a method body:
                if (this.expressionPtr <= -1 && !requireExtendedRecovery()) {
                    ...
                    if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_BLOCK_DELIMITER	// -> not within a
                                                                                             	// complex structure?
                        && this.scanner.startPosition > this.cursorLocation + 1				// -> is current token
                                                                               				// entirely beyond
                                                                               				// cursorLocation?
    ...
    protected void consumeToken(TerminalToken token) {
        ...
        if (previous == TokenNameIdentifier
            ...
            this.scanner.eofPosition
                = this.cursorLocation < Integer.MAX_VALUE ? this.cursorLocation + 1 : this.cursorLocation;
        }
            ...
            switch (token) {
                ...
                case TokenNamenew:
                    ...
                    this.qualifier = this.expressionPtr; // NB: even if there is no qualification, set it to the
                                                         // expression ptr so that the number of arguments are correctly
                                                         // computed
                    ...
                    switch (previous) {
                        ...
                        case TokenNameIdentifier: // e.g. fred[(]) or foo.fred[(])
                            if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER) == K_SELECTOR) {
                                int info = 0;
                                if (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER, 1)
                                    == K_BETWEEN_ANNOTATION_NAME_AND_RPAREN
                                    && (info
    ...
    public TypeReference createQualifiedAssistTypeReference(char[][] previousIdentifiers, char[] assistName,
        ...
        switch (topKnownElementKind(COMPLETION_OR_ASSIST_PARSER)) {
            ...
            default:
                TypeReference ref
                    = checkAndCreateModuleQualifiedAssistTypeReference(previousIdentifiers, assistName, positions);
                if (ref != null)
                    return ref;
                return new CompletionOnQualifiedTypeReference(previousIdentifiers, assistName, positions,
    ...
    boolean computeKeywords(int kind, List<char[]> keywords) {
        ...
        if (kind == K_BLOCK_DELIMITER
            || kind == K_CONTROL_STATEMENT_DELIMITER
            || kind == K_LAMBDA_EXPRESSION_DELIMITER
    ...
    protected StringLiteral createStringLiteral(char[] token, int start, int end, int lineNumber) {
        if (start <= this.cursorLocation && this.cursorLocation <= end) {
            ...
            if (this.cursorLocation != end || end == contentEnd) {
                CompletionOnStringLiteral stringLiteral
                    = new CompletionOnStringLiteral(token, start, end, contentStart, contentEnd, lineNumber);
    ...
    private MessageSend internalNewMessageSend() {
        ...
        if (this.assistNode == null && this.lParenPos > this.cursorLocation && nameStart <= this.cursorLocation + 1) {
            boolean nextIsCast
                = this.expressionPtr > -1 && this.expressionStack[this.expressionPtr] instanceof CastExpression;
            m = new CompletionOnMessageSendName(null, 0, 0, nextIsCast); // positions will be set in
                                                                         // consumeMethodInvocationName(), if that's who
                                                                         // called us
    ...
    public void parseBlockStatements(AbstractMethodDeclaration md, CompilationUnitDeclaration unit) {
        ...
        if ((md.bits & ASTNode.HasSyntaxErrors) != 0
    ...
    protected FieldDeclaration createFieldDeclaration(char[] assistName, int sourceStart, int sourceEnd) {
        if (this.indexOfAssistIdentifier() < 0
            || (this.currentElement instanceof RecoveredUnit && ((RecoveredUnit) this.currentElement).typeCount == 0)) {
