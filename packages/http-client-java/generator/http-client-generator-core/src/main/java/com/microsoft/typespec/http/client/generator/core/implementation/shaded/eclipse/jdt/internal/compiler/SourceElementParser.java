...
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.TerminalToken.TokenNameCOMMA;
...
public class SourceElementParser extends CommentRecorderParser {
    ...
    public SourceElementParser(ISourceElementRequestor requestor, IProblemFactory problemFactory,
        ...
        this.problemReporter
            = new ProblemReporter(DefaultErrorHandlingPolicies.exitAfterAllProblems(), options, problemFactory) {
                @Override
                public void record(CategorizedProblem problem, CompilationResult unitResult, ReferenceContext context,
                    ...
                    unitResult.record(problem, context, mandatoryError); // TODO (jerome) clients are trapping problems
                                                                         // either through factory or requestor... is
                                                                         // result storing needed?
    ...
    public void addUnknownRef(NameReference nameRef) {
        ...
        // - a name reference can become a type reference only during the cast case, it is then tagged later with the
        // Binding.TYPE bit
        // However since the indexer doesn't make the distinction between name reference and type reference, there is no
        // need
                ...
                for (Expression reference : references) {
                    ...
                    if (reference instanceof JavadocFieldReference) {
                        ...
                        if (constructor.type != null) {
                            ...
                            this.requestor.acceptConstructorReference(compoundName[compoundName.length - 1], argCount,
    ...
    protected void classInstanceCreation(boolean alwaysQualified) {
        ...
        this.reportReferenceInfo = false; // not to see the type reference reported in super call to
                                          // getTypeReference(...)
        ...
        if (length < 0) { // flag for precompiled type reference on base types
            ...
            if (length != numberOfIdentifiers || this.genericsLengthStack[this.genericsLengthPtr] != 0) {
                ...
                if (dim == 0) {
                    ...
                    if (this.reportReferenceInfo) {
                        this.requestor.acceptTypeReference(((ArrayQualifiedTypeReference) ref).tokens, ref.sourceStart,
                            ref.sourceEnd);
                    }
                }
            }
