...
public class CaptureBinding extends TypeVariableBinding {
    ...
    public CaptureBinding(WildcardBinding wildcard, ReferenceBinding sourceType, int start, int end, ASTNode cud,
        int captureID) {
        super(TypeConstants.WILDCARD_CAPTURE_NAME_PREFIX, wildcard.environment);
        this.wildcard = wildcard;
        this.modifiers = ClassFileConstants.AccPublic | ExtraCompilerModifiers.AccGenericSignature; // treat capture as
                                                                                                    // public
        ...
        if (wildcard.hasTypeAnnotations()) {
            ...
            // propagate from wildcard to capture - use super version, because our own method propagates type
            // annotations in the opposite direction:
            super.setTypeAnnotations(wildcard.getTypeAnnotations(),
            ...
            this.tagBits |= wildcard.tagBits
                & (TagBits.AnnotationNullMASK | TagBits.HasNullTypeAnnotation | TagBits.HasMissingType);
        }
    }

    // for subclass CaptureBinding18
    protected CaptureBinding(ReferenceBinding sourceType, char[] sourceName, int start, int end, int captureID,
    ...
    public char[] genericTypeSignature() {
        ...
        if (this.inRecursiveFunction) {
            ...
            return CharOperation.concat(new char[] { 'L' },
                CharOperation.concatWith(TypeConstants.JAVA_LANG_OBJECT, '.'), new char[] { ';' });
        }
    ...
    public void initializeBounds(Scope scope, ParameterizedTypeBinding capturedParameterizedType) {
        ...
        if (wildcardVariable == null) {
            ...
            switch (this.wildcard.boundKind) {
                case Wildcard.EXTENDS:
                    // still need to capture bound supertype as well so as not to expose wildcards to the outside
                    // (111208)
                    ...
                    if (originalWildcardBound.isInterface()) {
                        ...
                        if (capturedWildcardBound.isArrayType()
                            || TypeBinding.equalsEquals(capturedWildcardBound, this)) {
                            this.setSuperClass(scope.getJavaLangObject());
                        } else {
        ...
        // JLS 5.1.10
        // Let G name a generic type declaration (§8.1.2, §9.1.2) with n type parameters A1,...,An with corresponding
        // bounds U1,...,Un.
        // our equivalents:
        ...
        switch (this.wildcard.boundKind) {
            ...
            case Wildcard.SUPER:
                ...
                if (TypeBinding.equalsEquals(wildcardVariable.firstBound, substitutedVariableSuperclass)
                    || TypeBinding.equalsEquals(wildcardBound, substitutedVariableSuperclass)) {
    ...
    public TypeBinding upperBoundForProjection() {
        ...
        if (this.wildcard != null) {
            ...
            if (this.wildcard.boundKind == Wildcard.EXTENDS) {
                ...
                boolean superClassIsObject = TypeBinding.equalsEquals(this.superclass(),
                    this.environment.getResolvedJavaBaseType(TypeConstants.JAVA_LANG_OBJECT, null));
                if (supers.length == 0) {
                    upperBound = this.superclass();
                } else if (supers.length == 1) {
                    upperBound = superClassIsObject
                        ? supers[0]
                        : this.environment
                            .createIntersectionType18(new ReferenceBinding[] { this.superclass(), supers[0] });
                } else {
        ...
        if (!this.inRecursiveFunction) { // CaptureBinding18 can be recursive indeed
            ...
            try {
                if (this.wildcard != null) {
                    ...
                    nameBuffer.append(
                        this.wildcard.withoutToplevelNullAnnotation().nullAnnotatedReadableName(options, shortNames));
                    ...
                    if (otherUpperBounds != NO_TYPES)
                        nameBuffer.append(" & ..."); //$NON-NLS-1$ // only hint at more bounds, we currently don't
                                                     // evaluate null annotations on otherUpperBounds
                }
            } finally {
                this.inRecursiveFunction = false;
            }
            ...
            if (substitutedWildcard != this.wildcard) {  //$IDENTITY-COMPARISON$
                ...
                if (this.superInterfaces != null) {
                    ...
                    for (int i = 0; i < length; i++)
                        substitute.superInterfaces[i] = (ReferenceBinding) this.superInterfaces[i]
