...
public class ReturnStatement extends Statement {
    ...
    @Override
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {	// here requires
                                                                                                      	// to generate a
                                                                                                      	// sequence of
                                                                                                      	// finally
                                                                                                      	// blocks
        ...
        if (this.expression instanceof FunctionalExpression) {
            if (this.expression.resolvedType == null || !this.expression.resolvedType.isValidBinding()) {
                /*
                 * Don't descend without proper target types. For lambda shape analysis, what is pertinent is value vs
                 * void return and the fact that
        ...
        if (this.expression != null) {
            ...
            if (flowInfo.reachMode() == FlowInfo.REACHABLE) {
                ...
                if (compilerOptions.analyseResourceLeaks) {
                    ...
                    flowInfo = anylizeCloseableReturnExpression(this.expression, currentScope, owningTagBits,
        ...
        if (saveValueNeeded) {
            ...
            if (((this.bits & ASTNode.IsSynchronized) == 0)
                && this.expression != null
                && TypeBinding.equalsEquals(this.expression.resolvedType, TypeBinding.BOOLEAN)) {
                if (noAutoCloseables) { // can't abruptly return in the presence of autocloseables. See
                                        // https://bugs.eclipse.org/bugs/show_bug.cgi?id=367566
    ...
    public static FlowInfo anylizeCloseableReturnExpression(Expression returnExpression, BlockScope scope,
        ...
        FakedTrackingVariable trackingVariable = FakedTrackingVariable.getCloseTrackingVariable(returnExpression,
            flowInfo, flowContext, useOwningAnnotations);
        if (trackingVariable != null) {
            ...
            if (delegatingToCaller) {
                ...
                flowInfo
                    = FakedTrackingVariable.markPassedToOutside(scope, returnExpression, flowInfo, flowContext, true);
            }
        }
        // don't wait till after this statement, because then flowInfo would be DEAD_END & thus cannot serve nullStatus
        // any more:
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream) {
        ...
        if (needValueStore()) {
            ...
            this.expression.generateCode(currentScope, codeStream, needValue()); // no value needed if non-returning
                                                                                 // method
        ...
        if (this.statementsWithFinallyBlock != null) {
            for (int i = 0, max = this.statementsWithFinallyBlock.length; i < max; i++) {
                ...
                if (didEscape) {
                    ...
                    StatementWithFinallyBlock.reenterAllExceptionHandlers(this.statementsWithFinallyBlock, i,
                        codeStream);
                    return;
                }
            }
    ...
    private boolean needValueStore() {
        return this.expression != null
            && (this.expression.constant == Constant.NotAConstant
                || (this.expression.implicitConversion & TypeIds.BOXING) != 0)
    ...
    public void resolve(BlockScope scope) {
        ...
        LambdaExpression lambda = methodScope.referenceContext instanceof LambdaExpression
            ? (LambdaExpression) methodScope.referenceContext
            : null;
        TypeBinding methodType = lambda != null
            ? lambda.expectedResultType()
        ...
        if (TypeBinding.notEquals(methodType, expressionType)) // must call before computeConversion() and
                                                               // typeMismatchError()
        ...
        if (this.expression.isConstantValueOfTypeAssignableToType(expressionType, methodType)
            ...
            if (this.expression instanceof CastExpression) {
                if ((this.expression.bits & (ASTNode.UnnecessaryCast | ASTNode.DisableUnnecessaryCastCheck)) == 0) {
                    ...
                    if (TypeBinding.equalsEquals(((CastExpression) this.expression).expression.resolvedType,
                        methodType)) {
