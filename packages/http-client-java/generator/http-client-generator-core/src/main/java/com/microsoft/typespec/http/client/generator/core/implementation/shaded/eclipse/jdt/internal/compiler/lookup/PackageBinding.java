...
public abstract class PackageBinding extends Binding implements TypeConstants {
    ...
    /**
     * Is this package exported from its module? NB: to query this property use {@link #isExported()} to ensure
     * initialization.
     */
    Boolean isExported;
    ...
    public PackageBinding(char[][] compoundName, PackageBinding parent, LookupEnvironment environment,
        ...
        if (enclosingModule != null)
            ...
            this.enclosingModule = parent.enclosingModule; // stop-gap for any remaining calls that don't provide an
                                                           // enclosingModule (they should)
    ...
    PackageBinding addPackage(PackageBinding element, ModuleBinding module) {
        if ((element.tagBits & TagBits.HasMissingType) == 0)
            clearMissingTagBit();
    ...
    void addType(ReferenceBinding element) {
        ...
        if (this.environment.globalOptions.isAnnotationBasedNullAnalysisEnabled
            || this.environment.globalOptions.isAnnotationBasedResourceAnalysisEnabled)
            if (element.isAnnotationType() || element instanceof UnresolvedReferenceBinding) // unresolved types don't
                                                                                             // yet have the modifiers
                                                                                             // set
    ...
    ReferenceBinding getType(char[] name, ModuleBinding mod) {
        ...
        // at this point we have only checked accessibility of the package, accessibility of the type will be checked by
        // callers
    ...
    public Binding getTypeOrPackage(char[] name, ModuleBinding mod, boolean splitPackageAllowed) {
        ...
        ReferenceBinding referenceBinding = getType0(name);
        lookForType0: if (referenceBinding != null && referenceBinding != LookupEnvironment.TheNotFoundType) {
            referenceBinding = (ReferenceBinding) BinaryTypeBinding.resolveType(referenceBinding, this.environment,
                false /* no raw conversion for now */);
            if (referenceBinding.isNestedType()) {
                return new ProblemReferenceBinding(new char[][] { name }, referenceBinding,
            ...
            boolean isSameModule = (this instanceof SplitPackageBinding)
                ? referenceBinding.module() == mod
                : this.enclosingModule == mod;
            if (!isSameModule && referenceBinding.isValidBinding() && !mod.canAccess(referenceBinding.fPackage)) {
                problemBinding = new ProblemReferenceBinding(referenceBinding.compoundName, referenceBinding,
                    ProblemReasons.NotAccessible);
                break lookForType0;
            }
        ...
        lookForType: if (referenceBinding == null && problemBinding == null) { // have not looked for it before
            if ((referenceBinding = this.environment.askForType(this, name, mod)) != null) {
                if (referenceBinding.isNestedType()) {
                    return new ProblemReferenceBinding(new char[][] { name }, referenceBinding,
                        ProblemReasons.InternalNameProvided);
    ...
    void checkIfAnalysisAnnotationType(ReferenceBinding type) {
        ...
        if (this.environment.nullableAnnotationPackage == this
            ...
            if (!(type instanceof UnresolvedReferenceBinding)) // unresolved will need to check back for the resolved
                                                               // type
    ...
    public boolean hasCompilationUnit(boolean checkCUs) {
        if (this.knownTypes != null) {
            for (ReferenceBinding knownType : this.knownTypes.valueTable) {
                if (knownType != null
                    && knownType != LookupEnvironment.TheNotFoundType
