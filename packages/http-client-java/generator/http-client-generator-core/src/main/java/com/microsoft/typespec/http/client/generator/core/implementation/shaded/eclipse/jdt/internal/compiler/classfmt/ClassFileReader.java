...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.codegen.AnnotationTargetTypeConstants;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.BinaryTypeBinding.ExternalAnnotationStatus;
...
public class ClassFileReader extends ClassFileStruct implements IBinaryType {
    ...
    private static String printTypeModifiers(int modifiers) {
        ...
        if ((modifiers & ClassFileConstants.AccPublic) != 0)
            print.print("public "); //$NON-NLS-1$
        if ((modifiers & ClassFileConstants.AccPrivate) != 0)
            print.print("private "); //$NON-NLS-1$
        if ((modifiers & ClassFileConstants.AccFinal) != 0)
            print.print("final "); //$NON-NLS-1$
        if ((modifiers & ClassFileConstants.AccSuper) != 0)
            print.print("super "); //$NON-NLS-1$
        if ((modifiers & ClassFileConstants.AccInterface) != 0)
            print.print("interface "); //$NON-NLS-1$
        if ((modifiers & ClassFileConstants.AccAbstract) != 0)
            print.print("abstract "); //$NON-NLS-1$
        if ((modifiers & ExtraCompilerModifiers.AccSealed) != 0)
            print.print("sealed "); //$NON-NLS-1$
        print.flush();
        return out.toString();
    }
    ...
    public static ClassFileReader read(File file, boolean fullyInitialize) throws ClassFormatException, IOException {
        ...
        ClassFileReader classFileReader
            = new ClassFileReader(uri, classFileBytes, file.getAbsolutePath().toCharArray());
        if (fullyInitialize) {
    ...
    public static ClassFileReader read(byte[] classFileBytes, String fileName)
    ...
    public static ClassFileReader readFromJrt(File jrt, IModule module, String filename)

        throws ClassFormatException, IOException {
        return JRTUtil.getClassfile(jrt, filename, module == null ? null : new String(module.name()));
    }

    public static ClassFileReader readFromModule(File jrt, String moduleName, String filename,
    ...
    public static ClassFileReader read(java.util.zip.ZipFile zip, String filename, boolean fullyInitialize)
        ...
        try (InputStream stream = zip.getInputStream(ze)) {
            ...
            ClassFileReader classFileReader
                = new ClassFileReader(uri, Util.getInputStreamAsByteArray(stream), filename.toCharArray());
            if (fullyInitialize) {
                classFileReader.initialize();
            }
            return classFileReader;
    ...
    public ClassFileReader(byte[] classFileBytes, char[] fileName, boolean fullyInitialize)
        ...
        try {
            ...
            if (this.methodsCount != 0) {
                ...
                for (int i = 0; i < this.methodsCount; i++) {
                    this.methods[i] = isAnnotationType
                        ? AnnotationMethodInfo.createAnnotationMethod(this.reference, this.constantPoolOffsets,
                            readOffset, this.version)
                ...
                switch (attributeName[0]) {
                            ...
                            switch (attributeName[1]) {
                                case 'o':
                                    if (CharOperation.equals(attributeName, AttributeNamesConstants.SourceName)) {
                                        ...
                                        sourceFileNam
                                            = CharDeduplication.intern(utf8At(utf8Offset + 3, u2At(utf8Offset + 1)));
                                    }
                                    break;
                    ...
                    case 'R':
                        if (CharOperation.equals(attributeName,
                            AttributeNamesConstants.RuntimeVisibleAnnotationsName)) {
                            decodeAnnotations(readOffset, true);
                        } else if (CharOperation.equals(attributeName,
                            AttributeNamesConstants.RuntimeInvisibleAnnotationsName)) {
                            decodeAnnotations(readOffset, false);
                        } else if (CharOperation.equals(attributeName,
                            AttributeNamesConstants.RuntimeVisibleTypeAnnotationsName)) {
                            decodeTypeAnnotations(readOffset, true);
                        } else if (CharOperation.equals(attributeName,
                            AttributeNamesConstants.RuntimeInvisibleTypeAnnotationsName)) {
                        ...
                        if (CharOperation.equals(attributeName, AttributeNamesConstants.MissingTypesName)) {
                            ...
                            if (numberOfMissingTypes != 0) {
                                ...
                                for (int j = 0; j < numberOfMissingTypes; j++) {
                                    utf8Offset = this.constantPoolOffsets[u2At(
                                        this.constantPoolOffsets[u2At(missingTypeOffset)] + 1)];
                            ...
                            this.moduleDeclaration
                                = ModuleInfo.createModule(this.reference, this.constantPoolOffsets, readOffset);
                        ...
                        if (CharOperation.equals(attributeName, AttributeNamesConstants.PermittedSubclasses)) {
                            ...
                            if (this.permittedSubtypesCount != 0) {
                                ...
                                for (int j = 0; j < this.permittedSubtypesCount; j++) {
                                    this.permittedSubtypesNames[j]
                                        = CharDeduplication.intern(getConstantClassNameAt(u2At(offset)));
                                    offset += 2;
                                }
                            }
                        }
            ...
            throw new ClassFormatException(e, this.classFileName, ClassFormatException.ErrTruncatedInput, readOffset);
        }
    }
        ...
        if (CharOperation.equals(attributeName, AttributeNamesConstants.RecordClass)) {
            ...
            if (this.recordComponentsCount != 0) {
                ...
                for (int j = 0; j < this.recordComponentsCount; j++) {
                    RecordComponentInfo component = RecordComponentInfo.createComponent(this.reference,
                        this.constantPoolOffsets, offset, this.version);
     ...
     * If 'member' is given it must be either of IBinaryField or IBinaryMethod, in which case we're seeking annotations
     * for that member.
        ...
        if (numberOfAnnotations > 0) {
            ...
            for (int i = 0; i < numberOfAnnotations; i++) {
                ...
                AnnotationInfo newInfo
                    = new AnnotationInfo(this.reference, this.constantPoolOffsets, readOffset, runtimeVisible, false);
                ...
                if (standardTagBits != 0) {
                    ...
                    if ((this.version < ClassFileConstants.JDK9
                        || (standardTagBits & TagBits.AnnotationDeprecated) == 0)
    ...
    private void decodeTypeAnnotations(int offset, boolean runtimeVisible) {
        ...
        if (numberOfAnnotations > 0) {
            ...
            for (int i = 0; i < numberOfAnnotations; i++) {
                // With the last parameter being 'false', the data structure will not be flushed out
                TypeAnnotationInfo newInfo = new TypeAnnotationInfo(this.reference, this.constantPoolOffsets,
                    readOffset, runtimeVisible, false);
                readOffset += newInfo.readOffset;
                newInfos[i] = newInfo;
            }
    ...
    /**
     * Answer the source name if the receiver is a inner type. Return null if it is an anonymous class or if the
     * receiver is a top-level class.
     * e.g.
     * public class A {
     * public class B {
     * }
    ...
    public IBinaryNestedType[] getMemberTypes() {
        ...
        if (length != 0) {
            ...
            if (memberTypeIndex != memberTypes.length) {
                ...
                System.arraycopy(memberTypes, 0, (memberTypes = new IBinaryNestedType[memberTypeIndex]), 0,
                    memberTypeIndex);
            }
            return memberTypes;
        }
    ...
    public int getModifiers() {
        int modifiers;
        if (this.innerInfo != null) {
            modifiers = this.innerInfo.getModifiers() | (this.accessFlags & ClassFileConstants.AccDeprecated)
        ...
        try {
            ...
            if (this.permittedSubtypesNames != newPermittedSubtypesNames) {
                int newPermittedSubtypesLength
                    = newPermittedSubtypesNames == null ? 0 : newPermittedSubtypesNames.length;
    ...
    private boolean hasStructuralAnnotationChanges(IBinaryAnnotation[] currentAnnotations,
        IBinaryAnnotation[] otherAnnotations) {
    ...
    private boolean hasStructuralFieldChanges(FieldInfo currentFieldInfo, FieldInfo otherFieldInfo) {
        ...
        if (currentFieldInfo.getModifiers() != otherFieldInfo.getModifiers())
            return true;
        if ((currentFieldInfo.getTagBits() & TagBits.AnnotationDeprecated)
            != (otherFieldInfo.getTagBits() & TagBits.AnnotationDeprecated))
            return true;
        if (hasStructuralAnnotationChanges(currentFieldInfo.getAnnotations(), otherFieldInfo.getAnnotations()))
            return true;
        if (this.version >= ClassFileConstants.JDK1_8
            && hasStructuralTypeAnnotationChanges(currentFieldInfo.getTypeAnnotations(),
                otherFieldInfo.getTypeAnnotations()))
            return true;
    ...
    private boolean hasStructuralMethodChanges(MethodInfo currentMethodInfo, MethodInfo otherMethodInfo) {
        ...
        if (currentMethodInfo.getModifiers() != otherMethodInfo.getModifiers())
            return true;
        if ((currentMethodInfo.getTagBits() & TagBits.AnnotationDeprecated)
            != (otherMethodInfo.getTagBits() & TagBits.AnnotationDeprecated))
            return true;
        ...
        for (int i = 0; i < currentAnnotatedParamsCount; i++) {
            if (hasStructuralAnnotationChanges(currentMethodInfo.getParameterAnnotations(i, this.classFileName),
                otherMethodInfo.getParameterAnnotations(i, this.classFileName)))
                return true;
        }
        if (this.version >= ClassFileConstants.JDK1_8
            && hasStructuralTypeAnnotationChanges(currentMethodInfo.getTypeAnnotations(),
                otherMethodInfo.getTypeAnnotations()))
            return true;
    ...
    private boolean hasStructuralTypeAnnotationChanges(IBinaryTypeAnnotation[] currentTypeAnnotations,
        IBinaryTypeAnnotation[] otherTypeAnnotations) {
        ...
        if (currentTypeAnnotations != null) {
            loopCurrent: for (IBinaryTypeAnnotation currentAnnotation : currentTypeAnnotations) {
                if (!affectsSignature(currentAnnotation))
                    continue;
                if (otherTypeAnnotations == null)
                    return true;
                for (int i = 0; i < otherTypeAnnotations.length; i++) {
                    ...
                    if (otherAnnotation != null
                        && matchAnnotations(currentAnnotation.getAnnotation(), otherAnnotation.getAnnotation())
                            == Boolean.TRUE) {
    ...
    private boolean affectsSignature(IBinaryTypeAnnotation typeAnnotation) {
        ...
        if (targetType >= AnnotationTargetTypeConstants.LOCAL_VARIABLE
            && targetType <= AnnotationTargetTypeConstants.METHOD_REFERENCE_TYPE_ARGUMENT)
    ...
    public boolean isMember() {
        ...
        if (this.innerInfo.getEnclosingTypeName() == null)
            return false;
        char[] innerSourceName = this.innerInfo.getSourceName();
        return (innerSourceName != null && innerSourceName.length > 0);	 // protection against ill-formed attributes
                                                                       	 // (67600)
    }
    ...
    public String toString() {
        ...
        print.println(
            " this.superclassName: " + (getSuperclassName() == null ? "null" : new String(getSuperclassName()))); //$NON-NLS-2$ //$NON-NLS-1$
