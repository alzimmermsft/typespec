...
public interface OaIdl {
    ...
    public static class SAFEARRAY extends Structure implements Closeable {
        ...
        public static ByReference createSafeArray(int... size) {
            return createSafeArray(new VARTYPE(Variant.VT_VARIANT), size);
        }
         ...
         * @return SAFEARRAYWrapper or {@code NULL} if creation fails.
         */
        public static ByReference createSafeArray(VARTYPE vartype, int... size) {
            SAFEARRAYBOUND[] rgsabound = (SAFEARRAYBOUND[]) new SAFEARRAYBOUND().toArray(size.length);
            for (int i = 0; i < size.length; i++) {
                rgsabound[i].lLbound = new LONG(0);
                rgsabound[i].cElements = new ULONG(size[size.length - i - 1]);
            }
        ...
        public void putElement(Object arg, int... indices) {
            LONG[] paramIndices = new LONG[indices.length];
            for (int i = 0; i < indices.length; i++) {
                paramIndices[i] = new LONG(indices[indices.length - i - 1]);
            }
