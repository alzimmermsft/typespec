...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.Assignment;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.BinaryExpression;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.CompoundAssignment;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.IntLiteral;
...
public class CodeSnippetSingleNameReference extends SingleNameReference implements EvaluationConstants, ProblemReasons {
        ...
        if (assignment.expression.isCompactableOperation()) {
            ...
            if ((operation.left instanceof SingleNameReference)
                ...
                variableReference.generateCompoundAssignment(currentScope, codeStream,
                    this.syntheticAccessors == null ? null : this.syntheticAccessors[WRITE], operation.right, operator,
            ...
            if ((operation.right instanceof SingleNameReference)
                ...
                && (operation.left.constant != Constant.NotAConstant) // exclude non constant expressions, since could
                                                                      // have side-effect
                && (((operation.left.implicitConversion & IMPLICIT_CONVERSION_MASK) >> 4) != T_JavaLangString) // exclude
                                                                                                               // string
                                                                                                               // concatenation
                                                                                                               // which
                                                                                                               // would
                                                                                                               // occur
                                                                                                               // backwards
                && (((operation.right.implicitConversion & IMPLICIT_CONVERSION_MASK) >> 4) != T_JavaLangString)) { // exclude
                                                                                                                   // string
                                                                                                                   // concatenation
                                                                                                                   // which
                                                                                                                   // would
                ...
                // conversion
                variableReference.generateCompoundAssignment(currentScope, codeStream,
                    this.syntheticAccessors == null ? null : this.syntheticAccessors[WRITE], operation.left, operator,
                    operation.implicitConversion, valueRequired);
                if (valueRequired) {
                    codeStream.generateImplicitConversion(assignment.implicitConversion);
                }
        ...
        switch (this.bits & RestrictiveFlagMASK) {
            case Binding.FIELD: // assigning to a field
                ...
                if (codegenField.canBeSeenBy(getReceiverType(currentScope), this, currentScope)) {
                    ...
                    fieldStore(currentScope, codeStream, codegenField, null, this.actualReceiverType,
                        this.delegateThis == null /* implicit this */, valueRequired);
                    if (valueRequired) {
                        codeStream.generateImplicitConversion(assignment.implicitConversion);
                    }
                } else {
                    ...
                    assignment.expression.generateCode(currentScope, codeStream, true);
                    if (valueRequired) {
                        if ((TypeBinding.equalsEquals(codegenField.type, TypeBinding.LONG))
                            || (TypeBinding.equalsEquals(codegenField.type, TypeBinding.DOUBLE))) {
                            codeStream.dup2_x2();
                        } else {
                            codeStream.dup_x2();
                        }
                ...
                if (localBinding.resolvedPosition != -1) {
                    ...
                    if (assignment.expression.constant != Constant.NotAConstant) {
                        ...
                        if (valueRequired) {
                            ...
                            if ((TypeBinding.equalsEquals(localBinding.type, TypeBinding.LONG))
                                || (TypeBinding.equalsEquals(localBinding.type, TypeBinding.DOUBLE))) {
                                codeStream.pop2();
                            } else {
                                codeStream.pop();
                            }
                    ...
                    if (fieldConstant == Constant.NotAConstant) { // directly use inlined value for constant fields
                        if (codegenField.canBeSeenBy(getReceiverType(currentScope), this, currentScope)) {
                            TypeBinding someReceiverType
                                = this.delegateThis != null ? this.delegateThis.type : this.actualReceiverType;
                            TypeBinding constantPoolDeclaringClass = CodeStream.getConstantPoolDeclaringClass(
                                currentScope, codegenField, someReceiverType, true /* implicit this */);
                ...
                if (codegenField.isStatic()) {
                    if (codegenField.canBeSeenBy(getReceiverType(currentScope), this, currentScope)) {
                        ...
                        TypeBinding constantPoolDeclaringClass = CodeStream.getConstantPoolDeclaringClass(currentScope,
                            codegenField, someReceiverType, true /* implicit this */);
                        codeStream.fieldAccess(Opcodes.OPC_getstatic, codegenField, constantPoolDeclaringClass);
                    } else {
    ...
    public void generateReceiver(CodeStream codeStream) {
        codeStream.aload_0();
        if (this.delegateThis != null) {
            codeStream.fieldAccess(Opcodes.OPC_getfield, this.delegateThis, null /* default declaringClass */); // delegate
                                                                                                                // field
                                                                                                                // access
        }
    }
    ...
    public void manageSyntheticAccessIfNecessary(BlockScope currentScope, FlowInfo flowInfo, boolean isReadAccess) {
        ...
        if (this.binding instanceof ParameterizedFieldBinding) {
            ...
            if ((codegenField.type.tagBits & TagBits.HasTypeVariable) != 0) {
                this.genericCast = codegenField.type.genericCast(currentScope.boxing(parameterizedField.type)); // runtimeType
                                                                                                                // could
                                                                                                                // be
                                                                                                                // base
                                                                                                                // type
    ...
    public TypeBinding reportError(BlockScope scope) {
        ...
        if (this.binding instanceof ProblemFieldBinding
            && ((ProblemFieldBinding) this.binding).problemId() == NotFound) {
