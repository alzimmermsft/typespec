...
public class BundleLoader extends ModuleLoader {
    ...
    final PackageSource createExportPackageSource(ModuleWire importWire, Collection<BundleLoader> visited) {
        ...
        PackageSource exportSource
            = providerLoader.exportSources.createPackageSource(importWire.getCapability(), false);
    ...
    public ModuleClassLoader getModuleClassLoader() {
        ...
        if (System.getSecurityManager() == null) {
            ...
            result = AccessController
                .doPrivileged((PrivilegedAction<ModuleClassLoader>) () -> createClassLoaderPrivledged(cl,
                    generation.getBundleInfo().getStorage().getConfiguration(), BundleLoader.this, generation, hooks));
        }
    ...
    final void addExportedProvidersFor(String packageName, List<PackageSource> result,
        ...
        for (ModuleWire bundleWire : requiredBundleWires) {
            if (local != null
                || BundleNamespace.VISIBILITY_REEXPORT.equals(bundleWire.getRequirement()
                    .getDirectives()
        ...
        if (provider == null) {
            if (firstUseOfInvalidLoader.getAndSet(true)) {
                ...
                container.getEventPublisher()
                    .publishFrameworkEvent(FrameworkEvent.ERROR, wiring.getBundle(),
