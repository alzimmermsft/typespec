...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.internal.preferences.ExportedPreferences;
...
public class ProjectPreferences extends EclipsePreferences {
    ...
    static void deleted(IFolder folder) throws CoreException {
        ...
        boolean hasResourcesSettings
            = getFile(folder, PREFS_REGULAR_QUALIFIER).exists() || getFile(folder, PREFS_DERIVED_QUALIFIER).exists();
    ...
    static void deleted(IProject project) throws CoreException {
        ...
        // check if we need to notify the charset manager
        boolean hasResourcesSettings
            = getFile(project, PREFS_REGULAR_QUALIFIER).exists() || getFile(project, PREFS_DERIVED_QUALIFIER).exists();
        // remove the preferences
        removeNode(projectNode);
        // notifies the CharsetManager
        if (hasResourcesSettings) {
    ...
    static IFile getFile(IProject project, String qualifier) {
        return project.getFile(
            IPath.fromOSString(DEFAULT_PREFERENCES_DIRNAME).append(qualifier).addFileExtension(PREFS_FILE_EXTENSION));
    }

    private static Properties loadProperties(IFile file) throws BackingStoreException {
        Properties result = new Properties();
        try (InputStream input = new BufferedInputStream(file.getContents(true));) {
            result.load(input);
    ...
    private List<String> computeChildren() {
        ...
        for (IResource resource : members) {
            if (resource.getType() == IResource.FILE
                && PREFS_FILE_EXTENSION.equals(resource.getFullPath().getFileExtension())) {
