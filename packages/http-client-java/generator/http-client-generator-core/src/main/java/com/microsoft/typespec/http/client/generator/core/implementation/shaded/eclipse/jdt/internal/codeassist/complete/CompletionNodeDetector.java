...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.GenericAstVisitor;
...
public class CompletionNodeDetector extends ASTVisitor {
    ...
    /**
     * Sentinel that signals when searchNode was found as a child of a potential parent but in an unsupported location
     * of it.
     */
    ...
    public void endVisit(IfStatement ifStatement, BlockScope scope) {
        ...
        if (this.parent == ifStatement && this.searchedNode != ifStatement.condition) {
            ...
            // Remove the unwanted parent, but at the same time signal that we should not look for a parent in any
            // enclosing ASTNode:
    ...
    public void endVisit(WhileStatement whileStatement, BlockScope scope) {
        ...
        if (this.parent == whileStatement && this.searchedNode != whileStatement.condition) {
            // searchNode was found as a child of the whileStatement, but in a wrong position (only condition is
            // supported)
            // Remove the unwanted parent, but at the same time signal that we should not look for a parent in any
            // enclosing ASTNode:
            this.parent = NOT_A_PARENT;
    ...
    private void endVisit(ASTNode astNode) {
        ...
        if (this.result) {
            if ((this.parent == null || (this.parent instanceof ArrayInitializer && astNode instanceof Statement)) // heuristically
                                                                                                                   // get
                                                                                                                   // more
                                                                                                                   // context
    ...
    protected void checkUpdateOuter(ASTNode astNode) {
        if (this.containsPotentialPolyExpression && astNode instanceof Expression) {
            ...
            // resetting containsPotentialPolyExpression could become necessary when we search the outerExpression
            // within
            // a node larger than expressions, but currently CompletionParser.attachOrphanCompletionNode() is the only
            // client
        ...
        // the following corresponds to stuff in CompletionParser.buildMoreContext* that would set
    ...
    private boolean isPotentiallyPolyExpression(ASTNode node) {
        ...
        return node instanceof Invocation
            || node instanceof FunctionalExpression
