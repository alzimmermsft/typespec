...
public class JavaElementDeltaBuilder {
    ...
    public JavaElementDeltaBuilder(IJavaElement javaElement) {
        ...
        initialize();
        recordElementInfo(javaElement, (JavaModel) this.javaElement.getJavaModel(), 0);
    }
    ...
    private void findContentChange(JavaElementInfo oldInfo, JavaElementInfo newInfo, IJavaElement newElement) {
        if (oldInfo instanceof MemberElementInfo && newInfo instanceof MemberElementInfo) {
            ...
            if (oldInfo instanceof AnnotatableInfo && newInfo instanceof AnnotatableInfo) {
                findAnnotationChanges(((AnnotatableInfo) oldInfo).annotations, ((AnnotatableInfo) newInfo).annotations,
                    newElement);
            }
    ...
    private void findCategoryChange(Map<IJavaElement, String[]> oldCategoriesMap,
        Map<IJavaElement, String[]> newCategoriesMap) {
        if (oldCategoriesMap != null) {
            ...
            for (IJavaElement element : newCategoriesMap.keySet()) {
                this.delta.changed(element, IJavaElementDelta.F_CATEGORIES); // all categories for this element were
                                                                             // removed
            }
        }
    }
