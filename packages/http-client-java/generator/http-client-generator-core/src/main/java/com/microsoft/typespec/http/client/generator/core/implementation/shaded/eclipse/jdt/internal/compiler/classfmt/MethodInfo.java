...
public class MethodInfo extends ClassFileStruct implements IBinaryMethod, Comparable {
                ...
                switch (attributeName[0]) {
                    ...
                    case 'R':
                        ...
                        if (CharOperation.equals(attributeName,
                            ...
                            AttributeNamesConstants.RuntimeVisibleParameterAnnotationsName)) {
                            paramAnnotations = decodeParamAnnotations(readOffset, true, methodInfo);
                        } else if (CharOperation.equals(attributeName,
                            AttributeNamesConstants.RuntimeInvisibleParameterAnnotationsName)) {
                        ...
                        if (methodAnnotations != null) {
                            if (annotations == null) {
                                ...
                                System.arraycopy(methodAnnotations, 0, newAnnotations, length,
                            ...
                            if (parameterAnnotations == null) {
                                ...
                                for (int p = 0; p < numberOfParameters; p++) {
                                    int numberOfAnnotations
                                        = paramAnnotations[p] == null ? 0 : paramAnnotations[p].length;
                                    if (numberOfAnnotations > 0) {
                                        if (parameterAnnotations[p] == null) {
                                            ...
                                            AnnotationInfo[] newAnnotations
                                                = new AnnotationInfo[length + numberOfAnnotations];
                                            System.arraycopy(parameterAnnotations[p], 0, newAnnotations, 0, length);
                                            System.arraycopy(paramAnnotations[p], 0, newAnnotations, length,
                                                numberOfAnnotations);
                            ...
                            if (typeAnnotations == null) {
                                ...
                                TypeAnnotationInfo[] newAnnotations
                                    = new TypeAnnotationInfo[length + methodTypeAnnotations.length];
                                System.arraycopy(typeAnnotations, 0, newAnnotations, 0, length);
                                System.arraycopy(methodTypeAnnotations, 0, newAnnotations, length,
                                    methodTypeAnnotations.length);
    ...
    static AnnotationInfo[] decodeAnnotations(int offset, boolean runtimeVisible, int numberOfAnnotations,
            ...
            if (runtimeVisible) {
                ...
                for (int i = 0; i < numberOfAnnotations; i++) {
                    ...
                    if (standardAnnoTagBits != 0) {
                        if (methodInfo.version < ClassFileConstants.JDK9
                            || (standardAnnoTagBits & TagBits.AnnotationDeprecated) == 0) { // must retain enhanced
                                                                                            // deprecation
                            annos[i] = null;
                            continue;
                        }
    ...
    static TypeAnnotationInfo[] decodeTypeAnnotations(int offset, boolean runtimeVisible, MethodInfo methodInfo) {
        ...
        if (numberOfAnnotations > 0) {
            ...
            for (int i = 0; i < numberOfAnnotations; i++) {
                TypeAnnotationInfo newInfo = new TypeAnnotationInfo(methodInfo.reference,
                    methodInfo.constantPoolOffsets, readOffset + methodInfo.structOffset, runtimeVisible, false);
    ...
    public char[] getGenericSignature() {
        if (this.signatureUtf8Offset != -1) {
            if (this.signature == null) {
                ...
                this.signature = CharDeduplication
                    .intern(utf8At(this.signatureUtf8Offset + 3, u2At(this.signatureUtf8Offset + 1)));
            }
                ...
                if (entriesNumber == 0) {
                    ...
                    for (int j = 0; j < entriesNumber; j++) {
                        utf8Offset = this.constantPoolOffsets[u2At(
                            this.constantPoolOffsets[u2At(readOffset)] - this.structOffset + 1)] - this.structOffset;
