...
public class TypePattern extends Pattern implements IGenerateTypeCheck {
    ...
    public LocalVariableBinding[] bindingsWhenTrue() {
        return this.isUnnamed() || this.local.binding == null
            ? NO_VARIABLES
            : new LocalVariableBinding[] { this.local.binding };
    }
    ...
    public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        if (this.getEnclosingPattern() == null)
            patternInfo.markAsDefinitelyNonNull(this.local.binding); // can't say the same for members of a record being
                                                                     // deconstructed.
        return patternInfo;
    }
        ...
        if (isUnnamed()) {
            ...
            if (!this.isTotalTypeNode) {
                boolean checkCast = TypeBinding.notEquals(this.local.binding.type, this.outerExpressionType)
                    && (JavaFeature.PRIMITIVES_IN_PATTERNS.isSupported(currentScope.compilerOptions())
                        ? !this.local.binding.type.isBaseType()
                        : true);
                if (checkCast)
    ...
    public boolean isUnconditional(TypeBinding t, Scope scope) {
        ...
        return switch (route) {
            case IDENTITY_CONVERSION, BOXING_CONVERSION, BOXING_CONVERSION_AND_WIDENING_REFERENCE_CONVERSION -> true;
    ...
    public TypeBinding resolveType(BlockScope scope) {
            ...
            if (enclosingPattern instanceof RecordPattern) {
                ...
                if (recType != null) {
                    ...
                    if (components.length > this.index) {
                        ...
                        TypeVariableBinding[] mentionedTypeVariables
                            = rcb.type != null ? rcb.type.syntheticTypeVariablesMentioned() : Binding.NO_TYPE_VARIABLES;
                        this.resolvedType = mentionedTypeVariables.length > 0
                            ? rcb.type.upwardsProjection(scope, mentionedTypeVariables)
                            : rcb.type;
        ...
        if (this.local.binding != null) {
            this.local.binding.modifiers |= ExtraCompilerModifiers.AccOutOfFlowScope; // start out this way, will be
                                                                                      // BlockScope.include'd when
                                                                                      // definitely assigned
            CompilerOptions compilerOptions = scope.compilerOptions();
            if (!JavaFeature.UNNAMMED_PATTERNS_AND_VARS.isSupported(compilerOptions.sourceLevel,
