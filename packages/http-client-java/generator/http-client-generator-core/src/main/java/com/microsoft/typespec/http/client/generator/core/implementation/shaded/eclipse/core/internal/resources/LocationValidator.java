...
public class LocationValidator {
        ...
        for (IProject project : workspace.getRoot().getProjects(IContainer.INCLUDE_HIDDEN)) {
            ...
            for (IResource child : children) {
                if (child.isLinked()) {
                    ...
                    if (testLocation != null && FileUtil.isOverlapping(location, testLocation)) {
                        message = NLS.bind(Messages.links_overlappingResource, toString(location));
                        return new ResourceStatus(IResourceStatus.OVERLAPPING_LOCATION, resource.getFullPath(),
                            message);
                    }
                }
            }
    ...
    public IStatus validateProjectLocationURI(IProject context, URI unresolvedLocation) {
        ...
        if (unresolvedLocation != null) {
            if (URIUtil.equals(unresolvedLocation,
                URIUtil.toURI(Platform.getLocation().addTrailingSeparator().append(LocalMetaArea.F_METADATA)))) {
                isMetadataLocation = true;
            }
        } else if (context.getName().equals(LocalMetaArea.F_METADATA)) {
            isMetadataLocation = true;
        }

        String message;
        if (isMetadataLocation) {
            message = NLS.bind(Messages.resources_invalidPath, toString(
                URIUtil.toURI(Platform.getLocation().addTrailingSeparator().append(LocalMetaArea.F_METADATA))));
            return new ResourceStatus(IResourceStatus.INVALID_VALUE, null, message);
        }
        ...
        if (location.getScheme().equals(EFS.SCHEME_FILE)) {
            ...
            if (FileUtil.isPrefixOf(locationPath, defaultDefaultLocation)) {
                message = NLS.bind(Messages.resources_overlapWorkspace, toString(location),
                    defaultDefaultLocation.toOSString());
                return new ResourceStatus(IResourceStatus.INVALID_VALUE, null, message);
            }
            ...
            if (FileUtil.isPrefixOf(parentPath, defaultDefaultLocation)
                ...
                && (context == null || !locationPath.equals(defaultDefaultLocation.append(context.getName())))) {
                ...
                for (IResource child : children) {
                    if (child.isLinked()) {
                        ...
                        if (testLocation != null && FileUtil.isPrefixOf(testLocation, location)) {
                            ...
                            return new ResourceStatus(IResourceStatus.OVERLAPPING_LOCATION, context.getFullPath(),
                                message);
                        }
                    }
                }
