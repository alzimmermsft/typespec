...
public final class ArrayBinding extends TypeBinding implements HotSwappable {
    ...
    public static final FieldBinding ArrayLength = new FieldBinding(TypeConstants.LENGTH, TypeBinding.INT,
        ClassFileConstants.AccPublic | ClassFileConstants.AccFinal, null, Constant.NotAConstant);
        ...
        switch (actualType.kind()) {
            case Binding.ARRAY_TYPE:
                ...
                if (actualDim == this.dimensions) {
                    this.leafComponentType.collectSubstitutes(scope, actualType.leafComponentType(), inferenceContext,
                        constraint);
                } else if (actualDim > this.dimensions) {
                    ArrayBinding actualReducedType
                        = this.environment.createArrayType(actualType.leafComponentType(), actualDim - this.dimensions);
    ...
    public char[] nullAnnotatedReadableName(CompilerOptions options,
        boolean shortNames) /* java.lang.Object @o.e.j.a.NonNull[] */ {
    ...
    public void swapUnresolved(UnresolvedReferenceBinding unresolvedType, ReferenceBinding resolvedType,
        ...
        if (this.leafComponentType == unresolvedType) { //$IDENTITY-COMPARISON$
             ...
             * This means this array can still be found under the old key, but that is harmless (since the component
             * type
             * is always consulted (see TypeSystem.getArrayType()).
             * 
             * This also means that this array type is not a fully interned singleton: There is `this' object and there
             * is
             * the array that is being created down below that gets cached by the type system and doled out for all
             * further
             * array creations against the same (raw) component type, dimensions and annotations. This again is
             * harmless,
            ...
            this.tagBits |= this.leafComponentType.tagBits & (TagBits.HasTypeVariable | TagBits.HasDirectWildcard
                | TagBits.HasMissingType | TagBits.HasCapturedWildcard);
        }
    }
    ...
    public MethodBinding getCloneMethod(final MethodBinding originalMethod) {
        ...
        method.declaringClass = originalMethod.declaringClass; // cannot set array binding as declaring class, will be
                                                               // tweaked in CodeStream.getConstantPoolDeclaringClass()
