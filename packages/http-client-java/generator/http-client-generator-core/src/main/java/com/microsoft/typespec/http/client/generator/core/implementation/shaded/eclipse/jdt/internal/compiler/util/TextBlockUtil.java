...
public class TextBlockUtil {
    ...
    private static boolean getLineContent(StringBuilder result, char[] line, int start, int end, boolean merge,
        boolean lastLine) {
        int lastPointer = 0;
                                    ...
                                    if (digit >= 0 && digit <= 7) {
                                        ...
                                        if (pos <= end && ScannerHelper.isDigit(next = line[pos])) {
                                            ...
                                            if (zeroToThreeNot) {
                                                ...
                                                if (digit >= 0 && digit <= 7) { // has read \ZeroToThree OctalDigit
                                                                                // OctalDigit
                                                    number = (number * 8) + digit;
                                                } else {
                                                    // has read \ZeroToThree OctalDigit NonOctalDigit --> ignore last
                                                    // character
                                                }
                                            }
                                        } else {
