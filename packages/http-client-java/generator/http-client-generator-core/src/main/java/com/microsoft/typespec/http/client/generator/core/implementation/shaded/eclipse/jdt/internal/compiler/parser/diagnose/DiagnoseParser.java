 ...
 *******************************************************************************/
package com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.diagnose;

import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.TerminalToken.TokenNameLBRACE;
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.TerminalToken.TokenNameNotAToken;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.ConflictedParser;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.ParserBasicInformation;
...
//	private static final int ERROR_CODE = 1;
    ...
    public DiagnoseParser(Parser parser, TerminalToken firstToken, int start, int end, int[] intervalStartToSkip,
        int[] intervalEndToSkip, int[] intervalFlagsToSkip, CompilerOptions options) {
        this.parser = parser;
        this.options = options;
        this.lexStream = new LexStream(BUFF_SIZE, parser.scanner, intervalStartToSkip, intervalEndToSkip,
            intervalFlagsToSkip, firstToken, start, end);
    ...
    private void reallocateStacks() {
        ...
        if (old_stack_length == 0) {
            ...
            System.arraycopy(this.locationStack, 0, this.locationStack = new int[this.stackLength], 0,
                old_stack_length);
            System.arraycopy(this.locationStartStack, 0, this.locationStartStack = new int[this.stackLength], 0,
                old_stack_length);
            ...
            System.arraycopy(this.scopePosition, 0, this.scopePosition = new int[this.stackLength], 0,
                old_stack_length);
        }
        return;
    }
        ...
        try {
            ...
            try {
                ...
                CompilationResult compilationResult
                    = referenceContext != null ? referenceContext.compilationResult() : null;
    ...
    private void diagnoseParse0(boolean record) {
        ...
        try {
            ...
            TerminalToken tok = this.lexStream.kind(this.currentToken);
            ...
            do {
                ...
                while (act > ERROR_ACTION || act < ACCEPT_ACTION) { // SHIFT-REDUCE action or SHIFT action ?
                    ...
                    // if((tok != TokenNameRBRACE || (forceRecoveryToken != currentToken &&
                ...
                if (act == ERROR_ACTION) {
                    ...
                    try {
                        if (problemReporter.options.maxProblemsPerUnit
                            < this.parser.compilationUnit.compilationResult.problemCount) {
                    ...
                    if (candidate.symbol == 0) {
                        ...
                        tok = TerminalToken.of(candidate.symbol);
...
//
    private RepairCandidate errorRecovery(int error_token, boolean forcedError) {
        ...
        TerminalToken prevtokKind = this.lexStream.kind(prevtok);
        ...
        if (this.lexStream.kind(error_token).tokenNumber() == EOFT_SYMBOL) {
            reportError(EOF_CODE, Parser.terminal_index[EOFT_SYMBOL], prevtok, prevtok);
            candidate.symbol = 0;
            candidate.location = error_token;
            return candidate;
        }
        ...
        //
        int i;
        for (i = BUFF_UBOUND; this.lexStream.kind(this.buffer[i]).tokenNumber() == EOFT_SYMBOL; i--) {
            /* empty */}

        reportError(DELETION_CODE, Parser.terminal_index[prevtokKind.tokenNumber()],// Parser.terminal_index[lexStream.kind(prevtok)],
...
//
    private PrimaryRepairInfo checkPrimaryDistance(int stck[], int stack_top, PrimaryRepairInfo repair) {
        ...
        //
        if (this.buffer[repair.bufferPosition] != 0 && this.buffer[repair.bufferPosition + 1] != 0) {// do not merge the
                                                                                                     // first token
        ...
        //
        j = parseCheck(stck, stack_top, this.lexStream.kind(this.buffer[repair.bufferPosition + 1]).tokenNumber(),
    ...
    private RepairCandidate primaryDiagnosis(PrimaryRepairInfo repair) {
        ...
        switch (repair.code) {
            ...
            case BEFORE_CODE: {
                if (repair.symbol > NT_OFFSET)
                    name_index = getNtermIndex(this.stack[this.stateStackTop], repair.symbol, repair.bufferPosition);
                else
                    name_index = getTermIndex(this.stack, this.stateStackTop, repair.symbol, repair.bufferPosition);
            ...
            case INVALID_CODE: {
                name_index = getNtermIndex(this.stack[this.stateStackTop], repair.symbol, repair.bufferPosition + 1);
            ...
            case SUBSTITUTION_CODE: {
                if (repair.misspellIndex >= 6)
                    ...
                    name_index = getTermIndex(this.stack, this.stateStackTop, repair.symbol, repair.bufferPosition + 1);
            ...
            case MERGE_CODE: {
                reportError(repair.code, Parser.terminal_index[repair.symbol], curtok, this.lexStream.next(curtok));
                break;
            }

            case SCOPE_CODE: {
                for (int i = 0; i < this.scopeStackTop; i++) {
                    reportError(repair.code, -this.scopeIndex[i], this.locationStack[this.scopePosition[i]], prevtok,
                ...
                reportError(repair.code, -this.scopeIndex[this.scopeStackTop],
                    this.locationStack[this.scopePosition[this.scopeStackTop]], prevtok,
                    getNtermIndex(this.stack[this.stateStackTop], repair.symbol, repair.bufferPosition));
                break;
            }
        ...
        switch (repair.code) {
            ...
            default: {// deletion
                ...
                candidate.symbol = this.lexStream.kind(this.buffer[repair.bufferPosition + 1]).tokenNumber();
                this.lexStream.reset(this.buffer[repair.bufferPosition + 2]);
                break;
            }
        }
...
//
    private int getNtermIndex(int start, int sym, int buffer_position) {
        int highest_symbol = sym - NT_OFFSET, tok = this.lexStream.kind(this.buffer[buffer_position]).tokenNumber();
        this.lexStream.reset(this.buffer[buffer_position + 1]);
...
//
    ...
    private void scopeTrialCheck(int stck[], int stack_top, PrimaryRepairInfo repair, int indx) {
        if (indx > 20)
            return; // avoid too much recursive call to improve performance
        ...
        if (this.statePoolTop >= this.statePool.length) {
            System.arraycopy(this.statePool, 0, this.statePool = new StateInfo[this.statePoolTop * 2], 0,
                this.statePoolTop);
        }
        ...
        next: for (int i = 0; i < SCOPE_SIZE; i++) {
            ...
            if (act != ERROR_ACTION) {
                ...
                for (j = this.tempStackTop + 1;
                    j >= (max_pos + 1) && Parser.in_symbol(this.tempStack[j]) == Parser.scope_rhs[k]; j--) {
                    k++;
                }
                if (j == max_pos) {
                ...
                if (Parser.scope_rhs[k] == 0 && j < marked_pos) { // match?
                    ...
                    for (j = Parser.scope_state_set[i];
                        stck[stack_position] != Parser.scope_state[j] && Parser.scope_state[j] != 0; j++) {
                        /* empty */}
                    //
                    ...
                    if (Parser.scope_state[j] != 0) {     // state was found
                        ...
                        int distance
                            = parseCheck(stck, stack_position, Parser.scope_lhs[i] + NT_OFFSET, repair.bufferPosition);
                        //
...
//
    private boolean secondaryCheck(int stck[], int stack_top, int buffer_position, int distance) {
        ...
        for (top = stack_top - 1; top >= 0; top--) {
            j = parseCheck(stck, top, this.lexStream.kind(this.buffer[buffer_position]).tokenNumber(),
                buffer_position + 1);
    ...
    private RepairCandidate secondaryPhase(int error_token) {
        ...
        if (this.nextStackTop >= 0) {
            ...
            for (next_last_index = MAX_DISTANCE - 1;
                next_last_index >= 1 && this.lexStream.kind(this.buffer[next_last_index]).tokenNumber() == EOFT_SYMBOL;
                next_last_index--) {
                /* empty */}
            ...
            misplaced = misplacementRecovery(this.nextStack, this.nextStackTop, next_last_index, misplaced, true);
            if (misplaced.recoveryOnNextStack)
                misplaced.distance++;

            repair.numDeletions = this.nextStackTop + BUFF_UBOUND;
            repair = secondaryRecovery(this.nextStack, this.nextStackTop, next_last_index, repair, true);
        ...
        misplaced = misplacementRecovery(this.stack, this.stateStackTop, last_index, misplaced, false);

        repair = secondaryRecovery(this.stack, this.stateStackTop, last_index, repair, false);
        ...
        if (misplaced.distance > MIN_DISTANCE) {
            if (misplaced.numDeletions <= repair.numDeletions
                || (misplaced.distance - misplaced.numDeletions) >= (repair.distance - repair.numDeletions)) {
        ...
        if (repair.code == SECONDARY_CODE || repair.code == DELETION_CODE) {
            ...
            for (scope_repair.bufferPosition = 2;
                scope_repair.bufferPosition <= repair.bufferPosition && repair.code != SCOPE_CODE;
                scope_repair.bufferPosition++) {
                scope_repair = scopeTrial(this.stack, this.stateStackTop, scope_repair);
                j = (scope_repair.distance == MAX_DISTANCE ? last_index : scope_repair.distance);
        ...
        if (repair.code == 0 && this.lexStream.kind(this.buffer[last_index]).tokenNumber() == EOFT_SYMBOL) {
            ...
            scope_repair.distance = 0;
            for (top = this.stateStackTop; top >= 0 && repair.code == 0; top--) {
        ...
        switch (repair.code) {
            ...
            case DELETION_CODE:
                candidate.location = this.buffer[repair.bufferPosition];
                candidate.symbol = this.lexStream.kind(this.buffer[repair.bufferPosition]).tokenNumber();
...
//
    private SecondaryRepairInfo secondaryRecovery(int stck[], int stack_top, int last_index, SecondaryRepairInfo repair,
        ...
        for (int top = stack_top; top >= 0 && repair.numDeletions >= stack_deletions; top--) {
            ...
            for (int i = 2; i <= (last_index - MIN_DISTANCE + 1) && (repair.numDeletions >= (stack_deletions + i - 1));
                i++) {
        ...
        switch (repair.code) {
            case SCOPE_CODE: {
                if (repair.stackPosition < this.stateStackTop) {
                    reportError(DELETION_CODE, Parser.terminal_index[ERROR_SYMBOL],
                        this.locationStack[repair.stackPosition], this.buffer[1]);
                }
                for (int i = 0; i < this.scopeStackTop; i++) {
                    reportError(SCOPE_CODE, -this.scopeIndex[i], this.locationStack[this.scopePosition[i]],
                        this.buffer[1], Parser.non_terminal_index[Parser.scope_lhs[this.scopeIndex[i]]]);
                }

                repair.symbol = Parser.scope_lhs[this.scopeIndex[this.scopeStackTop]] + NT_OFFSET;
                this.stateStackTop = this.scopePosition[this.scopeStackTop];
                reportError(SCOPE_CODE, -this.scopeIndex[this.scopeStackTop],
                    this.locationStack[this.scopePosition[this.scopeStackTop]], this.buffer[1],
                    getNtermIndex(this.stack[this.stateStackTop], repair.symbol, repair.bufferPosition));
                break;
            }
            ...
            default: {
                reportError(repair.code,
                    (repair.code == SECONDARY_CODE
                        ? getNtermIndex(this.stack[repair.stackPosition], repair.symbol, repair.bufferPosition)
                        : Parser.terminal_index[ERROR_SYMBOL]),
                    this.locationStack[repair.stackPosition], this.buffer[repair.bufferPosition - 1]);
...
//
    private int parseCheck(int stck[], int stack_top, int first_token, int buffer_position) {
        ...
        if (first_token > NT_OFFSET) {
            ...
            ct = TerminalToken.of(first_token);
            this.lexStream.reset(this.buffer[buffer_position]);
        }
    ...
    private void reportPrimaryError(int msgCode, int nameIndex, int token, int scopeNameIndex) {
        ...
        if (currentKind == TerminalToken.TokenNameStringLiteral.tokenNumber()) {
        ...
        switch (msgCode) {
            case BEFORE_CODE:
                if (this.recoveryScanner != null) {
                    if (addedToken > -1) {
                        this.recoveryScanner.insertToken(TerminalToken.of(addedToken), -1, errorStart);
                    } else {
                ...
                if (this.reportProblem)
                    problemReporter().parseErrorInsertBeforeToken(errorStart, errorEnd, TerminalToken.of(currentKind),
                        errorTokenSource, errorTokenName, name);
                break;

            case INSERTION_CODE:
                if (this.recoveryScanner != null) {
                    if (addedToken > -1) {
                        this.recoveryScanner.insertToken(TerminalToken.of(addedToken), -1, errorEnd);
                    } else {
                        int[] template = getNTermTemplate(-addedToken);
                        if (template != null) {
                ...
                if (this.reportProblem)
                    problemReporter().parseErrorInsertAfterToken(errorStart, errorEnd, TerminalToken.of(currentKind),
                        errorTokenSource, errorTokenName, name);
                break;
            ...
            case DELETION_CODE:
                ...
                if (this.reportProblem)
                    problemReporter().parseErrorDeleteToken(errorStart, errorEnd, TerminalToken.of(currentKind),
                        errorTokenSource, errorTokenName);
                break;
            ...
            case INVALID_CODE:
                if (name.length() == 0) {
                    ...
                    if (this.reportProblem)
                        problemReporter().parseErrorReplaceToken(errorStart, errorEnd, TerminalToken.of(currentKind),
                            errorTokenSource, errorTokenName, name);
                } else {
                    if (this.recoveryScanner != null) {
                        if (addedToken > -1) {
                            this.recoveryScanner.replaceTokens(TerminalToken.of(addedToken), errorStart, errorEnd);
                        } else {
                            int[] template = getNTermTemplate(-addedToken);
                            if (template != null) {
                    ...
                    if (this.reportProblem)
                        problemReporter().parseErrorInvalidToken(errorStart, errorEnd, TerminalToken.of(currentKind),
                            errorTokenSource, errorTokenName, name);
                }
                break;
            ...
            case SCOPE_CODE:
                ...
                if (scopeNameIndex != 0) {
                    ...
                    if (this.reportProblem)
                        problemReporter().parseErrorInsertToComplete(errorStart, errorEnd, buf.toString(),
                            Parser.readableName[scopeNameIndex]);
                } else {
                    if (this.reportProblem)
                        problemReporter().parseErrorInsertToCompleteScope(errorStart, errorEnd, buf.toString());
                }
            ...
            case MERGE_CODE:
                ...
                if (this.reportProblem)
                    problemReporter().parseErrorMergeTokens(errorStart, errorEnd, name);
                break;
            ...
            case MISPLACED_CODE:
                ...
                if (this.reportProblem)
                    problemReporter().parseErrorMisplacedConstruct(errorStart, errorEnd);
                break;
            ...
            default:
                if (name.length() == 0) {
                    ...
                    if (this.reportProblem)
                        problemReporter().parseErrorNoSuggestion(errorStart, errorEnd, TerminalToken.of(currentKind),
                            errorTokenSource, errorTokenName);
                } else {
                    if (this.recoveryScanner != null) {
                        if (addedToken > -1) {
    ...
    private void reportMisplacedConstruct(int errorStart, int errorEnd, boolean reportImmediately) {
        /*
         * https://bugs.eclipse.org/bugs/show_bug.cgi?id=383046, we should be very careful about ever reporting the
         * diagnostic,
         * "Syntax error, insert ElidedSemicolonAndRightBrace to complete LambdaBody" as it is a synthetic token. Such a
         * diagnostic
         * is wasteful and confusing as it is not actionable by the programmer.
         * 
         * More importantly, the insertion of this synthetic token is also "normal" part of the business of parsing
         * lambdas.
         ...
         * 
         * However, not surfacing ANY error may be invitation for trouble - sometimes the DiagnoseParser can skip over
         * several tokens
         * in the "broken" part of input and eagerly reach for a cleaner state of affairs - this may result in NO other
         * diagnostics showing
         * up whatsoever.
         * 
         ...
         * 
         * The present fix attempts a low-tech solution to the problem. Rather than teach DiagnoseParser the sleight of
         * hand that the
         * normal parser performs in Parser.consumeElidedLeftBraceAndReturn and Parser.consumeExpression (and thereby
         * make the insertion
         * of ElidedSemicolonAndRightBrace a normal part of parsing rather than of recovery), we do three things now:
         * 
         * (a) Defer reporting this error message till the very end. (b) Surface this error ONLY if there are no other
         * syntax errors (so
        ...
        switch (msgCode) {
            ...
            case SCOPE_CODE:
                ...
                if (scopeNameIndex != 0) {
                    ...
                    if (this.reportProblem)
                        problemReporter().parseErrorInsertToCompletePhrase(errorStart, errorEnd, buf.toString());
                }
                break;
            ...
            case DELETION_CODE:
                ...
                if (this.reportProblem)
                    problemReporter().parseErrorDeleteTokens(errorStart, errorEnd);
                break;
            ...
            default:
                if (name.length() == 0) {
                    ...
                    if (this.reportProblem)
                        problemReporter().parseErrorNoSuggestionForTokens(errorStart, errorEnd);
                } else {
                    if (this.recoveryScanner != null) {
                        if (addedToken >= -1) {
                            this.recoveryScanner.replaceTokens(TerminalToken.of(addedToken), errorStart, errorEnd);
                    ...
                    if (this.reportProblem)
                        problemReporter().parseErrorReplaceTokens(errorStart, errorEnd, name);
                }
        }
        return;
    }
    ...
    public boolean atConflictScenario(TerminalToken token) {
         ...
         */
        return (token == TerminalToken.TokenNameLPAREN
            || token == TerminalToken.TokenNameAT
            || (token == TerminalToken.TokenNameLESS && !this.lexStream.awaitingColonColon()));
    }
