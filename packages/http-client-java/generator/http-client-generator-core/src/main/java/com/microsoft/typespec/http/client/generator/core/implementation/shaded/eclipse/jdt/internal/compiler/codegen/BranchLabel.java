...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.codegen.OperandStack.NullStack;
...
// If branch target can be reached in more ways than one, verify that operand stack for every edge/arc is mutually compatible
        ...
        for (int i = 0, size = stackNow.size(); i < size; i++) {
            ...
            if (TypeBinding.notEquals(tn, te)) {
                if (!tn.isCompatibleWith(te) && !te.isCompatibleWith(tn)) {
                    if (this.codeStream.classFile.referenceBinding.scope.lowerUpperBound(new TypeBinding[] { te, tn })
                        == null)
                        return false;
                }
            }
    ...
    protected void trackStackDepth(boolean branch) {
        /*
         * Control can reach an instruction with a label in two ways: (1) via a branch using that label or (2) by
         * falling through from the previous instruction.
         * In both cases, we KNOW the stack depth at the instruction from which control flows to the instruction with
         * the label.
         * Control cannot reach the instruction with a label by falling through if the previous instruction completes
         * abruptly via a goto/return/throw etc
         * this.codeStream.lastAbruptCompletion == this.position ==> instruction prior to the one with the label is a
         * goto/return/throw
         */
        ...
        if (this.targetStackDepth == -1) {
            ...
            } // else: previous instruction completes abruptly via goto/return/throw: Wait for a backward branch to be
              // emitted.
        } else {
            ...
            if (sourceDepthKnown) {
                if (this.targetStackDepth != this.codeStream.stackDepth
                    || !compatibleOperandStacks(this.codeStream.operandStack, this.operandStack)) {
                    this.codeStream.classFile.referenceBinding.scope.problemReporter()
                        .operandStackSizeInappropriate(
                            this.codeStream.classFile.referenceBinding.scope.referenceContext);
                    if (this.targetStackDepth < this.codeStream.stackDepth) {
    ...
    public int forwardReferenceCount() {
        if (this.delegate != null)
            return this.delegate.forwardReferenceCount();
        return this.forwardReferenceCount;
    }

    public int[] forwardReferences() {
        if (this.delegate != null)
            return this.delegate.forwardReferences();
