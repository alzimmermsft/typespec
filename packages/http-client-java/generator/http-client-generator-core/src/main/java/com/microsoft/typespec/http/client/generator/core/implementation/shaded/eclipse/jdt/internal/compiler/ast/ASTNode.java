...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.TypeReference.AnnotationPosition;
...
public abstract class ASTNode implements TypeConstants, TypeIds {
    ...
    public final static int Bit1 = 0x1;					// return type (operator) | name reference kind (name ref) | add
                                       					// assertion (type decl) | useful empty statement (empty
                                       					// statement)
    public final static int Bit2 = 0x2;					// return type (operator) | name reference kind (name ref) | has
                                       					// local type (type, method, field decl) | if type elided
                                       					// (local)
    public final static int Bit3 = 0x4;					// return type (operator) | name reference kind (name ref) |
                                       					// implicit this (this ref) | is argument(local)
    public final static int Bit4 = 0x8;					// return type (operator) | first assignment to local (name
                                       					// ref,local decl) | undocumented empty block (block, type and
                                       					// method decl)
    public final static int Bit5 = 0x10;					// value for return (expression) | has all method bodies
                                        					// (unit) | supertype ref (type ref) | resolved (field
                                        					// decl)| name ref (yield result value)
    public final static int Bit6 = 0x20;					// depth (name ref, msg) | ignore need cast check (cast
                                        					// expression) | error in signature (method declaration/
                                        					// initializer) | is recovered (annotation reference)
    public final static int Bit7 = 0x40;					// depth (name ref, msg) | need runtime checkcast (cast
                                        					// expression) | label used (labelStatement) |
                                        					// needFreeReturn (AbstractMethodDeclaration) | Used in
                                        					// Pattern Guard expression (NameReference)
    public final static int Bit8 = 0x80;					// depth (name ref, msg) | unsafe cast (cast expression) |
                                        					// is default constructor (constructor declaration) |
                                        					// isElseStatementUnreachable (if statement)
    public final static int Bit9 = 0x100;				// depth (name ref, msg) | operator (operator) | is local type
                                         				// (type decl) | isThenStatementUnreachable (if statement) | can
                                         				// be static
    public final static int Bit10 = 0x200;				// depth (name ref, msg) | operator (operator) | is anonymous
                                          				// type (type decl)
    public final static int Bit11 = 0x400;				// depth (name ref, msg) | operator (operator) | is member type
                                          				// (type decl)
    public final static int Bit12 = 0x800;				// depth (name ref, msg) | operator (operator) | has abstract
                                          				// methods (type decl)
    public final static int Bit13 = 0x1000;			// depth (name ref, msg) | operator (operator) | is secondary type
                                           			// (type decl)
    public final static int Bit14 = 0x2000;			// strictly assigned (reference lhs) | discard enclosing instance
                                           			// (explicit constr call) | hasBeenGenerated (type decl)
    public final static int Bit15 = 0x4000;			// is unnecessary cast (expression) | is varargs (type ref) |
                                           			// IsFinallyBlockEscaping (try statement) | superAccess (javadoc
                                           			// allocation expression/javadoc message send/javadoc return
                                           			// statement)
    public final static int Bit16 = 0x8000;			// in javadoc comment (name ref, type ref, msg)
    public final static int Bit17 = 0x10000;			// compound assigned (reference lhs) | unchecked (msg, alloc,
                                            			// explicit constr call)
    public final static int Bit18 = 0x20000;			// non null (expression) | onDemand (import reference)
    public final static int Bit19 = 0x40000;			// didResolve (parameterized qualified type ref/parameterized
                                            			// single type ref) | empty (javadoc return statement) |
                                            			// needReceiverGenericCast (msg/fieldref)
    public final static int Bit20 = 0x80000;			// contains syntax errors (method declaration, type declaration,
                                            			// field declarations, initializer), typeref: <> name ref:
                                            			// lambda capture)
    public final static int Bit21 = 0x100000;			// for all declarations that can contain type references that
                                             			// have type annotations | insideExpressionStatement
    public final static int Bit22 = 0x200000;			// parenthesis count (expression) | used (import reference)
                                             			// shadows outer local (local declarations)
    public final static int Bit23 = 0x400000;			// parenthesis count (expression) | second or later declarator
                                             			// in declaration (local declarations)
    ...
    public final static int Bit30 = 0x20000000;		// elseif (if statement) | try block exit (try statement) |
                                               		// fall-through (case statement) | ignore no effect assign
                                               		// (expression ref) | needScope (for statement) |
                                               		// IsAnyFinallyBlockEscaping (return|yield statement) | blockExit
                                               		// (synchronized statement)
    public final static int Bit31 = 0x40000000;		// local declaration reachable (local decl) | ignore raw type check
                                               		// (type ref) | discard entire assignment (assignment) |
                                               		// isSynchronized (return statement) | thenExit (if statement)
    ...
    public static final int OperatorMASK = Bit9 | Bit10 | Bit11 | Bit12 | Bit13; // 5 bits for operator ID - see
    ...
    public static final int DepthMASK = Bit6 | Bit7 | Bit8 | Bit9 | Bit10 | Bit11 | Bit12 | Bit13; // 8 bits for actual
                                                                                                   // depth value (max.
                                                                                                   // 255)
    ...
    public static final int IsMemberType = Bit11; // local member do not know it is local at parse time (need to look at
                                                  // binding)
    ...
    public static final int ArgumentsTypeElided = Bit2; // A lambda with var typed arguments is considered to be type
                                                        // elided, but the Arguments themselves are considered var
                                                        // typed.

    // for expression
    public static final int ParenthesizedSHIFT = 21; // Bit22 -> Bit29
    public static final int ParenthesizedMASK = Bit22 | Bit23 | Bit24 | Bit25 | Bit26 | Bit27 | Bit28 | Bit29; // 8 bits
                                                                                                               // for
                                                                                                               // parenthesis
                                                                                                               // count
                                                                                                               // value
    ...
    public static final int IsStrictlyAssigned = Bit14; // set only for true assignments, as opposed to compound ones.
                                                        // Used also for JEP 482 to allow assignment, but not read
    ...
    private static int checkInvocationArgument(BlockScope scope, Expression argument, TypeBinding parameterType,
        ...
        if (argumentType != TypeBinding.NULL && parameterType.kind() == Binding.WILDCARD_TYPE) { // intersection types
                                                                                                 // are tolerated
        ...
        TypeBinding checkedParameterType = parameterType; // originalParameterType == null ? parameterType :
                                                          // originalParameterType;
        if (TypeBinding.notEquals(argumentType, checkedParameterType)
    ...
    public static boolean checkInvocationArguments(BlockScope scope, Expression receiver, TypeBinding receiverType,
        ...
        if (arguments == null) {
            if (method.isVarargs()) {
                TypeBinding parameterType = ((ArrayBinding) params[paramLength - 1]).elementsType(); // no element was
                                                                                                     // supplied for
                                                                                                     // vararg parameter
            ...
            if (method.isVarargs()) {
                // 4 possibilities exist for a call to the vararg method foo(int i, long ... value) : foo(1), foo(1, 2),
                // foo(1, 2, 3, 4) & foo(1, new long[] {1, 2})
                ...
                for (int i = 0; i < lastIndex; i++) {
                    ...
                    invocationStatus
                        |= checkInvocationArgument(scope, arguments[i], params[i], argumentTypes[i], originalRawParam);
                }
                ...
                if (lastIndex <= argLength) { // https://bugs.eclipse.org/bugs/show_bug.cgi?id=337093
                    ...
                    if (paramLength != argLength
                        ...
                        parameterType = ((ArrayBinding) parameterType).elementsType(); // single element was provided
                                                                                       // for vararg parameter
                        if (!parameterType.isReifiable() && (method.tagBits & TagBits.AnnotationSafeVarargs) == 0) {
                            scope.problemReporter()
                                .unsafeGenericArrayForVarargs(parameterType, (ASTNode) invocationSite);
                        }
                        originalRawParam = rawOriginalGenericMethod == null
                            ? null
                            : ((ArrayBinding) rawOriginalGenericMethod.parameters[lastIndex]).elementsType();
                    }
                    for (int i = lastIndex; i < argLength; i++) {
                        invocationStatus |= checkInvocationArgument(scope, arguments[i], parameterType,
                            argumentTypes[i], originalRawParam);
                    }
                }
                if (paramLength == argLength) { // 70056
                    ...
                    if (lastArgType == TypeBinding.NULL) {
                        ...
                        if (varargsType.dimensions < dimensions) {
                            ...
                            && TypeBinding.notEquals(lastArgType.leafComponentType().erasure(),
            ...
            if (argsContainCast) {
                CastExpression.checkNeedForArgumentCasts(scope, receiver, receiverType, method, arguments,
        ...
        if ((invocationStatus & INVOCATION_ARGUMENT_WILDCARD) != 0) {
            ...
            if (scope.compilerOptions().reportUnavoidableGenericTypeProblems
                || receiver == null
                || !receiver.forcedToBeRaw(scope.referenceContext())) {
    ...
    public final boolean isFieldUseDeprecated(FieldBinding field, Scope scope, int filteredBits) {
        ...
        if ((field.modifiers & ExtraCompilerModifiers.AccRestrictedAccess) != 0) {
            ...
            if (restriction != null) {
                scope.problemReporter()
                    .forbiddenReference(field, this, restriction.classpathEntryType, restriction.classpathEntryName,
    ...
    public final boolean isMethodUseDeprecated(MethodBinding method, Scope scope, boolean isExplicitUse,
        ...
        if (isExplicitUse && (method.modifiers & ExtraCompilerModifiers.AccRestrictedAccess) != 0) {
            ...
            if (restriction != null) {
                scope.problemReporter()
                    .forbiddenReference(method, invocation, restriction.classpathEntryType,
    ...
    public final boolean isTypeUseDeprecated(TypeBinding type, Scope scope) {
        ...
        if (refType.hasRestrictedAccess()) {
            ...
            if (restriction != null) {
                scope.problemReporter()
                    .forbiddenReference(type, this, restriction.classpathEntryType, restriction.classpathEntryName,
                        restriction.getProblemId());
            }
        }
     ...
     * inspect the arguments to trigger another round of resolving with improved target types from the methods
     * parameters.
     ...
     * @param argumentTypes the argument types as collected from first resolving the invocation arguments and as used
     * for the method lookup.
     * @param scope scope for resolution.
     * @return either the original method or a problem method
     */
    public static MethodBinding resolvePolyExpressionArguments(Invocation invocation, MethodBinding method,
        TypeBinding[] argumentTypes, BlockScope scope) {
    ...
    private static MethodBinding resolvePolyExpressionArguments0(Invocation invocation, MethodBinding method,
        TypeBinding[] argumentTypes, BlockScope scope) {
        MethodBinding candidateMethod = method.isValidBinding()
            ? method
            : method instanceof ProblemMethodBinding ? ((ProblemMethodBinding) method).closestMatch : null;
        ...
        for (int i = 0, length = arguments == null ? 0 : arguments.length; i < length; i++) {
            ...
            if (argumentTypes[i] != null && argumentTypes[i].isPolyType()) {
                ...
                if (argument instanceof LambdaExpression) {
                    ...
                    // additional checks, because LE.resolveType may return a valid binding even in the presence of
                    // structural errors
                    ...
                    parameterType = InferenceContext18.getParameter(parameters, i, variableArity); // refresh after
                                                                                                   // update
                    if (!lambda.isCompatibleWith(parameterType, scope)) {
                        if (method.isValidBinding() && problemMethod == null) {
                            ...
                            if (lambda.reportShapeError(parameterType, scope)) {
                                problemMethod = new ProblemMethodBinding(candidateMethod, method.selector,
                                    originalArguments, ProblemReasons.ErrorAlreadyReported);
                            } else {
                                problemMethod = new ProblemMethodBinding(candidateMethod, method.selector,
                                    originalArguments, ProblemReasons.NotFound);
                            }
                        }
                        continue;
                    }
        ...
        if (method instanceof ParameterizedGenericMethodBinding) {
            ...
            if (ic18 != null && !ic18.isInexactVarargsInference())
                ic18.flushBoundOutbox(); // overload resolution is done, now perform the push of bounds from inner to
                                         // outer
        }
     ...
     * resolved.
     */
    public static AnnotationBinding[] resolveAnnotations(BlockScope scope, Annotation[] sourceAnnotations,
        Binding recipient, boolean copySE8AnnotationsToType) {
        ...
        if (recipient != null) {
            switch (recipient.kind()) {
                ...
                case Binding.LOCAL:
                    if (ExtendedTagBits.areAllAnnotationsResolved(recipient.extendedTagBits))
                        return annotations;
                            ...
                            for (int j = 0; j < length; j++) {
                                ...
                                if (annotationBinding != null) {
                                    ...
                                    if (annotationType != null
                                        && annotationType.id == TypeIds.T_JavaLangSuppressWarnings) {
                                        annot.recordSuppressWarnings(scope, declarationSourceStart,
                                            declarationSourceEnd, scope.compilerOptions().suppressWarnings);
                                    }
                                }
                            }
                        }
        ...
        /*
         * See if the recipient is meta-annotated with @Repeatable and if so validate constraints. We can't do this
         * during resolution of @Repeatable itself as @Target and
         * 
        ...
        if (annotations != null && length > 1) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                for (int j = i + 1; j < length; j++) {
                    ...
                    if (TypeBinding.equalsEquals(otherAnnotation.getAnnotationType(), annotationType)) {
                        if (distinctAnnotations == annotations) {
                            System.arraycopy(distinctAnnotations, 0,
                                distinctAnnotations = new AnnotationBinding[length], 0, length);
                        }
                        ...
                        if (annotationType.isRepeatableAnnotationType()) {
                            ...
                            if (container == null) {  // first encounter with a duplicate.
                                ...
                                container
                                    = new ContainerAnnotation(sourceAnnotations[i], containerAnnotationType, scope);
                                ...
                                Annotation.checkForInstancesOfRepeatableWithRepeatingContainerAnnotation(scope,
                                    annotationType, sourceAnnotations);
                            }
                            container.addContainee(sourceAnnotations[j]);
                        } else {
                            foundDuplicate = true;
                            scope.problemReporter()
                                .duplicateAnnotation(sourceAnnotations[j], scope.compilerOptions().sourceLevel);
                        }
                    }
                }
                if (container != null) {
                ...
                if (foundDuplicate) {
                    scope.problemReporter()
                        .duplicateAnnotation(sourceAnnotations[i], scope.compilerOptions().sourceLevel);
                }
            }
                    ...
                    if (implicitContainerAnnotations.containsKey(annotationType)) {
                        scope.problemReporter()
                            .repeatedAnnotationWithContainer(
                                (Annotation) implicitContainerAnnotations.get(annotationType), annotation);
                    }
                }
            }
        }
    ...
    /**
     * Resolve JSR308 annotations on a type reference, array creation expression or a wildcard. Type parameters go
     * directly to the method/ctor,
     * By construction the bindings associated with QTR, PQTR etc get resolved first and then annotations for different
     * levels get resolved
    ...
    /**
     * "early" handling of NonNullByDefault because for local variables annotations are resolved after their type
     * because of bug
     * 96991.
     */
    public static void handleNonNullByDefault(BlockScope scope, Annotation[] sourceAnnotations,
        ...
        if (defaultNullness != 0) {
            ...
            if (recorded) {
                if (target != null) {
                    scope.problemReporter()
                        .nullDefaultAnnotationIsRedundant(localDeclaration, new Annotation[] { lastNNBDAnnotation },
                            target);
                }
            }
        }
    ...
    // When SE8 annotations feature in SE7 locations, they get attributed to the declared entity. Copy/move these to the
    // type of the declared entity (field, local, argument etc.)
    public static void copySE8AnnotationsToType(BlockScope scope, Binding recipient, Annotation[] annotations,
        ...
        switch (recipient.kind()) {
            case Binding.LOCAL:
                recipientTargetMask
                    = recipient.isParameter() ? TagBits.AnnotationForParameter : TagBits.AnnotationForLocalVariable;
                break;
            ...
            case Binding.METHOD:
                ...
                recipientTargetMask
                    = method.isConstructor() ? TagBits.AnnotationForConstructor : TagBits.AnnotationForMethod;
                break;
        ...
        for (int i = 0, length = annotations.length; i < length; i++) {
            ...
            if ((metaTagBits & TagBits.AnnotationForTypeUse) != 0) {
                ...
                if (se8Annotations == null) {
                    ...
                    System.arraycopy(se8Annotations, 0, se8Annotations = new AnnotationBinding[se8count + 1], 0,
                        se8count);
        ...
        if (se8Annotations != null) {
            switch (recipient.kind()) {
                case Binding.LOCAL:
                    ...
                    if (Annotation.isTypeUseCompatible(typeRef, scope)) { // discard hybrid annotations on name
                                                                          // qualified types.
                        local.declaration.bits |= HasTypeAnnotations;
                        typeRef.bits |= HasTypeAnnotations;
                        int location
                            = local.isParameter() ? Binding.DefaultLocationParameter : 0 /* no default for locals */;
                        local.type = mergeAnnotationsIntoType(scope, se8Annotations, se8nullBits, se8NullAnnotation,
                            typeRef, location, local.type);
                ...
                case Binding.FIELD:
                    ...
                    if (Annotation.isTypeUseCompatible(fieldDeclaration.type, scope)) { // discard hybrid annotations on
                ...
                case Binding.RECORD_COMPONENT:
                    ...
                    if (Annotation.isTypeUseCompatible(recordComponent.type, scope)) { // discard hybrid annotations on
                                                                                       // name qualified types.
                        recordComponent.bits |= HasTypeAnnotations;
                        recordComponent.type.bits |= HasTypeAnnotations;
                        recordComponentBinding.type = mergeAnnotationsIntoType(scope, se8Annotations, se8nullBits,
                            se8NullAnnotation, recordComponent.type, Binding.DefaultLocationRecordComponent,
                            recordComponentBinding.type);
                ...
                case Binding.METHOD:
                    ...
                    if (!method.isConstructor()) {
                        ...
                        MethodDeclaration methodDecl
                            = (MethodDeclaration) sourceType.scope.referenceContext.declarationOf(method);
                        if (Annotation.isTypeUseCompatible(methodDecl.returnType, scope)) {
                            ...
                            method.returnType
                                = mergeAnnotationsIntoType(scope, se8Annotations, se8nullBits, se8NullAnnotation,
            ...
            for (int i = 0; i < length; i++) {
                ...
                long annotationTargetMask
                    = recipientAnnotation.getAnnotationType().getAnnotationTagBits() & TagBits.AnnotationTargetMASK;
                if (annotationTargetMask == 0 || (annotationTargetMask & recipientTargetMask) != 0)
                    recipientAnnotations[newLength++] = recipientAnnotation;
            }
            if (newLength != length) {
                System.arraycopy(recipientAnnotations, 0, recipientAnnotations = new AnnotationBinding[newLength], 0,
                    newLength);
    ...
    public static Annotation[] copyRecordComponentAnnotations(Scope scope, Binding recipient,
        ...
        Annotation[] filteredAnnotations
            = ASTNode.getRelevantAnnotations(annotations, recipientTargetMask, relevantAnnotations);
        AnnotationBinding[] recipientAnnotations
            = relevantAnnotations.toArray(new AnnotationBinding[relevantAnnotations.size()]);
        recipient.setAnnotations(recipientAnnotations, scope, true /* forceStore */);// forceStore since we require at
                                                                                     // codegen
        return filteredAnnotations;
    }

    private static TypeBinding mergeAnnotationsIntoType(BlockScope scope, AnnotationBinding[] se8Annotations,
        long se8nullBits, Annotation se8NullAnnotation, TypeReference typeRef, int location, TypeBinding existingType) {
        ...
        if (se8nullBits != 0) {
            if (typeRef instanceof ArrayTypeReference) { // NOTE: no corresponding code for ArrayQualifiedTypeReference
                                                         // is necessary
        ...
        if (se8nullBits != 0 && oldLeafType.isBaseType()) {
            scope.problemReporter()
                .illegalAnnotationForBaseType(typeRef, new Annotation[] { se8NullAnnotation }, se8nullBits);
            return existingType;
        }

        if (typeRef.dimensions() > 0) {
            location = Binding.DefaultLocationArrayContents; // moving from SE7 to SE8 position applies to the array
                                                             // contents!
        }

        long prevNullBits = oldLeafType.tagBits & TagBits.AnnotationNullMASK;
        if ((prevNullBits | se8nullBits) == TagBits.AnnotationNullMASK
            && typeRef.hasNullTypeAnnotation(AnnotationPosition.MAIN_TYPE)) {
            // would merging introduce a contradiction?
            if (!(oldLeafType instanceof TypeVariableBinding)) { // let type-use annotations override annotations on the
                                                                 // type parameter declaration
                if (prevNullBits != TagBits.AnnotationNullMASK && se8nullBits != TagBits.AnnotationNullMASK) { // conflict
                                                                                                               // caused
                                                                                                               // by the
                                                                                                               // merge?
            ...
            && location != Binding.DefaultLocationReturnType      // normal return type cases are handled in
                                                                  // MethodBinding.fillInDefaultNonNullness18()
            && location != Binding.DefaultLocationField           // normal field type cases are handled in
                                                                  // VariableBinding.fillInDefaultNonNullness()
            && location != Binding.DefaultLocationRecordComponent // record component type cases are handled in
                                                                  // VariableBinding.fillInDefaultNonNullness()
        ...
        if (se8Annotations.length > 0) {
            ...
            if (unionRef == null) {
                typeRef.resolvedType = existingType.isArrayType()
                    ? scope.environment()
                        .createArrayType(newLeafType, existingType.dimensions(), existingType.getTypeAnnotations())
                    : newLeafType;
            } else {
            ...
            if (annotations != null) {
                ...
                if ((length = annotations.length) >= 0) {
                    ...
                    for (int i = 0; i < length; i++) {
                        ...
                        if (!CharOperation.equals(TypeConstants.JAVA_LANG_DEPRECATED[2],
                            annotationTypeRef.getLastToken()))
                            continue;
    ...
    public static void resolveNullDefaultAnnotations(BlockScope scope, Annotation[] annotations, Binding recipient) {
        if (recipient != null) {
            if (annotations != null) {
                int length;
                if ((length = annotations.length) >= 0) {
                    if ((recipient.extendedTagBits & ExtendedTagBits.NullDefaultAnnotationResolved) != 0)
                        return;
                    for (int i = 0; i < length; i++) {
                        TypeReference annotationTypeRef = annotations[i].type;
                        // only resolve type name if 'NonNullByDefault' last token (or corresponding configured
                        // annotation name)
                        if (!scope.environment().isNonNullByDefaultSimpleName(annotationTypeRef.getLastToken()))
                            continue;
                        TypeBinding annotationType = annotations[i].type.resolveType(scope);
                        if (annotationType != null
                            && annotationType.isValidBinding()
                            && annotationType.hasTypeBit(TypeIds.BitNonNullByDefaultAnnotation)) {
                            annotations[i].recipient = recipient;
