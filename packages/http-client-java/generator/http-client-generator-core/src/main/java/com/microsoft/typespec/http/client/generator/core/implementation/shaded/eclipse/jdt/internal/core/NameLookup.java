...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.core.AbstractModule.AutoModule;
...
//$NON-NLS-0$
    ...
    public static final int ACCEPT_ALL
        = ACCEPT_CLASSES | ACCEPT_INTERFACES | ACCEPT_ENUMS | ACCEPT_ANNOTATIONS | ACCEPT_RECORDS;
    ...
    public NameLookup(JavaProject rootProject, IPackageFragmentRoot[] packageFragmentRoots,
        HashtableOfArrayToObject packageFragments, ICompilationUnit[] workingCopies, Map rootToResolvedEntries) {
        ...
        if (workingCopies == null) {
            ...
            for (ICompilationUnit workingCopy : workingCopies) {
                ...
                if (rootPosition == -1)
                    continue; // working copy is not visible from this project (see
                              // https://bugs.eclipse.org/bugs/show_bug.cgi?id=169970)
                    ...
                    if (typeLength == 0) {
                        ...
                        for (IType type : types) {
                            ...
                            if (existing == null) {
                                ...
                                System.arraycopy(existingTypes, 0, existingTypes = new IType[existingTypeLength + 1], 0,
                                    existingTypeLength);
    ...
    private void findAllTypes(String prefix, boolean partialMatch, int acceptFlags, IJavaElementRequestor requestor) {
        ...
        for (IPackageFragmentRoot packageFragmentRoot : this.packageFragmentRoots) {
    ...
    public Answer findType(String typeName, String packageName, boolean partialMatch, int acceptFlags,
        boolean checkRestrictions, IPackageFragmentRoot[] moduleContext, int release) {
        return findType(typeName, packageName, partialMatch, acceptFlags, true/* consider secondary types */,
            false/* do NOT wait for indexes */, checkRestrictions, null, moduleContext, release);
    }
    ...
    public Answer findType(String typeName, String packageName, boolean partialMatch, int acceptFlags,
        ...
        return findType(typeName, packageName, partialMatch, acceptFlags, considerSecondaryTypes, waitForIndexes,
            checkRestrictions, monitor, null,// no module
    ...
    public Answer findType(String typeName, String packageName, boolean partialMatch, int acceptFlags,
        boolean considerSecondaryTypes, boolean waitForIndexes, boolean checkRestrictions, IProgressMonitor monitor,
        IPackageFragmentRoot[] moduleContext, int release) {
        if (packageName == null || packageName.isEmpty()) {
            packageName = IPackageFragment.DEFAULT_PACKAGE_NAME;
        } else if (!typeName.isEmpty() && ScannerHelper.isLowerCase(typeName.charAt(0))) {
        ...
        for (IPackageFragment aPackage : packages) {
            type = findType(typeName, aPackage, partialMatch, acceptFlags, waitForIndexes, considerSecondaryTypes);
            if (type != null) {
                ...
                if (javaProject == null) {
                    javaProject = aPackage.getJavaProject();
                } else if (projects == null) {
                    if (!javaProject.equals(aPackage.getJavaProject())) {
                        projects = new HashSet(3);
                        projects.add(javaProject);
                        projects.add(aPackage.getJavaProject());
                    }
                } else {
                    projects.add(aPackage.getJavaProject());
                }
        ...
        if (considerSecondaryTypes && javaProject != null) {
            if (projects == null) {
                ...
                while (type == null && allProjects.hasNext()) {
                    type = findSecondaryType(packageName, typeName, (IJavaProject) allProjects.next(), waitForIndexes,
                        monitor);
                }
            }
        }
    ...
    static IModuleDescription getModuleDescription(JavaProject project, IPackageFragmentRoot root,
    ...
    private AccessRestriction getViolatedRestriction(String typeName, String packageName, ClasspathEntry entry,
        ...
        if (accessRuleSet != null) {
            ...
            accessRestriction
                = accessRuleSet.getViolatedRestriction(CharOperation.concatWith(packageChars, typeChars, '/'));
        }
            ...
            for (IPackageFragmentRoot moduleRoot : moduleContext) {
                if (moduleRoot instanceof PackageFragmentRoot) {
                    ...
                    if (((PackageFragmentRoot) moduleRoot).getPackageFragment(trimmedPkgName).exists())
                        return true;
                } else if (moduleRoot.getPackageFragment(String.join(".", pkgName)).exists()) //$NON-NLS-1$
                    return true;
            }
        } finally {
            if (VERBOSE)
    ...
    private void seekModuleAwarePartialPackageFragments(String name, IJavaElementRequestor requestor,
        IPackageFragmentRoot[] moduleContext) {
        long start = -1;
        if (VERBOSE)
            start = System.currentTimeMillis();
        ...
        try {
            ...
            for (IPackageFragmentRoot packageFragmentRoot : this.packageFragmentRoots) {
                ...
                if (packageFragmentRoot instanceof JrtPackageFragmentRoot) {
                    if (!prefixMatcher.matches(name, packageFragmentRoot.getElementName().toCharArray(), false)) {
                        continue;
                    }
                }
                module = getModuleDescription(this.rootProject, packageFragmentRoot, this.rootToModule,
                    this.rootToResolvedEntries::get);
    ...
    protected void seekTypesInBinaryPackage(String name, IPackageFragment pkg, boolean partialMatch, int acceptFlags,
        ...
        try {
            if (!partialMatch) {
                ...
                for (IJavaElement file : classFiles) {
                    if (requestor.isCanceled())
                        return;
                    // MatchName will never have the extension ".class" and the elementName always will.
                    String elementName = file.getElementName();
                    if (elementName.regionMatches(true /* ignore case */, 0, name, 0, matchLength)) {
                        if (file instanceof IOrdinaryClassFile) {
                            IType type = ((IOrdinaryClassFile) file).getType();
                            String typeName = type.getElementName();
                            if (!typeName.isEmpty() && !Character.isDigit(typeName.charAt(0))) { // not an anonymous
                                                                                                 // type
            ...
            if (!partialMatch) {
                try {
                    ...
                    for (IJavaElement cu : compilationUnits) {
                        ...
                        if (acceptType(type, acceptFlags, true/* a source type */)) { // accept type checks for
                                                                                      // existence
                            requestor.acceptType(type);
    ...
    protected boolean seekTypesInType(String prefix, int firstDot, IType type, IJavaElementRequestor requestor,
        ...
        try {
            ...
            for (IType iType : types) {
                if (requestor.isCanceled())
                    return false;
                if (iType.getElementName().toLowerCase().startsWith(memberPrefix))
                    if (isMemberTypePrefix) {
                        String subPrefix = prefix.substring(firstDot + 1);
                        return seekTypesInType(subPrefix, subPrefix.indexOf('.'), iType, requestor, acceptFlags);
                    } else {
                        if (acceptType(iType, acceptFlags, true/* a source type */)) {
                            requestor.acceptMemberType(iType);
                            return true;
                        }
                    }
            }
    ...
    protected boolean seekTypesInTopLevelType(String prefix, int firstDot, IType topLevelType,
        IJavaElementRequestor requestor, int acceptFlags) {
    ...
    protected boolean seekTypesInWorkingCopies(String name, IPackageFragment pkg, int firstDot, boolean partialMatch,
        ...
        if (!partialMatch) {
            ...
            if (typeMap != null) {
                ...
                } else if (object instanceof IType[] topLevelTypes) {
                    if (object == NO_TYPES) {
                        ...
                        if (packageInfoName.equals(name))
                            requestor.acceptType(
                                pkg.getCompilationUnit(packageInfoName.concat(SUFFIX_STRING_java)).getType(name));
                        return true;
                    }
    ...
    public void printTimeSpent() {
        ...
        trace(" -> seekModuleAwarePartialPackageFragments....." + this.timeSpentInSeekModuleAwarePartialPackageFragments //$NON-NLS-1$
            + "ms");   //$NON-NLS-1$
        trace(" -> seekPackageFragments......................." + this.timeSpentInSeekPackageFragments + "ms");  //$NON-NLS-1$ //$NON-NLS-2$
        trace(" -> seekPackageFragmentsWithModuleContext......" + this.timeSpentInSeekPackageFragmentsWithModuleContext //$NON-NLS-1$
            + "ms");   //$NON-NLS-1$
