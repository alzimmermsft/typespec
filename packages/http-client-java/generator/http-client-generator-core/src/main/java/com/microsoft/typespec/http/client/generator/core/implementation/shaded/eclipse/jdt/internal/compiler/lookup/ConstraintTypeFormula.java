...
class ConstraintTypeFormula extends ConstraintFormula {
    ...
    public Object reduce(InferenceContext18 inferenceContext) {
        ...
        switch (this.relation) {
            case COMPATIBLE:
                ...
                if (this.left.isProperType(true) && this.right.isProperType(true)) {
                    return this.left.isCompatibleWith(this.right, inferenceContext.scope)
                        || this.left.isBoxingCompatibleWith(this.right, inferenceContext.scope) ? TRUE : FALSE;
                }
                ...
                switch (this.right.kind()) {
                    ...
                    case Binding.PARAMETERIZED_TYPE: {
                        ...
                        TypeBinding gs = this.left.findSuperTypeOriginatingFrom(this.right);	// G<S1,S2,...> or
                                                                                            	// G<S1,S2,...>[]k
            ...
            case TYPE_ARGUMENT_CONTAINED:
                // 18.2.3:
                if (this.right.kind() != Binding.WILDCARD_TYPE) { // "If T is a type" ... all alternatives require
                                                                  // "wildcard"
                        ...
                        if (this.left.kind() != Binding.WILDCARD_TYPE) {
                            ...
                            switch (s.boundKind) {
                                case Wildcard.UNBOUND:
                                    return ConstraintTypeFormula.create(inferenceContext.object, t.bound, SUBTYPE,
                                        this.isSoft);
                                ...
                                case Wildcard.SUPER:
                                    return ConstraintTypeFormula.create(inferenceContext.object, t.bound, SAME,
                                        this.isSoft);
    ...
    boolean checkIVFreeTVmatch(TypeBinding one, TypeBinding two) {
        if (one instanceof InferenceVariable
            && two.isTypeVariable()
            && (two.tagBits & TagBits.AnnotationNullMASK) == 0) {
            ...
            if (this.right.kind() != Binding.WILDCARD_TYPE) {
                ...
                if (this.left.isArrayType() && this.right.isArrayType()) {
                    if (this.left.dimensions() == this.right.dimensions()) {
                        ...
                        return ConstraintTypeFormula.create(this.left.leafComponentType(),
                            this.right.leafComponentType(), SAME, this.isSoft);
    ...
    private Object reduceSubType(Scope scope, TypeBinding subCandidate, TypeBinding superCandidate) {
        ...
        if (superCandidate instanceof InferenceVariable)
            return new TypeBound((InferenceVariable) superCandidate, subCandidate, SUPERTYPE, this.isSoft); // normalize
                                                                                                            // to have
                                                                                                            // variable
                                                                                                            // on LHS
        switch (superCandidate.kind()) {
            ...
            case Binding.PARAMETERIZED_TYPE: {
                ...
                while (superCandidate != null
                    ...
                    && subCandidate != null) {
                    if (!addConstraintsFromTypeParameters(subCandidate, (ParameterizedTypeBinding) superCandidate,
                        constraints))
                        if (isFirst)
                            return FALSE;
                    isFirst = false;
            ...
            case Binding.ARRAY_TYPE:
                ...
                switch (subCandidate.kind()) {
                    case Binding.INTERSECTION_TYPE: {
                        ...
                        sPrimeArray
                            = findMostSpecificSuperArray(intersection.bound, intersection.otherBounds, intersection);
                        break;
                    }
            ...
            case Binding.POLY_TYPE:
                ...
                return reduceSubType(scope, subCandidate,
    ...
    private ArrayBinding findMostSpecificSuperArray(TypeBinding firstBound, TypeBinding[] otherUpperBounds,
        TypeBinding theType) {
    ...
    boolean addConstraintsFromTypeParameters(TypeBinding subCandidate, ParameterizedTypeBinding ca,
        ...
        if (!(cb instanceof ParameterizedTypeBinding)) {
            // if C is parameterized with its own type variables, there're no more constraints to be created here,
            // otherwise let's fail
        ...
        if (cb.isRawType() || bi == null || bi.length == 0)
            return (this.isSoft && InferenceContext18.SIMULATE_BUG_JDK_8026527) ? true : false; // FALSE would conform
                                                                                                // to the spec
