...
public class NonNullDefaultAwareTypeAnnotationWalker extends TypeAnnotationWalker {
    ...
    public NonNullDefaultAwareTypeAnnotationWalker(IBinaryTypeAnnotation[] typeAnnotations, int defaultNullness,
    ...
    private static IBinaryAnnotation getNonNullAnnotation(LookupEnvironment environment) {
        final char[] nonNullAnnotationName
            = CharOperation.concat('L', CharOperation.concatWith(environment.getNonNullAnnotationName(), '/'), ';');
    ...
    protected TypeAnnotationWalker restrict(long newMatches, int newPathPtr) {
        ...
        try {
            ...
            if (this.matches == newMatches
                ...
                && this.currentArrayContentIsNonNull == this.nextArrayContentIsNonNull)
                return this;
            ...
            return new NonNullDefaultAwareTypeAnnotationWalker(this.typeAnnotations, newMatches, newPathPtr,
                ...
                this.environment, this.nextArrayContentIsNonNull);
        } finally {
    ...
    public ITypeAnnotationWalker toMethodParameter(short index) {
        // don't set nextIsDefaultLocation, because signature-level nullness is handled by
        // ImplicitNullAnnotationVerifier (triggered per invocation via MessageSend.resolveType() et al)
        if (this.isEmpty)
            return restrict(this.matches, this.pathPtr);
    ...
    public ITypeAnnotationWalker toField() {
        // don't set nextIsDefaultLocation, because field-level nullness is handled by
        // BinaryTypeBinding.scanFieldForNullAnnotation
        if (this.isEmpty)
            return restrict(this.matches, this.pathPtr);
    ...
    public IBinaryAnnotation[] getAnnotationsAtCursor(int currentTypeId, boolean mayApplyArrayContentsDefaultNullness) {
        IBinaryAnnotation[] normalAnnotations = this.isEmpty
            ? NO_ANNOTATIONS
            : super.getAnnotationsAtCursor(currentTypeId, mayApplyArrayContentsDefaultNullness);
        if ((this.atDefaultLocation || (mayApplyArrayContentsDefaultNullness && this.currentArrayContentIsNonNull))
            && !(currentTypeId == -1)
            && // never apply default on type variable use or wildcard
            !(this.atTypeBound && currentTypeId == TypeIds.T_JavaLangObject)) // for CLIMB-to-top consider a j.l.Object
                                                                              // type bound as no explicit type bound
        {
                ...
                if (defaultNullness == Binding.NULL_UNSPECIFIED_BY_DEFAULT) {
                    if (walker instanceof NonNullDefaultAwareTypeAnnotationWalker) {
                        NonNullDefaultAwareTypeAnnotationWalker nonNullDefaultAwareTypeAnnotationWalker
                            = (NonNullDefaultAwareTypeAnnotationWalker) walker;
