...
public abstract class Annotation extends Expression {
    ...
    Annotation persistibleAnnotation = this;  // Emit this into class file, unless this is a repeating annotation, in
                                              // which case package this into the designated container.
    ...
    public static int[] getLocations(final Expression reference, final Annotation annotation) {
        ...
        class LocationCollector extends ASTVisitor {
            ...
            private int[] computeNestingDepth(TypeReference typeReference) {
                TypeBinding type
                    = typeReference.resolvedType == null ? null : typeReference.resolvedType.leafComponentType();
            ...
            private void inspectAnnotations(Annotation[] annotations) {
                for (int i = 0, length = annotations == null ? 0 : annotations.length;
                    this.continueSearch && i < length; i++) {
            ...
            private void inspectTypeArguments(TypeReference[] typeReferences) {
                for (int i = 0, length = typeReferences == null ? 0 : typeReferences.length;
                    this.continueSearch && i < length; i++) {
            ...
            public boolean visit(TypeReference typeReference, BlockScope scope) {
                if (this.continueSearch) {
                    ...
                    if (this.continueSearch) {
                        ...
                        int size = this.typePathEntries.size();
                        for (int i = levels - 1; this.continueSearch && i >= 0; i--) {  // traverse outwards, see
                                                                                        // comment below about type
                                                                                        // annotations from SE7
                                                                                        // locations.
                            this.typePathEntries.setSize(size);
                ...
                return false; // if annotation is not found in the type reference, it must be one from SE7 location,
                              // typePathEntries captures the proper path entries for them.
            }
     ...
     */
    private long detectStandardAnnotation(Scope scope, ReferenceBinding annotationType,
        MemberValuePair valueAttribute) {
        long tagBits = 0;
        switch (annotationType.id) {
            ...
            case TypeIds.T_JavaLangAnnotationRetention:
                if (valueAttribute != null) {
                    ...
                    if ((expr.bits & Binding.VARIABLE) == Binding.FIELD && expr instanceof Reference) { // anything but
                                                                                                        // Reference
                                                                                                        // would be a
                                                                                                        // type error
                                                                                                        // anyway
                                    ...
                                    if (field != null && field.declaringClass.id == T_JavaLangAnnotationElementType) {
                                        ...
                                        if ((tagBits & element) != 0) {
                                            scope.problemReporter()
                                                .duplicateTargetInTargetAnnotation(annotationType,
                                                    (NameReference) initExpr);
                                        } else {
                                            tagBits |= element;
                                        }
                                    }
    ...
    private long determineNonNullByDefaultTagBits(ReferenceBinding annotationType, MemberValuePair valueAttribute) {
        ...
        if (value instanceof BooleanConstant) {
            ...
            tagBits |= ((BooleanConstant) value).booleanValue()
                ? Binding.DefaultLocationsForTrueValue
                : Binding.NULL_UNSPECIFIED_BY_DEFAULT;
        } else if (value != null) {
            // non-boolean value signals type annotations, evaluate from DefaultLocation[] to bitvector a la
            // Binding#NullnessDefaultMASK:
    ...
    static String getRetentionName(long tagBits) {
        if ((tagBits & TagBits.AnnotationRuntimeRetention) == TagBits.AnnotationRuntimeRetention) {
            // TagBits.AnnotationRuntimeRetention combines both TagBits.AnnotationClassRetention &
    ...
    public void checkRepeatableMetaAnnotation(BlockScope scope) {
        ...
        // `this' is the @Repeatable meta annotation, its recipient is the *repeatable* annotation type - we are at the
        // declaration site, not the repeating use site.

        ReferenceBinding repeatableAnnotationType = (ReferenceBinding) this.recipient; // know it to be an annotation
                                                                                       // type. On target miss we don't
                                                                                       // get here
        ...
        repeatableAnnotationType.setContainerAnnotationType(containerAnnotationType); // For now. May be reset later to
                                                                                      // PRB in case of problems.
        checkContainerAnnotationType(valuePairs[0], scope, containerAnnotationType, repeatableAnnotationType, false); // false
                                                                                                                      // =>
                                                                                                                      // not
                                                                                                                      // use
                                                                                                                      // site,
    ...
    public static void checkContainerAnnotationType(ASTNode culpritNode, BlockScope scope,
        ReferenceBinding containerAnnotationType, ReferenceBinding repeatableAnnotationType, boolean useSite) {
        ...
        for (MethodBinding method : annotationMethods) {
            if (CharOperation.equals(method.selector, TypeConstants.VALUE)) {
                ...
                scope.problemReporter()
                    .containerAnnotationTypeHasWrongValueType(culpritNode, containerAnnotationType,
                        repeatableAnnotationType, method.returnType);
            } else {
                ...
                if ((method.modifiers & ClassFileConstants.AccAnnotationDefault) == 0) {
                    repeatableAnnotationType.tagAsHavingDefectiveContainerType();
                    scope.problemReporter()
                        .containerAnnotationTypeHasNonDefaultMembers(culpritNode, containerAnnotationType,
                            method.selector);
                }
            }
        }
        ...
        if (useSite)
            checkContainingAnnotationTargetAtUse((Annotation) culpritNode, scope, containerAnnotationType,
        ...
        if (containerTypeBits < annotationTypeBits) {
            repeatableAnnotationType.tagAsHavingDefectiveContainerType();
            scope.problemReporter()
                .containerAnnotationTypeHasShorterRetention(culpritNode, repeatableAnnotationType,
                    getRetentionName(annotationTypeBits), containerAnnotationType, getRetentionName(containerTypeBits));
        }

        if ((repeatableAnnotationType.getAnnotationTagBits() & TagBits.AnnotationDocumented) != 0
            && (containerAnnotationType.getAnnotationTagBits() & TagBits.AnnotationDocumented) == 0) {
            repeatableAnnotationType.tagAsHavingDefectiveContainerType();
            scope.problemReporter()
                .repeatableAnnotationTypeIsDocumented(culpritNode, repeatableAnnotationType, containerAnnotationType);
        }

        if ((repeatableAnnotationType.getAnnotationTagBits() & TagBits.AnnotationInherited) != 0
            && (containerAnnotationType.getAnnotationTagBits() & TagBits.AnnotationInherited) == 0) {
            repeatableAnnotationType.tagAsHavingDefectiveContainerType();
            scope.problemReporter()
                .repeatableAnnotationTypeIsInherited(culpritNode, repeatableAnnotationType, containerAnnotationType);
        }
    }

    // This is for error reporting for bad targets at annotation type declaration site (as opposed to the repeat site)
    private static void checkContainerAnnotationTypeTarget(ASTNode culpritNode, Scope scope,
        ReferenceBinding containerType, ReferenceBinding repeatableAnnotationType) {
        long tagBits = repeatableAnnotationType.getAnnotationTagBits();
        if ((tagBits & TagBits.AnnotationTargetMASK) == 0)
            tagBits = TagBits.AnnotationForDeclarationMASK; // absence of @Target meta-annotation implies all
                                                            // declaration targets not all targets.
        ...
        if ((containerAnnotationTypeTargets & ~targets) != 0) {
            class MissingTargetBuilder {
                ...
                void check(long targetMask, char[] targetName) {
                    if ((containerAnnotationTypeTargets & targetMask & ~targets) != 0) {
                        ...
                        if (targetMask == TagBits.AnnotationForType && (targets & TagBits.AnnotationForTypeUse) != 0) {
                            return;
                        }
                        add(targetName);
                    }
            ...
            if (builder.hasError()) {
                repeatableAnnotationType.tagAsHavingDefectiveContainerType();
                scope.problemReporter()
                    .repeatableAnnotationTypeTargetMismatch(culpritNode, repeatableAnnotationType, containerType,
                        builder.toString());
            }
        }
    }

    // This is for error reporting for bad targets at the repeated annotation use site (as opposed to repeatable
    // annotation type declaration site) - Leads to better message.
    public static void checkContainingAnnotationTargetAtUse(Annotation repeatingAnnotation, BlockScope scope,
        TypeBinding containerAnnotationType, TypeBinding repeatingAnnotationType) {
        // check (meta)target compatibility
        if (!repeatingAnnotationType.isValidBinding()) {
            // no need to check annotation usage if missing
            return;
        }
        if (isAnnotationTargetAllowed(repeatingAnnotation, scope, containerAnnotationType,
            repeatingAnnotation.recipient.kind()) != AnnotationTargetAllowed.YES) {
            scope.problemReporter()
                .disallowedTargetForContainerAnnotation(repeatingAnnotation, containerAnnotationType);
        }
    }
    ...
    public boolean isRuntimeTypeInvisible(boolean targetingTypeParameter) {
        ...
        if ((metaTagBits & (TagBits.AnnotationTargetMASK)) == 0) { // In the absence of explicit target, applicable only
                                                                   // to declaration sites
    ...
    public void recordSuppressWarnings(Scope scope, int startSuppresss, int endSuppress,
        ...
        if (isSuppressingWarnings && suppressWarningIrritants != null) {
            scope.referenceCompilationUnit()
                .recordSuppressWarnings(suppressWarningIrritants, this, startSuppresss, endSuppress,
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (this.recipient != null
            ...
            this.recipient.setAnnotations(new AnnotationBinding[] { this.compilerAnnotation }, true); // force storing
                                                                                                      // enhanced
                                                                                                      // deprecation
        }
        ...
        scope.referenceCompilationUnit()
            .recordSuppressWarnings(IrritantSet.NLS, null, this.sourceStart, this.declarationSourceEnd,
                scope.referenceContext());
        if (this.recipient != null) {
                ...
                switch (kind) {
                    case Binding.MODULE:
                        ...
                        if ((tagBits & TagBits.AnnotationSuppressWarnings) != 0) {
                            ...
                            recordSuppressWarnings(scope, 0, moduleDeclaration.declarationSourceEnd,
                                compilerOptions.suppressWarnings);
                        }
                    ...
                    case Binding.GENERIC_TYPE:
                        ...
                        if ((tagBits & TagBits.AnnotationRepeatable) == 0 || sourceType.isAnnotationType()) // don't set
                        ...
                        if ((tagBits & TagBits.AnnotationSuppressWarnings) != 0) {
                            ...
                            recordSuppressWarnings(scope, start, typeDeclaration.declarationSourceEnd,
                                compilerOptions.suppressWarnings);
                        }
                    ...
                    case Binding.METHOD:
                        ...
                        if ((tagBits & TagBits.AnnotationSuppressWarnings) != 0) {
                            ...
                            recordSuppressWarnings(scope, methodDeclaration.declarationSourceStart,
                                methodDeclaration.declarationSourceEnd, compilerOptions.suppressWarnings);
                        }
                    ...
                    case Binding.RECORD_COMPONENT:
                        ...
                        if ((tagBits & TagBits.AnnotationSuppressWarnings) != 0) {
                            ...
                            recordSuppressWarnings(scope, recordComponent.declarationSourceStart,
                                recordComponent.declarationSourceEnd, compilerOptions.suppressWarnings);
                        }
...
//						}
                    ...
                    case Binding.FIELD:
                        ...
                        if ((tagBits & TagBits.AnnotationSuppressWarnings) != 0) {
                            ...
                            FieldDeclaration fieldDeclaration
                                = sourceType.scope.referenceContext.declarationOf(sourceField);
                            recordSuppressWarnings(scope, fieldDeclaration.declarationSourceStart,
                                fieldDeclaration.declarationSourceEnd, compilerOptions.suppressWarnings);
                        }
                        if (defaultNullness != 0) {
                            sourceType = (SourceTypeBinding) sourceField.declaringClass;
                            FieldDeclaration fieldDeclaration
                                = sourceType.scope.referenceContext.declarationOf(sourceField);
                            // test merged value of defaultNullness contributed by this annotation and previous
                            // annotations on same target is redundant w.r.t. containing value
                            // (for targets other than fields the resulting value is tested only once after processing
                            // all annotations, but this is hard to do for fields)
                            Binding target = scope.parent.checkRedundantDefaultNullness(
                                defaultNullness | scope.localNonNullByDefaultValue(fieldDeclaration.sourceStart),
                                fieldDeclaration.sourceStart);
                            scope.recordNonNullByDefault(fieldDeclaration.binding, defaultNullness, this,
                                fieldDeclaration.declarationSourceStart, fieldDeclaration.declarationSourceEnd);
                            if (target != null) {
                                scope.problemReporter()
                                    .nullDefaultAnnotationIsRedundant(fieldDeclaration, new Annotation[] { this },
                                        target);
                            }
                        }
                        // fields don't yet have their type resolved, in 1.8 null annotations
                    ...
                    case Binding.LOCAL:
                        ...
                        if ((tagBits & TagBits.AnnotationSuppressWarnings) != 0) {
                            ...
                            recordSuppressWarnings(scope, localDeclaration.declarationSourceStart,
                                localDeclaration.declarationSourceEnd, compilerOptions.suppressWarnings);
                        }
                        // note: defaultNullness for local declarations has been already been handled earlier by
            ...
            checkAnnotationTarget(this, scope, annotationType, kind, this.recipient,
                tagBits & TagBits.AnnotationNullMASK);
        }
        return this.resolvedType;
    }
    ...
    private static AnnotationTargetAllowed isAnnotationTargetAllowed(Binding recipient, BlockScope scope,
        TypeBinding annotationType, int kind, long metaTagBits) {
        switch (kind) {
            case Binding.PACKAGE:
            ...
            case Binding.GENERIC_TYPE:
                if (((ReferenceBinding) recipient).isAnnotationType()) {
                    if ((metaTagBits & (TagBits.AnnotationForAnnotationType | TagBits.AnnotationForType
                    ...
                    if (CharOperation.equals(((ReferenceBinding) recipient).sourceName,
            ...
            case Binding.METHOD:
                ...
                if (methodBinding.isConstructor()) {
                    ...
                    MethodDeclaration methodDecl
                        = (MethodDeclaration) sourceType.scope.referenceContext.declarationOf(methodBinding);
            ...
            case Binding.FIELD:
                if ((metaTagBits & TagBits.AnnotationForField) != 0) {
                    ...
                    long recordComponentMask = TagBits.AnnotationForRecordComponent | TagBits.AnnotationForMethod
                        | TagBits.AnnotationForParameter | TagBits.AnnotationForTypeUse;
                    return (metaTagBits & recordComponentMask) != 0
            ...
            case Binding.RECORD_COMPONENT:
                 ...
                 */
                long recordComponentMask = TagBits.AnnotationForRecordComponent | TagBits.AnnotationForField
                    | TagBits.AnnotationForMethod | TagBits.AnnotationForParameter | TagBits.AnnotationForTypeUse;
                return (metaTagBits & recordComponentMask) != 0
                    ? AnnotationTargetAllowed.YES
                    : AnnotationTargetAllowed.NO;
    ...
    static AnnotationTargetAllowed isAnnotationTargetAllowed(Annotation annotation, BlockScope scope,
        TypeBinding annotationType, int kind) {
        ...
        if ((metaTagBits & TagBits.AnnotationTargetMASK) == 0) {
             ...
             */
            return kind == Binding.TYPE_USE
                ? AnnotationTargetAllowed.NO_DUE_TO_LACKING_TARGET
                : AnnotationTargetAllowed.YES;
        }
        return isAnnotationTargetAllowed(annotation.recipient, scope, annotationType, kind, metaTagBits);
    }

    static void checkAnnotationTarget(Annotation annotation, BlockScope scope, ReferenceBinding annotationType,
        int kind, Binding recipient, long tagBitsToRevert) {
        // check (meta)target compatibility
        if (!annotationType.isValidBinding()) {
            // no need to check annotation usage if missing
            return;
        ...
        AnnotationTargetAllowed annotationTargetAllowed
            = isAnnotationTargetAllowed(annotation, scope, annotationType, kind);
    ...
    /**
     * Check to see if a repeating annotation is in fact of a container annotation type for an annotation which is also
     * present at the same target.
     * 
     * @param scope The scope (for error reporting)
     * @param repeatedAnnotationType Type of annotation which has been repeated (to check for possibly being a container
     * for a repeatable annotation)
     * @param sourceAnnotations The annotations to check
     */
    public static void checkForInstancesOfRepeatableWithRepeatingContainerAnnotation(BlockScope scope,
        ReferenceBinding repeatedAnnotationType, Annotation[] sourceAnnotations) {
        ...
        if (!elementsType.isRepeatableAnnotationType())
            return; // Can't be a problem, then
    ...
    public static void isTypeUseCompatible(TypeReference reference, Scope scope, Annotation[] annotations) {
        ...
        nextAnnotation: for (Annotation annotation : annotations) {
            long metaTagBits = annotation.resolvedType.getAnnotationTagBits();
            if ((metaTagBits & TagBits.AnnotationForTypeUse) != 0
                && (metaTagBits & TagBits.AnnotationForDeclarationMASK) == 0) {
                ReferenceBinding currentType = (ReferenceBinding) resolvedType;
                while (currentType.isNestedType()) {
                    if (currentType.isStatic()) {
                        QualifiedTypeReference.rejectAnnotationsOnStaticMemberQualififer(scope, currentType,
    ...
    public Annotation getPersistibleAnnotation() {
        return this.persistibleAnnotation;      // will be this for non-repeating annotation, the container for the
                                                // first of the repeating ones and null for the followers.
    }

    public void setPersistibleAnnotation(ContainerAnnotation container) {
        this.persistibleAnnotation = container; // will be a legitimate container for the first of the repeating ones
                                                // and null for the followers.
    }
}
