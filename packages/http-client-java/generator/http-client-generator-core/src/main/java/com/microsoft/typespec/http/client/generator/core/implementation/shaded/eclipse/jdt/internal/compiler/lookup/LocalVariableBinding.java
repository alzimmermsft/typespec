...
public class LocalVariableBinding extends VariableBinding {
    ...
    public int useFlag; // for flow analysis (default is UNUSED), values < 0 indicate the number of compound uses
                        // (postIncrement or compoundAssignment)
    ...
    // note that the name of a variable should be chosen so as not to conflict with user ones (usually starting with a
    // space char is all needed)
     ...
     * without parameter names (see
     * org.eclipse.jdt.internal.core.util.BindingKeyResolver.SyntheticLocalVariableBinding):
    ...
    public char[] computeUniqueKey(boolean isLeaf) {
        ...
        if (scope != null) {
            ...
            for (int i = 0; i < scope.localIndex; i++) { // use linear search assuming the number of locals per scope is
                                                         // low
    ...
    public void recordInitializationStartPC(int pc) {
        ...
        if (this.initializationCount > 0) {
            ...
            if (previousEndPC == pc) {
                this.initializationPCs[((this.initializationCount - 1) << 1) + 1] = -1; // reuse previous interval (its
                                                                                        // range will be augmented)
                return;
            }
        }
        ...
        if (this.initializationCount > 0) {
            ...
            for (int i = 0; i < this.initializationCount; i++) {
                ...
                s += String.valueOf(this.initializationPCs[i << 1]) + "-" //$NON-NLS-1$
                    + ((this.initializationPCs[(i << 1) + 1] == -1)
                        ? "?" //$NON-NLS-1$
    ...
    public void checkEffectiveFinality(Scope scope, Expression node) {
        if ((this.tagBits & (TagBits.HasToBeEffectivelyFinal | TagBits.IsEffectivelyFinal))
            == TagBits.HasToBeEffectivelyFinal) {
            if ((node.bits & ASTNode.IsCapturedOuterLocal) != 0)
                scope.problemReporter()
                    .localMustBeEffectivelyFinal(this, node, false /* resource ? */, true /* outer local ? */);
            else if ((node.bits & ASTNode.IsUsedInPatternGuard) != 0)
                scope.problemReporter().cannotReferToNonFinalLocalInGuard(this, node);
            else if (node.resolvedType != null
                && node.resolvedType.findSuperTypeOriginatingFrom(TypeIds.T_JavaLangAutoCloseable,
                    false /* AutoCloseable is not a class */) != null)
                scope.problemReporter()
                    .localMustBeEffectivelyFinal(this, node, true /* resource ? */, false /* outer local ? */);
            else
                scope.problemReporter()
                    .localMustBeEffectivelyFinal(this, node, false /* resource ? */, false /* outer local ? */);
        }
    }
    ...
    public static LocalVariableBinding[] merge(LocalVariableBinding[] left, LocalVariableBinding[] right) {
        ...
        System.arraycopy(left, 0, left = new LocalVariableBinding[leftCount + right.length], 0, leftCount);
        System.arraycopy(right, 0, left, leftCount, right.length);
        return left;
    }
