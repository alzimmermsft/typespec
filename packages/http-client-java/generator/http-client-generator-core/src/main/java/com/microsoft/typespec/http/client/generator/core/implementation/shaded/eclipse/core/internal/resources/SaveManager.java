...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.internal.localstore.SafeChunkyOutputStream;
...
public class SaveManager implements IElementInfoFlattener, IStringPoolParticipant {
	...
	protected void broadcastLifecycle(final int lifecycle, Map<String, SaveContext> contexts, final MultiStatus warnings, IProgressMonitor monitor) {
		...
		try {
			for (final Iterator<Entry<String, SaveContext>> it = contexts.entrySet().iterator(); it.hasNext();) {
				Entry<String, SaveContext> entry = it.next();
	...
	protected String[] getSaveParticipantPluginIds() {
		synchronized (saveParticipants) {
			return saveParticipants.keySet().toArray(new String[0]);
		}
	}
	...
	protected void resetSnapshots(IResource resource) throws CoreException {
		...
		File file = workspace.getMetaArea().getMarkersSnapshotLocationFor(resource).toFile();
		if (file.exists()) {
			file.delete();
		}
		if (file.exists()) {
	...
	static class InternalMonitorWrapper extends ProgressMonitorWrapper {
				...
				try {
					...
					switch (kind) {
						...
						case ISaveContext.PROJECT_SAVE :
							...
							resetSnapshots(project);
							IStatus result = saveMetaInfo(project);
							if (!result.isOK()) {
	 ...
	 */
	protected void saveMetaInfo(MultiStatus problems) throws CoreException {
		...
		for (IProject root : roots) {
			if (root.isAccessible()) {
				IStatus result = saveMetaInfo((Project) root);
				if (!result.isOK()) {
	 ...
	 */
	protected IStatus saveMetaInfo(Project project) throws CoreException {
	...
	public void visitAndSave(final IResource root) throws CoreException {
		...
		forEachProjectInParallel(this::visitAndSave);
	}
	...
	private void forEachProjectInParallel(CoreConsumer<IProject> consumer) throws CoreException {
		IProject[] projects = workspace.getRoot().getProjects(IContainer.INCLUDE_HIDDEN);
		if (projects.length == 0) {
			return;
		}
		SubMonitor subMointor = SubMonitor.convert(null, projects.length);
		IStatus[] stats;
	...
	public void visitAndSnap(final IResource root) throws CoreException {
		...
		SafeChunkyOutputStream safeMarkerStream;
	...
	protected void writeTree(Map<String, ElementTree> statesToSave, DataOutputStream output, IProgressMonitor monitor)
		...
		try {
			...
			writeWorkspaceFields(output);

			// save plugin info
			output.writeInt(statesToSave.size()); // write the number of plugins we are saving
			for (Entry<String, ElementTree> entry : statesToSave.entrySet()) {
				String pluginId = entry.getKey();
	...
	protected void writeWorkspaceFields(DataOutputStream output) throws IOException {
