        ...
        if (this.completionNode != null) {
            if (this.completionNode instanceof CompletionOnJavadocTag) {
                ...
                if (this.completionNode instanceof CompletionOnJavadocParamNameReference) {
                    ...
                    if (scope.kind == Scope.METHOD_SCOPE) {
                        paramNameReference.missingParams
                            = missingParamTags(paramNameReference.binding, (MethodScope) scope);
                    } else if (scope.kind == Scope.CLASS_SCOPE) {
                        paramNameReference.missingParams
                            = missingRecordComponentNames(paramNameReference.binding, (ClassScope) scope);
                    }
                    if (paramNameReference.token == null || paramNameReference.token.length == 0) {
                        paramNameReference.missingTypeParams
                            = missingTypeParameterTags(paramNameReference.binding, scope);
                    }
                } else if (this.completionNode instanceof CompletionOnJavadocTypeParamReference) {
                    CompletionOnJavadocTypeParamReference typeParamReference
            ...
            if (completionOnJavadocQualifiedTypeReference != null) {
                ...
                CompletionOnJavadocAllocationExpression alloc
                    = (CompletionOnJavadocAllocationExpression) this.completionNode;
        ...
        for (int i = 0; i < componentSize; i++) {
            ...
            for (int j = 0; j < paramTagsSize && !found; j++) {
                ...
                if (field == param.binding) {
                    if (param.binding == paramNameRefBinding) { // do not count first occurrence of component name
                                                                // reference
                        paramNameRefCount++;
            ...
            for (int i = 0; i < typeParametersLength; i++) {
                ...
                for (int j = 0; j < paramTypeParamLength && !found; j++) {
                    if (TypeBinding.equalsEquals(parameter.binding, this.paramTypeParameters[j].resolvedType)) {
                        if (parameter.binding == paramNameRefBinding) { // do not count first occurence of param nmae
                                                                        // reference
                            paramNameRefCount++;
                            found = paramNameRefCount > 1;
                        } else {
