...
public final class TreeMultiset<E extends @Nullable Object> extends AbstractSortedMultiset<E> implements Serializable {
    ...
    public void forEachEntry(ObjIntConsumer<? super E> action) {
        checkNotNull(action);
        for (AvlNode<E> node = firstNode(); node != header && node != null && !range.tooHigh(node.getElement());
            node = node.succ()) {
    ...
    private static <T extends @Nullable Object> void successor(AvlNode<T> a, AvlNode<T> b, AvlNode<T> c) {
        successor(a, b);
        successor(b, c);
    }
