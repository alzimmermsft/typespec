...
abstract public class TypeBinding extends Binding {
    ...
    public static final ReferenceBinding TYPE_USE_BINDING
        = new ReferenceBinding() { /* used for type annotation resolution. */
            {
    ...
    public final static BaseTypeBinding BYTE
        = new BaseTypeBinding(TypeIds.T_byte, TypeConstants.BYTE, new char[] { 'B' });

    public final static BaseTypeBinding SHORT
        = new BaseTypeBinding(TypeIds.T_short, TypeConstants.SHORT, new char[] { 'S' });

    public final static BaseTypeBinding CHAR
        = new BaseTypeBinding(TypeIds.T_char, TypeConstants.CHAR, new char[] { 'C' });

    public final static BaseTypeBinding LONG
        = new BaseTypeBinding(TypeIds.T_long, TypeConstants.LONG, new char[] { 'J' });

    public final static BaseTypeBinding FLOAT
        = new BaseTypeBinding(TypeIds.T_float, TypeConstants.FLOAT, new char[] { 'F' });

    public final static BaseTypeBinding DOUBLE
        = new BaseTypeBinding(TypeIds.T_double, TypeConstants.DOUBLE, new char[] { 'D' });

    public final static BaseTypeBinding BOOLEAN
        = new BaseTypeBinding(TypeIds.T_boolean, TypeConstants.BOOLEAN, new char[] { 'Z' });
    ...
    public TypeBinding(TypeBinding prototype) {  // faithfully copy most instance state - clone operation should
                                                 // specialize/override suitably.
    ...
    /*
     * Answer the receiver's enclosing type... null if the receiver is a top level type or is an array or a non
     * reference type.
     */
        ...
        switch (kind()) {
            case Binding.ARRAY_TYPE:
                ...
                if (!(arrayType.leafComponentType instanceof ReferenceBinding))
                    return null;
                TypeBinding leafSuperType
                    = arrayType.leafComponentType.findSuperTypeOriginatingFrom(otherType.leafComponentType());
     ...
     * If the receiver is already erasure compatible, then it will returned. If not, then will return the alternate
     * lowest
        ...
        switch (kind()) {
            case Binding.TYPE_PARAMETER:
                ...
                if (variable.superclass != null
                    && variable.superclass.findSuperTypeOriginatingFrom(declaringClass) != null) {
            ...
            case Binding.INTERSECTION_TYPE:
                ...
                if (intersection.superclass != null
                    && intersection.superclass.findSuperTypeOriginatingFrom(declaringClass) != null) {
...
// version that allows to capture a type bound using 'scope':
     ...
     * i.e. {@link #isParameterizedType()} is not equivalent to testing <code>type.kind() ==
     * Binding.PARAMETERIZED_TYPE</code>
     */
    public final boolean isParameterizedTypeWithActualArguments() {
        return (kind() == Binding.PARAMETERIZED_TYPE) && ((ParameterizedTypeBinding) this).arguments != null;
    }
    ...
    public boolean isParameterizedWithOwnVariables() {
        ...
        if (enclosing != null && enclosing.erasure().isGenericType() && !enclosing.isParameterizedWithOwnVariables()) {
            return false;
        }
        return true;
    }
     ...
     * @return the current type after a substitution (either 'this' unmodified or a new type with the substitution
     * molded in).
     */
        ...
        switch (otherArgument.kind()) {
            ...
            case Binding.TYPE_PARAMETER:
                ...
                if (otherVariable.firstBound == null) // unbound variable
                    return false;
                TypeBinding otherEliminatedType
                    = Scope.convertEliminatingTypeVariables(otherVariable, genericType, rank, null);
...
// JLS3: 4.5.1.1
                ...
                if (capture instanceof CaptureBinding18) {
                    ...
                    if (cb18.firstBound != null) {
                        if (cb18.lowerBound != null)
                            return false; // type containment is not defined for variables with both upper and lower
                                          // bound
                        ...
                        otherType = capture.environment.createWildcard(null, 0, cb18.firstBound, otherBounds,
                            Wildcard.EXTENDS);
            ...
            case Binding.INTERSECTION_TYPE:
                ...
                switch (otherWildcard.boundKind) {
                    case Wildcard.EXTENDS:
                        if (otherBound instanceof IntersectionTypeBinding18) {
                            TypeBinding[] intersectingTypes
                                = ((IntersectionTypeBinding18) otherBound).intersectingTypes;
                        ...
                        if (match != null && (match = match.leafComponentType()).isRawType()) {
                            return TypeBinding.equalsEquals(match, otherBound.leafComponentType()); // forbide:
                                                                                                    // Collection <= ?
                                                                                                    // extends
                                                                                                    // Collection<?>
                                                                                                    // forbide:
                    ...
                    case Wildcard.SUPER:
                        ...
                        if (match != null && (match = match.leafComponentType()).isRawType()) {
                            return TypeBinding.equalsEquals(match, lowerBound.leafComponentType()); // forbide:
                                                                                                    // Collection <= ?
                                                                                                    // super
                                                                                                    // Collection<?>
                                                                                                    // forbide:
                            ...
                            switch (wildcard.boundKind) {
                                case Wildcard.EXTENDS:
                                    ...
                                    if (otherWildcard.boundKind == Wildcard.UNBOUND
                                        && TypeBinding.equalsEquals(wildcard.bound,
                                            wildcard.typeVariable().upperBound()))
                                        continue nextArgument;
                                    break;
                                ...
                                case Wildcard.UNBOUND:
                                    ...
                                    if (otherWildcard.boundKind == Wildcard.EXTENDS
                                        && TypeBinding.equalsEquals(otherWildcard.bound,
                                            otherWildcard.typeVariable().upperBound()))
                                        continue nextArgument;
                                    break;
                            }
                            break;
    ...
    public char[] nullAnnotatedReadableName(CompilerOptions options,
        boolean shortNames) /* e.g.: java.lang.Object @o.e.j.a.NonNull[] */ {
        if (shortNames)
...
// return a name that can be passed to Signature.createTypeSignature
    ...
    /**
     * Return the single abstract method of a functional interface, or one of {@code null} or
     * {@link ReferenceBinding#samProblemBinding}, if the receiver is not a functional interface as defined in JLS 9.8.
     * In particular {@code null} is answered if the receiver is not a reference type, or is a problem type.
     * 
     * @param scope scope
     * @param replaceWildcards Should wildcards be replaced following JLS 9.8? Say false for lambdas with explicit
     * argument types which should apply 18.5.3
     *
     * @return The single abstract method of a functional interface, or one of {@code null} or
     * {@link ReferenceBinding#samProblemBinding}, if the receiver is not a functional interface.
     */
    ...
    /**
     * Return the primordial type from which the receiver was cloned. Not all types track a prototype, only
     * {@link SourceTypeBinding},
     * {@link BinaryTypeBinding} and {@link UnresolvedReferenceBinding} do so as of now. In fact some types e.g
     * {@link ParameterizedTypeBinding}
     * should not do so. Deflecting a query to a prototype would lead to wrong results in the case of
     * {@link ParameterizedTypeBinding}
     */
