...
public class FieldReference extends Reference implements InvocationSite {
    ...
    public char[] token;
    public FieldBinding binding;															// exact binding resulting
                                															// from lookup
    ...
    public FlowInfo analyseAssignment(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo,
        ...
        if (isCompound) { // check the variable part is initialized if blank final
            if (this.binding.isBlankFinal()
                ...
                FlowInfo fieldInits = flowContext
                    .getInitsForFinalBlankInitializationCheck(this.binding.declaringClass.original(), flowInfo);
        ...
        flowInfo = this.receiver.analyseCode(currentScope, flowContext, flowInfo, !this.binding.isStatic())
            .unconditionalInits();
        ...
        if (this.binding.isFinal()) {
            ...
            if (this.binding.isBlankFinal()
                ...
                && (!(currentScope.methodScope().referenceContext instanceof ConstructorDeclaration cd)
                ...
                if (currentScope.methodScope().referenceContext instanceof ConstructorDeclaration cd
                    && cd.isCompactConstructor())
            ...
            if (!isCompound
                && this.receiver.isThis()
                && !(this.receiver instanceof QualifiedThisReference)
                && TypeBinding.equalsEquals(this.receiver.resolvedType, this.binding.declaringClass) // inherited fields
                                                                                                     // are not tracked
                                                                                                     // here
                && ((this.receiver.bits & ASTNode.ParenthesizedMASK) == 0)) { // (this).x is forbidden
                flowInfo.markAsDefinitelyAssigned(this.binding);
    ...
    public boolean checkNPE(BlockScope scope, FlowContext flowContext, FlowInfo flowInfo, int ttlForFieldCheck) {
        ...
        return checkNullableFieldDereference(scope, this.binding, this.nameSourcePosition, flowContext,
            ttlForFieldCheck);
    }
    ...
    public void generateAssignment(BlockScope currentScope, CodeStream codeStream, Assignment assignment,
        ...
        fieldStore(currentScope, codeStream, codegenBinding,
            ...
            this.actualReceiverType, this.receiver.isImplicitThis(), valueRequired);
        if (valueRequired) {
        ...
        if (valueRequired
            ...
            if (codegenBinding.declaringClass == null) { // array length
                ...
                if (this.syntheticAccessors == null || this.syntheticAccessors[FieldReference.READ] == null) {
                    ...
                    codeStream.invoke(Opcodes.OPC_invokestatic, this.syntheticAccessors[FieldReference.READ],
            ...
            if (isThisReceiver) {
                if (isStatic) {
                    // if no valueRequired, still need possible side-effects of <clinit> invocation, if field belongs to
                    // different class
                    if (TypeBinding.notEquals(this.binding.original().declaringClass,
                        this.actualReceiverType.erasure())) {
                        MethodBinding accessor
                            = this.syntheticAccessors == null ? null : this.syntheticAccessors[FieldReference.READ];
                        if (accessor == null) {
    ...
    public void generatePostIncrement(BlockScope currentScope, CodeStream codeStream, CompoundAssignment postIncrement,
        ...
        fieldStore(currentScope, codeStream, codegenBinding,
            this.syntheticAccessors == null ? null : this.syntheticAccessors[FieldReference.WRITE],
            this.actualReceiverType, this.receiver.isImplicitThis(), false);
    }
        ...
        if (this.binding.isPrivate()) {
            if (!currentScope.enclosingSourceType().isNestmateOf(codegenBinding.declaringClass)
                ...
                this.syntheticAccessors[isReadAccess ? FieldReference.READ : FieldReference.WRITE]
                    = ((SourceTypeBinding) codegenBinding.declaringClass).addSyntheticMethod(codegenBinding,
                        isReadAccess, false /* not super ref in remote type */);
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (this.receiver instanceof ThisReference) {
            this.receiver.bits |= this.bits & ASTNode.IsStrictlyAssigned; // mark as assignment-lhs, where 'this.f' is
                                                                          // legal even in early construction context
        ...
        if (receiverCast) {
            ...
            if (TypeBinding.equalsEquals(((CastExpression) this.receiver).expression.resolvedType,
        ...
        if (TypeBinding.notEquals(this.actualReceiverType, oldReceiverType)
            && TypeBinding.notEquals(this.receiver.postConversionType(scope), this.actualReceiverType)) { // record need
                                                                                                          // for
                                                                                                          // explicit
                                                                                                          // cast at
                                                                                                          // codegen
        ...
        if (fieldType != null) {
            if ((this.bits & ASTNode.IsStrictlyAssigned) == 0) {
                fieldType = fieldType.capture(scope, this.sourceStart, this.sourceEnd);	// perform capture conversion if
                                                                                       	// read access
            }
    ...
    public VariableBinding nullAnnotatedVariableBinding(boolean supportTypeAnnotations) {
        if (this.binding != null) {
            if (supportTypeAnnotations || ((this.binding.tagBits & TagBits.AnnotationNullMASK) != 0)) {
                return this.binding;
            }
        }
        return null;
