...
public class PackageFragmentRoot extends Openable implements IPackageFragmentRoot {
    ...
    SourceMapper createSourceMapper(IPath sourcePath, IPath rootPath) throws JavaModelException {
        ...
        SourceMapper mapper = new SourceMapper(sourcePath, rootPath == null ? null : rootPath.toOSString(),
            getJavaProject().getOptions(true),// cannot use workspace options if external jar is 1.5 jar and workspace
                                              // options are 1.4 options
            encoding);
...
//			}
    ...
    protected void computeFolderChildren(IContainer folder, boolean isIncluded, String[] pkgName, ArrayList vChildren,
        ...
        try {
            ...
            if (length > 0) {
                ...
                for (IResource member : members) {
                    ...
                    switch (member.getType()) {
                        ...
                        case IResource.FILE:
                            // inclusion filter may only include files, in which case we still want to include the
                            // immediate parent package (lazily)
                            if (!hasIncluded
            ...
            throw new JavaModelException(e, IJavaModelStatusConstants.ELEMENT_DOES_NOT_EXIST); // could be thrown by
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof PackageFragmentRoot other))
            return false;
        return resource().equals(other.resource()) && this.getParent().equals(other.getParent());
    }
        ...
        switch (token.charAt(0)) {
            case JEM_PACKAGEFRAGMENT:
                ...
                if (memento.hasMoreTokens()) {
                    ...
                    if (firstChar == JEM_CLASSFILE
                        || firstChar == JEM_MODULAR_CLASSFILE
                        || firstChar == JEM_COMPILATIONUNIT
                        || firstChar == JEM_COUNT) {
    ...
    public IClasspathEntry getResolvedClasspathEntry() throws JavaModelException {
        ...
        Map<IPath, IClasspathEntry> rootPathToResolvedEntries
            = project.getPerProjectInfo().getRootPathToResolvedEntries();
                ...
                if (pkg.getElementName().isEmpty()) {
                    ...
                    if (getKind() == IPackageFragmentRoot.K_SOURCE) {
                        ICompilationUnit unit
                            = ((PackageFragment) pkg).getCompilationUnit(TypeConstants.MODULE_INFO_FILE_NAME_STRING);
