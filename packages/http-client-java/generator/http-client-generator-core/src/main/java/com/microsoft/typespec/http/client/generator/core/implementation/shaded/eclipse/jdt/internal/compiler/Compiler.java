...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.SourceTypeCollisionException;
...
//		    System.out.println();
    ...
    public void accept(ICompilationUnit sourceUnit, AccessRestriction accessRestriction) {
        if (this.lookupEnvironment.unitBeingCompleted != null) {
            ...
            if (CharOperation.equals(fileName1, fileName2)
                && !CharOperation.equals(fileName1, TypeConstants.MODULE_INFO_FILE_NAME))
                return;
        }
    ...
    public void accept(ISourceType[] sourceTypes, PackageBinding packageBinding, AccessRestriction accessRestriction) {
        this.problemReporter.abortDueToInternalError(Messages.bind(Messages.abort_againstSourceModel,
            new String[] { String.valueOf(sourceTypes[0].getName()), String.valueOf(sourceTypes[0].getFileName()) }));
    }
    ...
    protected synchronized void addCompilationUnit(ICompilationUnit sourceUnit, CompilationUnitDeclaration parsedUnit) {
        ...
        if (this.totalUnits == size)
            ...
            System.arraycopy(this.unitsToProcess, 0, (this.unitsToProcess = new CompilationUnitDeclaration[size * 2]),
                0, this.totalUnits);
     ...
     */
    protected void reportWorked(int currentUnitIndex) {
        if (this.progress != null) {
            if (this.progress.isCanceled()) {
                // Only AbortCompilation can stop the compiler cleanly.
                // We check cancellation again following the call to compile.
            ...
            this.progress.worked(1, (this.totalUnits * this.remainingIterations) - currentUnitIndex - 1);
        }
    }
    ...
    private void compile(ICompilationUnit[] sourceUnits, boolean lastRound) {
        ...
        try {
            ...
            if (this.annotationProcessorManager == null) {
                ...
                ICompilationUnit[] originalUnits = sourceUnits.clone(); // remember source units in case a source type
                                                                        // collision occurs
                try {
                    beginToCompile(sourceUnits);
                    if (!lastRound) {
        ...
        try {
            if (this.useSingleThread) {
                ...
                for (int i = startingIndex; i < this.totalUnits; i++) {
                    ...
                    this.stats.generateTime += System.currentTimeMillis() - acceptStart; // record accept time as part
                                                                                         // of generation
                    if (this.options.verbose)
                ...
                try (ProcessTaskManager processingTask = new ProcessTaskManager(this, startingIndex)) {
                    int acceptedCount = 0;
                    // process all units (some more could be injected in the loop by the lookup environment)
                    // the processTask can continue to process units until its fixed sized cache is full then it must
                    // wait
            ...
            if (!lastRound) {
                if (this.annotationProcessorManager != null && this.totalUnits > this.annotationProcessorStartIndex) {
                    ...
                    processCompiledUnits(backup, false);
                }
            }
        } catch (AbortCompilation e) {
        ...
        if (result != null && !result.hasBeenAccepted) {
            ...
            if (abortException.problem != null) {
                recordDistantProblem: {
                    ...
                    if (distantProblem instanceof DefaultProblem) { // fixup filename TODO (philippe) should improve API
        ...
        try {
            if (this.options.sourceLevel != ClassFileConstants.getLatestJDKLevel()) {
                this.problemReporter.abortDueToPreviewEnablingNotAllowed(
                    CompilerOptions.versionFromJdkLevel(this.options.sourceLevel), CompilerOptions.getLatestVersion());
            }
        } catch (AbortCompilation a) {
            // best effort to find a way for reporting this problem: report on the first source
            if (a.compilationResult == null) {
                a.compilationResult
                    = new CompilationResult(sourceUnits[0], 0, maxUnits, this.options.maxProblemsPerUnit);
            }
            throw a;
        }
    }
    ...
    private void abortIfVersionNotAllowed(ICompilationUnit[] sourceUnits, int maxUnits) {
        try {
            ...
            if (this.options.sourceLevel < firstSupportedJdkLevel
                ...
                long badVersion = Math.min(this.options.complianceLevel,
                    Math.min(this.options.sourceLevel, this.options.targetJDK));
    ...
    protected void processAnnotations() {
        ...
        do {
            ...
            if (top < this.totalUnits) {
                length = this.totalUnits - top; // NOTE: Reuse the same variable, but make sure it's not used after this
                                                // point
            ...
            if (newUnitSize != 0) {
                ICompilationUnit[] newProcessedUnits = newUnits.clone(); // remember new units in case a source type
                                                                         // collision occurs
                try {
