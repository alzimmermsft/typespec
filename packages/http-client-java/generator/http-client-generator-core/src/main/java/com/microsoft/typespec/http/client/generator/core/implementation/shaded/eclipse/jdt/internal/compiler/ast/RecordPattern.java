...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.InferenceContext18;
...
public class RecordPattern extends Pattern {
    ...
    public TypeBinding resolveType(BlockScope scope) {
        ...
        if (this.resolvedType.isRawType()) {
            if (this.outerExpressionType instanceof ReferenceBinding) {
                ReferenceBinding binding
                    = inferRecordParameterization(scope, (ReferenceBinding) this.outerExpressionType);
        ...
        RecordComponentBinding[] components
            = this.resolvedType.capture(scope, this.sourceStart, this.sourceEnd).components();
        for (int i = 0; i < components.length; i++) {
    ...
    public void generateCode(BlockScope currentScope, CodeStream codeStream, BranchLabel patternMatchLabel,
        ...
        for (int i = 0; i < length; i++) {
            ...
            ExceptionLabel exceptionLabel
                = new ExceptionLabel(codeStream, TypeBinding.wellKnownType(currentScope, T_JavaLangThrowable));
            ...
            if (p instanceof RecordPattern || !p.isTotalTypeNode) {
                ...
                if (p instanceof TypePattern) {
                    ...
                    codeStream.instance_of(p.resolvedType); // lastComponent ? named ? ([C, boolean] : [R, C, boolean])
                                                            // : ([boolean] : [R, boolean])
                }
                BranchLabel innerTruthLabel = new BranchLabel(codeStream);
                codeStream.ifne(innerTruthLabel); // lastComponent ? named ? ([C] : [R, C]) : ([] : [R])
                int pops = p.isUnnamed() ? 0 : TypeIds.getCategory(componentType.id); // Not going to store into the
                                                                                      // component pattern binding, so
                                                                                      // need to pop, the duped value.
                Pattern current = p;
        ...
        if (p.isTotalTypeNode && !componentType.isPrimitiveType() && p instanceof TypePattern tp) {
            ...
            if (providedType != null && providedType.isPrimitiveType()) {
                PrimitiveConversionRoute route
                    = Pattern.findPrimitiveConversionRoute(componentType, providedType, currentScope);
                if (route != PrimitiveConversionRoute.NO_CONVERSION_ROUTE || !componentType.isPrimitiveType()) {
