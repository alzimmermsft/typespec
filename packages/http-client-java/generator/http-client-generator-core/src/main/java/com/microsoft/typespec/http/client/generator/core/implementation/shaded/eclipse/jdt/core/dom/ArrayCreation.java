...
public class ArrayCreation extends Expression {
     ...
     */
    private final NodeList dimensions = new NodeList(DIMENSIONS_PROPERTY);
    ...
    ASTNode clone0(AST target) {
        ...
        result.setInitializer((ArrayInitializer) ASTNode.copySubtree(target, getInitializer()));
        return result;
    }
    ...
    @Override
    int treeSize() {
        int size = memSize() + (this.arrayType == null ? 0 : getType().treeSize())
            + (this.optionalInitializer == null ? 0 : getInitializer().treeSize()) + this.dimensions.listSize();
        return size;
    }
}
