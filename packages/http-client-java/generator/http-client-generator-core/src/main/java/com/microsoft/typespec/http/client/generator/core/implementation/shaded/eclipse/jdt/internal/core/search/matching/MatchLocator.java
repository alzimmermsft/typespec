...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.search.*;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.env.IBinaryMethod;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.util.HashtableOfIntValues;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.core.hierarchy.HierarchyResolver;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.core.search.IndexSelector;
...
public com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.ICompilationUnit[] workingCopies;
...
public static class WorkingCopyDocument extends JavaSearchDocument {
	public com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.ICompilationUnit workingCopy;
	WorkingCopyDocument(com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.ICompilationUnit workingCopy, SearchParticipant participant) {
...
public static SearchDocument[] addWorkingCopies(SearchPattern pattern, SearchDocument[] indexMatches, com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.ICompilationUnit[] copies, SearchParticipant participant) {
	...
	if (remainingWorkingCopiesSize != 0) {
		...
		int index = length;
		for (WorkingCopyDocument element : workingCopyDocuments.values()) {
 ...
 */
private static HashMap<String, WorkingCopyDocument> workingCopiesThatCanSeeFocus(com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.ICompilationUnit[] copies, SearchPattern pattern, SearchParticipant participant) {
	if (copies == null) return new HashMap<>();
	HashMap<String, WorkingCopyDocument> result = new HashMap<>();
	for (com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.ICompilationUnit workingCopy : copies) {
...
public static IBinaryType classFileReader(IType type) {
	...
	try {
		...
		if (com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.util.Util.isJrt(rootPath)) {
...
protected IJavaElement createHandle(LambdaExpression lambdaExpression, IJavaElement parent) {
	com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.core.LambdaExpression lambdaElement = LambdaFactory.createLambdaExpression((JavaElement) parent, lambdaExpression);
 ...
 */
protected IJavaElement createHandle(AbstractVariableDeclaration fieldDeclaration, TypeDeclaration typeDeclaration, IJavaElement parent) {
	if (!(parent instanceof IType)) return parent;
	IType type = (IType) parent;
...
public void initialize(JavaProject project, int possibleMatchSize) throws JavaModelException {
	...
	map.put(CompilerOptions.OPTION_TaskTags, com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.util.Util.EMPTY_STRING);
...
public void locateMatches(SearchDocument[] searchDocuments) throws CoreException {
	...
	ArrayList<com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.ICompilationUnit> copies = new ArrayList<>();
	for (int i = 0; i < docsLength; i++) {
	...
	try {
		...
		for (int i = 0; i < docsLength; i++) {
			...
			Openable openable;
			com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.ICompilationUnit workingCopy = null;
 ...
 * search pattern (i.e. the ones in the matching nodes set)
 */
protected void reportMatching(AbstractVariableDeclaration variable, FieldDeclaration[] otherFields, TypeDeclaration type, IJavaElement parent, int accuracy, boolean typeInHierarchy, MatchingNodeSet nodeSet) throws CoreException {
	IJavaElement enclosingElement = null;
	if (accuracy > -1) {
		enclosingElement = createHandle(variable, type, parent);
		if (encloses(enclosingElement)) {
			int offset = variable.sourceStart;
			SearchMatch match = newDeclarationMatch(enclosingElement, variable.getBinding(), accuracy, offset, variable.sourceEnd-offset+1);
			if (variable.initialization instanceof AllocationExpression) {
				reportAccurateEnumConstructorReference(match, (FieldDeclaration) variable, (AllocationExpression) variable.initialization);
			} else {
				report(match);
			}
		}
	...
	if (variable.annotations != null) {
		...
		reportMatching(variable.annotations, enclosingElement, otherElements, variable.getBinding(), nodeSet, true, true);
	}
				...
				if ((this.matchContainer & PatternLocator.FIELD_CONTAINER) == 0) {
					...
					if (encloses(enclosingElement)) {
						for (ASTNode node : nodes) {
							...
							this.patternLocator.matchReportReference(node, enclosingElement, null, otherElements, variable.getBinding(), level.intValue(), this);
						}
					}
				}
			}
		...
		for (int i = 0; i < length; i++) {
			...
			if (first >= 0) {
				...
				if (i > first) {
					...
					otherFields[i-1-first] = (FieldDeclaration) variable;
				}
				// On last field, report match with all other elements
				if (last) {
					for (int j=first; j<=i; j++) {
						Integer level = nodeSet.matchingNodes.remove(variables[j]);
						int value = (level != null && matchedClassContainer) ? level.intValue() : -1;
						reportMatching(variables[j], otherFields, type, enclosingElement, value, typeInHierarchy, nodeSet);
					}
					first = -1;
					otherFields = null;
				}
				...
				Integer level = nodeSet.matchingNodes.remove(variable);
				int value = (level != null && matchedClassContainer) ? level.intValue() : -1;
				reportMatching(variable, null, type, enclosingElement, value, typeInHierarchy, nodeSet);
			}
		}
	}
