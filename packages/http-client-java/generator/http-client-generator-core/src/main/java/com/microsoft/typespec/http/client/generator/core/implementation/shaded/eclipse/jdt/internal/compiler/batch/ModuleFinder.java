...
public class ModuleFinder {
    ...
    public static List<Classpath> findModules(File f, String destinationPath, Parser parser,
        Map<String, String> options, boolean isModulepath, String release) {
        List<Classpath> collector = new ArrayList<>();
        scanForModules(destinationPath, parser, options, isModulepath, !f.isDirectory(), collector, f, release);
        return collector;
    }

    protected static Classpath findModule(final File file, String destinationPath, Parser parser,
        Map<String, String> options, boolean isModulepath, String release) {
        Classpath modulePath = FileSystem.getClasspath(file.getAbsolutePath(), null, !isModulepath, null,
    ...
    protected static void scanForModules(String destinationPath, Parser parser, Map<String, String> options,
        boolean isModulepath, boolean thisAnAutomodule, List<Classpath> collector, final File file, String release) {
        Classpath entry = FileSystem.getClasspath(file.getAbsolutePath(), null, !isModulepath, null,
            destinationPath == null ? null : (destinationPath + File.separator + file.getName()), options, release);
        if (entry != null) {
    ...
    protected static IModule scanForModule(Classpath modulePath, final File file, Parser parser,
        boolean considerAutoModules, String release) {
        IModule module = null;
        if (file.isDirectory()) {
            String[] list = file.list(new FilenameFilter() {
                ...
                public boolean accept(File dir, String name) {
                    if (dir == file
                        && (name.equalsIgnoreCase(IModule.MODULE_INFO_CLASS)
                    ...
                    case IModule.MODULE_INFO_JAVA:
                        ...
                        if (!modName.equals(file.getName())) {
                            throw new IllegalArgumentException(
                                "module name " + modName + " does not match expected name " + file.getName()); //$NON-NLS-1$ //$NON-NLS-2$
                        }
                        break;
                }
            }
    ...
    static class AddExport {
        ...
        public final IPackageExport export;
