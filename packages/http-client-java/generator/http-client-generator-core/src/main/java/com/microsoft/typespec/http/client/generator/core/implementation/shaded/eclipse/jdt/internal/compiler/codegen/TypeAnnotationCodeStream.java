...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.ArrayAllocationExpression;
...
public class TypeAnnotationCodeStream extends StackMapFrameCodeStream {
    ...
    public void checkcast(TypeReference typeReference, TypeBinding typeBinding, int currentPosition) {
        /*
         * We use a slightly sub-optimal generation for intersection casts by resorting to a runtime cast for every
         * intersecting type, but in
         * reality this should not matter. In its intended use form such as (I & Serializable) () -> {}, no cast is
         * emitted at all. Also note
    ...
    public void invokeDynamic(int bootStrapIndex, int argsSize, int returnTypeSize, char[] selector, char[] signature,
        ...
        if (lhsTypeReference != null && (lhsTypeReference.bits & ASTNode.HasTypeAnnotations) != 0) {
            if (isConstructorReference) {
                addAnnotationContext(lhsTypeReference, this.position, 0,
                    AnnotationTargetTypeConstants.CONSTRUCTOR_REFERENCE);
            } else {
                addAnnotationContext(lhsTypeReference, this.position, 0,
                    AnnotationTargetTypeConstants.METHOD_REFERENCE);
            }
        }
        if (typeArguments != null) {
            int targetType = isConstructorReference
        ...
        super.invokeDynamic(bootStrapIndex, argsSize, returnTypeSize, selector, signature, isConstructorReference,
            lhsTypeReference, typeArguments, type);
    }
