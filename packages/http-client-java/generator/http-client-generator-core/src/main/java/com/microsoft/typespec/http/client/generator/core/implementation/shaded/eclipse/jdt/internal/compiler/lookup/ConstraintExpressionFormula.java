...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.InferenceContext18.SuspendedInferenceRecord;
...
class ConstraintExpressionFormula extends ConstraintFormula {
    ...
    public Object reduce(InferenceContext18 inferenceContext) throws InferenceFailureException {
        ...
        if (this.relation == POTENTIALLY_COMPATIBLE) {
            /*
             * 15.12.2.1: ... The definition of potential applicability goes beyond a basic arity check to also take
             * into account the presence and "shape" of functional interface
             * target types. In some cases involving type argument inference, a lambda expression appearing as a method
             * invocation argument cannot be properly typed until after
             * overload resolution. These rules allow the form of the lambda expression to still be taken into account,
             * discarding obviously incorrect target types that might
        ...
        if (!canBePolyExpression(this.left)) {
            ...
            if (exprType == null || !exprType.isValidBinding()) {
                if (this.left instanceof MessageSend
                    && ((MessageSend) this.left).actualReceiverType instanceof InferenceVariable)
            ...
            if (this.left instanceof Invocation) {
                ...
                SuspendedInferenceRecord prevInvocation
                    = inferenceContext.enterPolyInvocation(invocation, invocation.arguments());
                ...
                try {
                    ...
                    TypeBinding[] argumentTypes
                        = arguments == null ? Binding.NO_PARAMETERS : new TypeBinding[arguments.length];
                    ...
                    if (previousMethod instanceof ParameterizedGenericMethodBinding) {
                        ...
                        if (innerCtx == null) {
                            /*
                             * No inference context -> the method was likely manufactured by Scope.findExactMethod ->
                             * assume it wasn't really poly after all.
                        ...
                        inferenceContext.inferenceKind
                            = inferenceContext.getInferenceKind(previousMethod, argumentTypes);
                        boolean isDiamond = method.isConstructor() && this.left.isPolyExpression(method);
                        inferInvocationApplicability(inferenceContext, method, argumentTypes, isDiamond,
                            inferenceContext.inferenceKind);
                        // b2 has been lifted, inferring poly invocation type amounts to lifting b3.
                    }
                ...
                if (lambda == null)
                    return FALSE; // not strictly unreduceable, but proceeding with TRUE would likely produce secondary
                                  // errors
    ...
    public static ReferenceBinding findGroundTargetType(InferenceContext18 inferenceContext, BlockScope scope,
        ...
        if (lambda.argumentsTypeElided()) {
            ...
            try {
                return inferenceContext.inferFunctionalInterfaceParameterization(lambda, scope,
    ...
    private Object reduceReferenceExpressionCompatibility(ReferenceExpression reference,
        ...
        if (reference.isExactMethodReference()) {
            ...
            if (r != TypeBinding.VOID) {
                TypeBinding rAppl = potentiallyApplicable.isConstructor() && !reference.isArrayConstructorReference()
            ...
            // Note: we currently don't distinguish search for a potentially-applicable method from searching the
            ...
            if (needsInference(reference, original)) {
                ...
                if (t.isParameterizedType()) {
                    MethodBinding capturedFunctionType = ((ParameterizedTypeBinding) t).getSingleAbstractMethod(
                        inferenceContext.scope, true, reference.sourceStart, reference.sourceEnd);
                    argumentTypes = capturedFunctionType.parameters;
                } else {
                    argumentTypes = functionType.parameters;
                }
                SuspendedInferenceRecord prevInvocation
                    = inferenceContext.enterPolyInvocation(reference, reference.createPseudoExpressions(argumentTypes));
                ...
                try {
                    ...
                    inferInvocationApplicability(inferenceContext, original, argumentTypes,
                        original.isConstructor()/* mimic a diamond? */, innerInferenceKind);
            ...
            TypeBinding rPrime = compileTimeDecl.isConstructor()
                ...
                : compileTimeDecl.returnType.capture(inferenceContext.scope, reference.sourceStart(),
                    reference.sourceEnd());
    ...
    private int determineInferenceKind(MethodBinding original, TypeBinding[] argumentTypes,
    ...
    static void inferInvocationApplicability(InferenceContext18 inferenceContext, MethodBinding method,
        TypeBinding[] arguments, boolean isDiamond, int checkType) {
        // 18.5.1
        TypeVariableBinding[] typeVariables = method.getAllTypeVariables(isDiamond);
        InferenceVariable[] inferenceVariables = inferenceContext.createInitialBoundSet(typeVariables); // creates
                                                                                                        // initial bound
                                                                                                        // set B
        ...
        inferenceContext.createInitialConstraintsForParameters(method.parameters,
            checkType == InferenceContext18.CHECK_VARARG, varArgsType, method);
    ...
    static boolean inferPolyInvocationType(InferenceContext18 inferenceContext, InvocationSite invocationSite,
        ...
        if (typeArguments == null) {
            ...
            if (parameterizedType != null && parameterizedType.arguments != null) {
                ...
                ParameterizedTypeBinding gbeta
                    = inferenceContext.environment.createParameterizedType(parameterizedType.genericType(), betas,
                        parameterizedType.enclosingType(), parameterizedType.getTypeAnnotations());
                inferenceContext.currentBounds.captures.put(gbeta, parameterizedType); // established: both types have
                                                                                       // nonnull arguments
                        ...
                        if (arguments[i].isWildcard()) {
                            ...
                            switch (wc.boundKind) {
                                case Wildcard.EXTENDS:
                                    inferenceContext.currentBounds.addBound(
                                        new TypeBound(betas[i], wc.bound(), SUBTYPE), inferenceContext.environment);
                                    break;

                                case Wildcard.SUPER:
                                    inferenceContext.currentBounds.addBound(
                                        new TypeBound(betas[i], wc.bound(), SUPERTYPE), inferenceContext.environment);
                                    break;
                            }
                        }
                    }
            ...
            if (rTheta.leafComponentType() instanceof InferenceVariable) { // https://bugs.openjdk.java.net/browse/JDK-8062082
                ...
                if (inferenceContext.currentBounds.condition18_5_2_bullet_3_3_1(alpha, targetLeafType)) {
                    toResolve = true;
                } else if (inferenceContext.currentBounds.condition18_5_2_bullet_3_3_2(alpha, targetLeafType,
                    inferenceContext)) {
                    toResolve = true;
    ...
    Collection<InferenceVariable> inputVariables(final InferenceContext18 context) {
        ...
        if (this.left instanceof LambdaExpression) {
            ...
            if (this.right.isFunctionalInterface(context.scope)) {
                ...
                if (withWildCards != null) {
                    targetType = ConstraintExpressionFormula.findGroundTargetType(context, lambda.enclosingScope,
                        lambda, withWildCards);
                }
                if (targetType == null) {
                ...
                if (sam.returnType != TypeBinding.VOID) {
                    ...
                    for (int i = 0, length = resultExpressions == null ? 0 : resultExpressions.length; i < length;
                        i++) {
                        variables.addAll(new ConstraintExpressionFormula(resultExpressions[i], r, COMPATIBLE)
            ...
            variables.addAll(
                new ConstraintExpressionFormula(expr.valueIfTrue, this.right, COMPATIBLE).inputVariables(context));
            variables.addAll(
                new ConstraintExpressionFormula(expr.valueIfFalse, this.right, COMPATIBLE).inputVariables(context));
            return variables;
