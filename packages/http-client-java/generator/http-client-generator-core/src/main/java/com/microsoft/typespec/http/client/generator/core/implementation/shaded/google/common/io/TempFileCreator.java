...
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.google.common.base.StandardSystemProperty.JAVA_IO_TMPDIR;
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.google.common.base.StandardSystemProperty.USER_NAME;
...
abstract class TempFileCreator {
    ...
    private static TempFileCreator pickSecureCreator() {
        ...
        try {
            ...
            int jellyBean = (int) Class.forName("android.os.Build$VERSION_CODES").getField("JELLY_BEAN").get(null);
            /*
    ...
    private static final class JavaNioCreator extends TempFileCreator {
        ...
        File createTempDir() {
            try {
                return java.nio.file.Files
                    .createTempDirectory(Paths.get(JAVA_IO_TMPDIR.value()), /* prefix= */ null,
                        directoryPermissions.get())
                    .toFile();
            } catch (IOException e) {
        ...
        File createTempFile(String prefix) throws IOException {
            return java.nio.file.Files
                .createTempFile(Paths.get(JAVA_IO_TMPDIR.value()), /* prefix= */ prefix, /* suffix= */ null,
                    filePermissions.get())
                .toFile();
        }
        ...
        private static PermissionSupplier userPermissions() {
            try {
                UserPrincipal user
                    = FileSystems.getDefault().getUserPrincipalLookupService().lookupPrincipalByName(getUsername());
                ImmutableList<AclEntry> acl = ImmutableList.of(AclEntry.newBuilder()
                    .setType(ALLOW)
                    .setPrincipal(user)
    ...
    private static final class JavaIoCreator extends TempFileCreator {
        ...
        @Override
        File createTempFile(String prefix) throws IOException {
            return File.createTempFile(/* prefix= */ prefix, /* suffix= */ null,
