...
public class BindingKeyParser {
    ...
    static class Scanner {
        ...
        boolean isAtParametersStart() {
            ...
            return this.index > 0
                ...
                && ((currentChar = this.source[this.index]) == '<' || currentChar == '%');
        }
        ...
        boolean isAtWildcardStart() {
            return this.index < this.source.length && this.source[this.index] == '{';   // e.g {1}+Ljava/lang/String;
        }
        ...
        int nextToken() {
            ...
            while (this.index <= length) {
                ...
                switch (currentChar) {
                    ...
                    case 'Z':
                        // base type
                        if (this.index == previousTokenEnd && (this.index == 0 || this.source[this.index - 1] != '.')) { // case
                                                                                                                         // of
                                                                                                                         // field
                                                                                                                         // or
                                                                                                                         // method
    ...
    public void parse(boolean pauseAfterFullyQualifiedName) {
        ...
        if (this.scanner.isAtParametersStart()) {
            ...
            if (this.scanner.isAtTypeParameterStart()) {
                ...
                // parameterized type
                parseParameterizedType(null/* top level type or member type with raw enclosing type */,
                    false/* no raw */);
    ...
    private boolean parseModule() {
        if (this.scanner.isAtModuleStart()) {
            ...
            this.keyStart = 1;
            if (this.scanner.nextToken() == Scanner.MODULE && this.scanner.nextToken() == Scanner.END) {
    ...
    private void parseFullyQualifiedName() {
        ...
        switch (this.scanner.nextToken()) {
            ...
            case Scanner.WILDCARD:
                ...
                if (!CharOperation.endsWith(this.scanner.getTokenSource(),
                    new char[] { '/', 'p', 'a', 'c', 'k', 'a', 'g', 'e', '-' })) {
                    malformedKey();
                    return;
                }
    ...
    private void parseInnerType() {
        ...
        if (Character.isDigit(typeName[0])) {
            ...
            typeName = nextToken == Scanner.END
                ? this.scanner.source
                : CharOperation.subarray(this.scanner.source, this.keyStart, this.scanner.index + 1);
    ...
    private void parseCapture18() {
        // syntax: ^{int#int}
        if (this.scanner.nextToken() != Scanner.CAPTURE18)
            return;
