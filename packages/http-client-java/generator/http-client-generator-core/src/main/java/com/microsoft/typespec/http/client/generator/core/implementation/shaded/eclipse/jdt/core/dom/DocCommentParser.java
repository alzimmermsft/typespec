...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.AbstractCommentParser;
...
class DocCommentParser extends AbstractCommentParser {
     ...
     *
     * If annotation checking is enabled, will also construct an Annotation node, which will be stored into
     * Parser.annotation
            ...
            if (name != null) {
                for (JavaDocRegion region : regions) {
                    if (name.equals(region.getTagName())) {
                        if (!this.isRegionToBeEnded(region) && !this.hasRegionEnded(region)) {
...
//				typeRef.setSourceRange(start, end-start+1);
                    ...
                    if (peekChar() == '[') {
                        ...
                        if (size == 0) {
                            ...
                            if (lastFragment.getNodeType() == ASTNode.TAG_ELEMENT) {
                                lastFragment.setSourceRange(lastFragment.getStartPosition(),
                ...
                switch (tagName[0]) {
                    case 'c':
                        if (length == TAG_CATEGORY_LENGTH && CharOperation.equals(TAG_CATEGORY, tagName)) {
                            ...
                            valid = parseIdentifierTag(false); // TODO (frederic) reconsider parameter value when
                                                               // @category will be significant in spec
    ...
    private void pushSnippetText(char[] text, int start, int end, boolean addNewLine, Object snippetTag,
        boolean isExternalSnippet) {
        // Create text element
    ...
    protected boolean pushThrowName(Object typeRef) {
        ...
        throwsTag.setSourceRange(this.tagSourceStart,
            this.scanner.getCurrentTokenEndPosition() - this.tagSourceStart + 1);
        ...
        if (size > 1) {
            ...
            if (lastNode instanceof TagElement lastTag) {
                if (!lastTag.getTagName().equals(TagElement.TAG_RETURN)) {
                    ...
                    if (secondLastNode instanceof TagElement prevTag
                        && prevTag.getTagName().equals(TagElement.TAG_RETURN)) {
                        fragments.remove(size - 1);
                        prevTag.fragments().add(lastNode);
                        this.inlineTagStart = prevTag.getStartPosition();
                        this.inlineTagStarted = true;
                        prevTag.setSourceRange(prevTag.getStartPosition(),
                            lastNode.getStartPosition() + lastNode.getLength() - prevTag.getStartPosition());
                    }
                } else {
