...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.core.util.CharArrayBuffer;
...
//	protected int fInsertionPosition;
     ...
     */
    DOMType(char[] document, int[] sourceRange, String name, int[] nameRange, int[] commentRange, int flags,
        int[] modifierRange, int[] typeRange, int[] superclassRange, int[] extendsRange, String[] implementsList,
        int[] implementsRange, int[] implementsKeywordRange, int[] openBodyRange, int[] closeBodyRange,
        boolean isClass) {
     ...
     * @param isClass - true is the type is a class, false if it is an interface
     */
    DOMType(char[] document, int[] sourceRange, String name, int[] nameRange, int flags, String[] implementsList,
        boolean isClass) {
        this(document, sourceRange, name, nameRange, new int[] { -1, -1 }, flags, new int[] { -1, -1 },
            new int[] { -1, -1 }, new int[] { -1, -1 }, new int[] { -1, -1 }, implementsList, new int[] { -1, -1 },
            new int[] { -1, -1 }, new int[] { -1, -1 }, new int[] { sourceRange[1], sourceRange[1] }, isClass);
        ...
        if (isClass()) {
            ...
            if (getMask(MASK_TYPE_HAS_SUPERCLASS)) {
                if (this.fExtendsRange[0] < 0) {
                    ...
                    buffer.append(this.fDocument, this.fExtendsRange[0],
                        this.fExtendsRange[1] + 1 - this.fExtendsRange[0]);
                }
                if (this.fSuperclass != null) {
                    buffer.append(this.fSuperclass);
                } else {
                    buffer.append(this.fDocument, this.fSuperclassRange[0],
                        this.fSuperclassRange[1] + 1 - this.fSuperclassRange[0]);
                }
            }
            if (getMask(MASK_TYPE_HAS_INTERFACES)) {
                ...
                if (this.fImplementsRange[0] < 0) {
                    ...
                    buffer.append(this.fDocument, this.fImplementsRange[0],
                        this.fImplementsRange[1] + 1 - this.fImplementsRange[0]);
                }
                if (this.fInterfaces != null) {
                    buffer.append(this.fInterfaces);
                } else {
                    buffer.append(this.fDocument, this.fInterfacesRange[0],
                        this.fInterfacesRange[1] + 1 - this.fInterfacesRange[0]);
                }
            }
            if (hasInterfaces) {
                if (this.fImplementsRange[0] < 0) {
                    ...
                    buffer.append(this.fDocument, this.fInterfacesRange[1] + 1,
                        this.fOpenBodyRange[0] - this.fInterfacesRange[1] - 1);
                }
            } else {
                if (this.fSuperclassRange[0] < 0) {
                    buffer.append(' ');
                } else if (this.fImplementsRange[0] > 0) {
                    buffer.append(this.fDocument, this.fSuperclassRange[1] + 1,
                        this.fImplementsRange[0] - this.fSuperclassRange[1] - 1);
                    buffer.append(this.fDocument, this.fInterfacesRange[1] + 1,
                        this.fOpenBodyRange[0] - this.fInterfacesRange[1] - 1);
                } else {
                    buffer.append(this.fDocument, this.fSuperclassRange[1] + 1,
                        this.fOpenBodyRange[0] - this.fSuperclassRange[1] - 1);
                }
            }
        } else {
            if (getMask(MASK_TYPE_HAS_INTERFACES)) {
                ...
                if (this.fImplementsRange[0] < 0) {
                    ...
                    buffer.append(this.fDocument, this.fNameRange[1] + 1,
                        this.fOpenBodyRange[0] - this.fNameRange[1] - 1);
                }
            }
        }
    ...
    public String getSuperclass() {
        ...
        if (getMask(MASK_TYPE_HAS_SUPERCLASS)) {
            if (this.fSuperclass != null) {
                ...
                return new String(this.fDocument, this.fSuperclassRange[0],
                    this.fSuperclassRange[1] + 1 - this.fSuperclassRange[0]);
            }
        } else {
            return null;
    ...
    void normalize(ILineStartFinder finder) {
        ...
        try {
            ...
            while (currentToken != TerminalToken.TokenNameLBRACE && currentToken != TerminalToken.TokenNameEOF) {
        ...
        if (first != null) {
            ...
            try {
                TerminalToken currentToken = scanner.getNextToken();
                while (currentToken != TerminalToken.TokenNameRBRACE && currentToken != TerminalToken.TokenNameEOF) {
                    currentToken = scanner.getNextToken();
                }
    ...
    /**
     * @see IDOMType#setTypeParameters(String[])
     * @since 3.0
     */
    @Override
