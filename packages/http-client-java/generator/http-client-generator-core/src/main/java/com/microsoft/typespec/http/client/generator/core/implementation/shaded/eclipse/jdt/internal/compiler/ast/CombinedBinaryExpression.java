        ...
        if (this.referencesTable == null) {
            ...
            if ((((this.bits & ASTNode.OperatorMASK) >> ASTNode.OperatorSHIFT) == OperatorIds.PLUS)
                ...
                if (this.constant != Constant.NotAConstant) {
                    ...
                    codeStream
                        .invokeStringConcatenationAppendForType(this.implicitConversion & TypeIds.COMPILE_TYPE_MASK);
                } else {
            ...
            if ((((this.bits & ASTNode.OperatorMASK) >> ASTNode.OperatorSHIFT) == OperatorIds.PLUS)
                ...
                for (restart = this.arity - 1; restart >= 0; restart--) {
                    if (((((cursor = this.referencesTable[restart]).bits & ASTNode.OperatorMASK)
                        ...
                        cursor.buildStringForConcatenation(blockScope, codeStream,
                            cursor.implicitConversion & TypeIds.COMPILE_TYPE_MASK, recipe, argTypes);
                        break;
                    }
                }
                restart++;
                if (restart == 0) { // reached the leftmost expression
                    cursor.left.buildStringForConcatenation(blockScope, codeStream,
                        cursor.left.implicitConversion & TypeIds.COMPILE_TYPE_MASK, recipe, argTypes);
                }
                for (int i = restart; i < this.arity; i++) {
                    cursor = this.referencesTable[i];
                    cursor.right.buildStringForConcatenation(blockScope, codeStream,
                        cursor.right.implicitConversion & TypeIds.COMPILE_TYPE_MASK, recipe, argTypes);
                }
                this.right.buildStringForConcatenation(blockScope, codeStream,
                    this.right.implicitConversion & TypeIds.COMPILE_TYPE_MASK, recipe, argTypes);
            } else {
                super.buildStringForConcatenation(blockScope, codeStream, typeID, recipe, argTypes);
            }
        }
            ...
            if ((((this.bits & ASTNode.OperatorMASK) >> ASTNode.OperatorSHIFT) == OperatorIds.PLUS)
                ...
                for (restart = this.arity - 1; restart >= 0; restart--) {
                    if (((((cursor = this.referencesTable[restart]).bits & ASTNode.OperatorMASK)
                        ...
                        cursor.generateOptimizedStringConcatenationCreation(blockScope, codeStream,
                            cursor.implicitConversion & TypeIds.COMPILE_TYPE_MASK);
                        break;
                    }
                }
