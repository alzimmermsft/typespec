...
public class NullAnnotationMatching {
    ...
    public static final NullAnnotationMatching NULL_ANNOTATIONS_OK
        = new NullAnnotationMatching(Severity.OK, FlowInfo.UNKNOWN, null);
    public static final NullAnnotationMatching NULL_ANNOTATIONS_OK_NONNULL
        = new NullAnnotationMatching(Severity.OK, FlowInfo.NON_NULL, null);
    public static final NullAnnotationMatching NULL_ANNOTATIONS_UNCHECKED
        = new NullAnnotationMatching(Severity.UNCHECKED, FlowInfo.UNKNOWN, null);
    public static final NullAnnotationMatching NULL_ANNOTATIONS_MISMATCH
        = new NullAnnotationMatching(Severity.MISMATCH, FlowInfo.UNKNOWN, null);
    ...
    public enum CheckMode {
        ...
        BOUND_CHECK,
        /**
         * similar to COMPATIBLE, but for type variables we look for instantiations, rather than treating them as "free
         * type variables".
         */
        BOUND_SUPER_CHECK,
    ...
    public NullAnnotationMatching withNullStatus(int updatedNullStatus) {
        return updatedNullStatus == this.nullStatus
            ? this
    ...
    public static int checkAssignment(BlockScope currentScope, FlowContext flowContext, VariableBinding var,
        ...
        if (!usesNullTypeAnnotations) {
            ...
            if (expression instanceof ConditionalExpression && expression.isPolyExpression()) {
                ...
                for (int i = 1, l = resExprs.length; i < l; ++i) {
                    re = resExprs[i];
                    int otherStatus = NullAnnotationMatching.checkAssignment(currentScope, flowContext, var, flowInfo,
                        re.nullStatus(flowInfo, flowContext), re, re.resolvedType);
                    identicalStatus &= status0 == otherStatus;
                }
                return identicalStatus ? status0 : nullStatus; // if not all branches agree use the precomputed & merged
                                                               // nullStatus
            }
            ...
            NullAnnotationMatching annotationStatus = analyse(var.type, providedType, null, null, nullStatus,
                expression, CheckMode.COMPATIBLE, var.kind() == Binding.LOCAL);
        ...
        } else if (lhsTagBits == TagBits.AnnotationNullable && nullStatus == FlowInfo.UNKNOWN) {	// provided a legacy
                                                                                                	// type?
            if (usesNullTypeAnnotations
                && providedType.isTypeVariable()
                && (providedType.tagBits & TagBits.AnnotationNullMASK) == 0)
                return FlowInfo.POTENTIALLY_NULL | FlowInfo.POTENTIALLY_NON_NULL;		// -> free type variable can
                                                                                 		// mean either nullable or
                                                                                 		// nonnull
            return FlowInfo.POTENTIALLY_NULL | FlowInfo.POTENTIALLY_UNKNOWN;			// -> combine info from lhs &
                                                                            			// rhs
        }
        return nullStatus;
    }
     ...
     * 
     * @param nullStatus we are only interested in NULL or NON_NULL, -1 indicates that we are in a recursion, where flow
     * info is ignored
     ...
     * Find any mismatches between the two given types, which are caused by null type annotations.
     * 
     * @param providedSubstitute in inheritance situations this maps the providedType into the realm of the subclass,
     * needed for TVB identity checks.
    ...
    public static NullAnnotationMatching analyse(TypeBinding requiredType, TypeBinding providedType,
        ...
        CheckMode mode) {
        return analyse(requiredType, providedType, providedSubstitute, substitution, nullStatus, providedExpression,
            mode, true);
    }
    ...
    public static NullAnnotationMatching analyse(TypeBinding requiredType, TypeBinding providedType,
        TypeBinding providedSubstitute, Substitution substitution, int nullStatus, Expression providedExpression,
        CheckMode mode, boolean localFlow) {
        ...
        try {
            ...
            if (requiredType instanceof TypeVariableBinding
                && substitution != null
                && (mode == CheckMode.EXACT || mode == CheckMode.COMPATIBLE || mode == CheckMode.BOUND_SUPER_CHECK)) {
            ...
            if (mode == CheckMode.BOUND_CHECK && requiredType instanceof TypeVariableBinding) {
                ...
                if (!passedBoundCheck) {
                    ...
                    if (superClass != null && (superClass.hasNullTypeAnnotations() || substitution != null)) { // annotations
                                                                                                               // may
                                                                                                               // enter
                                                                                                               // when
                                                                                                               // substituting
                    ...
                    if (superInterfaces != null) {
                        for (TypeBinding superInterface : superInterfaces) {
                            if (superInterface.hasNullTypeAnnotations() || substitution != null) { // annotations may
                                                                                                   // enter when
                                                                                                   // substituting a
                                                                                                   // nested type
                                                                                                   // variable
                                NullAnnotationMatching status = analyse(superInterface, providedType, null,
            ...
            if (requiredType instanceof ArrayBinding) {
                ...
                if (requiredDimsTagBits != null) {
                    ...
                    if (requiredType.dimensions() == providedType.dimensions()) {
                        ...
                        for (int i = 0; i <= dims; i++) {
                            ...
                            if (i == 0
                                && requiredBits == TagBits.AnnotationNullable
                                && nullStatus != -1
                                ...
                                Severity dimSeverity = computeNullProblemSeverity(requiredBits, providedBits,
                                    currentNullStatus, i == 0 ? mode : mode.toDetail(), null);
                                if (i > 0
                                    && dimSeverity == Severity.UNCHECKED
                                    && providedExpression instanceof ArrayAllocationExpression
                                    && providedBits == 0
                                    && requiredBits != 0) {
                                    Expression[] dimensions
                                        = ((ArrayAllocationExpression) providedExpression).dimensions;
            ...
            } else if (requiredType.hasNullTypeAnnotations()
                ...
                if (requiredBits == TagBits.AnnotationNullable
                    ...
                    if (!severity.isAnyMismatch()
                        ...
                        okStatus = okNonNullStatus(providedExpression,
                            localFlow || requiredBits == TagBits.AnnotationNonNull);
                }
                if (severity != Severity.MISMATCH && nullStatus != FlowInfo.NULL) {  // null value has no details
                    ...
                    if (requiredType.isParameterizedType() && providedSuper instanceof ParameterizedTypeBinding) { // TODO(stephan):
                                                                                                                   // handle
                                                                                                                   // providedType.isRaw()
                        ...
                        TypeBinding[] providedSubstitutes
                            = (providedSubstituteSuper instanceof ParameterizedTypeBinding)
                        ...
                        if (requiredArguments != null
                            ...
                            for (int i = 0; i < requiredArguments.length; i++) {
                                TypeBinding providedArgSubstitute
                                    = providedSubstitutes != null ? providedSubstitutes[i] : null;
                    ...
                    if (requiredEnclosing != null && providedEnclosing != null) {
                        TypeBinding providedEnclSubstitute
                            = providedSubstitute != null ? providedSubstitute.enclosingType() : null;
    ...
    public static NullAnnotationMatching okNonNullStatus(final Expression providedExpression,
        boolean warnUnsafeInterpretation) {
            ...
            if (method != null && method.isValidBinding()) {
                ...
                if (warnUnsafeInterpretation
                    ...
                    final int severity = ((BinaryTypeBinding) originalDeclaringClass).externalAnnotationStatus
                        == ExternalAnnotationStatus.NO_EEA_FILE ? ProblemSeverities.Warning : ProblemSeverities.Info; // reduce
                                                                                                                      // severity
                                                                                                                      // if
                                                                                                                      // not
                                                                                                                      // configured
                    ...
                    return new NullAnnotationMatching(Severity.LEGACY_WARNING, FlowInfo.UNKNOWN, null) {
                        ...
                        public void report(Scope scope) {
                            scope.problemReporter()
                                .nonNullTypeVariableInUnannotatedBinary(scope.environment(), method, providedExpression,
                                    severity);
                        }
                    };
                }
    ...
    /**
     * Are both types identical wrt the unannotated type and any null type annotations? Only unstructured types and
        ...
        if (TypeBinding.notEquals(requiredType, providedType)) {
            if (requiredType instanceof CaptureBinding) {
                ...
                if (lowerBound != null && areSameTypes(lowerBound, providedType, providedSubstitute))
                    return (requiredType.tagBits & TagBits.AnnotationNullMASK)
                        == (providedType.tagBits & TagBits.AnnotationNullMASK);
    ...
    /**
     * Provided that both types are {@link TypeBinding#equalsEquals}, return the one that is more likely to show null at
     * runtime.
     */
     ...
     * @param requiredType the required type, used, e.g., to check if it is a type variable (possibly: "free type
     * variable")?
    ...
    private static Severity computeNullProblemSeverity(long requiredBits, long providedBits, int nullStatus,
        CheckMode mode, TypeBinding requiredType) {
                ...
                if (requiredType instanceof WildcardBinding) {
                    ...
                    if (wildcard.boundKind == Wildcard.SUPER && providedBits == TagBits.AnnotationNonNull) {
                        ...
                        if (bound != null && (bound.tagBits & TagBits.AnnotationNullMASK) != 0)
                            return Severity.OK; // when the wildcard is annotated via its bound, there is not
                                                // annotated->unannotated conversion
                        return Severity.UNCHECKED_TO_UNANNOTATED;
                    }
                }
            }
    ...
    public static MethodBinding checkForContradictions(MethodBinding method, Object location, Scope scope) {
        ...
        if (searchContradiction.typeWithContradiction != null) {
            ...
            scope.problemReporter()
                .contradictoryNullAnnotationsInferred(method, start, end, location instanceof FunctionalExpression);
        ...
        for (int i = 0; i < method.parameters.length; i++) {
            ...
            if (searchContradiction.typeWithContradiction != null) {
                ...
                if (arguments != null && i < arguments.length)
                    ...
                    scope.problemReporter()
                        .contradictoryNullAnnotationsInferred(method, start, end,
    ...
    public static TypeBinding strongerType(TypeBinding type1, TypeBinding type2, LookupEnvironment environment) {
        ...
        return mergeTypeAnnotations(type2, type1, true, environment); // don't bother to distinguish unannotated vs.
                                                                      // @Nullable, since both can accept null
    }
    ...
    public static TypeBinding[] weakerTypes(TypeBinding[] parameters1, TypeBinding[] parameters2,
        ...
        for (int i = 0; i < newParameters.length; i++) {
            ...
            if ((tagBits1 & TagBits.AnnotationNullable) != 0)
                newParameters[i] = mergeTypeAnnotations(parameters1[i], parameters2[i], true, environment);		// @Nullable
                                                                                                           		// must
                                                                                                           		// be
                                                                                                           		// preserved
            else if ((tagBits2 & TagBits.AnnotationNullable) != 0)
                newParameters[i] = mergeTypeAnnotations(parameters2[i], parameters1[i], true, environment);		// @Nullable
                                                                                                           		// must
                                                                                                           		// be
                                                                                                           		// preserved
            else if ((tagBits1 & TagBits.AnnotationNonNull) == 0)
                newParameters[i] = mergeTypeAnnotations(parameters1[i], parameters2[i], true, environment);		// unannotated
                                                                                                           		// must
                                                                                                           		// be
                                                                                                           		// preserved
            else
                newParameters[i] = mergeTypeAnnotations(parameters2[i], parameters1[i], true, environment);		// either
                                                                                                           		// unannotated,
                                                                                                           		// or
                                                                                                           		// both
                                                                                                           		// are
    ...
    public String toString() {
        ...
        if (this == NULL_ANNOTATIONS_MISMATCH)
            return "MISMATCH"; //$NON-NLS-1$
        if (this == NULL_ANNOTATIONS_OK_NONNULL)
            return "OK NonNull"; //$NON-NLS-1$
