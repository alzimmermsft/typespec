...
public class RawTypeBinding extends ParameterizedTypeBinding {
    ...
    public RawTypeBinding(ReferenceBinding type, ReferenceBinding enclosingType, LookupEnvironment environment) {
        ...
        if (enclosingType == null
            || !this.hasEnclosingInstanceContext()
    ...
    public char[] computeUniqueKey(boolean isLeaf) {
        ...
        if (isMemberType() && (enclosingType().isParameterizedType() || enclosingType().isRawType())) {
            ...
            if (genericType().typeVariables() != Binding.NO_TYPE_VARIABLES) // don't show <> if raw only due to a
                                                                            // generic enclosing
                sig.append('<').append('>');
            sig.append(';');
        } else {
    ...
    public TypeBinding withoutToplevelNullAnnotation() {
        if (!hasNullTypeAnnotations())
            return this;
        ReferenceBinding unannotatedGenericType
            = (ReferenceBinding) this.environment.getUnannotatedType(this.genericType());
        AnnotationBinding[] newAnnotations = this.environment.filterNullTypeAnnotations(this.typeAnnotations);
        return this.environment.createRawType(unannotatedGenericType, this.enclosingType(), newAnnotations);
    }

    /**
     * @see org.eclipse.jdt.internal.compiler.lookup.ParameterizedTypeBinding#createParameterizedMethod(MethodBinding)
     */
    @Override
    ...
    protected void initializeArguments() {
        ...
        for (int i = 0; i < length; i++) {
            // perform raw conversion on variable upper bound - could cause infinite regression if arguments were
            // initialized lazily
