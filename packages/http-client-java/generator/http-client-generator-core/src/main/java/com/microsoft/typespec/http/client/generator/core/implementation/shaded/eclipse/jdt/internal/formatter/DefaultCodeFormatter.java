...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.Comment;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.core.dom.TypeDeclaration;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.core.SourceModule;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.formatter.linewrap.CommentWrapExecutor;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.formatter.linewrap.WrapPreparator;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jface.text.IRegion;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jface.text.Region;
...
public class DefaultCodeFormatter extends CodeFormatter {
    ...
    public TextEdit format(int kind, String source, IRegion[] regions, int indentationLevel, String lineSeparator) {
        ...
        TextEditsBuilder resultBuilder
            = new TextEditsBuilder(this.sourceString, this.formatRegions, this.tokenManager, this.workingOptions);
    ...
    private void findHeader() {
        if (this.astRoot instanceof CompilationUnit unit) {
            ...
            ASTNode firstElement
                = !types.isEmpty() ? types.get(0) : unit.getModule() != null ? unit.getModule() : unit.getPackage();
    ...
    private TextEdit formatComments(String source, int kind) {
        ...
        CommentsPreparator commentsPreparator
            = new CommentsPreparator(this.tokenManager, this.workingOptions, this.sourceLevel);
        ...
        TextEditsBuilder resultBuilder
            = new TextEditsBuilder(source, this.formatRegions, this.tokenManager, this.workingOptions);
    ...
    private void assertRegionPreconditions(IRegion[] regions, int maxLength) {
        ...
        if (first.getOffset() < 0 || first.getLength() < 0 || first.getOffset() + first.getLength() > maxLength) {
            throw new IllegalArgumentException("range exceeded:" + maxLength //$NON-NLS-1$
                + Arrays.asList(regions)
                    .stream()
                    .map(r -> "[off:" + r.getOffset() + "length:" + r.getLength() + "]") //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        ...
        for (int i = 1; i < regionsLength; i++) {
            ...
            if (lastOffset > current.getOffset()) {
                throw new IllegalArgumentException("regions not sorted:" + Arrays.asList(regions) //$NON-NLS-1$
                    .stream()
                    .map(r -> "[off:" + r.getOffset() + "length:" + r.getLength() + "]") //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                    .collect(Collectors.joining(", ")));   //$NON-NLS-1$
            }
    ...
    private void updateWorkingOptions(int indentationLevel, String lineSeparator, int kind) {
        this.workingOptions.line_separator
            = lineSeparator != null ? lineSeparator : this.originalOptions.line_separator;
        ...
        this.workingOptions.comment_format_javadoc_comment
            = this.originalOptions.comment_format_javadoc_comment && canFormatComment(kind, K_JAVA_DOC);
        this.workingOptions.comment_format_block_comment
            = this.originalOptions.comment_format_block_comment && canFormatComment(kind, K_MULTI_LINE_COMMENT);
        this.workingOptions.comment_format_line_comment
            = this.originalOptions.comment_format_line_comment && canFormatComment(kind, K_SINGLE_LINE_COMMENT);
    }
