...
public class IntersectionCastTypeReference extends TypeReference {
    ...
    public TypeBinding resolveType(BlockScope scope, boolean checkBounds, int location) {
        ...
        nextType: for (int i = 0; i < length; i++) {
            ...
            if (i == 0) {
                ...
                if (type.isArrayType()) { // javac rejects the pedantic cast: (X[] & Serializable & Cloneable) new X[0],
        ...
        IntersectionTypeBinding18 intersectionType
            = (IntersectionTypeBinding18) scope.environment().createIntersectionType18(intersectingTypes);
        ...
        nextInterface: for (int i = 0, interfaceCount = interfaces.length; i < interfaceCount; i++) {
            ...
            if (itsSuperclass != null
                && scope.hasErasedCandidatesCollisions(itsSuperclass, one, invocations, intersectionType, this))
                continue nextInterface;
    ...
    public void traverse(ASTVisitor visitor, ClassScope scope) {
        throw new UnsupportedOperationException(
            "Unexpected traversal request: IntersectionTypeReference in class scope"); //$NON-NLS-1$
    }
