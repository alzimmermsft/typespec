...
public class CompletionOnJavadocTag extends JavadocSingleNameReference
    ...
    public void filterPossibleTags(Scope scope) {
        if (this.possibleTags == null
            || this.possibleTags.length == 0
            || (this.completionFlags & ALL_POSSIBLE_TAGS) != 0) {
            return;
        }
        int kind = scope.kind;
        ...
        switch (kind) {
            case Scope.COMPILATION_UNIT_SCOPE:
                ...
                if (compilationUnit != null
                    && (compilationUnit.types.length > 0
                        && compilationUnit.types[0].name == CompletionParser.FAKE_TYPE_NAME)) {
                            ...
                            switch (scope.kind) {
                                case Scope.CLASS_SCOPE:
                                    ...
                                    boolean isRecordWithComponent
                                        = typeDecl.isRecord() && typeDecl.recordComponents.length > 0;
                                    if (((ClassScope) scope).referenceContext.binding.isGenericType()
                                        || isRecordWithComponent) {
