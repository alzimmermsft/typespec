...
public class FlowContext implements TypeConstants {
    ...
    // inside an assertFalse or a not-expression checks for equality / inequality have reversed meaning for syntactic
    // analysis for fields:
...
// check happened in an instanceof expression
    ...
    public void recordNullCheckedFieldReference(Reference reference, int timeToLive) {
        if (this.nullCheckedFieldReferences == null) {
            ...
            System.arraycopy(this.timesToLiveForNullCheckInfo, 0, this.timesToLiveForNullCheckInfo = new int[len + 2],
                0, len);
    ...
    public boolean isNullcheckedFieldAccess(Reference reference) {
        if (this.nullCheckedFieldReferences == null)  // always null unless
                ...
                if ((caughtExceptions = exceptionContext.handledExceptions) != Binding.NO_EXCEPTIONS) {
                    ...
                    for (ReferenceBinding caughtException : caughtExceptions) {
                        ...
                        if (abruptlyExitedLoops != null && caughtException != null && state != Scope.NOT_RELATED) {
                            ...
                            exceptionFlow = FlowInfo.DEAD_END; // don't use flow info on first round, flow info will be
                                                               // evaluated during loopback simulation
                        }
                        switch (state) {
                            case Scope.EQUAL_OR_MORE_SPECIFIC:
                                exceptionContext.recordHandlingException(caughtException,
                                    exceptionFlow.unconditionalInits(), raisedException, raisedException, // precise
                                                                                                          // exception
                                                                                                          // that will
                                                                                                          // be caught
                        ...
                        for (int raisedIndex = 0; raisedIndex < raisedCount; raisedIndex++) {
                            ...
                            if ((raisedException = raisedExceptions[raisedIndex]) != null) {
                                ...
                                if (abruptlyExitedLoops != null
                                    ...
                                    exceptionFlow = FlowInfo.DEAD_END; // don't use flow info on first round, flow info
                                                                       // will be evaluated during loopback simulation
                                }
                                switch (state) {
                                    case Scope.EQUAL_OR_MORE_SPECIFIC:
                                        exceptionContext.recordHandlingException(caughtException,
    ...
    public FlowInfo getInitsForFinalBlankInitializationCheck(TypeBinding declaringType, FlowInfo flowInfo) {
        ...
        do {
            if (current instanceof InitializationFlowContext) {
                ...
                current = exceptionContext.initializationParent == null
     ...
     */
    protected final void recordNullReference(LocalVariableBinding local, ASTNode location, int checkType,
        FlowInfo nullInfo) {
     ...
     */
    protected void recordNullReferenceWithAnnotationStatus(LocalVariableBinding local, ASTNode location, int checkType,
        FlowInfo nullInfo, NullAnnotationMatching nullAnnotationStatus) {
        // default implementation: do nothing
    }
     ...
     * @param providedType the type of the provided value, i.e., either expression or an element thereof (in
     * ForeachStatements)
    ...
    public void recordNullityMismatch(BlockScope currentScope, Expression expression, TypeBinding providedType,
        ...
        if (expression.localVariableBinding() != null) { // flowContext cannot yet handle non-localvar expressions
                                                         // (e.g., fields)
            ...
            while (currentContext != null) {
                ...
                if (currentContext.internalRecordNullityMismatch(expression, providedType, flowInfo, nullStatus,
                    annotationStatus, expectedType, ASSIGN_TO_NONNULL | isInsideAssert))
                    return;
        ...
        if (annotationStatus != null)
            currentScope.problemReporter()
                .nullityMismatchingTypeAnnotation(expression, providedType, expectedType, annotationStatus);
        else
            currentScope.problemReporter()
                .nullityMismatch(expression, providedType, expectedType, nullStatus,
