...
public abstract class FunctionalExpression extends Expression {
    ...
    public MethodBinding binding;                 // Code generation binding. May include synthetics. See
                                                  // getMethodBinding()
    ...
    public boolean shouldCaptureInstance = false; // Whether the expression needs access to instance data of enclosing
                                                  // type
    ...
    // Notify the compilation unit that it contains some functional types, taking care not to add any transient copies.
    // this is assumed not to be a copy
    ...
    public TypeBinding resolveType(BlockScope blockScope, boolean skipKosherCheck) {
        ...
        MethodBinding sam = this.expectedType == null
            ? null
        ...
        if (skipKosherCheck || kosherDescriptor(blockScope, sam, true)) {
            if (this.expectedType instanceof IntersectionTypeBinding18) {
                ReferenceBinding[] intersectingTypes
                    = ((IntersectionTypeBinding18) this.expectedType).intersectingTypes;
                for (ReferenceBinding intersectingType : intersectingTypes) {
                    if (intersectingType.findSuperTypeOriginatingFrom(TypeIds.T_JavaIoSerializable,
    ...
    public MethodBinding[] getRequiredBridges() {
            ...
            void collectBridges(ReferenceBinding[] interfaces) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    for (int j = 0, count = methods == null ? 0 : methods.length; j < count; j++) {
                        ...
                        if (inheritedMethod == null || this.method == inheritedMethod)  // descriptor declaring class
                                                                                        // may not be same functional
                                                                                        // interface target type.
                            continue;
                        if (inheritedMethod.isStatic() || inheritedMethod.redeclaresPublicObjectMethod(this.scope))
                            continue;
                        inheritedMethod
                            = MethodVerifier.computeSubstituteMethod(inheritedMethod, this.method, this.environment);
                        ...
                        if (!originalOverride.areParameterErasuresEqual(originalInherited)
                            || TypeBinding.notEquals(originalOverride.returnType.erasure(),
                                originalInherited.returnType.erasure()))
            ...
            void add(MethodBinding inheritedMethod) {
                ...
                for (int i = 0; i < length; i++) {
                    if (this.bridges[i].areParameterErasuresEqual(inheritedMethod)
                        && TypeBinding.equalsEquals(this.bridges[i].returnType.erasure(),
                            inheritedMethod.returnType.erasure()))
                        return;
                }
        ...
        if (this.expectedType instanceof IntersectionTypeBinding18) {
            functionalType
                = (ReferenceBinding) ((IntersectionTypeBinding18) this.expectedType).getSAMType(this.enclosingScope);
        } else {
