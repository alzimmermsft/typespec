...
public abstract class ImmutableSet<E> extends ImmutableCollection<E> implements Set<E> {
    ...
    private static <E> ImmutableSet<E> constructUnknownDuplication(int n, Object... elements) {
        ...
        return construct(n,
            Math.max(ImmutableCollection.Builder.DEFAULT_INITIAL_CAPACITY, IntMath.sqrt(n, RoundingMode.CEILING)),
            elements);
    }
        ...
        ImmutableSet<E> build() {
            switch (distinct) {
                ...
                default:
                    ...
                    Object[] elements = (distinct == dedupedElements.length)
                        ? dedupedElements
                        : Arrays.copyOf(dedupedElements, distinct);
                    return new RegularImmutableSet<E>(elements, hashCode, requireNonNull(hashTable),
                        hashTable.length - 1);
            }
        }
    ...
    private static final class JdkBackedSetBuilderImpl<E> extends SetBuilderImpl<E> {
        ...
        ImmutableSet<E> build() {
            switch (distinct) {
                ...
                default:
                    return new JdkBackedImmutableSet<E>(delegate,
