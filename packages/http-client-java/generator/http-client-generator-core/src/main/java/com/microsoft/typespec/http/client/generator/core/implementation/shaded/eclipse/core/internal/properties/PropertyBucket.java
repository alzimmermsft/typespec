 ...
 *******************************************************************************/
package com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.internal.properties;

import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.internal.localstore.Bucket;
...
public class PropertyBucket extends Bucket {
    public static class PropertyEntry extends Entry {
        ...
        static String[][] delete(String[][] existing, QualifiedName propertyName) {
            ...
            if (existing.length == 1) {
                return (existing[0][0].equals(propertyName.getQualifier())
                    && existing[0][1].equals(propertyName.getLocalName())) ? null : existing;
            }
            ...
            if (deletePosition < existing.length - 1) {
                ...
                System.arraycopy(existing, deletePosition + 1, newValue, deletePosition,
                    newValue.length - deletePosition);
            }
            return newValue;
        }
        ...
        static String[][] insert(String[][] existing, QualifiedName propertyName, String propertyValue) {
            ...
            newValue[insertPosition]
                = new String[] { propertyName.getQualifier(), propertyName.getLocalName(), propertyValue };
            if (insertPosition < existing.length) {
                System.arraycopy(existing, insertPosition, newValue, insertPosition + 1,
                    existing.length - insertPosition);
            }
            return newValue;
        }
        ...
        private static int search(String[][] existing, QualifiedName propertyName) {
            return Arrays.binarySearch(existing,
                new String[] { propertyName.getQualifier(), propertyName.getLocalName(), null }, COMPARATOR);
        }
    ...
    public PropertyBucket() {
        super(!Boolean.getBoolean(
            "com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.PropertyCache.disable")); //$NON-NLS-1$
    }
