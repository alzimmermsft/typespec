...
public class CommentRecorderParser extends Parser {
    ...
    public void checkComment() {
        ...
        nextComment: for (lastCommentIndex = this.scanner.commentPtr; lastCommentIndex >= 0; lastCommentIndex--) {
            ...
            if (this.javadocParser.shouldReportProblems) {
                this.javadocParser.reportProblems
                    = this.currentElement == null || commentSourceEnd > this.lastJavadocEnd;
            } else {
        ...
        if (lastCommentIndex >= 0 && checkDeprecated) {
            ...
            if (this.forStartPosition == 0 || this.forStartPosition < lastCommentStart) {// only if there is no "for" in
                                                                                         // between
    ...
    public int flushCommentsDefinedPriorTo(int position) {
        ...
        if (validCount > 0) {
            ...
            while (index < lastCommentIndex && (immediateCommentEnd = -this.scanner.commentStops[index + 1]) > 0) { // only
                                                                                                                    // tolerating
                                                                                                                    // non-javadoc
                                                                                                                    // comments
                                                                                                                    // (non-javadoc
                ...
                if (com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.util.Util
                    .getLineNumber(position, this.scanner.lineEnds, 0, this.scanner.linePtr)
    ...
    private void pushOnCommentsStack(int start, int end) {
        ...
        for (int i = start; i <= end; i++) {
            if (this.scanner.commentPtr < i)
                break;
            // First see if comment hasn't been already stored
            int scannerStart
                = this.scanner.commentStarts[i] < 0 ? -this.scanner.commentStarts[i] : this.scanner.commentStarts[i];
            ...
            if (commentStart == -1 || scannerStart > commentStart) {
                ...
                if (++this.commentPtr >= stackLength) {
                    System.arraycopy(this.commentStarts, 0,
                        this.commentStarts = new int[stackLength + CommentIncrement], 0, stackLength);
                    System.arraycopy(this.commentStops, 0, this.commentStops = new int[stackLength + CommentIncrement],
                        0, stackLength);
                }
