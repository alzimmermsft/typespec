...
public final class JavaConventions {
    ...
    private static final Map<String, Set<String>> restrictedIdentifiersMap;
    private static final Map<String, Set<String>> restrictedIdentifierPreviewMap;
    private static final List<String> javaVersions;
     ...
     */
    private static synchronized char[] scannedIdentifier(String id, String sourceLevel, String complianceLevel,
        String previewEnabled, boolean allowRestrictedKeyWords) {
        if (id == null) {
            return null;
        }
        // Set scanner for given source and compliance levels
        SCANNER.sourceLevel = sourceLevel == null
            ? CompilerOptions.getFirstSupportedJdkLevel()
            : CompilerOptions.versionToJdkLevel(sourceLevel);
        SCANNER.complianceLevel = complianceLevel == null
            ? CompilerOptions.getFirstSupportedJdkLevel()
        ...
        try {
            ...
            if (token != TerminalToken.TokenNameIdentifier)
                return null;
            if (SCANNER.currentPosition == SCANNER.eofPosition) { // to handle case where we had an
                                                                  // ArrayIndexOutOfBoundsException
                try {
                    char[] src = SCANNER.getCurrentIdentifierSource();
                    src = scanForRestrictedKeyWords(src, sourceLevel, complianceLevel, SCANNER.previewEnabled,
                        allowRestrictedKeyWords);
                    return src;
    ...
    private static char[] scanForRestrictedKeyWords(char[] id, String sourceLevel, String complianceLevel,
        boolean previewEnabled, boolean allowRestrictedKeyWords) {
    ...
    public static IStatus validateImportDeclaration(String name, String sourceLevel, String complianceLevel) {
        ...
        if (name.charAt(name.length() - 1) == '*') {
            if (name.charAt(name.length() - 2) == '.') {
                ...
                return new Status(IStatus.ERROR, JavaCore.PLUGIN_ID, -1, Messages.convention_import_unqualifiedImport,
                    null);
            }
        }
     ...
     * @deprecated Use
     * {@link #validateJavaTypeName(String id, String sourceLevel, String complianceLevel, String previewEnabled)}
     * instead
     */
    ...
    private static IStatus internalValidateJavaTypeName(String name, String sourceLevel, String complianceLevel,
     ...
     * @since 3.1
     * TODO (frederic) finalize for all possible options (JavaCore, DefaultCodeFormatterOptions, AssistOptions) and open
     * to API
     */
    /*
     ...
     * errors.add(new JavaModelStatus(IStatus.ERROR, Util.bind("convention.compiler.invalidCompilerOption",
     * compliance==null?"":compliance, JavaCore.COMPILER_COMPLIANCE))); //$NON-NLS-1$ //$NON-NLS-2$
     * }
     * 
     ...
     * // source is not valid
     * errors.add(new JavaModelStatus(IStatus.ERROR, Util.bind("convention.compiler.invalidCompilerOption",
     * source==null?"":source, JavaCore.COMPILER_SOURCE))); //$NON-NLS-1$ //$NON-NLS-2$
     * }
     * 
     * // Verify target value
     * if (targetLevel == 0) {
     ...
     * // target is not valid
     * errors.add(new JavaModelStatus(IStatus.ERROR, Util.bind("convention.compiler.invalidCompilerOption",
     * target==null?"":target, JavaCore.COMPILER_CODEGEN_TARGET_PLATFORM))); //$NON-NLS-1$ //$NON-NLS-2$
     * }
     * }
     * 
     ...
     * errors.add(new JavaModelStatus(IStatus.ERROR, Util.bind("convention.compiler.incompatibleTargetForSource",
     * target, JavaCore.VERSION_1_5))); //$NON-NLS-1$
     * }
     * else
     * // target must be 1.4 if source is 1.4
     * if (sourceLevel >= ClassFileConstants.JDK1_4 && targetLevel < ClassFileConstants.JDK1_4) {
     * errors.add(new JavaModelStatus(IStatus.ERROR, Util.bind("convention.compiler.incompatibleTargetForSource",
     * target, JavaCore.VERSION_1_4))); //$NON-NLS-1$
     * }
     * // target cannot be greater than compliance level
     * if (complianceLevel < targetLevel){
     * errors.add(new JavaModelStatus(IStatus.ERROR, Util.bind("convention.compiler.incompatibleComplianceForTarget",
     * compliance, JavaCore.VERSION_1_4))); //$NON-NLS-1$
     * }
     * // compliance must be 1.5 if source is 1.5
     * if (source.equals(JavaCore.VERSION_1_5) && complianceLevel < ClassFileConstants.JDK1_5) {
     * errors.add(new JavaModelStatus(IStatus.ERROR, Util.bind("convention.compiler.incompatibleComplianceForSource",
     * compliance, JavaCore.VERSION_1_5))); //$NON-NLS-1$
     * } else
