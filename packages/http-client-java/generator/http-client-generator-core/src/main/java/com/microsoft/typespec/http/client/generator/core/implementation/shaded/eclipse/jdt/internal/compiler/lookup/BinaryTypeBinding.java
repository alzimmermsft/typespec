...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.classfmt.AnnotationInfo;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.classfmt.ExternalAnnotationProvider.IMethodAnnotationWalker;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.classfmt.MethodInfoWithAnnotations;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.classfmt.NonNullDefaultAwareTypeAnnotationWalker;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.classfmt.TypeAnnotationWalker;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.env.*;
...
public class BinaryTypeBinding extends ReferenceBinding {
    ...
    protected Map<Binding, AnnotationHolder> storedAnnotations = null; // keys are this ReferenceBinding & its fields
                                                                       // and methods, value is an AnnotationHolder
    ...
    public enum ExternalAnnotationStatus {
        FROM_SOURCE, NOT_EEA_CONFIGURED, NO_EEA_FILE, TYPE_IS_ANNOTATED;
    ...
    public ExternalAnnotationStatus externalAnnotationStatus = ExternalAnnotationStatus.NOT_EEA_CONFIGURED; // unless
                                                                                                            // proven
                                                                                                            // differently
    ...
    static Object convertMemberValue(Object binaryValue, LookupEnvironment env, char[][][] missingTypeNames,
        ...
        if (binaryValue instanceof ClassSignature)
            return env.getTypeFromSignature(((ClassSignature) binaryValue).getTypeName(), 0, -1, false, null,
                missingTypeNames, ITypeAnnotationWalker.EMPTY_ANNOTATION_WALKER);
        ...
        if (binaryValue instanceof EnumConstantSignature) {
            ...
            ReferenceBinding enumType = (ReferenceBinding) env.getTypeFromSignature(ref.getTypeName(), 0, -1, false,
                null, missingTypeNames, ITypeAnnotationWalker.EMPTY_ANNOTATION_WALKER);
    ...
    public TypeBinding clone(TypeBinding outerType) {
        ...
        /*
         * BinaryTypeBinding construction is not "atomic" and is split between the constructor and cachePartsFrom and
         * between the two
         * stages of construction, clone can kick in when LookupEnvironment.createBinaryTypeFrom calls
         * PackageBinding.addType. This
         * can result in some URB's being resolved, which could trigger the clone call, leaving the clone with
         * semi-initialized prototype.
         * Fortunately, the protocol for this type demands all clients to use public access methods, where we can
         * deflect the call to the
    ...
    static AnnotationBinding createAnnotation(IBinaryAnnotation annotationInfo, LookupEnvironment env,
        ...
        for (int i = 0; i < length; i++)
            pairs[i] = new ElementValuePair(binaryPairs[i].getName(),
                convertMemberValue(binaryPairs[i].getValue(), env, missingTypeNames, false), null);
        ...
        ReferenceBinding annotationType
            = env2.getTypeFromConstantPoolName(typeName, 1, typeName.length - 1, false, missingTypeNames);
        ...
        switch (type.kind()) {
            ...
            case Binding.ARRAY_TYPE:
                ...
                if (leafComponentType.hasNullTypeAnnotations() && environment.usesNullTypeAnnotations()) {
                    ...
                    arrayBinding.nullTagBitsPerDimension[arrayBinding.dimensions]
                        = leafComponentType.tagBits & TagBits.AnnotationNullMASK;
                }
                break;
    ...
    private static TypeBinding resolveType(TypeBinding type, LookupEnvironment environment,
        boolean convertGenericToRawType, boolean convertRawToGenericType) {
    ...
    public BinaryTypeBinding(PackageBinding packageBinding, IBinaryType binaryType, LookupEnvironment environment,
        ...
        this.typeVariables
            = typeSignature != null && typeSignature.length > 0 && typeSignature[0] == Util.C_GENERIC_START
                ? null // is initialized in cachePartsFrom (called from LookupEnvironment.createBinaryTypeFrom())...
                       // must set to null so isGenericType() answers true
    ...
    private void cachePartsFrom2(IBinaryType binaryType, boolean needFieldsAndMethods) {
        ...
        try {
            ...
            if (memberTypeStructures != null) {
                ...
                if (size > 0) {
                    ...
                    for (int i = 0; i < size; i++) {
                        // attempt to find each member type if it exists in the cache (otherwise - resolve it when
                        // requested)
                        this.memberTypes[i] = this.environment.getTypeFromConstantPoolName(
                            memberTypeStructures[i].getName(), 0, -1, false, null /* could not be missing */);
                    }
            ...
            if (this.environment.globalOptions.isAnnotationBasedNullAnalysisEnabled) {
                // need annotations on the type before processing null annotations on members respecting any
                // @NonNullByDefault:
            ...
            if (this.environment.globalOptions.isAnnotationBasedResourceAnalysisEnabled
                && binaryType.getAnnotations() != null) {
                this.tagBits |= scanForOwningAnnotation(binaryType.getAnnotations());
            }
            ITypeAnnotationWalker walker
                = getTypeAnnotationWalker(binaryType.getTypeAnnotations(), Binding.NO_NULL_DEFAULT);
            ITypeAnnotationWalker toplevelWalker
                = binaryType.enrichWithExternalAnnotationsFor(walker, null, this.environment);
            ...
            if (typeSignature != null) {
                ...
                if (wrapper.signature[wrapper.start] == Util.C_GENERIC_START) {
                    ...
                    wrapper.start++; // skip '<'
                    this.typeVariables
                        = createTypeVariables(wrapper, true, missingTypeNames, toplevelWalker, true/* class */);
                    wrapper.start++; // skip '>'
            ...
            if (typeSignature == null) {
                if (superclassName != null) {
                    ...
                    this.superclass = this.environment.getTypeFromConstantPoolName(superclassName, 0, -1, false,
                        missingTypeNames, toplevelWalker.toSupertype((short) -1, superclassName));
                ...
                if (interfaceNames != null) {
                    ...
                    if (size > 0) {
                        ...
                        for (short i = 0; i < size; i++)
                            // attempt to find each superinterface if it exists in the cache (otherwise - resolve it
                            // when requested)
                            this.superInterfaces[i] = this.environment.getTypeFromConstantPoolName(interfaceNames[i], 0,
                                -1, false, missingTypeNames, toplevelWalker.toSupertype(i, interfaceNames[i]));
                        this.tagBits |= TagBits.HasUnresolvedSuperinterfaces;
                    }
                }
            } else {
                // attempt to find the superclass if it exists in the cache (otherwise - resolve it when requested)
                this.superclass = (ReferenceBinding) this.environment.getTypeFromTypeSignature(wrapper, typeVars, this,
                    missingTypeNames, toplevelWalker.toSupertype((short) -1, wrapper.peekFullType()));
                this.tagBits |= TagBits.HasUnresolvedSuperclass;

                this.superInterfaces = Binding.NO_SUPERINTERFACES;
                if (!wrapper.atEnd()) {
                    // attempt to find each superinterface if it exists in the cache (otherwise - resolve it when
                    // requested)
                    ArrayList types = new ArrayList(2);
                    short rank = 0;
                    do {
                        types.add(this.environment.getTypeFromTypeSignature(wrapper, typeVars, this, missingTypeNames,
                            toplevelWalker.toSupertype(rank++, wrapper.peekFullType())));
                    } while (!wrapper.atEnd());
            ...
            if (permittedSubtypesNames != null) {
                ...
                if (size > 0) {
                    ...
                    for (short i = 0; i < size; i++)
                        // attempt to find each permitted type if it exists in the cache (otherwise - resolve it when
                        // requested)
                        this.permittedTypes[i] = this.environment.getTypeFromConstantPoolName(permittedSubtypesNames[i],
                            0, -1, false, missingTypeNames);
                }
            }
            ...
            if (needFieldsAndMethods) {
                ...
                IBinaryMethod[] iMethods
                    = createMethods(binaryType.getMethods(), binaryType, sourceLevel, missingTypeNames);
                ...
                if (this.environment.globalOptions.isAnnotationBasedNullAnalysisEnabled) {
                    ...
                    if (iMethods != null) {
                        for (int i = 0; i < iMethods.length; i++) {
                            // (not using walker, which has defaultNullness, because defaults on parameters & return
                            // will be applied
                            // by ImplicitNullAnnotationVerifier, triggered per invocation via MessageSend.resolveType()
                            // et al)
            ...
            if (this.environment.globalOptions.storeAnnotations) {
                ...
                for (IBinaryAnnotation annotation : declAnnotations) {
                    if (annotation.isDeprecatedAnnotation()) {
                        AnnotationBinding[] annotationBindings = createAnnotations(
                            new IBinaryAnnotation[] { annotation }, this.environment, missingTypeNames);
                        ...
                        for (ElementValuePair elementValuePair : annotationBindings[0].getElementValuePairs()) {
                            if (CharOperation.equals(elementValuePair.name, TypeConstants.FOR_REMOVAL)) {
                                if (elementValuePair.value instanceof BooleanConstant
                                    && ((BooleanConstant) elementValuePair.value).booleanValue()) {
    ...
    private final static VariableBindingInitialization<FieldBinding> FIELD_INITIALIZATION
        = new VariableBindingInitialization<>() {
            ...
            FieldBinding createBinding(BinaryTypeBinding self, IBinaryField binaryField, TypeBinding type) {
                return new FieldBinding(binaryField.getName(), type,
                    binaryField.getModifiers() | ExtraCompilerModifiers.AccUnresolved, self, binaryField.getConstant());
            }
    ...
    private final static VariableBindingInitialization<RecordComponentBinding> RECORD_INITIALIZATION
        = new VariableBindingInitialization<>() {
            ...
            RecordComponentBinding createBinding(BinaryTypeBinding self, IBinaryField binaryField, TypeBinding type) {
                return new RecordComponentBinding(binaryField.getName(), type,
                    binaryField.getModifiers() | ExtraCompilerModifiers.AccUnresolved, self);
            }
    ...
    private void createFields(IBinaryField[] iFields, IBinaryType binaryType, long sourceLevel,
        char[][][] missingTypeNames, VariableBindingInitialization initialization) {
        if (!isPrototype())
            throw new IllegalStateException();
                ...
                if (size > 0) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        ITypeAnnotationWalker walker = getTypeAnnotationWalker(binaryField.getTypeAnnotations(),
                            getNullDefaultFrom(declAnnotations));
                        walker = binaryType.enrichWithExternalAnnotationsFor(walker, iFields[i], this.environment);
                        walker = walker.toField();
                        TypeBinding type = fieldSignature == null
                            ? this.environment.getTypeFromSignature(binaryField.getTypeName(), 0, -1, false, this,
                                missingTypeNames, walker)
                            : this.environment.getTypeFromTypeSignature(new SignatureWrapper(fieldSignature),
                                Binding.NO_TYPE_VARIABLES, this, missingTypeNames, walker);
                        ...
                        if (declAnnotations != null) {
                            for (IBinaryAnnotation annotation : declAnnotations) {
                                char[] typeName = annotation.getTypeName();
                                if (CharOperation.equals(typeName, ConstantPool.PREVIEW_FEATURE)
                                    && field instanceof FieldBinding realField) {
                        ...
                        if (firstAnnotatedFieldIndex < 0
                            ...
                            if (forceStoreAnnotations)
                                storedAnnotations(true, true); // for Java 9 @Deprecated we need to force storing
                                                               // annotations
                        }
                        field.id = i; // ordinal
                    ...
                    if (firstAnnotatedFieldIndex >= 0) {
                        for (int i = firstAnnotatedFieldIndex; i < size; i++) {
                            IBinaryField binaryField = iFields[i];
                            fields1[i].setAnnotations(
                                createAnnotations(binaryField.getAnnotations(), this.environment, missingTypeNames),
                                false);
                        }
                    }
                }
    ...
    private MethodBinding createMethod(IBinaryMethod method, IBinaryType binaryType, long sourceLevel,
        char[][][] missingTypeNames) {
        if (!isPrototype())
            throw new IllegalStateException();
        ...
        if (isInterface() && (methodModifiers & ClassFileConstants.AccAbstract) == 0) {
            ...
            if (((methodModifiers & ClassFileConstants.AccStatic) == 0
                ...
                // i.e. even at 1.7- we record AccDefaultMethod when reading a 1.8+ interface to avoid errors caused by
                // default methods added to a library
         ...
         */
        ITypeAnnotationWalker walker
            = getTypeAnnotationWalker(method.getTypeAnnotations(), getNullDefaultFrom(declAnnotations));
        ...
        if (methodSignature == null) { // no generics
            ...
            if (size > 0) {
                ...
                for (int i = 0; i < numOfParams; i++) {
                    ...
                    if (i >= startIndex) {   // skip the synthetic arg if necessary
                        // checking for param specific non-null default is not necessary here as no type arguments are
                        // present (application to params themselves is handled by ImplicitNullAnnotationVerifier)
                        parameters[i - startIndex] = this.environment.getTypeFromSignature(methodDescriptor, index, end,
                            false, this, missingTypeNames, walker.toMethodParameter(visibleIdx++));
                        // 'paramAnnotations' line up with 'parameters'
                        // int parameter to method.getParameterAnnotations() include the synthetic arg
                        if (paramAnnotations != null)
                            paramAnnotations[i - startIndex]
                                = createAnnotations(method.getParameterAnnotations(i - startIndex, this.fileName),
                ...
                if (size > 0) {
                    ...
                    for (int i = 0; i < size; i++)
                        exceptions[i] = this.environment.getTypeFromConstantPoolName(exceptionTypes[i], 0, -1, false,
                            missingTypeNames, walker.toThrows(i));
                }
            }

            if (!method.isConstructor())
                returnType = this.environment.getTypeFromSignature(methodDescriptor, index + 1, -1, false, this,
                    missingTypeNames, walker.toMethodReturn());   // index is currently pointing at the ')'
            ...
            if (startIndex > 0 && argumentNamesLength > 0) {
                ...
                if (startIndex >= argumentNamesLength) {
                    ...
                    System.arraycopy(argumentNames, startIndex, slicedArgumentNames, 0,
                        argumentNamesLength - startIndex);
            ...
            // MethodTypeSignature = ParameterPart(optional) '(' TypeSignatures ')' return_typeSignature ['^'
            ...
            if (wrapper.signature[wrapper.start] == Util.C_PARAM_START) {
                ...
                if (wrapper.signature[wrapper.start] == Util.C_PARAM_END) {
                    ...
                    while (wrapper.signature[wrapper.start] != Util.C_PARAM_END) {
                        IBinaryAnnotation[] binaryParameterAnnotations
                            = method.getParameterAnnotations(rank, this.fileName);
                        ITypeAnnotationWalker updatedWalker
                            = NonNullDefaultAwareTypeAnnotationWalker.updateWalkerForParamNonNullDefault(walker,
                                getNullDefaultFrom(binaryParameterAnnotations), this.environment);
                        types.add(this.environment.getTypeFromTypeSignature(wrapper, typeVars, this, missingTypeNames,
                            updatedWalker.toMethodParameter(rank)));
                        rank++;
                    }
                    wrapper.start++; // skip ')'
                    ...
                    if (this.environment.globalOptions.storeAnnotations) {
                        ...
                        for (int i = 0; i < numParam; i++)
                            paramAnnotations[i] = createAnnotations(method.getParameterAnnotations(i, this.fileName),
                                this.environment, missingTypeNames);
                    }
                }
            }
                    ...
                    if (size > 0) {
                        ...
                        for (int i = 0; i < size; i++)
                            exceptions[i] = this.environment.getTypeFromConstantPoolName(exceptionTypes[i], 0, -1,
                                false, missingTypeNames, walker.toThrows(i));
                    }
                }
            }
        }
        ...
        if (receiverAnnotations != null && receiverAnnotations.length > 0) {
            result.receiver = this.environment.createAnnotatedType(this,
                createAnnotations(receiverAnnotations, this.environment, missingTypeNames));
        }
        ...
        if (this.environment.globalOptions.storeAnnotations || forceStoreAnnotations) {
            ...
            result.setAnnotations(createAnnotations(annotations, this.environment, missingTypeNames), paramAnnotations,
                isAnnotationType()
                    ? convertMemberValue(method.getDefaultValue(), this.environment, missingTypeNames, true)
                    : null,
                this.environment);
        }
        ...
        switch (this.id) {
            case TypeIds.T_JavaUtilObjects:
                if (CharOperation.equals(method.getSelector(), TypeConstants.REQUIRE_NON_NULL)) {
                    ...
                    if (eeaSource != null) {
                        walker
                            = ExternalAnnotationProvider.synthesizeForMethod(eeaSource.toCharArray(), this.environment);
                    }
                }
                break;
        }
     ...
     */
    private IBinaryMethod[] createMethods(IBinaryMethod[] iMethods, IBinaryType binaryType, long sourceLevel,
        char[][][] missingTypeNames) {
        if (!isPrototype())
            throw new IllegalStateException();
        boolean save = this.environment.mayTolerateMissingType;
    ...
    private TypeVariableBinding[] createTypeVariables(SignatureWrapper wrapper, boolean assignVariables,
        char[][][] missingTypeNames, ITypeAnnotationWalker walker, boolean isClassTypeParameter) {
        if (!isPrototype())
            throw new IllegalStateException();
                    ...
                    default:
                        if (pendingVariable) {
                            ...
                            TypeVariableBinding typeVariable
                                = new TypeVariableBinding(variableName, this, rank, this.environment);
                            AnnotationBinding[] annotations = BinaryTypeBinding.createAnnotations(
                                walker.toTypeParameter(isClassTypeParameter, rank++).getAnnotationsAtCursor(0, false),
                                this.environment, missingTypeNames);
                            if (annotations != null && annotations != Binding.NO_ANNOTATIONS)
                                typeVariable.setTypeAnnotations(annotations,
                                    this.environment.globalOptions.isAnnotationBasedNullAnalysisEnabled);
        ...
        for (int i = 0; i < rank; i++) {
            initializeTypeVariable(result[i], result, wrapper, missingTypeNames,
                walker.toTypeParameterBounds(isClassTypeParameter, i));
    ...
    private MethodBinding findMethod(char[] methodDescriptor, char[][][] missingTypeNames) {
        ...
        if (numOfParams > 0) {
            ...
            for (int i = 0; i < numOfParams; i++) {
                ...
                // not interested in type annotations, type will be used for comparison only, and erasure() is used if
                // needed
                TypeBinding param = this.environment.getTypeFromSignature(methodDescriptor, index, end, false, this,
                    missingTypeNames, ITypeAnnotationWalker.EMPTY_ANNOTATION_WALKER);
        ...
        loop: for (MethodBinding currentMethod : methods2) {
            ...
            if (parameterLength == currentMethodParameterLength) {
                for (int j = 0; j < currentMethodParameterLength; j++) {
                    if (TypeBinding.notEquals(parameters[j], parameters2[j])
                        && TypeBinding.notEquals(parameters[j].erasure(), parameters2[j].erasure())) {
                        continue loop;
                    }
                }
            ...
            for (int i = members.length; --i >= 0;) {
                ...
                if (memberType instanceof UnresolvedReferenceBinding) {
                    ...
                    if (name.length == (prefixLength + typeName.length)) // enclosing $ typeName
                        if (CharOperation.fragmentEquals(typeName, name, prefixLength, true)) // only check trailing
                                                                                              // portion
                            return members[i] = (ReferenceBinding) resolveType(memberType, this.environment,
...
// NOTE: member types of binary types are resolved when needed
    ...
    private void initializeTypeVariable(TypeVariableBinding variable, TypeVariableBinding[] existingVariables,
        SignatureWrapper wrapper, char[][][] missingTypeNames, ITypeAnnotationWalker walker) {
        if (!isPrototype())
            throw new IllegalStateException();
        ...
        if (wrapper.signature[wrapper.start] == Util.C_COLON) {
            ...
            TypeBinding typeFromTypeSignature = this.environment.getTypeFromTypeSignature(wrapper, existingVariables,
                this, missingTypeNames, walker.toTypeBound(rank++));
    ...
    public boolean isEquivalentTo(TypeBinding otherType) {
        ...
        switch (otherType.kind()) {
            ...
            case Binding.PARAMETERIZED_TYPE:
                 ...
                 * not. See https://bugs.eclipse.org/bugs/show_bug.cgi?id=186565 &&
...
// NOTE: member types of binary types are resolved when needed
    ...
    public ReferenceBinding[] memberTypes() {
        ...
        for (int i = this.memberTypes.length; --i >= 0;)
            this.memberTypes[i] = (ReferenceBinding) resolveType(this.memberTypes[i], this.environment,
                false /* no raw conversion for now */);
        this.tagBits &= ~TagBits.HasUnresolvedMemberTypes;
        return maybeSortedMemberTypes();
    }
...
// NOTE: the return type, arg & exception types of each method of a binary type are resolved when needed
    ...
    public ReferenceBinding containerAnnotationType() {
        ...
        if (this.containerAnnotationType instanceof UnresolvedReferenceBinding) {
            this.containerAnnotationType = (ReferenceBinding) BinaryTypeBinding
                .resolveType(this.containerAnnotationType, this.environment, false);
        }
        ...
        try {
            ...
            for (int i = method.parameters.length; --i >= 0;) {
                TypeBinding resolvedType
                    = resolveType(method.parameters[i], this.environment, true /* raw conversion */);
            ...
            for (int i = method.thrownExceptions.length; --i >= 0;) {
                ReferenceBinding resolvedType = (ReferenceBinding) resolveType(method.thrownExceptions[i],
                    this.environment, true /* raw conversion */);
    ...
    AnnotationBinding[] retrieveAnnotations(Binding binding) {
        ...
        return AnnotationBinding.addStandardAnnotations(super.retrieveAnnotations(binding),
    ...
    public void tagAsHavingDefectiveContainerType() {
        ...
        if (this.containerAnnotationType != null && this.containerAnnotationType.isValidBinding())
            this.containerAnnotationType = new ProblemReferenceBinding(this.containerAnnotationType.compoundName,
                this.containerAnnotationType, ProblemReasons.DefectiveContainerAnnotationType);
    }
...
//pre: null annotation analysis is enabled
    private void scanFieldForNullAnnotation(IBinaryField field, VariableBinding fieldBinding, boolean isEnum,
        ITypeAnnotationWalker externalAnnotationWalker) {
        if (!isPrototype())
            throw new IllegalStateException();
        ...
        if (!CharOperation.equals(this.fPackage.compoundName, TypeConstants.JAVA_LANG_ANNOTATION) // avoid dangerous
                                                                                                  // re-entry via
        ...
        if (annotations != null) {
            for (IBinaryAnnotation annotation : annotations) {
                ...
                if (annotationTypeName[0] != Util.C_RESOLVED)
                    continue;
                int typeBit
                    = this.environment.getAnalysisAnnotationBit(signature2qualifiedTypeName(annotationTypeName));
        ...
        if (!explicitNullness) {
            int nullDefaultFromField = getNullDefaultFrom(field.getAnnotations());
            if (nullDefaultFromField == Binding.NO_NULL_DEFAULT
                ? hasNonNullDefaultForType(fieldBinding.type, DefaultLocationField, -1)
                : (nullDefaultFromField & DefaultLocationField) != 0) {
                fieldBinding.tagBits |= TagBits.AnnotationNonNull;
            }
        }
    ...
    private void scanMethodForNullAnnotation(IBinaryMethod method, MethodBinding methodBinding,
        ITypeAnnotationWalker externalAnnotationWalker, boolean useNullTypeAnnotations) {
        if (!isPrototype())
            throw new IllegalStateException();
        if (isEnum()) {
            int purpose = 0;
                    ...
                    if (this.environment.usesNullTypeAnnotations()) {
                        if (methodBinding.returnType != null && !methodBinding.returnType.hasNullTypeAnnotations()) {
                            methodBinding.returnType
                                = this.environment.createNonNullAnnotatedType(methodBinding.returnType);
                        }
                    }
        ...
        if (numParamAnnotations > 0) {
            for (int j = 0; j < numVisibleParams; j++) {
                if (numParamAnnotations > 0) {
                    ...
                    ITypeAnnotationWalker parameterWalker
                        = externalAnnotationWalker.toMethodParameter((short) (j + startIndex));
                    IBinaryAnnotation[] paramAnnotations
                        = parameterWalker != ITypeAnnotationWalker.EMPTY_ANNOTATION_WALKER
                    ...
                    if (paramAnnotations != null) {
                        for (IBinaryAnnotation paramAnnotation : paramAnnotations) {
                            ...
                            int typeBit = this.environment
                                .getAnalysisAnnotationBit(signature2qualifiedTypeName(annotationTypeName));
                            if (typeBit == TypeIds.BitNonNullAnnotation) {
                                ...
                                if (this.environment.usesNullTypeAnnotations()) {
                                    if (methodBinding.parameters[j] != null
                                        ...
                                        methodBinding.parameters[j]
                                            = this.environment.createAnnotatedType(methodBinding.parameters[j],
...
// pre: null annotation analysis is enabled
    private void scanTypeForNullDefaultAnnotation(IBinaryType binaryType, PackageBinding packageBinding) {
        ...
        if (CharOperation.equals(CharOperation.splitOn('/', binaryType.getName()), nonNullByDefaultAnnotationName))
            return; // don't recursively apply @NNBD on @NNBD, neither directly nor via the 'enclosing'
                    // package-info.java
        ...
        if (packageBinding.getDefaultNullness() == Binding.NO_NULL_DEFAULT
            && !isPackageInfo
            && ((this.typeBits & (TypeIds.BitAnyNullAnnotation)) == 0)) {
            // this will scan the annotations in package-info
            ReferenceBinding packageInfo
                = packageBinding.getType(TypeConstants.PACKAGE_INFO_NAME, packageBinding.enclosingModule);
            if (packageInfo == null) {
                packageBinding.setDefaultNullness(Binding.NO_NULL_DEFAULT);
            }
        }
    ...
    /**
     * given an application of @NonNullByDefault convert the annotation argument (if any) into a bitvector a la
     * {@link Binding#NullnessDefaultMASK}
     */
// pre: null annotation analysis is enabled
    static int getNonNullByDefaultValue(IBinaryAnnotation annotation, LookupEnvironment environment) {
        ...
        if (elementValuePairs == null || elementValuePairs.length == 0) {
            ...
            ReferenceBinding annotationType = environment.getType(typeName, environment.UnNamedModule); // TODO(SHMOD):
                                                                                                        // null
                                                                                                        // annotations
                                                                                                        // from a
                                                                                                        // module?
        ...
        if (methodTypeAnnotations != null) {
            ...
            if (receiverTypeAnnotations != null) {
                for (IBinaryAnnotation binaryAnnotation : receiverTypeAnnotations) {
                    int typeBit = this.environment
                        .getAnalysisAnnotationBit(signature2qualifiedTypeName(binaryAnnotation.getTypeName()));
            ...
            if (CharOperation
                .equals(
                    annotationOnAnnotation
                        .getAnnotationType().compoundName[annotationOnAnnotation.type.compoundName.length - 1],
    ...
    static char[][] signature2qualifiedTypeName(char[] typeSignature) {
        return CharOperation.splitOn('/', typeSignature, 1, typeSignature.length - 1); // cut off leading 'L' and
                                                                                       // trailing ';'
    }
                ...
                if (CharOperation.equals(annotationTypeName, ConstantPool.JAVA_LANG_ANNOTATION_REPEATABLE)) {
                    ...
                    if (elementValuePairs != null && elementValuePairs.length == 1) {
                        ...
                        if (value instanceof ClassSignature) {
                            this.containerAnnotationType = (ReferenceBinding) this.environment.getTypeFromSignature(
                                ((ClassSignature) value).getTypeName(), 0, -1, false, null, missingTypeNames,
    ...
    public ReferenceBinding superclass() {
        ...
        if ((this.typeBits & (TypeIds.BitAutoCloseable | TypeIds.BitCloseable)) != 0) // avoid the side-effects of
                                                                                      // hasTypeBit()!
    ...
    private void breakLoop() {
        ...
        while (currentSuper != null) {
            if ((currentSuper.tagBits & TagBits.EndHierarchyCheck) != 0 && prevSuper instanceof BinaryTypeBinding) {
                ((BinaryTypeBinding) prevSuper).superclass
...
// NOTE: superInterfaces of binary types are resolved when needed
    ...
    public ReferenceBinding[] permittedTypes() {
        ...
        for (int i = this.permittedTypes.length; --i >= 0;)
            this.permittedTypes[i]
                = (ReferenceBinding) resolveType(this.permittedTypes[i], this.environment, false, true); // re-resolution
                                                                                                         // seems
                                                                                                         // harmless;
                                                                                                         // while
                                                                                                         // permitted
            ...
            if (this.superInterfaces != Binding.NO_SUPERINTERFACES) {
                ...
                for (int i = 0, length = this.superInterfaces.length; i < length; i++) {
                    if (i > 0)
                        buffer.append(", "); //$NON-NLS-1$
                    buffer
                        .append((this.superInterfaces[i] != null) ? this.superInterfaces[i].debugName() : "NULL TYPE"); //$NON-NLS-1$
                }
            }
        } else {
