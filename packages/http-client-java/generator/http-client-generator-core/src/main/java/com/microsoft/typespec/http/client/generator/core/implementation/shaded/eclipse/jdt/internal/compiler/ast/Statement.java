...
//		}
    ...
    /**
     * Lambda shape analysis: *Assuming* this is reachable, analyze if this completes normally i.e control flow can
     * reach the textually next statement.
     * For blocks, we don't perform intra-reachability analysis. We assume the lambda body is free of intrinsic control
     * flow errors (if such errors
    ...
    /**
     * Lambda shape analysis: *Assuming* this is reachable, analyze if this completes by continuing i.e control flow
     * cannot reach the textually next statement.
     * This is necessitated by the fact that continue claims to not complete normally. So this is necessary to
     * discriminate between do { continue; } while (false);
    ...
    protected void analyseArguments(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo,
        MethodBinding methodBinding, Expression[] arguments) {
        ...
        if (arguments != null) {
            ...
            if (considerTypeAnnotations || hasJDK15FlowAnnotations) {
                ...
                if (methodBinding.isVarargs()) {
                    ...
                    if (!passThrough)
                        numParamsToCheck--; // with non-passthrough varargs last param is fed from individual args ->
                                            // don't check
                }
            }
            if (considerTypeAnnotations) {
                for (int i = 0; i < numParamsToCheck; i++) {
                    ...
                    Boolean specialCaseNonNullness
                        = hasJDK15FlowAnnotations ? methodBinding.getParameterNullness(i) : null;
                ...
                if (!passThrough && varArgsType instanceof ArrayBinding) {
                    ...
                    Boolean specialCaseNonNullness
                        = hasJDK15FlowAnnotations ? methodBinding.getParameterNullness(varArgPos) : null;
                ...
                for (int i = 0; i < numParamsToCheck; i++) {
                    if ((methodBinding.parameterFlowBits[i] & MethodBinding.PARAM_NONNULL) != 0) {
                        ...
                        int nullStatus = argument.nullStatus(flowInfo, flowContext); // slight loss of precision: should
                                                                                     // also use the null info from the
                                                                                     // receiver.
                        if (nullStatus != FlowInfo.NON_NULL) // if required non-null is not provided
                            flowContext.recordNullityMismatch(currentScope, argument, argument.resolvedType,
                                expectedType, flowInfo, nullStatus, null);
                    }
                }
            }
        }
    ...
    void analyseOneArgument18(BlockScope currentScope, FlowContext flowContext, FlowInfo flowInfo,
        ...
        if (argument instanceof ConditionalExpression && argument.isPolyExpression()) {
            ...
            ce.internalAnalyseOneArgument18(currentScope, flowContext, expectedType, ce.valueIfTrue, flowInfo,
                ce.ifTrueNullStatus, expectedNonNullness, originalExpected);
            ce.internalAnalyseOneArgument18(currentScope, flowContext, expectedType, ce.valueIfFalse, flowInfo,
                ce.ifFalseNullStatus, expectedNonNullness, originalExpected);
            return;
        } else if (argument instanceof SwitchExpression se && se.isPolyExpression()) {
            for (Expression rExpression : se.resultExpressions()) {
                se.internalAnalyseOneArgument18(currentScope, flowContext, expectedType, rExpression, flowInfo,
        ...
        internalAnalyseOneArgument18(currentScope, flowContext, expectedType, argument, flowInfo, nullStatus,
    ...
    void internalAnalyseOneArgument18(BlockScope currentScope, FlowContext flowContext, TypeBinding expectedType,
        Expression argument, FlowInfo flowInfo, int nullStatus, Boolean expectedNonNullness,
        TypeBinding originalExpected) {
        // here we consume special case information generated in the ctor of ParameterizedGenericMethodBinding (see
        // there):
        int statusFromAnnotatedNull = expectedNonNullness == Boolean.TRUE ? nullStatus : 0;

        NullAnnotationMatching annotationStatus = NullAnnotationMatching.analyse(expectedType, argument.resolvedType,
            null, null, nullStatus, argument, CheckMode.COMPATIBLE, false);
        ...
        if (statusFromAnnotatedNull == FlowInfo.NULL) {
            ...
            currentScope.problemReporter()
                .nullityMismatchingTypeAnnotation(argument, argument.resolvedType, expectedType, annotationStatus);
            ...
            flowContext.recordNullityMismatch(currentScope, argument, argument.resolvedType, expectedType, flowInfo,
    ...
    /* package */ void checkAgainstNullAnnotation(BlockScope scope, FlowContext flowContext, FlowInfo flowInfo,
        Expression expr, boolean localFlow) {
        ...
        if (useTypeAnnotations) {
            ...
            if ((tagBits & TagBits.AnnotationNonNull) != 0) {
                flowContext.recordNullityMismatch(scope, expr, expr.resolvedType, methodBinding.returnType, flowInfo,
                    nullStatus, null);
            }
        }
    }

    protected void checkAgainstNullTypeAnnotation(BlockScope scope, TypeBinding requiredType, Expression expression,
        FlowContext flowContext, FlowInfo flowInfo) {
        checkAgainstNullTypeAnnotation(scope, requiredType, expression, flowContext, flowInfo, true);
    }

    protected void checkAgainstNullTypeAnnotation(BlockScope scope, TypeBinding requiredType, Expression expression,
        FlowContext flowContext, FlowInfo flowInfo, boolean localFlow) {
        if (expression instanceof ConditionalExpression && expression.isPolyExpression()) {
            // drill into both branches using existing nullStatus per branch:
            ConditionalExpression ce = (ConditionalExpression) expression;
            internalCheckAgainstNullTypeAnnotation(scope, requiredType, ce.valueIfTrue, ce.ifTrueNullStatus,
                flowContext, flowInfo, localFlow);
            internalCheckAgainstNullTypeAnnotation(scope, requiredType, ce.valueIfFalse, ce.ifFalseNullStatus,
                flowContext, flowInfo, localFlow);
            return;
        } else if (expression instanceof SwitchExpression se && se.isPolyExpression()) {
            for (Expression rExpression : se.resultExpressions()) {
                internalCheckAgainstNullTypeAnnotation(scope, requiredType, rExpression,
    ...
    private void internalCheckAgainstNullTypeAnnotation(BlockScope scope, TypeBinding requiredType,
        Expression expression, int nullStatus, FlowContext flowContext, FlowInfo flowInfo, boolean localFlow) {
        NullAnnotationMatching annotationStatus = NullAnnotationMatching.analyse(requiredType, expression.resolvedType,
            null, null, nullStatus, expression, CheckMode.COMPATIBLE, localFlow);
        if (annotationStatus.isDefiniteMismatch()) {
            scope.problemReporter()
                .nullityMismatchingTypeAnnotation(expression, expression.resolvedType, requiredType, annotationStatus);
        } else {
            if (annotationStatus.wantToReport())
                annotationStatus.report(scope);
            if (annotationStatus.isUnchecked()) {
                flowContext.recordNullityMismatch(scope, expression, expression.resolvedType, requiredType, flowInfo,
                    nullStatus, annotationStatus);
            }
        }
    }
                ...
                case Scope.BLOCK_SCOPE: {
                    ...
                    if (bs.enclosingCase != null) {
                        if (bs.enclosingCase.swich instanceof SwitchExpression se)
                            return se;
                        if (implicitYield)
                            return null; // do not ascend to enclosing: implicit yield always binds to closest
...
// Inspect AST nodes looking for a break statement, descending into nested control structures only when necessary (looking for a break with a specific label.)
    public boolean breaksOut(final char[] label) {
        return new ASTVisitor() {
            ...
            public boolean visit(Block block, BlockScope scope) {
                if ((block.bits & BlockShouldEndDead) != 0) { // switch rule blocks don't fall through and have an
                                                              // implicit break unless they dead-end already.
                    Statement ultimateStatement
                        = block.statements == null ? null : block.statements[block.statements.length - 1];
            ...
            public boolean visit(YieldStatement yieldStatement, BlockScope skope) {
                if (yieldStatement.isImplicit && yieldStatement.switchExpression == null) // implicit yield in a switch
                                                                                          // rule with a statement
                                                                                          // expression implies an
                                                                                          // implicit break;
                    this.breaksOut = true;
                return false;
            }
     ...
     * where this analysis triggers only from do {} while (false); situations. See
     * LabeledStatement.continuesAtOuterLabel
     */
...
// complaintLevel = 0 if was reachable up until now, 1 if fake reachable (deadcode), 2 if fatal unreachable (error)
    ...
    public boolean isBoxingCompatible(TypeBinding expressionType, TypeBinding targetType, Expression expression,
        Scope scope) {
...
// for switch statement
        ...
        return new ASTVisitor() {
            ...
            public boolean visit(TypePattern typePattern, BlockScope blockScope) {
                if (typePattern.local != null
                    && (includeUnnamedOnes || (typePattern.local.name.length != 1 || typePattern.local.name[0] != '_')))
    ...
    /**
     * Implementation of {@link InvocationSite#invocationTargetType}
     ...
     */
    protected MethodBinding findConstructorBinding(BlockScope scope, Invocation site, ReferenceBinding receiverType,
