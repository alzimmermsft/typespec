...
public class BundleTracker<T> implements BundleTrackerCustomizer<T> {
	...
	private final class Tracked extends AbstractTracked<Bundle, T, BundleEvent> implements SynchronousBundleListener {
         ...
         */
		@Override
		T customizerAdding(final Bundle item) {
			return customizer.addingBundle(item, null);
		}
         ...
         */
		@Override
		void customizerRemoved(final Bundle item, final T object) {
			customizer.removedBundle(item, null, object);
		}
	}
}
