...
public class MethodDeclaration extends AbstractMethodDeclaration {
    ...
    public MethodDeclaration(CompilationResult compilationResult) {
        ...
        this.bits |= ASTNode.CanBeStatic; // Start with this assumption, will course correct during resolve and
                                          // analyseCode.
    }
    ...
    public void analyseCode(ClassScope classScope, FlowContext flowContext, FlowInfo flowInfo) {
        ...
        try {
            ...
            if (this.statements != null) {
                ...
                int complaintLevel = (flowInfo.reachMode() & FlowInfo.UNREACHABLE) == 0
    ...
    public void resolveStatements() {
        ...
        RecordComponent recordComponent = declaringClass != null
            && declaringClass.isRecord()
            && (this.arguments == null || this.arguments.length == 0)
                ? declaringClass.getRecordComponent(this.selector)
                : null;
        if (recordComponent != null) { // this is an accessor - does it pass muster ?
            if (this.returnType != null
                && TypeBinding.notEquals(this.returnType.resolvedType, recordComponent.type.resolvedType))
                this.scope.problemReporter()
                    .componentAccessorReturnTypeMismatch(this.returnType, recordComponent.type.resolvedType);
            if (this.typeParameters != null)
        ...
        if (this.typeParameters != null) {
            for (TypeParameter typeParameter : this.typeParameters) {
                ...
                if (returnsUndeclTypeVar
                    && TypeBinding.equalsEquals(typeParameter.binding, this.returnType.resolvedType)) {
        ...
        checkOverride: {
            ...
            if (hasOverrideAnnotation && !hasUnresolvedArguments) {
                ...
                if ((bindingModifiers & (ClassFileConstants.AccStatic | ExtraCompilerModifiers.AccOverriding))
                ...
                if ((bindingModifiers & (ClassFileConstants.AccStatic | ExtraCompilerModifiers.AccImplementing))
                    == ExtraCompilerModifiers.AccImplementing)
                    break checkOverride;
                // claims to override, and doesn't actually do so
                this.scope.problemReporter().methodMustOverride(this, complianceLevel);
            } else {
                // In case of a concrete class method, we have to check if it overrides(in 1.5 and above) OR implements
                // a method(1.6 and above).
                // Also check if the method has a signature that is override-equivalent to that of any public method
                // declared in Object.
                if (!declaringClass.isInterface()) {
                    ...
                    // In case of a interface class method, we have to check if it overrides a method (isImplementing
                    // returns true in case it overrides)
                    // Also check if the method has a signature that is override-equivalent to that of any public method
                    // declared in Object.
                    if (compilerOptions.reportMissingOverrideAnnotationForInterfaceMethodImplementation
                        && (((bindingModifiers & (ClassFileConstants.AccStatic | ExtraCompilerModifiers.AccOverriding))
                            == ExtraCompilerModifiers.AccOverriding) || this.binding.isImplementing())) {
                        // actually overrides, but did not claim to do so
        ...
        switch (TypeDeclaration.kind(this.scope.referenceType().modifiers)) {
            ...
            case TypeDeclaration.RECORD_DECL:
                ...
                if ((this.modifiers & ExtraCompilerModifiers.AccSemicolonBody) != 0) {
                    ...
                    if (((this.modifiers & ClassFileConstants.AccNative) != 0)
                        ...
                        || (declaringClass instanceof LocalTypeBinding)
                        || returnsUndeclTypeVar) {
            ...
            case TypeDeclaration.INTERFACE_DECL:
                if ((this.modifiers & (ExtraCompilerModifiers.AccSemicolonBody | ClassFileConstants.AccAbstract))
                    ...
                    if (isPrivateMethod
                        || ((this.modifiers & (ClassFileConstants.AccStatic | ExtraCompilerModifiers.AccDefaultMethod))
                            != 0)) {
                        this.scope.problemReporter().methodNeedBody(this);
                    }
                }
        ...
        if (compilerOptions.getSeverity(CompilerOptions.OverridingMethodWithoutSuperInvocation)
            != ProblemSeverities.Ignore) {
            if (this.binding != null) {
                int bindingModifiers = this.binding.modifiers;
                if ((bindingModifiers & (ExtraCompilerModifiers.AccOverriding | ExtraCompilerModifiers.AccImplementing))
                    == ExtraCompilerModifiers.AccOverriding
