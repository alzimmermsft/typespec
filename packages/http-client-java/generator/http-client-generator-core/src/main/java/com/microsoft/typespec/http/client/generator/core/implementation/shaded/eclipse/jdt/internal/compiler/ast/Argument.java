...
public class Argument extends LocalDeclaration {
        ...
        if (this.isUnnamed(scope) && !scope.isLambdaScope()) {
            scope.problemReporter()
                .illegalUseOfUnderscoreAsAnIdentifier(this.sourceStart, this.sourceEnd,
                    scope.compilerOptions().sourceLevel > ClassFileConstants.JDK1_8, true);
        }

        TypeBinding newTypeBinding = createBinding(scope, typeBinding); // basically a no-op if createBinding() was
                                                                        // called before
        ...
        if (existingVariable != null && existingVariable.isValidBinding()) {
            ...
            if (localExists && this.hiddenVariableDepth == 0) {
                if (!this.isUnnamed(scope)) {
                    ...
                    } else if (scope.referenceContext instanceof ConstructorDeclaration cd
    ...
    public TypeBinding resolveForCatch(BlockScope scope) {
        ...
        if ((this.type.bits & ASTNode.IsUnionType) != 0) {
            this.binding
                = new CatchParameterBinding(this, exceptionType, this.modifiers | ClassFileConstants.AccFinal, false); // argument
                                                                                                                       // decl,
                                                                                                                       // but
                                                                                                                       // local
                                                                                                                       // var
            ...
            this.binding = new CatchParameterBinding(this, exceptionType, this.modifiers, false); // argument decl, but
                                                                                                  // local var (where
                                                                                                  // isArgument = false)
        }
