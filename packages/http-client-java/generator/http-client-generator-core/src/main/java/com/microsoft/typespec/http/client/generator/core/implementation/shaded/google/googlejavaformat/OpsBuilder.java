...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.google.common.collect.ArrayListMultimap;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.google.googlejavaformat.Indent.Const;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.google.googlejavaformat.Input.Tok;
...
public final class OpsBuilder {
    ...
    public ImmutableList<Tok> peekTokens(int startPosition, Predicate<Tok> predicate) {
        ImmutableList<? extends Token> tokens = input.getTokens();
        Preconditions.checkState(tokens.get(tokenI).getTok().getPosition() == startPosition,
            "Expected the current token to be at position %s, found: %s", startPosition, tokens.get(tokenI));
    ...
    public final void guessToken(String token) {
        token(token, Doc.Token.RealOrImaginary.IMAGINARY, ZERO, /* breakAndIndentTrailingComment= */ Optional.empty());
    }

    public final void token(String token, Doc.Token.RealOrImaginary realOrImaginary, Indent plusIndentCommentsBefore,
