        ...
        for (OperationGroup operationGroup : operationGroups) {
            ...
            for (Operation operation : operationGroup.getOperations()) {
                ...
                for (Request request : operation.getRequests()) {
                    Stream<Parameter> newParameters
                        = Stream.concat(operation.getParameters().stream(), request.getParameters().stream());
                            ...
                            if (parameter.getSchema() instanceof BinarySchema) {
                                if (request.getParameters()
                                    ...
                                    .noneMatch(p -> p.getProtocol() != null
                                        ...
                                        && "content-length"
                                            .equalsIgnoreCase(p.getLanguage().getDefault().getSerializedName()))) {
        ...
        for (ObjectSchema objectSchema : codeModel.getSchemas().getObjects()) {
            ...
            for (Property property : objectSchema.getProperties()) {
                if (property.getExtensions() != null
                    && property.getExtensions().isXmsClientFlatten()
                    && property.getSchema() instanceof ObjectSchema) {
                    ObjectSchema flattenedSchema = (ObjectSchema) property.getSchema();

                    boolean isPolymorphic
                        = flattenedSchema.getDiscriminator() != null || flattenedSchema.getDiscriminatorValue() != null;
                    if (isPolymorphic) {
//            LOGGER.warn("x-ms-client-flatten is not allowed on polymorphic model '{}', on property '{}'", flattenedSchema.getLanguage().getJava().getName(), property.getLanguage().getJava().getName());
    ...
    private static boolean nonNullNextLink(Operation operation) {
        return operation.getExtensions().getXmsPageable().getNextLinkName() != null
            && !operation.getExtensions().getXmsPageable().getNextLinkName().isEmpty();
    }
    ...
    private static class OperationSignature {
        ...
        public boolean equals(Object o) {
            ...
            return Objects.equals(operationGroup, that.operationGroup)
     ...
     * Current operation and generated nextOperation share the same extension instance. next page operation's
     * nextOperation property should
     * always point to itself for it to be recognized as the next page operation(see
     * ClientMethodMapper#createPageableClientMethods in javagen module).
     *
    ...
    private void addPagingNextOperation(Client client, OperationGroup operationGroup, Operation operation) {
        ...
        if (operationGroup.getOperations()
            .stream()
