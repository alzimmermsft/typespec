...
public class BinaryTypeConverter extends TypeConverter {
	...
	private AbstractMethodDeclaration convert(IMethod method, IType type) throws JavaModelException {
		...
		com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.TypeParameter[] typeParams = null;
	...
	private TypeDeclaration convert(IType type, IType alreadyComputedMember,TypeDeclaration alreadyComputedMemberDeclaration) throws JavaModelException {
		...
		if (typeParameters != null && typeParameters.length > 0) {
			int parameterCount = typeParameters.length;
			com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.TypeParameter[] typeParams = new org.eclipse.jdt.internal.compiler.ast.TypeParameter[parameterCount];
			for (int i = 0; i < parameterCount; i++) {
				ITypeParameter typeParameter = typeParameters[i];
				typeParams[i] =
					createTypeParameter(
