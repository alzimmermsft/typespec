...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.parser.TypeConverter;
...
public class BinaryTypeConverter extends TypeConverter {
    ...
    public TypeDeclaration buildTypeDeclaration(IType type, CompilationUnitDeclaration compilationUnit)
        ...
        if (packageName.length > 0) {
            compilationUnit.currentPackage
                = new ImportReference(packageName, new long[] { 0 }, false, ClassFileConstants.AccDefault);
        }
    ...
    private AbstractMethodDeclaration convert(IMethod method, IType type) throws JavaModelException {
        ...
        com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.TypeParameter[] typeParams
            = null;
        ...
        if (method.isConstructor()) {
            ...
            MethodDeclaration decl = type.isAnnotation()
                ? new AnnotationMethodDeclaration(this.compilationResult)
        ...
        for (int i = 0; i < argumentCount; i++) {
            ...
            methodDeclaration.arguments[i]
                = new Argument(argumentNames[i].toCharArray(), 0, typeReference, ClassFileConstants.AccDefault);
    ...
    private TypeDeclaration convert(IType type, IType alreadyComputedMember,
        ...
        if (typeParameters != null && typeParameters.length > 0) {
            int parameterCount = typeParameters.length;
            com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.TypeParameter[] typeParams
                = new org.eclipse.jdt.internal.compiler.ast.TypeParameter[parameterCount];
            for (int i = 0; i < parameterCount; i++) {
                ITypeParameter typeParameter = typeParameters[i];
                typeParams[i] = createTypeParameter(typeParameter.getElementName().toCharArray(),
                    stringArrayToCharArray(typeParameter.getBounds()), 0, 0);
        ...
        for (int i = 0; i < memberTypeCount; i++) {
            if (alreadyComputedMember != null
                && alreadyComputedMember.getFullyQualifiedName().equals(memberTypes[i].getFullyQualifiedName())) {
        ...
        if (count != methodCount) {
            System.arraycopy(typeDeclaration.methods, 0,
                typeDeclaration.methods = new AbstractMethodDeclaration[count + neededCount], 0, count + neededCount);
        }
