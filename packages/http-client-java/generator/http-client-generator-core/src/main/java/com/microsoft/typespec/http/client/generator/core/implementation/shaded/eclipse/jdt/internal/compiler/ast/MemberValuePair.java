...
public class MemberValuePair extends ASTNode {
    ...
    public void resolveTypeExpecting(BlockScope scope, TypeBinding requiredType) {
        ...
        this.value.setExpectedType(requiredType); // needed in case of generic method invocation - looks suspect,
                                                  // generic method invocation here ???
        ...
        if (this.value instanceof ArrayInitializer) {
            ...
            scope.problemReporter()
                .annotationValueMustBeArrayInitializer(this.binding.declaringClass, this.name, this.value);
            this.value.resolveType(scope);
        ...
        if (!(this.value.isConstantValueOfTypeAssignableToType(valueType, requiredType)
            ...
            if (!(requiredType.isArrayType()
                ...
                if (leafType.isAnnotationType() && !valueType.isAnnotationType()) {
                    scope.problemReporter()
                        .annotationValueMustBeAnnotation(this.binding.declaringClass, this.name, this.value, leafType);
                } else {
            ...
            scope.compilationUnitScope()
                .recordTypeConversion(requiredType.leafComponentType(), valueType.leafComponentType());
        ...
        checkAnnotationMethodType: {
            switch (leafType.erasure().id) {
                ...
                case T_JavaLangString:
                    if (this.value instanceof ArrayInitializer) {
                        ...
                        if (expressions != null) {
                            for (Expression expression : expressions) {
                                ...
                                if (expression.constant == Constant.NotAConstant) {
                                    scope.problemReporter()
                                        .annotationValueMustBeConstant(this.binding.declaringClass, this.name,
                                            expression, false);
                                }
                            }
                        }
                        ...
                        if (valueType.isArrayType()) {
                            scope.problemReporter()
                                .annotationValueMustBeArrayInitializer(this.binding.declaringClass, this.name,
                                    this.value);
                        } else {
                            scope.problemReporter()
                                .annotationValueMustBeConstant(this.binding.declaringClass, this.name, this.value,
                                    false);
                        }
                    }
                ...
                case T_JavaLangClass:
                    if (this.value instanceof ArrayInitializer) {
                        ...
                        if (expressions != null) {
                            for (Expression currentExpression : expressions) {
                                if (!(currentExpression instanceof ClassLiteralAccess)) {
                                    scope.problemReporter()
                                        .annotationValueMustBeClassLiteral(this.binding.declaringClass, this.name,
                                            currentExpression);
                                }
                            }
                        }
                        ...
                        scope.problemReporter()
                            .annotationValueMustBeClassLiteral(this.binding.declaringClass, this.name, this.value);
                    }
                    break checkAnnotationMethodType;
            }
            if (leafType.isEnum()) {
                if (this.value instanceof NullLiteral) {
                    scope.problemReporter()
                        .annotationValueMustBeConstant(this.binding.declaringClass, this.name, this.value, true);
            ...
            if (leafType.isAnnotationType()) {
                if (!valueType.leafComponentType().isAnnotationType()) { // check annotation type and also reject null
                                                                         // literal
                    scope.problemReporter()
                        .annotationValueMustBeAnnotation(this.binding.declaringClass, this.name, this.value, leafType);
                } else if (this.value instanceof ArrayInitializer) {
                    ...
                    if (expressions != null) {
                        for (Expression currentExpression : expressions) {
                            if (currentExpression instanceof NullLiteral
                                || !(currentExpression instanceof Annotation)) {
                                scope.problemReporter()
                                    .annotationValueMustBeAnnotation(this.binding.declaringClass, this.name,
                                        currentExpression, leafType);
                            }
                        }
                    }
