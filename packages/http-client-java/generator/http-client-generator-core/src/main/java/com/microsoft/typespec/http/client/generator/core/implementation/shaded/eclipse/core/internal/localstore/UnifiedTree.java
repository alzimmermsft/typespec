...
public class UnifiedTree {
    ...
    private static boolean disable_advanced_recursive_link_checks = System.getProperty(
        "com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.core.resources.disable_advanced_recursive_link_checks") //$NON-NLS-1$
        != null;
    ...
    protected void addChildren(UnifiedTreeNode node) {
        ...
        if (!unknown
            && (parentType == IResource.FOLDER || parentType == IResource.PROJECT)
            && parent.exists(flags, true)) {
            List<IResource> members;
            try {
                IContainer container = (IContainer) parent;
                members = Arrays
                    .asList(container.members(IContainer.INCLUDE_TEAM_PRIVATE_MEMBERS | IContainer.INCLUDE_HIDDEN));
            ...
            while (workspaceIndex < members.size()) {
                ...
                if (target.isLinked()) {
                    ...
                    if (localInfo.getAttribute(EFS.ATTRIBUTE_SYMLINK)
                        && localInfo.isDirectory()
    ...
    protected void addChildrenFromFileSystem(UnifiedTreeNode node, List<IFileInfo> childInfos, int index) {
        ...
        for (IFileInfo info : childInfos.subList(index, childInfos.size())) {
            ...
            if (!info.getAttribute(EFS.ATTRIBUTE_SYMLINK)
                || !info.isDirectory()
                || !isRecursiveLink(node.getStore(), info)) {
    ...
    /**
     * Creates a child node for a location in the file system. Does nothing and returns null if the location does not
     * correspond to a valid file/folder.
     */
     ...
     * for all files.
     */
    protected UnifiedTreeNode createNode(IResource resource, IFileStore store, IFileInfo info,
    ...
    protected List<IFileInfo> getLocalList(UnifiedTreeNode node) {
        try {
            ...
            IFileInfo[] list;
            if (fileTree != null
                && (fileTree.getTreeRoot().equals(store) || fileTree.getTreeRoot().isParentOf(store))) {
    ...
    private boolean isRecursiveLink(IFileStore parentStore, IFileInfo localInfo) {
        ...
        try {
            ...
            if (pathPrefixHistory.containsAsPrefix(childPath)) {
                ...
                // child points into a different portion of the tree that we visited already before, or will certainly
                // visit.
