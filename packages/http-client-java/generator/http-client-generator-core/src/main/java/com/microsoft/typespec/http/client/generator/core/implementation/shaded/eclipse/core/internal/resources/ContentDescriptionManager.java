...
public class ContentDescriptionManager
    implements IRegistryChangeListener, IContentTypeManager.IContentTypeChangeListener, ILifecycleListener {
    ...
    private static final QualifiedName CACHE_STATE
        = new QualifiedName(ResourcesPlugin.PI_RESOURCES, "contentCacheState"); //$NON-NLS-1$

    public static final String FAMILY_DESCRIPTION_CACHE_FLUSH
        = ResourcesPlugin.PI_RESOURCES + ".contentDescriptionCacheFamily"; //$NON-NLS-1$
            ...
            if (getCacheState() != ABOUT_TO_FLUSH) {
                ...
                if (newDescription.getContentType().getDefaultDescription().equals(newDescription)) {
                    ...
                    if (newDescription.getContentType().equals(defaultForName)) {
                        // it is a default description for the obvious content type given its file name, we don't have
                        // to cache
    ...
    private IContentDescription readDescription(File file) throws CoreException {
        ...
        try (InputStream contents = new LazyFileInputStream(file.getStore());) {
    ...
    public void handleEvent(LifecycleEvent event) {
        ...
        switch (event.kind) {
            case LifecycleEvent.POST_PROJECT_CHANGE:
                // if the project changes, its natures may have changed as well (content types may be associated to
                // natures)
