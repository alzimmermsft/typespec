...
//					}
     ...
     * Boolean generation for {@code <=}
     */
    public void generateOptimizedLessThanOrEqual(BlockScope currentScope, CodeStream codeStream, BranchLabel trueLabel,
        BranchLabel falseLabel, boolean valueRequired) {
        int promotedTypeID = (this.left.implicitConversion & TypeIds.IMPLICIT_CONVERSION_MASK) >> 4;
        // both sides got promoted in the same way
        if (promotedTypeID == TypeIds.T_int) {
    ...
    public void buildStringForConcatenation(BlockScope blockScope, CodeStream codeStream, int typeID,
        ...
        if ((((this.bits & ASTNode.OperatorMASK) >> ASTNode.OperatorSHIFT) == OperatorIds.PLUS)
            ...
            if (this.constant != Constant.NotAConstant) {
                ...
                this.left.buildStringForConcatenation(blockScope, codeStream,
                    this.left.implicitConversion & TypeIds.COMPILE_TYPE_MASK, recipe, argTypes);
