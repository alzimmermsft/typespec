...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.CaseStatement.LabelExpression;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.SwitchStatement.SingletonBootstrap;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.codegen.*;
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.codegen.StackMapFrameCodeStream.ExceptionMarker;
...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.problem.ShouldNotImplement;
...
public class ClassFile implements TypeConstants, TypeIds {
    ...
    private static void createProblemType(TypeDeclaration typeDeclaration, ClassFile parentClassFile,
        ...
        if (methodDecls != null) {
            if (typeBinding.isInterface()) {
                // We generate a clinit which contains all the problems, since we may not be able to generate problem
                // methods (< 1.8) and problem constructors (all levels).
    ...
    public void addAttributes() {
        ...
        if (this.referenceBinding.isNestedType() && !this.referenceBinding.isMemberType()) {
        ...
        if (typeDeclaration != null) {
            ...
            if (annotations != null) {
                ...
                if (typeDeclaration.isPackageInfo())
                    ...
                    targetMask = TagBits.AnnotationForType | TagBits.AnnotationForTypeUse; // 9.7.4 ... applicable to
                                                                                           // type declarations or in
                                                                                           // type contexts
    ...
    private int addFieldAttributes(FieldBinding fieldBinding, int fieldAttributeOffset) {
        ...
        if (fieldDeclaration != null) {
            ...
            if ((this.produceAttributes & ClassFileConstants.ATTR_TYPE_ANNOTATION) != 0) {
                ...
                attributesNumber = completeRuntimeTypeAnnotations(attributesNumber, null, node -> size > 0,
                    () -> allTypeAnnotationContexts);
            }
        }
        ...
        if (recordComponent != null) {
            ...
            if ((this.produceAttributes & ClassFileConstants.ATTR_TYPE_ANNOTATION) != 0) {
                ...
                if (annotations != null && (recordComponent.bits & ASTNode.HasTypeAnnotations) != 0) {
                    recordComponent.getAllAnnotationContexts(AnnotationTargetTypeConstants.RECORD_COMPONENT,
                        allTypeAnnotationContexts);
                }
                ...
                if (recordComponentType != null && ((recordComponentType.bits & ASTNode.HasTypeAnnotations) != 0)) {
                    recordComponentType.getAllAnnotationContexts(AnnotationTargetTypeConstants.RECORD_COMPONENT,
                        allTypeAnnotationContexts);
                }
                int size = allTypeAnnotationContexts.size();
                attributesNumber = completeRuntimeTypeAnnotations(attributesNumber, null, node -> size > 0,
    ...
    private void addMissingAbstractProblemMethod(MethodDeclaration methodDeclaration, MethodBinding methodBinding,
        CategorizedProblem problem, CompilationResult compilationResult) {
        // always clear the strictfp/native/abstract bit for a problem method
        generateMethodInfoHeader(methodBinding, methodBinding.modifiers
            & ~(ClassFileConstants.AccStrictfp | ClassFileConstants.AccNative | ClassFileConstants.AccAbstract));
        int methodAttributeOffset = this.contentsOffset;
        int attributeNumber = generateMethodInfoAttributes(methodBinding);
        ...
        completeCodeAttributeForMissingAbstractProblemMethod(methodBinding, codeAttributeOffset,
            compilationResult.getLineSeparatorPositions(), problem.getSourceLineNumber());
    ...
    public void addProblemClinit(CategorizedProblem[] problems) {
        ...
        completeCodeAttributeForClinit(codeAttributeOffset, problemLine, null);
        if (this.contentsOffset + 2 >= this.contents.length) {
            resizeContents(2);
        }
     ...
     * @param problems org.eclipse.jdt.internal.compiler.problem.Problem[]
     */
    public void addProblemConstructor(AbstractMethodDeclaration method, MethodBinding methodBinding,
        ...
        // return codeStream.generateCodeAttributeForProblemMethod(comp.options.runtimeExceptionNameForCompileError, "")
        this.codeStream.generateCodeAttributeForProblemMethod(problemString);
        completeCodeAttributeForProblemMethod(method, methodBinding, codeAttributeOffset,
            ((SourceTypeBinding) methodBinding.declaringClass).scope.referenceCompilationUnit().compilationResult
     ...
     */
    public void addProblemConstructor(AbstractMethodDeclaration method, MethodBinding methodBinding,
        CategorizedProblem[] problems, int savedOffset) {
     ...
     * @param problems org.eclipse.jdt.internal.compiler.problem.Problem[]
     */
    public void addProblemMethod(AbstractMethodDeclaration method, MethodBinding methodBinding,
        CategorizedProblem[] problems) {
    ...
    public void addSpecialMethods(TypeDeclaration typeDecl) {
        ...
        generateMissingAbstractMethods(this.referenceBinding.scope.referenceType().missingAbstractMethods,
            this.referenceBinding.scope.referenceCompilationUnit().compilationResult);
                ...
                for (int i = emittedSyntheticsCount, max = currentSyntheticsCount; i < max; i++) {
                    ...
                    switch (syntheticMethod.purpose) {
                        ...
                        case SyntheticMethodBinding.BridgeMethod:
                            // generate a method info to emulate an access to a non-accessible method / super-method or
                            // bridge method
                        ...
                        case SyntheticMethodBinding.LambdaMethod:
                            ...
                            continueScanningSynthetics = true; // lambda code generation could schedule additional
                                                               // nested lambdas for code generation.
                            break;
    ...
    private void addSyntheticRecordCanonicalConstructor(TypeDeclaration typeDecl,
        ...
        completeCodeAttributeForSyntheticMethod(methodBinding, codeAttributeOffset,
            ((SourceTypeBinding) methodBinding.declaringClass).scope.referenceCompilationUnit().compilationResult
                .getLineSeparatorPositions());
        completeMethodInfo(methodBinding, methodAttributeOffset, attributeNumber);
    }

    private void addSyntheticRecordOverrideMethods(TypeDeclaration typeDecl, SyntheticMethodBinding methodBinding,
        int purpose) {
    ...
    public void addSyntheticSwitchTable(SyntheticMethodBinding methodBinding) {
        ...
        completeCodeAttributeForSyntheticMethod(true, methodBinding, codeAttributeOffset,
            ((SourceTypeBinding) methodBinding.declaringClass).scope.referenceCompilationUnit().compilationResult
                .getLineSeparatorPositions(),
            ((SourceTypeBinding) methodBinding.declaringClass).scope);
        // update the number of attributes
        this.contents[methodAttributeOffset++] = (byte) (attributeNumber >> 8);
        this.contents[methodAttributeOffset] = (byte) attributeNumber;
    }
    ...
    public void completeCodeAttribute(int codeAttributeOffset, MethodScope scope) {
        ...
        if (code_length > 65535) {
            if (this.codeStream.methodDeclaration != null) {
                this.codeStream.methodDeclaration.scope.problemReporter()
                    .bytecodeExceeds64KLimit(this.codeStream.methodDeclaration);
            } else {
                this.codeStream.lambdaExpression.scope.problemReporter()
                    .bytecodeExceeds64KLimit(this.codeStream.lambdaExpression);
            }
        }
        ...
        if (max_stack > 65535) {
            if (this.codeStream.methodDeclaration != null) {
                this.codeStream.methodDeclaration.scope.problemReporter()
                    .operandStackExceeds64KLimit(this.codeStream.methodDeclaration);
            } else {
                this.codeStream.lambdaExpression.scope.problemReporter()
                    .operandStackExceeds64KLimit(this.codeStream.lambdaExpression);
            }
        }
                    ...
                    if (this.codeStream.methodDeclaration != null) {
                        this.codeStream.methodDeclaration.scope.problemReporter()
                            .abortDueToInternalError(
                                Messages.bind(Messages.abort_invalidExceptionAttribute,
                                    new String(this.codeStream.methodDeclaration.selector)),
                                this.codeStream.methodDeclaration);
                    } else {
                        this.codeStream.lambdaExpression.scope.problemReporter()
                            .abortDueToInternalError(
                                Messages.bind(Messages.abort_invalidExceptionAttribute,
                                    new String(this.codeStream.lambdaExpression.binding.selector)),
...
//						stackMapFrameCodeStream.addExceptionMarker(handlerPC, exceptionLabel.exceptionType);
                    ...
                    if (exceptionLabel.exceptionType == null) {
                        ...
                        if (exceptionLabel.exceptionType == TypeBinding.NULL) {
                            ...
                            nameIndex = this.constantPool
                                .literalIndexForType(ConstantPool.JavaLangClassNotFoundExceptionConstantPoolName);
                        } else {
        ...
        if ((this.produceAttributes & ClassFileConstants.ATTR_VARS) != 0) {
            final boolean methodDeclarationIsStatic = this.codeStream.methodDeclaration != null
        ...
        if (addStackMaps) {
            attributesNumber += generateStackMapTableAttribute(
                this.codeStream.methodDeclaration != null
                    ...
                    : this.codeStream.lambdaExpression.binding,
                code_length, codeAttributeOffset, max_locals, false, scope);
        }
    ...
    public int generateTypeAnnotationsOnCodeAttribute() {
        ...
        List<AnnotationContext> allTypeAnnotationContexts
            = ((TypeAnnotationCodeStream) this.codeStream).allTypeAnnotationContexts;
        ...
        for (int i = 0, max = this.codeStream.allLocalsCounter; i < max; i++) {
            ...
            int targetType = ((localVariable.tagBits & TagBits.IsResource) == 0)
                ? AnnotationTargetTypeConstants.LOCAL_VARIABLE
                : AnnotationTargetTypeConstants.RESOURCE_VARIABLE;
        ...
        for (int i = 0, max = this.codeStream.exceptionLabelsCounter; i < max; i++) {
            ...
            if (exceptionLabel.exceptionTypeReference != null
                && (exceptionLabel.exceptionTypeReference.bits & ASTNode.HasTypeAnnotations) != 0) {
                exceptionLabel.exceptionTypeReference.getAllAnnotationContexts(
                    AnnotationTargetTypeConstants.EXCEPTION_PARAMETER, i, allTypeAnnotationContexts,
                    exceptionLabel.se7Annotations);
            }
        }
        int size = allTypeAnnotationContexts.size();
        attributesNumber
            = completeRuntimeTypeAnnotations(attributesNumber, null, node -> size > 0, () -> allTypeAnnotationContexts);
        return attributesNumber;
    }
    ...
    public void completeCodeAttributeForClinit(int codeAttributeOffset, Scope scope) {
        ...
        if (code_length > 65535) {
            this.codeStream.methodDeclaration.scope.problemReporter()
                .bytecodeExceeds64KLimit(this.codeStream.methodDeclaration.scope.referenceType());
        }
        if (localContentsOffset + 20 >= this.contents.length) {
            resizeContents(20);
        }
...
//						stackMapFrameCodeStream.addExceptionMarker(handlerPC, exceptionLabel.exceptionType);
        ...
        if ((this.produceAttributes & ClassFileConstants.ATTR_STACK_MAP_TABLE) != 0) {
            attributesNumber
                += generateStackMapTableAttribute(null, code_length, codeAttributeOffset, max_locals, true, scope);
        }
    ...
    public void completeCodeAttributeForMissingAbstractProblemMethod(MethodBinding binding, int codeAttributeOffset,
        ...
        this.contents = this.codeStream.bCodeStream;
        int localContentsOffset = this.codeStream.classFileOffset;
        // codeAttributeOffset is the position inside localContents byte array before we started to write// any
        // information about the codeAttribute// That means that to write the attribute_length you need to offset by 2
        // the value of codeAttributeOffset// to get the right position, 6 for the max_stack etc...
        int max_stack = this.codeStream.stackMax;
        this.contents[codeAttributeOffset + 6] = (byte) (max_stack >> 8);
        this.contents[codeAttributeOffset + 7] = (byte) max_stack;
        int max_locals = this.codeStream.maxLocals;
        ...
        if ((this.produceAttributes & ClassFileConstants.ATTR_LINES) != 0) {
            if (problemLine == 0) {
                problemLine
                    = Util.getLineNumber(binding.sourceStart(), startLineIndexes, 0, startLineIndexes.length - 1);
            }
            attributesNumber += generateLineNumberAttribute(problemLine);
        }
        ...
        if ((this.produceAttributes & ClassFileConstants.ATTR_STACK_MAP_TABLE) != 0) {
            attributesNumber
                += generateStackMapTableAttribute(binding, code_length, codeAttributeOffset, max_locals, false, null);
        }

        // then we do the local variable attribute
        // update the number of attributes// ensure first that there is enough space available inside the localContents
        // array
        if (codeAttributeAttributeOffset + 2 >= this.contents.length) {
            resizeContents(2);
        }
    ...
    public void completeCodeAttributeForSyntheticMethod(boolean hasExceptionHandlers, SyntheticMethodBinding binding,
        int codeAttributeOffset, int[] startLineIndexes, Scope scope) {
        // reinitialize the contents with the byte modified by the code stream
        this.contents = this.codeStream.bCodeStream;
        int localContentsOffset = this.codeStream.classFileOffset;
        // codeAttributeOffset is the position inside contents byte array before we started to write
            ...
            for (int i = 0, max = this.codeStream.exceptionLabelsCounter; i < max; i++) {
                ...
                if (exceptionLabel != null) {
                    ...
                    if ((maxRange & 1) != 0) {
                        ...
                        problemReporter.abortDueToInternalError(Messages.bind(Messages.abort_invalidExceptionAttribute,
                            new String(binding.selector), problemReporter.referenceContext));
                    }
                    while (iRange < maxRange) {
                        int start = exceptionLabel.ranges[iRange++]; // even ranges are start positions
                        this.contents[localContentsOffset++] = (byte) (start >> 8);
                        ...
                        if (exceptionLabel.exceptionType == null) {
                            ...
                            switch (exceptionLabel.exceptionType.id) {
                                case T_null:
                                    /* represents ClassNotFoundException, see class literal access */
                                    nameIndex = this.constantPool.literalIndexForType(
                                        ConstantPool.JavaLangClassNotFoundExceptionConstantPoolName);
                                    break;

                                case T_long:
                                    /* represents NoSuchFieldError, see switch table generation */
                                    nameIndex = this.constantPool
                                        .literalIndexForType(ConstantPool.JavaLangNoSuchFieldErrorConstantPoolName);
                                    break;
        ...
        if (addStackMaps) {
            attributesNumber
                += generateStackMapTableAttribute(binding, code_length, codeAttributeOffset, max_locals, false, scope);
        }
    ...
    private void completeArgumentAnnotationInfo(AbstractVariableDeclaration[] arguments,
        ...
        for (int i = 0, max = arguments.length; i < max; i++) {
            ...
            if ((argument.bits & ASTNode.HasTypeAnnotations) != 0) {
                argument.getAllAnnotationContexts(AnnotationTargetTypeConstants.METHOD_FORMAL_PARAMETER, i,
            ...
            if (methodDeclaration != null) {
                if ((methodDeclaration.bits & ASTNode.HasTypeAnnotations) != 0) {
                    ...
                    if (receiver != null && (receiver.type.bits & ASTNode.HasTypeAnnotations) != 0) {
                        receiver.type.getAllAnnotationContexts(AnnotationTargetTypeConstants.METHOD_RECEIVER,
                            allTypeAnnotationContexts);
                    }
                }
                Annotation[] annotations = methodDeclaration.annotations;
                if (annotations != null
                    && !methodDeclaration.isClinit()
                    && (methodDeclaration.isConstructor() || binding.returnType.id != T_void)) {
                    // at source level type annotations have not been moved from declaration to type use position,
                    // collect them now:
                    methodDeclaration.getAllAnnotationContexts(AnnotationTargetTypeConstants.METHOD_RETURN,
                        allTypeAnnotationContexts);
                }
                if (!methodDeclaration.isConstructor()
                    && !methodDeclaration.isClinit()
                    && binding.returnType.id != T_void) {
                    ...
                    if ((typeReference.bits & ASTNode.HasTypeAnnotations) != 0) {
                        typeReference.getAllAnnotationContexts(AnnotationTargetTypeConstants.METHOD_RETURN,
                            allTypeAnnotationContexts);
                    }
                }
                ...
                if (thrownExceptions != null) {
                    for (int i = 0, max = thrownExceptions.length; i < max; i++) {
                        ...
                        thrownException.getAllAnnotationContexts(AnnotationTargetTypeConstants.THROWS, i,
                            allTypeAnnotationContexts);
                    }
                }
                ...
                if (typeParameters != null) {
                    for (int i = 0, max = typeParameters.length; i < max; i++) {
                        ...
                        if ((typeParameter.bits & ASTNode.HasTypeAnnotations) != 0) {
                            typeParameter.getAllAnnotationContexts(AnnotationTargetTypeConstants.METHOD_TYPE_PARAMETER,
                                i, allTypeAnnotationContexts);
                        }
                    }
                }
            } else if (binding instanceof SyntheticMethodBinding syntheticMethod
                && syntheticMethod.isCanonicalConstructor()) {
    ...
    private int generateNestMembersAttribute() {
        ...
        List<String> nestedMembers
            = nestMembers.stream().map(s -> new String(s.constantPoolName())).sorted().collect(Collectors.toList());
    ...
    private int generateModuleAttribute(ModuleDeclaration module) {
        ...
        int module_version_idx
            = moduleVersion == null ? 0 : this.constantPool.literalIndex(moduleVersion.toCharArray());
        ...
        for (int i = 0; i < module.exportsCount; i++) {
            ...
            int nameIndex
                = this.constantPool.literalIndexForPackage(CharOperation.replaceOnCopy(ref.pkgName, '.', '/'));
            this.contents[localContentsOffset++] = (byte) (nameIndex >> 8);
            this.contents[localContentsOffset++] = (byte) (nameIndex);
        ...
        for (int i = 0; i < module.usesCount; i++) {
            int nameIndex = this.constantPool
                .literalIndexForType(module.uses[i].serviceInterface.resolvedType.constantPoolName());
            this.contents[localContentsOffset++] = (byte) (nameIndex >> 8);
            this.contents[localContentsOffset++] = (byte) (nameIndex);
        }
        attrLength += usesSize;
        ...
        for (int i = 0; i < module.servicesCount; i++) {
            ...
            int nameIndex = this.constantPool
                .literalIndexForType(module.services[i].serviceInterface.resolvedType.constantPoolName());
            this.contents[localContentsOffset++] = (byte) (nameIndex >> 8);
            this.contents[localContentsOffset++] = (byte) (nameIndex);
            ...
            if (defaultValueBinding.isEnum()) {
                ...
                if (defaultValue instanceof ClassLiteralAccess) {
                    ...
                    final int classInfoIndex
                        = this.constantPool.literalIndex(classLiteralAccess.targetType.signature());
    ...
    private int generateEnclosingMethodAttribute() {
        ...
        int enclosingTypeIndex
            = this.constantPool.literalIndexForType(this.referenceBinding.enclosingType().constantPoolName());
    ...
    private int generateInnerClassAttribute(int numberOfInnerClasses, ReferenceBinding[] innerClasses) {
        ...
        for (int i = 0; i < numberOfInnerClasses; i++) {
            ...
            if (innerClass.isMemberType()) {
                ...
                int outerClassIndex
                    = this.constantPool.literalIndexForType(innerClass.enclosingType().constantPoolName());
        ...
        if ((functional instanceof LambdaExpression
            ...
            if (indexForAltMetaFactory == 0) {
                ReferenceBinding javaLangInvokeLambdaMetafactory
                    = this.referenceBinding.scope.getJavaLangInvokeLambdaMetafactory();
                indexForAltMetaFactory
                    = this.constantPool.literalIndexForMethodHandle(ClassFileConstants.MethodHandleRefKindInvokeStatic,
                        javaLangInvokeLambdaMetafactory, ConstantPool.ALTMETAFACTORY,
            ...
            this.contents[localContentsOffset++]
                = (byte) (4 + (markerInterfaces == null ? 0 : 1 + markerInterfaces.length)
                    + (bridges == null ? 0 : 1 + bridges.length));

            int functionalDescriptorIndex
                = this.constantPool.literalIndexForMethodType(functional.descriptor.original().signature());
            this.contents[localContentsOffset++] = (byte) (functionalDescriptorIndex >> 8);
            this.contents[localContentsOffset++] = (byte) functionalDescriptorIndex;

            int methodHandleIndex = this.constantPool.literalIndexForMethodHandle(functional.binding.original()); // Speak
                                                                                                                  // of
                                                                                                                  // "
                                                                                                                  // implementation"
                                                                                                                  // (erased)
            ...
            if (indexForMetaFactory == 0) {
                ...
                indexForMetaFactory
                    = this.constantPool.literalIndexForMethodHandle(ClassFileConstants.MethodHandleRefKindInvokeStatic,
                        javaLangInvokeLambdaMetafactory, ConstantPool.METAFACTORY,
            ...
            int methodHandleIndex
                = this.constantPool.literalIndexForMethodHandle(functional.binding instanceof PolymorphicMethodBinding
                    ? functional.binding
                    : functional.binding.original()); // Speak of " implementation" (erased) version here, adaptations
                                                      // described below.
            this.contents[localContentsOffset++] = (byte) (methodHandleIndex >> 8);
            this.contents[localContentsOffset++] = (byte) methodHandleIndex;
    ...
    private int addBootStrapRecordEntry(int localContentsOffset, TypeDeclaration typeDecl, Map<String, Integer> fPtr) {
        ...
        if (indexForObjectMethodBootStrap == 0) {
            ReferenceBinding javaLangRuntimeObjectMethods
                = this.referenceBinding.scope.getJavaLangRuntimeObjectMethods();
            indexForObjectMethodBootStrap = this.constantPool.literalIndexForMethodHandle(
                ClassFileConstants.MethodHandleRefKindInvokeStatic, javaLangRuntimeObjectMethods,
        ...
        String names = Arrays.stream(recordComponents)
        ...
        for (RecordComponentBinding component : recordComponents) {
            int methodHandleIndex = this.constantPool.literalIndexForMethodHandleFieldRef(
                ClassFileConstants.MethodHandleRefKindGetField, recordName, component.name, component.type.signature());
    ...
    private int addBootStrapTypeCaseConstantEntry(int localContentsOffset, LabelExpression caseConstant,
        Map<String, Integer> fPtr) {
        final int contentsEntries = 10;
        if (contentsEntries + localContentsOffset >= this.contents.length) {
            resizeContents(contentsEntries);
        ...
        if (idx == 0) {
            ...
            idx = this.constantPool.literalIndexForMethodHandle(ClassFileConstants.MethodHandleRefKindInvokeStatic,
                constantBootstrap, ConstantPool.INVOKE_METHOD_METHOD_NAME,
                ConstantPool.JAVA_LANG_INVOKE_CONSTANTBOOTSTRAP_SIGNATURE, false);
        ...
        if (idx == 0) {
            ...
            idx = this.constantPool.literalIndexForMethodHandle(ClassFileConstants.MethodHandleRefKindInvokeStatic,
                enumDesc, "of".toCharArray(), //$NON-NLS-1$
                ConstantPool.JAVA_LANG_ENUMDESC_OF_SIGNATURE, false);
        ...
        idx = this.constantPool.literalIndexForDynamic(caseConstant.classDescIdx,
            ConstantPool.INVOKE_METHOD_METHOD_NAME, ConstantPool.JAVA_LANG_CONST_CLASSDESC);
        this.contents[localContentsOffset++] = (byte) (idx >> 8);
        this.contents[localContentsOffset++] = (byte) idx;

        String enumerator = caseConstant.expression instanceof QualifiedNameReference qnr
            ? new String(qnr.tokens[qnr.tokens.length - 1])
            : caseConstant.expression.toString();
    ...
    private int addClassDescBootstrap(int localContentsOffset, TypeBinding type, Map<String, Integer> fPtr) {
        ...
        if (idx == 0) {
            ...
            idx = this.constantPool.literalIndexForMethodHandle(ClassFileConstants.MethodHandleRefKindInvokeStatic,
                classDesc, "of".toCharArray(),  //$NON-NLS-1$
                ConstantPool.JAVA_LANG_CLASSDESC_OF_SIGNATURE, true);
    ...
    private int addSingletonBootstrap(int localContentsOffset, SingletonBootstrap sb, Map<String, Integer> fPtr) {
        ...
        if (idx == 0) {
            idx = this.constantPool.literalIndexForMethodHandle(ClassFileConstants.MethodHandleRefKindInvokeStatic,
                this.referenceBinding.scope.getJavaLangInvokeConstantBootstraps(), sb.selector(), sb.signature(),
                false);
            fPtr.put(sb.id(), idx);
        }
        this.contents[localContentsOffset++] = (byte) (idx >> 8);
    ...
    private int addBootStrapTypeSwitchEntry(int localContentsOffset, SwitchStatement switchStatement,
        Map<String, Integer> fPtr) {
        LabelExpression[] constants = switchStatement.labelExpressions;
        ...
        if (indexFortypeSwitch == 0) {
            ReferenceBinding javaLangRuntimeSwitchBootstraps
                = this.referenceBinding.scope.getJavaLangRuntimeSwitchBootstraps();
            indexFortypeSwitch = this.constantPool.literalIndexForMethodHandle(
                ClassFileConstants.MethodHandleRefKindInvokeStatic, javaLangRuntimeSwitchBootstraps,
        ...
        this.contents[numArgsLocation] = (byte) numArgs;
        localContentsOffset += 2;
        for (LabelExpression c : constants) {
            if (c.isPattern()) {
                int typeOrDynIndex;
                if (c.expression.resolvedType.isPrimitiveType()) {
                    ...
                    typeOrDynIndex = this.constantPool.literalIndexForDynamic(c.primitivesBootstrapIdx,
                        c.type.signature(), ConstantPool.JavaLangClassSignature);
                } else {
                ...
                int typeIndex = this.constantPool.literalIndexForDynamic(c.enumDescIdx,
                    ConstantPool.INVOKE_METHOD_METHOD_NAME, ConstantPool.JAVA_LANG_ENUM_ENUMDESC);
                ...
                int intValIdx = this.constantPool.literalIndex(c.constant.stringValue());
                this.contents[localContentsOffset++] = (byte) (intValIdx >> 8);
                this.contents[localContentsOffset++] = (byte) intValIdx;
            } else {
                if (c.expression instanceof NullLiteral)
                    continue;
                int valIdx = switch (c.type.id) {
                    ...
                    case TypeIds.T_byte, TypeIds.T_char, TypeIds.T_short, TypeIds.T_int -> this.constantPool
                        .literalIndex(c.intValue());
                    case TypeIds.T_long -> this.constantPool.literalIndex(c.constant.longValue());
                    case TypeIds.T_float -> this.constantPool.literalIndex(c.constant.floatValue());
                    case TypeIds.T_double -> this.constantPool.literalIndex(c.constant.doubleValue());
                    default -> throw new IllegalArgumentException("Switch has unexpected type: " + switchStatement); //$NON-NLS-1$
                };
    ...
    private int addBootStrapEnumSwitchEntry(int localContentsOffset, SwitchStatement switchStatement,
        Map<String, Integer> fPtr) {
        final int contentsEntries = 10;
        ...
        if (indexForenumSwitch == 0) {
            ReferenceBinding javaLangRuntimeSwitchBootstraps
                = this.referenceBinding.scope.getJavaLangRuntimeSwitchBootstraps();
            indexForenumSwitch = this.constantPool.literalIndexForMethodHandle(
                ClassFileConstants.MethodHandleRefKindInvokeStatic, javaLangRuntimeSwitchBootstraps,
        ...
        for (LabelExpression c : constants) {
            if (c.isPattern()) {
                ...
                if (c.expression instanceof NullLiteral)
                    continue;
                String enumerator = c.expression instanceof QualifiedNameReference qnr
                    ? new String(qnr.tokens[qnr.tokens.length - 1])
    ...
    private int addBootStrapStringConcatEntry(int localContentsOffset, String recipe, Map<String, Integer> fPtr) {
        ...
        if (indexForStringConcat == 0) {
            ...
            indexForStringConcat = this.constantPool.literalIndexForMethodHandle(
                ClassFileConstants.MethodHandleRefKindInvokeStatic, stringConcatBootstrap,
        ...
        int intValIdx = this.constantPool.literalIndex(recipe);
        this.contents[localContentsOffset++] = (byte) (intValIdx >> 8);
        this.contents[localContentsOffset++] = (byte) intValIdx;
    ...
    private int generateLocalVariableTableAttribute(int code_length, boolean methodDeclarationIsStatic,
        ...
        if (!methodDeclarationIsStatic && !isSynthetic) {
            ...
            this.contents[localContentsOffset++] = (byte) (nameIndex >> 8);
            this.contents[localContentsOffset++] = (byte) nameIndex;
            declaringClassBinding = (SourceTypeBinding) (this.codeStream.methodDeclaration != null
        ...
        for (int i = 0, max = this.codeStream.allLocalsCounter; i < max; i++) {
            ...
            if (localVariable.declaration == null
                && !(localVariable.declaringScope != null
                    && localVariable.declaringScope.referenceContext()instanceof ConstructorDeclaration cd
                    && cd.isCompactConstructor()))
                continue;
            final TypeBinding localVariableTypeBinding = localVariable.type;
            boolean isParameterizedType
                = localVariableTypeBinding.isParameterizedType() || localVariableTypeBinding.isTypeVariable();
            ...
            for (int j = 0; j < initializationCount; j++) {
                ...
                if (startPC != endPC) { // only entries for non zero length
                    if (endPC == -1) {
                        localVariable.declaringScope.problemReporter()
                            .abortDueToInternalError(
        ...
        final boolean currentInstanceIsGeneric = !methodDeclarationIsStatic
                ...
                if (component != null) {
                    ...
                    if ((this.produceAttributes & ClassFileConstants.ATTR_TYPE_ANNOTATION) != 0) {
                        ...
                        if (annotations != null && (component.bits & ASTNode.HasTypeAnnotations) != 0) {
                            component.getAllAnnotationContexts(AnnotationTargetTypeConstants.METHOD_RETURN,
                                allTypeAnnotationContexts);
                        }
                        TypeReference componentType = component.type;
                        if (componentType != null && ((componentType.bits & ASTNode.HasTypeAnnotations) != 0)) {
                            componentType.getAllAnnotationContexts(AnnotationTargetTypeConstants.METHOD_RETURN,
                                allTypeAnnotationContexts);
                        }
                        int size = allTypeAnnotationContexts.size();
                        attributesNumber = completeRuntimeTypeAnnotations(attributesNumber, null, node -> size > 0,
                ...
                if ((methodBinding.tagBits & TagBits.HasParameterAnnotations) != 0) {
                    ...
                    if (arguments != null) {
                        ...
                        if (parameterCount > argumentCount) { // synthetics prefixed
                            ...
                            System.arraycopy(arguments, 0, arguments = new Argument[parameterCount], redShift,
                                argumentCount);
    ...
    private int completeRuntimeTypeAnnotations(int attributesNumber, ASTNode node, Predicate<ASTNode> condition,
        ...
        if (condition.test(node)) {
            ...
            if (allTypeAnnotationContexts.size() > 0) {
                AnnotationContext[] allTypeAnnotationContextsArray
                    = new AnnotationContext[allTypeAnnotationContexts.size()];
    ...
    public void addSyntheticDeserializeLambda(SyntheticMethodBinding methodBinding,
        ...
        int code_length = this.codeStream.position;
        if (code_length > 65535) {
            this.referenceBinding.scope.problemReporter()
                .bytecodeExceeds64KLimit(methodBinding, this.referenceBinding.sourceStart(),
                    this.referenceBinding.sourceEnd());
        }
        completeCodeAttributeForSyntheticMethod(methodBinding, codeAttributeOffset,
            ((SourceTypeBinding) methodBinding.declaringClass).scope.referenceCompilationUnit().compilationResult
                .getLineSeparatorPositions());
    ...
    public void generateMethodInfoHeaderForClinit() {
        ...
        this.contents[this.contentsOffset++]
            = (byte) ((ClassFileConstants.AccDefault | ClassFileConstants.AccStatic) >> 8);
    ...
    private void generateMissingTypesAttribute() {
        ...
        // we don't need to resize as we interate from 0 to numberOfMissingTypes when recording the indexes in the
        // .class file
        ...
        for (int i = 0; i < argumentsLength; i++) {
            ...
            if (annotations != null) {
                for (Annotation a : annotations) {
                    Annotation annotation;
                    if ((annotation = a.getPersistibleAnnotation()) == null)
                        continue; // already packaged into container.
                    long annotationMask = annotation.resolvedType != null
                        ? annotation.resolvedType.getAnnotationTagBits() & TagBits.AnnotationTargetMASK
                        : 0;
                    if (annotationMask != 0 && (annotationMask & TagBits.AnnotationForParameter) == 0)
                        continue;
                    if (annotation.isRuntimeInvisible()) {
    ...
    private int generateMethodParameters(final MethodBinding binding) {
        ...
        if (needSynthetics) {
            ...
            boolean anonymousWithLocalSuper
                = declaringClass.isAnonymousType() && declaringClass.superclass().isLocalType();
            boolean anonymousWithNestedSuper
                = declaringClass.isAnonymousType() && declaringClass.superclass().isNestedType();
            boolean isImplicitlyDeclared
                = ((!declaringClass.isPrivate()) || declaringClass.isAnonymousType()) && !anonymousWithLocalSuper;
            ReferenceBinding[] syntheticArgumentTypes = declaringClass.syntheticEnclosingInstanceTypes();
            if (syntheticArgumentTypes != null) {
                for (int i = 0, count = syntheticArgumentTypes.length; i < count; i++) {
                    // This behaviour tracks JLS 15.9.5.1
                    // This covers that the parameter ending up in a nested class must be mandated "on the way in", even
                    // if it
                    // isn't the first. The practical relevance of this is questionable, since the constructor call will
                    // be
                    ...
                    boolean couldForwardToMandated = anonymousWithNestedSuper
                        ...
                        : true;
                    int modifier = couldForwardToMandated && isImplicitlyDeclared
                        ? ClassFileConstants.AccMandated
                        : ClassFileConstants.AccSynthetic;
                    char[] name = CharOperation.concat(TypeConstants.SYNTHETIC_ENCLOSING_INSTANCE_PREFIX,
        ...
        if (targetParameters != Binding.NO_PARAMETERS) {
            if (binding.isCanonicalConstructor() && methodDeclaration == null) { // synthetic
                ...
                AbstractVariableDeclaration[] arguments
                    = methodDeclaration == null ? null : methodDeclaration.arguments(true);
                for (int i = 0, max = targetParameters.length,
                    argumentsLength = arguments != null ? arguments.length : 0; i < max; i++) {
                    if (argumentsLength > i && arguments[i] != null) {
                        AbstractVariableDeclaration argument = arguments[i];
                        int modifiers = argument.getBinding()instanceof VariableBinding variable
                            ? variable.modifiers
        ...
        if (needSynthetics) {
            ...
            for (int i = 0; i < count; i++) {
                length = writeArgumentName(syntheticOuterArguments[i].name,
                    syntheticOuterArguments[i].modifiers | ClassFileConstants.AccSynthetic, length);
            }
        ...
        if (length > 0) {
            ...
            int methodParametersNameIndex
                = this.constantPool.literalIndex(AttributeNamesConstants.MethodParametersName);
    ...
    private int writeArgumentName(char[] name, int modifiers, int oldLength) {
        ...
        int flags = modifiers
            & (ClassFileConstants.AccFinal | ClassFileConstants.AccSynthetic | ClassFileConstants.AccMandated);
    ...
    private int generateStackMapTableAttribute(MethodBinding methodBinding, int code_length, int codeAttributeOffset,
        int max_locals, boolean isClinit, Scope scope) {
        int attributesNumber = 0;
        int localContentsOffset = this.contentsOffset;
        StackMapFrameCodeStream stackMapFrameCodeStream = (StackMapFrameCodeStream) this.codeStream;
        ...
        if (stackMapFrameCodeStream.hasFramePositions()) {
            ...
            List<StackMapFrame> realFrames = traverse(isClinit ? null : methodBinding, max_locals, this.contents,
                codeAttributeOffset + 14, code_length, frames, isClinit, scope);
                            ...
                            for (int i = index; i < currentFrame.locals.length && numberOfDifferentLocals > 0; i++) {
                                ...
                                if (info == null) {
                                    ...
                                    switch (info.id()) {
                                        ...
                                        case T_short:
                                            this.contents[localContentsOffset++]
                                                = (byte) VerificationTypeInfo.ITEM_INTEGER;
                                            break;

                                        case T_float:
                                            this.contents[localContentsOffset++]
                                                = (byte) VerificationTypeInfo.ITEM_FLOAT;
                                            break;

                                        case T_long:
                                            this.contents[localContentsOffset++]
                                                = (byte) VerificationTypeInfo.ITEM_LONG;
                                            i++;
                                            break;

                                        case T_double:
                                            this.contents[localContentsOffset++]
                                                = (byte) VerificationTypeInfo.ITEM_DOUBLE;
                                            i++;
                                            break;
                                        ...
                                        default:
                                            ...
                                            switch (info.tag) {
                                                ...
                                                case VerificationTypeInfo.ITEM_OBJECT:
                                                    int indexForType = this.constantPool
                                                        .literalIndexForType(info.constantPoolName());
                                ...
                                switch (currentFrame.stackItems[0].id()) {
                                    ...
                                    default:
                                        ...
                                        switch (tag) {
                                            ...
                                            case VerificationTypeInfo.ITEM_OBJECT:
                                                int indexForType
                                                    = this.constantPool.literalIndexForType(info.constantPoolName());
                                                this.contents[localContentsOffset++] = (byte) (indexForType >> 8);
                                                this.contents[localContentsOffset++] = (byte) indexForType;
                                        }
                                }
    ...
    private final int i4At(byte[] reference, int relativeOffset, int structOffset) {
        ...
        return ((reference[position++] & 0xFF) << 24) + ((reference[position++] & 0xFF) << 16)
    ...
    public void initialize(SourceTypeBinding aType, ClassFile parentClassFile, boolean createProblemType) {
        ...
        accessFlags
            &= ~(ClassFileConstants.AccStrictfp | ClassFileConstants.AccProtected | ClassFileConstants.AccPrivate
                | ClassFileConstants.AccStatic | ClassFileConstants.AccSynchronized | ClassFileConstants.AccNative);
    ...
    private void initializeDefaultLocals(StackMapFrame frame, MethodBinding methodBinding, int maxLocals,
        int codeLength) {
        if (maxLocals != 0) {
            ...
            if (isConstructor || !methodBinding.isStatic()) {
                LocalVariableBinding localVariableBinding
                    = new LocalVariableBinding(ConstantPool.This, methodBinding.declaringClass, 0, false);
            ...
            if (isConstructor) {
                if (methodBinding.declaringClass.isEnum()) {
                    LocalVariableBinding localVariableBinding = new LocalVariableBinding(" name".toCharArray(), //$NON-NLS-1$
                        this.referenceBinding.scope.getJavaLangString(), 0, false);
                    ...
                    frame.putLocal(resolvedPosition,
                        new VerificationTypeInfo(this.referenceBinding.scope.getJavaLangString()));
                    resolvedPosition++;

                    localVariableBinding
                        = new LocalVariableBinding(" ordinal".toCharArray(), TypeBinding.INT, 0, false); //$NON-NLS-1$
                    localVariableBinding.resolvedPosition = resolvedPosition;
                    this.codeStream.record(localVariableBinding);
                    localVariableBinding.recordInitializationStartPC(0);
                    localVariableBinding.recordInitializationEndPC(codeLength);
                ...
                if (methodBinding.declaringClass.isNestedType()) {
                    ...
                    if ((enclosingInstanceTypes = methodBinding.declaringClass.syntheticEnclosingInstanceTypes())
                        != null) {
                        for (int i = 0, max = enclosingInstanceTypes.length; i < max; i++) {
                            ...
                            LocalVariableBinding localVariableBinding = new LocalVariableBinding(
                                (" enclosingType" + i).toCharArray(), enclosingInstanceTypes[i], 0, false); //$NON-NLS-1$
                            localVariableBinding.resolvedPosition = resolvedPosition;
                            this.codeStream.record(localVariableBinding);
                            localVariableBinding.recordInitializationStartPC(0);
                            localVariableBinding.recordInitializationEndPC(codeLength);
                    ...
                    if ((syntheticArguments = methodBinding.declaringClass.syntheticOuterLocalVariables()) != null) {
                        for (int i = 0, max = syntheticArguments.length; i < max; i++) {
                            ...
                            LocalVariableBinding localVariableBinding
                                = new LocalVariableBinding((" synthetic" + i).toCharArray(), typeBinding, 0, false); //$NON-NLS-1$
                            localVariableBinding.resolvedPosition = resolvedPosition;
                            this.codeStream.record(localVariableBinding);
                            localVariableBinding.recordInitializationStartPC(0);
                            localVariableBinding.recordInitializationEndPC(codeLength);
                ...
                if (methodBinding instanceof SyntheticMethodBinding smb
                    && smb.purpose == SyntheticMethodBinding.DeserializeLambda) {
                    // For the branching complexities in the generated $deserializeLambda$ we need the local variable
                    LocalVariableBinding lvb = new LocalVariableBinding(" synthetic0".toCharArray(), //$NON-NLS-1$
                        this.referenceBinding.scope.getJavaLangInvokeSerializedLambda(), 0, true);
    ...
    public void recordInnerClasses(TypeBinding binding, boolean onBottomForBug445231) {
        ...
        this.innerClassesBindings.put(innerClass.erasure().unannotated(), onBottomForBug445231);  // should not emit yet
                                                                                                  // another inner class
                                                                                                  // for Outer.@Inner
                                                                                                  // Inner.
    ...
    private List<StackMapFrame> filterFakeFrames(Set<Integer> realJumpTargets, Map<Integer, StackMapFrame> frames,
        int codeLength) {
    ...
    private TypeBinding getTypeBinding(char[] typeConstantPoolName, Scope scope, boolean checkcast) {
        if (typeConstantPoolName.length == 1 && !checkcast) {
            ...
            if (typeConstantPoolName.length - dimensions == 1) {
                ...
                char[] typeName
                    = CharOperation.subarray(typeConstantPoolName, dimensions + 1, typeConstantPoolName.length - 1);
                ...
                if (!type.isValidBinding()) {
                    ...
                    } else if ((problemReferenceBinding.problemId() & ProblemReasons.NotFound) != 0
                        && this.innerClassesBindings != null) {
            ...
            char[] typeName = checkcast
                ? typeConstantPoolName
                : CharOperation.subarray(typeConstantPoolName, 1, typeConstantPoolName.length - 1);
            TypeBinding type = (TypeBinding) scope.getTypeOrPackage(CharOperation.splitOn('/', typeName));
            if (!type.isValidBinding()) {
                ProblemReferenceBinding problemReferenceBinding = (ProblemReferenceBinding) type;
                if ((problemReferenceBinding.problemId() & ProblemReasons.InternalNameProvided) != 0
    ...
    private boolean isLikelyLocalTypeName(char[] typeName) {
        ...
        while (dollarPos != -1 && dollarPos + 1 < typeName.length) {
            if (Character.isDigit(typeName[dollarPos + 1]))
                return true; // name segment starts with a digit => likely a local type (but still "$0" etc. could be
    ...
    private TypeBinding getANewArrayTypeBinding(char[] typeConstantPoolName, Scope scope) {
        if (typeConstantPoolName[0] == '[') {
            ...
            if (typeConstantPoolName.length - dimensions == 1) {
                ...
                // array of object types
                char[] elementTypeClassName
                    = CharOperation.subarray(typeConstantPoolName, dimensions + 1, typeConstantPoolName.length - 1);
                TypeBinding type
                    = (TypeBinding) scope.getTypeOrPackage(CharOperation.splitOn('/', elementTypeClassName));
                if (!type.isValidBinding()) {
                    ProblemReferenceBinding problemReferenceBinding = (ProblemReferenceBinding) type;
                    if ((problemReferenceBinding.problemId() & ProblemReasons.InternalNameProvided) != 0
                        || (problemReferenceBinding.problemId() & ProblemReasons.NotVisible) != 0) {
    ...
    public List<StackMapFrame> traverse(MethodBinding methodBinding, int maxLocals, byte[] bytecodes, int codeOffset,
        ...
        while (true) {
            ...
            switch (opcode) {
                ...
                case Opcodes.OPC_ifle:
                    ...
                    addRealJumpTarget(realJumpTarget, jumpPC, frames,
                        createNewFrame(jumpPC, frame, isClinit, methodBinding), scope);
                    pc += 3;
                    break;
                ...
                case Opcodes.OPC_getstatic:
                    ...
                    char[] descriptor = utf8At(poolContents, constantPoolOffsets[utf8index] + 3,
                        u2At(poolContents, 1, constantPoolOffsets[utf8index]));
                ...
                case Opcodes.OPC_getfield:
                    ...
                    descriptor = utf8At(poolContents, constantPoolOffsets[utf8index] + 3,
                        u2At(poolContents, 1, constantPoolOffsets[utf8index]));
                    frame.numberOfStackItems--;
                ...
                case Opcodes.OPC_invokevirtual:
                    ...
                    char[] name = utf8At(poolContents, constantPoolOffsets[utf8index] + 3,
                        u2At(poolContents, 1, constantPoolOffsets[utf8index]));
                ...
                case Opcodes.OPC_invokespecial:
                    ...
                    descriptor = utf8At(poolContents, constantPoolOffsets[utf8index] + 3,
                        u2At(poolContents, 1, constantPoolOffsets[utf8index]));
                    utf8index = u2At(poolContents, 1, constantPoolOffsets[nameAndTypeIndex]);
                    name = utf8At(poolContents, constantPoolOffsets[utf8index] + 3,
                        u2At(poolContents, 1, constantPoolOffsets[utf8index]));
                    frame.numberOfStackItems -= getParametersCount(descriptor);
                ...
                case Opcodes.OPC_new:
                    ...
                    char[] className = utf8At(poolContents, constantPoolOffsets[utf8index] + 3,
                        u2At(poolContents, 1, constantPoolOffsets[utf8index]));
                    typeBinding = getNewTypeBinding(className, scope);
                    VerificationTypeInfo verificationTypeInfo
                        = new VerificationTypeInfo(VerificationTypeInfo.ITEM_UNINITIALIZED, typeBinding);
                ...
                case Opcodes.OPC_anewarray:
                    index = u2At(bytecodes, 1, pc);
                    utf8index = u2At(poolContents, 1, constantPoolOffsets[index]);
                    className = utf8At(poolContents, constantPoolOffsets[utf8index] + 3,
                        u2At(poolContents, 1, constantPoolOffsets[utf8index]));
                    frame.numberOfStackItems--;
                    ...
                    if (typeBinding != null) {
                        if (typeBinding.isArrayType()) {
                            ...
                            frame.addStackItem(new VerificationTypeInfo(
                                scope.createArrayType(arrayBinding.leafComponentType(), arrayBinding.dimensions + 1)));
                        } else {
                ...
                default: // should not occur
                    if (this.codeStream.methodDeclaration != null) {
                        this.codeStream.methodDeclaration.scope.problemReporter()
                            .abortDueToInternalError(
                                Messages.bind(Messages.abort_invalidOpcode,
                                    new Object[] {
                                        Byte.valueOf(opcode),
                                        Integer.valueOf(pc),
    ...
    private StackMapFrame createNewFrame(int currentPC, StackMapFrame frame, boolean isClinit,
    ...
    private void addRealJumpTarget(Set<Integer> realJumpTarget, int pc, Map<Integer, StackMapFrame> frames,
        StackMapFrame frame, Scope scope) {
        realJumpTarget.add(Integer.valueOf(pc));
    ...
    private final long u4At(byte[] reference, int relativeOffset, int structOffset) {
        int position = relativeOffset + structOffset;
        return (((reference[position++] & 0xFFL) << 24) + ((reference[position++] & 0xFF) << 16)
            + ((reference[position++] & 0xFF) << 8) + (reference[position] & 0xFF));
    }
