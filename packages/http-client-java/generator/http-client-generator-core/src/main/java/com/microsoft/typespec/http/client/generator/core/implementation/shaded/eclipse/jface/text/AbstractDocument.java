 ...
 * interfaces {@link IDocumentExtension},
 * {@link IDocumentExtension2},
 * {@link IDocumentExtension3},
 * {@link IDocumentExtension4}, as well as
 * {@link IRepairableDocument}.
 * <p>
 *
 * An <code>AbstractDocument</code> supports the following implementation
 * plug-ins:
 * <ul>
 * <li>a text store implementing {@link ITextStore} for
 * storing and managing the document's content,</li>
 * <li>a line tracker implementing {@link ILineTracker}
...
public abstract class AbstractDocument implements IDocument, IDocumentExtension, IDocumentExtension2,
    IDocumentExtension3, IDocumentExtension4, IRepairableDocument, IRepairableDocumentExtension {
    ...
    static private class RegisteredReplace {
         ...
         */
        RegisteredReplace(IDocumentListener owner, IReplace replace) {
            fOwner = owner;
            fReplace = replace;
        }
    }
    ...
    private final ListenerList<IDocumentListener> fPrenotifiedDocumentListeners
        = new ListenerList<>(ListenerList.IDENTITY);
    /** The registered document partitioning listeners */
    private final ListenerList<IDocumentPartitioningListener> fDocumentPartitioningListeners
        = new ListenerList<>(ListenerList.IDENTITY);
    ...
    public void addPosition(String category, Position position)
        ...
        if (endPositions == null)
            throw new BadPositionCategoryException(category);
        endPositions.add(computeIndexInPositionList(endPositions, position.offset + position.length - 1, false),
            position);
    }
        ...
        if (fDocumentPartitioners != null) {
            ...
            for (Entry<String, IDocumentPartitioner> entry : fDocumentPartitioners.entrySet()) {
                ...
                if (partitioner instanceof IDocumentPartitionerExtension) {
                    ...
                    if (r != null)
                        fDocumentPartitioningChangedEvent.setPartitionChange(partitioning, r.getOffset(),
                            r.getLength());
                } else {
                    if (partitioner.documentChanged(event))
                        fDocumentPartitioningChangedEvent.setPartitionChange(partitioning, 0,
     ...
     * @since 2.0
     * @deprecated as of 3.0. Use <code>doFireDocumentChanged2(DocumentEvent)</code> instead; this method will be
     * removed.
     */
    @Deprecated
    ...
    private void removeFromPositionsList(List<Position> positions, Position position, boolean orderedByOffset) {
        ...
        int index = computeIndexInPositionList(positions,
            orderedByOffset ? position.offset : position.offset + position.length - 1, orderedByOffset);
    ...
    @Deprecated
    @Override
    public int search(int startPosition, String findString, boolean forwardSearch, boolean caseSensitive,
        boolean wholeWord) throws BadLocationException {
        try {
            IRegion region = getFindReplaceDocumentAdapter().find(startPosition, findString, forwardSearch,
                caseSensitive, wholeWord, false);
    ...
    public DocumentRewriteSession startRewriteSession(DocumentRewriteSessionType sessionType) {
        ...
        fireRewriteSessionChanged(
            new DocumentRewriteSessionEvent(this, fDocumentRewriteSession, DocumentRewriteSessionEvent.SESSION_START));
