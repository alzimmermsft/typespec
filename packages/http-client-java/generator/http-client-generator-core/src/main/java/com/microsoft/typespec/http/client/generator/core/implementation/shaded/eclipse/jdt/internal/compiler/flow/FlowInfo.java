...
import com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.ast.IfStatement;
...
public abstract class FlowInfo {
    ...
    public final static int UNROOTED = 64; // marks a flowInfo that may be appended to another flowInfo (accepting
                                           // incoming nulls/nonnulls, see UFI.iNBit/iNNBit).
    ...
    public void markNullStatus(LocalVariableBinding local, int nullStatus) {
        switch (nullStatus) {
            ...
            default:
                ...
                if ((nullStatus
                    & (FlowInfo.POTENTIALLY_NULL | FlowInfo.POTENTIALLY_NON_NULL | FlowInfo.POTENTIALLY_UNKNOWN)) == 0)
                    markAsDefinitelyUnknown(local);
        }
    }
    ...
    public static UnconditionalFlowInfo mergedOptimizedBranchesIfElse(FlowInfo initsWhenTrue, boolean isOptimizedTrue,
        ...
        IfStatement ifStatement, boolean reportDeadCodeInKnownPattern) {
        UnconditionalFlowInfo mergedInfo;
        if (isOptimizedTrue) {
            if (initsWhenTrue == FlowInfo.DEAD_END && allowFakeDeadBranch) {
                if (!reportDeadCodeInKnownPattern) {
                    // https://bugs.eclipse.org/bugs/show_bug.cgi?id=256796
                    // do not report code even after if-else as dead as a consequence of analysis done in known dead
                    // code pattern
            ...
            mergedInfo = initsWhenTrue.addPotentialInitializationsFrom(initsWhenFalse.nullInfoLessUnconditionalCopy())
                .unconditionalInits();
            ...
            // https://bugs.eclipse.org/bugs/show_bug.cgi?id=415997, classify unreachability precisely,
            // IsElseStatementUnreachable could be due to null analysis
            if ((mergedInfo.tagBits & FlowInfo.UNREACHABLE_OR_DEAD) != 0
                && (initsWhenFalse.tagBits & FlowInfo.UNREACHABLE) == FlowInfo.UNREACHABLE_BY_NULLANALYSIS) {
                mergedInfo.tagBits &= ~UNREACHABLE_OR_DEAD;
                mergedInfo.tagBits |= UNREACHABLE_BY_NULLANALYSIS;
            }
        } else if ((flowInfo.tagBits & FlowInfo.UNREACHABLE) == 0
            && (ifStatement.bits & ASTNode.IsThenStatementUnreachable) != 0
            && initsWhenTrue != FlowInfo.DEAD_END
            && initsWhenFalse != FlowInfo.DEAD_END) {
            ...
            // We don't do this if both if and else branches themselves are in an unreachable code
            // or if any of them is a DEAD_END (e.g. contains 'return' or 'throws')
            mergedInfo = initsWhenFalse.addPotentialInitializationsFrom(initsWhenTrue.nullInfoLessUnconditionalCopy())
                .unconditionalInits();
            // if a variable is only initialized in one branch and not initialized in the other,
            // then we need to cast a doubt on its initialization in the merged info
            mergedInfo.mergeDefiniteInitsWith(initsWhenTrue.unconditionalCopy());
            // https://bugs.eclipse.org/bugs/show_bug.cgi?id=415997, classify unreachability precisely,
            // IsThenStatementUnreachable could be due to null analysis
            if ((mergedInfo.tagBits & FlowInfo.UNREACHABLE_OR_DEAD) != 0
                && (initsWhenTrue.tagBits & FlowInfo.UNREACHABLE) == FlowInfo.UNREACHABLE_BY_NULLANALYSIS) {
                mergedInfo.tagBits &= ~UNREACHABLE_OR_DEAD;
                mergedInfo.tagBits |= UNREACHABLE_BY_NULLANALYSIS;
            }
        } else {
