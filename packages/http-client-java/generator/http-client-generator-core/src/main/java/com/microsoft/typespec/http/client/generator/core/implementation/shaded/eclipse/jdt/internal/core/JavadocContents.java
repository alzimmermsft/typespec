...
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.core.ExternalJavadocSupport.*;
...
public class JavadocContents implements IJavadocContents {
    ...
    public String getMethodDoc(IMethod child) throws JavaModelException {
        if (this.content == null || !(child instanceof BinaryMethod binaryMethod))
            return null;
    ...
    protected Range computeChildRange(int anchorEndStart, char[] anchor, int indexOfBottom) {
        ...
        if (indexOfBottom != -1) {
            ...
            int indexOfEndLink
                = CharOperation.indexOf(ANCHOR_SUFFIX, this.content, false, anchorEndStart + anchor.length);
            if (indexOfEndLink != -1) {
    ...
    protected void computeChildrenSections() {
        ...
        this.indexOfFieldsBottom = this.indexOfConstructorDetails != -1
        ...
        this.indexOfAllMethodsTop
            = this.indexOfConstructorDetails != -1 ? this.indexOfConstructorDetails : this.indexOfMethodDetails;
    ...
    protected String computeMethodAnchorPrefixEnd(BinaryMethod method) throws JavaModelException {
        ...
        if (declaringType.isMember()) {
            // might need to remove a part of the signature corresponding to the synthetic argument (only for
            // constructor)
    ...
    protected void computeTypeRange() throws JavaModelException {
        ...
        if (indexOfNextSummary == -1 && this.type.isEnum()) {
            ...
            indexOfNextSummary
                = CharOperation.indexOf(ENUM_CONSTANT_SUMMARY, this.content, false, indexOfNextSeparator);
        }
        if (indexOfNextSummary == -1 && this.type.isAnnotation()) {
            // try to find required enum constant summary start
            indexOfNextSummary = CharOperation.indexOf(ANNOTATION_TYPE_REQUIRED_MEMBER_SUMMARY, this.content, false,
                indexOfNextSeparator);
            if (indexOfNextSummary == -1) {
                // try to find optional enum constant summary start
                indexOfNextSummary = CharOperation.indexOf(ANNOTATION_TYPE_OPTIONAL_MEMBER_SUMMARY, this.content, false,
                    indexOfNextSeparator);
            }
        }
        if (indexOfNextSummary == -1) {
        ...
        int indexOfFirstParagraph
            = CharOperation.indexOf(P.toCharArray(), this.content, false, start, indexOfNextSummary);
        int indexOfFirstDiv
            = CharOperation.indexOf(DIV_CLASS_BLOCK.toCharArray(), this.content, false, start, indexOfNextSummary);
