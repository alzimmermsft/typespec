...
public class UnresolvedReferenceBinding extends ReferenceBinding implements HotSwappable {
    ...
    void addWrapper(HotSwappable wrapper, LookupEnvironment environment) {
        if (this.resolvedType != null) {
            ...
            // when the ParameterizedType for Unresolved B is created with args B<T>.M, the Unresolved B is resolved
            // before the wrapper is added
    ...
    public int depth() {
        ...
        return CharOperation.occurencesOf('$', this.compoundName[last], 1); // leading '$' must be part of the class
                                                                            // name, so start at 1.
    }
    ...
    ReferenceBinding resolve(LookupEnvironment environment, boolean convertGenericToRawType) {
        ...
        if (targetType == null) {
            ...
            if (targetType == this || targetType == null) { //$IDENTITY-COMPARISON$
                if (this.fPackage instanceof SplitPackageBinding) // leverage SplitPackageBinding to avoid duplicate
                                                                  // creation of BinaryTypeBinding
...
// bug 491354: this complements the NameLookup#seekTypes(..), which performs the same adaptation
                targetType = environment.askForType(this.fPackage, CharOperation.replaceOnCopy(typeName, '.', '$'),
                    this.fPackage.enclosingModule);
            }
            if (targetType == null || targetType == this) { // could not resolve any better, error was already reported
                                                            // against it //$IDENTITY-COMPARISON$
// report the missing class file first - only if not resolving a previously missing type
                if ((this.tagBits & TagBits.HasMissingType) == 0 && !environment.mayTolerateMissingType) {
                    environment.problemReporter.isClassPathCorrect(this.compoundName,
                        environment.root.unitBeingCompleted, environment.missingClassFileLocation, false,
                        this.requestingType);
                }
    ...
    @Override
    public void swapUnresolved(UnresolvedReferenceBinding unresolvedType, ReferenceBinding unannotatedType,
        LookupEnvironment environment) {
        if (this.resolvedType != null)
            return;
        ReferenceBinding annotatedType = (ReferenceBinding) unannotatedType.clone(null);
        this.resolvedType = annotatedType;
        annotatedType.setTypeAnnotations(getTypeAnnotations(),
    ...
    public String toString() {
        ...
        return "Unresolved type " //$NON-NLS-1$
