...
public class ParameterizedGenericMethodBinding extends ParameterizedMethodBinding implements Substitution {
    ...
    public TypeBinding targetType; // used to distinguish different PGMB created for different target types (needed
                                   // because inference contexts are remembered per PGMB)

    /**
     * Perform inference of generic method type parameters and/or expected type
     * <p>
     * In 1.8+ if the expected type is not yet available due to this call being an argument to an outer call which is
     * not overload-resolved yet,
    ...
    public static MethodBinding computeCompatibleMethod(MethodBinding originalMethod, TypeBinding[] arguments,
         ...
         * to under constraints and resultant glb application per 15.12.2.8, using this.typeArguments to drive the
         * bounds
         * check against itself is doomed to fail. For, the variable T would after substitution be I<I<T>> and would
         * fail
         * bounds check against I<T>. Use the inferred types from the context directly - see that there is one round of
         * extra substitution that has taken place to properly substitute a remaining unresolved variable which also
         * appears
         * in a formal bound (So we really have a bounds mismatch between I<I<T>> and I<I<I<T>>>, in the absence of a
         * fix.)
         */
        ...
        for (int i = 0, length = typeVariables.length; i < length; i++) {
            ...
            /*
             * https://bugs.eclipse.org/bugs/show_bug.cgi?id=375394, To avoid spurious bounds check failures due to
             * circularity in formal bounds,
             * we should eliminate only the lingering embedded type variable references after substitution, not alien
            ...
            if (substitute instanceof TypeVariableBinding) {
                ...
                substituteForChecks
                    = Scope.substitute(new LingeringTypeVariableEliminator(typeVariables, null, scope), substitute); // while
                                                                                                                     // using
                                                                                                                     // this
                                                                                                                     // for
                                                                                                                     // bounds
            ...
            switch (typeVariable.boundCheck(substitution, substituteForChecks, scope, null)) {
                case MISMATCH:
                    ...
                    TypeBinding[] augmentedArguments = new TypeBinding[argLength + 2]; // append offending substitute
                                                                                       // and typeVariable
                    ...
                    return new ProblemMethodBinding(methodSubstitute, originalMethod.selector, augmentedArguments,
    ...
    public static MethodBinding computeCompatibleMethod18(MethodBinding originalMethod, TypeBinding[] arguments,
        final Scope scope, InvocationSite invocationSite) {
        ...
        for (int i = 0, length = arguments.length, parametersLength = parameters.length; i < length; i++) {
            ...
            if (argument.isPrimitiveType() != parameter.isPrimitiveType()) { // Scope.cCM incorrectly but harmlessly
                                                                             // uses isBaseType which answers true for
                                                                             // null.
        ...
        arguments = argumentsCopy; // either way, this allows the engine to update arguments without harming the
                                   // callers.
        ...
        try {
            ...
            final boolean isPolyExpression = invocationSite instanceof Expression
            ...
            if (arguments.length == parameters.length) {
                infCtx18.inferenceKind
                    = requireBoxing ? InferenceContext18.CHECK_LOOSE : InferenceContext18.CHECK_STRICT; // engine may
                                                                                                        // still slip
                                                                                                        // into loose
                                                                                                        // mode and
                                                                                                        // adjust level.
            ...
            if (expectedType != null
                || !invocationSite.getExpressionContext().definesTargetType()
                || !isPolyExpression) {
                ...
                if (hasReturnProblem)
                    result = provisionalResult; // let's prefer a type error regarding the return type over reporting no
                                                // match at all
            }
            if (result != null) {
                ...
                if (solutions != null) {
                    methodSubstitute = scope.environment()
                        .createParameterizedGenericMethod(originalMethod, solutions, infCtx18.usesUncheckedConversion,
                            hasReturnProblem, expectedType);
                    if (InferenceContext18.DEBUG) {
                        System.out.println("Method substitute: " + methodSubstitute); //$NON-NLS-1$
                    }
                    if (invocationSite instanceof Invocation
                        && allArgumentsAreProper
                        && (expectedType == null || expectedType.isProperType(true)))
                        infCtx18.forwardResults(result, (Invocation) invocationSite, methodSubstitute, expectedType);
                    try {
                        if (infCtx18.missingType != null
                            && (expectedType == null || (expectedType.tagBits & TagBits.HasMissingType) == 0)) { // here
                                                                                                                 // the
                                                                                                                 // context
                                                                                                                 // will
                                                                                                                 // have
                                                                                                                 ...
                                                                                                                 // keep
                                                                                                                 // quiet
                            ProblemMethodBinding problemMethod = new ProblemMethodBinding(originalMethod,
                                originalMethod.selector, parameters, ProblemReasons.MissingTypeInSignature);
                        ...
                        if (hasReturnProblem) { // illegally working from the provisional result?
                            MethodBinding problemMethod
                                = infCtx18.getReturnProblemMethodIfNeeded(expectedType, methodSubstitute);
                        ...
                        if (invocationTypeInferred) {
                            ...
                            MethodBinding problemMethod
                                = methodSubstitute.boundCheck18(scope, arguments, invocationSite);
                        ...
                        if (!infCtx18.hasPrematureOverloadResolution()) {
                            if (invocationSite instanceof Invocation)
                                ((Invocation) invocationSite).registerInferenceContext(methodSubstitute, infCtx18); // keep
                                                                                                                    // context
                                                                                                                    // so
                                                                                                                    // we
                                                                                                                    // can
                                ...
                                ((ReferenceExpression) invocationSite).registerInferenceContext(methodSubstitute,
                                    infCtx18); // keep context so we can finish later
                        }
                    }
                    return methodSubstitute;
                }
        ...
        for (int i = 0, length = originalTypeVariables.length; i < length; i++) {
            ...
            switch (typeVariable.boundCheck(substitution, substitute, scope, location)) {
                case MISMATCH:
                    ...
                    augmentedArguments[argLength] = substitute;
                    augmentedArguments[argLength + 1] = typeVariable;
                    return new ProblemMethodBinding(methodSubstitute, this.originalMethod.selector, augmentedArguments,
                        ProblemReasons.ParameterBoundMismatch);
    ...
    /**
     * Create raw generic method for raw type (double substitution from type vars with raw type arguments, and erasure
     * of method variables)
    ...
    public ParameterizedGenericMethodBinding(MethodBinding originalMethod, TypeBinding[] typeArguments,
        LookupEnvironment environment, boolean inferredWithUncheckConversion, boolean hasReturnProblem,
        ...
        if (inferredWithUncheckConversion
            && originalMethod.isConstructor()
            && this.declaringClass.isParameterizedType()) {
            this.declaringClass = (ReferenceBinding) environment.convertToRawType(this.declaringClass.erasure(), false); // for
                                                                                                                         // diamond
                                                                                                                         // invocations
        }
        this.typeVariables = Binding.NO_TYPE_VARIABLES;
        ...
        if (inferredWithUncheckConversion) { // JSL 18.5.2: "If unchecked conversion was necessary..."
            ...
            for (int i = 0; i < originalMethod.thrownExceptions.length; i++) {
                this.thrownExceptions[i]
                    = (ReferenceBinding) getErasure18_5_2(originalMethod.thrownExceptions[i], environment);
            }
        } else {
    ...
    TypeBinding getErasure18_5_2(TypeBinding type, LookupEnvironment env) {
        ...
        type = Scope.substitute(this, type); // compliant with Bug JDK-8135087: Erasure for unchecked invocation happens
                                             // after inference
     ...
     * p.X<U> { <T> void bar(T t, U u) { new X<String>().bar(this, "") } } -->
    ...
    private static class LingeringTypeVariableEliminator implements Substitution {
         ...
         */
        public LingeringTypeVariableEliminator(TypeVariableBinding[] variables, TypeBinding[] substitutes,
            Scope scope) {
        ...
        public TypeBinding substitute(TypeVariableBinding typeVariable) {
            if (typeVariable.rank >= this.variables.length
                || TypeBinding.notEquals(this.variables[typeVariable.rank], typeVariable)) {   // not kosher, don't
                                                                                               // touch.
                return typeVariable;
            }
            if (this.substitutes != null) {
                return Scope.substitute(new LingeringTypeVariableEliminator(this.variables, null, this.scope),
                    this.substitutes[typeVariable.rank]);
            }
            ReferenceBinding genericType = (ReferenceBinding) (typeVariable.declaringElement instanceof ReferenceBinding
                ? typeVariable.declaringElement
                : null);
            return this.scope.environment()
                .createWildcard(genericType, typeVariable.rank, null, null, Wildcard.UNBOUND,
                    typeVariable.getTypeAnnotations());
        }
    ...
    public TypeBinding substitute(TypeVariableBinding originalVariable) {
        ...
        if (originalVariable.rank < length
            && TypeBinding.equalsEquals(variables[originalVariable.rank], originalVariable)) {
