...
public class Javadoc extends ASTNode {
    ...
    public void resolve(ClassScope scope) {
        ...
        if (this.returnStatement != null) {
            scope.problemReporter()
                .javadocUnexpectedTag(this.returnStatement.sourceStart, this.returnStatement.sourceEnd);
        }
                ...
                if (!methDecl.isConstructor()) {
                    if (overriding && this.seeReferences[i] instanceof JavadocMessageSend) {
                        ...
                        if (messageSend.binding != null
                            && messageSend.binding.isValidBinding()
                            && messageSend.actualReceiverType instanceof ReferenceBinding) {
                            ReferenceBinding methodReceiverType = (ReferenceBinding) messageSend.actualReceiverType;
                            TypeBinding superType
                                = methDecl.binding.declaringClass.findSuperTypeOriginatingFrom(methodReceiverType);
                            if (superType != null
                                ...
                                if (methScope.environment()
                                    .methodVerifier()
                                    .doesMethodOverride(methDecl.binding, messageSend.binding.original())) {
                                    superRef = true;
                                }
                            }
                        }
                    ...
                    if (allocationExpr.binding != null && allocationExpr.binding.isValidBinding()) {
                        ...
                        if (superType != null
                            ...
                            MethodBinding superConstructor
                                = methScope.getConstructor(superType, methDecl.binding.parameters, allocationExpr);
                            if (superConstructor.isValidBinding()
                                ...
                                // work 'against' better inference in 1.8 (otherwise comparing (G<T> with G<Object>)
                                // would fail):
            ...
            if (reportMissing && methDecl != null) {
                if (methDecl.isMethod()) {
                    ...
                    if (meth.binding.returnType != TypeBinding.VOID) {
                        ...
                        methScope.problemReporter()
                            .javadocMissingReturnTag(meth.returnType.sourceStart, meth.returnType.sourceEnd,
                                methDecl.binding.modifiers);
                    }
                }
            }
        ...
        if (reference instanceof JavadocFieldReference) {
            ...
            if (fieldRef.methodBinding != null) {
                ...
                if (fieldRef.tagValue == JavadocTagConstants.TAG_VALUE_VALUE) {
                    ...
                    scope.problemReporter()
                        .javadocInvalidValueReference(fieldRef.sourceStart, fieldRef.sourceEnd, scopeModifiers);
                } else if (fieldRef.actualReceiverType != null) {
                    if (scope.kind != Scope.MODULE_SCOPE
                        && scope.enclosingSourceType().isCompatibleWith(fieldRef.actualReceiverType)) {
                    ...
                    if (CharOperation.equals(resolvedType.sourceName(), fieldRef.token)) {
                        ...
                        fieldRef.methodBinding
                            = scope.findMethod(resolvedType, fieldRef.token, Binding.NO_TYPES, fieldRef, false);
                    }
                }
            }
            ...
            if (!hasProblems
                && fieldRef.binding != null
                && fieldRef.binding.isValidBinding()
        ...
        if (!hasProblems && (reference instanceof JavadocModuleReference)) {
            ...
            if (mRef != null) {
                ...
                if (mType != null && verifyModuleReference(reference, reference, scope, mType, mType.modifiers)) {
                    ...
                    if ((tRef instanceof JavadocSingleTypeReference || tRef instanceof JavadocQualifiedTypeReference)
                        && tRef.resolvedType instanceof ReferenceBinding) {
        ...
        if (reference instanceof JavadocMessageSend) {
            ...
            if (msgSend.tagValue == JavadocTagConstants.TAG_VALUE_VALUE) { // cannot refer to method for @value tag
                if (scopeModifiers == -1)
                    scopeModifiers = scope.getDeclarationModifiers();
                scope.problemReporter()
                    .javadocInvalidValueReference(msgSend.sourceStart, msgSend.sourceEnd, scopeModifiers);
            }

            // Verify type references
            if (!hasProblems
                && msgSend.binding != null
                && msgSend.binding.isValidBinding()
        ...
        // Verify that constructor reference are not used for @value tags
            ...
            if (alloc.tagValue == JavadocTagConstants.TAG_VALUE_VALUE) { // cannot refer to method for @value tag
                if (scopeModifiers == -1)
                    scopeModifiers = scope.getDeclarationModifiers();
                scope.problemReporter()
                    .javadocInvalidValueReference(alloc.sourceStart, alloc.sourceEnd, scopeModifiers);
            }

            // Verify type references
            if (!hasProblems
                && alloc.binding != null
                && alloc.binding.isValidBinding()
    ...
    private void resolveParamTags(ClassScope scope, boolean reportMissing) {
        ...
        if (!typeDecl.isRecord())
            return;
        Function<JavadocSingleNameReference, Binding> resolveNameRef = (nameRef) -> {
            nameRef.resolve(scope);
            return nameRef.binding;
        };
        Function<AbstractVariableDeclaration, Binding> resolveArgumentOrComponent = (arg) -> {
            return typeDecl.binding.getField(arg.name, false);
        };
        resolveParamTags(typeDecl.initializerScope, reportMissing, typeDecl.recordComponents, true, typeDecl.modifiers, // not
                                                                                                                        // used
            resolveNameRef, resolveArgumentOrComponent);
    }
    ...
    private void resolveParamTags(MethodScope scope, boolean reportMissing, boolean considerParamRefAsUsage) {
        ...
        resolveParamTags(scope, reportMissing, methodDecl.arguments(), !methodDecl.isCompactConstructor(),
            methodDecl.binding.modifiers, resolveNameRef, resolveArgumentOrComponent);
    }

    private void resolveParamTags(MethodScope scope, boolean reportMissing, AbstractVariableDeclaration[] arguments,
        boolean reportAllUndocumented, int modifiers, Function<JavadocSingleNameReference, Binding> resolveNameRef,
                ...
                if (lBinding instanceof VariableBinding varBinding) {
                    ...
                    for (int j = 0; j < maxBindings && !found; j++) {
                        if (bindings[j] == param.binding) {
                            scope.problemReporter()
                                .javadocDuplicatedParamTag(param.token, param.sourceStart, param.sourceEnd, modifiers);
                            found = true;
                        }
                    }
                    if (!found) {
            ...
            if (reportMissing) {
                for (int i = 0; i < argumentsSize; i++) {
                    ...
                    if (lBinding instanceof VariableBinding argBinding) {
                        ...
                        if (!found) {
                            scope.problemReporter()
                                .javadocMissingParamTag(arg.name, arg.sourceStart, arg.sourceEnd, modifiers);
                        }
                    }
                }
            }
        ...
        if (usesTagsSize == 0) {
            if (reportMissing) {
                for (int i = 0; i < usesSize; i++) {
                    ...
                    scope.problemReporter()
                        .javadocMissingUsesTag(uses.serviceInterface, uses.sourceStart, uses.sourceEnd,
                            moduleDecl.binding.modifiers);
                }
            }
        } else {
                    ...
                    if (usesRef.resolvedType != null && usesRef.resolvedType.isValidBinding()) {
                        ...
                        for (int j = 0; j < maxBindings && !found; j++) {
                            if (bindings[j].equals(usesRef.resolvedType)) {
                                scope.problemReporter()
                                    .javadocDuplicatedUsesTag(usesRef.sourceStart, usesRef.sourceEnd);
                                found = true;
                            }
                        }
                        if (!found) {
        ...
        if (providesTagsSize == 0) {
            if (reportMissing) {
                for (int i = 0; i < providesSize; i++) {
                    ...
                    scope.problemReporter()
                        .javadocMissingProvidesTag(provides.serviceInterface, provides.sourceStart, provides.sourceEnd,
                            moduleDecl.binding.modifiers);
                }
            }
        } else {
                    ...
                    if (providesRef.resolvedType != null && providesRef.resolvedType.isValidBinding()) {
                        ...
                        for (int j = 0; j < maxBindings && !found; j++) {
                            if (bindings[j].equals(providesRef.resolvedType)) {
                                scope.problemReporter()
                                    .javadocDuplicatedProvidesTag(providesRef.sourceStart, providesRef.sourceEnd);
                                found = true;
                            }
                        }
                        if (!found) {
            ...
            if (reportMissing) {
                for (int i = 0; i < providesSize; i++) {
                    ...
                    if (!found) {
                        scope.problemReporter()
                            .javadocMissingProvidesTag(provides.serviceInterface, provides.sourceStart,
                                provides.sourceEnd, moduleDecl.binding.modifiers);
                    }
                }
            }
        }
            ...
            if (paramTypeParamLength == 0) {
                if (reportMissing) {
                    for (int i = 0, l = typeParametersLength; i < l; i++) {
                        scope.problemReporter()
                            .javadocMissingParamTag(parameters[i].name, parameters[i].sourceStart,
                                parameters[i].sourceEnd, modifiers);
                    }
                }
                        ...
                        if (paramBindind.isTypeVariable()) {
                            ...
                            for (int j = 0; j < i && !duplicate; j++) {
                                if (TypeBinding.equalsEquals(bindings[j], param.resolvedType)) {
                                    scope.problemReporter()
                                        .javadocDuplicatedParamTag(param.token, param.sourceStart, param.sourceEnd,
                                            modifiers);
                                    duplicate = true;
                                }
                            }
                            ...
                            scope.problemReporter()
                                .javadocUndeclaredParamTagName(param.token, param.sourceStart, param.sourceEnd,
                                    modifiers);
                        }
                    }
                }
                ...
                for (int i = 0; i < typeParametersLength; i++) {
                    ...
                    if (!found && reportMissing) {
                        scope.problemReporter()
                            .javadocMissingParamTag(parameter.name, parameter.sourceStart, parameter.sourceEnd,
                                modifiers);
                    }
                }
                ...
                for (int i = 0; i < paramTypeParamLength; i++) {
                    if (bindings[i] != null) {
                        JavadocSingleTypeReference param = this.paramTypeParameters[i];
                        scope.problemReporter()
                            .javadocUndeclaredParamTagName(param.token, param.sourceStart, param.sourceEnd, modifiers);
                    }
                }
            }
        }
                ...
                for (int i = 0; i < boundExceptionLength; i++) {
                    ...
                    if (exceptionBinding != null && exceptionBinding.isValidBinding()) { // flag only valid class name
                        ...
                        if (j < thrownExceptionLength) {
                            methScope.problemReporter()
                                .javadocMissingThrowsTag(md.thrownExceptions[j], md.binding.modifiers);
                        }
                    }
                }
            }
                ...
                if (!found && reportMissing) {
                    if (exceptionBinding != null && exceptionBinding.isValidBinding()) { // flag only valid class name
                        ...
                        if (k < thrownExceptionLength) {
                            methScope.problemReporter()
                                .javadocMissingThrowsTag(md.thrownExceptions[k], md.binding.modifiers);
                        }
                    }
                }
            }
    ...
    private void verifyTypeReference(Expression reference, Expression typeReference, Scope scope,
        ...
        if (resolvedType.isValidBinding()) {
            ...
            if (!canBeSeen(scope.compilerOptions().reportInvalidJavadocTagsVisibility, modifiers)) {
                scope.problemReporter()
                    .javadocHiddenReference(typeReference.sourceStart, reference.sourceEnd, scope, modifiers);
                return;
            }
            ...
            if (reference != typeReference) {
                if (!canBeSeen(scope.compilerOptions().reportInvalidJavadocTagsVisibility, resolvedType.modifiers)) {
                    scope.problemReporter()
                        .javadocHiddenReference(typeReference.sourceStart, typeReference.sourceEnd, scope,
                ...
                if (scope.kind != Scope.MODULE_SCOPE) {
                            ...
                            if (TypeBinding.notEquals(topLevelType, topLevelScope.referenceContext.binding)) {
                                ...
                                if (!hasValidImport) {
                                    if (scopeModifiers == -1)
                                        scopeModifiers = scope.getDeclarationModifiers();
                                    scope.problemReporter()
                                        .javadocInvalidMemberTypeQualification(typeReference.sourceStart,
                    ...
                    if (typeReference instanceof JavadocQualifiedTypeReference
                        ...
                        if (topLevelScope.getCurrentPackage() == resolvedType.getPackage()
                            ...
                            // https://bugs.eclipse.org/bugs/show_bug.cgi?id=221539: references can be partially
                            // qualified
             ...
             *
             * We are concerned only about the Single type references (i.e. without any package) If they are not in
             * default package,
             * then report an error. References with qualified yet incorrect names would have already been taken care
             * of.
             */
    ...
    private boolean verifyModuleReference(Expression reference, Expression typeReference, Scope scope,
        ModuleBinding moduleType, int modifiers) {
        ...
        if (moduleType != null && moduleType.isValidBinding()) {
            ...
            if (mBinding == null) {
                scope.problemReporter()
                    .javadocInvalidModuleQualification(typeReference.sourceStart, typeReference.sourceEnd,
                        scopeModifiers);
                return bindingFound;
            }
