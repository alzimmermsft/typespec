...
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.MethodBinding.PARAM_NONNULL;
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.MethodBinding.PARAM_NOTOWNING;
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.MethodBinding.PARAM_NULLABLE;
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.MethodBinding.PARAM_NULLITY;
import static com.microsoft.typespec.http.client.generator.core.implementation.shaded.eclipse.jdt.internal.compiler.lookup.MethodBinding.PARAM_OWNING;
...
public abstract class AbstractMethodDeclaration extends ASTNode implements ProblemSeverities, ReferenceContext {
     ...
     * </ul>
     */
    static void analyseArguments(LookupEnvironment environment, FlowInfo flowInfo, FlowContext flowContext,
        Argument[] methodArguments, MethodBinding methodBinding) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (usesOwningAnnotations && local.type.hasTypeBit(TypeIds.BitAutoCloseable | TypeIds.BitCloseable)) {
                    ...
                    int initialNullStatus
                        = (local.tagBits & TagBits.AnnotationOwning) != 0 ? FlowInfo.NULL : FlowInfo.NON_NULL; // defaulting
                                                                                                               // to
                                                                                                               // not-owning
                    local.closeTracker = new FakedTrackingVariable(local, methodArguments[i], flowInfo, flowContext,
                        initialNullStatus, usesOwningAnnotations);
                    local.closeTracker.owningState = FakedTrackingVariable.owningStateFromTagBits(owningTagBits,
                        FakedTrackingVariable.NOT_OWNED_PER_DEFAULT);
                }
            }
        }
    }
        ...
        for (int i = 0, max = parameters.length; i < max; i++) {
            ...
            if (size > 0xFF) {
                this.scope.problemReporter()
                    .noMoreAvailableSpaceForArgument(this.scope.locals[i], this.scope.locals[i].declaration);
            }
        }
    }
    ...
    public void resolveReceiver() {
        ...
        if (this.receiver.modifiers != 0) {
            this.scope.problemReporter()
                .illegalModifiers(this.receiver.declarationSourceStart, this.receiver.declarationSourceEnd);
        }
        ...
        if (this.isConstructor()) {
            ...
            if (declaringClass.isStatic()
                || (declaringClass.tagBits & (TagBits.IsLocalType | TagBits.IsMemberType))
                    == 0) { /* neither member nor local type */
                this.scope.problemReporter().disallowedThisParameter(this.receiver);
                return; // No need to do further validation
            }
        ...
        if (this.scope.compilerOptions().isAnnotationBasedResourceAnalysisEnabled
            && this.receiver.type.resolvedType != null) {
    ...
    void validateNullAnnotations(boolean useTypeAnnotations) {
        ...
        if (!useTypeAnnotations) {
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    if (nullity != 0) {
                        long nullAnnotationTagBit
                            = nullity == PARAM_NONNULL ? TagBits.AnnotationNonNull : TagBits.AnnotationNullable;
                        if (!this.scope.validateNullAnnotation(nullAnnotationTagBit, this.arguments[i].type,
            ...
            int length = this.binding.parameters.length;
            for (int i = 0; i < length; i++) {
                this.scope.validateNullAnnotation(this.binding.parameters[i].tagBits, this.arguments[i].type,
                    this.arguments[i].annotations);
