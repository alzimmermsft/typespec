...
public class BinaryMethod extends BinaryMember implements IMethod {
    ...
    public boolean equals(Object o) {
        if (!(o instanceof BinaryMethod other))
            return false;
    ...
    public ILocalVariable[] getParameters() throws JavaModelException {
        ...
        for (int i = 0; i < length; i++) {
            if (i < startIndex) {
                LocalVariable localVariable = new LocalVariable(this, DeduplicationUtil.toString(argumentNames[i]), 0,
                    -1, 0, -1, this.parameterTypes[i], null, -1, true);
                localVariables[i] = localVariable;
                localVariable.annotations = Annotation.NO_ANNOTATIONS;
            } else {
                LocalVariable localVariable = new LocalVariable(this, new String(argumentNames[i]), 0, -1, 0, -1,
                    this.parameterTypes[i], null, -1, true);
                localVariables[i] = localVariable;
                IAnnotation[] annotations = getAnnotations(localVariable,
                    info.getParameterAnnotations(i - startIndex, getDeclaringType().getElementName().toCharArray()));
    ...
    public int getFlags() throws JavaModelException {
        ...
        if (((IType) this.getParent()).isInterface()
    ...
    public String[] getParameterNames() throws JavaModelException {
        ...
        if (paramCount != 0) {
            ...
            if (javadocContents == null) {
                ...
                // need to extract the part relative to the binary method since javadoc contains the javadoc for the
                // declaring type
                try {
        ...
        // If still no parameter names, produce fake ones, but don't cache them
...
// https://bugs.eclipse.org/bugs/show_bug.cgi?id=299384
    ...
    public boolean isSimilar(IMethod method) {
        return areSimilarMethods(getElementName(), getParameterTypes(), method.getElementName(),
    ...
    public JavaElement resolved(Binding binding) {
        SourceRefElement resolvedHandle = new ResolvedBinaryMethod(this.getParent(), this.name, this.parameterTypes,
