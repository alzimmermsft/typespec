...
public class FluentModelPropertyRegion {
    ...
    public static class FluentModelPropertyRegionMethod extends FluentModelPropertyMethod {
        ...
        public FluentModelPropertyRegionMethod(FluentResourceModel model, FluentMethodType type,
            FluentInterfaceStage stage, ClientModel clientModel, ModelProperty modelProperty,
            LocalVariable localVariable, String baseName) {
            super(model, type, stage, clientModel, modelProperty, localVariable,
                CodeNamer.getModelNamer().modelPropertySetterName(baseName), "Specifies the region for the resource.");
            ...
            this.implementationMethodTemplate
                = MethodTemplate.builder().methodSignature(this.getImplementationMethodSignature()).method(block -> {
                    if (fluentResourceModel.getInnerModel() == clientModel) {
                        block.line("this.%1$s().%2$s(%3$s.toString());", ModelNaming.METHOD_INNER_MODEL,
                            modelProperty.getSetterName(), modelProperty.getName());
                    } else {
                        block.line("this.%1$s.%2$s(%3$s.toString());", localVariable.getName(),
                            modelProperty.getSetterName(), modelProperty.getName());
                    }
        ...
        protected String getBaseMethodSignature() {
            return String.format("%1$s(%2$s %3$s)", this.name, FluentType.REGION, modelProperty.getName());
        }
