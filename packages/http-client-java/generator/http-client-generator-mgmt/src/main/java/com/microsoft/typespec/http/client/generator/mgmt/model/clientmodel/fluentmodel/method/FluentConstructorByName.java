...
public class FluentConstructorByName extends FluentMethod {
    ...
    public static FluentConstructorByName constructorMethodWithConstantResourceName(FluentResourceModel model,
        FluentMethodType type, ClassType managerType, ResourceLocalVariables resourceLocalVariables) {
        return new FluentConstructorByName(model, type, null, null, managerType, resourceLocalVariables);
    }

    public FluentConstructorByName(FluentResourceModel model, FluentMethodType type, IType resourceNameType,
        String propertyNameForResourceName, ClassType managerType, ResourceLocalVariables resourceLocalVariables) {
        super(model, type);
    ...
    public String getImplementationMethodSignature() {
        if (constantResourceName) {
            return String.format("%1$s(%2$s %3$s)", implementationReturnValue.getType().toString(),
                managerType.getFullName(), ModelNaming.MODEL_PROPERTY_MANAGER);
        } else {
            return String.format("%1$s(%2$s name, %3$s %4$s)", implementationReturnValue.getType().toString(),
                resourceNameType.toString(), managerType.getFullName(), ModelNaming.MODEL_PROPERTY_MANAGER);
        }
    }
