...
public final class FluentProxyMethodMapper extends ProxyMethodMapper {
    ...
    protected void buildUnexpectedResponseExceptionFields(ProxyMethod.Builder builder, Operation operation,
         ...
         * HttpMethod.valueOf(operation.getRequests().get(0).getProtocol().getHttp().getMethod().toUpperCase());
         * final boolean isResourceModify = httpMethod == HttpMethod.PUT || httpMethod == HttpMethod.POST || httpMethod
         * == HttpMethod.PATCH || httpMethod == HttpMethod.DELETE;
         ...
         * unexpectedResponseExceptionTypes.put(ClassType.ClientAuthenticationException,
         * Collections.singletonList(HttpResponseStatus.UNAUTHORIZED));
         * }
         * if (!expectedStatusCodes.contains(HttpResponseStatus.NOT_FOUND)) {
         * unexpectedResponseExceptionTypes.put(ClassType.ResourceNotFoundException,
         * Collections.singletonList(HttpResponseStatus.NOT_FOUND));
         * }
         * if (isResourceModify && !expectedStatusCodes.contains(HttpResponseStatus.CONFLICT)) {
         * unexpectedResponseExceptionTypes.put(ClassType.ResourceModifiedException,
         * Collections.singletonList(HttpResponseStatus.CONFLICT));
         * }
         * if (!expectedStatusCodes.contains(HttpResponseStatus.TOO_MANY_REQUESTS)) {
         * unexpectedResponseExceptionTypes.put(ClassType.TooManyRedirectsException,
         * Collections.singletonList(HttpResponseStatus.TOO_MANY_REQUESTS));
         * }
         * if (hasETagHeader && !expectedStatusCodes.contains(HttpResponseStatus.PRECONDITION_FAILED)) {
         * unexpectedResponseExceptionTypes.put(ClassType.ResourceExistsException,
         * Collections.singletonList(HttpResponseStatus.PRECONDITION_FAILED));
         * }
