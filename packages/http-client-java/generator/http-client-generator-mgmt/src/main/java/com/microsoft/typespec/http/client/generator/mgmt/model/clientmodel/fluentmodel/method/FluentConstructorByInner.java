...
public class FluentConstructorByInner extends FluentMethod {
    ...
    public FluentConstructorByInner(FluentResourceModel model, FluentMethodType type,
        List<MethodParameter> pathParameters, ResourceLocalVariables resourceLocalVariables, ClassType managerType,
            ...
            .method(block -> {
                ...
                pathParameters.forEach(p -> {
                    ...
                    if (urlPathSegments.hasScope()) {
                        valueFromIdText
                            = String.format("%1$s.getValueFromIdByParameterName(%2$s.id(), \"%3$s\", \"%4$s\")",
                                ModelNaming.CLASS_RESOURCE_MANAGER_UTILS, ModelNaming.MODEL_PROPERTY_INNER,
                                urlPathSegments.getPath(), p.getSerializedName());
                    } else {
                        valueFromIdText = String.format("%1$s.getValueFromIdByName(%2$s.id(), \"%3$s\")",
                            ModelNaming.CLASS_RESOURCE_MANAGER_UTILS, ModelNaming.MODEL_PROPERTY_INNER,
                            urlSegmentNameByParameterName.get(p.getSerializedName()));
                    }
    ...
    public String getImplementationMethodSignature() {
        return String.format("%1$s(%2$s %3$s, %4$s %5$s)", implementationReturnValue.getType().toString(),
            fluentResourceModel.getInnerModel().getName(), ModelNaming.MODEL_PROPERTY_INNER, managerType.getFullName(),
