...
public class FluentResourceModelImplementationTemplate implements IJavaTemplate<FluentResourceModel, JavaFile> {
    ...
    private static final FluentResourceModelImplementationTemplate INSTANCE
        = new FluentResourceModelImplementationTemplate();
            ...
            classBlock -> {
                ...
                if (model.getCategory() == ModelCategory.IMMUTABLE || model.getResourceUpdate() == null) {
                    ...
                    classBlock.packagePrivateConstructor(
                        String.format("%1$s(%2$s %3$s, %4$s %5$s)", model.getImplementationType().getName(),
